...
mkdir -p build
source "$(dirname "$0")/download-artifact.sh" "build" --step "$BUILDKITE_GROUP_KEY-build-deps"
source "$(dirname "$0")/download-artifact.sh" "build" --step "$BUILDKITE_GROUP_KEY-build-zig" --split
source "$(dirname "$0")/download-artifact.sh" "build" --step "$BUILDKITE_GROUP_KEY-build-cpp" --split
cd build
