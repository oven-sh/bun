...
- Carry WebKit's build changes into `scripts/build/deps/webkit.ts` and the file lists in `scripts/build/deps/webkit-{bmalloc,wtf,jsc,llint,inspector}-sources.ts` (bun compiles JSC itself; nothing reads WebKit's cmake or its tree at configure time, and the lists are maintained by hand). In the WebKit clone, diff the old base against the new one for exactly these files:
  ...
  - **Guarded** (the build stops until acknowledged): generator invocations. `webkit-check-cmake.ts` parses `Source/JavaScriptCore/CMakeLists.txt` and `Source/WTF/wtf/PlatformJSCOnly.cmake` after the fetch and compares every `add_custom_command` (plus the macros wrapping them and the variables feeding them: `OFFLINE_ASM_ARGS`, `LLINT_ASM`, `JavaScriptCore_BUILTINS_SOURCES`, …) against `scripts/build/deps/webkit-cmake.snapshot`. If the bump changed any, the build fails with the diff: carry it into the matching `gen()` call in `webkit.ts` (a new `add_custom_command` → a new `gen()` edge), then refresh the snapshot with `bun scripts/build/deps/webkit-check-cmake.ts vendor/WebKit --update` and commit it.
  - **Silent** (builds fine, behaves differently — read the diff): `WEBKIT_OPTION_DEFINE` / `WEBKIT_OPTION_DEFAULT_PORT_VALUE` / `SET_AND_EXPOSE_TO_BUILD` changes → the `rows` table (cmakeconfig.h); compile flags (`WEBKIT_PREPEND_GLOBAL_COMPILER_FLAGS`, `WEBKIT_ADD_TARGET_CXX_FLAGS`, `set_source_files_properties(... COMPILE_OPTIONS ...)`) → `webkitFlags()` or a per-file `cflags` on that source; DerivedSources headers added to `JavaScriptCore_PRIVATE_FRAMEWORK_HEADERS` → the generated-header list in `frameworkHeaders()`.
