...
- Carry WebKit's build changes into `scripts/build/deps/webkit.ts` and the file lists in `scripts/build/deps/webkit-{bmalloc,wtf,jsc}-sources.ts` (bun compiles JSC itself; nothing reads WebKit's cmake or its tree at configure time, and the lists are maintained by hand). In the WebKit clone, diff the old base against the new one for exactly these files:
