...
- Subtracts the time a file sat waiting for its docker-compose service to come up (`test/docker/index.ts`'s `awaitService`, which `describeWithContainer` uses, prints `Container ready via docker-compose: ... (waited Nms)`; in the parallel bucket the line is attributed via bun's `<path>:` output block). That wait is paid by whichever file on the shard touches the service first, so it is not the file's cost and does not belong in the slow-test list; the `max_container_wait` column and a stderr list of the longest waits report it separately. A file in that list tagged `not in test/docker/prestart-map.mjs` is fixed by adding it to the map; an untagged one ran before its service was up anyway (the runner puts the files a PR modifies at the front of the shard, so on a PR build that is usually the PR's own docker-backed files; otherwise the shard had too little other work).
