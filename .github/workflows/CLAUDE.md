...
- Cargo does not rerun the driver for a crate it considers fresh, and neither `MORDANT_BASELINE_WRITE` nor the baseline file is part of what makes a crate stale. So a second run over a warm `target/mordant/check` replays the cached warnings but does not write `over-baseline.txt` again, and a baseline write over a warm directory records nothing. CI always starts cold. `bun run rust:mordant:baseline` removes `target/mordant/check` first.
