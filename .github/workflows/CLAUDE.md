...
- The baseline holds a count for each lint and file, not which findings. A file within its count prints nothing. A file that goes over prints every finding of that lint as a warning, followed by one line with the number found and the number the baseline allows: the new finding is among them, and it need not be the last one in the file. Mordant writes each crate that went over, with how many findings over, to `target/mordant/over-baseline.txt` (relative to the workspace root). The job deletes that file, runs `cargo mordant`, and fails if the file is non-empty; absent or empty means clean. Fixing baselined findings needs no baseline update.
