...
- `mordant` is the only job that runs on `push` to `main`, and only when `Cargo.toml` or this workflow changes: those two files hold everything its cache keys are made of. That run exists to save the two caches where pull requests can restore them. A cache saved by a `pull_request` run is scoped to that pull request's merge ref, so no other pull request can read it. A cache saved on the default branch is readable from every pull request. The push run skips the over-baseline check, because `actions/cache` saves only when the job succeeds. For the same reason `cancel-in-progress` is off for `refs/heads/main`: a newer push waits for the running one. After a `DYLINT_VERSION` or mordant bump merges, pull requests miss (and save their own copy) until that push run finishes. If that push run fails, re-run it: nothing else saves the caches on `main` until the next push that matches. A `workflow_dispatch` run on `main` saves them only when the over-baseline check passes, because it does not skip that check.
- Two builds here resolve their dependencies with no `Cargo.lock`, so cargo picks the newest compatible release of every crate. On a miss of the second cache, cargo-dylint builds its driver in a temp dir. On every run, `lolhtml` runs `cargo test` in `vendor/lolhtml`, and the fork does not commit a lockfile. A compile error in a third-party crate in either step (`jiff 0.2.36` in the `mordant` step, yanked within the hour, was one) is a bad upstream release: re-run the job once a fixed release is out.
