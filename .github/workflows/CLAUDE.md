...
- `mordant.toml` at the workspace root is the configuration. Mordant reads only its `[mordant]` table, which points `baseline` at `mordant-baseline.toml` (per-(lint, file) counts of the findings that predate the job) and lists the lints this repo has switched off under `disabled`, each with its reason.
- The baseline holds a count for each lint and file, not which findings. A file within its count prints nothing. A file that goes over prints every finding of that lint as a warning, followed by one line with the number found and the number the baseline allows: the new finding is among them, and it need not be the last one in the file. When any crate goes over, `cargo mordant` prints one closing `error: mordant: N finding(s) over the baseline in ...` line and exits with status 101, which fails the job, and lists each such crate with its count in `target/mordant/over-baseline.txt` (relative to the workspace root). Fixing baselined findings needs no baseline update.
...
- The baseline file and `MORDANT_BASELINE_WRITE` are inputs of every member's compilation: mordant's driver names them in the dep-info. So a baseline write, or a change to `mordant-baseline.toml`, re-lints every member on the next run, and a warm `target/mordant/check` is safe to keep. On a warm run where nothing changed, cargo replays a crate's warnings and does not run the driver, but each compilation saved its count under `target/mordant/over_baseline_counts/`, so `cargo mordant` still judges every crate, rewrites `over-baseline.txt` and exits 101 as on a cold run (`unused_pub` is judged again on every run). A `target/mordant` left by a mordant build older than the pin has no saved counts, and the run then stops with an error that says to remove that directory.
