...
| `mordant` | `mordant`             | `cargo mordant --workspace`                  | advisory (`continue-on-error`) |
...
- Cargo does not rerun the driver for a crate it considers fresh, and neither `MORDANT_BASELINE_WRITE` nor the baseline file is part of what makes a crate stale. So a second run over a warm `target/mordant/check` replays the cached warnings but does not write `over-baseline.txt` again, and a baseline write over a warm directory records nothing. CI always starts cold. `bun run rust:mordant:baseline` removes `target/mordant/check` first. `bun run rust:mordant` keeps it, so that a source edit re-lints only the crates it touches. When `mordant-baseline.toml` changed and a crate's source did not (a pull, a hand edit), the warnings replayed for that crate were weighed against the old baseline: remove `target/mordant/check` before that run.
- Locally, `bun run rust:mordant` is the same invocation and `bun run rust:mordant:baseline` regenerates the baseline (`MORDANT_BASELINE_WRITE=1`). Both need mordant installed once, with the two values from the job's `env:`: `rustup toolchain install <MORDANT_TOOLCHAIN> --profile minimal --component rustc-dev --component llvm-tools-preview`, then `cargo +<MORDANT_TOOLCHAIN> install --locked --git https://github.com/scarletindustries/mordant --rev <MORDANT_REV>`. `cargo mordant --version` prints the commit of the installed build. Both also expect `build/debug/codegen`, `vendor/lolhtml` and `vendor/rust-argon2` to exist, which any normal `bun bd` leaves behind.
