...
**Stay unchanged — bind uws_* C++ shim exports (libuwsockets.cpp / libuwsockets_h3.cpp / bun-uws):**
...
`ListenSocket.rs` (8), `Loop.rs` (9 us_*), `SocketContext.rs` (4: ssl_ctx fns),
...
(uws_dispatch.rs), `BUN_SOCKET_KIND_*` statics (SocketKind.rs),
`us_socket_buffered_js_write` (uws_jsc.rs, called from JSHS C++).
