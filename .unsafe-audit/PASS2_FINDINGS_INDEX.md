...
| 5 | U2.x8 | `http/AsyncHTTP.rs:117`, `http/lib.rs:176`, `runtime/node/node_fs.rs:2397`, `bun_alloc/lib.rs:3267`, `bun_core/string/mod.rs:1765`, `jsc/lib.rs:2013`, `jsc/ZigString.rs:70,102` | Deallocation/free through a pointer derived from shared provenance (`from_ref(...).cast_mut()` / slice `.as_ptr().cast_mut()`). This is SB-UB on the dealloc path; the mechanical fix is to retain the original owning `Box`/`NonNull`/`*mut`. | PASS2-ptr-cast |
