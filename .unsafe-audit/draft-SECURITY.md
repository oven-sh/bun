...
## INTERNAL: pre-publication soundness items

> The section below is for **Bun maintainers only**. It must be removed or moved to a private channel before this file is published to the public repo. It is included here so that the team has a single document tracking what needs to be fixed before the public SECURITY.md ships.

The following Tier-1 memory-safety findings from the 2026-05-15 audit baseline are unfixed at the time of writing. **Publishing the public SECURITY.md before these are addressed would be premature** — once SECURITY.md commits to "the runtime maintains memory safety against malformed lockfiles and HTTP traffic," that claim must be true.

### P0 supply-chain primitives (4 findings, must fix before publication)

1. **PUB-INSTALL-1** — `install/lockfile/Package.rs:3320-3478` `Meta::has_install_script`: niche-violating `transmute<u8, HasInstallScript>` on every `bun install` over an attacker-planted `bun.lockb`. Fix: `match`/`TryFrom`. ~20 LOC.
2. **PUB-INSTALL-2** — `install/lib.rs:1128` `Meta::origin`: same niche-violating shape on the `Origin` enum. Fix: `match`/`TryFrom`. ~20 LOC.
3. **PUB-INSTALL-3** — `install/yarn.rs:918-925`: `&mut [Dependency]` over uninitialised Vec capacity; `Dependency` embeds `#[repr(u8)] DependencyVersionTag`. Fix: `Vec::push` loop or `MaybeUninit::write_slice`. ~30 LOC.
4. **PUB-INSTALL-4** — `install/lockfile/Tree.rs:1020`: attacker-controlled dependency ID feeds `deps.get_unchecked(dep_id as usize)`. Fix: `.get(...).ok_or(...)`. ~5 LOC.

All four are confirmed by Codex's adversarial review (see `CODEX_PASS3_FINAL_REVIEW.md` § "Findings That Should Stay Strong"). The mechanism is identical: the Zig source used `@enumFromInt` which panics on invalid discriminants in safe build modes; the Rust port reproduced the syntax with `transmute<u8, Enum>` which is unconditionally UB. This is the canonical Zig→Rust translation gap.

**Attack scenario:** plant a malicious `bun.lockb` or `yarn.lock` in a public package. Every developer who clones and runs `bun install` triggers UB. Worst case: arbitrary OOB read → information disclosure → potentially RCE through subsequent memory corruption.

These four findings should be addressed in PR set 1 of the audit's recommended landing order. They are mechanical fixes (~75 LOC total). Once landed, the public SECURITY.md can ship with the "memory safety against malformed lockfiles" commitment intact.

### P0 network-input primitives (2 findings, should fix before publication)

5. **H9** — `vendor-shim picohttp/lib.rs:383`: writes NUL through `cast_mut()` of a `&[u8]`-derived pointer on every inbound HTTP/1.1 request and fetch response. SB-UB. Fix: require mutable input or owning mutable buffer. ~5 LOC.
6. **H5** — `http/lib.rs:631,2218`: `request_content_len_buf: [u8; 11]` overflows on `body_len >= 10^11`, silently emits `Content-Length: 0` while body bytes are still written. Request-smuggling primitive. Fix: `[u8; 21]` and length-checking adapter. ~5 LOC.

The H9 finding is a memory-safety bug (SB-UB on every HTTP request); the H5 finding is a request-smuggling primitive that is mechanically a buffer-sizing bug. Both are public-facing.

### Pre-publication checklist

- [ ] PUB-INSTALL-1 fixed and merged.
- [ ] PUB-INSTALL-2 fixed and merged.
- [ ] PUB-INSTALL-3 fixed and merged.
- [ ] PUB-INSTALL-4 fixed and merged.
- [ ] H9 fixed and merged.
- [ ] H5 fixed and merged.
- [ ] Reproducer tests added under `test/regression/issue/<N>.test.ts` for each.
- [ ] Codex re-review confirms the fixes close the original obligations.
- [ ] Public soundness-debt dashboard ships at `audit/soundness-debt-dashboard.md`.
- [ ] This "INTERNAL" section deleted from the file and the file moved to repo root as `SECURITY.md`.
- [ ] README links the SECURITY.md.
- [ ] `Cargo.toml` `[package.metadata.security]` (if used) references SECURITY.md.

### Tier-2 / Tier-3 backlog (not blocking publication, but tracked)

- ~32 Tier-2 unsafe-contract defects (cross-thread task traits, output writer aliasing, scratch-buffer lifetimes, JsCell/RacyCell Send/Sync bounds, Watcher ownership race, Blob/VirtualMachine Send/Sync, etc.).
- ~58 Tier-3 watchlist items.
- 11 strict-provenance migration items.

These are valuable to address but are not blocking the public SECURITY.md commitment. They are tracked in the public soundness-debt dashboard once it ships.

---

> End of INTERNAL section. The public-facing portion of this document ends above.
