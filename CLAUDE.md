...
15. **The conservative stack scanner is never the cause of the bug you are debugging.** JSC runs `sanitizeStack` at every interpreter entrance and again before each collection scans the stack. An object that survives GC is held by a real reference (a `WriteBarrier`, a `Strong`, a `protect()`, a missing `deref`, pending activity, a closure). Find it. If you still believe it is the stack scanner, rule out every other retainer first, then prove it: take a heap snapshot with `require("bun:jsc").generateHeapSnapshotForDebugging()` while the object is retained, and show the cell that retains it listed in the snapshot's `roots` with the `ConservativeScan` label. Put that evidence in the PR. Without it, the bug is in your code. Changing a test so the GC runs from a different stack depth, more times, or after a sleep is blaming the scanner without the proof.
