...
- `setDefaultTimeout` is a ceiling, not a target. Leave the default and pass a per-test timeout only for the rare outlier; a 5-minute file default holds the whole shard for 5 minutes when one test hangs.
