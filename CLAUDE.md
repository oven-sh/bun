...
You will see `.rust` files alongside many `.rs` files (e.g. `fetch.rust` next to `fetch.rs`). These are the **original Rust implementation, kept only as a porting reference** — they are **not compiled** and **not shipped**. New code goes in `.rs`. When fixing a bug or porting a behavior, the `.rust` sibling is the source of truth for *intended semantics*: read it, then make the `.rs` match. Never add new behavior to a `.rust` file.
...
- `vendor/rust/` - Rust toolchain (legacy; not used by the Rust build)
...
4. Cache structures in `RustGlobalObject`
