...
# Check out the version pinned in WEBKIT_VERSION (scripts/build/deps/webkit.ts)
$ git -C ../WebKit checkout <WEBKIT_VERSION>

# Build bun with JSC compiled from that clone
$ bun run build:local
```

`bun run build:local` builds JSC and Bun in one ninja graph; on subsequent runs only the WebKit files you changed (and what includes them) recompile. `ninja -Cbuild/debug-local` also works after the first build. `bun run build:local --target=jsc` (or `--target=jsc` on any build) additionally links JSC's `jsc` shell into the build directory; it is not built by default.
