...
```bash#Fedora
$ sudo dnf install clang22 llvm22 lld22 cmake git golang libtool ninja-build pkg-config ruby libatomic-static libstdc++-static sed unzip which libicu-devel 'perl(Math::BigInt)'
```
...
Bun requires LLVM 22.1.8 (`clang` is part of LLVM). This version is enforced by the build system — mismatching versions will cause memory allocation failures at runtime. In most cases, you can install LLVM through your system package manager:
...
$ wget https://apt.llvm.org/llvm.sh -O - | sudo bash -s -- 22 all
```
...
```bash#openSUSE Tumbleweed
$ sudo zypper install clang22 lld22 llvm22
```

{% /codetabs %}

If none of the above solutions apply, you will have to install it [manually](https://github.com/llvm/llvm-project/releases/tag/llvmorg-22.1.8).

Make sure Clang/LLVM 22 is in your path:
...
# use path+="$(brew --prefix llvm@22)/bin" if you are using zsh
$ export PATH="$(brew --prefix llvm@22)/bin:$PATH"
```

```bash#Arch
# use fish_add_path if you're using fish
$ export PATH="$PATH:/usr/lib/llvm22/bin"
```
