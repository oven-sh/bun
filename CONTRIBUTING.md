...
To work on JSC itself you want a full clone with history instead. Clone it, then build with the `*-local` profiles, which compile whatever is checked out (set `$BUN_WEBKIT_PATH` if the clone lives somewhere other than `vendor/WebKit`):
