...
- `./src/codegen/generate-classes.ts` -- Generates `build/debug/codegen/ZigGeneratedClasses*` and `generated_classes.rs`, which provide Rust & C++ bindings for JavaScriptCore classes implemented in Rust. In `**/*.classes.ts` files, we define the interfaces for various classes, methods, prototypes, getters/setters etc which the code generator reads to generate boilerplate code implementing the JavaScript objects in C++ and wiring them up to Rust.
- `./src/codegen/cppbind.ts` -- Generates automatic Rust bindings (`cpp.rs`) for C++ functions marked with `[[ZIG_EXPORT]]` attributes (the attribute name is kept for historical reasons).
