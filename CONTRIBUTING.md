...
Bun requires LLVM 20 (`clang` is part of LLVM). This version requirement is to match WebKit (precompiled), as mismatching versions will cause memory allocation failures at runtime. In most cases, you can install LLVM through your system package manager:
...
```bash#openSUSE Tumbleweed
$ sudo zypper install clang20 lld20 llvm20
```

{% /codetabs %}

If none of the above solutions apply, you will have to install it [manually](https://github.com/llvm/llvm-project/releases/tag/llvmorg-20.1.8).

Make sure Clang/LLVM 20 is in your path:
...
# use path+="$(brew --prefix llvm@20)/bin" if you are using zsh
$ export PATH="$(brew --prefix llvm@20)/bin:$PATH"
```

```bash#Arch
# use fish_add_path if you're using fish
$ export PATH="$PATH:/usr/lib/llvm20/bin"
```
