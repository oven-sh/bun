...
The final defensible public tally: **6 P0 + 40 T1/T1-equivalent entries + ~32 T2 + ~58 T3**. The 40-entry dashboard keeps strict memory-safety findings, non-UB security defects, and crash-reliability items labelled separately; do not quote it as "40 memory-safety bugs."
...
Bun's maintainers have ALREADY removed **2,989** unsafe blocks in tagged commits (per `PASS4-soundness-archeology.md`). Every Tier-1 finding the audit reports maps to a maintainer commit class. This audit isn't novel-bug-finding from outsiders — it's the NEXT BATCH of bugs the project's own remediation campaign hasn't reached yet.
