...
| image pages COW-dirtied | 35–39 | 1,710 malloc-payload pages dirtied for **0.54 MB of changed bytes**; 445k immortal cells of which 2,666 changed (0.32 MB) dirtied 7 MB of cell pages. Owners of the malloc-side writes (profiled image + `dirtymap`): bytecode-cache decoder shared tables, TDZ links (refcounts), `ExecutableBase` link states, structure transition tables, root-cert strings, watchpoint tables — bookkeeping that post-restore code re-creation writes into. Fix theme: at freeze, detach or finish the lazy bookkeeping so post-restore work allocates fresh |
