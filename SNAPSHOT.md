...
Claude Code as a single file: `cli` 359 MB → 601 MB with the image embedded (gzip transport ~184 MB); bare run **34.4 MB at the prompt, 0.16 s CPU**, turns work. (Two bugs on the way, both "our own state is in `__DATA` and the overlay rewrites it": the embedded-source base offset, like `environ` before.) Possible follow-up: embed the zstd form and inflate to the cache when on-disk size matters more than first-run cost; `mach_vm_remap` from one mapping instead of N `mmap`s (Jarred's suggestion) as a restore-time optimization.
