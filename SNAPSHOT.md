...
## Reboots (Aug 8): extern-library fixups on Darwin too

The first reboot of this machine since the work began invalidated every image: the dyld shared cache slides per boot, and the Darwin restore only had the `libsBase` guard ("built against system libraries at X, now at Y; booting normally"). Now Darwin records the same extern-library fixup table Linux does — one entry per *segment* of every loaded dylib (a whole-cache range would be 5.6 GiB wide, too coarse a test for "is this word a pointer"), compacted to the segments actually referenced. Restore matches segments by name+size; shared-cache segments the builder had dlopen'd but this process has not (CC: libsqlite3 → CoreFoundation/Foundation/…) are rebased by the cache delta without loading them (loading them before the overlay made ObjC state inconsistent → `dispatch_once` recursion trap via CFLog); non-cache libraries are dlopen'd by recorded path. Identity: the shared cache UUID (`hdr.spare[4]`) — an OS update still falls back to a plain boot; the inflated-image cache key uses it too, so cached images survive reboots. Cost, measured on the CC late-cut image: 37,688 pointers in 612 pages → **9.6 MB dirtied after a reboot** (nothing while the cache has not moved: zero deltas are skipped). Table: 165–200 segments × 280 B. The mechanism with non-zero deltas is exercised on every Linux restore (ASLR per exec); the Darwin lib enumeration gets its first real non-zero-delta run at the next reboot — check `BUN_IMAGE_VERBOSE=1` prints `rebased N extern-library pointers` and the REPL comes up.
...
## Where the restored process's memory actually is (Aug 8, late; empty cwd, after a forced full GC + mi_collect)

Measured with the tooling build (`-DBUN_HEAPIMAGE_TOOLING=1`, `BUN_MEMDEBUG` + `gc`/`newcells`/`dirtymap`) and vmmap:

| dirty memory | MB | what it is |
|---|---|---|
| fresh mimalloc arenas (above the image) | 60–100 | **only ~10 MB of it is live** (`mi_heap_visit_blocks` after the collect); the arena map is red: pages that were filled during the post-restore burst (code re-creation, hydrate, managers), then mostly freed, each pinned by a few surviving blocks. 16 KB OS pages make hole punching nearly useless here. General Bun problem (plain boots pay it too); the lever is segregating burst-transient allocations from what survives, e.g. per-compile heaps for the JIT threads and a scratch heap for hydrate |
| image pages COW-dirtied | 35–39 | 1,710 malloc-payload pages dirtied for **0.54 MB of changed bytes**; 445k immortal cells of which 2,666 changed (0.32 MB) dirtied 7 MB of cell pages. Owners of the malloc-side writes (profiled image + `dirtymap`): bytecode-cache decoder shared tables, TDZ links (refcounts), `ExecutableBase` link states, structure transition tables, root-cert strings (a static-string change for these was tried and reverted: review showed stock WTF still refcounts static impls, and the effect was below run-to-run noise), watchpoint tables — bookkeeping that post-restore code re-creation writes into. Fix theme: at freeze, detach or finish the lazy bookkeeping so post-restore work allocates fresh |
| new JS heap blocks | 25–30 | 10.7 MB of live cells in ~1,900 blocks (35% occupancy): every directory opens fresh blocks because image blocks are sealed. Image blocks themselves are ~90% full, so defragmenting before capture is worth ≤10 MB; letting allocation reuse *already dirty* image blocks would be the image-specific win |
| new JIT code | 8 | |
| everything else | ~10 | |
...
## Shipping state (end of Aug 8)

| Piece | Where | State |
|---|---|---|
| mimalloc | oven-sh/mimalloc#13 (draft) | 240-line delta, generic embedder hooks; this branch pins to its head and no longer vendors files |
| JSC/WTF | oven-sh/WebKit#397 (draft) | full CI matrix green (39/39); this branch pins `WEBKIT_VERSION` to its preview artifact, so it builds without a local WebKit and is CI-buildable; commits still need reorganizing into a reviewable series |
| Bun runtime | this branch (`claude/lowmem-cc`), merged with main on Aug 8 | `HeapImage.{h,cpp}` + `HeapImageTooling.cpp` (opt-in), `run_command.rs` snapshot/restore, `Bun.unsafe.snapshot` options, `--compile-image`, docs, 12 tests; `rust:check-all` green. Merging with main surfaced and fixed a real branch bug: deciding the argv passthrough offset in the parser broke `bun -e code a b` (existing tests caught it once run) |
| Independent fix that fell out | oven-sh/bun#37223 | `node:https` pool names digest large TLS parts |
| Claude Code | `claude/lowmem-snapshot` | one mode; late cut in a hermetic HOME, `hydrateAfterRestore`, `envGate`; build-script diagnostics passthrough. Needs a rebase onto its main and tests for the restore path |
| Open decision | macOS malloc override | official macOS builds keep mimalloc's override off; images need it. `test/napi` is 100% green on main with it on; broader local runs are only meaningful per directory (some directories hang/interfere when run whole, on the baseline too) |

Next for the Bun side: open the branch as a draft PR to get the CI matrix on it (it is buildable now), then split: (1) tooling-free
runtime + tests, (2) `--compile-image`, (3) tooling file. Perf claims should be re-measured with this branch built against
its *prebuilt* WebKit artifact vs main against main's — same compiler and allocator on both sides, which no earlier
comparison had.
