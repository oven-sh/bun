...
## Linux: the Aug-6 regression, explained

Root cause (found via `strace -k`): on the extern-library-fixup path (Linux default) the data-segment overlay is *deferred* to a libc-free critical section, and that section had been placed **after** the fresh post-restore heap was created — so the overlay wiped the allocator state (heap list / arena table / page map / hint pointer) the fresh heap had just been registered in; the next arena reserve consulted the builder's hint pointer (0 → `MI_HINT_BASE`, occupied by the image → kernel-placed), and later page-map lookups for those blocks faulted or returned NULL ("memory allocation of N bytes failed", `_mi_unchecked_ptr_page`). macOS was unaffected because its overlay happens in-place inside the region loop. Fix: the critical section (overlay + fixups + `environ` + hint floor) now runs immediately after the region loop, before any allocator setup. Along the way: builders no longer float their heap; a *restoring* process floors its own early heap at +64 GiB (`MIMALLOC_HINT_FLOOR`, set by the re-exec parent / tests), the hint floor is re-applied after the overlay, and the dedicated fresh arena is macOS-default / Linux opt-in (`BUN_IMAGE_FRESHARENA`).
