...
### How a compiled executable knows it is image-capable (no environment)

`bun build --compile` outputs carry `BUN_COMPILED` (the `__BUN`/`.bun` section header) with a non-zero size; plain `bun` has 0. That is readable before `main` by anything statically linked, so on macOS: our mimalloc uses deterministic address hints from the very first allocation when `bun_is_compiled_executable()`, VM init pins the JIT pool address, and — only when an image is actually being built or was restored (`Bun__heapImageActive()`) — the LLInt+DFG tier defaults apply. Result: the macOS self re-exec is purely for the ASLR spawn flag and injects **no** environment (the restored child's env contains only `BUN_IMAGE_REEXECED`). At restore, the dotenv loader drops the builder's process-derived entries and reloads this process's `environ`, and `process.env`/`Bun.env` are rebuilt from it (a run-time `MY_VAR=x ./app` is visible; builder-only vars are gone). Linux still injects `MIMALLOC_*`/`BUN_IMAGE_JIT_ADDR`/`BUN_JSC_*` via one re-exec: enabling the pre-main allocator path there made post-restore allocation fail ("memory allocation of N bytes failed" right after thread adoption) — the restorer's first arena was created kernel-placed and the overlaid allocator state didn't cover it; needs a gdb session on a quiet box (`bun_heap_image_mode` is `#if __APPLE__` in `vendor/mimalloc/src/os.c`).
