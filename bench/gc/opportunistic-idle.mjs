...
const deadlineEnv = process.env.BUN_GC_OPPORTUNISTIC_DEADLINE_MS;
const label = deadlineEnv && Number(deadlineEnv) > 0 ? `opportunistic(${deadlineEnv}ms)` : "baseline";
...
for (let i = 0; i < ITERATIONS; i++) {
  ...
  for (let j = 0; j < BURST_STRINGS; j++) {
    arr.push({ id: j, text: "x".repeat(512) + j, nested: { a: j * 2, b: [j, j + 1, j + 2] } });
  }
  sink += arr.length;
  ...
  peakSamples.push(peakRss);
  idleSamples.push(idleRss);
  console.log(
    `iter=${i} peak_rss=${(peakRss / 1024 / 1024).toFixed(1)}MB peak_heap=${(peakHeap / 1024 / 1024).toFixed(1)}MB ` +
      `idle_rss=${(idleRss / 1024 / 1024).toFixed(1)}MB idle_heap=${heapMB()}MB`,
  );
}

const avg = arr => arr.reduce((a, b) => a + b, 0) / arr.length / 1024 / 1024;
console.log(`--- ${label} ---`);
console.log(`peak: avg=${avg(peakSamples).toFixed(1)}MB max=${(Math.max(...peakSamples) / 1024 / 1024).toFixed(1)}MB`);
console.log(
  `idle: avg=${avg(idleSamples).toFixed(1)}MB max=${(Math.max(...idleSamples) / 1024 / 1024).toFixed(1)}MB sink=${sink}`,
);
