...
const recommended_zig_version = "0.12.0-dev.3152+90c1a2c41";
...
fn addInternalPackages(b: *Build, step: *Compile, _: std.mem.Allocator, _: []const u8, target: anytype) !void {
    const io: *Module = brk: {
    ...
    const async_: *Module = brk: {
        if (target.result.os.tag.isDarwin() or target.result.os.tag == .linux or target.result.os.tag == .freebsd) {
            break :brk b.createModule(.{
...
pub fn build_(b: *Build) !void {
...

    {
        ...
        obj.root_module.omit_frame_pointer = optimize != .Debug;
...

    {
        ...
        var headers_obj: *Compile = b.addExecutable(.{
            .name = "vlq-bench",
...

    {
        ...
        var headers_obj: *Compile = b.addObject(.{
            .name = "tgz",
...

    {
        ...
        var headers_obj: *Compile = b.addTest(.{
            .root_source_file = LazyPath.relative(test_file orelse "src/main.zig"),
            .target = target,
        });
        headers_obj.filters = if (test_filter) |filter| &.{filter} else &.{};
...
pub fn configureObjectStep(b: *std.Build, obj: *Compile, obj_step: *std.Build.Step, comptime Target: type, target: Target) !void {
