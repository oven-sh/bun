...
let $debug_log_enabled = ((env) => (
  ...
  || (env.BUN_DEBUG_NODE_CHILD_PROCESS)
  || (env.DEBUG_NODE_CHILD_PROCESS)
))(Bun.env);
let $debug_pid_prefix = Bun.env.SHOW_PID === '1';
let $debug_log = $debug_log_enabled ? (...args) => {
  // warn goes to stderr without colorizing
  console.warn(($debug_pid_prefix ? `[${process.pid}] ` : '') + (Bun.enableANSIColors ? '\x1b[90m[child_process]\x1b[0m' : '[child_process]'), ...args);
} : () => {};
// build2/tmp/node/child_process.ts
...
var execFileSync = function(file, args, options) {
...
var abortChildProcess = function(child, killSignal2, reason) {
  if (!child)
    return;
  try {
...
var ERR_CHILD_PROCESS_IPC_REQUIRED = function(name) {
...
var StreamModule = @getInternalField(@internalModuleRegistry, 39) || @createInternalModuleById(39);
var {
  constants: { signals }
} = @getInternalField(@internalModuleRegistry, 28) || @createInternalModuleById(28);
var { promisify } = @getInternalField(@internalModuleRegistry, 48) || @createInternalModuleById(48);
