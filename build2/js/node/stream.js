...
let $debug_log_enabled = ((env) => (
  ...
  || (env.BUN_DEBUG_NODE_STREAM)
  || (env.DEBUG_NODE_STREAM)
))(Bun.env);
let $debug_pid_prefix = Bun.env.SHOW_PID === '1';
let $debug_log = $debug_log_enabled ? (...args) => {
  // warn goes to stderr without colorizing
  console.warn(($debug_pid_prefix ? `[${process.pid}] ` : '') + (Bun.enableANSIColors ? '\x1b[90m[stream]\x1b[0m' : '[stream]'), ...args);
} : () => {};
// build2/tmp/node/stream.ts
...
var createNativeStreamReadable = function(nativeType, Readable) {
  ...
  var NativeReadable = class NativeReadable2 extends Readable {
    ...
    _read(maxToRead) {
      $debug_log("NativeReadable._read", this.__id);
      if (this.#pendingRead) {
        $debug_log("pendingRead is true", this.__id);
        return;
      }
      var ptr = this.#bunNativePtr;
      $debug_log("ptr @ NativeReadable._read", ptr, this.__id);
      if (ptr === 0) {
        this.push(null);
        return;
      }
      if (!this.#constructed) {
        $debug_log("NativeReadable not constructed yet", this.__id);
    ...
    #internalConstruct(ptr) {
      ...
      $debug_log("NativeReadable internal `start` result", result, this.__id);
      if (typeof result === "number" && result > 1) {
        this.#hasResized = true;
        $debug_log("NativeReadable resized", this.__id);
        this.#highWaterMark = Math.min(this.#highWaterMark, result);
      }
      if (drainFn) {
        const drainResult = drainFn(ptr);
        $debug_log("NativeReadable drain result", drainResult, this.__id);
    ...
    #getRemainingChunk(maxToRead = this.#highWaterMark) {
      ...
      $debug_log("chunk @ #getRemainingChunk", chunk, this.__id);
    ...
    #handleResult(result, view, isClosed) {
      $debug_log("result, isClosed @ #handleResult", result, isClosed, this.__id);
    ...
    #internalRead(view, ptr) {
      ...
      if (@isPromise(result)) {
        ...
        return result.then((result2) => {
          this.#pendingRead = false;
          $debug_log("pending no longerrrrrrrr (result returned from pull)", this.__id);
          this.#remainingChunk = this.#handleResult(result2, view, closer[0]);
        }, (reason) => {
          $debug_log("error from pull", reason, this.__id);
    ...
    _destroy(error, callback) {
      ...
      $debug_log("NativeReadable destroyed", this.__id);
      cancel(ptr, error);
      callback(error);
    }
    ref() {
...
var NativeWritable = function(pathOrFdOrSink, options = {}) {
...
var NativeWritable_internalConstruct = function(cb) {
...
var NativeWritable_lazyConstruct = function(stream) {
...
var NativeWritable_internalDestroy = function(error, cb) {
...
var NativeWritable_internalFinal = function(cb) {
...
var EE = @lazy("events");
var StringDecoder = @requireNativeModule("string_decoder").StringDecoder;
...
var require_validators = __commonJS({
  "node_modules/readable-stream/lib/internal/validators.js"(exports, module) {
    ...
    function parseFileMode(value, name, def) {
      ...
      validateInt32(value, name, 0, 4294967295);
      return value;
    }
...
var require_readable = __commonJS({
  "node_modules/readable-stream/lib/internal/streams/readable.js"(exports, module) {
    ...
    Readable.prototype.on = function(ev, fn) {
      ...
      if (ev === "data") {
        ...
        if (state.flowing !== false) {
          $debug_log("in flowing mode!", this.__id);
          this.resume();
        } else {
          $debug_log("in readable mode!", this.__id);
        }
      } else if (ev === "readable") {
        $debug_log("readable listener added!", this.__id);
        if (!state.endEmitted && !state.readableListening) {
          state.readableListening = state.needReadable = true;
          state.flowing = false;
          state.emittedReadable = false;
          $debug_log("on readable - state.length, reading, emittedReadable", state.length, state.reading, state.emittedReadable, this.__id);
          if (state.length) {
          ...
          $debug_log("end already emitted...", this.__id);
        }
      }
      return res;
    };
    ...
    function emitReadable(stream, state) {
      $debug_log("NativeReadable - emitReadable", stream.__id);
    ...
    function readableAddChunk(stream, chunk, encoding, addToFront) {
      $debug_log("readableAddChunk", chunk, stream.__id);
    ...
    function addChunk(stream, state, chunk, addToFront) {
      $debug_log("adding chunk", stream.__id);
      $debug_log("chunk", chunk.toString(), stream.__id);
      if (state.flowing && state.length === 0 && !state.sync && stream.listenerCount("data") > 0) {
        ...
        $debug_log("needReadable @ addChunk", state.needReadable, stream.__id);
        if (state.needReadable)
          emitReadable(stream, state);
      }
      $debug_log("about to maybereadmore");
      maybeReadMore(stream, state);
    }
    ...
    Readable.prototype.read = function(n) {
      ...
      if (n === 0 && state.needReadable && ((state.highWaterMark !== 0 ? state.length >= state.highWaterMark : state.length > 0) || state.ended)) {
        $debug_log("read: emitReadable or endReadable", state.length, state.ended, this.__id);
      ...
      if (n === 0 && state.ended) {
        $debug_log("read: calling endReadable if length 0 -- length, state.ended", state.length, state.ended, this.__id);
        if (state.length === 0)
          endReadable(this);
        return null;
      }
      let doRead = state.needReadable;
      $debug_log("need readable", doRead, this.__id);
      if (state.length === 0 || state.length - n < state.highWaterMark) {
        doRead = true;
        $debug_log("length less than watermark", doRead, this.__id);
      }
      if (state.ended || state.reading || state.destroyed || state.errored || !state.constructed) {
        $debug_log("state.constructed?", state.constructed, this.__id);
        doRead = false;
        $debug_log("reading, ended or constructing", doRead, this.__id);
      } else if (doRead) {
        $debug_log("do read", this.__id);
        state.reading = true;
        state.sync = true;
        if (state.length === 0)
        ...
        try {
          ...
          if (@isPromise(result)) {
            $debug_log("async _read", this.__id);
            const peeked = Bun.peek(result);
            $debug_log("peeked promise", peeked, this.__id);
            if (peeked !== result) {
              result = peeked;
            }
          }
          if (@isPromise(result) && result?.then && @isCallable(result.then)) {
            $debug_log("async _read result.then setup", this.__id);
      ...
      $debug_log("n @ fromList", n, this.__id);
      let ret;
      if (n > 0)
        ret = fromList(n, state);
      else
        ret = null;
      $debug_log("ret @ read", ret, this.__id);
      if (ret === null) {
        state.needReadable = state.length <= state.highWaterMark;
        $debug_log("state.length while ret = null", state.length, this.__id);
        n = 0;
      } else {
        state.length -= n;
        if (state.multiAwaitDrain) {
    ...
    Readable.prototype.pipe = function(dest, pipeOpts) {
      ...
      $debug_log("pipe count=%d opts=%j", state.pipes.length, pipeOpts, src.__id);
      ...
      function onend() {
        $debug_log("onend", src.__id);
        dest.end();
      }
      let ondrain;
      let cleanedUp = false;
      function cleanup() {
        $debug_log("cleanup", src.__id);
      ...
      function pause() {
        if (!cleanedUp) {
          if (state.pipes.length === 1 && state.pipes[0] === dest) {
            $debug_log("false write response, pause", 0, src.__id);
            state.awaitDrainWriters = dest;
            state.multiAwaitDrain = false;
          } else if (state.pipes.length > 1 && state.pipes.includes(dest)) {
            $debug_log("false write response, pause", state.awaitDrainWriters.size, src.__id);
      ...
      src.on("data", ondata);
      function ondata(chunk) {
        $debug_log("ondata", src.__id);
        const ret = dest.write(chunk);
        $debug_log("dest.write", ret, src.__id);
        if (ret === false) {
          pause();
        }
      }
    ...
    function nReadingNextTick(self) {
      $debug_log("on readable nextTick, calling read(0)", self.__id);
      self.read(0);
    }
    Readable.prototype.resume = function() {
      const state = this._readableState;
      if (!state.flowing) {
        $debug_log("resume", this.__id);
    ...
    Readable.prototype.pause = function() {
      $debug_log("call pause flowing=%j", this._readableState.flowing, this.__id);
      if (this._readableState.flowing !== false) {
        $debug_log("pause", this.__id);
    ...
    function endReadable(stream) {
      const state = stream._readableState;
      $debug_log("endEmitted @ endReadable", state.endEmitted, stream.__id);
      if (!state.endEmitted) {
        state.ended = true;
    ...
    function endReadableNT(state, stream) {
      $debug_log("endReadableNT -- endEmitted, state.length", state.endEmitted, state.length, stream.__id);
      if (!state.errored && !state.closeEmitted && !state.endEmitted && state.length === 0) {
        state.endEmitted = true;
        stream.emit("end");
        $debug_log("end emitted @ endReadableNT", stream.__id);
...
var require_writable = __commonJS({
  "node_modules/readable-stream/lib/internal/streams/writable.js"(exports, module) {
    ...
    function needFinish(state, tag) {
      ...
      $debug_log("needFinish", needFinish2, tag);
      return needFinish2;
    }
    ...
    function finishMaybe(stream, state, sync) {
      $debug_log("finishMaybe -- state, sync", state, sync, stream.__id);
...
var _pathOrFdOrSink = Symbol("pathOrFdOrSink");
var _fileSink = Symbol("fileSink");
var _native = Symbol("native");
