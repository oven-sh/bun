...
$$capture_start$$(function(console) {  const { inspect, formatWithOptions } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 48/*node:util*/) || __intrinsic__createInternalModuleById(48/*node:util*/));
  const { isBuffer } = (__intrinsic__requireNativeModule("buffer"));
  ...
  function stripVTControlCharacters(str) {
    return (RegExpPrototypeSymbolReplace as any).__intrinsic__call(ansi, str, "");
  }
  ...
  const renderRow = (row, columnWidths) => {
    ...
    for (let i = 0; i < row.length; i++) {
      ...
      out +=
        (StringPrototypeRepeat as any).__intrinsic__call(" ", needed) + cell + StringPrototypeRepeat.__intrinsic__call(" ", Math.ceil(needed));
  ...
  const table = (head, columns) => {
    const columnWidths = ArrayPrototypeMap.__intrinsic__call(head, h => getStringWidth(h)) as number[];
    const longestColumn = Math.max(...(ArrayPrototypeMap as any).__intrinsic__call(columns, a => a.length));
    const rows: any = __intrinsic__newArrayWithSize(longestColumn);

    for (let i = 0; i < head.length; i++) {
      const column = columns[i];
      for (let j = 0; j < longestColumn; j++) {
        if (rows[j] === undefined) rows[j] = [];
        const value = (rows[j][i] = ObjectPrototypeHasOwnProperty.__intrinsic__call(column, j) ? column[j] : "");
    ...
    const divider = ArrayPrototypeMap.__intrinsic__call(columnWidths, i =>
      StringPrototypeRepeat.__intrinsic__call(tableChars.middleMiddle, i + 2),
    );

    let result =
      tableChars.topLeft +
      ArrayPrototypeJoin.__intrinsic__call(divider, tableChars.topMiddle) +
      tableChars.topRight +
      "\n" +
      renderRow(head, columnWidths) +
      "\n" +
      tableChars.leftMiddle +
      ArrayPrototypeJoin.__intrinsic__call(divider, tableChars.rowMiddle) +
    ...
    result +=
      tableChars.bottomLeft + ArrayPrototypeJoin.__intrinsic__call(divider, tableChars.bottomMiddle) + tableChars.bottomRight;
  ...
  function Console(this: any, options /* or: stdout, stderr, ignoreErrors = true */) {
    ...
    if (!stdout || typeof stdout.write !== "function") {
      ...
      __intrinsic__throwTypeError("stdout is not a writable stream");
    }
    if (!stderr || typeof stderr.write !== "function") {
      // throw new ERR_CONSOLE_WRITABLE_STREAM("stderr");
      __intrinsic__throwTypeError("stderr is not a writable stream");
    }

    if (typeof colorMode !== "boolean" && colorMode !== "auto") {
      // throw new ERR_INVALID_ARG_VALUE("colorMode", colorMode);
      __intrinsic__throwTypeError("colorMode must be a boolean or 'auto'");
    }
    ...
    [kWriteToConsole]: {
      ...
      value: function (streamSymbol, string) {
        ...
        if (groupIndent.length !== 0) {
          if (StringPrototypeIncludes.__intrinsic__call(string, "\n")) {
            // ?!
            string = (RegExpPrototypeSymbolReplace.__intrinsic__call as any)(/\n/g, string, `\n${groupIndent}`);
          }
  ...
  const consoleMethods: any = {
    ...
    assert(expression, ...args) {
      if (!expression) {
        ...
        this.warn.__intrinsic__apply(this, args);
      }
    },
    ...
    group(...data) {
      if (data.length > 0) {
        this.log.__intrinsic__apply(this, data);
      }
      this[kGroupIndent] += StringPrototypeRepeat.__intrinsic__call(" ", this[kGroupIndentationWidth]);
    },

    groupEnd() {
      this[kGroupIndent] = StringPrototypeSlice.__intrinsic__call(
        this[kGroupIndent],
        0,
    ...
    table(tabularData, properties) {
      ...
      const mapIter = __intrinsic__isMapIterator(tabularData);
      ...
      if (isKeyValue || __intrinsic__isMap(tabularData)) {
        const keys = [];
        const values = [];
        let length = 0;
        if (mapIter) {
          for (; i < tabularData.length / 2; ++i) {
            ArrayPrototypePush.__intrinsic__call(keys, _inspect(tabularData[i * 2]));
            ArrayPrototypePush.__intrinsic__call(values, _inspect(tabularData[i * 2 + 1]));
            length++;
          }
        } else {
          for (const { 0: k, 1: v } of tabularData) {
            ArrayPrototypePush.__intrinsic__call(keys, _inspect(k));
            ArrayPrototypePush.__intrinsic__call(values, _inspect(v));
            length++;
          }
        }
      ...
      const setIter = __intrinsic__isSetIterator(tabularData);
      // if (setIter) tabularData = previewEntries(tabularData);

      const setlike = setIter || mapIter || __intrinsic__isSet(tabularData);
      if (setlike) {
        const values = [];
        let length = 0;
      ...
      for (; i < indexKeyArray.length; i++) {
        ...
        if (properties === undefined && primitive) {
          ...
          for (const key of keys) {
            map[key] ??= [];
            if ((primitive && properties) || !ObjectPrototypeHasOwnProperty.__intrinsic__call(item, key)) map[key][i] = "";
      ...
      if (hasPrimitives) {
        ArrayPrototypePush.__intrinsic__call(keys, valuesKey);
        ArrayPrototypePush.__intrinsic__call(values, valuesKeyArray);
      }
      ArrayPrototypeUnshift.__intrinsic__call(keys, indexKey);
      ArrayPrototypeUnshift.__intrinsic__call(values, indexKeyArray);
  ...
  function pad(value) {
    return StringPrototypePadStart.__intrinsic__call(`${value}`, 2, "0");
  }
  ...
  function formatTime(ms) {
    ...
    if (hours !== 0 || minutes !== 0) {
      ({ 0: seconds, 1: ms } = (StringPrototypeSplit.__intrinsic__call as any)(NumberPrototypeToFixed.__intrinsic__call(seconds, 3), "."));
      const res = hours !== 0 ? `${hours}:${pad(minutes)}` : minutes;
      return `${res}:${pad(seconds)}.${ms} (${hours !== 0 ? "h:m" : ""}m:ss.mmm)`;
    }

    if (seconds !== 0) {
      return `${NumberPrototypeToFixed.__intrinsic__call(seconds, 3)}s`;
    }

    return `${Number(NumberPrototypeToFixed.__intrinsic__call(ms, 3))}ms`;
  }
  ...
  const isArray = v => __intrinsic__isJSArray(v) || __intrinsic__isTypedArrayView(v) || isBuffer(v);
