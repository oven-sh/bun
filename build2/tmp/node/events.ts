...
// Sourced from src/js/node/events.js
...
function emitError(emitter, args) {
  ...
  if (errorMonitor) {
    for (var handler of ArrayPrototypeSlice.__intrinsic__call(errorMonitor)) {
...
const emitWithRejectionCapture = function emit(type, ...args) {
  ...
  for (let i = 0, { length } = maybeClonedHandlers; i < length; i++) {
    ...
    if (result !== undefined && __intrinsic__isPromise(result)) {
...
function onceWrapper(type, listener, ...args) {
  ...
  listener.__intrinsic__apply(this, args);
}
...
function checkListener(listener) {
  if (typeof listener !== "function") {
    __intrinsic__throwTypeError("The listener must be a function");
  }
}
...
class EventEmitterAsyncResource extends EventEmitter {
  ...
  constructor(options) {
    if (!AsyncResource) {
      AsyncResource = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 10/*node:async_hooks*/) || __intrinsic__createInternalModuleById(10/*node:async_hooks*/)).AsyncResource;
    }
...
$$EXPORT$$(EventEmitter).$$EXPORT_END$$;
