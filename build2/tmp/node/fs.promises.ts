...
// Sourced from src/js/node/fs.promises.ts
...
function watch(
  ...
  if (filename instanceof URL) {
    ...
    __intrinsic__throwTypeError("Expected path to be a string or Buffer");
  }
...
function cp(src, dest, options) {
  ...
  if (options.dereference || options.filter || options.preserveTimestamps || options.verbatimSymlinks) {
    if (!lazy_cp) lazy_cp = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 3/*internal/fs/cp-sync.ts*/) || __intrinsic__createInternalModuleById(3/*internal/fs/cp-sync.ts*/));
