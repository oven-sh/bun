...
// Sourced from src/js/node/util.js


// Hardcoded module "node:util"
const types = (__intrinsic__requireNativeModule("util/types"));
/** @type {import('node-inspect-extracted')} */
const utl = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 7/*internal/util/inspect*/) || __intrinsic__createInternalModuleById(7/*internal/util/inspect*/));
...
function deprecate(fn, msg, code) {
  ...
  function deprecated() {
    ...
    return fn.__intrinsic__apply(this, arguments);
  }
  return deprecated;
}
...
var promisify = function promisify(original) {
  if (typeof original !== "function") __intrinsic__throwTypeError('The "original" argument must be of type Function');
  if (kCustomPromisifiedSymbol && original[kCustomPromisifiedSymbol]) {
    var fn = original[kCustomPromisifiedSymbol];
    if (typeof fn !== "function") {
      __intrinsic__throwTypeError('The "util.promisify.custom" argument must be of type Function');
    }
  ...
  function fn() {
    ...
    try {
      original.__intrinsic__apply(this, args);
    } catch (err) {
      promiseReject(err);
    }
    return promise;
...
function callbackify(original) {
  if (typeof original !== "function") {
    __intrinsic__throwTypeError('The "original" argument must be of type Function');
  }
  function callbackified() {
    var args = Array.prototype.slice.__intrinsic__call(arguments);
    var maybeCb = args.pop();
    if (typeof maybeCb !== "function") {
      __intrinsic__throwTypeError("The last argument must be of type Function");
    }
    var self = this;
    var cb = function () {
      return maybeCb.__intrinsic__apply(self, arguments);
    };
