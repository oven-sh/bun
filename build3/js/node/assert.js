(function (){"use strict";// build3/tmp/node/assert.ts
...
var require_errors = __commonJS({
  "assert/build/internal/errors.js"(exports, module2) {
    ...
    function endsWith(str, search, this_len) {
      return (this_len === undefined || this_len > str.length) && (this_len = str.length), str.substring(this_len - search.length, this_len) === search;
    }
    ...
    createErrorType("ERR_INVALID_ARG_TYPE", function(name, expected, actual) {
      assert === undefined && (assert = require_assert()), assert(typeof name == "string", "'name' must be a string");
      var determiner;
    ...
    createErrorType("ERR_MISSING_ARGS", function() {
      ...
      assert === undefined && (assert = require_assert()), assert(args.length > 0, "At least one arg needs to be specified");
...
`);
      ...
      i > 3 && (end = `
`.concat(blue, "...").concat(white).concat(end), skipped = true), other !== "" && (end = `
...
`.concat(green, "+ actual").concat(white, " ").concat(red, "- expected").concat(white), skippedMsg = " ".concat(blue, "...").concat(white, " Lines skipped");
      for (i = 0;i < maxLines; i++) {
        ...
        if (actualLines.length < i + 1)
          cur > 1 && i > 2 && (cur > 4 ? (res += `
`.concat(blue, "...").concat(white), skipped = true) : cur > 3 && (res += `
...
`.concat(green, "+").concat(white, " ").concat(actualLines[i]), printedLines++;
          ...
          divergingLines && endsWith(expectedLine, ",") && expectedLine.slice(0, -1) === actualLine && (divergingLines = false, actualLine += ","), divergingLines ? (cur > 1 && i > 2 && (cur > 4 ? (res += `
`.concat(blue, "...").concat(white), skipped = true) : cur > 3 && (res += `
  `.concat(actualLines[i - 2]), printedLines++), res += `
  `.concat(actualLines[i - 1]), printedLines++), lastPos = i, res += `
...
`) : other = " ".concat(operator, " ").concat(other)), _this = _possibleConstructorReturn(this, _getPrototypeOf(AssertionError2).@call(this, "".concat(_res).concat(other)));
      ...
      return _createClass(AssertionError2, [
        {
          key: "toString",
          value: function() {
...
var require_assert = __commonJS({
  "assert/build/assert.js"(exports, module2) {
    ...
    function fail(actual, expected, message, operator, stackStartFn) {
      ...
      message !== undefined && (errArgs.message = message);
      var err = new AssertionError(errArgs);
      throw internalMessage && (err.message = internalMessage, err.generatedMessage = true), err;
    }
    assert.fail = fail;
    ...
    assert.deepEqual = function deepEqual(actual, expected, message) {
      if (arguments.length < 2)
        throw new ERR_MISSING_ARGS("actual", "expected");
      isDeepEqual(actual, expected, false) || innerFail({
        actual,
        expected,
        message,
        operator: "deepEqual",
    ...
    assert.deepStrictEqual = function deepStrictEqual(actual, expected, message) {
      if (arguments.length < 2)
        throw new ERR_MISSING_ARGS("actual", "expected");
      isDeepEqual(actual, expected, true) || innerFail({
        actual,
        expected,
        message,
    ...
    var Comparison = function Comparison2(obj, keys, actual) {
      ...
      _classCallCheck(this, Comparison2), keys.forEach(function(key) {
        (key in obj) && (actual !== undefined && typeof actual[key] == "string" && isRegExp(obj[key]) && obj[key].test(actual[key]) ? _this[key] = actual[key] : _this[key] = obj[key]);
      });
    };
    ...
    function expectedException(actual, expected, msg, fn) {
      ...
      return expected.prototype !== undefined && actual instanceof expected ? true : Error.isPrototypeOf(expected) ? false : expected.@call({}, actual) === true;
    }
    ...
    function expectsNoError(stackStartFn, actual, error, message) {
      if (actual !== NO_EXCEPTION_SENTINEL) {
        if (typeof error == "string" && (message = error, error = undefined), !error || expectedException(actual, error)) {
