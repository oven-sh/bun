...
// Sourced from src/js/bun/sqlite.ts
...
class Database {
  constructor(filenameGiven, options) {
    if (typeof filenameGiven === "undefined") {
      ...
      __intrinsic__throwTypeError(`Expected 'filename' to be a string, got '${typeof filenameGiven}'`);
    }
    ...
    if (typeof options === "object" && options) {
      ...
      if ("readOnly" in options) __intrinsic__throwTypeError('Misspelled option "readOnly" should be "readonly"');
  ...
  query(query) {
    if (typeof query !== "string") {
      __intrinsic__throwTypeError(`Expected 'query' to be a string, got '${typeof query}'`);
    }
  ...
  transaction(fn, self) {
    if (typeof fn !== "function") __intrinsic__throwTypeError("Expected first argument to be a function");
...
const wrapTransaction = (fn, db, { begin, commit, rollback, savepoint, release, rollbackTo }) =>
  function transaction(this, ...args) {
    ...
    try {
      before.run();
      const result = fn.__intrinsic__apply(this, args);
      after.run();
      return result;
    } catch (ex) {
      if (db.inTransaction) {
