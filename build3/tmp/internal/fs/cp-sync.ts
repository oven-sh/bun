...
// Sourced from src/js/internal/fs/cp-sync.ts
...
const normalizePathToArray = path =>
  ArrayPrototypeFilter.__intrinsic__call(StringPrototypeSplit.__intrinsic__call(resolve(path), sep), Boolean);
...
const {
  ...
  unlinkSync,
  utimesSync,
} = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 21/*node:fs*/) || __intrinsic__createInternalModuleById(21/*node:fs*/));
const { dirname, isAbsolute, join, parse, resolve, sep } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 30/*node:path*/) || __intrinsic__createInternalModuleById(30/*node:path*/));
const { isPromise } = (__intrinsic__requireNativeModule("util/types"));
