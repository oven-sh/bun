...
// Sourced from src/js/internal/fs/cp.ts
...
const { chmod, copyFile, lstat, mkdir, opendir, readlink, stat, symlink, unlink, utimes } = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 22/*node:fs/promises*/) || __intrinsic__createInternalModuleById(22/*node:fs/promises*/));
...
function getStats(src, dest, opts) {
  ...
  return SafePromiseAll([
    statFunc(src),
    PromisePrototypeThen.__intrinsic__call(statFunc(dest), undefined, err => {
...
const normalizePathToArray = path => ArrayPrototypeFilter.__intrinsic__call(StringPrototypeSplit(resolve(path), sep), Boolean);
