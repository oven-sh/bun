...
// Sourced from src/js/node/fs.js
...
const promises = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 22/*node:fs/promises*/) || __intrinsic__createInternalModuleById(22/*node:fs/promises*/));
const Stream = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 39/*node:stream*/) || __intrinsic__createInternalModuleById(39/*node:stream*/));
const { isArrayBufferView } = (__intrinsic__requireNativeModule("util/types"));

var _writeStreamPathFastPathSymbol = Symbol.for("Bun.NodeWriteStreamFastPath");
var _fs = Symbol.for("#fs");

const constants = __intrinsic__processBindingConstants.fs;
...
var access = function access(...args) {
  ...
  copyFile = function copyFile(...args) {
    ...
    if (typeof callback !== "function") {
      const err = __intrinsic__makeTypeError("Callback must be a function");
  ...
  exists = function exists(path, callback) {
    ...
    try {
      fs.exists.__intrinsic__apply(fs, [path]).then(
...
function getValidatedPath(p) {
  ...
  if (typeof p !== "string") __intrinsic__throwTypeError("Path must be a string or URL.");
  return (_pathModule ??= (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 30/*node:path*/) || __intrinsic__createInternalModuleById(30/*node:path*/))).resolve(p);
}
function watchFile(filename, options, listener) {
  ...
  if (typeof listener !== "function") {
    __intrinsic__throwTypeError("listener must be a function");
  }
...
function callbackify(fsFunction, args) {
  const callback = args[args.length - 1];
  try {
    var result = fsFunction.__intrinsic__apply(fs, args.slice(0, args.length - 1));
    result.then(
  ...
  class ReadStream extends Stream._getNativeReadableStreamPrototype(2, Stream.Readable) {
    constructor(pathOrFd, options = defaultReadStreamOptions) {
      if (typeof options !== "object" || !options) {
        __intrinsic__throwTypeError("Expected options to be an object");
      }
      ...
      if (fd != null) {
        if (typeof fd !== "number") {
          __intrinsic__throwTypeError("Expected options.fd to be a number");
        }
        ...
        if (pathOrFd.length === 0) {
          __intrinsic__throwTypeError("Expected path to be a non-empty string");
        }
        tempThis.path = tempThis.file = tempThis[readStreamPathOrFdSymbol] = pathOrFd;
      } else if (typeof pathOrFd === "number") {
        pathOrFd |= 0;
        if (pathOrFd < 0) {
          __intrinsic__throwTypeError("Expected fd to be a positive integer");
        }
        tempThis.fd = tempThis[readStreamPathOrFdSymbol] = pathOrFd;

        tempThis.autoClose = false;
      } else {
        __intrinsic__throwTypeError("Expected a path or file descriptor");
      }
      ...
      var native = __intrinsic__direct(stream);
      if (!native) {
    ...
    #internalRead(n) {
      ...
      (IS_BUN_DEVELOPMENT?$debug_log("n @ fs.ReadStream.#internalRead, after clamp", n):void 0);
      ...
      if (this.#fileSize === -1 && bytesRead === 0 && pos === undefined) {
        ...
        (IS_BUN_DEVELOPMENT?$debug_log("fileSize", this.#fileSize):void 0);
      }

      // At this point, we know the file size and how much we want to read of the file
      this[kIoDone] = false;
      var res = super._read(n);
      (IS_BUN_DEVELOPMENT?$debug_log("res -- undefined? why?", res):void 0);
      if (__intrinsic__isPromise(res)) {
        var then = res?.then;
        if (then && __intrinsic__isCallable(then)) {
          res.then(
            () => {
              this[kIoDone] = true;
...
var WriteStreamClass = (WriteStream = function WriteStream(path, options = defaultWriteStreamOptions) {
  ...
  if (fd != null) {
    ...
    if (path.length === 0) {
      __intrinsic__throwTypeError("Expected a non-empty path");
    }
  ...
  NativeWritable.__intrinsic__call(this, tempThis.fd, {
    ...options,
    decodeStrings: false,
    autoDestroy,
    emitClose,
  ...
  if (typeof fs?.write !== "function") {
    __intrinsic__throwTypeError("Expected fs.write to be a function");
  }

  if (typeof fs?.close !== "function") {
    __intrinsic__throwTypeError("Expected fs.close to be a function");
  }

  if (typeof fs?.open !== "function") {
    __intrinsic__throwTypeError("Expected fs.open to be a function");
  }
...
WriteStreamPrototype[writeStreamPathFastPathCallSymbol] = function WriteStreamPathFastPathCallSymbol(
  ...
  return Bun.write(this[_writeStreamPathFastPathSymbol], readStream[readStreamPathOrFdSymbol]).then(
    ...
    err => {
      readStream[kIoDone] = this[kIoDone] = true;
      WriteStream_errorOrDestroy.__intrinsic__call(this, err);
...
function WriteStream_handleWrite(er, bytes) {
  if (er) {
    return WriteStream_errorOrDestroy.__intrinsic__call(this, er);
  }
...
WriteStreamPrototype._destroy = function _destroy(err, cb) {
  ...
  if (this[kIoDone]) {
    this.once(kIoDone, () => WriteStream_internalClose.__intrinsic__call(this, err, cb));
    return;
  }

  WriteStream_internalClose.__intrinsic__call(this, err, cb);
};
...
WriteStreamPrototype.write = function write(chunk, encoding, cb) {
  ...
  const callback = native
    ? (err, bytes) => {
        this[kIoDone] = false;
        WriteStream_handleWrite.__intrinsic__call(this, err, bytes);
        this.emit(kIoDone);
        if (cb) !err ? cb() : cb(err);
      }
    : () => {};
  ...
  if (this._write) {
    ...
    return NativeWritable.prototype.write.__intrinsic__call(this, chunk, encoding, callback, native);
  }
};
...
WriteStreamPrototype.end = function end(chunk, encoding, cb) {
  var native = this.pos === undefined;
  return NativeWritable.prototype.end.__intrinsic__call(this, chunk, encoding, cb, native);
};
...
function cpSync(src, dest, options) {
  ...
  if (options.dereference || options.filter || options.preserveTimestamps || options.verbatimSymlinks) {
    if (!lazy_cpSync) lazy_cpSync = (__intrinsic__getInternalField(__intrinsic__internalModuleRegistry, 3/*internal/fs/cp-sync.ts*/) || __intrinsic__createInternalModuleById(3/*internal/fs/cp-sync.ts*/));
...
function _toUnixTimestamp(time, name = "time") {
  ...
  __intrinsic__throwTypeError(`Expected ${name} to be a number or Date`);
}
