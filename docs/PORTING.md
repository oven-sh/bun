...
| `bun.ptr.TaggedPointer` / `TaggedPointerUnion` | **Don't.** Per §Dispatch, store `(tag: u8, ptr: *mut ())` as two fields. The 8→16 byte cost is negligible (~dozens of instances). If a struct truly needs 8-byte packing for cache-line reasons, leave `// PERF(port): was TaggedPointer pack`. |
