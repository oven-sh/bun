...
Generates a presigned URL for an S3 object. This URL can be used to grant temporary access to the object.

```ts
interface S3FilePresignOptions {
  /**
   * The HTTP method to be used with the presigned URL.
   * @example "GET"
   * @example "PUT"
   * @example "POST"
   * @example "DELETE"
