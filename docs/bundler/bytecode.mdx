...
By default, Bun compiles every function in the bundle to bytecode, including functions nested inside other functions. Use `--bytecode-depth` to compile only the top `N` levels of nesting. Functions past that limit are compiled from source when they are first called.
