...
When using `--compile` to create standalone executables, bytecode supports both CommonJS and ESM output formats:

```bash terminal icon="terminal"
# ESM bytecode
bun build --compile --bytecode --format=esm ./cli.ts --outfile=mycli

# CJS bytecode
bun build --compile --bytecode --format=cjs ./cli.ts --outfile=mycli
```

When `--format` is not specified, bytecode defaults to CommonJS for backwards compatibility.
