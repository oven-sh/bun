...
The two modes differ in how much parsing they skip. With `--compile`, Bun also embeds module metadata (import and export information), so the engine skips parsing entirely. The `.jsc` sidecar accelerates the evaluate phase, while the module graph is still parsed from source.
