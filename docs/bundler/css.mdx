...
The `:dir()` pseudo-class styles elements based on their text direction (RTL or LTR), as determined by the document or explicit direction attributes, so you can write direction-aware styles without JavaScript.
...
The `:lang()` pseudo-class targets elements based on their language. Modern CSS accepts multiple language codes in a single `:lang()`, so you can group rules for related languages.
...
The `:is()` pseudo-class function (formerly `:matches()`) takes a selector list and matches if any selector in the list matches, so you can group selectors instead of repeating them.
...
The `:not()` pseudo-class excludes elements that match a selector. The modern version accepts multiple arguments, so one `:not()` can exclude several patterns.
...
The `system-ui` generic font family uses the device's native UI font, so you don't have to specify a font stack for each platform.
...
CSS modules support composing class selectors together, so you can reuse style rules across multiple classes.
