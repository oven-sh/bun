...
3. Bun visits each JavaScript module once. A CSS file that several modules import appears once, at the **first** place the walk reaches it.
4. Bun then expands the `@import` rules inside those CSS files with CSS semantics: when the same file is imported more than once this way, the **last** occurrence is the one that stays.
