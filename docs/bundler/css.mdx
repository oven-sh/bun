...
In a CSS module, Bun also scopes the names and classes of [view transitions](https://developer.mozilla.org/en-US/docs/Web/API/View_Transition_API): the values of `view-transition-name` and `view-transition-class`, and the argument of `::view-transition-group()`, `::view-transition-group-children()`, `::view-transition-image-pair()`, `::view-transition-old()`, and `::view-transition-new()`.

A class in one of these pseudo-elements is part of the imported object, like a class in a regular selector:
...
// the scoped name, for example "slide_123"
element.style.viewTransitionClass = styles.slide;
```
