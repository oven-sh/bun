...
The prefixed `:-webkit-any()` and `:-moz-any()` forms accept only compound selectors. When an argument has a combinator, as in `h2:not(.sidebar *, footer *)`, Bun emits only the `:not(:is())` form.
