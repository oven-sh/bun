...
To change this, pass [`cssTarget`](/bundler#csstarget) to `Bun.build` (or `--css-target` to `bun build`) with the oldest browsers you support. Syntax those browsers understand is emitted as written, without the fallbacks described below.
