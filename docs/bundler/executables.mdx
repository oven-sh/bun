...
<Note>
  On x64, Bun requires a CPU with SSE4.2 (Nehalem, 2008+); wider SIMD is dispatched at runtime when available. Bun 1.3
  and earlier shipped a separate `-modern` (Haswell/AVX2) build alongside a `-baseline` build. Both suffixes are still
  accepted by `--target`; for Bun 1.4 and later they resolve to the same x64 binary.
</Note>
