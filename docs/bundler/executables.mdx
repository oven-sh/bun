...
### Bytecode order files

An executable maps its bytecode from disk and reads the parts it runs. Most programs run a small share of their code at startup, and that share is spread over the whole file. An order file records what one run read, and the next build places it together at the front of the bytecode. The executable then reads fewer pages at startup and keeps fewer in memory.

Record an order file by running the executable with `BUN_BYTECODE_ORDER_OUT`. The executable writes the file when it exits. `%p` in the path becomes the process id.

```sh
bun build --compile --bytecode ./app.ts --outfile myapp
BUN_BYTECODE_ORDER_OUT=./myapp.order ./myapp
...
The order file names functions, strings and modules by a hash of their source text. It contains no source code. The recording covers the main thread and every `Worker`. The order file changes where bytecode is placed and nothing else. Two builds from the same sources and the same order files are identical.

An order file from an earlier version of the program still applies. Entries that match nothing in the build are ignored. A function's hash covers its text and everything nested in it, with names the minifier picks left out, so an edit changes the hashes of the edited function and of the functions around it and of nothing else. The file also lists the functions the recorded build had and its run did not use, so the next build can tell which functions are new or changed. Bun places those right after the ones the run used, because new code is more likely to run than code a recorded run skipped.

Pass several files to cover several ways of starting the program, such as an interactive mode and a one-shot command: `--bytecode-order=interactive.order,oneshot.order` or `bytecodeOrder: ["interactive.order", "oneshot.order"]`. Bun places what the first file lists in that file's order, then what the second file adds in its order, and so on. List the most important way of starting the program first. Its functions are placed as if it were the only file.

To check how well an order file still fits, call `bytecodeOrderStats()` from `bun:jsc` in the running executable. It counts the functions the current thread has loaded from the part the order file listed (`hot`), from functions new since the recording (`unknown`), and from everything else (`cold`). A growing share of `cold` means it is time to record again.
...
interface CompileBuildOptions {
  ...
  bytecodeOrder?: string | string[]; // Order file(s) to lay the bytecode out by (requires bytecode: true)
