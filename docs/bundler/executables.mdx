...
A standalone executable keeps the JIT reluctant to compile while it starts up: code that only runs during startup rarely pays back its compile time, and the JIT threads compete with initialization. The window ends as soon as the main thread becomes interactive — the first stdin data it reads, its first write to the terminal or to stdout/stderr (`process.stdout`/`process.stderr`, `console`, `Bun.stdout.writer()`, `fs.writeSync`), the first time the event loop sits idle for 100ms — or after `BUN_STARTUP_JIT_DEFERRAL_MS` milliseconds (default `1500`). `BUN_STARTUP_JIT_DEFERRAL=0` (or `_MS=0`) disables it; `BUN_JSC_verboseOSR=1` logs which event ended it.
