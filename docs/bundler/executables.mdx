...
Standalone executables get three startup optimizations, each on by default when it applies:

| `Bun.build`                   | CLI to turn off             | What it does                                                                                                                       |
| ----------------------------- | --------------------------- | ---------------------------------------------------------------------------------------------------------------------------------- |
| `optimize.bytecode`           | `--no-optimize-bytecode`    | With `--bytecode`, runs JavaScriptCore's build-time optimization passes over the generated bytecode.                               |
| `optimize.prelinkModules`     | `--no-prelink-modules`      | With `--bytecode --format=esm`, embeds a pre-resolved module graph so imports and exports are not resolved by name at startup.     |
| `optimize.startupJITDeferral` | `--no-startup-jit-deferral` | Keeps the JIT reluctant to compile while the program starts: code that only runs during startup rarely pays back its compile time. |

The JIT deferral window ends as soon as the main thread becomes interactive — the first stdin data it reads, its first write to a terminal or to stdout/stderr, the first time the event loop sits idle for 100ms — or after `maxMs` (default `1500`; `--startup-jit-deferral=<ms>` or `optimize: { startupJITDeferral: { maxMs } }`; `0` turns it off). A program that knows when it became interactive, e.g. right after its first render, can end it itself with `Bun.unsafe.endStartupJITDeferral()` from the main thread. At run time `BUN_STARTUP_JIT_DEFERRAL=0` (or `1`) and `BUN_STARTUP_JIT_DEFERRAL_MS=<ms>` override what the build chose; `BUN_JSC_verboseOSR=1` logs which event ended the window.
...
await Bun.build({
  ...
  bytecode: true,
  format: "esm",
  optimize: { startupJITDeferral: { maxMs: 500 } },
});
```
