...
`--snapshot` goes one step further than bytecode: after producing the executable, `bun build` runs it once, and once its startup work has drained, the runtime writes a snapshot of the whole process — every module already evaluated, every object your startup code built, JIT-compiled code included — and embeds it (zstd-compressed) into the executable. Later launches map that snapshot instead of booting, and continue in the event loop. Nothing in the app has to change for this:

```bash icon="terminal" terminal
bun build --compile --bytecode --snapshot ./app.ts --outfile myapp
```

The snapshot step also works on its own, against an executable built earlier, in place — so a CI job can cross-compile every target in one place and take each snapshot on a machine of the target platform, or refresh it later. Running it again replaces the previous snapshot:

```bash icon="terminal" terminal
bun build --compile ./app.ts --outfile myapp     # anywhere
bun build --snapshot --outfile myapp             # on the target platform
```

From JavaScript, `Bun.build()` takes the same thing as a top-level option (it requires `compile`): `snapshot: true`, or `snapshot: { mode, io }` with the settings described below.

#### What the build may touch

While the snapshot is being taken, the app must not do anything whose result would be frozen into every later launch, so by default `fetch()`, `node:fs`, subprocesses, DNS and sockets are refused with an error saying so, and if the app never becomes quiet the build keeps the plain executable and prints what kept it busy. An app that legitimately reads its own files or runs helpers while starting up can be allowed to, with `--snapshot-io=local` (files, subprocesses, local sockets, the resolver) or `--snapshot-io=network` (the network as well); with either, the build prints every such use, attributed to the code that made it, so what ended up in the snapshot can be reviewed.

#### Choosing the moment yourself

With `--snapshot=manual` the runtime waits instead for the app to say when it is ready:

```ts app.ts icon="/icons/typescript.svg"
process.on("restore", () => {
  // Runs first thing in every process started from the snapshot, before the
  // first event-loop turn: re-read whatever depends on this machine.
});

// Returns immediately in every process except the one the build runs to take the snapshot.
Bun.unsafe.snapshot({
  timers: "keep", // timers armed now survive, with their remaining time preserved ("cancel" drops them; by default they block the snapshot)
  envGate: ["APP_MODE"], // launches where these variables differ from the build's boot normally
});
```

#### What survives, and what does not

The runtime refreshes what it owns in a restored process: `process.argv`, `process.env`, the working directory, `process.pid`, clocks (`performance.now()`, `process.uptime()`), every source of randomness, timers, the DNS cache, and its own threads. Files and sockets the build process had open are gone; the app gets hangups for them after `'restore'`.

What it cannot refresh is anything your code derived from those before the snapshot was taken:

- A value read from `process.env`, `os.homedir()`, `process.cwd()` and the like and kept in a variable is the build machine's value in every launch. Holding on to `process.env` itself is fine — the object is updated in place — but a copy of it (`{ ...process.env }`, `dotenv`-style merging) is not. When the snapshot is written, the build prints which variables were read and where `process.env` was copied; move those reads into the `'restore'` listener, or list variables that legitimately change what the app is in `envGate`.
- Random values and timestamps produced before the snapshot (a session id generated at startup, `Date.now()` stored in a variable) are the same in every launch. Generate them in `'restore'`.
- Native addons and `bun:ffi` libraries that hold native state, and workers that are still alive when the snapshot is taken, are not supported.

A launch that cannot use its snapshot (a different OS build, an `envGate` variable that differs, `BUN_SNAPSHOT=0` in the environment) boots normally and says nothing; `BUN_SNAPSHOT_VERBOSE=1` makes it explain.

<Warning>
  The snapshot is a copy of the process's memory, and it ships inside the executable: whatever the build process had
  read — environment variables, files, credentials — is in it. Take snapshots in an environment that holds nothing
  secret. A process started from a snapshot also runs with address-space randomization disabled, because the snapshot
  has to map at fixed addresses.
</Warning>

<Note>
  The saving is startup work: an app that builds a lot of state at startup starts in the time it takes to map a file,
  and the snapshotted state stays shared, file-backed and clean until written to. State that must be rebuilt per machine
