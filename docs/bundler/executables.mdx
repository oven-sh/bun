...
| `compile.jitPolicy`       | `--compile-jit-policy`   | Starts with the JIT `8`x more reluctant to compile: code that only runs during startup rarely pays back its compile time.      |

The JIT policy returns to normal (`1`) as soon as the main thread becomes interactive — the first stdin data it reads, its first write to a terminal or to stdout/stderr, the first time the event loop sits idle for 100ms. A program that knows when startup is over, e.g. right after its first render, can call `Bun.unsafe.setJITPolicy(1)` itself. `--compile-jit-policy=1` or `compile: { jitPolicy: 1 }` starts in the normal policy; at run time `BUN_STARTUP_JIT_DEFERRAL=0` does the same (`=1` forces the default `8`), and `BUN_JSC_verboseOSR=1` logs which event ended the window.

```ts build.ts icon="/icons/typescript.svg"
await Bun.build({
  entrypoints: ["./app.ts"],
  compile: { outfile: "./myapp", jitPolicy: 1 },
  bytecode: true,
  format: "esm",
});
```
...
interface CompileBuildOptions {
  ...
  jitPolicy?: number; // JIT tier-up threshold scale at startup (default: 8; 1 = normal)
  windows?: {
