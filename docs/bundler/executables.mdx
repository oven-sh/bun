...
<Note>
  In earlier versions of Bun, `.env` and `bunfig.toml` were autoloaded by default. If your executable relied on `.env`
  from the working directory, pass `--compile-autoload-dotenv` when building.
</Note>
