...
The order file names functions and modules by a hash of their syntax, and strings by a hash of their contents. It contains no source code. The recording covers the main thread and every `Worker`. The order file changes where bytecode is placed and nothing else. Two builds from the same sources and the same order files are identical.

An order file from an earlier version of the program still applies. Entries that match nothing in the build are ignored. A function's hash covers its syntax and everything nested in it. It does not depend on the names a minifier picks, so it is the same in every build with the same minify settings. An edit changes the hash of the edited function and of the functions around it. The file also lists the functions the recorded build had and its run did not use, so the next build can tell which functions are new or changed. Bun places those right after the ones the run used, because new code is more likely to run than code a recorded run skipped.
