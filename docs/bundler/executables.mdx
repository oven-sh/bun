...
A launch that cannot use its snapshot (a different OS build, an `envGate` variable that differs) boots normally. `Bun.startupSnapshot.epoch()` tells the app which happened.
