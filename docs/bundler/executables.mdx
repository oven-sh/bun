...
### Heap images (experimental)

`--compile-image` goes one step further than bytecode: after producing the executable, `bun build` runs it once, and when the app reports that it has finished starting up, the runtime writes an image of the whole process heap — every module already evaluated, every object your startup code built, JIT-compiled code included — and embeds it (zstd-compressed) into the executable. Later launches map that image instead of booting, and continue in the event loop.

```bash icon="terminal" terminal
bun build --compile --bytecode --compile-image ./app.ts --outfile myapp
```

The app decides when the image is taken. Once startup work that does not depend on the machine it will later run on is done, call:

```ts app.ts icon="/icons/typescript.svg"
process.on("restore", () => {
  // Runs first thing in every process started from the image, before the
  // first event-loop turn: re-read whatever depends on this machine.
});

if (process.env.BUN_IMAGE_OUT) {
  Bun.unsafe.snapshot(process.env.BUN_IMAGE_OUT, {
    timers: "keep", // timers armed now keep running after restore, re-based on the new clock ("cancel" drops them; by default armed timers block the snapshot)
    envGate: ["APP_MODE"], // launches where these variables differ from the build boot normally
  });
}
```

What the runtime refreshes on its own in a restored process: `process.argv`, `process.env`, the working directory, `process.pid`, clocks (`performance.now()`, `process.uptime()`), every source of randomness, timers, the DNS cache, and its own threads. Files and sockets the build process had open are gone; the app gets hangups for them after `'restore'`.

What it cannot refresh is anything your code derived from those before the image was taken:

- A value read from `process.env`, `os.homedir()`, `process.cwd()` and the like and kept in a variable is the build machine's value in every launch. Holding on to `process.env` itself is fine — the object is updated in place — but a copy of it (`{ ...process.env }`, `dotenv`-style merging) is not. When the image is written, the build prints which variables were read before the freeze and whether `process.env` was copied; move those reads into the `'restore'` listener, or list variables that legitimately change what the app is in `envGate`.
- Random values and timestamps produced before the freeze (a session id generated at startup, `Date.now()` stored in a variable) are the same in every launch. Generate them in `'restore'`.
- Native addons and `bun:ffi` libraries that hold native state, and workers that are still alive at the freeze, are not supported.

While the image is being built, `fetch()` and other network use are refused, and the snapshot waits for in-flight work to finish; timers still armed at that point block it unless you pass `timers`.

<Warning>
  The image is a copy of the process's memory at the freeze, and it ships inside the executable: whatever the build
  process had read — environment variables, files, credentials — is in it. Build images in an environment that holds
  nothing secret. A process started from an image also runs with address-space randomization disabled, because the
  image has to map at fixed addresses.
</Warning>

<Note>
  The saving is startup work: an app that builds a lot of state at startup starts in the time it takes to map a file,
  and the imaged state stays shared, file-backed and clean until written to. State that must be rebuilt per machine
