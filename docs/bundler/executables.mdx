...
if (process.env.BUN_IMAGE_OUT) {
  Bun.unsafe.snapshot(process.env.BUN_IMAGE_OUT, {
    keepTimers: true, // timers armed now keep running after restore, re-based on the new clock
...
<Note>The saving is startup work: an app that builds a lot of state at startup starts in the time it takes to map a file, and the imaged state stays shared, file-backed and clean until written to. State that must be rebuilt per machine costs the same as before.</Note>
