...
Standalone executables can optionally load configuration files from the directory where they are run. All four autoloads are **disabled by default** so that a compiled binary behaves the same regardless of what files happen to be in the working directory:

- **`.env`** and **`bunfig.toml`** loading is **disabled** — a `bunfig.toml` in cwd could otherwise `preload` arbitrary code into your compiled app
...
<Note>
  In earlier versions of Bun, `.env` and `bunfig.toml` were autoloaded by default. If your executable relied on `.env` from the working directory, pass `--compile-autoload-dotenv` when building.
</Note>

### Enabling config loading at runtime

If your executable needs to read any of these files from the working directory at runtime, opt in per file:

<Tabs>
  <Tab title="CLI">
    ```bash icon="terminal" terminal
    # Enable runtime loading of .env files
    bun build --compile --compile-autoload-dotenv ./app.ts --outfile myapp

    # Enable runtime loading of bunfig.toml
    bun build --compile --compile-autoload-bunfig ./app.ts --outfile myapp
  ...
  <Tab title="JavaScript">
    ...
    await Bun.build({
      ...
      compile: {
        ...
        autoloadDotenv: true, // Enable .env loading
        autoloadBunfig: true, // Enable bunfig.toml loading
...
interface CompileBuildOptions {
  ...
  autoloadDotenv?: boolean; // Load .env files (default: false)
  autoloadBunfig?: boolean; // Load bunfig.toml (default: false)
  windows?: {
