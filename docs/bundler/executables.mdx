...
What the runtime refreshes on its own in a restored process: `process.argv`, `process.env`, the working directory, `process.pid`, clocks (`performance.now()`, `process.uptime()`), every source of randomness, timers, the DNS cache, and its own threads. What it cannot know about is yours: files and sockets the build process had open are gone (the app gets hangups for them after `'restore'`), and anything you computed from the build machine's environment must be recomputed in the `'restore'` listener.

While the image is being built, the runtime refuses to talk to the network, so nothing fetched during the build can end up frozen into the executable; set `BUN_IMAGE_ALLOW_LOCAL_IO=1` for the build run if startup legitimately reads files, runs helpers or binds local sockets. An image is only used when the executable is launched with the same arguments as the build run, on the same OS build (system libraries may move between reboots; that is handled), and when the `envGate` variables match; in every other case, and with `BUN_IMAGE=0`, the executable boots normally. The first launch inflates the image into `$XDG_CACHE_HOME/bun/images` (or `~/.cache/bun/images`).
