...
- **Images** (`<img src>`, `<img srcset>`, `<picture><source srcset>`, `<video poster>`, `<input type="image" src>`, SVG `<image href>` and `<use href>`) are copied and hashed. Each candidate in a `srcset` is rewritten on its own and keeps its descriptor.
- **Media** (`<video src>`, `<audio src>`, `<source src>`, `<track src>`) are copied and hashed
- **Embedded documents** (`<object data>`, `<embed src>`) are copied and hashed, unless they point at another HTML page
- **`<link href>`** with `rel="icon"` (also `shortcut icon`, `apple-touch-icon`, `apple-touch-startup-image`, `mask-icon`), `rel="manifest"`, or `rel="preload"` with `as="font|image|audio|video"` (and its `imagesrcset`) is copied and hashed
...
A `?query` or `#fragment` on the URL is kept on the rewritten path, so `<use href="./icons.svg#menu">` becomes `<use href="./icons-a1b2c3.svg#menu">`. URLs with a scheme (`https://...`, `data:...`), protocol-relative URLs (`//cdn.example.com/...`) and same-document references (`#id`) are left as-is.

A script, a stylesheet, an `<img>`, `<video>`, `<audio>` or `<source>` URL, or an icon, manifest or preload `<link>` that does not resolve to a file is a build error. For the other attributes above (`<track>`, `<object>`, `<embed>`, `<input>`, SVG, `imagesrcset`, the other icon `rel` values, `<meta>`), the file can live somewhere else: Bun leaves the URL as written and prints a warning.
