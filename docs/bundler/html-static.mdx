...
- **Images** (`<img>`, `<picture>`, `<video poster>`, `<input type="image">`, SVG `<image>`) are copied and hashed. Each candidate in a `srcset` is rewritten on its own and keeps its descriptor.
- **Media** (`<video>`, `<audio>`, `<source>`) are copied and hashed
- **Icons, manifests and preloads**: Bun copies and hashes the file of a `<link>` with `rel="icon"`, `rel="shortcut icon"`, `rel="apple-touch-icon"`, `rel="mask-icon"` or `rel="manifest"`. Bun does the same for `<link rel="preload">` with `as="font"`, `as="image"`, `as="audio"` or `as="video"`, and for each candidate of its `imagesrcset`.
- **Social images**: Bun copies and hashes the file in `<meta property="og:image" content>` and `<meta name="twitter:image" content>`

A `?query` or `#fragment` on the URL is kept on the rewritten path, so `<img src="./icons.svg#menu">` becomes `<img src="./icons-a1b2c3.svg#menu">`. URLs with a scheme (`https://...`, `data:...`), protocol-relative URLs (`//cdn.example.com/...`) and same-document references (`#id`) are left as-is.

Bun fails the build when a script, a stylesheet, an `<img>`, `<video>`, `<audio>` or `<source>` URL, an icon, a manifest or a preload names a file that does not exist. These URLs are optional: `<input type="image">`, SVG `<image>`, `imagesrcset`, the `<meta>` images, and every icon `rel` other than `icon` and `apple-touch-icon`. For an optional URL, Bun looks for the file on disk. If the file is not there, Bun leaves the URL as written and `bun build` prints a warning.

Bun leaves these references as written: `<iframe src>`, `<a href>`, `<object data>`, `<embed src>`, `<track src>`, SVG `<use href>`, `url()` in a `style="..."` attribute or a `<style>` block, `import` specifiers in an inline `<script>` block, and everything inside `<noscript>`.
