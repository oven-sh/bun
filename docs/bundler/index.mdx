...
| `"linked"`   | A separate `*.js.map` file is created alongside each `*.js` bundle using a `//# sourceMappingURL` comment to link the two. You can customize the base URL in this comment with `publicPath` in `Bun.build` or `--public-path` on the CLI.<br/><br/>`js<br/>// <bundled code here><br/><br/>//# sourceMappingURL=bundle.js.map<br/>`                                                             |
...
In `Bun.build`, `sourcemap: true` means `"linked"` when `outdir` is set and `"inline"` when it is not. `sourcemap: false` means `"none"`. On the CLI, `--sourcemap` with no value means `linked`.

On the CLI, a `linked` or `external` sourcemap is a separate file. Set `--outdir` so that Bun has a directory to write it to.

When `Bun.build` runs without `outdir`, it does not write `"linked"` and `"external"` sourcemaps to disk. Each one is a `BuildArtifact` in `outputs` with `kind: "sourcemap"`, and it is also the `sourcemap` property of the artifact it belongs to.
...
Bun adds the banner to every JavaScript output file (entry points and chunks). CSS and HTML output files do not get it.
...
Bun adds the footer to every JavaScript output file (entry points and chunks). CSS and HTML output files do not get it.
...
interface BuildConfig {
  ...
  sourcemap?: "none" | "linked" | "inline" | "external" | boolean; // default: "none", true -> "linked" with outdir, "inline" without
  /**
   ...
   *
   * Added to every JavaScript output (entry points and chunks), not to CSS or HTML outputs.
   */
  banner?: string;
  /**
