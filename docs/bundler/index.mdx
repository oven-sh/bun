...
Nothing folds into the chunk of an entrypoint whose name has no `[hash]` (the default `naming`). Whatever loads that file can add a query string to its URL (`index.js?v=123`), and an `import "./index.js"` in another chunk would then run the file a second time. The shared code goes into one hashed chunk next to the entrypoint, and that chunk runs before the entrypoint's own file. A module that only this entrypoint imports, and that must run before some of the shared code, goes into the same chunk, so that import order is kept; `import.meta.url` and `import.meta.dir` in such a module then name that chunk. Modules that only declare functions, classes and literals do not count, and modules that come after the shared code stay where they are. A module that imports its way back to the entrypoint cannot move, and neither can the ones after it. Entrypoints that an HTML file loads, and entrypoints with `[hash]` in `naming.entry`, still take the fold.
