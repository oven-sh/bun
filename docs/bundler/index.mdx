...
An ES module that the bundler wraps in a lazy `__esm` closure is not affected. The bundler wraps an ES module that is `require()`d, an ES module that is `import()`ed without code splitting, and the modules those import. It hoists their declarations out of the closure as `var`, so a read before initialization still gets `undefined`. A CommonJS module keeps the declared kind inside its `__commonJS` wrapper, as in Node.
