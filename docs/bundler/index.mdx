...
Each entry is an esbuild-style target string: a browser name plus version (`chrome100`, `safari16.4`, `firefox115`, `edge110`, `ios16`, `opera100`), an ECMAScript year (`es2020`), or `esnext`. If no entry names a browser (for example only `esnext`), CSS is not downleveled at all.
