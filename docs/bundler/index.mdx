...
| any of the above from an importer that is not an ES module by type, when `dep` assigns both `exports.__esModule` and `exports.default` | `default` is `exports.default`, as `bun run` gives it: static imports keep the CommonJS wrapper, a split `import()` unwraps it |
