...
Chunks run the modules in the same order as the unbundled code. An ESM `import` runs before the code of the module that contains it, so a module that an entrypoint runs before a shared chunk cannot stay in the entrypoint's own output: it becomes a chunk that the entrypoint imports first. Only modules with top-level side effects need this. When two entrypoints run two modules of one shared chunk in opposite orders, the modules get a chunk each, so each entrypoint keeps its order. esbuild keeps no order across chunks ([evanw/esbuild#399](https://github.com/evanw/esbuild/issues/399)).
