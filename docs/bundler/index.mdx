...
A build fails with `Refusing to overwrite input file` when an output path is also one of the build's input files, for example `outdir: "."` with a `.js` entry point. This check covers the files that the build reads from disk. It does not cover a file on disk at the path of an in-memory [`files`](#files) entry.
