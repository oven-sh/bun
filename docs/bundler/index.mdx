...
| `.css`                                                | CSS files imported from JavaScript are bundled into one `.css` file per entrypoint, in import order. See [Bundler > CSS](/bundler/css#importing-css-from-javascript) for the ordering rules and how `import()`, `splitting`, and multiple entrypoints are handled.                                                          |
