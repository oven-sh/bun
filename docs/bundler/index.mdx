...
└── out
    ├── entry-a.js
    ├── entry-b.js
    └── chunk-dqmx6gc8w41kz.js
```

The generated `chunk-dqmx6gc8w41kz.js` file contains the shared code. To avoid collisions, the file name includes a content hash by default. The `bun build` CLI names this chunk `entry-a-t268ez5gm0q3v.js` instead of `chunk-<hash>.js`. Customize this with [`naming`](#naming).
