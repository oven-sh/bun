...
Resolve modules from the path of the symlink itself instead of the real path it points to, mirroring Node.js' `--preserve-symlinks` flag. Imports inside a symlinked file (for example a shared source folder linked into a project) resolve relative to the link's location, so the `node_modules` of the consuming project are found. Defaults to `false`.
...
<Tabs>
  ...
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    bun build ./index.ts --outdir ./out --preserve-symlinks
    ```
  </Tab>
</Tabs>
...
interface BuildConfig {
  ...
  /**
   * Resolve modules from the path of the symlink itself rather than the
   * real path it points to, mirroring Node.js' `--preserve-symlinks` flag.
   *
   * Equivalent to `--preserve-symlinks` in `bun build`.
   *
   * @default false
   */
