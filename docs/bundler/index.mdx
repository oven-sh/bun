...
| `.css`                                                | CSS files imported from JavaScript are bundled into one `.css` file per entrypoint, in the order a depth-first walk of the module graph reaches them. See [Bundler > CSS](/bundler/css#importing-css-from-javascript) for the ordering rules and how `import()`, `splitting`, and multiple entrypoints are handled.         |
...
Code splitting applies to JavaScript only. Each entrypoint's CSS output still contains all the CSS its module graph imports, including CSS behind `import()`, and each `import()` chunk that imports CSS also gets a CSS file of its own. See [CSS > With code splitting](/bundler/css#with-code-splitting).
