...
An entrypoint that another entrypoint imports keeps its own code in its own output file, and the other entrypoint imports from that file by the export names it has. So `import.meta.main`, `import.meta.path` and the other `import.meta` fields in the imported entrypoint describe its output file, and an `if (import.meta.main)` block in it runs only when that file is the process entry. The exception is an importer that needs more than the entrypoint's exports, for example `import * as ns` used as a value, or an entrypoint that is a CommonJS module: the entrypoint's code then moves to a shared chunk and its output file re-exports it.
