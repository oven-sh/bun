...
A list of import paths to **never** externalize. Defaults to `[]`.

`internal` takes precedence over both `external` and `packages: "external"`. This is the inverse of `external` — it's most useful combined with `packages: "external"` to keep specific dependencies inside the bundle while externalizing everything else.
...
await Bun.build({
  ...
  outdir: './out',
  packages: 'external',
  internal: ['typeorm'], // bundle typeorm, externalize every other package
});
```

```bash terminal icon="terminal"
bun build ./index.tsx --outdir ./out --packages external --internal typeorm
```

Wildcards are supported:

```bash terminal icon="terminal"
bun build ./index.tsx --outdir ./out --packages external --internal 'aws-sdk/*'
```
