...
The generated `chunk-3vkq0m8tzp.js` file contains the shared code. To avoid collisions, the file name includes a content hash by default. The `bun build` CLI names this chunk `entry-a-k2n8w0xq4d.js` instead of `chunk-<hash>.js`. Customize this with [`naming`](#naming).
