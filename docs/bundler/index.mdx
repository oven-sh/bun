...
Nothing folds into the chunk of an entrypoint whose name has no `[hash]` (the default `naming`). Whatever loads that file can add a query string to its URL (`index.js?v=123`), and an `import "./index.js"` in another chunk would then run the file a second time. The shared code goes into one hashed chunk next to the entrypoint. If that code runs something when it loads, the other modules that only this entrypoint imports go into the same chunk, so that all of them still run in import order; `import.meta.url` and `import.meta.dir` in those modules then name that chunk. A module that imports its way back to the entrypoint stays in the entrypoint's chunk. Entrypoints that an HTML file loads, and entrypoints with `[hash]` in `naming.entry`, still take the fold.
