...
Either way, `require()` of an ES module returns what it returns in Node.js and in Bun's runtime: a copy of the module namespace object with `__esModule: true` added, or, when the module exports the name `module.exports` (`export { EventEmitter as "module.exports" }`), the value of that export. The polyfills that `target: "browser"` uses for Node.js builtins export that name, so `require("events")` is the `EventEmitter` class and `require("path")` is the same object as `import path from "path"`.
