...
The generated sourcemap maps to the files the bundler read. If an input file is itself generated code with its own `//# sourceMappingURL` comment (inline or a `.map` file next to it), that input sourcemap is not read, so the output map points at the generated file rather than at its original source.
