...
Either way, `require()` of an ES module returns what it returns in Node.js and in Bun's runtime: a copy of the module namespace object with `__esModule: true` added, or, when the module exports the name `module.exports` (`export { EventEmitter as "module.exports" }`), the value of that export. A polyfill that `target: "browser"` substitutes for a Node.js builtin exports that name when the builtin's `module.exports` is more than its set of named exports, so `require("events")` is the `EventEmitter` class, `require("assert")` is callable, and `require("path")` is the same object as `import path from "path"`.
