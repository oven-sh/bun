...
An entry that starts with `./`, `../` or `/` names a single file instead of a package. It is resolved from the current working directory. As in esbuild, a relative import of that file is rewritten to be relative to `outdir` (the directory of `outfile`, or the current working directory when the bundle is not written to disk). An import that spells out the absolute path is left as written.
