...
The module keeps every export as soon as one use can't be followed: the namespace is passed, returned, stored, spread or iterated (`() => import(x)`, `fn(ns)`, `{ ...ns }`, `Object.keys(ns)`), accessed with a computed key or optional chaining, handed to a non-arrow `.then` callback, reached via `import * as ns` elsewhere, or read under a direct `eval`. CommonJS targets always keep everything. A `sideEffects: false` module loaded only by a bare `import(x);` contributes nothing to the bundle, like a bare `import "x"` of it would.
