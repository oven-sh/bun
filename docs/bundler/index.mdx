...
- `[hash]` - A hash of the bundle contents: 8 lowercase alphanumeric characters. `[hash9]` through `[hash13]` print more of the same hash for apps with thousands of chunks, where two chunks sharing an 8-character name (a build error) becomes plausible; `[hash13]` is the full 64-bit hash.
