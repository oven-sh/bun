...
Set this to `false` to keep the declared kind. A module the bundler merges into the output scope then throws the same `ReferenceError: Cannot access 'x' before initialization` as the unbundled code.

A module that the bundler wraps in a lazy closure is not affected. The bundler wraps a module that is `require()`d, a module that is `import()`ed without code splitting, and the modules those import. It hoists their declarations out of the closure as `var`, so a read before initialization still gets `undefined`.
