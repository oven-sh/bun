...
`mangleProps` renames every property whose name matches a regex to a short name, consistently across the bundle. It is off unless you set it, and `minify: true` does not turn it on. Renaming is unsafe for any property that code outside the bundle reads, so pick a pattern that matches only names you control:
...
<Tabs>
  <Tab title="JavaScript">
    ...
    await Bun.build({
      ...
      outdir: './out',
      minify: {
        mangleProps: /_$/, // rename foo_, bar_, ...
      },
    })
    ```
  </Tab>
  <Tab title="CLI">
    ```bash terminal icon="terminal"
    bun build ./index.tsx --outdir ./out --mangle-props '_$'
    ```
  </Tab>
</Tabs>

`mangleProps` also accepts an object with `include`, `exclude`, `reserved`, `quoted`, and `cache`, and the build result carries the assigned names as `mangleCache`. See [property mangling](/bundler/minifier#property-mangling) for the rules and the options.
...
interface BuildOutput {
  ...
  mangleCache?: Record<string, string | false>; // only when minify.mangleProps is set
}
...
interface BuildConfig {
  ...
  minify?:
    ...
    | {
        ...
        identifiers?: boolean;
        /**
         * Rename the properties whose name matches this RegExp to short names.
         * Off unless set; `minify: true` does not enable it. Unsafe for
         * properties that code outside the bundle reads.
         * See /bundler/minifier#property-mangling.
         */
...
interface ManglePropsOptions {
  include: RegExp; // property names to rename
  exclude?: RegExp; // matching names keep their name, CLI: --reserve-props
  reserved?: string[]; // exact names that keep their name
  quoted?: boolean; // default false, also rename x["name"] and { "name": 1 }, CLI: --mangle-quoted
  cache?: Record<string, string | false>; // names from an earlier build's mangleCache; false keeps the name
}
