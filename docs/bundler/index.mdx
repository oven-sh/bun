...
When a function returns the same primitive value on every call, the bundler replaces each call of the function with that value. The value can come from [`define`](#define) or from [`feature()`](#features), so a small helper function works like a build-time constant.
...
- The body is empty, or it has only `return` and `if` statements and always returns the same `true`, `false`, `null`, `undefined`, number, or string of at most 64 bytes.
...
The bundler does not replace `new f()`, `f.call()`, `f?.()`, a tagged template, or a call in the argument of `import()` or `require()`. A call above the declaration is replaced only for a top-level function, in a file that has `import` or `export` syntax.
...
- A condition calls the import, with no argument or with literal arguments only. A condition is the test of `if` or `?:`, or an operand of `&&`, `||`, `??` or `!`. When one import of an `import` statement qualifies, the bundler also replaces the other calls of the imports of that statement.
- The export is a top-level function declaration in an ES module. The bundler does not replace a call of `export const f = () => ...`, because the call can throw before the file of `f` runs. It does not replace a CommonJS export.
...
- The bundler does not replace `ns.isDev()` on `import * as ns`, an import between files that import each other, or an import that an `onResolve` plugin resolves.

The bundler parses a file two times when a condition in it calls an import in this way, because it must parse the imported file first.
