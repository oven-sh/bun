...
- `input[src]`
- `link[href]` with `rel="stylesheet"`, `rel="manifest"`, an icon `rel` (`icon`, `shortcut icon`, `apple-touch-icon`, `apple-touch-icon-precomposed`, `apple-touch-startup-image`, `mask-icon`), `as="style|font|image|video|audio|worker"`, or `type="font/..."`
- `link[imagesrcset]` (each candidate)
- `meta[content]` with `property="og:image|og:image:url|og:image:secure_url|og:audio|og:audio:secure_url|og:video|og:video:secure_url"` or `name="twitter:image|msapplication-TileImage|msapplication-config|msapplication-*logo"`
- `object[data]`
- `script[src]`
- `source[src]`
