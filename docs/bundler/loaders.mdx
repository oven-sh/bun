...
During bundling, Bun combines the CSS files that an entry point imports into one `.css` file per entry point, in the order they are imported. [Importing CSS from JavaScript](/bundler/css#importing-css-from-javascript) describes the exact order and what happens with `import()`, code splitting, and multiple entry points.
