...
`bun-types` does not declare a `*.md` module, because frameworks such as Astro declare their own `*.md` module and a second declaration would override their default export. To type the import in a plain Bun project, add a `.d.ts` file with:
