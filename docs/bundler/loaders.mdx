...
- `meta[content]` with `property="og:image|og:image:url|og:image:secure_url|og:audio|og:audio:secure_url|og:video|og:video:secure_url"` or `name="twitter:image|msapplication-TileImage|msapplication-*logo"`
- `object[data]`
- `script[src]`
- `source[src]`
