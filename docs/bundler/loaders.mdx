...
`.js` `.cjs` `.mjs` `.mts` `.cts` `.ts` `.tsx` `.jsx` `.css` `.json` `.jsonc` `.json5` `.toml` `.yaml` `.yml` `.xml` `.txt` `.md` `.markdown` `.wasm` `.node` `.html` `.sh`
...
JSON5 files can be directly imported. Bun parses them with its native [JSON5 parser](/runtime/json5). JSON5 is a superset of JSON that adds comments, trailing commas, unquoted keys, and single-quoted strings.
...
During bundling, the parsed JSON5 is inlined into the bundle as a JavaScript object, identical to the `json` loader.
...
Markdown files can be directly imported. Bun renders them to HTML with [`Bun.markdown`](/runtime/markdown), and the import resolves to the rendered HTML as a string.

```js
import html from "./README.md";
console.log(html); // => "<h1>My project</h1>\n..."
...
`bun-types` does not declare a `*.md` module, because frameworks such as Astro declare their own `*.md` module and a second declaration would override their default export. To type the import in a plain Bun project, add a `.d.ts` file with the following, plus the same block for `"*.markdown"` if you use that extension:
...
During bundling, the rendered HTML is inlined into the bundle as a string.

```js
var README_default = `<h1>My project</h1>
<p>Some <strong>bold</strong> text.</p>
`;
```

If a `.md` file is passed as an entrypoint, it is converted to a `.js` module that `export default`s the rendered HTML.
