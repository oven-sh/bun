...
Compiles Markdown, JSX, and embedded JavaScript expressions into a module whose default export is a component. Leading
`import` and named `export` declarations are preserved. A leading YAML frontmatter block becomes a named `frontmatter`
export. Tables, strikethrough, and task lists are enabled by default.
...
<Note>
  This compiler currently supports a subset of MDX syntax. Default-export layouts and import/export declarations after
  Markdown content are not supported. Its expression scanner does not support regular expression literals containing
  braces. Documents using these features require an external MDX compiler.
</Note>
