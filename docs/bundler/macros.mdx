...
Bun's transpiler must be able to serialize the result of the macro to inline it into the AST. All JSON-compatible data structures are supported, plus `undefined` and `BigInt` (inlined as a BigInt literal):
