...
Macros run during the transpiler's visiting phase, after the file is parsed into an AST. They run in the order their calls appear in the file; a macro module is not loaded until the transpiler reaches a call to one of its exports. The transpiler waits for each macro to finish before continuing, and awaits any Promise a macro returns, so macros may be `async` and macro modules may use top-level `await`.

All macros in a process run in a single dedicated JavaScript VM on its own thread, separate from your application: they do not share globals or module state with the program being run or bundled, and `process.exit()` inside a macro is an error. Bun's bundler parses files on several threads, and each of them hands its macro calls to that VM; while one macro is awaiting I/O, macros from other files keep running there. A macro module cannot itself invoke macros.
...
Bun's transpiler must be able to serialize the result of the macro to inline it into the AST. All JSON-compatible data structures are supported, plus `undefined`, `RegExp` (inlined as a regular expression literal) and `BigInt` (inlined as a BigInt literal):
