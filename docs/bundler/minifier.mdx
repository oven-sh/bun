...
<ParamField path="--minify-whitespace" type="boolean">
  ...
  - Use `--emit-dce-annotations` to re-emit DCE annotations, disabled by default when `--minify-whitespace` is active
</ParamField>
...
<ParamField path="--minify-syntax" type="boolean">
  ...
  - Use `--keep-names` to preserve original function and class names
</ParamField>
