...
<ParamField path="--minify-whitespace" type="boolean">
  Removes all unnecessary whitespace, newlines, and formatting from the output. Use <code>--emit-dce-annotations</code> to re-enable DCE annotations, which are disabled by default when this flag is active.
</ParamField>

### Syntax minification

<ParamField path="--minify-syntax" type="boolean">
  Rewrites JavaScript syntax to shorter equivalent forms and performs constant folding, dead code elimination, and other optimizations. Use <code>--keep-names</code> to preserve original function and class names.
</ParamField>

### Identifier minification

<ParamField path="--minify-identifiers" type="boolean">
  Renames local variables and function names to shorter identifiers using frequency-based optimization. Use <code>--keep-names</code> to preserve original function and class names.
</ParamField>
...
When minifying **identifiers**, **syntax**, or **both**, you may want to preserve original function and class names for debugging purposes. Use the `--keep-names` flag:
...
- **`--minify-identifiers`:** Maximum size reduction

> **Note:** Use `--keep-names` for better stack traces when combining `--minify-syntax` or `--minify-identifiers`.
