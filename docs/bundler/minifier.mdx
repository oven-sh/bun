...
Converts template literals with no substitutions to regular strings. A template literal with a line break stays a template literal, because the raw line break is shorter than `\n`.
...
Chooses the optimal quote character to minimize escapes. A string with `\n` becomes a template literal with a raw line break. Without `--minify-syntax`, `\n` stays escaped in a quoted string.
