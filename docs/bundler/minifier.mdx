...
With `mangleQuoted` to also mangle quoted property names and string literal accesses:
...
By default, quoted property names and bracket notation accesses are not mangled:

```ts Input
const obj = {
  "foo_": 1,  // Quoted - not mangled by default
  bar_: 2,    // Not quoted - mangled
};
console.log(obj["foo_"], obj.bar_);
```

```js Output (without mangleQuoted)
const obj = {
  "foo_": 1,
  a: 2,
};
console.log(obj["foo_"], obj.a);
```

With `mangleQuoted: true`, quoted properties are also mangled:

```js Output (with mangleQuoted)
const obj = {
  a: 1,
  b: 2,
};
