...
Removes side-effect-free expressions from comma sequences. An anonymous function or class keeps one `0`, because `y = () => {}` names the function `"y"` and `y = (0, () => {})` does not. Every fold that returns one operand (`true && x`, `null ?? x`, `true ? x : z`, `[x][0]`) leaves the same `(0, x)` for such a value.
