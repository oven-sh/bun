...
- External packages are not part of the bundle, so their own code keeps its property names. Bun still renames your accesses to their properties when the names match (`pkg.name_`, `const { name_ } = pkg`), so exclude or reserve those names. A member of a namespace import (`import * as ns from "pkg"; ns.name_`) is an export name and keeps its spelling.
...
- Imported JSON, TOML and CSS module objects keep their keys. Bun reserves those keys so no generated name collides with them, but an access such as `data.name_` is still renamed when the name matches.
- A CommonJS module that assigns a matching name to `exports` (`exports.name_ = 1`) stays a CommonJS module in the bundle, and `exports.name_` is renamed like every other property. `import { name_ } from "./lib.cjs"` binds to the export name and does not follow the renaming, so reserve export names that you import by name.
- With `emitDecoratorMetadata`, a `design:type` reference to a type inside a TypeScript namespace keeps the original member name.
