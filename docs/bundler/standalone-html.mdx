...
| `<track src="./en.vtt">`                                                                           | `<track src="data:text/vtt;base64,...">`                                    |
...
| `<link rel="preload" as="font" href="./font.woff2">`, `<link rel="modulepreload" href="./app.js">` | Removed: the file it points at is already inline                            |
...
- `<iframe src>`, `<a href>`, `<link rel="prefetch">`, and any other reference from one HTML page to another (`<object data="./page.html">`)
- SVG `<use href="./icons.svg#menu">`: browsers do not load a `data:` URL from `<use>`. Copy the `<symbol>` into the page and reference it as `<use href="#menu">`.
- `<link rel="preload" as="fetch">` and other preloads of URLs that are read at runtime
...
- A `<track>`, `<object>`, `<embed>`, `<input>`, SVG or `<meta>` URL whose file is not on disk. Bun prints a warning for it.
