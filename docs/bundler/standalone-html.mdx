...
Bun inlines every local asset it finds in your HTML: anything with a relative path, of any file type, is embedded into the output file. [What is not inlined](#what-is-not-inlined) lists the exceptions.
...
| `<img src="./icons.svg#menu">`                   | `<img src="data:image/svg+xml;base64,...#menu">`                            |
...
| `<input type="image" src="./go.png">`            | `<input type="image" src="data:image/png;base64,...">`                      |
| SVG `<image href="./photo.jpg">`                 | `<image href="data:image/jpeg;base64,...">`                                 |
| `<link rel="preload" as="image" href="./a.png">` | Removed: the file is inline in the page                                     |
| `<link rel="modulepreload" href="./app.js">`     | Removed: the script is inline in the page                                   |
...
Bun leaves these references as written. If one of them points at a local file, put that file next to the output:

- `<iframe src>`, `<a href>`, `<object data>`, `<embed src>` and `<link rel="prefetch">`
- `<track src>` and SVG `<use href="./icons.svg#menu">`. Browsers do not load a `data:` URL from `<use>`. Copy the `<symbol>` into the page and use `<use href="#menu">`.
- `<meta property="og:image" content>` and the other `<meta>` images. A crawler reads them, and a crawler needs an `https://` URL.
- `<link rel="preload" as="fetch">` and other preloads of a URL that your code reads at runtime
- `url()` in a `style="..."` attribute or a `<style>` block. Put the CSS in a `.css` file and `<link>` it.
- `import` specifiers in an inline `<script type="module">` block. Use `<script src>`.
- Everything inside `<noscript>`
- URLs that JavaScript builds at runtime, such as `fetch("./data.json")` or `new Worker(new URL("./worker.js", import.meta.url))`. Import the file (`import data from "./data.json"`) so that the bundler can see it.
- An `<input type="image">`, SVG `<image>` or icon `<link>` URL whose file is not on disk. `bun build` prints a warning for it.
