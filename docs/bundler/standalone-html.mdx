...
A page printed to stdout is one file, so `--sourcemap=linked` and `--sourcemap=external` need `--outdir` or `--outfile`. `--sourcemap=inline` works everywhere, and `--sourcemap` with no value means `inline` when the page goes to stdout.
