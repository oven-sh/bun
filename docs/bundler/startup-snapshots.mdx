...
This is experimental. It is available on macOS and Linux.
...
For a program that starts, does one job and exits, the requirement is that the job runs *after* the snapshot is restored, not while the modules are being loaded. Import everything at the top level and put the program itself in `Bun.startupSnapshot.main()`:
...
- macOS and Linux only. On other platforms `--snapshot` reports that it is unavailable.
