...
See §2.1. The source-of-truth table (118 files) is derived from scanning each file's imports against the group-B crate list plus intra-crate `crate::{virtual_machine,module_loader,console_object,event_loop,rare_data}` paths; Step 6.7's grep enforces zero hits over group-A. Files that become A after edits carving group-B references out (Step 6.2/6.7): `JSGlobalObject`, `CachedBytecode`, `uuid`, `webcore_types` (minus S3 block), `BuildMessage`, `ResolveMessage`, `error`. Files that are structurally B despite small import count: `AbortSignal` (embeds `EventLoopTimer`, calls `VirtualMachine::timer_*`), `FetchHeaders.to_uws_response`, `SystemError` uws-helper.
