...
<Note>
  Use Bun with frameworks supported by Vercel, like Next.js, Express, Hono, or Nitro. Vercel also has an experimental
  Bun framework preset (a `server.ts` entrypoint plus a `bun.lock`, with `bunVersion` set) that deploys a plain
  `Bun.serve()` server as a Vercel Function.
</Note>
