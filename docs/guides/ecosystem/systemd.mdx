...
If your application starts a webserver, non-`root` users cannot listen on ports 80 or 443 by default. To allow Bun to listen on these ports when run by a non-`root` user, use the following command (this requires `sudo` permissions). This step isn't necessary when running as `root`.

```bash terminal icon="terminal"
setcap CAP_NET_BIND_SERVICE=+eip /home/YOUR_USER/.bun/bin/bun
```

The capability is attached to the `bun` binary itself and is lost when the binary is replaced, for example by `bun upgrade`, so re-run the command after upgrading. Alternatively, add `AmbientCapabilities=CAP_NET_BIND_SERVICE` to the `[Service]` section of the service file instead.
