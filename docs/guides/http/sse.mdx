...
In Bun, `new Response` accepts an async generator function directly. Each `yield` flushes a chunk to the client, and if the client disconnects, the generator's `finally` block runs so you can clean up.
