...
`bun install` does not take proxy settings (`HTTP_PROXY`, `HTTPS_PROXY`, `NO_PROXY`), `NODE_TLS_REJECT_UNAUTHORIZED`, or `GIT_*` variables from these files. Set those in the environment that runs `bun install`.
