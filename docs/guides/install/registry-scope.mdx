...
Your `bunfig.toml` can reference environment variables. Bun automatically loads environment variables from `.env.local`, `.env.[NODE_ENV]`, and `.env`. See [Docs > Environment variables](/runtime/env) for more information.
...
See [Docs > Package manager](/pm) for complete documentation of Bun's package manager.
