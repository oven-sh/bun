...
To add npm dependencies to a particular workspace, `cd` to that directory and run `bun add` as you normally would. Bun detects that you are in a workspace, adds the dependency to that workspace's `package.json`, and updates the root lockfile. New workspaces use [isolated installs](/pm/isolated-installs) by default, so the package is installed into the root `node_modules/.bun` store and symlinked from the workspace's own `node_modules`; with `--linker hoisted` it is hoisted into the root `node_modules` instead.
