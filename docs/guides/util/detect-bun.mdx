...
<Note>
  In TypeScript, this is a type error unless `@types/bun` is installed. Install it with `bun add -d @types/bun`. If you're using TypeScript 6.0 or later, you also need `"types": ["bun"]` in your `compilerOptions`. See [TypeScript 6 and 7](/typescript-6) for details.
</Note>
