...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    <Tab title="macOS Installer">
      Download the universal `.pkg` installer from the [latest GitHub release](https://github.com/oven-sh/bun/releases/latest/download/bun-darwin-universal.pkg) and double-click it. It installs `bun` and `bunx` to `/usr/local/bin`, adds them to your `PATH`, and sets `BUN_INSTALL` to `~/.bun` for global packages. Works on both Apple Silicon and Intel Macs.
      ...
      rm -rf ~/.bun  # optional: global packages + completions
      ```

      The installer also appends a block marked `# bun (installed via .pkg)` to your shell configs (`~/.zshrc`, `~/.zprofile`, `~/.bash_profile`, `~/.bashrc`, and/or `~/.config/fish/config.fish`) — whichever exist, or it creates the default config for your login shell if none do. Delete that block (and any files it created) for a fully clean uninstall. Leaving it is harmless.
    </Tab>
