...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    <Tab title="macOS Installer">
      ...
      The installer also appends a block marked `# bun (installed via .pkg)` to your shell config (`~/.zshrc`, `~/.bash_profile`, or `~/.config/fish/config.fish`) — delete it if you want a fully clean uninstall. Leaving it is harmless.
    </Tab>
