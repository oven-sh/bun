...
Bun ships one x64 binary per platform. It targets the Nehalem architecture (SSE4.2 and POPCNT required); wider SIMD is dispatched at runtime when the CPU supports it.

| Platform | Intel Requirement            | AMD Requirement    |
| -------- | ---------------------------- | ------------------ |
| x64      | Nehalem (1st gen Core, 2008) | Bulldozer or newer |
| arm64    | N/A                          | N/A                |

<Note>
  Bun requires SSE4.2 on x64 and macOS 13.0 or later. Before Bun 1.4, the default x64 download required AVX2 and a
  separate `-baseline` build supported older CPUs. The `-baseline` download URLs and npm packages still resolve as
  aliases to the standard build.
</Note>
