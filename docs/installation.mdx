...
<Tabs>
  ...
  <Tab title="Windows">
    ...
    <Tab title="macOS Installer">
      ...
      The installer also appends a block marked `# bun (installed via .pkg)` to your shell configs (`~/.zshrc`, `~/.zprofile`, `~/.bash_profile`, `~/.bashrc`, and/or `~/.config/fish/config.fish`, whichever exist) — delete it from each if you want a fully clean uninstall. Leaving it is harmless.
    </Tab>
