...
<Tabs>
  <Tab title="macOS & Linux">
    ...
    <Note>
      **Linux users:** The install script needs `curl` or `wget`, and `unzip` (`sudo apt install unzip`). If `unzip` is missing, it also accepts `busybox`, `7z`, `bsdtar`, or `python3`. We recommend kernel version 5.6 or higher. Bun runs on kernels as old as 3.10 (RHEL 7) with graceful degradation of newer syscalls. Use `uname -r` to check your kernel version.
      </Note>
