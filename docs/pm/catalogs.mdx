...
To add a new dependency to the catalog (or refresh an existing entry), run `bun add` with `--catalog` (or `--catalog=<name>` for a named catalog) from the workspace package or from the root:

```bash terminal icon="terminal"
bun add react --catalog
```

Bun writes the resolved version to the catalog in the root `package.json` and `"catalog:"` to the `package.json` you ran the command in. See [`bun add --catalog`](/pm/cli/add#--catalog).

The catalog is only consulted when `--catalog` is passed: `bun add react` writes a concrete range to the current `package.json` even if `react` is already in the catalog (Bun has no equivalent of pnpm's `catalogMode`).
