...
- Only stable releases are candidates. When no newer release is safe, the highest safe older release the ranges accept is installed instead and marked `(downgrade)`. `no published version fixes:` lists versions for which no safe release exists in either direction, each with the advisories that cover it; the section ends with one `bun audit fix --ignore ...` command carrying every one of those advisories, which silences them all (the other way out is replacing the package). `manifest could not be fetched:` lists versions whose registry request failed, and nothing is installed for them.
