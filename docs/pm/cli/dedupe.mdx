...
- `removed`: one object per row of the text output, in the same order. `name` and `version` identify the version Bun removes (or would remove); `resolvedTo` lists the locked version(s) its dependents now use. It is empty (`[]`) when nothing depended on the removed version anymore.
