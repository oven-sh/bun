...
- `installed`: the number of packages the run installed. `null` with `--check`, `--dry-run` and `--lockfile-only`, and whenever no install ran.

When there is nothing to remove, `removed` is `[]`. The exit codes are the same as without `--json`: `--check` exits `1` when `removed` is not empty, `--dry-run` and a real run exit `0`. Errors are still reported on stderr. For example, to list the duplicate versions one per line:
