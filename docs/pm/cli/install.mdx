...
Bun copies the workspace packages list and catalogs to the `workspaces` field in `package.json`:
...
Bun copies the following pnpm configuration into the fields it reads itself:

- **Overrides**: Copied from `pnpm.overrides` in `package.json` and from `overrides` in `pnpm-workspace.yaml` to root-level `overrides` in `package.json`
- **Patched Dependencies**: Copied from `pnpm.patchedDependencies` in `package.json` and from `patchedDependencies` in `pnpm-workspace.yaml` to root-level `patchedDependencies` in `package.json`

The `pnpm` section of `package.json` is left as it is, so the repository keeps working for anyone still running `pnpm install` on it.
...
Once you no longer need pnpm, you can remove `pnpm-lock.yaml`, `pnpm-workspace.yaml`, and the `pnpm` section of `package.json`. Bun reads none of them after the migration, so changes made to them later are not picked up.
