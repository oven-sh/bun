...
```bash terminal icon="terminal"
# First: Install packages normally to populate the cache
bun install

# Later: Install from cache only (no network requests)
bun install --offline
```
...
You can also enable offline mode using the `BUN_CONFIG_OFFLINE` environment variable:

```bash terminal icon="terminal"
BUN_CONFIG_OFFLINE=1 bun install
```
