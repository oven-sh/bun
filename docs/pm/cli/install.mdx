...
To protect against supply chain attacks where malicious packages are quickly published, Bun enforces a minimum age requirement for npm packages. Package versions published more recently than the specified threshold will be filtered out during installation.

For new projects (lockfile `configVersion` ≥ 2) this defaults to **two days**. Projects created with older Bun versions default to disabled. Pass `0` to disable the filter.
...
# minimum age config
minimumReleaseAge = "2d" # or a number of seconds; 0 disables
minimumReleaseAgeExcludes = ["@types/node", "typescript"]
```
