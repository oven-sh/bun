...
`--offline` does not install an optional dependency that is not cached, or the dependencies of that package. A package that an installed package requires is still an error.
