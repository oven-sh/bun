...
Use the official [`oven-sh/setup-bun`](https://github.com/oven-sh/setup-bun) action with the GitLab `.gitlab-ci.yml` syntax to install `bun` in a GitLab pipeline:
...
Before Bun 1.2, the lockfile was binary and called `bun.lock`. To upgrade an old lockfile to the new format, run `bun install --save-text-lockfile --frozen-lockfile --lockfile-only`, then delete `bun.lock`.
