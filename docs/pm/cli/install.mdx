...
| `BUN_CONFIG_HTTP_RETRY_COUNT`      | How many times a failed request is retried (default 5)                                                                                       |
| `BUN_CONFIG_HTTP_RETRY_BACKOFF`    | Base delay in ms between retries; doubles per attempt, capped at 40×, ±20% jitter, and a `Retry-After` header wins when longer (default 250) |
