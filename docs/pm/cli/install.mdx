...
- `patch:` dependencies: patches in your project (for example `.yarn/patches`) are added to `patchedDependencies` in `package.json` (for npm, git and tarball packages; Bun does not patch `file:` folders or workspaces, so those patches are reported and skipped), and `patch:` ranges in `package.json` are rewritten to the plain range they patch; Yarn's builtin compatibility patches are dropped, and a package with several `&`-joined patch files is left unpatched with a warning, since `patchedDependencies` takes one patch per package
- `file:` folders and tarballs, tarball URLs, and git / `github:` dependencies (pinned to the locked commit)
- `portal:` and `link:` dependencies, which are installed like `file:` dependencies; Bun does not support these protocols in `package.json`, so the ranges are rewritten to `file:` and a warning reports that the folder is now copied instead of linked
