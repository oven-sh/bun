...
- Preserves the `bundledDependencies` names of a package, so `bun prune` keeps the copies its tarball ships
...
- `bundledDependencies: true` is not migrated, because `pnpm-lock.yaml` does not name the packages it covers
