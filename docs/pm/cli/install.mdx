...
`package.json` disagrees with `bun.lock` when a dependency would resolve to a different package, and also when every package stays as locked but the next `bun install` would rewrite `bun.lock`: a version range written differently (`^1.0.0` changed to `>=1.0.0`), a dependency moved between `dependencies`, `devDependencies`, `optionalDependencies` and `peerDependencies`, or a changed `overrides`, `catalog`, `trustedDependencies` or `patchedDependencies` entry, like `pnpm install --frozen-lockfile` and `yarn install --immutable`. The error names the section and the `package.json` it is in. Run `bun install` and commit the updated `bun.lock` to fix it. `bun ci` and `bun install --production` run the same check, so a Dockerfile that installs with `--production` also fails on a `bun.lock` that is out of date. A bumped workspace `version` on its own is accepted. A release tool that also rewrites the ranges other workspaces declare on it needs one `bun install` before the commit. A workspace project has two exceptions, because a pruned checkout can look like this: a removed `trustedDependencies` name is accepted (`bun.lock` records one list for all workspaces), and a `trustedDependencies` or `patchedDependencies` section is only compared when `bun.lock` already records it. With the legacy binary `bun.lockb`, only resolution, `overrides` and `catalog` changes are detected.
...
An install is only certain to cache the packages that it places in `node_modules`. To warm the cache for an `--offline` install, run an install on the same platform that places at least the same packages. An install with `--production` or `--omit` does not cache the groups it leaves out.
