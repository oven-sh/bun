...
Pass `--json` to get the same information as a JSON array, for scripts and CI checks. Nothing else is written to stdout, so the output can be piped straight into `jq` or `JSON.parse`:
...
Each object is one row of the table:

- `name`: the dependency's name in `package.json`
- `current`, `update`, `latest`: the three version columns, as strings. With `minimumReleaseAge` configured these are the versions the table shows, that is, versions that are too new are left out.
...
- `workspace`: the name of the workspace package that declares the dependency
- `catalog`: `null` for a normal dependency. For a [catalog](/pm/catalogs) dependency, the catalog's name, or `"default"` for the unnamed catalog. Unlike the table, a catalog entry used by several workspaces produces one object per workspace.

Objects are grouped by `type` in the same order as the table. The array is empty (`[]`) when nothing is outdated. Dependency filters, `--filter` and `-r` work as usual. For example, to fail a CI step when a dependency can be updated without leaving its `package.json` range:
