...
To check several dependencies, pass more than one name or pattern. `bun outdated` matches each pattern against the dependency name as written in `package.json`. The table lists a dependency when at least one pattern matches it. A pattern that starts with `!` removes the dependencies it matches:

```sh terminal icon="terminal"
bun outdated typescript 'eslint*' '!eslint-plugin-sonarjs'
```
