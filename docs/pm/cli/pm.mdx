...
Pass `--json` to get the same list as a JSON object, for scripts and CI checks. Nothing else is written to stdout, so the output can be piped into `jq` or `JSON.parse`. `--json` combines with `--all` and `--trusted`:
...
```json
{
  ...
  "dependencies": [
    { "name": "react", "version": "18.2.0", "path": "node_modules/react" },
    { "name": "react", "version": "17.0.2", "path": "node_modules/react-17" },
    { "name": "typescript", "version": "5.0.4", "path": "node_modules/typescript" },
    { "name": "ui", "version": "workspace:packages/ui", "path": "node_modules/ui" }
  ]
}
```
...
- `dependencies`: one object per listed package, in the order of the text output:
  - `name`: the name of the installed package. For an `npm:` alias such as `"react-17": "npm:react@17"` this is the real package name (`react`), not the alias.
  ...
  - `path`: the folder the package is installed in, relative to `path`, always with forward slashes. An aliased package keeps its alias here (`node_modules/react-17`).

With `--all`, a package that has its own nested `node_modules` folder also has a `dependencies` array with the same entry shape, recursively. A package without one has no `dependencies` key. With `--all --trusted`, `dependencies` is the flat list of trusted packages the text output prints, each with the folder it is installed in.
...
```json
{
  ...
  "path": "/path/to/project",
  "dependencies": [
    { "name": "loose-envify", "version": "1.4.0", "path": "node_modules/loose-envify" },
    {
      "name": "react",
      "version": "18.2.0",
      "path": "node_modules/react",
      "dependencies": [
        { "name": "scheduler", "version": "0.23.0", "path": "node_modules/react/node_modules/scheduler" }
      ]
    },
    { "name": "typescript", "version": "5.0.4", "path": "node_modules/typescript" }
  ]
...
`dependencies` is `[]` when nothing is installed, or when `--trusted` matches nothing. A missing lockfile is still reported on stderr with exit code `1`, and nothing is written to stdout.
