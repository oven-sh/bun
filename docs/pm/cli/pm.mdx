...
- `1 + 1` vs `2`, `'a'` vs `"a"`, `!0` vs `true`, a re-wrapped call or a whole-file reformat are not changes. Lines folded away this way are counted in the file header (`3 equivalent hidden`), and a file with nothing left reads `formatting only`.
