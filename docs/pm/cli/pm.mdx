...
`--json` prints an array with one object, the same shape as `npm pack --json`. Nothing else is written to stdout, so the output can be piped into `jq` or `JSON.parse`. Errors still go to stderr, and the exit code does not change.
...
Pass `--json` to get the list as a JSON object, for scripts and CI checks. The shape is the one of `pnpm ls --json`: the root package, then its dependencies grouped by the `package.json` section they are declared in. Nothing else is written to stdout, so the output can be piped into `jq` or `JSON.parse`. `--json` combines with `--all` and `--trusted`:
...
```json
{
  ...
  "peerDependencies": {},
  "workspaces": {
    "ui": { "from": "ui", "version": "workspace:packages/ui", "path": "/path/to/project/node_modules/ui" }
  }
}
```
...
- `dependencies`, `devDependencies`, `optionalDependencies`, `peerDependencies`: the root's dependencies from that `package.json` section, keyed by the name they are installed under. `workspaces` lists the workspace packages. All five groups are always present, `{}` when empty. A package the root declares in two sections is listed in both. Each entry has:
  ...
  - `path`: the absolute path of the folder the package is installed in

With `--all`, the document also has `transitiveDependencies`: every other installed package, one entry per `node_modules` folder it is installed in, sorted by name. It is an array because the same name can be installed in more than one folder, for example a nested `node_modules` that holds a second version. With `--trusted`, every group lists only the trusted packages.
...
```json
{
  ...
  "transitiveDependencies": [
    { "from": "loose-envify", "version": "1.4.0", "path": "/path/to/project/node_modules/loose-envify" },
    { "from": "scheduler", "version": "0.23.0", "path": "/path/to/project/node_modules/react/node_modules/scheduler" },
    { "from": "scheduler", "version": "0.24.0", "path": "/path/to/project/node_modules/scheduler" }
  ]
}
```

A missing lockfile is still reported on stderr with exit code `1`, and nothing is written to stdout.
