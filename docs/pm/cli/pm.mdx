...
Pass `--prod` to skip `devDependencies`, and `--json` to get a machine-readable object keyed by license, where each entry has `name`, `versions` (in semver order) and, when present, the `homepage` and `author` of the newest listed version:
