...
Pass `--dry-run` to print the directories that would be deleted without deleting them.
...
Pass `--dry-run` to run the migration without writing `bun.lock`.
...
- `--dry-run`: Print the scripts that would run and the packages that would be trusted, without running scripts or writing `package.json` and `bun.lock`.
- `--ignore-scripts`: Add the packages to `trustedDependencies` in `package.json` without running their scripts. The next `bun install` runs them and records the trust in `bun.lock`. Only the flag does this: `ignoreScripts` in `bunfig.toml` or `.npmrc` does not stop `bun pm trust` from running the scripts you name.
...
Options:
  ...
  --dry-run            Print the new version without writing, running scripts, or tagging
...
Supports `patch`, `minor`, `major`, `premajor`, `preminor`, `prepatch`, `prerelease`, `from-git`, or specific versions like `1.2.3`. By default it creates a git commit and tag; pass `--no-git-tag-version` to skip them. Pass `--dry-run` to print the new version without writing `package.json`, running lifecycle scripts, or touching git.
...
`set`, `delete`, and `fix` accept `--dry-run`, which prints the resulting `package.json` instead of writing it.
