...
- `<package>`: The name of the package to explain, or the alias a `package.json` gives it. Supports glob patterns like `@org/*` or `*-lodash`.
...
Find the package behind an alias. With `"lodash3": "npm:lodash@^3.0.0"` in `package.json`, both `bun why lodash3` and `bun why lodash` print:
...
```txt
lodash@3.10.1
  └─ my-app@1.0.0 (requires lodash3@npm:lodash@^3.0.0)
```
