...
When `bun run --filter`, `--parallel`, or `--sequential` receives `SIGINT`, `SIGTERM`, or `SIGHUP`, Bun sends the same signal to every script that is still running. Bun starts no more scripts. When the scripts exit, Bun ends itself with that signal, even if every script exited with 0. A shell reports this as exit code `128 + signal` (130, 143, or 129). When Bun is PID 1 in a container, Bun exits with that code instead, because PID 1 does not die of such a signal.
