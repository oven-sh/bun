...
`bun install`, `bun add`, `bun remove`, `bun update` and `bun outdated` support `--filter` (pass it after the subcommand, or as `--filter=<pattern>` before it); for `bun add`/`bun remove` it selects which workspace `package.json` files are edited. You can also use it to run scripts in multiple packages at once.
