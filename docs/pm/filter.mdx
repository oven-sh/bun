...
When `bun run --filter`, `--parallel`, or `--sequential` receives `SIGINT`, `SIGTERM`, or `SIGHUP`, Bun sends the same signal to every script that is still running. Bun starts no more scripts. When the scripts exit, Bun exits with `128 + signal` (130, 143, or 129), even if every script exited with 0.
