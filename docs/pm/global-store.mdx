...
- the package has a **peer dependency whose types come from a project-installed `@types/*` package** (e.g. a library peering on `react` in a project that installs `@types/react`). TypeScript type-checks the package's declaration files at their realpath and resolves their imports by walking `node_modules` upward from there, falling back to `@types/*`; from `<cache>/links/` that walk can never reach the project's `@types` packages, so the peer's types would silently resolve as `any`. Keeping the entry project-local keeps the hidden hoisted layer — and the `@types` package — on the resolution path. A package that declares the `@types/*` package as a real dependency of its own still shares globally.
