...
The global virtual store is **opt-in**. It is only used by the isolated linker.
...
With `globalStore = false` (the default), the isolated linker materializes each package into `<project>/node_modules/.bun/` the way pnpm does — canonical paths stay inside the project and bundlers, resolvers, and scripts that canonicalize symlinks continue to see a familiar layout.
