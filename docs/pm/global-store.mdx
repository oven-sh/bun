...
<Warning>
`bun pm cache rm` deletes `<cache>/links/`. Every project that uses the global store symlinks into that directory, so each of them breaks until you run `bun install` in it again. Do not run `bun pm cache rm` to refresh stale registry metadata. Use `bun install --force` or `bun add <pkg> --force` instead. See [Cache](/pm/cli/install#cache).
</Warning>
