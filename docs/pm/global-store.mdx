...
- `bun patch <pkg>` made an **editable copy** of the package (the entry holds a `.bun-patch` file until `bun patch --commit` or a forced install builds the package again);
