...
Store entries can optionally be materialized once into a [global virtual store](/pm/global-store) at `<cache>/links/`, with `node_modules/.bun/<pkg>@<ver>` symlinked into it. Opt in with `install.globalStore = true` in bunfig.toml or `BUN_INSTALL_GLOBAL_STORE=1`. Warm installs after `rm -rf node_modules` then only create one symlink per package instead of copying every package's files again — roughly **7× faster** on a typical mid-size project. By default (`globalStore = false`), packages are materialized inside `<project>/node_modules/.bun/` so canonical paths stay in-project. See the [global store docs](/pm/global-store) for the full layout, benchmarks, and tradeoffs.

### Backend strategies

When an entry isn't using the global store, Bun materializes it under the project using one of:
