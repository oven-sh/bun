...
A `trustedDependencies` entry for a package from one of these sources applies only when your `package.json` or a
workspace declares that dependency. The one exception is a `file:` directory that another `file:` directory of yours
declares: those package.json files are yours too, so the entry applies at any depth of such a chain.
