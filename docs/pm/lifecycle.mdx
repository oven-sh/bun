...
<Note>
  The npm `dependencies` script (which npm runs after `node_modules` is changed by commands like `npm install` or
  `npm remove`, per the [npm scripts docs](https://docs.npmjs.com/cli/v10/using-npm/scripts#dependencies)) is not
  implemented in Bun and will not run.
</Note>
