...
- `preprepare`: Before `prepare` (root, `git:`, and `github:` packages only)
- `prepare`: After `postinstall`, and before the package is packed or published (root, `git:`, `github:`, and workspace packages)
- `postprepare`: After `prepare` (root, `git:`, and `github:` packages only)
