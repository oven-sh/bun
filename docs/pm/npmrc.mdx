...
The equivalent `bunfig.toml` option is [`install.lifecycle`](/runtime/bunfig#install-lifecycle):
...
Control how packages are installed in `node_modules`:

```ini .npmrc icon="npm"
# npm style (hoisted is the default)
install-strategy=hoisted

# pnpm style (symlinked)
install-strategy=linked
```

Bun also supports the yarn/pnpm `node-linker` option:
