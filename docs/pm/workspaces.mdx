...
- **Dependencies can be de-duplicated.** If `a` and `b` share a common dependency, it is _hoisted_ to the root `node_modules` directory. This saves disk space and avoids the "dependency hell" of multiple versions of a package installed at once.
