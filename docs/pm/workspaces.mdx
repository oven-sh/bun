...
cannot affect the cache or other projects. All other workspaces keep hoisting to the root as usual. The setting is recorded in the lockfile (`bun.lock` or `bun.lockb`) for the workspace, so installs from the lockfile reproduce the same layout.
If the lockfile does not record the setting yet (Bun 1.4.0 and earlier ignored it), a `--frozen-lockfile` install keeps the recorded layout and prints a note. If the lockfile records the setting and a `package.json` changes it, a `--frozen-lockfile` install fails. Run `bun install` to update the lockfile.
