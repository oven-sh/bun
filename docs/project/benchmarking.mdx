...
## Native heap profiling (pprof)

`Bun.pprof.heap` samples the allocations of Bun's native allocator and writes a [pprof](https://github.com/google/pprof) heap profile. Each sample has the JavaScript and native frames that allocated. Use it to find what holds native memory: `ArrayBuffer` contents, strings, buffers inside Bun's APIs.
...
To profile a whole run without changing the code:

```sh terminal icon="terminal"
bun --pprof-heap=heap.pb.gz script.js
```

Open the profile with any pprof viewer:

```sh terminal icon="terminal"
go tool pprof -http=: heap.pb.gz
go tool pprof -top -sample_index=alloc_space heap.pb.gz
```

[speedscope](https://www.speedscope.app) opens the same file. For continuous profiling, serve `Bun.pprof.heap.profile()` from an endpoint and point a profiler that scrapes pprof endpoints, such as Pyroscope or Parca, at it:
...
Bun.serve({
  hostname: "127.0.0.1",
  routes: {
    "/debug/pprof/heap": () => new Response(Bun.pprof.heap.profile()),
  },
  fetch: () => new Response("hello"),
});
```

A profile names the functions and files of the program, so keep the endpoint off public interfaces: bind it to `127.0.0.1` as above, or put it behind the authentication the scraper uses.

| API / flag                                  | Description                                                       |
| ------------------------------------------- | ----------------------------------------------------------------- |
| `Bun.pprof.heap.start({ sampleInterval? })` | Start profiling. `sampleInterval` is in bytes (default `524288`)  |
| `Bun.pprof.heap.profile()`                  | The profile so far as a gzipped `Uint8Array`. Profiling continues |
| `Bun.pprof.heap.stop()`                     | Stop and return the profile                                       |
| `Bun.pprof.heap.isRunning`                  | Whether a profile is running                                      |
| `--pprof-heap[=<path>]`                     | Profile the whole run and write the profile on exit               |
| `--pprof-heap-interval=<bytes>`             | The sample interval for `--pprof-heap`                            |

Notes:

- The profile has four sample types: `alloc_objects`, `alloc_space`, `inuse_objects`, `inuse_space`. `inuse_*` is what was allocated since `start()` and not freed.
- There is one profile per process. It includes Workers and Bun's own threads. Samples have a `thread` label, and a `worker` label on a Worker's JavaScript thread. Filter with `-tagfocus=thread=...`.
- The profiler's own memory is bounded: about 16 MiB at the most (65,536 distinct stacks with a million frames between them). Past that, samples of new stacks are counted in one sample with the single frame `(other stacks)`, and a function that cannot be recorded is `(truncated)`, so totals stay right. `go tool pprof -comments` shows how full it is.
- JavaScript objects are not sampled one by one. The garbage-collected heap shows up as the blocks it takes from the allocator. Use `--heap-prof` or `Bun.generateHeapSnapshot()` for objects.
- Native frames are addresses in a release build of Bun. To see function names, download the `bun-profile` build of the same version and pass it first: `go tool pprof -http=: ./bun-profile heap.pb.gz`.
- In an executable built with `bun build --compile`, and in code loaded from `--bytecode` output, a JavaScript frame has the line where its function starts, and a function that has not been named yet is `(anonymous)`.
- On macOS, memory that system frameworks allocate is not part of the profile.
- On Windows x64, samples have native frames only.
