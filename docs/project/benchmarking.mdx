...
CPU and heap profiling flags also work with `bun test`:

```sh terminal icon="terminal"
bun test --cpu-prof-md
bun test --heap-prof-md
```

<Note>
  Profiling is not supported with multiple `--parallel` workers. Run tests without `--parallel`, or use `--parallel=1`,
  to profile the test process.
</Note>
