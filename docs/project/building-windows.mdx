...
scoop install nodejs-lts go rustup nasm ruby perl python zstd ccache
```

Install LLVM 21.1.8 from its GitHub release installer (`win64` on x64, `woa64` on ARM64); `scoop install llvm@21.1.8` no longer resolves to a valid download:

```ps1
$llvm = if ([System.Runtime.InteropServices.RuntimeInformation]::OSArchitecture -eq "Arm64") { "woa64" } else { "win64" }
Invoke-WebRequest -Uri "https://github.com/llvm/llvm-project/releases/download/llvmorg-21.1.8/LLVM-21.1.8-$llvm.exe" -OutFile "$env:TEMP\LLVM-21.1.8.exe"
Start-Process -FilePath "$env:TEMP\LLVM-21.1.8.exe" -ArgumentList "/S" -Wait
```
...
(`ruby`, `perl` and `python` are the interpreters JavaScriptCore's code generators run under; `zstd` packs the ICU data.)
...
- Bun's C/C++ and JavaScriptCore compile with `-flto=thin`
...
There is no LTO for arm64: LLVM's CodeView emitter can't handle ARM64 NEON tuple registers when JSC goes through LTO codegen.
