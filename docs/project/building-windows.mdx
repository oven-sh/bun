...
To compile JavaScriptCore from source instead of downloading the prebuilt (`--webkit=source`, or `bun run build:local` for your own WebKit clone), also install the code generators' interpreters:
