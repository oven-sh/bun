...
4. A Windows sysroot: an [xwin](https://github.com/Jake-Shadle/xwin) splat of the MSVC CRT and Windows SDK laid out like a Visual Studio install. Note that downloading these components means accepting Microsoft's license terms for them.
...
The build looks for the sysroot at `/opt/winsysroot` (or `/opt/xwin`) automatically; elsewhere, set `WINDOWS_SYSROOT=<path>` or pass `--winsysroot=<path>`.
...
Output lands in `build/debug-windows-x64/bun-debug.exe`, `build/release-windows-arm64/bun-profile.exe` + `bun.exe`, etc. Equivalent raw flags: `bun run build --os=windows --arch=aarch64`.
