...
<Note>
	...
	that get installed into `$Env:PATH` that will conflict with MSVC and break the build.
</Note>
...
<Note>
	It is not recommended to install `ninja` / `cmake` into your global path, because you may run into
	a situation where you try to build bun without .\scripts\vs-shell.ps1 sourced.
</Note>
