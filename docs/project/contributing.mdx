...
`bun run build:local` handles everything: configuring JSC, building JSC, and building Bun. On subsequent runs, JSC rebuilds incrementally if any WebKit sources changed. Running ninja directly on `build/debug-local` also works after the first build, as long as it is the ninja the build runs (`bun scripts/build.ts --configure-only` prints it; a different ninja version starts the build log over and rebuilds everything), and builds both Bun and JSC.
