...
On Linux and macOS, a file that `extract()` creates gets its read and execute permissions from the archive. Its owner can also read and write it. The group and others can write the file only if they can read it and the process umask allows it, as with any new file. Bun does not restore the setuid, setgid, and sticky bits. An entry with no permission bits counts as `0o644`. A file that already exists keeps its mode.
