...
On Linux and macOS, a file that `extract()` creates gets the permission bits stored in the archive, plus read and write permission for its owner. The process umask applies. Bun does not restore the setuid, setgid, and sticky bits. A file whose entry has no permission bits gets `0o644`. A file that already exists keeps its mode.
