...
Force **every** package to resolve through one registry. This setting overrides all other registry configuration on the machine: `install.registry`, `install.scopes`, `.npmrc`, `NPM_CONFIG_REGISTRY`, and the `--registry` flag.

Use it on centrally managed devices. Set it once in the global `~/.bunfig.toml`, or in the `BUN_CONFIG_FORCE_REGISTRY` environment variable. `bun install` then sends every registry request to that registry, whatever a project configures.
...
A project-level `bunfig.toml` cannot change or unset a value that the global bunfig sets. When the forced registry overrides a registry that a project, an `.npmrc`, or `--registry` configures, `bun install` prints a note that names the forced registry.
...
4. The token of the registry it replaces, if that registry is on the same host and the forced registry does not change `https` to `http`.

Bun never sends a token that is configured for a different host to the forced registry.

For device-wide enforcement, **prefer the `BUN_CONFIG_FORCE_REGISTRY` environment variable** over the global bunfig. `bun run` and `bun <file>` do not read the global `~/.bunfig.toml` (see [`bun run`](#bun-run) below). The runtime auto-installer, which runs when a script imports a package that is not on disk, therefore sees only the project bunfig. The package manager itself reads the environment variable, so it covers every code path, auto-install included.
