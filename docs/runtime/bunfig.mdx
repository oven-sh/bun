...
When set to `true`, `bun install` fails if any *transitive* dependency is
resolved from a non-registry source — a git URL, github reference, remote
or local tarball, local folder (`file:`), symlink (`link:`), or a
`workspace:` reference pulled in by a non-workspace parent. Direct
