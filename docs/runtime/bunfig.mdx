...
This is intended for centrally-managed devices: set it once in the global `~/.bunfig.toml` (or via the `BUN_CONFIG_FORCE_REGISTRY` environment variable) and `bun install` will always use that registry regardless of what any project checks in.
