...
Configures native [OpenTelemetry tracing](/runtime/opentelemetry) (the table's presence enables it; `enabled = false` turns it off). `OTEL_*` environment variables override these values.
...
serviceName = "my-api"
headers = { "x-api-key" = "..." }
# or a vendor preset instead of endpoint/headers (credentials from the vendor's env vars):
# exporter = "datadog"
```

Keys: `enabled`, `endpoint`, `headers`, `serviceName`, `exporter` (one of the [presets](/runtime/opentelemetry#sending-to-a-hosted-backend)).
