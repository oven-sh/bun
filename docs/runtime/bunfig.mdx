...
When a command runs from a subdirectory, Bun looks for `bunfig.toml` in the working directory first, then walks up parent directories until it reaches the project root, found the same way `--filter` finds it: the nearest directory with a `package.json`, or the workspace root whose `workspaces` globs include that directory. The walk does not continue past the project root, does not run when the working directory is inside `node_modules`, and relative `preload` entries resolve against the directory containing the `bunfig.toml` (other relative paths, like `[test] root`, still resolve against the working directory). Compiled executables only read `bunfig.toml` from the directory they are run in.
