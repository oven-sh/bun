...
Load a persistent test environment module. Bun evaluates the module once per
process or parallel worker, then calls its default export's `setup` function
for every test file. The setup function can return a teardown function or an
object with a `teardown` method.
...
The environment enables file isolation. It cannot be combined with
`--no-isolate`. The equivalent CLI option is
`--environment ./test-environment.ts`.
