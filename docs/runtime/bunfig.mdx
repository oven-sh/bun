...
For example, with `snapshotFloatPrecision = 15`, `expect(Math.sin(1)).toMatchSnapshot()` serializes `0.841470984807897` instead of `0.8414709848078965`. Only numbers are affected — numbers already embedded inside strings (e.g. `"rgba(200.123, 1, 1, 1)"`) are left as-is.
