...
Set this in the global `.bunfig.toml` (see [Global vs. local](#global-vs-local)). Bun ignores it in a project `bunfig.toml`, because a project file must not send writes outside the project.
...
Set this in the global `.bunfig.toml`. Bun ignores it in a project `bunfig.toml`, for the same reason as `globalDir`.
