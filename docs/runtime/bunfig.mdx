...
The check inspects the specifier written in each nested `package.json`,
not the final resolution, so enabling this does not interact with
`install.linkWorkspacePackages` — a registry package asking for a plain
semver range is still allowed even if Bun redirects it to a local
workspace copy.
