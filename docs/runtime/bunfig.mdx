...
- Bun finds descendants by walking down the process tree from itself. A process whose parent has already exited is out of reach when you run a file directly or run `bun test`. `bun run <script>` additionally kills the process group of the script, which covers a process that the script left running in the background.
- On Linux, the kernel kills Bun itself when the parent goes away, so Bun does not get to walk the tree. The kernel kills the children that Bun spawned from its main thread in the same way, and a child that is itself a Bun inherits the option and does the same for its own children. The children of other programs, and processes spawned from a `Worker`, keep running.
