...
Configure a minimum age for npm package versions. Package versions published more recently than this threshold will be filtered out during installation. Accepts either a number of seconds or an [`ms`](https://npmjs.com/package/ms)-style duration string like `"2d"`, `"1 week"`, or `"48h"`. Defaults to `"2d"` (two days) for new projects (lockfile `configVersion` ≥ 2); projects created with older Bun versions default to disabled. Set to `0` to disable.
