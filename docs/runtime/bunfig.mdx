...
On Linux and macOS, Bun terminates a descendant only when the kernel lets Bun send it a signal. A descendant that runs as a different user than Bun keeps running unless Bun has that permission. On macOS, Bun has it when it runs as root. On Linux, Bun has it when it holds the `CAP_KILL` capability, which root normally holds and which a container can drop. This rule applies to a process spawned with the `uid` option of `Bun.spawn` or `child_process`, and to a command run through a setuid program such as `sudo`. On Linux, the kernel applies the same permission check to the parent-death signal that Bun arms on each child it spawns.
