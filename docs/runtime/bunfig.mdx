...
When `true`, Bun watches the process that spawned it and exits as soon as that parent goes away, even if the parent was force-killed and never got a chance to forward a signal. On its own exit, Bun also terminates its descendant processes so nothing it spawned outlives it. Useful when Bun is launched by a supervisor (Electron, a CI runner, a thin shim) that may be force-killed.
...
On Linux and macOS, Bun terminates a descendant only when the operating system lets Bun send it a signal. A descendant that runs as a different user than Bun keeps running. That includes a process spawned with the `uid` option of `Bun.spawn` or `child_process`, and a process started through a setuid program such as `sudo`. Bun terminates such descendants only when it runs as root. On Linux, root also needs the `CAP_KILL` capability, which a container can drop. The Linux kernel applies the same check when it delivers the parent-death signal that Bun arms on each child, so such a descendant also survives when Bun itself is force-killed.
