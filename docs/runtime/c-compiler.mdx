...
`include` is an optional list of directories to search for headers, equivalent to `-I` in gcc/clang. Relative paths are resolved from the current working directory.
