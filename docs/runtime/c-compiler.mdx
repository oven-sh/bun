...
Pass `--no-ffi-cc` (or `--no-addons`) to disable the C compiler together with the rest of `bun:ffi`. Any call to `cc()` then throws an error with the code `ERR_FFI_DISABLED`. See [Disabling `bun:ffi`](/runtime/ffi#disabling-bun-ffi).
