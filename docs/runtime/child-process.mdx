...
| -------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Windows  | The child runs in a Job Object. The kernel notifies Bun when the tree commits more than `maxMemory`, and Bun terminates the job.                         |
| Linux    | When Bun can create a memory cgroup (v2 or v1), the child starts inside one with `memory.max` set, and the kernel kills the tree. Otherwise Bun samples. |
...
The sampler runs slowly while every tree is far from its limit and every millisecond once any tree is within 15% of it. `killSignal` applies when Bun does the kill. When the kernel does it (Linux cgroup, Windows job), the tree is killed at once, as with `SIGKILL`.
