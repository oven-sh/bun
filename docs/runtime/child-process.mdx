...
- **`terminal.close()` may block on older Windows** (Server 2019 / pre-21H1) if a child is still attached, because `ClosePseudoConsole` waits for the output pipe to drain. Call `proc.kill()` before `terminal.close()` if you need to tear down with a running child on those versions.
