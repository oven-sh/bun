...
| `name`  | Terminal type (sets `TERM` environment variable)                     | `"xterm-256color"` |
| `data`  | Callback when data is received `(terminal, data) => void`            | —                  |
| `exit`  | Callback when EOF is received `(terminal, exitCode, signal) => void` | —                  |
...
You can also create a terminal independently and pass its file descriptors to `Bun.spawn`:
...
// Spawn a process using the terminal's file descriptors
const proc = Bun.spawn(["bash"], {
  stdin: terminal.stdin, // slave fd for child's stdin
  stdout: terminal.stdin, // slave fd for child's stdout
  stderr: terminal.stdin, // slave fd for child's stderr
});

terminal.write("echo hello\n");
```

This approach gives you more control over the terminal lifecycle, but the `terminal` option in `Bun.spawn` is simpler for most use cases.
...
interface Subprocess extends AsyncDisposable {
  ...
  readonly readable: ReadableStream<Uint8Array> | number | undefined | null;
