...
- **`process.on('SIGWINCH')` in the child does not fire under ConPTY.** `process.stdout.columns`/`rows` _do_ update after `resize()`, but libuv's resize-event detection relies on a conhost window hook that ConPTY's headless conhost doesn't emit. Poll `process.stdout.columns` instead. This affects any libuv-based child (Node.js included).
