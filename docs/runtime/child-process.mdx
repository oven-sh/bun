...
- **`terminal.close()` may block on Windows before 11 24H2** (build 26100) if a child is still attached, because [`ClosePseudoConsole`](https://learn.microsoft.com/en-us/windows/console/closepseudoconsole) waits for the output pipe to drain on those versions. Kill the attached process first if you need to tear down with a running child.
