...
A single background thread samples every limited tree. It samples slowly while all trees are far from their limits and every millisecond once any tree is within 15% of its limit, so overshoot is small even for a process allocating as fast as it can. The measurement is physical footprint on macOS (the number Activity Monitor shows), resident anonymous memory on Linux, and committed memory of a Job Object on Windows, where the Job Object also guarantees the whole tree is terminated.
