...
- **`process.on('SIGWINCH')` in the child can fire late under ConPTY.** ConPTY reports a resize only to a process that reads stdin in raw mode. A Bun child in raw mode gets `SIGWINCH` at once. Otherwise Bun compares the console size when its event loop next wakes, so an idle child does not get the signal until something else wakes it. A Node.js child (or any other libuv-based child) gets `SIGWINCH` under ConPTY only in raw mode. `process.stdout.columns`/`rows` do update after `resize()`.
