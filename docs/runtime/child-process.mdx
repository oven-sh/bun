...
- `advanced`: (default) Messages are serialized using Node.js's V8 serialization wire format, which supports [everything `structuredClone` supports](https://developer.mozilla.org/en-US/docs/Web/API/Web_Workers_API/Structured_clone_algorithm) such as `Buffer`, `Map`, `Set`, typed arrays, and circular references. This does not support transferring ownership of objects.
...
Both serialization modes work between a `bun` process and a Node.js process in either direction, so you can use `serialization: "advanced"` to pass `Buffer`s, `Map`s, and other structured-clone types across the runtime boundary.
