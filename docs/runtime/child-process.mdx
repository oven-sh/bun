...
- **`process.on('SIGWINCH')` in the child can fire late under ConPTY** unless the child is reading stdin in raw mode. ConPTY reports a resize only to a raw-mode reader. Otherwise Bun checks the console size when its event loop next wakes. `process.stdout.columns`/`rows` do update after `resize()`.
