...
- **`process.on('SIGWINCH')` in the child does not fire under ConPTY** unless the child is reading stdin in raw mode: ConPTY reports a resize only to the reader of its input. A Bun child in raw mode gets `SIGWINCH` at once, as does a Node.js child (or any other libuv-based child). `process.stdout.columns`/`rows` do update after `resize()`.
