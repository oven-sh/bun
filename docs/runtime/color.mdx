...
`"css"` is the only output format that accepts colors without concrete color channels, such as `currentColor`, system colors like `Canvas`, and `light-dark()`. Every other format needs to resolve the input to red, green, and blue values, so those inputs return `null` instead.

```ts
Bun.color("currentColor", "css"); // "currentColor"
Bun.color("currentColor", "number"); // null
```
