...
DST transitions follow the same rules as `new Date(year, month, day, hour, minute)`:

- **Spring-forward** — schedules in the missing hour fire at the next valid local minute.
- **Fall-back** — schedules in the duplicated hour fire once, at the first occurrence.
