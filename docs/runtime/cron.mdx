...
On headless servers and CI environments where the current user's Security Identifier (SID) cannot be resolved — such as service accounts created by [NSSM](https://nssm.cc/) or similar tools — Bun automatically falls back to running the task as `SYSTEM`. This ensures `Bun.cron()` works on any Windows machine, but the task runs with elevated privileges and a different user profile (`%SYSTEMROOT%\system32\config\systemprofile` instead of `%USERPROFILE%`).
