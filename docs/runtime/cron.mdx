...
`Date | null` — the next matching time, or `null` if no match exists within ~4 years (e.g. February 30th).
...
DST transitions follow ECMAScript `Date` semantics (matching the npm `croner` package):
...
- **Fall-back** — a schedule in the duplicated hour fires once, at the first occurrence. Wildcard schedules (`* * * * *`, `0 * * * *`) also fire once per wall-clock minute, so the repeated hour is skipped in real time.

This differs from Vixie cron (which runs gap jobs immediately at the end of the gap) and `cron-parser` (which fires through the repeated fall-back hour).
...
| `handler`  | `(this: CronJob) => void \| Promise<void>` | Called on each fire. Inside a `function` callback, `this` is the `CronJob` (so `this.stop()` works). |
