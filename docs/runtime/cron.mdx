...
DST transitions match Vixie cron and the popular npm parsers (`cron-parser`, `croner`):

- **Spring-forward** — a schedule that lands in the missing hour fires that day, shifted forward by the gap (e.g. `30 2 * * *` runs at 3:30 on the spring-forward day).
- **Fall-back** — a schedule in the duplicated hour fires once, at the first occurrence.
