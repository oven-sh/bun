...
In-process cron respects `bun:test` fake timers. The schedule is computed against the mocked `Date.now()`, so you can drive fires deterministically:
...
Bun.cron("0 0 * * *", handler);

jest.advanceTimersByTime(30_000); // → 2025-01-02T00:00:00Z
expect(handler).toHaveBeenCalledTimes(1);
```

Without fake timers, cron fires on the real wall clock.
