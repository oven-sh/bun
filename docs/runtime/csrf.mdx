...
Bun provides a built-in API for generating and verifying [CSRF (Cross-Site Request Forgery)](https://owasp.org/www-community/attacks/csrf) tokens through `Bun.CSRF`. Tokens are signed with HMAC and include expiration timestamps to protect against replay attacks.
...
- `secret` (string, optional) — The secret key used to sign the token. If not provided, Bun generates a random in-memory default secret (unique per process).
- `options` (object, optional):

| Option      | Type     | Default       | Description                                                                                            |
| ----------- | -------- | ------------- | ------------------------------------------------------------------------------------------------------ |
| `expiresIn` | `number` | `86400000`    | Milliseconds until the token expires. `0` means never expires.                                         |
...
const permanentToken = Bun.CSRF.generate("my-secret", {
  expiresIn: 0,
});
...
| `maxAge`    | `number` | `86400000`    | Maximum token age in milliseconds, independent of the token's own `expiresIn`. `0` disables.         |
...
const server = Bun.serve({
  routes: {
    ...
    "POST /submit": async req => {
      ...
      if (!csrfToken || !Bun.CSRF.verify(csrfToken, { secret: SECRET })) {
...
If you omit the `secret` parameter in both `generate()` and `verify()`, Bun uses a random secret generated once per process. This is convenient for single-process applications but won't work across multiple servers or after a restart.

```ts title="default-secret.ts" icon="/icons/typescript.svg"
// Both use the same in-memory default secret
