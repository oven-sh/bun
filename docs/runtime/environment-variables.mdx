...
| `BUN_CONFIG_HTTP_IDLE_TIMEOUT`           | Sets the idle timeout for an in-flight `fetch` request, in seconds. The timer covers the TLS handshake, the response headers, and the response body. If it fires, the request fails with a timeout error. Defaults to `300`. Set it to `0` to disable the timer.                                                                                                              |
