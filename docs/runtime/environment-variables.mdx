...
Bun automatically _expands_ environment variables in unquoted and backtick-quoted values, so you can reference variables defined anywhere in the file, including in later assignments. Single- and double-quoted values are not expanded.
