...
| `BUN_CONFIG_MAX_HTTP_DECOMPRESSED_SIZE`  | Maximum number of bytes `fetch()` will decompress from a `gzip`, `deflate`, `br`, or `zstd` response body before failing with a decode error. Guards against compression-bomb responses. Defaults to 2 GiB. Set to `0` to disable the cap.                                                                                                                                                                                                                                                                                                                                |
