...
Bun automatically _expands_ environment variables in unquoted, double-quoted, and backtick-quoted values, so you can reference variables defined anywhere in the file, including in later assignments. Single-quoted values are not expanded.
