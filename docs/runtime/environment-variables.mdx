...
| `SSL_CERT_FILE`                          | Path to a PEM file of CA certificates. Bun trusts these certificates in addition to its bundled root certificates, for example in `fetch`, `node:https`, `node:tls`, and `WebSocket`. If unset, Bun reads `/etc/ssl/cert.pem`. If set to an empty string, Bun reads no file. OpenSSL introduced this variable. Bun keeps the name for compatibility.                          |
| `SSL_CERT_DIR`                           | Directories of CA certificates that Bun trusts in addition to its bundled root certificates. Name each certificate file by its subject hash (`<hash>.0`), the layout that `openssl rehash` creates. Separate directories with `:` (`;` on Windows). If unset, Bun reads `/etc/ssl/certs`. OpenSSL introduced this variable. Bun keeps the name for compatibility.             |
