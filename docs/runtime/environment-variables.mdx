...
| `BUN_OTEL`                               | If `BUN_OTEL=1`, Bun enables native [OpenTelemetry tracing](/runtime/opentelemetry) at startup and reads the standard `OTEL_*` variables (`OTEL_EXPORTER_OTLP_ENDPOINT`, `OTEL_SERVICE_NAME`, …). Not required when tracing is started from code with `Bun.otel.start()` or from a `[telemetry]` table in bunfig.toml; `OTEL_SDK_DISABLED=true` turns it off.                 |
