...
| `BUN_WATCHER_USE_POLLING`                | If `BUN_WATCHER_USE_POLLING=1`, `--watch` and `--hot` poll the filesystem instead of using native change events. Use this for Docker bind mounts, WSL `/mnt/...` paths, and network filesystems where native events do not fire. `BUN_WATCHER_POLL_INTERVAL` sets the poll interval in milliseconds (default `100`).                                                          |
