...
Workers inherit the setting from their parent. A Worker can also set it for itself with `execArgv: ["--no-ffi-cc"]`, but cannot re-enable what its parent disabled. The flags also work through `BUN_OPTIONS`, and standalone executables can bake them in with `bun build --compile --compile-exec-argv="--no-ffi-cc"`.
