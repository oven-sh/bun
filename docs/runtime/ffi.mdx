...
To track when a `TypedArray` is no longer in use from C or FFI, pass a callback and an optional context pointer to `toArrayBuffer` or `toBuffer`. Bun calls the callback later, once the garbage collector frees the underlying `ArrayBuffer` JavaScript object. The callback may run on a garbage-collector thread rather than the JavaScript thread, so it must not call back into JavaScript or assume thread-local state.
...
The callback can also be the `ptr` of a `JSCallback`. JavaScript cannot run during a garbage collection, so Bun queues the call and runs the function later, on the JavaScript thread that created the `JSCallback`. That is also the case when the buffer was transferred to a `Worker` and freed there. The same applies when a C callback calls a `JSCallback` from inside the collection: the C caller gets `0` back, as it does from a thread-safe callback. Keep the `JSCallback` open until the last call arrives. Bun drops the call when the thread that created the `JSCallback` exits first, for example a `Worker` that ends while such a buffer is alive.
...
const deallocator = new JSCallback(
  (bytes, deallocatorContext) => {
    // runs on the JavaScript thread, after the collection that freed the ArrayBuffer
    myNativeLibraryFree(bytes);
  },
  { args: ["ptr", "ptr"], returns: "void" },
);

toArrayBuffer(bytes, byteOffset, byteLength, deallocator.ptr);
```
