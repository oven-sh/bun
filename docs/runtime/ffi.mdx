...
Don't use raw pointers outside of FFI. To disable `bun:ffi` for a process, pass `--no-ffi-cc` or `--no-addons` (see [Disabling `bun:ffi`](#disabling-bun-ffi)).
...
Pass `--no-ffi-cc` to disable `bun:ffi` for a process. The module still imports, but every function in it that reaches native code or memory throws an error with the code `ERR_FFI_DISABLED`: `dlopen`, `native.dlopen`, `cc`, `JSCallback`, `CFunction`, `linkSymbols`, `ptr`, `read.*`, `toBuffer`, `toArrayBuffer`, `CString`, and `viewSource`. The same functions on `Bun.FFI` throw too. Constants such as `FFIType` and `suffix` stay available.

```sh
bun --no-ffi-cc ./app.ts
```

`--no-addons` disables `bun:ffi` in the same way. It also makes `process.dlopen` and `require()` of `.node` files throw, and disables the `node-addons` export condition. Neither flag affects SQLite extension loading (`loadExtension`, `Database.setCustomSQLite`).

Workers inherit the setting from their parent. A Worker can also set it for itself with `execArgv: ["--no-ffi-cc"]`, but cannot re-enable what its parent disabled. Macros that `Bun.build()` runs follow the flags of the process. The flags also work through `BUN_OPTIONS`, and standalone executables can bake them in with `bun build --compile --compile-exec-argv="--no-ffi-cc"`.
