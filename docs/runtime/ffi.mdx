...
Tell the native code to stop calling the callback before you call `close()`. After `close()`, `ptr` is `null`, and passing the `JSCallback` to a native function throws a `TypeError`.
