...
Note: `void` is only valid as a `returns` type — it is the default return type and cannot be used as an argument. `usize`/`isize` are pointer-sized integer aliases of `u64`/`i64`.
