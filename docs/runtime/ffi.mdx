...
`--no-addons` disables `bun:ffi` in the same way, in addition to `process.dlopen` and the `node-addons` export condition. Pass `--no-addons` to block every way of loading native code.
