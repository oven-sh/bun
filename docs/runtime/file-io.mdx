...
- `ReadableStream`
- `Bun.Archive`
- `Bun.Image` (encoded in the pipeline's output format)
- An array of `BlobPart`s, as `new Blob()` accepts

Any other value throws a `TypeError` with code `ERR_INVALID_ARG_TYPE`.
