...
The MIME type is inferred from the file extension (`.txt` above maps to `text/plain;charset=utf-8`). An unknown extension gives `application/octet-stream`. Override it by passing a second argument to `Bun.file`.
