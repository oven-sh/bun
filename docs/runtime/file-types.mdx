...
CSS files can be directly imported.

```ts
import styles from "./styles.css";
```

_In the runtime_, CSS files are treated like the [`file`](#file) loader - the import resolves to an absolute path to the CSS file on disk.

```bash
bun run app.ts
```
```txt
/path/to/project/styles.css
```

_In the bundler_, CSS files are parsed and bundled together, handling `@import` statements and `url()` references. All imported CSS files are bundled together into a single `.css` file in the output directory.
