...
document and buffers the output until it is read. Either way the document goes
through the parser at most 256 KiB per turn of the event loop, so a large
document does not keep timers and other connections waiting until its rewrite is
done. A body that is already in memory (a string, `Blob`, or `ArrayBuffer` body)
and fits in 256 KiB is rewritten before `transform()` returns; a larger one
continues from the event loop like a streamed input. Because the rewrite
outlives `transform()`, an error thrown by an async handler (or a Promise it
returns that rejects) rejects the response body instead of throwing from
`transform()`:
...
synchronously: the whole document is rewritten inside the call, however large it
is, and it cannot wait for a handler that needs the event loop to turn (a timer,
I/O, a `fetch`). Such a handler makes `transform()` throw a `TypeError`, and the
rewrite fails without running any further handlers:
