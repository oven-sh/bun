...
The escaping is the same in every element, `<script>` and `<style>` included (this matches Cloudflare Workers). A browser does not decode entities inside `<script>`, `<style>`, `<xmp>`, `<iframe>`, `<noembed>`, `<noframes>` or `<plaintext>`, so text inserted there keeps the entities: `el.append("if (a < b) run()")` on a `<script>` produces `if (a &lt; b) run()`, a syntax error. To insert script or style source, pass `{ html: true }` and make sure the string cannot close the element early:

```ts
// JSON state: "<" only occurs inside JSON strings, where "\u003c" means the same
// thing, so this is lossless and rules out "</script>" and "<!--".
...
For JavaScript source, refuse `</script` and `<!--` (ASCII case-insensitive) the same way. `<textarea>` and `<title>` decode entities like ordinary elements, so the default escaping is correct there.
