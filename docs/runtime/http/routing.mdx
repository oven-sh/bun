...
Some files have content and report a size of `0`. On Linux, the files on `procfs` and `cgroupfs` do. Bun reads such a file before it sends the response headers:

- Bun sends `Content-Length` when the content fits in one 256 KB read buffer.
- Bun streams larger content without `Content-Length`. Over HTTP/1.1 the response uses chunked transfer encoding.
- Bun adds no `Last-Modified` or `ETag` header and ignores the `Range` header of the request. The size and the modification time of such a file do not describe its content.

File routes, directory routes, and a `Response` with a `BunFile` body from a `fetch` handler all serve such a file this way.
