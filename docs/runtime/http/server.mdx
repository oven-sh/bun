...
Override the idle timeout for an individual request. Pass `0` to disable the timeout entirely for that request. `seconds` must be an integer from `0` to `255`, the same range as `idleTimeout`. Other values throw.
