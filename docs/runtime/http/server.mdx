...
These methods take the `Request` that Bun passed to your handler. A copy of it (`req.clone()`, `new Request(req, init)`, `new Request(url, req)`) works too, so middleware that re-wraps the request does not lose them. The same applies to `server.upgrade()`.
