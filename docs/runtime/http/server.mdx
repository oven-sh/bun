...
A `0` means drop it. There is no retransmission on this path, and the next datagram is worth more than a late one.

`maxDatagramSize` is the server's own limit capped by the `max_datagram_frame_size` the peer advertised, less the
session's frame prefix. A `0` there means the peer offered no datagram support at all — that session carries no
...
`drain()` asks the peer to wind the session up without ending it. Datagrams still flow and the `close` handler does not
run until something actually closes the session; a browser surfaces the signal as `WebTransport.draining`. Use it to let
sessions finish before a redeploy, and `close()` when they should stop now.
