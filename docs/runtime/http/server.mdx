...
Set `http1: false` to refuse HTTP/1.x clients. Over TLS, clients that offer ALPN without `h2` fail the handshake; clients that send no ALPN at all, and cleartext connections that don't start with the preface, receive `505 HTTP Version Not Supported`. WebSocket clients must then support HTTP/2 Extended CONNECT ([RFC 8441](https://www.rfc-editor.org/rfc/rfc8441)).

`server.upgrade()` accepts both an HTTP/1.1 WebSocket upgrade and an HTTP/2 Extended CONNECT request. WebSockets over HTTP/3, server push, and response trailers (needed by gRPC) are not supported.
