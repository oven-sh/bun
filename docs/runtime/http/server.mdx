...
<Note>
  `http3` is not supported with unix domain sockets: QUIC requires a UDP port. `http1: false` requires `http2: true` or `http3: true`.
</Note>
...
With `tls`, Bun picks the protocol per connection using ALPN. Clients that offer `h2` (browsers, `curl`, gRPC) get HTTP/2. Everyone else gets HTTP/1.1.
...
Set `http1: false` to refuse HTTP/1.x clients. Over TLS, the handshake fails for clients that don't offer `h2`. Without TLS, connections that don't start with the preface receive `505 HTTP Version Not Supported`.

`server.upgrade()` only works on HTTP/1.1 requests. WebSockets over HTTP/2 are not supported.
