...
- `1+` — The number of bytes sent, including the WebSocket frame header. An empty payload still writes a 2-byte frame, so a successful send is always greater than zero.
