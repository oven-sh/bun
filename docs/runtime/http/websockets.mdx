...
For HTTP/1.1, a successful upgrade sends `101 Switching Protocols`. For HTTP/2, Bun uses Extended CONNECT and sends a `200` response as defined by [RFC 8441](https://www.rfc-editor.org/rfc/rfc8441). To attach additional response headers, pass them to `server.upgrade()`.
