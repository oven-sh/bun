...
A paused connection reads no frames at all. It does not answer a ping. It does not see a close from the peer, so `readyState` stays `OPEN` until `resume()`. `close()` and `terminate()` work while paused.
