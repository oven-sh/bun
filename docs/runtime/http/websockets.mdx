...
By default, Bun closes a WebSocket connection that has been idle for 120 seconds. Configure this with the `idleTimeout` parameter. The value is in seconds. The maximum is `960` (16 minutes), and `0` disables the timeout. Values from `1` to `7` round up to `8`.
...
namespace Bun {
  export function serve(params: {
    ...
    websocket?: {
      ...
      idleTimeout?: number; // default: 120 (seconds), max: 960
