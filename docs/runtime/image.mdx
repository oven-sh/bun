...
Bun decodes a damaged JPEG when libjpeg-turbo can finish the decode with only a warning. Examples are stray bytes between segments, a missing end-of-image marker, and truncated scan data. Blocks with no data decode as mid-grey in a baseline file and with less detail in a progressive file, the same pixels `djpeg` writes. A JPEG that libjpeg-turbo cannot finish rejects with `ERR_IMAGE_DECODE_FAILED`.
