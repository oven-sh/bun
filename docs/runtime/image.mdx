...
`Bun.write(dest, img)` and `Bun.file(dest).write(img)` also accept a pipeline. They encode in the chained format and do not infer a format from the destination path. When no format was chained, they use the source format, except that BMP, TIFF and GIF sources (decode only) are written as PNG.
