...
`Bun.write(dest, img)` and `Bun.file(dest).write(img)` also accept a pipeline. They encode in the chained format (or the source format when none was chained) and do not infer a format from the destination path.
