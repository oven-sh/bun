...
`.write()` keeps the encode-then-writeFile on the thread pool so encoded bytes never touch the JS heap. If you didn't chain a format method, the extension picks one (`.jpg`/`.png`/`.webp`/`.heic`/`.avif`).
