...
Bun applies no color transform and does not deliver the ICC profile. JPEG, PNG and WebP pixels are the decoder's output, in the space the source profile describes. Formats the system backend decodes (HEIC, AVIF, TIFF) arrive as sRGB. Sharp converts to sRGB by default, so the two differ for a source with a non-sRGB profile. Alpha is straight, not premultiplied.

Nothing else from the source travels with the plane: no EXIF, no GPS, no profile. What consumes `data` sees pixels and nothing about where they came from.
