...
For `fit: "contain"`, pass `background: { r, g, b, alpha }` to set the letterbox color (RGB 0–255, alpha 0–1). The default is transparent black — renders as black in JPEG (alpha dropped) and a transparent letterbox in PNG/WebP, so callers can composite the output without an extra step.
