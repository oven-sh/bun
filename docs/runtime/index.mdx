...
At the default depth, Bun prints `[Object ...]` in place of the object in `c`.
...
At a depth of `4`, Bun prints every level.

```bash terminal icon="terminal"
bun --console-depth=4 console.ts
```
...
The depth applies to arrays, `Map` objects, and `Set` objects too. Past the depth, Bun prints them as `[Array ...]`, `[Map ...]`, and `[Set ...]`. An empty array, `Map`, or `Set` prints as `[]`, `Map {}`, or `Set {}` at any depth.
