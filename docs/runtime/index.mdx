...
If there is no script with the name you typed, Bun points at the closest ones it knows: the `package.json` scripts, the executables in `node_modules/.bin`, and, for the `bun <script>` shorthand, its own commands.
...
```txt
error: Script not found "claen"
note: did you mean "bun run clean"?
```
