...
<Note>
  With the automatic runtime, Bun chooses between `jsx`/`jsx-runtime` and `jsxDEV`/`jsx-dev-runtime` based on build mode
  (`NODE_ENV` / `--production`), not on whether `jsx` is `"react-jsx"` or `"react-jsxdev"`. Both values enable the
  automatic runtime; a development build emits `jsxDEV` for either.
</Note>
