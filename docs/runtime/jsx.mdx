...
Bun supports `.jsx` and `.tsx` files out of the box. Bun's internal transpiler can convert JSX syntax into vanilla JavaScript before execution, or preserve JSX syntax for another tool to transform later.
...
<Note>**Note** — Only applicable when `jsx` is `react-jsx` or `react-jsxdev`; `jsx: "preserve"` leaves imports up to a later transform.</Note>
