...
Replace any HTML element with a custom React component by passing it as an option keyed by tag name:
...
<Markdown text={content} components={{ pre: Code, a: Link, h2: Heading }} headingIds />;
```

Where `Markdown` is:

```tsx
function Markdown({ text, components, ...options }) {
  return Bun.markdown.react(text, { ...components, ...options });
}
```
...
| ------------ | ---------------------------- | ------------------------------------------------------- |
| `h1`–`h6`    | `{ id?, children }`          | Headings. `id` is set when `headingIds` is enabled      |
...
By default, elements use `Symbol.for('react.transitional.element')` as the `$$typeof` symbol. For React 18 and older, set `reactVersion: 18`:

```tsx
function Markdown({ text }: { text: string }) {
  return Bun.markdown.react(text, { reactVersion: 18 });
}
```

### Parser options

All [parser options](#options) work with `react()`:
