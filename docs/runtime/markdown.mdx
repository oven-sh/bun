...
Bun parses the input with a spec-compliant HTML5 parser, so malformed markup is recovered the same way a browser recovers it. The conversion never throws on bad HTML. Only `<body>` content is converted. `<script>`, `<style>`, `<template>`, and `<noscript>` elements are dropped, and comments are ignored.
...
Characters in text that Markdown would otherwise treat as syntax (`*`, `_`, `` ` ``, `[`, `\`, a leading `-`, `#`, `>` or `1.`) are backslash-escaped, so the output renders back to the same text.
...
Deeply nested markup is handled without recursion limits in the parser. Like browsers, Bun stops nesting elements past a depth of 512 and keeps their text in the deepest open element.
