...
- `Bun.unstable_markdown.html()` — render Markdown to an HTML string
- `Bun.unstable_markdown.render()` — render Markdown with custom callbacks for each element
- `Bun.unstable_markdown.react()` — render Markdown to React JSX elements

---

## `Bun.unstable_markdown.html()`
...
```ts
const html = Bun.unstable_markdown.html(`
| Feature      | Status |
|-------------|--------|
| Tables       | ~~done~~ |
| Strikethrough| ~~done~~ |
...
```ts
const html = Bun.unstable_markdown.html("some markdown", {
...
Bun.unstable_markdown.html("Visit www.example.com", { autolinks: true });

// Enable only specific types
Bun.unstable_markdown.html("Visit www.example.com", {
...
Bun.unstable_markdown.html("## Hello World", { headings: true });
// '<h2 id="hello-world"><a href="#hello-world">Hello World</a></h2>\n'

// Enable only heading IDs (no autolink)
Bun.unstable_markdown.html("## Hello World", { headings: { ids: true } });
// '<h2 id="hello-world">Hello World</h2>\n'
```

---

## `Bun.unstable_markdown.render()`
...
```ts
const result = Bun.unstable_markdown.render("# Hello **world**", {
...
```ts
const plaintext = Bun.unstable_markdown.render("# Hello **world**", {
...
```ts
const result = Bun.unstable_markdown.render("# Title\n\n![logo](img.png)\n\nHello", {
...
````ts
const result = Bun.unstable_markdown.render("```js\nconsole.log('hi')\n```", {
  code: (children, meta) => {
...
```ts
const result = Bun.unstable_markdown.render("Visit www.example.com", {
...
const el = Bun.unstable_markdown.react(content, {
  pre: Code,
  a: Link,
  h2: Heading,
}, { headings: { ids: true } });
...
```tsx
function Markdown({ text }: { text: string }) {
  return Bun.unstable_markdown.react(text, undefined, { reactVersion: 18 });
}
```
...
```tsx
const el = Bun.unstable_markdown.react("## Hello World", undefined, {
  headings: { ids: true },
  autolinks: true,
});
```
