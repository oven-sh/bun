...
| `list`       | `{ ordered: boolean, start?: number, depth: number }`                   | Ordered or unordered list. `start` is the start number for ordered lists. `depth` is nesting level (0 = top-level) |
| `listItem`   | `{ index: number, depth: number, ordered: boolean, start?: number, checked?: boolean }` | List item. `index` is 0-based position within parent list. `depth` is the parent list's nesting level. `ordered`/`start` reflect the parent list. `checked` is set for task list items |
| `hr`         | —                                                                       | Horizontal rule                                                                                   |
| `table`      | —                                                                       | Table block                                                                                       |
| `thead`      | —                                                                       | Table head                                                                                        |
| `tbody`      | —                                                                       | Table body                                                                                        |
...
const result = Bun.markdown.render("1. first\n   1. sub-a\n   2. sub-b\n2. second", {
  listItem: (children, { index, depth, ordered, start }) => {
    ...
    const marker = !ordered ? "-"
      : depth === 0 ? `${n}.`
      : depth === 1 ? `${String.fromCharCode(96 + n)}.`
      : `${toRoman(n)}.`;
