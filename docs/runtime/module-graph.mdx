...
`graph.dispose()` also drops the graph's module registry and `require.cache`. An error in what still runs in the graph's context, such as a tick it had queued, still goes to `onError`. From then on:
