...
A `Bun.ModuleGraph` loads modules into the current global another time. Each graph has its own module instances, its own `require.cache`, and its own values for the names you pass as `globals`. Parsed code and bytecode are shared between graphs; ES modules share JIT-compiled code too (see [Shared compiled code](#shared-compiled-code)). The first module a graph `import()`s is that graph's main module: `import.meta.main` and `require.main === module` are true in it, and in no other module of the graph.
...
Graphs whose `globals` have the same names share one copy of each ES module's compiled code, JIT-compiled code included. Graphs with other `globals` names compile a copy of their own.

While one graph runs a module, JavaScriptCore optimizes the module's code for that one instance. When a second graph that shares the code imports the same module, JavaScriptCore discards the optimized code and compiles it again so that it serves every instance. The graph that is already running slows down while that happens. In a test app of 120 small modules, the running graph dropped to about a tenth of its speed and needed one to three seconds to reach a steady speed again.

The steady speed of the shared code depends on what the modules declare:

- Functions, imports and module-level variables run about as fast as before, however many graphs there are. JavaScriptCore recompiles this code once, when the second graph appears. A version of the test app that uses no classes ran as fast with fifty graphs as with one.
- A `class` is a different class in each graph, because each graph evaluates the declaration. The shared code that creates or uses its instances handles one kind of object for each graph. That code runs slower with several graphs than with one, and JavaScriptCore can recompile it again each time another graph starts to run it. The test app creates small objects of classes with private fields in its hot code. It ran 15% to 45% slower with two graphs and 25% to 80% slower with ten. How much depends on the graph and on how the calls are spread over the graphs.

`dispose()` does not bring the former speed back. After the other graphs are disposed, the first graph runs as it did with them.

A [`Worker`](/runtime/workers) or a separate process compiles its own copy of the code, so its speed does not depend on how many other instances exist. Measure your app with the number of graphs you plan to run.
