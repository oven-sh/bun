...
A `Bun.ModuleGraph` loads modules into the current global another time. Each graph has its own module instances, its own `require.cache`, and its own values for the names you pass as `globals`. Parsed code and bytecode are shared between graphs; ES modules share JIT-compiled code too.
...
- A process `Bun.spawn` starts is started and then killed, so a very short command can finish first.
...
- An import that was still waiting for a file rejects.
- An import whose module is suspended in a top-level `await` is left alone. If what the module awaits was the graph's, such as a timer or a request, it went with the graph and the import never settles.

A host that may dispose a graph while importing into it should race the import the same way as a call, as shown below.
...
The context follows the caller. A host function a graph calls, such as one passed in `globals`, runs in that graph's context. The asynchronous work it starts is the graph's and is dropped with it at `dispose()`. For work that must finish whatever happens to the caller, such as an audit log or billing, leave the graph's context with a snapshot taken in the host:
...
An error belongs to the graph whose code threw it, and stays that graph's through the promises that carry it on. If nobody handles the promise `graph.import()` returned and a module of the graph threw while it was being evaluated, that goes to `onError` too. That covers a throw at the module's top level, after an `await`, or in a dependency.
...
A rejection can also reach an unhandled promise through a `.then()` with no rejection handler, or through an `async` function that returned the promise. It currently arrives without a context and takes the process-wide path. `await` it, or add a handler, inside the graph.
