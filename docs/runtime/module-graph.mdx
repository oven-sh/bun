...
- Functions, imports and module-level variables run about as fast as before, however many graphs there are. JavaScriptCore recompiles this code once, when the second graph appears. A version of the test app that uses no classes ran 2% slower with two graphs, with ten and with fifty.
- A `class` is a different class in each graph, because each graph evaluates the declaration. The shared code that creates or uses its instances handles one kind of object for each graph. JavaScriptCore recompiles that code again when a third graph starts to run it. That code runs slower with two graphs than with one, and slower again with more. The test app creates small objects of classes with private fields in its hot code. It ran 15% to 45% slower with two graphs and 40% to 80% slower with ten.
