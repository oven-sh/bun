...
The host's own code runs in the context it is called in. `run()` calls a function of the host's in the graph's context:

```ts
graph.run(() => Bun.serve({ port: 0, fetch: app.fetch })); // the server is the graph's and stops with it
```
...
Uncaught exceptions and unhandled rejections go to the `onError` of the graph in whose context they happen, instead of the process-wide handlers. It is the same context that owns what the code opens: a graph's code and everything it starts run in the graph's context, and so does what the host calls through `run()`.
