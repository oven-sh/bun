...
Uncaught exceptions and unhandled rejections go to the handlers of the graph in whose context they happen, instead of the process-wide handlers. The two handlers are the ones `process` has:
...
As in Node.js, an unhandled rejection of a graph with no `unhandledRejection` goes to its `uncaughtException` with the origin `"unhandledRejection"`. A reason that is not an error reaches `uncaughtException` as an error with the code `ERR_UNHANDLED_REJECTION` that names it; `unhandledRejection` is given the reason as it is.

The process is not told of a rejection that a graph was given: it gets no `rejectionHandled` event when that promise is handled later.
