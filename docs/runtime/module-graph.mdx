...
A `Bun.ModuleGraph` loads modules into the current global another time. Each graph has its own module instances, its own `require.cache`, and its own values for the names you pass as `globals`. Graphs whose `globals` have the same names share the compiled code of their ES modules (see [Compiled code](#compiled-code)). The first module a graph `import()`s is that graph's main module: `import.meta.main` and `require.main === module` are true in it, and in no other module of the graph.
...
Graphs whose `globals` have the same names share the compiled code of their ES modules: the parsed code, the bytecode and the JIT-compiled code. A graph whose `globals` have other names compiles its own copy, and so does the host.

Sharing saves memory and makes a graph faster to load. It does not make every graph start warm, because JIT-compiled code that graphs share has to be right for all of them:

- While a module has one instance, the engine optimizes the module's code for that instance. When a second graph loads the module, the engine discards that optimized code and optimizes the code again for any instance. The second graph starts cold, and the first one slows down until the code is optimized again. Graphs that come later start with the code the others have warmed up.
- A class that a module defines is a different class in each graph, so code that uses it is optimized for the classes of all the graphs that run it. That code runs slower with several graphs than with one, more so when the class has private fields, and it can be optimized again when another graph starts to run it.
