...
A thread-safe function keeps the process running only for as long as the graph that created it lives, unless script outside that graph has also asked it to (`napi_ref_threadsafe_function`): then it keeps the process running until the addon lets go, whichever graph is disposed. Its calls always run in the context of the graph that created it. If an addon keeps one thread-safe function for everybody and calls your JavaScript callbacks from it, load the addon in the host first, so that the function is the host's.
