...
Bun matches the whole specifier. Any other subpath, such as `undici/package.json`, resolves to the installed package. For a replaced package, `require.resolve()` and `import.meta.resolve()` return the specifier itself, for example `"undici"`, instead of a file path.
