...
Returns a query-string-signed URL — typically an S3 object URL to hand to a browser. Takes the same signing options as `Bun.aws.fetch`, plus `method` (default `"GET"`) and `expiresIn` (seconds, default `900`, max `604800`).
