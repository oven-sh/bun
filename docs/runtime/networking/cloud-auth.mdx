...
Bun can authenticate requests to AWS and Google Cloud the same way their CLIs and SDKs do: from environment variables, config files, SSO sessions, or the instance / container / pod the code is running on. Credentials are resolved off the JavaScript thread, cached for the whole process, and refreshed shortly before they expire.

```ts
// SigV4-signed request with whatever credentials this machine has
const queues = await fetch("https://sqs.us-east-1.amazonaws.com/?Action=ListQueues", { aws: true });

// Bearer token from Application Default Credentials
const buckets = await fetch("https://storage.googleapis.com/storage/v1/b?project=my-project", { gcp: true });
```
...
await Bun.aws.credentials({ profile: "prod" }); // a specific profile
...
Signs the request with [Signature Version 4](https://docs.aws.amazon.com/IAM/latest/UserGuide/reference_sigv.html). `true` infers the service and region from `*.amazonaws.com` / `*.on.aws` hostnames and uses ambient credentials; pass an object to override any part.

```ts
await fetch("https://dynamodb.us-east-1.amazonaws.com/", {
  method: "POST",
  aws: true,
  headers: {
...
// API Gateway / Lambda function URL behind IAM auth, custom domain
await fetch("https://api.example.com/orders", {
  aws: { service: "execute-api", region: "eu-west-1", profile: "prod" },
});
```

| Option                                           | Default                                                       |
| ------------------------------------------------ | ------------------------------------------------------------- |
| `service`, `region`                              | inferred from the hostname; region falls back to `AWS_REGION` |
...
Bun adds `Authorization`, `x-amz-date`, `x-amz-security-token` (temporary credentials) and, for S3, `x-amz-content-sha256`; setting those yourself alongside `aws` is an error. `ReadableStream` bodies can only be signed for S3-style services (as `UNSIGNED-PAYLOAD`), since other services need the body's SHA-256 up front.

A signature is only valid for the exact host and path it was computed for, so signed requests default to `redirect: "manual"` — a `307` from S3 pointing at another region comes back as a `307` rather than being followed unsigned. Pass `redirect: "follow"` explicitly if you really want that.

### `Bun.aws.presign(url, options?)`

Returns a query-string-signed URL — typically an S3 object URL to hand to a browser. Takes the same options as `fetch`'s `aws`, plus `method` (default `"GET"`) and `expiresIn` (seconds, default `900`, max `604800`).

```ts
const download = Bun.aws.presign("https://my-bucket.s3.eu-west-1.amazonaws.com/report.pdf", { expiresIn: 3600 });
const upload = Bun.aws.presign("https://my-bucket.s3.eu-west-1.amazonaws.com/upload.bin", { method: "PUT" });
```

`presign` is synchronous. If ambient credentials haven't been resolved yet it resolves them once, blocking; `await Bun.aws.credentials()` beforehand to avoid that. For `s3://`-style paths and ACLs, [`S3File.presign()`](/runtime/s3#presigning-urls) is usually more convenient.
...
### `Bun.gcp.accessToken()` / `Bun.gcp.idToken()`
...
```ts
await fetch("https://pubsub.googleapis.com/v1/projects/my-project/topics", { gcp: true });
await fetch("https://www.googleapis.com/…", { gcp: { scopes: "devstorage.read_only" } });
await fetch("https://my-service-abc123.a.run.app/", { gcp: { audience: "https://my-service-abc123.a.run.app" } });
```

`true` / `{ scopes }` sends `Authorization: Bearer <access token>`; `{ audience }` sends an ID token instead. When the credentials carry a `quota_project_id` (or `GOOGLE_CLOUD_QUOTA_PROJECT` is set), `x-goog-user-project` is added unless you set it yourself.
