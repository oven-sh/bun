...
A session's `onStats` runs once per request that reached the HTTP client, when the connection is done with it: the response body ended, or the request failed or was aborted. When `fetch` rejects, `onStats` runs first. `onStats` is an option of `Bun.FetchSession` only.

```ts
let stats;
using session = new Bun.FetchSession({ onStats: s => (stats = s) });
try {
  await session.fetch("https://example.com/upload", { method: "POST", body });
} catch (error) {
