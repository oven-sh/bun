...
A `Bun.FetchSession` holds connection settings and owns a keep-alive pool. Pass it as `session` to each request that should use it. Bun never shares a connection between two sessions, or between a session and plain `fetch`.
