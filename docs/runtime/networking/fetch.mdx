...
A request that passes its own `checkServerIdentity` opens a connection of its own and closes it afterwards, so that another request's function never vouches for it. To verify once per connection and reuse the connection, put the function on a [`Bun.FetchContext`](#fetch-contexts).
...
A `Bun.FetchContext` holds connection settings and owns a keep-alive pool. Pass it as `context` to each request that should use it. Bun never shares a connection between two contexts, or between a context and plain `fetch`.
...
context.close(); // closes the idle connections
```
...
| `lookup`    | Resolves the host of each connection. See [Choosing the address to connect to](#choosing-the-address-to-connect-to).                                                                                                      |
...
An option given on the request takes precedence over the context's. A request's `tls` replaces the context's `tls` as a whole.

#### Choosing the address to connect to

`lookup` receives the hostname and port Bun is about to dial and returns the IP address to use. Bun keeps the URL as it is: the `Host` header, the TLS server name, certificate verification and `NO_PROXY` matching use the original hostname.
...
Bun calls `lookup` for every connection the request needs, including the connections of redirects, and before it considers a pooled connection. A request only reuses a connection to the address its own `lookup` returned. When `lookup` throws or rejects, `fetch` rejects with that error. Through a proxy, `lookup` receives the proxy's hostname. Bun also passes a hostname that is already an IP address to `lookup`. While `lookup` is pending the request has no connection and no timeout runs, so pass a `signal` if `lookup` might not settle. `lookup` works with HTTP/1.1 only.
...
| Field                                         | Description                                                |
| --------------------------------------------- | ---------------------------------------------------------- |
| `bytesWritten`                                | Request bytes handed to the socket, before TLS.            |
| `requestBodyBytesSent`                        | The body's share of `bytesWritten`, as framed on the wire. |
| `responseStarted`                             | Whether any byte of a response arrived.                    |
| `socketReused`                                | Whether the request used a pooled connection.              |
