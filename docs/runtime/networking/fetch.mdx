...
`fetch` can sign a request for any AWS API with SigV4, or attach a Google Cloud bearer token, using the machine's ambient credentials:

```ts
await fetch("https://sqs.us-east-1.amazonaws.com/?Action=ListQueues", { aws: true });
await fetch("https://storage.googleapis.com/storage/v1/b?project=my-project", { gcp: true });
```
