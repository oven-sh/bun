...
When an origin is at its limit, further requests to that origin are queued and sent as soon as one of its in-flight requests ends. If the process-wide ceiling is reached, additional requests are also queued until any in-flight request completes. Otherwise requests to other origins are unaffected, so a single slow or unresponsive host never blocks requests to healthy ones.
