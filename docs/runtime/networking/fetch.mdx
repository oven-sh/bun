...
Bun uses BoringSSL, the same TLS library as Chrome, so a Chrome fingerprint is reproduced in full. Two things are outside of what BoringSSL lets a client configure: the order of the extensions (BoringSSL uses its own fixed order, or a random one with `permuteExtensions`) and the three TLS 1.3 cipher suites, which always come first, either as `4865-4866-4867` or as `4867-4865-4866`. A JA3 string that names a cipher suite, group or extension BoringSSL cannot send (for example Firefox's `delegated_credentials` extension 34) throws a `TypeError`.
