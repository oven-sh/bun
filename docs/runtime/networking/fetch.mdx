...
Bun uses BoringSSL, the same TLS library as Chrome, so a Chrome ClientHello is reproduced up to two things BoringSSL does not let a client configure: the order of the extensions (BoringSSL uses its own fixed order, or a random one with `permuteExtensions`) and the three TLS 1.3 cipher suites, which always come first, either as `4865-4866-4867` or as `4867-4865-4866`. A JA3 string whose cipher suites, groups or extension set BoringSSL cannot reproduce throws a `TypeError`: for example one that names Firefox's `delegated_credentials` extension (34), or one that leaves out an extension BoringSSL always sends, such as `signature_algorithms` (13).

These options apply to `fetch`. A connection that resumes an earlier TLS 1.3 session adds the `pre_shared_key` extension (41), as browsers do. The HTTP/2 part of a browser fingerprint (SETTINGS values, header order) is not covered.
