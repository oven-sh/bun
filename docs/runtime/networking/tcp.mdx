...
The `key` and `cert` fields expect the _contents_ of your TLS key and certificate. This can be a string, `BunFile`, `TypedArray`, `Buffer`, or an array of these. A `BunFile` is read synchronously when the options are parsed, like `fs.readFileSync`, and must point to a regular file.
