...
Call `close()` when you are done with a socket to free the underlying handle:

```ts
client.close();
```

After `close()`, the socket's `closed` property is `true` and the socket can no longer send or receive datagrams.
