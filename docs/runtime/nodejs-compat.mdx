...
🟢 Fully implemented. `ReadStream` extends `net.Socket` over a native TTY handle, as in Node. `WriteStream` extends `fs.WriteStream` rather than `net.Socket`, and constructing it on a non-TTY fd returns a stream with `isTTY` set to `false` instead of throwing.
