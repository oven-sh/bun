...
🟢 Fully implemented. `http.Server` does not extend `net.Server`. `listen({ fd })` serves on the file descriptor on Linux and macOS. In a `node:cluster` worker the number names a file descriptor of the primary, as in Node.js. Bun ignores `listen(handle)` and the `ipv6Only` and `signal` options of `listen()`. `keepAlive`/`keepAliveInitialDelay` on the server are no-ops.
