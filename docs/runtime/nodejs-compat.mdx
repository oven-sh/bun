...
🟡 Missing `syncBuiltinESMExports`, `Module#load()`. Overriding `require.cache` is supported for ESM & CJS modules (`module._cache` is the same object). Overriding `Module._load` or `Module._resolveFilename` is supported and intercepts CJS `require()`. `module._pathCache` is a no-op. `module.register` is not implemented; we recommend [`Bun.plugin`](/runtime/plugins) instead.
