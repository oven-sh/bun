...
🟢 Fully implemented. As a Bun extension, `new CompressionStream(format, { level })` sets the compression level: `0` to `9` for `gzip`, `deflate` and `deflate-raw`, `0` to `11` for `brotli`, and `1` to `22` for `zstd`. A level outside that range, or a number that is not an integer, throws a `RangeError`.
