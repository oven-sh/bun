...
🟢 Fully implemented. As in browsers, this is a `TransformStream` whose readable side has a `highWaterMark` of 0, so a `writer.write()` settles only once the readable side is read from. Node.js instead buffers up to 64 KiB of unread writes. Start reading (or `pipeTo()`) before awaiting a write.
...
🟢 Fully implemented. The same backpressure note as [`CompressionStream`](#compressionstream) applies: a `writer.write()` settles only once the readable side is read from.
