...
🟢 Fully implemented. `http.Server` does not extend `net.Server`. `listen({ fd })` serves on the file descriptor on Linux and macOS. In a `node:cluster` worker it emits an `ENOTSUP` error, where Node.js resolves the file descriptor in the primary. Bun ignores `listen(handle)` and the `ipv6Only` and `signal` options of `listen()`. `keepAlive`/`keepAliveInitialDelay` on the server are no-ops.
