...
subscriptions can only use the Pub/Sub methods (`.subscribe()`,
`.subscribeBuffer()`, `.unsubscribe()`) and `.ping()`. To send other commands
to Redis, create a separate connection with `.duplicate()`:
...
The message can be a string or binary data (`Buffer`, a typed array, or a
`Blob`). Binary payloads are written to the wire as-is:

```typescript redis.ts icon="/icons/typescript.svg"
await client.publish("updates", Buffer.from([0x00, 0x01, 0x02]));
```

### Subscriptions

Subscribe to channels with the `.subscribe()` method. The listener receives
each message decoded as a string:

```typescript redis.ts icon="/icons/typescript.svg"
await client.subscribe(channel, (message, channel) => {});
```

To receive a message's raw bytes instead, use `.subscribeBuffer()`. The
listener is called with a `Uint8Array` (a `Buffer`, like `.getBuffer()`), so
binary payloads round-trip unchanged. As with `.subscribe()`, publish from a
separate connection, since a subscribed client can only use the Pub/Sub
methods:

```typescript redis.ts icon="/icons/typescript.svg"
const subscriber = await client.duplicate();
await subscriber.subscribeBuffer("updates", (message, channel) => {
  console.log(message); // <Buffer 00 01 02>
});
await client.publish("updates", new Uint8Array([0, 1, 2]));
```

String and buffer listeners can be registered on the same channel; each
receives the message in its own form.

Unsubscribe with the `.unsubscribe()` method, which works the same way for
listeners registered with either `.subscribe()` or `.subscribeBuffer()`:
