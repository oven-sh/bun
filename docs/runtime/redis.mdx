...
subscriptions can only call `.ping()`, `.subscribe()`, `.unsubscribe()`,
`.psubscribe()` and `.punsubscribe()`. To send other commands to Redis, create
a separate connection with `.duplicate()`:
...
Subscribe to channels matching a glob-style pattern with `.psubscribe()`. The
listener receives the message and the concrete channel it was published to:
...
Unsubscribe from patterns with `.punsubscribe()`:

```typescript redis.ts icon="/icons/typescript.svg"
await client.punsubscribe(); // Unsubscribe from all patterns.
await client.punsubscribe(pattern); // Unsubscribe a particular pattern.
await client.punsubscribe(pattern, listener); // Unsubscribe a particular listener.
```
