...
listener is called with a `Uint8Array`, so binary payloads round-trip
unchanged. As with `.subscribe()`, publish from a separate connection, since a
subscribed client can only use the Pub/Sub methods:

```typescript redis.ts icon="/icons/typescript.svg"
const subscriber = await client.duplicate();
await subscriber.subscribeBuffer("updates", (message, channel) => {
  console.log(message); // Uint8Array(3) [ 0, 1, 2 ]
});
