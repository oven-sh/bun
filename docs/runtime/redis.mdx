...
5. An attempt that fails before the server accepts the handshake counts as one attempt. This includes an error reply to `HELLO` (for example a wrong password or `max number of clients reached`), a failed TLS handshake, and an expired `connectionTimeout`. If the last attempt fails this way, the client rejects the queued commands with that error.
