...
By default, Bun generates a `GET` URL that expires in 24 hours. Presigning is synchronous and purely local when keys are configured; with [ambient credentials](#ambient-credentials-profiles-sso-iam-roles) the first call resolves them (blocking once), so servers that only presign may want to `await Bun.aws.credentials()` at startup.
...
Resolution happens off the JavaScript thread on first use; the result is cached for the process and refreshed a few minutes before it expires. See [AWS & Google Cloud auth](/runtime/networking/cloud-auth) for the full order, `Bun.aws.credentials()`, and signing arbitrary AWS requests with `Bun.aws.fetch`.
