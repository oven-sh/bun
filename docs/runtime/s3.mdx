...
`end()` stores the bytes written so far under the key, also when you call it from a `catch` block. If the source of the data fails before the data is complete, pass the error to `end()`. The argument must be `instanceof Error`. With any other value, `end()` completes the upload as it does without an argument.

With an error, Bun does not complete the upload, so it does not create or replace the object under the key. An object that is already under the key stays as it is. Bun cancels the parts that wait in the queue. If S3 has already returned an upload id, Bun sends `AbortMultipartUpload` for it. The promise that `end(error)` returns and a pending `flush()` reject with `error`. Bun marks these rejections as handled, so you do not have to await them.
...
Bun sends S3 requests through the proxy that `HTTP_PROXY`, `HTTPS_PROXY`, `ALL_PROXY` and `NO_PROXY` select for the endpoint, the same way [`fetch`](/runtime/networking/fetch#proxying-requests) does.
