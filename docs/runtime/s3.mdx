...
Limits and permissions depend on the service. AWS S3 allows up to 10 tags per object, with keys up to 128 characters and values up to 256 characters, counted in UTF-16 code units. Writing tags requires `s3:PutObjectTagging` in addition to the upload permission. Reading requires `s3:GetObjectTagging`. Bun validates string values and Unicode encoding locally and leaves service-specific limits to the server. These operations require an endpoint that supports S3 object tagging. See the [AWS object tagging guide](https://docs.aws.amazon.com/AmazonS3/latest/userguide/object-tagging.html).
