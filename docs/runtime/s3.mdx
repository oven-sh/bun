...
With an error, Bun does not complete the upload, and nothing is stored under the key. Bun cancels the parts that wait in the queue. If S3 has already returned an upload id, Bun sends `AbortMultipartUpload` for it. The promise that `end(error)` returns and a pending `flush()` reject with `error`. Bun marks these rejections as handled, so you do not have to await them.
