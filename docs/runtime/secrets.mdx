...
- Persisted with the `CRED_PERSIST_LOCAL_MACHINE` flag: scoped to the current user on the current machine, and not roamed to other machines with a domain profile
