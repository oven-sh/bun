...
<Note>
  This API is mostly useful for local development tools. We may later add a `provider` option for production deployment
  secrets.
</Note>
...
- `options.timeout` (number, optional) - Milliseconds to wait for the credential store. Defaults to `30000`. See [Timeouts](#timeouts)
...
Every call waits at most `options.timeout` milliseconds for the credential store. The default is 30 seconds. When the deadline passes, the promise rejects with an error whose `code` is `ERR_SECRETS_TIMEOUT`, and the pending call no longer keeps the process alive. On Linux, Bun also cancels the libsecret call, which dismisses an open unlock prompt.

Pass `0`, `Infinity`, or `null` to wait until the credential store answers.

```typescript
try {
  await Bun.secrets.get({ service: "my-app", name: "token", timeout: 5_000 });
} catch (error) {
  if (error.code === "ERR_SECRETS_TIMEOUT") {
    console.error("The keyring did not answer within 5 seconds");
  }
}
```
