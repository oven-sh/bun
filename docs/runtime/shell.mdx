...
A buffer target holds a fixed number of bytes. When it is full, the shell closes the command's output pipe. A command that writes more gets `EPIPE` or `SIGPIPE` on its next write and stops, the same as a writer whose reader has gone away.
