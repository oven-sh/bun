...
A buffer target holds a fixed number of bytes. Output that fits is read to the end. When a command writes more than the buffer holds, the shell closes the command's output pipe. The command then gets `EPIPE`, `ECONNRESET`, or `SIGPIPE` on its next write and stops, the same as a writer whose reader has gone away.
