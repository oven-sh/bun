...
Bun Shell does not implement all of the POSIX shell language. A script that uses one of these constructs fails with an error before any command runs:

- loops and other compound commands: `for`, `while`, `until`, `case`, `select`
...
await $`for f in *.txt; do echo $f; done`;
// error: "for" is a reserved word that Bun Shell does not support yet.
// To run a command named "for", quote it.
```

To run a program whose name is one of these words, quote the name (`"for"`) or interpolate it (`${"for"}`).
