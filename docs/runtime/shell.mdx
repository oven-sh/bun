...
Bun reads the whole `Response` body when the command starts, so the body must be in memory by then. A `Response` whose body is a `ReadableStream` throws an error. So does a `fetch()` response that is still downloading. Buffer the body first:
