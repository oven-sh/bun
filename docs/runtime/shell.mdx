...
// Use the response body as stdin.
await $`cat < ${await response.blob()} | wc -c`; // 1256
```
...
- `Blob` (reads from the blob)
- `Response` (reads from the body)

Bun reads the whole `Response` body when the command starts, so the body must be in memory by then. A `Response` whose body is a `ReadableStream` throws an error. So does a `fetch()` response that is still downloading ([#42785](https://github.com/oven-sh/bun/issues/42785) tracks support for both). Buffer the body into a `Blob` first:
...
await $`cat < ${await response.blob()} | wc -c`;
```
