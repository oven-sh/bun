...
A buffer does not grow. If `>` or `2>` sends more output than the buffer holds, the buffer keeps the bytes that fit. The command still runs to its end, and then exits with a non-zero code.
