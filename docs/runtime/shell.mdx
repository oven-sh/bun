...
- `ReadableStream` (streams into the command's stdin)

### Example: Redirect a `ReadableStream` to stdin

A `ReadableStream` streams into the command as the command reads it. Bun does not buffer the whole stream first.
...
const result = await $`bun -e "process.stdin.pipe(process.stdout)" < ${response.body}`.text();
```

These rules apply to a `ReadableStream` redirect:

- Only `<` accepts a stream. `>` and `2>` throw.
- The command must be an external command. A [builtin command](#builtin-commands) throws. To give a stream to a builtin, buffer the stream first, for example `< ${await new Response(stream).blob()}`.
- The stream must be unlocked and unread. A locked or used stream throws.
- If the stream errors, Bun writes `bun: Failed to pipe ReadableStream to stdin: <error>` to the command's stderr and the command fails. The exit code is the command's own exit code when that is not zero, and `1` otherwise.
- If the command exits before it has read the whole stream, Bun cancels the stream.
- An async iterable is converted to a stream, as it is for the `stdin` of `Bun.spawn`.
