...
### `json` and `jsonb` values (PostgreSQL)

When a parameter has the type `json` or `jsonb`, Bun serializes an object or an array with `JSON.stringify`. Bun sends a string as it is, so the string must contain JSON text. `node-postgres` treats a string the same way. Code that calls `JSON.stringify` on the value first works with both clients.

```ts
// Both statements store the object {"theme": "dark"}
await sql`INSERT INTO settings (data) VALUES (${{ theme: "dark" }})`;
await sql`INSERT INTO settings (data) VALUES (${JSON.stringify({ theme: "dark" })})`;

// To store a string, a number, a boolean, or null, pass it through JSON.stringify.
// A bare null binds as SQL NULL, not as the JSON value null.
await sql`INSERT INTO settings (data) VALUES (${JSON.stringify("dark")})`;
await sql`INSERT INTO settings (data) VALUES (${JSON.stringify(null)})`;
```

A string that is not JSON text, such as `${"dark"}`, fails with `invalid input syntax for type json`.
