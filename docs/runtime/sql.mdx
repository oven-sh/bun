...
Without an explicit type, strings bind as `TEXT`, integers as `INTEGER` or `BIGINT`, other numbers as `DOUBLE PRECISION`, booleans as `BOOLEAN`, dates as `TIMESTAMPTZ`, buffers as `BYTEA`. Objects, mixed kinds and empty arrays bind as `JSON`. `null` and `undefined` elements bind as `NULL`.
