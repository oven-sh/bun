...
A `number` parameter binds as an integer type (`int4` or `int8` on PostgreSQL, `INT` or `BIGINT` on MySQL) when the value is a safe integer (`Number.isSafeInteger()`). Every other `number`, and `-0`, binds as a double (`float8` or `DOUBLE`). The parameter type also sets the type of a result that derives from it. `select ${2 ** 40} as x` returns `x` as a string, because the result column is `int8`.
