...
On SQLite, a multi-statement string reports the first statement's command, and `affectedRows` reports the batch's total changes when that first statement is a write (otherwise `0`). On PostgreSQL and MySQL, a query that produces several result sets (for example `.simple()` with several statements) resolves to a plain array of result arrays, and the metadata is on each inner array.
