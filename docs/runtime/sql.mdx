...
`release()` closes the client at once: new queries and `begin()` calls reject. If a transaction started with `reserved.begin()` is still running, the connection goes back to the pool after that transaction commits or rolls back. The promise `release()` returns resolves when the pool has the connection back.
