...
To cancel a query, call `cancel()` on the query object.

```ts
const query = sql`SELECT pg_sleep(30)`.execute();
setTimeout(() => query.cancel(), 100);

// PostgresError: canceling statement due to user request (SQLSTATE 57014)
await query;
```

On PostgreSQL, what `cancel()` does depends on how far the query got:

- A query that Bun has not sent yet never runs. It rejects with `ERR_POSTGRES_QUERY_CANCELLED`.
- A query that the server is running can only be stopped from a separate connection. Bun opens one to the same server, encrypted when the session is, and sends a `CancelRequest` on it. This is a request. If the server stops the query, the query rejects with SQLSTATE `57014`. If the query finishes first, it resolves as usual, and the request can stop the next query on that connection instead. The server also ignores a request that arrives before the query does, which can happen when you cancel a query with megabytes of parameters at once.
- A query that Bun already sent behind another query on the same connection also rejects with `ERR_POSTGRES_QUERY_CANCELLED`, but the server still runs it. The `hint` of the error says so. An `INSERT` or `UPDATE` in such a query still takes effect, so do not retry it blindly.
