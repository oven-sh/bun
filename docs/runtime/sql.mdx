...
- Several `listen()` calls on one channel share a single server-side subscription and each callback receives every notification. `unlisten(channel, callback)` removes one of them; `unlisten(channel)` removes them all. A callback (either argument) that throws is reported as an uncaught exception and stays subscribed.
