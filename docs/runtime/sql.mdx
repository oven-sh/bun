...
You can call `close()` on the pool again while it is still closing. Every call resolves when the first `close()` resolves, and a later `close({ timeout: 0 })` closes immediately.
