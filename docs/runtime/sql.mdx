...
A `number` parameter binds as an integer type (`int4` or `int8` on PostgreSQL, `INT` or `BIGINT` on MySQL) when the value is a safe integer (`Number.isSafeInteger()`). Every other `number`, and `-0`, binds as a double (`float8` or `DOUBLE`).
