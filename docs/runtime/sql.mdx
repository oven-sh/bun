...
Bun returns numbers that exceed the range of a 53-bit integer as strings:
...
The client automatically handles authentication plugin switching when the server requests it, including secure password exchange over non-SSL connections.
