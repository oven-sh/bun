...
- A query that the server is running can only be stopped from a separate connection, so Bun opens one and sends a `CancelRequest` on it. This is a request. If the server stops the query, the query rejects with SQLSTATE `57014`. If the query finishes first, it resolves as usual.
