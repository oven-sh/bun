...
const sql = new SQL({
  ...
  // Decode DATE/DATETIME/TIMESTAMP as UTC instead of local time (default: false)
  utcDate: false,
...
const d = new Date("2024-01-15T05:30:45.678Z");
await sql`INSERT INTO t (ts) VALUES (${d})`;
const [{ ts }] = await sql`SELECT ts FROM t`;
console.log(ts.toISOString() === d.toISOString()); // true, in any TZ
```

The default is `false` for backwards compatibility. This option currently only affects the MySQL / MariaDB adapter.
...
| FLOAT, DOUBLE                           | number                   |                                                                                                      |
| DATE                                    | Date                     | Components interpreted as local time, or UTC with the `utcDate` option                               |
| DATETIME, TIMESTAMP                     | Date                     | Components interpreted as local time, or UTC with the `utcDate` option                               |
