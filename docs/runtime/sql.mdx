...
- `sql.notify(channel, payload?)` publishes a payload to a channel (an omitted payload sends `""`).
...
The subscription is an async disposable, so `await using` removes the listener automatically at the end of the scope:

```ts
{
  await using sub = await sql.listen("user_events", payload => console.log(payload));
  // ...
} // scope exit runs the equivalent of `await sub.unlisten()`
```

##### Connection lifecycle

Each `Bun.SQL` instance opens at most one **dedicated** PostgreSQL connection for `LISTEN`. All channels share that single connection, separate from the regular query pool. The connection is created lazily on the first `sql.listen()` call and closed when the last subscription is removed (a later `listen()` recreates it) or on `sql.close()`.
...
The payload is optional: `sql.notify(channel)` sends an empty payload, matching PostgreSQL's bare `NOTIFY channel` for signal-only channels where the channel name is the whole message. A non-string payload (including `null`) is rejected rather than coerced.

The handles returned by `sql.reserve()` and `sql.begin()` carry the same three methods. Their `notify()` runs on that handle's own connection, so inside `sql.begin()` the notification participates in the transaction: PostgreSQL queues it and delivers it only on COMMIT, and drops it on ROLLBACK. `listen()`/`unlisten()` always operate on the adapter-wide dedicated listen connection, regardless of which handle they are called on.
