...
The `sql.array` helper binds a JavaScript array as a typed PostgreSQL array parameter:

```ts
// The element type is inferred from the values
await sql`INSERT INTO tags (items) VALUES (${sql.array(["red", "blue", "green"])})`;
// Binds: $1::TEXT[] with '{"red","blue","green"}'

// Works with numeric arrays too
await sql`SELECT * FROM products WHERE id = ANY(${sql.array([1, 2, 3])})`;
// Binds: $1::INTEGER[] with '{1,2,3}'

// Pass the element type explicitly when inference is not enough
await sql`INSERT INTO events (ids) VALUES (${sql.array(["a0eebc99-..."], "UUID")})`;
```

Without an explicit type, strings bind as `TEXT`, integers as `INTEGER` or `BIGINT`, other numbers as `DOUBLE PRECISION`, bigints as `BIGINT` (or `NUMERIC` when one is outside the 64-bit range), booleans as `BOOLEAN`, dates as `TIMESTAMPTZ`, buffers as `BYTEA`. A mix of numbers and bigints binds as `BIGINT` or `NUMERIC`. Objects, any other mix of kinds, empty arrays and arrays of only `null` or `undefined` bind as `JSON`. `null` and `undefined` elements bind as `NULL`.
