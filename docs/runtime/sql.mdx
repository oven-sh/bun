...
Without an explicit type, strings bind as `TEXT`, integers as `INTEGER` or `BIGINT`, other numbers as `DOUBLE PRECISION`, bigints as `BIGINT` (or `NUMERIC` when one is outside the 64-bit range), booleans as `BOOLEAN`, dates as `TIMESTAMPTZ`, buffers as `BYTEA`. Objects, mixed kinds, empty arrays and arrays of only `null` bind as `JSON`. `null` and `undefined` elements bind as `NULL`.
