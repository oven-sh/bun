...
- `affectedRows`: the number of rows an `INSERT`, `UPDATE`, or `DELETE` changed. `0` for statements that do not write. On SQLite, `null` when the count is unknown: a write wrapped in a CTE without `RETURNING`.
