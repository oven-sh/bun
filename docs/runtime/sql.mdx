...
To cancel a running query, call `cancel()` on the query object. A cancelled query
...
PostgreSQL can only stop a running query from a separate connection, so `cancel()`
opens one and sends a `CancelRequest` on it. A query that has not reached the
server yet never runs at all, and rejects with `ERR_POSTGRES_QUERY_CANCELLED`.
