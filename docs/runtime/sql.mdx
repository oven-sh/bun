...
const { state, unlisten } = await sql.listen("orders", payload => {
  console.log("new order", JSON.parse(payload));
});
console.log("listening on backend", state.pid);
...
`listen()` resolves once PostgreSQL has acknowledged the `LISTEN`, so a `notify()` issued after it resolves is delivered. The subscription is also an async disposable:
...
- Several `listen()` calls on one channel share a single server-side subscription and each callback receives every notification. `unlisten(channel, callback)` removes one of them; `unlisten(channel)` removes them all. A callback that throws is reported as an uncaught exception.
- `state` is one object per client, updated in place on reconnect, so a reference from any `listen()` always describes the backend currently holding the subscriptions.
- Channel names can be any non-empty string and are quoted as identifiers for you. PostgreSQL limits payloads to 8000 bytes by default.
...
The payload is optional: `sql.notify("cache-invalidated")` is PostgreSQL's bare `NOTIFY`. Reserved and transaction handles also have `listen()` and `unlisten()`; those always use the client's shared listen connection.
