...
A `verify-ca` or `verify-full` sslmode (from the connection string, `PGSSLMODE`, or the `tls` option) always verifies the server certificate. `NODE_TLS_REJECT_UNAUTHORIZED=0` relaxes the default for `require` and `prefer` only. Pass `tls: { rejectUnauthorized: false }` to opt out explicitly.
