...
The `lastInsertRowid` property is the ID of the last row inserted into the database. The `changes` property is the number of rows that the query inserts, updates, or deletes itself. Rows that a trigger or a foreign key action such as `ON DELETE CASCADE` changes do not count. A schema statement such as `CREATE VIRTUAL TABLE` can report rows that a virtual table module, for example FTS5, writes to its own tables.
