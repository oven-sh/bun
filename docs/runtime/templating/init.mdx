...
`bun init` infers settings with sane defaults and is non-destructive when run multiple times. When a `package.json` already exists, `bun init` only adds the fields that are missing. It never changes an existing `type` or `private` value, keeps the file's indentation, and prints `~ package.json (updated)` if it changed anything.
