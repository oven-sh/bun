...
Bun's TOML parser implements the full [TOML v1.1.0 specification](https://github.com/toml-lang/toml/releases/tag/1.1.0) and passes the complete official [toml-test](https://github.com/toml-lang/toml-test) conformance suite, with one deliberate deviation: integers beyond JavaScript's safely representable range throw instead of returning an imprecise number.

- **Strings**: basic (`"..."`) and literal (`'...'`), including multi-line, with all escapes (`\uXXXX`, `\UXXXXXXXX`, and TOML 1.1's `\xHH` and `\e`)
