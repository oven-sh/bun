...
<Warning>
  **Unstable.** This method exposes Bun's internal AST. The output is **not**
  ESTree-compatible and the shape (node kinds, field names, nesting) may change
  or be removed in any patch release without a deprecation period. Use it for
