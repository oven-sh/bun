...
Parse code and return the AST as a plain JavaScript object tree. Every node has
a `kind` (a snake_case tag like `"s_local"`, `"e_binary"`, `"b_identifier"`)
and a `loc` (byte offset into the input, or `null`).

```ts example.ts icon="/icons/typescript.svg"
const t = new Bun.Transpiler({ loader: "ts" });
const ast = t.unstable_parse(`export const x: number = 1 + 2;`);

console.log(ast.stmts[0]);
// {
//   kind: "s_local",
//   loc: 7,
//   declKind: "k_const",
...
The top-level result carries `stmts`, `symbols`, `importRecords`, `hashbang`,
`directive`, `exportsKind`, and `approximateNewlineCount`. Identifier nodes
resolve their `Ref` to the symbol's original name string.
...
class Bun.Transpiler {
  ...
  unstable_parse(code: string, loader?: Loader): UnstableAST
}
