...
Transpile code synchronously with the `.transformSync()` method. Modules are not resolved and the code is not executed. The result is a string of vanilla JavaScript code, unless the transpiler was constructed with `sourcemap: "external"` or `"linked"`, in which case it is a `{ code, map }` object — see [Source maps](#source-maps).
