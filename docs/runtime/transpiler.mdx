...
[Macros](/bundler/macros) run in a dedicated JavaScript VM on its own thread; the calling thread waits for the result. They do not share globals or module state with your application. The optional `context` object passed to `transformSync` reaches the macro as a structured clone, appended after the call's own arguments.
