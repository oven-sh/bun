...
// Plain text slicing (works like String.prototype.slice)
sliceAnsi("hello world", 0, 5); // "hello"
sliceAnsi("hello world", 6, 11); // "world"

// Preserves ANSI color codes
sliceAnsi("\x1b[31mhello\x1b[39m", 1, 4); // "\x1b[31mell\x1b[39m"

// Negative indices work like String.prototype.slice
sliceAnsi("hello", -2); // "lo"
sliceAnsi("hello", 0, -1); // "hell"
sliceAnsi("hello", -3, -1); // "ll"
```

### CJK and full-width characters

CJK (Chinese, Japanese, Korean) characters are counted as width 2, matching how they display in terminals:

```ts
// CJK characters (width 2 each)
Bun.sliceAnsi("你好世界", 0, 4); // "你好" (2 chars × 2 width = 4)
Bun.sliceAnsi("你好世界", 4, 8); // "世界"

// Mixed ASCII and CJK
Bun.sliceAnsi("a你b好c", 0, 1); // "a"
Bun.sliceAnsi("a你b好c", 1, 3); // "你" (width 2)
Bun.sliceAnsi("a你b好c", 3, 4); // "b"

// Japanese text
Bun.sliceAnsi("日本語テスト", 0, 4); // "日本"

// Korean text
Bun.sliceAnsi("한국어", 0, 2); // "한"
Bun.sliceAnsi("한국어", 2, 4); // "국"
Bun.sliceAnsi("한국어", 4, 6); // "어"

// Colored CJK text (styles preserved)
Bun.sliceAnsi("\x1b[31m你好\x1b[39m世界", 0, 4); // "\x1b[31m你好\x1b[39m"
Bun.sliceAnsi("\x1b[31m你好\x1b[39m世界", 2, 6); // "\x1b[31m好\x1b[39m世"
```

### Emoji handling

Emoji and grapheme clusters are handled correctly, never splitting multi-codepoint sequences:

```ts
// Basic emoji (width 2)
Bun.sliceAnsi("👋hello", 0, 2); // "👋"
Bun.sliceAnsi("👋hello", 2, 7); // "hello"

// Emoji with skin tone modifier (single grapheme, width 2)
Bun.sliceAnsi("👋🏽hello", 0, 2); // "👋🏽"

// Flag emoji (regional indicators, width 2)
Bun.sliceAnsi("🇺🇸hello", 0, 2); // "🇺🇸"

// ZWJ family emoji (width 2, never split)
Bun.sliceAnsi("👨‍👩‍👧‍👦hello", 0, 2); // "👨‍👩‍👧‍👦"
Bun.sliceAnsi("👨‍👩‍👧‍👦hello", 2, 7); // "hello"
...
Use the `ellipsis` option to add an ellipsis when content is cut. This makes `Bun.sliceAnsi` a drop-in replacement for the `cli-truncate` npm package:

```ts
// Truncate from end
Bun.sliceAnsi("unicorn", 0, 4, "…"); // "uni…"
Bun.sliceAnsi("unicorn", 0, 4, "..."); // "u..."

// Truncate from start (negative index)
Bun.sliceAnsi("unicorn", -4, undefined, "…"); // "…orn"

// The ellipsis counts against the visible-width budget
Bun.sliceAnsi("hello world", 0, 8, "…"); // "hello w…" (7 chars + 1 ellipsis = 8)

// Ellipsis inherits active styles (colors, bold, etc.)
Bun.sliceAnsi("\x1b[31mhello world\x1b[39m", 0, 6, "…");
// The ellipsis will be red too
```

### Negative indices

Negative indices count from the end of the string, just like `String.prototype.slice`:

```ts
// Negative start
Bun.sliceAnsi("hello world", -5); // "world"
Bun.sliceAnsi("hello world", -11); // "hello world"

// Negative end
Bun.sliceAnsi("hello world", 0, -6); // "hello"
Bun.sliceAnsi("hello world", 0, -1); // "hello worl"

// Both negative
Bun.sliceAnsi("hello world", -5, -1); // "worl"

// Works with CJK (remember: width 2 per character)
Bun.sliceAnsi("你好世界", -4); // "世界"
Bun.sliceAnsi("你好世界", 0, -4); // "你好"
```
...
interface SliceAnsiOptions {
  /**
   * If set, insert this string at cut edges. Counted against the width budget.
   * The ellipsis appears inside active SGR styles but outside hyperlinks.
   */
  ellipsis?: string;

  /**
   * Treat East Asian Width "Ambiguous" characters as 1 column (narrow)
   * instead of 2 (wide). Affects Greek, Cyrillic, and some symbols.
   *
   * @default true
   */
  ambiguousIsNarrow?: boolean;
...
You can pass options in multiple ways for convenience:

```ts
// Full options object
Bun.sliceAnsi(str, 0, 10, { ellipsis: "…", ambiguousIsNarrow: true });

// String shorthand for ellipsis only
Bun.sliceAnsi(str, 0, 10, "…");

// Boolean shorthand for ambiguousIsNarrow only
Bun.sliceAnsi(str, 0, 10, false);

// Positional ambiguousIsNarrow (when using ellipsis)
Bun.sliceAnsi(str, 0, 10, "…", false);
```
