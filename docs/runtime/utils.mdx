...
test("peek", () => {
  ...
  expect(peek(rejected)).toBeInstanceOf(Error);
  expect(peek(rejected)).toHaveProperty("message", "Successfully tested promise rejection");

  // The rejection is still unhandled, so handle it here.
  // `bun test` fails a test that leaves a rejection unhandled.
  rejected.catch(() => {});
});
```
...
test("peek.status", () => {
  ...
  // `peek.status` does not mark the promise as handled either
  rejected.catch(() => {});
});
```
