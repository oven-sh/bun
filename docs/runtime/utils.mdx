...
Following that algorithm, a `Buffer` comes back as a `Uint8Array`. Pass `preserveBuffers: true` to get a `Buffer` back instead, which is what `node:v8`'s `serialize` does.

```js
import { serialize, deserialize } from "bun:jsc";

deserialize(serialize(Buffer.from("bun"))); // => Uint8Array
deserialize(serialize(Buffer.from("bun"), { preserveBuffers: true })); // => Buffer
```
