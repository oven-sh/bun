...
The `timestamp` parameter defaults to the current time in milliseconds. When `timestamp` is omitted, successive calls are strictly increasing: the counter is reseeded to a new pseudo-random integer whenever the clock moves forward (the high bit of the 12-bit counter is kept clear so at least 2048 values remain before rollover), and if the clock has not advanced Bun reuses the last emitted timestamp and increments the counter. On counter rollover Bun bumps the emitted timestamp forward instead of wrapping the counter (RFC 9562 §6.2). When you pass a `timestamp` explicitly it is embedded verbatim (useful for backfilling historical rows); the counter is reseeded whenever the value changes and otherwise increments within the 12-bit field. Explicit calls keep their own counter state and never influence zero-argument calls. The counter is atomic and threadsafe, so calls to `Bun.randomUUIDv7()` from many Workers in the same process at the same timestamp don't produce colliding counter values.
