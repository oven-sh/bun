...
<Note>
  ...
  refreshing the page. Bun's `--hot` is the server-side equivalent of this experience. For browser HMR, use Bun's
  full-stack development server, which supports Hot Module Replacement out of the box — see [Hot
...
<Note>
**Note** — For client-side hot reloading with `import.meta.hot` (modeled after Vite's HMR API), use Bun's full-stack development server. See [Hot reloading](/bundler/hot-reloading).
