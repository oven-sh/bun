...
Bun's watcher relies on the operating system's native file-change APIs (inotify on Linux, kqueue on macOS, `ReadDirectoryChangesW` on Windows). These do not fire for files that live on certain mounted filesystems, including:

- Docker bind mounts where the host is Windows or macOS
- WSL paths under `/mnt/c/...`
- NFS, SMB, and other network shares

On Linux, Bun automatically falls back to polling when it detects that the project root is on one of these filesystems (9p, NFS, SMB/CIFS) and prints a note. For other setups, or to force the behavior, set `BUN_WATCHER_USE_POLLING=1` to switch `--watch` and `--hot` to a polling backend that periodically `stat()`s each watched file:

```bash terminal icon="terminal"
BUN_WATCHER_USE_POLLING=1 bun --watch index.ts
```

The poll interval defaults to 100 ms and can be changed with `BUN_WATCHER_POLL_INTERVAL` (in milliseconds). Polling uses more CPU than native watching; set `BUN_WATCHER_USE_POLLING=0` to disable it if Bun enables it and you prefer native events.

Polling tracks changes to files that are already in the module graph. Creating a new file that satisfies a previously failing import is not detected until a watched file is edited or the process is restarted.
