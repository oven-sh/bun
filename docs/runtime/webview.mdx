...
Pass `userAgent` to replace the engine's default `User-Agent` string. It applies to every request the view makes and to `navigator.userAgent` in the page:

```ts
const view = new Bun.WebView({
  userAgent: "my-bot/1.0 (+https://example.com/bot)",
});
```
...
`navigate()` resolves when the main frame's `load` event fires. After it resolves, `view.url`, `view.title` and `view.status` reflect the new page, and `view.loading` is `false`.
...
A server that answers with an error status (`404`, `500`, `429`) still delivers a page, so `navigate()` resolves. Read `view.status` to tell an error page from content:

```ts
await view.navigate("https://example.com/missing");
if (view.status !== 200) {
  console.log("server answered", view.status); // 404
}
```

`view.status` is `null` when the page did not come from an HTTP response (`data:`, `about:blank`, `file:`).
...
<Note>
  With the Chrome backend, `onNavigated` fires when the navigation commits, before the title and status are known, so
  `title` and `status` are `undefined` in the callback. Read `view.title` and `view.status` after `navigate()` resolves.
</Note>
...
| `userAgent` | `string`                                        | engine default                            | The `User-Agent` header and `navigator.userAgent`. See [User agent](#user-agent).              |
...
| `status`             | `number \| null` _(readonly)_            | The main frame's HTTP status code. `null` for `data:`, `about:blank` and `file:` pages.     |
| `loading`            | `boolean` _(readonly)_                   | `true` while a navigation is in flight.                                                     |
| `onNavigated`        | `((url, title, status) => void) \| null` | Fires after each successful navigation, before the `navigate()` promise resolves.           |
