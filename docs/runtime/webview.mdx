...
<Note>
  ...
  Views that omit `dataStore` share that process-wide default context, cookies included. Pass `dataStore: "ephemeral"`
  explicitly to give a view a browser context of its own: its cookies and storage are shared with no other view and are
  discarded when it closes.
</Note>

### Per-view proxy (Chrome)

With the Chrome backend, `proxy` routes one view's requests through a proxy server. Each view can have a different one. A string names the server; the object form adds `bypass`, a list of hosts that skip the proxy in Chrome's bypass-rule syntax.
