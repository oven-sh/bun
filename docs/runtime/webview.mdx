...
By default, each view uses **ephemeral** in-memory storage of its own — cookies, `localStorage`, IndexedDB, and cache are shared with no other view and are discarded when the view closes. To persist state across runs, pass a directory:
...
<Note>
  ...
  not per-view. Bun spawns Chrome once per Bun process, so the first view's directory wins for all subsequent views, and
  every view with a `directory` shares that process-wide default context, cookies included. An ephemeral view gets a
  browser context of its own (a CDP `Target.createBrowserContext`) inside that one Chrome. If Chrome refuses to create
  one (an enterprise policy that disables incognito), the view's first `navigate()` rejects with an error that says so,
  and `dataStore: {directory}` is the way to use the default context.
</Note>

### Per-view proxy (Chrome)

With the Chrome backend, `proxy` routes one view's requests through a proxy server. Each view can have a different one. A string names the server; the object form adds `bypass`, a list of hosts that skip the proxy in Chrome's bypass-rule syntax. Chrome skips the proxy for loopback addresses on its own; the rule `"<-loopback>"` removes that implicit bypass.

```ts
const view = new Bun.WebView({
  backend: "chrome",
  proxy: { server: "http://proxy.internal:8080", bypass: ["*.internal", "<-loopback>"] },
});
```

A proxied view always uses in-memory storage, so `proxy` cannot be combined with `dataStore: { directory }`.
...
By default, before spawning, Bun reads the `DevToolsActivePort` file from standard profile directories to check whether a Chrome-family browser is **already running** with remote debugging enabled. If found, Bun connects to that browser over WebSocket instead of spawning a new one — your views open as windows in your existing browser. An ephemeral view (the default) opens as an incognito window with a browser context of its own, so it does not see the browser's logged-in cookies. Pass `dataStore: { directory }` to open the view in the browser's own profile instead; Bun did not launch that browser, so the directory value is not used.
...
| `proxy`     | `string` \| `{ server: string, bypass?: string[] }` | —                                         | (`chrome` only) Proxy for this view's requests. See [Per-view proxy](#per-view-proxy-chrome).  |
