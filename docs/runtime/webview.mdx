...
if (view.status !== null && view.status >= 400) {
...
With the WebKit backend, `view.status` is already updated when `onNavigated` runs. With the Chrome backend, `onNavigated` fires when the navigation commits, and `view.title` and `view.status` are updated when `navigate()` resolves.
