...
With `dataStore: "ephemeral"`, a view uses in-memory storage of its own — cookies, `localStorage`, IndexedDB, and cache are shared with no other view and are discarded when the view closes. This is the default on the WebKit backend. On the Chrome backend, views that omit `dataStore` share one process-wide context (see the note below). To persist state across runs, pass a directory:
...
Views that share the same `directory` share cookies and storage. Pass `dataStore: "ephemeral"` to opt back into in-memory storage explicitly.
...
<Note>
  ...
  Views that omit `dataStore` share that process-wide default context, cookies included. Isolation is not the default
  here: pass `dataStore: "ephemeral"` explicitly to give a view a browser context of its own.
</Note>
