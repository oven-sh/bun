...
Bun queues messages posted to a worker until the worker installs a `"message"` listener (`self.onmessage` or `self.addEventListener("message", ...)`), so you don't need to wait for the `"open"` event before sending, and a worker that installs its handler after a top-level `await` still receives everything in order. (Browsers deliver as soon as the worker script has run and drop messages that arrive before a handler exists.)
