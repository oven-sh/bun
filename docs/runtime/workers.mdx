...
A Web Worker can request its own cooperative shutdown with `self.close()` (the WHATWG [`DedicatedWorkerGlobalScope#close`](https://html.spec.whatwg.org/multipage/workers.html#dom-dedicatedworkerglobalscope-close) API). Unlike `process.exit()`, the task that called `close()` runs to completion, so any `postMessage` in the same synchronous block still reaches the parent. Tasks already queued on the worker's event loop (buffered `"message"` events, pending `setTimeout(fn, 0)`, etc.) are discarded per the spec. The parent sees a clean `CloseEvent` with `code: 0`, `wasClean: true`.

```ts worker.ts icon="/icons/typescript.svg"
self.postMessage("goodbye");
self.close();
```

`self.close()` is defined on Web Workers (`new Worker(url)`) and on the main thread (where it is a no-op); it is not a global inside `node:worker_threads` workers, matching Node.js.
