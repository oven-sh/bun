...
`self.close()` is only defined on Web Workers (`new Worker(url)`); it is not a global inside `node:worker_threads` workers, matching Node.js. On the main thread it is a no-op.
