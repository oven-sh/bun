...
- External DTDs and external entities are never fetched or read, so there is no XXE surface. A reference to an entity that could only have been declared in an external DTD is kept as written (`&nbsp;` stays `&nbsp;`).
...
A value with a string `name` and a `children` or `attributes` property is written as a node; anything else is a compact object and must have exactly one key naming the root element. Strings, numbers, booleans, bigints and `Date`s (as ISO strings) become text, `null` becomes an empty element, and `undefined`, functions and symbols are skipped — the same rules as `JSON.stringify`.
