...
`Bun.XML` is a value transform like `JSON.parse` rather than an event (SAX-style) or DOM API, and it is not a drop-in replacement for an existing npm package. Its compact shape follows the `@attr` / `#text` convention that xmltodict, Json.NET and others share, so moving over is mostly a matter of key names:
...
| values                          | strings, as written                                        | `"007"` → `7`, `"true"` → `true`, trimmed, by default       | strings                                  |
