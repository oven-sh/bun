...
The `git mv` pass **is** the §6 relocation table from the plan — it's the prerequisite that makes the Layer-1-⊬-`bun_jsc` rule enforceable in Zig *before* any Rust exists. Do it first; it derisks the crate boundary and gives agents an unambiguous target path for every file.
