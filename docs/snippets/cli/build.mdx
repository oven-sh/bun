...
<ParamField path="--react-compiler" type="boolean">
  Run the React Compiler over `.jsx`/`.tsx` files, automatically memoizing
  components and hooks. Output mode is derived from `--target` (`browser` →
  client, `bun`/`node` → ssr). Experimental.
</ParamField>
