...
<ParamField path="--no-prelink-modules" type="boolean">
  With <code>--compile --bytecode --format=esm</code>: don't embed the pre-resolved module graph
</ParamField>

<ParamField path="--startup-jit-deferral" type="number" default="1500">
  With <code>--compile</code>: keep the JIT reluctant to compile until the program becomes interactive, for at most this many
  milliseconds (<code>--startup-jit-deferral=&lt;ms&gt;</code>). <code>--no-startup-jit-deferral</code> or <code>0</code> turns it off
</ParamField>
