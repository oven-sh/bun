...
<ParamField path="--css-chunking" type="boolean">
  Write one shared CSS file for entry points whose CSS imports are identical, instead of one identical file each. Bun
  has done this by default since v1.3.10, so the flag currently has no additional effect. See [CSS > Multiple entry
