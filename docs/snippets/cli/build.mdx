...
<ParamField path="--react-compiler" type="boolean">
  **Experimental.** Automatically memoize React components and hooks with React Compiler, built into the bundler — no Babel plugin or extra dependencies required. Compiled output uses <code>react/compiler-runtime</code>, which ships with React 19+. Honors <code>"use no memo"</code> and skips <code>node_modules</code>
</ParamField>
