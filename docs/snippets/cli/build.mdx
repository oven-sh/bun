...
<ParamField path="--no-split-require" type="boolean">
  With `--splitting` and `--target bun`, keep a `require()`'d ES module in the calling chunk instead of emitting a chunk
  that Bun loads at the call
</ParamField>
