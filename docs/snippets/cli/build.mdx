...
<ParamField path="--compile-jit-policy" type="number" default="1">
  Multiply JavaScriptCore's JIT tier-up thresholds by this in the executable until it calls
  <code>Bun.unsafe.setJITPolicy(1)</code>; <code>1</code> is the normal JIT policy
</ParamField>
