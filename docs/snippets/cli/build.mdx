...
<ParamField path="--css-chunking" type="boolean">
  When several entry points need exactly the same stylesheets in the same order, write that CSS once and point each of
  them at the one file. Since v1.3.10 Bun always does this for JavaScript and HTML entry points, so the flag only adds
  `.css` files passed directly as entry points to the sharing. See [CSS > Multiple entry
  points](/bundler/css#multiple-entry-points).
</ParamField>
