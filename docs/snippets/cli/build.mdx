...
<ParamField path="--internal" type="string">
  Never externalize these modules (supports wildcards). Takes precedence over
  <code>--external</code> and <code>--packages external</code>. Useful with
  <code>--packages external</code> to keep specific dependencies in the bundle.
</ParamField>
