...
<ParamField path="--zod-compiler" type="boolean">
  Compile zod v4 schemas at build time. A schema expression such as `z.object({ ... })` becomes a small wrapper whose
  `parse` and `safeParse` run a validator compiled from the schema. The wrapper only constructs the real zod schema
  when a parse fails or when anything else on it is used, so errors always come from zod itself. Schemas the
  compiler cannot analyze are left as written. Experimental.
</ParamField>
