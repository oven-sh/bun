...
<ParamField path="--no-bundle" type="boolean">
  Transpile only — do not bundle. Each entry point is transpiled on its own and its imports are left as written.
  Transpiler options (`--define`, `--env`, `--drop`, `--loader`, the `--jsx-*` flags, `--minify`, `--tsconfig-override`)
  and output options (`--outdir`, `--outfile`, `--root`, `--entry-naming`, `--watch`) apply. Options that describe a
  bundle (`--splitting`, `--external`, `--packages`, `--banner`, `--footer`, `--metafile`, `--metafile-md`,
  `--bytecode`) are accepted but have no effect in this mode. `--compile` cannot be combined with `--no-bundle`
</ParamField>
