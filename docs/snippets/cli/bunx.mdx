...
<ParamField path="--minimum-release-age" type="number">
  ...
  threshold. A warm `bunx` cache is treated as stale whenever this flag is set to a positive value, so a previous
  cached install can never bypass the gate. The value `0` is the documented disable spelling and leaves the cache
  untouched.
</ParamField>
