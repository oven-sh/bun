...
<ParamField path="--offline" type="boolean">
  Disallow network requests; fail if a package is not available locally
</ParamField>

<ParamField path="--tarball-dir" type="string">
  Read package tarballs from a directory of `<name>@<version>.tgz` files instead of downloading them
</ParamField>
