...
<ParamField path="--no-addons" type="boolean">
  Throw an error if <code>process.dlopen</code> is called, disable <code>bun:ffi</code>, and disable export condition{" "}
  <code>node-addons</code>
</ParamField>

<ParamField path="--no-ffi-cc" type="boolean">
  Disable <code>bun:ffi</code>: <code>dlopen()</code>, <code>cc()</code>, <code>JSCallback</code> and every other FFI
