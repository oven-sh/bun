...
<ParamField path="--interactive" type="boolean">
  Open the Node.js-compatible REPL (<code>node:repl</code>). When combined with <code>-e</code>, evaluates the script first, then enters the REPL. Distinct from <code>bun repl</code>, which is Bun's native REPL.
</ParamField>
