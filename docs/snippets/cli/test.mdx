...
<ParamField path="--changed" type="string">
  Run only the test files affected by files that git reports as changed. Pass a commit, branch, or tag to compare
  against, for example <code>--changed=main</code>. Without a value, compares against <code>HEAD</code>.
</ParamField>
