...
When one argument is a path, every argument is treated as a path. A path that does not exist, or a directory with no test files, is reported as `Test filter "<path>" had no matches`. The other paths still run, and the exit code is 1. Pass `--pass-with-no-tests` to exit 0 instead.
