...
When one argument is a path, every argument is treated as a path. A path that selects no test file is reported as `Test filter "<path>" had no matches`. The other paths still run. If a path does not exist, the exit code is 1. Pass `--pass-with-no-tests` to exit 0 instead.
