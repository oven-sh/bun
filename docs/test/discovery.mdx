...
If no test file is found, or a filter or `--test-name-pattern` matches nothing, `bun test` reports it and exits with code 1. Pass `--pass-with-no-tests` to exit with code 0 instead, for example in a shared CI step that runs before a package has any tests:
