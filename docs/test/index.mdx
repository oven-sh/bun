...
In CI, make sure the ref exists in the checkout. `actions/checkout` fetches only the triggering commit by default, so `git diff origin/main` fails until you fetch that branch:
...
**3. Skip the tests your change cannot reach: [`--changed`](#run-only-affected-tests-with---changed).** On a pull request, `bun test --changed=origin/main` runs only the test files whose import graph reaches a file in the diff. Fetch the base branch first (see [`--changed` with `--shard`](#--changed-with---shard)). The flag filters before `--shard` splits.
