...
In CI, make sure the commit exists in the checkout. `actions/checkout` fetches only the triggering commit by default, so the diff fails until you fetch the base commit. A depth of 1 is enough, because the diff only needs the tree of that commit:
