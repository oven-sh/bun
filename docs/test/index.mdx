...
With both flags, Bun watches every local source file in the import graph. That includes files that only tests outside the current selection import. When you save a file, Bun restarts and runs the tests that the change affects. The process stays alive even if the first run selects no tests.
...
**3. Skip the tests your change cannot reach: [`--changed`](#run-only-affected-tests-with---changed).** On a pull request, `bun test --changed=origin/main` runs only the test files whose import graph reaches a file in the diff. The flag filters before `--shard` splits.
