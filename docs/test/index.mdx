...
To pass arguments through to your tests, add them after a `--` separator. Everything after `--` is appended to `process.argv` inside the test process instead of being treated as a filter.

```bash terminal icon="terminal"
bun test ./my.test.ts -- --graphql-url http://localhost/graphql
# inside my.test.ts: process.argv.slice(2) is ["--graphql-url", "http://localhost/graphql"]
```
