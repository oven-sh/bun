...
Without a value, `--changed` compares the working tree against `HEAD`. With `--changed=<ref>`, Bun runs `git diff <ref>` instead. Untracked files count as changed in both forms. Bun resolves `tsconfig.json` `paths` aliases such as `@/*` when it scans imports, so a test that imports a changed file through an alias runs too. Changes inside `node_modules` do not select any tests.
...
Bun applies `--changed` first and then splits the remaining test files across shards. Every shard must use the same `--changed=<ref>` so that each shard filters the same set. See [Splitting a suite across CI machines](/test/parallel#splitting-a-suite-across-ci-machines-with---shard).

In CI, make sure the ref exists in the checkout. `actions/checkout` fetches only the triggering commit by default, so `git diff origin/main` fails until you fetch that branch. A depth of 1 is enough, because the diff only needs the tip of the base branch:

```yaml
- uses: actions/checkout@v4
- run: git fetch --depth=1 origin main
- run: bun test --changed=origin/main
```
...
**3. Skip the tests your change cannot reach: [`--changed`](#run-only-affected-tests-with---changed).** On a pull request, `bun test --changed=origin/main` runs only the test files whose import graph reaches a file in the diff. Fetch the base branch first (see [`--changed` with `--shard`](#--changed-with---shard)). The flag filters before `--shard` splits. Keep `--update-timings` on full-suite runs only: under `--shard` it writes timings for the files that ran, so a filtered run shrinks the table.
