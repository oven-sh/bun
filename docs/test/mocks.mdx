...
When hoisting applies, the remaining static imports in the test file are rewritten to `await import(...)`. Bun reads named and default bindings through the awaited namespace object, so `mock.module()` re-mocks inside a test still update them. Imports from `bun:test` (and `@jest/globals` / `vitest`), imports with `with { type: ... }` attributes, and `import defer` are left as static imports.
