...
`jest.resetModules()` (and its `vi.resetModules()` alias) clears the module registry so the next `require()` or dynamic `import()` re-evaluates the module from scratch. This is useful when a module keeps top-level state that should not bleed between tests:
