...
`mock.restore()` restores every mock at once, instead of calling `mockFn.mockRestore()` on each one. It also reverts modules overridden with `mock.module()` back to their original exports.
