...
`mock.restore()` restores every mock at once, instead of calling `mockFn.mockRestore()` on each one. It also reverts modules overridden with `mock.module()` during a test back to their original exports. Module mocks installed from a preload script are left in place so `afterEach(mock.restore)` does not tear down global test setup.
