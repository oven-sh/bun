...
| `mockFn.mock.instances`                   | Array of instances created with `new mockFn()` |
...
Mock functions can be called with `new`. If the implementation is a class or a plain function, it is constructed with the mock's `new.target`, so the instance is a real instance of it. Every construction is recorded in `.mock.instances`:
...
test("mocking a constructor", () => {
  const CounterMock = mock(Counter);

  const counter = new CounterMock(41);

  expect(counter).toBeInstanceOf(Counter);
  expect(counter.value).toBe(41);
  expect(CounterMock.mock.instances).toEqual([counter]);
});
```

An implementation that cannot be constructed (an arrow function, for instance) is instead called with the new instance as `this`, and `new` returns that instance unless the implementation returns an object of its own.
