...
Bun automatically hoists top-level `jest.mock()`, `vi.mock()`, and `mock.module()` calls in test files, matching Jest and Vitest. When a test file contains one of these calls at the top level, Bun moves it above the file's static `import` statements and rewrites those imports to evaluate afterwards, so the mock is installed before the module graph runs.

```ts title="user-service.test.ts" icon="/icons/typescript.svg"
import { test, expect, mock } from "bun:test";
import { getUser } from "./user-service"; // user-service imports "./api-client"

// Hoisted above the import of ./user-service, so user-service sees the mock.
mock.module("./api-client", () => ({
  fetchUser: async (id: string) => ({ id, name: "Mocked" }),
}));

test("getUser", async () => {
  expect((await getUser("1")).name).toBe("Mocked");
});
```

Only top-level calls are hoisted. Calls inside `describe`, `test`, or other callbacks run where they are written. As in Jest, `jest.doMock()` is never hoisted.

Because the remaining static imports in the test file are rewritten to `await import(...)` when hoisting applies, the bindings they introduce are ordinary variables rather than live ESM bindings within that file. This matches what Jest and Vitest do. Imports from `bun:test` (and `@jest/globals` / `vitest`) and imports that use `with { type: ... }` attributes or `import defer` are left as static imports.

For mocks that should apply to every test file (for example, replacing a database client across the whole suite), use `--preload` to load them before any test file runs:
...
To prevent the original module from being evaluated at all, either place the `mock.module()` call at the top level of your test file so it is hoisted, or use `--preload` to load your mocks before any test file runs.
...
Top-level `jest.mock()` / `vi.mock()` / `mock.module()` calls in a test file are hoisted so they run before that file's imports. For mocks that must apply across many test files, use `--preload`.
