...
`--shard` composes with [`--changed`](/test/index#run-only-affected-tests-with---changed). Bun applies the changed-files filter first, then sorts and slices what remains. Give every shard the same commit sha in `--changed=<ref>` so that each one filters the same set of files.
