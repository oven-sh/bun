...
| ---------------------------------- | ------------------------------- | -------------------------------------------------------------------------------------------------------------------- |
| `--parallel[=N\|max:N]`            | test **files**, in processes    | Runs files across `N` worker processes (default: number of CPU cores). Implies `--isolate`; `--no-isolate` opts out. |
...
bun test --parallel=max:8   # one per core, but never more than 8
```

`max:N` is for running the same command on machines of different sizes. A 32-core workstation that also has a browser and an editor open shouldn't hand all 32 cores to the test runner, but a 4-core CI box dedicated to the job should use all 4 — `--parallel=max:8` gives you 8 on the first and 4 on the second. A fixed `--parallel=8` would oversubscribe the CI box.
