...
```json title=".bun-test-timings.json" icon="file-json"
{
  ...
  "updated": ["src/router.test.ts", "test/db/migrate.test.ts", "test/integration/build.test.ts"]
}
```

- Paths are relative to the project root; values are wall-clock milliseconds for the whole file.
- `--update-timings` merges into the table it read rather than replacing it, so the file is always complete: a sharded run refreshes the entries for the files it ran, carries the rest through unchanged, and lists what it actually measured under `updated`. Entries for files that no longer exist are left alone; delete the file to start over.
...
`--timings` can be passed more than once. All the files are merged into one table before anything is scheduled, and `--update-timings` writes to the **first** path. When the same test file has an entry in several of them, a value that timings file's writer measured itself (listed in its `updated`) wins over a value it merely carried through, so shard outputs merge the same way regardless of the order they're given in.

That makes sharded CI self-contained — each shard writes its own file, and every shard reads all of last run's files:
...
jobs:
  test:
    ...
    steps:
      ...
      - run: bun install
      # last successful run's per-shard timings (missing on the very first run; bun creates the directory)
      ...
      - run: |
          bun test --parallel --shard=${{ matrix.shard }}/4 --update-timings \
            --timings=.bun-test-timings/${{ matrix.shard }}.json \
            $(ls .bun-test-timings/*.json 2>/dev/null | sed 's/^/--timings=/')
      - uses: actions/upload-artifact@v4
        with:
          name: timings-${{ matrix.shard }}
          path: .bun-test-timings/${{ matrix.shard }}.json
  save-timings:
    needs: test
...
Every shard must read the _same set_ of timings files for the shards to add up to the whole suite, which is why the files a run reads come from the previous run rather than from sibling shards still in flight.
