...
The [`environment`](/test/configuration#persistent-test-environment) option
creates one persistent environment host in each worker. Its module graph stays
loaded while the worker handles multiple files. Bun still gives every file a
fresh test global and calls environment setup and teardown around that file.
An environment requires isolation, so `--environment` and `--no-isolate`
cannot be combined.
