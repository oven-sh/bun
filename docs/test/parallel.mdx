...
- `--update-timings` merges into the table it read rather than replacing it, so the file is always complete: a sharded run refreshes the entries for the files it ran and carries the rest through unchanged. Entries for files that no longer exist are left alone; delete the file to start over.
- Files with no entry yet are assumed to take the median time when cutting shards, and are started first under `--parallel`.
- Every shard must read the _same_ timings file for the shards to add up to the whole suite — read the committed copy, not one that another shard has just updated.
...
Each shard's output is the committed table with that shard's entries refreshed, so to fold them back into one file take, per test file, whichever value changed relative to the committed copy:

```ts title="merge-timings.ts" icon="/icons/typescript.svg"
// usage: bun merge-timings.ts .bun-test-timings.json timings-*/.bun-test-timings.json
const [basePath, ...shardPaths] = process.argv.slice(2);
const base = (await Bun.file(basePath).json()).files;
const files = { ...base };
for (const path of shardPaths) {
  for (const [file, ms] of Object.entries((await Bun.file(path).json()).files)) {
    if (base[file] !== ms) files[file] = ms;
  }
}
await Bun.write(basePath, JSON.stringify({ version: 1, files }, null, 2) + "\n");
```
