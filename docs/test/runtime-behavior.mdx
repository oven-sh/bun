...
- `128 + signal number`: A signal interrupted the run, for example `130` for `SIGINT` (Ctrl+C) and `143` for `SIGTERM`

### Signal Handling

`SIGINT` (Ctrl+C) and `SIGTERM` stop the run at once: tests in progress do not finish, and `afterEach` and `afterAll` hooks do not run. With [`--parallel`](/test/parallel), the coordinator also terminates its workers and prints the files that did not finish.

```bash terminal icon="terminal"
# Stops the run. The shell reports exit code 143 (128 + 15)
