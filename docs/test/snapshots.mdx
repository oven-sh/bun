...
`--update-snapshots` updates the `.snap` file of each test file that takes a snapshot. It keeps the order of the entries, replaces the values that changed, and adds new entries at the end. It keeps the entries of a test that did not run or did not pass, for example a test that `-t`, `test.only`, or `test.skip` leaves out. It removes the other entries that no test took. If the file has a statement that Bun cannot read as an entry, Bun writes the file again from the snapshots of this run only.
