...
Use the `repeats` option to run a test multiple times regardless of pass/fail status. The test fails if any iteration fails. This is useful for detecting flaky tests or stress testing.
...
test(
  ...
  () => {
    // Run this 20 times to ensure it's not flaky
