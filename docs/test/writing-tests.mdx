...
A fixture setup function receives the context as its first parameter and a `use` function as its second. Call `await use(value)` to provide the fixture value; code after `await use(value)` is the fixture's teardown and runs after the test finishes, even if the test fails. Teardown runs in reverse setup order.
