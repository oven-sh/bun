...
`.only` has no effect when it selects no test that can run. For example, Bun skips a `test.only()` inside `describe.skip()`, and the other tests in the file still run. Unlike in Jest, a `describe.only()` inside `describe.skip()` also stays skipped.
