...
Legend: **mv** = `git mv` whole file. **split** = move named declarations into a new file (rest stays). **absorb** = move file into a directory owned by a *different* viable target.
...
| `cxx` (`:79-95`) | `s|src/bun.js/bindings|src/jsc/bindings|g`, `s|src/bun.js/modules|src/jsc/modules|g`, `s|src/bun.js/webview|src/runtime/webview|`, add `"src/highway_sys/*.cpp"`, `"src/simdutf_sys/*.cpp"`, drop `"src/vm/*.cpp"` (now under `jsc/bindings/vm/`), `"src/deps/*.cpp"` stays until `libuwsockets*.cpp` relocates |
