...
function generateBash(): string {
  ...
  // Global flags whose value is a separate word; the first_word scan must step
  // over that value so `bun --cwd dir <TAB>` doesn't treat "dir" as the subcommand.
  const globalValueFlags = data.globalFlags
    .filter(f => f.hasValue && !isShortOnly(f))
