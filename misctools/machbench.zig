...
pub fn machMain(runs: usize) anyerror!void {
    defer Output.flush();
    waker = try bun.Async.Waker.init(bun.heap.default_allocator);

    const args = try std.process.argsAlloc(bun.heap.default_allocator);
...
pub fn userMain(runs: usize) anyerror!void {
    defer Output.flush();
    user_waker = try bun.Async.UserFilterWaker.init(bun.heap.default_allocator);
