...
type Facts = {
  types: Record<string, { size: number; align: number; fields: Record<string, { offset: number; size: number | null }> }>;
...
for (const [name, ours] of Object.entries(image.types)) {
  ...
  if (ours.size !== theirs.size) differences.push(`type ${name}: size ${ours.size} in the image, ${theirs.size} in the headers`);
  else same++;
  if (ours.align !== theirs.align) differences.push(`type ${name}: alignment ${ours.align} in the image, ${theirs.align} in the headers`);
  else same++;
  for (const [field, ourField] of Object.entries(ours.fields)) {
    ...
    if (ourField.offset !== theirField.offset) differences.push(`field ${name}.${field}: offset ${ourField.offset} in the image, ${theirField.offset} in the headers`);
    else same++;
    if (ourField.size === null) notCompared.push(`field ${name}.${field}: its size, the structure is packed`);
    else if (ourField.size !== theirField.size) differences.push(`field ${name}.${field}: size ${ourField.size} in the image, ${theirField.size} in the headers`);
    else same++;
  }
}
for (const [name, ours] of Object.entries(image.constants)) {
  ...
  const equal = BigInt(ours) === BigInt(theirs) || BigInt.asUintN(32, BigInt(ours)) === BigInt.asUintN(32, BigInt(theirs)) && BigInt.asIntN(32, BigInt(ours)) === BigInt(ours) || BigInt.asUintN(64, BigInt(ours)) === BigInt.asUintN(64, BigInt(theirs));
  if (!equal) differences.push(`constant ${name}: ${ours} in the image, ${theirs} in the headers`);
  else same++;
}
if (headers.system && headers.system !== "win32") console.log(`NOTE: the headers are the ones of ${headers.system}, not of Windows: this comparison checks the tools only`);
