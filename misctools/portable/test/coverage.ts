 ...
 * that no run entered.
 *
 *   bun test/coverage.ts [--images <dir>] [--out <dir>] [--runs 2]
 *
 * --images is the output directory of build.ts with the images in it (default: build/portable/<arch> in
 * the repository, for this machine), --out is where the result goes (default: coverage in --images).
 *
 ...
 * Windows only.
 * Needs: build.ts test-image (the test images), build.ts jsc (the image).
 */

import { mkdirSync, readdirSync, rmSync, writeFileSync } from "node:fs";
import { join, resolve } from "node:path";
import { REPOSITORY, TREE, hostArch, llvmBin } from "../flags.ts";
...
const images = resolve(option("images", join(REPOSITORY, "build", "portable", hostArch())));
const out = resolve(option("out", join(images, "coverage")));
...
must([
  ...
  "-o",
  hostLinux,
  join(tree, "host/host_posix.c"),
  join(tree, "test/coverage_hook.c"),
  "-lpthread",
]);
...
for (const way of ["", "winmem", "overlay"])
  for (const t of small) {
    const r = run([hostLinux, join(images, t.image), ...t.args], {
      ...process.env,
      BUN_HOST_TEST: way,
      LLVM_PROFILE_FILE: join(out, "posix-%p.profraw"),
    });
...
const scenarios = (modes: string, profile: string) =>
  run(
    [
      ...
      hostLinux,
      "--out",
      join(out, `scenarios-${modes.replace(/,/g, "-")}.json`),
    ],
    { ...process.env, LLVM_PROFILE_FILE: join(out, profile) },
    "inherit",
  );
...
for (const [name, binary, source] of [["posix", hostLinux, "host_posix.c"]] as const) {
  const raw = readdirSync(out)
    .filter(f => f.startsWith(`${name}-`) && f.endsWith(".profraw"))
    .map(f => join(out, f));
  must([`${llvm}/llvm-profdata`, "merge", "-o", join(out, `${name}.profdata`), ...raw]);
  const exported = JSON.parse(
    must([`${llvm}/llvm-cov`, "export", "-format=text", `-instr-profile=${join(out, `${name}.profdata`)}`, binary]),
  );
  ...
  result[name] = {
    source,
    profiles: raw.length,
    functions: ran.length + never.length,
    functions_never_run: never,
    lines: lines.count,
    lines_run: lines.covered,
  };
  console.log(
    `${source}: ${raw.length} runs, ${ran.length} of ${ran.length + never.length} functions ran, ${lines.covered} of ${lines.count} lines. Never run: ${never.join(", ") || "none"}`,
  );
  writeFileSync(
    join(out, `${name}-lines.txt`),
    must([`${llvm}/llvm-cov`, "show", `-instr-profile=${join(out, `${name}.profdata`)}`, binary]),
  );
}
