...
//                             [--modes direct,hosted,winmem,overlay] [--only name,name] [--native <jsc of the same WebKit>]
//
...
async function once(s: Scenario, mode: string, index: number): Promise<string | null> {
  ...
  let problem = (s.codes ?? [0]).includes(code) ? expect(stdout) : `exit code ${code}${proc.signalCode ? ` (${proc.signalCode})` : ""}`;
...
for (const s of scenarios) {
  ...
  const wanted = s.sameAsNative && existsSync(native) ? [...modes, "native"] : modes;
  for (const mode of wanted) {
    ...
    row[mode] = tally;
    console.log(`${s.name.padEnd(20)} ${mode.padEnd(8)} ${tally.passes} of ${tally.runs}${tally.failures.length ? "   " + tally.failures[0] : ""}`);
  }
  results.push(row);
}
...
const failed = results.some(r => Object.values(r).some(v => typeof v === "object" && v !== null && "failures" in v && (v as Tally).failures.length));
