...
import {
  ...
  MD4 as MD4Polyfill,
  MD5 as MD5Polyfill,
  SHA1 as SHA1Polyfill,
  SHA224 as SHA224Polyfill,
  SHA256 as SHA256Polyfill,
  SHA384 as SHA384Polyfill,
  SHA512 as SHA512Polyfill,
  SHA512_256 as SHA512_256Polyfill,
} from "./bun/hashes.js";
...
export const gc = (
  globalThis.gc
    ...
    : () => {
        const err = new Error(
          "[bun-polyfills] Garbage collection polyfills are only available when Node.js is ran with the --expose-gc flag.",
        );
...
Object.setPrototypeOf(env, {
  toJSON(this: typeof env) {
    return { ...this };
  },
});
// @ts-expect-error supports-color types are unbelievably bad
export const enableANSIColors = (await import("supports-color")).createSupportsColor()
  .hasBasic satisfies typeof Bun.enableANSIColors;
...
peek_.status = (promise => {
  return util.inspect(promise).includes("<pending>")
    ? "pending"
    : util.inspect(promise).includes("<rejected>")
    ? "rejected"
    : "fulfilled";
...
export const inspect = ((arg: any): string =>
  util.inspect(arg, {
...
export const resolveSync = ((id: string, parent: string) =>
  import.meta.resolveSync(id, parent)) satisfies typeof Bun.resolveSync;
export const resolve = (async (id: string, parent: string) =>
  import.meta.resolve!(id, parent)) satisfies typeof Bun.resolve;
...
export const generateHeapSnapshot = (async (): Promise<HeapSnapshot> => {
  process.emitWarning(
    "The polyfill for Bun.generateHeapShot is asynchronous, unlike the original which is synchronous.",
    {
      type: "BunPolyfillWarning",
      code: "BUN_POLYFILLS_ASYNC_GENERATE_HEAP_SNAPSHOT",
      detail:
        "This is due to v8.getHeapSnapshot() returning a stream in Node.js. This is not a bug, but a limitation of the polyfill.",
    },
  );
...
export const serve = (() => {
  throw new NotImplementedError("Bun.serve", serve);
}) satisfies typeof Bun.serve;

export const file = ((
  path: string | URL | Uint8Array | ArrayBufferLike | number,
...
export const write = (async (
  dest: BunFileBlob | Bun.PathLike,
  input: string | Blob | TypedArray | ArrayBufferLike | Bun.BlobPart[] | Response | BunFileBlob,
  ...
  if (!isFileBlob(dest)) {
    ...
    return new Promise((resolve, reject) => {
      if (typeof input === "string")
        return fs.write(fd, input, (err, written) => (err ? reject(err) : resolve(written)));
      if (input instanceof Uint8Array)
        return fs.write(fd, input, (err, written) => (err ? reject(err) : resolve(written)));
      if (input instanceof ArrayBuffer)
        return fs.write(fd, new Uint8Array(input), (err, written) => (err ? reject(err) : resolve(written)));
...
export const spawn = ((...args) => {
  ...
  for (let i = 1; i < 3; i++) {
    // this intentionally skips stdin
  ...
  if (subpAsNode.stdout) {
    ...
    Reflect.set(rstream, "destroy", function (this: ReadableStream, err?: Error) {
      void (err ? this.cancel(String(err)) : this.cancel()).catch(() => {
        /* if it fails its already closed */
      });
      return this;
    });
  ...
  Object.defineProperty(subp, "readable", {
    get(this: Subprocess) {
      return this.stdout;
    },
  });
  ...
  if (!subpAsNode.pid)
    return subpAsNode.once("error", (err: SystemError) => {
...
export const spawnSync = ((...args): SyncSubprocess => {
  ...
  if (opts.stdio[0] && typeof opts.stdio[0] !== "string") {
    ...
    else if (opts.stdio[0] instanceof Response || opts.stdio[0] instanceof Request)
      throw new NotImplementedError("Bun.spawnSync({ stdin: <Response|Request> })", spawnSync);
...
export const readableStreamToArrayBuffer = ((
...
export const readableStreamToBlob = (async (stream: ReadableStream<any>) => {
  ...
  return new Blob(parts as Bun.BlobPart[]);
...
export const concatArrayBuffers = (buffers => {
  ...
  for (const chunk of buffers) {
    view.set(
      new Uint8Array(chunk instanceof ArrayBuffer || chunk instanceof SharedArrayBuffer ? chunk : chunk.buffer),
      offset,
    );
