declare module "bun" {
  ...
  interface S3Options extends BlobPropertyBag {
    ...
    /**
     * The storage class for the object. By default, Amazon S3 stores newly
     * created objects in the `STANDARD` storage class.
     *
     * @example
     * ```ts
  ...
  interface S3File extends Blob {
     ...
     * The URL grants temporary access to the file without exposing credentials.
     *
  ...
  interface S3ListObjectsResponse {
    /**
     * Keys that share the same prefix, grouped together (up to 1,000). When counting the
     * total number of returns by this API operation, each group counts as one item.
     *
     * A response can contain CommonPrefixes only if you specify a delimiter.
     *
     * CommonPrefixes contains all (if there are any) keys between Prefix and the next
     * occurrence of the string specified by a delimiter. These keys act like
     * subdirectories in the directory specified by Prefix.
     *
    ...
    contents?: {
      ...
      /**
       * The entity tag is a hash of the object. The ETag reflects changes only to the contents of an object, not its metadata. The ETag may or may not be an MD5 digest of the object data, depending on how the object was created and how it is encrypted:
       *
       * - Objects created by the PUT Object, POST Object, or Copy operation, or through the AWS Management Console, and are encrypted by SSE-S3 or plaintext, have ETags that are an MD5 digest of their object data.
       * - Objects created by the PUT Object, POST Object, or Copy operation, or through the AWS Management Console, and are encrypted by SSE-C or SSE-KMS, have ETags that are not an MD5 digest of their object data.
       * - If an object is created by either the Multipart Upload or Part Copy operation, the ETag is not an MD5 digest, regardless of the method of encryption. The AWS Management Console uploads or copies objects larger than 16 MB as a Multipart Upload, so their ETags are not MD5 digests.
       *
      ...
      restoreStatus?: {
        ...
        /** Indicates when the restored copy expires. This value is populated only if the object has already been restored. */
  ...
  /**
   * A configured S3 bucket instance for managing files. It creates {@link S3File}
   * instances and provides methods for common operations.
   *
   * @example
   * ```ts
   * // Basic bucket setup
  ...
  class S3Client {
     ...
     * overridden by passing options to the methods.
     ...
     * });
     *
     * // Get a reference to a file in the bucket:
    ...
    /**
     * Generate a presigned URL that grants temporary access to a file, for
     * uploads or downloads, without exposing credentials.
     *
     * @param path The path to the file in the bucket
