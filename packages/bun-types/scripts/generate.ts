...
const generateLabels = async () => {
  ...
  let labels = (await Bun.file(labelsFilePath).text()).split(
    "# Don't touch! Auto generated by scripts/generate.ts"
  )[0];
...
const generateLabeler = async () => {
  ...
  labeler +=
    ["chore:", "  - any: ['*']", `    all: [${choreAll.join(", ")}]`].join(
      "\n"
    ) + "\n\n";

  for (const file of dotFiles) {
    labeler +=
      [`'types:${file.replace(".d.ts", "")}':`, `  - '${file}'`].join("\n") +
      "\n\n";
  }
