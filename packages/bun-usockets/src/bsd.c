...
int bsd_sendmmsg(LIBUS_SOCKET_DESCRIPTOR fd, void *msgvec, unsigned int vlen,
  ...
  struct us_internal_udp_packet_buffer *packet_buffer =
      (struct us_internal_udp_packet_buffer *)msgvec;

  /* Let's just use sendto here */
  /* Winsock does not have sendmsg, while macOS has, however, we simply
use sendto since both macOS and Winsock has it.
   * Besides, you should use Linux either way to get best performance
with the sendmmsg */
  ...
  for (int i = 0; i < LIBUS_UDP_MAX_NUM; i++) {
    ...
    int ret = sendto(fd, packet_buffer->buf[i], packet_buffer->len[i], flags,
                     (struct sockaddr *)&packet_buffer->addr[i],
                     sizeof(struct sockaddr_in));

    if (ret == -1) {
      // if we fail then we need to buffer up, no that's not our
problem
            // we do need to register poll out though and have a callback 
for it
            return i;
    }
...
void bsd_udp_buffer_set_packet_payload(struct us_udp_packet_buffer_t *send_buf,
...
#if defined(_WIN32) || defined(__APPLE__)
  struct us_internal_udp_packet_buffer *packet_buffer =
      (struct us_internal_udp_packet_buffer *)send_buf;

  memcpy(packet_buffer->buf[index], payload, length);
  memcpy(&packet_buffer->addr[index], peer_addr,
  ...
  memcpy(ss[index].msg_hdr.msg_name, peer_addr,
         /*ss[index].msg_hdr.msg_namelen*/ sizeof(struct sockaddr_in));
  ...
  memcpy(((char *)ss[index].msg_hdr.msg_iov->iov_base) + offset, payload,
         length);
#endif
}

/* The maximum UDP payload size is 64kb, but in IPV6 you can have
jumbopackets larger than so.
 * We do not support those jumbo packets currently, but will safely ignore
them.
 * Any sane sender would assume we don't support them if we consistently
drop them.
...
void *bsd_create_udp_packet_buffer() {
  ...
  struct us_internal_udp_packet_buffer *b =
      malloc(sizeof(struct us_internal_udp_packet_buffer) +
             LIBUS_UDP_MAX_SIZE * LIBUS_UDP_MAX_NUM);

  for (int i = 0; i < LIBUS_UDP_MAX_NUM; i++) {
    b->buf[i] = ((char *)b) + sizeof(struct us_internal_udp_packet_buffer) +
                LIBUS_UDP_MAX_SIZE * i;
  }
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_listen_socket_unix(const char *path,
  ...
  int size =
      offsetof(struct sockaddr_un, sun_path) + strlen(server_address.sun_path);
#ifdef _WIN32
  _unlink(path);
#else
  unlink(path);
  ...
  if (bind(listenFd, (struct sockaddr *)&server_address, size) ||
      listen(listenFd, 512)) {
    bsd_close_socket(listenFd);
    return LIBUS_SOCKET_ERROR;
  }
...
#else
  // we should iterate all control messages once, after recvmmsg and
  then only fetch them with these functions struct msghdr *mh =
      &((struct mmsghdr *)msgvec)[index].msg_hdr;
  for (struct cmsghdr *cmsg = CMSG_FIRSTHDR(mh); cmsg != NULL;
       cmsg = CMSG_NXTHDR(mh, cmsg)) {
...
static int bsd_do_connect(struct addrinfo *rp, int *fd) {
  while (rp != NULL) {
    ...
    int resultFd =
        bsd_create_socket(rp->ai_family, rp->ai_socktype, rp->ai_protocol);
    if (resultFd < 0) {
      return LIBUS_SOCKET_ERROR;
    }
    *fd = resultFd;
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_connect_socket(const char *host, int port,
  ...
  LIBUS_SOCKET_DESCRIPTOR fd = bsd_create_socket(
  ...
  if (source_host) {
    ...
    if (!getaddrinfo(source_host, NULL, NULL, &interface_result)) {
      int ret = bind(fd, interface_result->ai_addr,
                     (socklen_t)interface_result->ai_addrlen);
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_connect_socket_unix(const char *server_path,
  ...
  if (connect(fd, (struct sockaddr *)&server_address, size) != 0 &&
      errno != EINPROGRESS) {
    bsd_close_socket(fd);
    return LIBUS_SOCKET_ERROR;
  }
