...
int bsd_sendmmsg(LIBUS_SOCKET_DESCRIPTOR fd, struct udp_sendbuf *sendbuf,
                 int flags) {
  ...
  for (int i = 0; i < sendbuf->num; i++) {
    while (1) {
      ...
      if (!addr || addr->sa_family == AF_UNSPEC) {
        ...
        ret = sendto(fd, sendbuf->payloads[i], sendbuf->lengths[i], flags, addr,
                     len);
...
int bsd_recvmmsg(LIBUS_SOCKET_DESCRIPTOR fd, struct udp_recvbuf *recvbuf,
                 int flags) {
#if defined(_WIN32)
  socklen_t addr_len = sizeof(struct sockaddr_storage);
  while (1) {
    ssize_t ret = recvfrom(fd, recvbuf->buf, LIBUS_RECV_BUFFER_LENGTH, flags,
                           (struct sockaddr *)&recvbuf->addr, &addr_len);
    if (ret < 0) {
...
#else
  while (1) {
    int ret = recvmmsg(fd, (struct mmsghdr *)&recvbuf->msgvec,
                       LIBUS_UDP_RECV_COUNT, flags, 0);
    if (ret >= 0 || errno != EINTR)
      return ret;
  }
#endif
...
void bsd_udp_setup_recvbuf(struct udp_recvbuf *recvbuf, void *databuf,
                           size_t databuflen) {
#if defined(_WIN32)
  recvbuf->buf = databuf;
...
int bsd_udp_setup_sendbuf(struct udp_sendbuf *buf, size_t bufsize,
                          void **payloads, size_t *lengths, void **addresses,
                          int num) {
#if defined(_WIN32)
  ...
  return num;
#else
  // TODO: can we skip empty messages altogether? Do we really need to send
  // 0-length messages?
  buf->has_empty = 0;
  ...
  size_t count = (bufsize - sizeof(struct udp_sendbuf)) /
...
int bsd_udp_packet_buffer_local_ip(struct udp_recvbuf *msgvec, int index,
                                   char *ip) {
#if defined(_WIN32) || defined(__APPLE__)
...
int bsd_udp_packet_buffer_payload_length(struct udp_recvbuf *msgvec,
                                         int index) {
#if defined(_WIN32)
...
static LIBUS_SOCKET_DESCRIPTOR
win32_set_nonblocking(LIBUS_SOCKET_DESCRIPTOR fd) {
#if _WIN32
  if (fd != LIBUS_SOCKET_ERROR) {
...
char *bsd_addr_get_ip(struct bsd_addr_t *addr) { return addr->ip; }

int bsd_addr_get_ip_length(struct bsd_addr_t *addr) { return addr->ip_length; }

int bsd_addr_get_port(struct bsd_addr_t *addr) { return addr->port; }
...
#if defined(SOCK_CLOEXEC) && defined(SOCK_NONBLOCK)
    // Linux, FreeBSD
    accepted_fd = accept4(fd, (struct sockaddr *)addr, &addr->len,
...
ssize_t bsd_write2(LIBUS_SOCKET_DESCRIPTOR fd, const char *header,
...
#define MSG_NOSIGNAL 0
    ...
    ssize_t rc =
        send(fd, buf, length,
             ((msg_more != 0) * MSG_MORE) | MSG_NOSIGNAL | MSG_DONTWAIT);
#else
    // use TCP_NOPUSH
...
static int us_internal_bind_and_listen(LIBUS_SOCKET_DESCRIPTOR listenFd,
                                       struct sockaddr *listenAddr,
                                       socklen_t listenAddrLength,
                                       int backlog) {
  int result;
  do
...
inline __attribute__((always_inline)) LIBUS_SOCKET_DESCRIPTOR
bsd_bind_listen_fd(LIBUS_SOCKET_DESCRIPTOR listenFd,
                   struct addrinfo *listenAddr, int port, int options) {

  if ((options & LIBUS_LISTEN_EXCLUSIVE_PORT)) {
#if _WIN32
    int optval2 = 1;
    setsockopt(listenFd, SOL_SOCKET, SO_EXCLUSIVEADDRUSE, (void *)&optval2,
               sizeof(optval2));
#endif
  } else {
#if defined(SO_REUSEPORT)
    int optval2 = 1;
    setsockopt(listenFd, SOL_SOCKET, SO_REUSEPORT, (void *)&optval2,
               sizeof(optval2));
#endif
  }
  ...
  int optval3 = 1;
  setsockopt(listenFd, SOL_SOCKET, SO_REUSEADDR, (void *)&optval3,
             sizeof(optval3));
#endif

#ifdef IPV6_V6ONLY
  int disabled = 0;
  setsockopt(listenFd, IPPROTO_IPV6, IPV6_V6ONLY, (void *)&disabled,
             sizeof(disabled));
#endif

  if (us_internal_bind_and_listen(listenFd, listenAddr->ai_addr,
                                  (socklen_t)listenAddr->ai_addrlen, 512)) {
    return LIBUS_SOCKET_ERROR;
  }
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_listen_socket(const char *host, int port,
  ...
  for (struct addrinfo *a = result; a != NULL; a = a->ai_next) {
    if (a->ai_family == AF_INET6) {
      ...
      listenAddr = a;
      if (bsd_bind_listen_fd(listenFd, listenAddr, port, options) !=
          LIBUS_SOCKET_ERROR) {
        freeaddrinfo(result);
        return listenFd;
      }
...
static LIBUS_SOCKET_DESCRIPTOR
bsd_create_unix_socket_address(const char *path, size_t path_len,
                               int *dirfd_linux_workaround_for_unix_path_len,
  ...
  *addrlen = sizeof(struct sockaddr_un);
  ...
  // As a workaround, we can use /proc/self/fd/ as a directory to shorten the
  // path
  if (path_len >= sizeof(server_address->sun_path) && path[0] != '\0') {
    ...
    // if the path is just a single character, or the path is too long, we
    // cannot use this method
    if (dirname_len < 2 ||
        (path_len - dirname_len + 1) >= sizeof(server_address->sun_path)) {
      errno = ENAMETOOLONG;
      return LIBUS_SOCKET_ERROR;
    }
    ...
    int socket_dir_fd =
        open(dirname_buf, O_CLOEXEC | O_PATH | O_DIRECTORY, 0700);
    if (socket_dir_fd == -1) {
      errno = ENAMETOOLONG;
      return LIBUS_SOCKET_ERROR;
    }
    ...
    int sun_path_len =
        snprintf(server_address->sun_path, sizeof(server_address->sun_path),
                 "/proc/self/fd/%d/%s", socket_dir_fd, path + dirname_len);
...
static LIBUS_SOCKET_DESCRIPTOR
internal_bsd_create_listen_socket_unix(const char *path, int options,
                                       struct sockaddr_un *server_address,
                                       size_t addrlen) {
  LIBUS_SOCKET_DESCRIPTOR listenFd = LIBUS_SOCKET_ERROR;
  ...
  if (us_internal_bind_and_listen(listenFd, (struct sockaddr *)server_address,
                                  (socklen_t)addrlen, 512)) {
#if defined(_WIN32)
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_listen_socket_unix(const char *path,
                                                      size_t len, int options) {
  int dirfd_linux_workaround_for_unix_path_len = -1;
  struct sockaddr_un server_address;
  size_t addrlen = 0;
  if (bsd_create_unix_socket_address(path, len,
                                     &dirfd_linux_workaround_for_unix_path_len,
                                     &server_address, &addrlen)) {
    return LIBUS_SOCKET_ERROR;
  }

  LIBUS_SOCKET_DESCRIPTOR listenFd = internal_bsd_create_listen_socket_unix(
      path, options, &server_address, addrlen);
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_udp_socket(const char *host, int port) {
  ...
  for (struct addrinfo *a = result; a && listenFd == LIBUS_SOCKET_ERROR;
       a = a->ai_next) {
    if (a->ai_family == AF_INET6) {
      listenFd =
          bsd_create_socket(a->ai_family, a->ai_socktype, a->ai_protocol);
  ...
  if (port != 0) {
    ...
    int enabled = 1;
    setsockopt(listenFd, SOL_SOCKET, SO_REUSEADDR, (void *)&enabled,
               sizeof(enabled));
  }
...
#define IPV6_RECVPKTINFO IPV6_PKTINFO
  ...
  int enabled = 1;
  if (setsockopt(listenFd, IPPROTO_IPV6, IPV6_RECVPKTINFO, (void *)&enabled,
                 sizeof(enabled)) == -1) {
    if (errno == 92) {
      if (setsockopt(listenFd, IPPROTO_IP, IP_PKTINFO, (void *)&enabled,
                     sizeof(enabled)) != 0) {
  ...
  if (setsockopt(listenFd, IPPROTO_IPV6, IPV6_RECVTCLASS, (void *)&enabled,
                 sizeof(enabled)) == -1) {
    if (errno == 92) {
      if (setsockopt(listenFd, IPPROTO_IP, IP_RECVTOS, (void *)&enabled,
                     sizeof(enabled)) != 0) {
...
int bsd_connect_udp_socket(LIBUS_SOCKET_DESCRIPTOR fd, const char *host,
                           int port) {
  struct addrinfo hints, *result;
  memset(&hints, 0, sizeof(struct addrinfo));
...
//     return -1;
// #else
//     // we should iterate all control messages once, after recvmmsg and then
//     only fetch them with these functions struct msghdr *mh = &((struct
//     mmsghdr *) msgvec)[index].msg_hdr; for (struct cmsghdr *cmsg =
//     CMSG_FIRSTHDR(mh); cmsg != NULL; cmsg = CMSG_NXTHDR(mh, cmsg)) {
...
static int bsd_do_connect_raw(LIBUS_SOCKET_DESCRIPTOR fd, struct sockaddr *addr,
                              size_t namelen) {
#ifdef _WIN32
  while (1) {
...
static int convert_null_addr(const struct sockaddr_storage *addr,
  ...
  if (addr->ss_family == AF_INET) {
    ...
    if (memcmp(&addr6->sin6_addr, &in6addr_any, sizeof(struct in6_addr)) == 0) {
      ...
      memcpy(&((struct sockaddr_in6 *)result)->sin6_addr, &in6addr_loopback,
...
static int is_loopback(struct sockaddr_storage *sockaddr) {
  if (sockaddr->ss_family == AF_INET) {
    ...
    return memcmp(&addr->sin6_addr, &in6addr_loopback,
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_connect_socket(struct sockaddr_storage *addr,
                                                  int options) {
  LIBUS_SOCKET_DESCRIPTOR fd =
      bsd_create_socket(addr->ss_family, SOCK_STREAM, 0);
  if (fd == LIBUS_SOCKET_ERROR) {
    return LIBUS_SOCKET_ERROR;
  }
  ...
  // This sets the socket to fail quickly if no connection can be established to
  // localhost, instead of waiting for the default 2 seconds. This is necessary
  // because we always try to connect using IPv6 first, but it's possible that
  // whatever we want to connect to is only listening on IPv4. see
  // https://github.com/libuv/libuv/blob/bf61390769068de603e6deec8e16623efcbe761a/src/win/tcp.c#L806
  ...
  if (is_loopback(addr)) {
    ...
    WSAIoctl(fd, SIO_TCP_INITIAL_RTO, &retransmit_ioctl,
             sizeof(retransmit_ioctl), NULL, 0, &bytes, NULL, NULL);
  }

#endif
  int rc = bsd_do_connect_raw(fd, (struct sockaddr *)addr,
...
static LIBUS_SOCKET_DESCRIPTOR internal_bsd_create_connect_socket_unix(
    const char *server_path, size_t len, int options,
    struct sockaddr_un *server_address, const size_t addrlen) {
...
LIBUS_SOCKET_DESCRIPTOR bsd_create_connect_socket_unix(const char *server_path,
  ...
  size_t addrlen = 0;
  int dirfd_linux_workaround_for_unix_path_len = -1;
  if (bsd_create_unix_socket_address(server_path, len,
                                     &dirfd_linux_workaround_for_unix_path_len,
                                     &server_address, &addrlen)) {
    return LIBUS_SOCKET_ERROR;
  }

  LIBUS_SOCKET_DESCRIPTOR fd = internal_bsd_create_connect_socket_unix(
      server_path, len, options, &server_address, addrlen);
