...
static X509 *root_cert_instances[1] = {NULL};
    ...
    int us_ssl_ctx_use_certificate_chain(SSL_CTX * ctx, const char *content) {
      ...
      if (ret) {
        ...
        while ((ca = PEM_read_bio_X509(
          ...
          // Note that we must not free r if it was successfully added to the
...
#define CASE_X509_ERR(CODE)                                                    \
  case X509_V_ERR_##CODE:                                                      \
    ...
    break;
      switch (err) {
        // if you modify anything in here, *please* update the respective
        // section in doc/api/tls.md as well
...
#undef CASE_X509_ERR
    ...
    struct us_bun_verify_error_t us_internal_verify_error(
      ...
      long x509_verify_error = // NOLINT(runtime/int)
          us_internal_verify_peer_certificate(
              ssl, X509_V_ERR_UNABLE_TO_GET_ISSUER_CERT);
    ...
    SSL_CTX *create_ssl_context_from_bun_options(
      ...
      /* The following are helpers. You may easily implement whatever you want
       * by using the native handle directly */

      /* Important option for lowering memory usage, but lowers performance
       * slightly */
      if (options.ssl_prefer_low_memory_usage) {
        SSL_CTX_set_mode(ssl_context, SSL_MODE_RELEASE_BUFFERS);
      }
      ...
      if (options.passphrase) {
        ...
        SSL_CTX_set_default_passwd_cb_userdata(
            ssl_context, (void *)strdup(options.passphrase));
        SSL_CTX_set_default_passwd_cb(ssl_context, passphrase_cb);
      }
      ...
      if (options.cert_file_name) {
        ...
        for (unsigned int i = 0; i < options.cert_count; i++) {
          if (us_ssl_ctx_use_certificate_chain(ssl_context, options.cert[i]) !=
              1) {
            free_ssl_context(ssl_context);
            return NULL;
          }
      ...
      if (options.ca_file_name) {
        ...
        for (unsigned int i = 0; i < options.ca_count; i++) {
          ...
          if (!add_ca_cert_to_ctx_store(ssl_context, options.ca[i],
                                        cert_store)) {
            free_ssl_context(ssl_context);
            return NULL;
          }
    ...
    void *us_internal_ssl_socket_context_find_server_name_userdata(
      ...
      /* We can use sni_find because looking up a "wildcard pattern" will match
       * the exact literal "wildcard pattern" first, before it matches by the
       * very wildcard itself, so it works fine (exact match is the only thing
       * we care for here) */
    ...
    /* Returns either nullptr or the previously set user data attached to this
     * SSL's selected SNI context */
    ...
    int sni_cb(SSL * ssl, int *al, void *arg) {
      ...
      if (ssl) {
        ...
        if (hostname && hostname[0]) {
          /* Try and resolve (match) required hostname with what we have
           * registered */
    ...
    us_internal_create_ssl_socket_context(
      ...
      /* Otherwise ee continue by creating a non-SSL context, but with larger
       * ext to hold our SSL stuff */
      struct us_internal_ssl_socket_context_t *context =
          (struct us_internal_ssl_socket_context_t *)us_create_socket_context(
              0, loop,
    ...
    struct us_internal_ssl_socket_t *
    us_internal_ssl_socket_context_connect_unix(
        struct us_internal_ssl_socket_context_t * context,
    ...
    void us_internal_ssl_socket_context_on_open(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_open)(struct us_internal_ssl_socket_t * s, int is_client,
    ...
    void us_internal_ssl_socket_context_on_close(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_close)(struct us_internal_ssl_socket_t * s, int code,
                        void *reason)) {
    ...
    void us_internal_ssl_socket_context_on_data(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_data)(struct us_internal_ssl_socket_t * s, char *data,
                       int length)) {
    ...
    void us_internal_ssl_socket_context_on_writable(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_writable)(struct us_internal_ssl_socket_t * s)) {
    ...
    void us_internal_ssl_socket_context_on_timeout(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_timeout)(struct us_internal_ssl_socket_t * s)) {
    ...
    void us_internal_ssl_socket_context_on_long_timeout(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_long_timeout)(struct us_internal_ssl_socket_t * s)) {
      us_socket_context_on_long_timeout(
          0, (struct us_socket_context_t *)context,
          (struct us_socket_t * (*)(struct us_socket_t *)) on_long_timeout);
    }

    /* We do not really listen to passed FIN-handler, we entirely override it
     * with our handler since SSL doesn't really have support for half-closed
     * sockets */
    void us_internal_ssl_socket_context_on_end(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_end)(struct us_internal_ssl_socket_t *)) {
    ...
    void us_internal_ssl_socket_context_on_connect_error(
        struct us_internal_ssl_socket_context_t * context,
        struct us_internal_ssl_socket_t *
            (*on_connect_error)(struct us_internal_ssl_socket_t *, int code)) {
    ...
    int us_internal_ssl_socket_write(struct us_internal_ssl_socket_t * s,
      ...
      struct us_internal_ssl_socket_context_t *context =
          (struct us_internal_ssl_socket_context_t *)us_socket_context(0,
                                                                       &s->s);
      ...
      // it makes literally no sense to touch this here! it should start at 0
      // and ONLY be set and reset by the on_data function! the way is is now,
      // triggering a write from a read will essentially delete all input data!
      // what we need to do is to check if this ever is non-zero and print a
      // warning
      ...
      if (written > 0) {
        ...
        if (err == SSL_ERROR_WANT_READ) {
          ...
          // these two errors may add to the error queue, which is per thread
          // and must be cleared
          ERR_clear_error();
    ...
    int us_internal_ssl_socket_is_shut_down(struct us_internal_ssl_socket_t *
                                            s) {
    ...
    void us_internal_ssl_socket_shutdown(struct us_internal_ssl_socket_t * s) {
      if (!us_socket_is_closed(0, &s->s) &&
        ...
        // then this data is not for me but this is not correct as it is
        // currently anyways, any data available should be properly reset
        loop_ssl_data->ssl_read_input_length = 0;

        // essentially we need two of these: one for CURRENT CALL and one for
        // CURRENT SOCKET WITH DATA if those match in the BIO function then you
        // may read, if not then you may not read we need ssl_read_socket to be
        // set in on_data and checked in the BIO
        loop_ssl_data->ssl_socket = &s->s;
    ...
    struct us_internal_ssl_socket_t *
    us_internal_ssl_socket_context_adopt_socket(
        struct us_internal_ssl_socket_context_t * context,
    ...
    struct us_internal_ssl_socket_t *ssl_wrapped_context_on_close(
      ...
      struct us_wrapped_socket_context_t *wrapped_context =
          (struct us_wrapped_socket_context_t *)
              us_internal_ssl_socket_context_ext(context);

      if (wrapped_context->events.on_close) {
        wrapped_context->events.on_close((struct us_socket_t *)s, code, reason);
      }

      // writting here can cause the context to not be writable anymore but its
      // the user responsability to check for that
      if (wrapped_context->old_events.on_close) {
        wrapped_context->old_events.on_close((struct us_socket_t *)s, code,
                                             reason);
      }
    ...
    struct us_internal_ssl_socket_t *ssl_wrapped_context_on_writable(
        struct us_internal_ssl_socket_t * s) {
      struct us_internal_ssl_socket_context_t *context =
          (struct us_internal_ssl_socket_context_t *)us_socket_context(0,
                                                                       &s->s);
    ...
    struct us_internal_ssl_socket_t *ssl_wrapped_context_on_data(
      ...
      if (wrapped_context->old_events.on_data) {
        wrapped_context->old_events.on_data((struct us_socket_t *)s, data,
                                            length);
      }
      // ssl wrapped data
    ...
    struct us_internal_ssl_socket_t *ssl_wrapped_context_on_timeout(
        struct us_internal_ssl_socket_t * s) {
      struct us_internal_ssl_socket_context_t *context =
          (struct us_internal_ssl_socket_context_t *)us_socket_context(0,
                                                                       &s->s);
    ...
    struct us_internal_ssl_socket_t *ssl_wrapped_context_on_long_timeout(
        struct us_internal_ssl_socket_t * s) {
      struct us_internal_ssl_socket_context_t *context =
          (struct us_internal_ssl_socket_context_t *)us_socket_context(0,
                                                                       &s->s);
    ...
    struct us_internal_ssl_socket_t *ssl_wrapped_context_on_end(
        struct us_internal_ssl_socket_t * s) {
      struct us_internal_ssl_socket_context_t *context =
          (struct us_internal_ssl_socket_context_t *)us_socket_context(0,
                                                                       &s->s);
    ...
    struct us_internal_ssl_socket_t *ssl_wrapped_on_connect_error(
      ...
      if (wrapped_context->old_events.on_connect_error) {
        wrapped_context->old_events.on_connect_error((struct us_socket_t *)s,
                                                     code);
      }
      return s;
    }

    struct us_internal_ssl_socket_t *us_internal_ssl_socket_open(
    ...
    struct us_internal_ssl_socket_t *us_internal_ssl_socket_wrap_with_tls(
      ...
      struct us_wrapped_socket_context_t *wrapped_context =
          (struct us_wrapped_socket_context_t *)
              us_internal_ssl_socket_context_ext(tls_context);
      ...
      // no need to wrap open because socket is already open (only new context
      // will be called so we can configure hostname and ssl stuff normally here
      // before handshake)
      tls_context->on_open =
          (struct us_internal_ssl_socket_t *
           (*)(struct us_internal_ssl_socket_t *, int, char *, int))
              events.on_open;
      ...
      // we need to wrap these events because we need to call the old context
      // events as well
      us_socket_context_on_connect_error(
          0, context,
          (struct us_socket_t * (*)(struct us_socket_t *, int))
      ...
      struct us_internal_ssl_socket_t *socket =
          (struct us_internal_ssl_socket_t *)us_socket_context_adopt_socket(
              0, context, s,
              sizeof(struct us_internal_ssl_socket_t) -
                  sizeof(struct us_socket_t) + socket_ext_size);
      socket->ssl = NULL;
