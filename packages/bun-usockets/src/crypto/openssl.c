...
restart:
  while (1) {
     ...
     * paused the socket. Stop draining once the recv-buffer ciphertext and any
     * plaintext SSL_read already decrypted are fully consumed, so we do not
     * process further records (e.g. close_notify) the caller asked to defer. */
    if (s->flags.is_paused && loop_ssl_data->ssl_read_input_length == 0 &&
        SSL_pending(s_ssl(s)) == 0) {
