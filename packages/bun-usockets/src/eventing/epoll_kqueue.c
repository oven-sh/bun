...
#define GET_READY_POLL(loop, index)                                            \
  (struct us_poll_t *)loop->ready_polls[index].data.ptr
#define SET_READY_POLL(loop, index, poll)                                      \
  loop->ready_polls[index].data.ptr = (void *)poll
#else
#define GET_READY_POLL(loop, index)                                            \
  (struct us_poll_t *)loop->ready_polls[index].udata
#define SET_READY_POLL(loop, index, poll)                                      \
  loop->ready_polls[index].udata = (uint64_t)poll
#endif
...
void *us_poll_ext(struct us_poll_t *p) { return p + 1; }
...
int us_poll_events(struct us_poll_t *p) {
  return ((p->state.poll_type & POLL_TYPE_POLLING_IN) ? LIBUS_SOCKET_READABLE
                                                      : 0) |
         ((p->state.poll_type & POLL_TYPE_POLLING_OUT) ? LIBUS_SOCKET_WRITABLE
                                                       : 0);
}

LIBUS_SOCKET_DESCRIPTOR us_poll_fd(struct us_poll_t *p) { return p->state.fd; }
...
void us_internal_poll_set_type(struct us_poll_t *p, int poll_type) {
  p->state.poll_type =
      poll_type | (p->state.poll_type & POLL_TYPE_POLLING_MASK);
}
...
static ssize_t sys_epoll_pwait2(int epfd, struct epoll_event *events,
...
static int bun_epoll_pwait2(int epfd, struct epoll_event *events, int maxevents,
                            const struct timespec *timeout) {
  int ret;
...
void us_loop_run(struct us_loop_t *loop) {
  ...
  while (loop->num_polls) {
    ...
    loop->num_ready_polls =
        bun_epoll_pwait2(loop->fd, loop->ready_polls, 1024, NULL);
#else
    do {
      loop->num_ready_polls =
          kevent64(loop->fd, NULL, 0, loop->ready_polls, 1024, 0, NULL);
    ...
    for (loop->current_ready_poll = 0;
      ...
      if (LIKELY(poll)) {
...
#else
        const struct kevent64_s *current_kevent =
            &loop->ready_polls[loop->current_ready_poll];
        ...
        // aggregate the events into a single kevent struct Note: EV_ERROR only
        // sets the error in data as part of changelist. Not in this call!
        int events = 0 | ((filter & EVFILT_READ) ? LIBUS_SOCKET_READABLE : 0) |
                     ((filter & EVFILT_WRITE) ? LIBUS_SOCKET_WRITABLE : 0);
        const int error =
            (flags & (EV_ERROR | EV_EOF)) ? ((int)fflags || 1) : 0;
#endif
        /* Always filter all polls by what they actually poll for (callback
         * polls always poll for readable) */
...
void us_loop_run_bun_tick(struct us_loop_t *loop,
  ...
  struct us_internal_callback_t *timer_callback =
  ...
  loop->num_ready_polls =
      bun_epoll_pwait2(loop->fd, loop->ready_polls, 1024, timeout);
#else
  do {
    loop->num_ready_polls =
        kevent64(loop->fd, NULL, 0, loop->ready_polls, 1024, 0, timeout);
  } while (IS_EINTR(loop->num_ready_polls));
#endif
  ...
  for (loop->current_ready_poll = 0;
    ...
    if (LIKELY(poll)) {
      ...
      // the events into a single kevent struct
      int events = 0 | ((filter & EVFILT_READ) ? LIBUS_SOCKET_READABLE : 0) |
                   ((filter & EVFILT_WRITE) ? LIBUS_SOCKET_WRITABLE : 0);

      // Note: EV_ERROR only sets the error in data as part of changelist. Not
      // in this call!
      const int error = (flags & (EV_ERROR | EV_EOF)) ? ((int)fflags || 1) : 0;
#endif
      /* Always filter all polls by what they actually poll for (callback polls
       * always poll for readable) */
      events &= us_poll_events(poll);
      if (events || error) {
        us_internal_dispatch_ready_poll(poll, error, events);
      }
...
void us_internal_loop_update_pending_ready_polls(struct us_loop_t *loop,
...
#else
  /* Ready polls may contain same poll twice under kqueue, as one poll may hold
   * two filters */
   ...
   * have a fast path with no seeking in cases of: current poll being us AND we
   * only poll for one thing */

  for (int i = loop->current_ready_poll;
       i < loop->num_ready_polls && num_entries_possibly_remaining; i++) {
    if (GET_READY_POLL(loop, i) == old_poll) {

      // if new events does not contain the ready events of this poll then
      // remove (no we filter that out later on)
...
int kqueue_change(int kqfd, int fd, int old_events, int new_events,
                  void *user_data) {
  struct kevent64_s change_list[2];
  int change_length = 0;

  /* Do they differ in readable? */
  if ((new_events & LIBUS_SOCKET_READABLE) !=
      (old_events & LIBUS_SOCKET_READABLE)) {
    EV_SET64(&change_list[change_length++], fd, EVFILT_READ,
             (new_events & LIBUS_SOCKET_READABLE) ? EV_ADD : EV_DELETE, 0, 0,
             (uint64_t)(void *)user_data, 0, 0);
  }

  /* Do they differ in writable? */
  if ((new_events & LIBUS_SOCKET_WRITABLE) !=
      (old_events & LIBUS_SOCKET_WRITABLE)) {
    EV_SET64(&change_list[change_length++], fd, EVFILT_WRITE,
             (new_events & LIBUS_SOCKET_WRITABLE) ? EV_ADD : EV_DELETE, 0, 0,
             (uint64_t)(void *)user_data, 0, 0);
  }

  int ret;
  do {
    ret = kevent64(kqfd, change_list, change_length, change_list, change_length,
                   KEVENT_FLAG_ERROR_EVENTS, NULL);
  } while (IS_EINTR(ret));
...
struct us_poll_t *us_poll_resize(struct us_poll_t *p, struct us_loop_t *loop,
  ...
  if (p != new_p && events) {
    ...
    /* This is needed for epoll also (us_change_poll doesn't update the old
     * poll) */
...
void us_poll_change(struct us_poll_t *p, struct us_loop_t *loop, int events) {
  ...
  if (old_events != events) {
    ...
    // us_internal_loop_update_pending_ready_polls(loop, p, p, old_events,
    // events);
  }
}
...
void us_poll_stop(struct us_poll_t *p, struct us_loop_t *loop) {
  ...
  us_internal_loop_update_pending_ready_polls(loop, p, 0, old_events,
                                              new_events);
}
...
struct us_timer_t *us_create_timer(struct us_loop_t *loop, int fallthrough,
  ...
  struct us_poll_t *p = us_create_poll(
      loop, fallthrough, sizeof(struct us_internal_callback_t) + ext_size);
...
struct us_timer_t *us_create_timer(struct us_loop_t *loop, int fallthrough,
  ...
  struct us_internal_callback_t *cb =
      us_calloc(1, sizeof(struct us_internal_callback_t) + ext_size);
...
void us_timer_set(struct us_timer_t *t, void (*cb)(struct us_timer_t *t),
  ...
  // Avoid the system call overhead of re-adding this timer to the event loop
  // only to receive EEXIST
  ...
  us_poll_start((struct us_poll_t *)t, internal_cb->loop,
                LIBUS_SOCKET_READABLE);
}
#else
void us_timer_close(struct us_timer_t *timer, int fallthrough) {
  struct us_internal_callback_t *internal_cb =
      (struct us_internal_callback_t *)timer;

  struct kevent64_s event;
  EV_SET64(&event, (uint64_t)(void *)internal_cb, EVFILT_TIMER, EV_DELETE, 0, 0,
           (uint64_t)internal_cb, 0, 0);
  int ret;
  do {
    ret = kevent64(internal_cb->loop->fd, &event, 1, &event, 1,
                   KEVENT_FLAG_ERROR_EVENTS, NULL);
  } while (IS_EINTR(ret));
...
void us_timer_set(struct us_timer_t *t, void (*cb)(struct us_timer_t *t),
  ...
  EV_SET64(&event, ptr, EVFILT_TIMER, EV_ADD | (repeat_ms ? 0 : EV_ONESHOT), 0,
           ms, (uint64_t)internal_cb, 0, 0);
...
void us_internal_async_set(struct us_internal_async *a,
  ...
  us_poll_start((struct us_poll_t *)a, internal_cb->loop,
                LIBUS_SOCKET_READABLE);
}
...
struct us_internal_async *us_internal_create_async(struct us_loop_t *loop,
  ...
  kern_return_t kr =
      mach_port_allocate(self, MACH_PORT_RIGHT_RECEIVE, &cb->port);
  ...
  kr =
      mach_port_insert_right(self, cb->port, cb->port, MACH_MSG_TYPE_MAKE_SEND);
  if (UNLIKELY(kr != KERN_SUCCESS)) {
    return NULL;
  }
  ...
  kr = mach_port_set_attributes(self, cb->port, MACH_PORT_LIMITS_INFO,
...
void us_internal_async_close(struct us_internal_async *a) {
  ...
  struct kevent64_s event;
  uint64_t ptr = (uint64_t)(void *)internal_cb;
  EV_SET64(&event, ptr, EVFILT_MACHPORT, EV_DELETE, 0, 0,
           (uint64_t)(void *)internal_cb, 0, 0);
...
void us_internal_async_set(struct us_internal_async *a,
  ...
  // EVFILT_MACHPORT benchmarks faster than EVFILT_USER when using multiple
  // threads Very old versions of macOS required them to be portsets instead of
  // ports but that is no longer the case There are not many examples on the
  // internet of using machports this way you can find one in Chromium's
  // codebase.
  struct kevent64_s event;
