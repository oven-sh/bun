...
int kqueue_change(int kqfd, int fd, int old_events, int new_events, void *user_data) {
    ...
    if(!is_readable && !is_writable) {
        ...
        EV_SET64(&change_list[change_length++], fd, EVFILT_WRITE, is_writable ? EV_ADD | EV_ONESHOT : EV_DELETE, 0, 0, (uint64_t)(void*)user_data, 0, 0);
    }
    int ret;
    do {
...
int us_poll_start_rc(struct us_poll_t *p, struct us_loop_t *loop, int events) {
    ...
    if(!(events & LIBUS_SOCKET_READABLE) && !(events & LIBUS_SOCKET_WRITABLE)) {
        ...
        events &= ~EPOLLONESHOT;
    } else if (events & LIBUS_SOCKET_WRITABLE) {
        events |= EPOLLONESHOT;
    }
    event.events = events;
    event.data.ptr = p;
