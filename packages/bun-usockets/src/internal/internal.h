...
struct us_connecting_socket_t {
     ...
     * closed_connecting_head. Guards against double-enqueue (double detach +
     * corrupted close list) and lets the drain skip a c that has already been
     * released but whose memory is not yet reclaimed by
     * us_internal_free_closed_sockets. */
    unsigned int closed : 1, shutdown : 1, shutdown_read : 1, pending_resolve_callback : 1, scheduled_for_free : 1;
    unsigned char timeout;
    unsigned char long_timeout;
    unsigned char kind;
    uint16_t port;
