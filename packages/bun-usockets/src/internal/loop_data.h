...
struct us_internal_loop_data_t {
    ...
    size_t iteration_nr;
    void* jsc_vm;
    /* Reentrancy depth of a loop tick: us_loop_run / us_loop_run_bun_tick on
     * epoll/kqueue, us_loop_run / us_loop_pump on libuv. When >1, we are inside
     * a nested tick (e.g. waitForPromise from a poll callback). Freeing closed
