...
void us_internal_loop_post(struct us_loop_t *loop) {
     ...
     * waitForPromise → us_loop_run_bun_tick, or us_loop_run / us_loop_pump on
     * libuv). The inner tick must not free closed sockets: the outer tick's
     * dispatch is mid-iteration and may still hold a pointer to one (it reads
     * s->flags right after on_data returns). Defer to the outermost tick's
     * loop_post. */
