...
void us_internal_dispatch_ready_poll(struct us_poll_t *p, int error, int eof, int events) {
    ...
    case POLL_TYPE_SEMI_SOCKET: {
             ...
             * but they poll for different events. A connecting socket polls
             * WRITABLE (us_socket_write* never reaches the kernel before the
             * connect completes, see us_internal_socket_is_connecting); listen
             * sockets only ever poll READABLE. Test the WRITABLE bit rather
             * than exact equality so an added READABLE interest cannot hide
             * the connect-complete. */
