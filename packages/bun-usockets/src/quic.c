...
void on_udp_socket_data_client(struct us_udp_socket_t *s,
  ...
  for (int i = 0; i < packets; i++) {
    ...
    int ret = lsquic_engine_packet_in(context->client_engine, payload, length,
...
void on_udp_socket_data(struct us_udp_socket_t *s,
                        struct us_udp_packet_buffer_t *buf, int packets) {
  ...
  for (int i = 0; i < packets; i++) {
    ...
    int ret = lsquic_engine_packet_in(context->engine, payload, length,
                                      (struct sockaddr *)&local_addr, peer_addr,
                                      (void *)s, 0);
    // printf("Engine returned: %d\n", ret);
  }
...
/* Let's use this on Windows and macOS where it is not defined (todo: put in
 * bsd.h) */
#ifndef UIO_MAXIOV
...
int send_packets_out(void *ctx, const struct lsquic_out_spec *specs,
                     unsigned n_specs) {
#ifndef _WIN32
  ...
  struct us_udp_socket_t *last_socket =
      (struct us_udp_socket_t *)specs[0].peer_ctx;

  int sent = 0;
  for (int i = 0; i < n_specs; i++) {
    /* Send this run if we need to */
    if (run_length == UIO_MAXIOV || specs[i].peer_ctx != last_socket) {
      int ret = bsd_sendmmsg(us_poll_fd((struct us_poll_t *)last_socket), hdrs,
                             run_length, 0);
      if (ret != run_length) {
        if (ret == -1) {
    ...
    hdrs[run_length].msg_hdr.msg_namelen =
        (AF_INET == specs[i].dest_sa->sa_family ? sizeof(struct sockaddr_in)
...
void on_conn_closed(lsquic_conn_t *c) {
  us_quic_socket_context_t *context =
      (us_quic_socket_context_t *)lsquic_conn_get_ctx(c);
...
lsquic_stream_ctx_t *on_new_stream(void *stream_if_ctx, lsquic_stream_t *s) {
  ...
  // the ext size of streams and conn's are set by the listen/connect calls,
  // which are the calls that create the UDP socket so we need conn to point to
  // the UDP socket to get that ext_size set in listen/connect calls, back here.
...
int header_set_ptr(struct lsxpack_header *hdr, struct header_buf *header_buf,
...
void us_quic_socket_context_send_headers(us_quic_socket_context_t *context,
  ...
  if (lsquic_stream_send_headers((lsquic_stream_t *)s, &headers,
                                 has_body ? 0 : 1)) { // pass 0 if data
...
int us_quic_stream_is_client(us_quic_stream_t *s) {
  us_quic_socket_context_t *context =
      (us_quic_socket_context_t *)lsquic_conn_get_ctx(
          lsquic_stream_conn((lsquic_stream_t *)s));

  int is_client = 0;
  if (lsquic_conn_get_engine(lsquic_stream_conn((lsquic_stream_t *)s)) ==
...
static void on_read(lsquic_stream_t *s, lsquic_stream_ctx_t *h) {
  ...
  /* The user data of the connection owning the stream, points to the socket
   * context */
  us_quic_socket_context_t *context =
      (us_quic_socket_context_t *)lsquic_conn_get_ctx(lsquic_stream_conn(s));

  /* This object is (and must be) fetched from a stream by
   * calling lsquic_stream_get_hset() before the stream can be read. */
  /* This call must precede calls to lsquic_stream_read(),
   * lsquic_stream_readv(), and lsquic_stream_readf(). */
  ...
  // all of this logic should be moved to uws and WE here should only hand over
  // the data
  ...
  if (nr == 0) {
    ...
    // if we for some reason could not read even though we were told to read, we
    // just ignore it this should not really happen but whatever
  } else {
  ...
  if (nr == 0) {
    ...
    if (lsquic_stream_wantwrite(s, 1)) {
      ...
      // we happened to be polling for writable so leave the connection open
      // until on_write eventually closes it
      ...
      // I guess we can close it since we have called shutdown before this so
      // data should flow out
...
static void on_write(lsquic_stream_t *s, lsquic_stream_ctx_t *h) {
  ...
  // here we might want to check if the user did write to failure or not, and if
  // the user did not write, stop polling for writable i think that is what we
  // do for http1
}
...
struct ssl_ctx_st *get_ssl_ctx(void *peer_ctx, const struct sockaddr *local) {
  ...
  int b = SSL_CTX_use_PrivateKey_file(ctx, options->key_file_name,
                                      SSL_FILETYPE_PEM);
...
SSL_CTX *sni_lookup(void *lsquic_cert_lookup_ctx, const struct sockaddr *local,
                    const char *sni) {
...
void *hsi_create_header_set(void *hsi_ctx, lsquic_stream_t *stream,
  ...
  // I guess we just return whatever here, what we return here is gettable via
  // the stream
...
// one header set allocates one 8kb buffer from a linked list of available
// buffers
...
hsi_prepare_decode(void *hdr_set, struct lsxpack_header *hdr, size_t space) {

  // printf("hsi_prepare_decode\n");

  if (!hdr) {
    char *mem = take();
    hdr = (struct lsxpack_header *)mem; // malloc(sizeof(struct
                                        // lsxpack_header));
...
// this will be for both client and server, but will be only for either h3 or
// raw quic
us_quic_socket_context_t *
us_create_quic_socket_context(struct us_loop_t *loop,
                              us_quic_socket_context_options_t options,
                              int ext_size) {

  printf("Creating socket context with ssl: %s\n", options.key_file_name);

  // every _listen_ call creates a new udp socket that feeds inputs to the
  // engine in the context every context has its own send buffer and udp send
  // socket (not bound to any port or ip?)

  // or just make it so that once you listen, it will listen on that port for
  // input, and the context will use the first udp socket for output as it
  // doesn't matter which one is used

  /* Holds all callbacks */
  us_quic_socket_context_t *context =
      malloc(sizeof(struct us_quic_socket_context_s) + ext_size);
...
us_quic_socket_context_listen(us_quic_socket_context_t *context,
                              const char *host, int port, int ext_size) {
  /* We literally do create a listen socket */
  return (us_quic_listen_socket_t *)us_create_udp_socket(
      context->loop, /*context->recv_buf*/ NULL, on_udp_socket_data,
      on_udp_socket_writable, host, port, context);
  // return NULL;
}

/* A client connection is its own UDP socket, while a server connection makes
 * use of the shared listen UDP socket */
us_quic_socket_t *
us_quic_socket_context_connect(us_quic_socket_context_t *context,
                               const char *host, int port, int ext_size) {
  printf("Connecting..\n");
  ...
  struct us_udp_socket_t *udp_socket = us_create_udp_socket(
      context->loop, /*context->recv_buf*/ NULL, on_udp_socket_data_client,
      on_udp_socket_writable, 0, 0, context);
  ...
  // we need 1 socket for servers, then we bind multiple ports to that one
  // socket

  void *client = lsquic_engine_connect(
      context->client_engine, LSQVER_I001, (struct sockaddr *)local_addr,
      (struct sockaddr *)addr, udp_socket, (lsquic_conn_ctx_t *)udp_socket,
      "sni", 0, 0, 0, 0, 0);
