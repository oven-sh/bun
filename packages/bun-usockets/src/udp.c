...
void us_udp_buffer_set_packet_payload(struct us_udp_packet_buffer_t *send_buf,
  ...
  bsd_udp_buffer_set_packet_payload(send_buf, index, offset, payload, length,
                                    peer_addr);
}
...
struct us_internal_udp_t {
  ...
  void *user;
  /* An UDP socket can only ever be bound to one single port regardless
of how
   * many interfaces it may listen to. Therefore we cache the port after
creation
   * and use it to build a proper and full sockaddr_in or sockaddr_in6
...
us_create_udp_socket(struct us_loop_t *loop, struct us_udp_packet_buffer_t *buf,
                     void (*data_cb)(struct us_udp_socket_t *,
                                     struct us_udp_packet_buffer_t *, int),
                     void (*drain_cb)(struct us_udp_socket_t *),
  ...
  struct us_poll_t *p = us_create_poll(
      loop, fallthrough, sizeof(struct us_internal_udp_t) + ext_size);
