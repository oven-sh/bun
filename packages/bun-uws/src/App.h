...
static_assert(sizeof(struct us_bun_socket_context_options_t) ==
...
public:
  ...
  bool publish(std::string_view topic, std::string_view message,
               unsigned char opCode, bool compress = false) {
  ...
  TemplatedApp &&listen(int options,
                        ...
                        std::string path) {
    handler(httpContext
                ? httpContext->listen_unix(path.data(), path.length(), options)
                : nullptr);
    return std::move(*this);
  }
