...
private:
    ...
    /* TCP listen handlers get (listen socket or nullptr, dnsError). dnsError is the
     * getaddrinfo(3) return code when the host failed to resolve, else 0; any other
     * failure leaves the listen socket null with dnsError == 0 and errno set. */
