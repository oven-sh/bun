...
struct Http3ResponseData {
    ...
    /* Set when the final response header block can't be sent yet because an
     * interim (100-continue) block is still draining in lsquic (#33082). The
     * buffered headers stay in hdrs/hdrBuf and body bytes route to
     * backpressure until the writable event flushes them in order. */
