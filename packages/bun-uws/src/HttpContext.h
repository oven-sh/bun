...
private:
    ...
    static us_socket_t *onData(us_socket_t *s, char *data, int length) {
        ...
        auto result = httpResponseData->template consumePostPadded<IsNodeHttp>(httpContextData->maxHeaderSize, httpResponseData->isConnectRequest, httpContextData->flags.requireHostHeader,httpContextData->flags.useStrictMethodValidation, httpContextData->flags.useInsecureHTTPParser, httpContextData->flags.useLenientTransferEncoding, nodeHttpRequestTrailers, &httpResponseData->chunkedExtensionsByteCount, data, (unsigned int) length, s, [httpContextData](void *s, HttpRequest *httpRequest) -> void * {
            ...
            /* Bun.serve, RFC 9112 9.6: a response marked this connection close and
             * sawConnectionClose did not see it (a Connection: close response header).
             * Process nothing behind it: the reset below would drop the mark, and the
             * pending check would cut short a response still in flight. Latch, because
             * the socket can stay open and the parser holds this request's framing, then
             * run onData's tail. Before the timeout reset, which would disarm the timeout
             * of a socket left to onWritable. */
            if constexpr (!IsNodeHttp) {
                if (httpResponseData->state & HttpResponseData<SSL>::HTTP_CONNECTION_CLOSE) [[unlikely]] {
