...
private:
    ...
    static us_socket_t *onData(us_socket_t *s, char *data, int length) {
        ...
        if (httpContextData->upgradedWebSocket) {
             ...
             * WebSocket now, as the loop would have for a read of its own. Not when
             * upgradedWebSocket names another connection (upgrade() adopts in place, so ours is
             * s): the field is per context, and a microtask of this dispatch, or an earlier
             * upgrade from a request body handler, can set it. */
