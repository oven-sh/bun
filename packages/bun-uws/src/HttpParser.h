...
private:
  ...
  static unsigned int getHeaders(char *postPaddedBuffer, char *end,
                                 ...
                                 void *reserved, bool *isAncientHttp) {
...
public:
  void *consumePostPadded(
      char *data, unsigned int length, void *user, void *reserved,
      MoveOnlyFunction<void *(void *, HttpRequest *)> &&requestHandler,
      MoveOnlyFunction<void *(void *, std::string_view, bool)> &&dataHandler,
      MoveOnlyFunction<void *(void *)> &&errorHandler) {
    ...
    if (remainingStreamingBytes) {
      ...
      if (consumed.first) {
        ...
        if (fallback.length() == MAX_FALLBACK_SIZE) {
          // note: you don't really need error handler, just return something
          // strange! we could have it return a constant pointer to denote
          // error!
