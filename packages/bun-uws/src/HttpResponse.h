...
public:
    ...
    us_socket_t *upgrade(UserData&& userData, std::string_view secWebSocketKey, std::string_view secWebSocketProtocol,
        ...
        /* upgradedWebSocket tells onData that the socket it parses changed.
         * A handler can upgrade another connection's parked request from
         * inside the parse; to the parser that is an "async" upgrade and must
         * leave the parsed socket alone. Decided before the adoption below,
         * which may move this socket. */
        ...
        if (httpContextData->flags.isParsingHttp && upgradingParsedSocket) {
