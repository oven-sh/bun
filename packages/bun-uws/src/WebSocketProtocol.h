...
protected:
  ...
  static inline void unmaskImprecise(char *dst, char *src, char *mask,
                                     unsigned int length) {
  ...
  consumeMessage(T payLength, char *&src, unsigned int &length,
    ...
    if (payLength + MESSAGE_HEADER <= length) {
      if (isServer) {
        ...
        if (Impl::handleFragment(src + MESSAGE_HEADER - 4, payLength, 0,
      ...
      if (isServer) {
        ...
        unmaskImprecise(src, src + MESSAGE_HEADER, wState->mask,
                        length - MESSAGE_HEADER);
        rotateMask(4 - (length - MESSAGE_HEADER) % 4, wState->mask);
      } else {
        src += MESSAGE_HEADER;
      }
      Impl::handleFragment(src, length - MESSAGE_HEADER, wState->remainingBytes,
                           wState->state.opCode[wState->state.opStack], fin,
                           wState, user);
      return true;
    }
  }
