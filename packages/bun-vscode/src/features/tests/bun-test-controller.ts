...
export class BunTestController implements vscode.Disposable {
    ...
    private handleOpenDocument(document: vscode.TextDocument): void {
        if (this.isTestFile(document) &&
            !this.testController.items.get(document.uri.toString())) {
    ...
    private isTestFile(document: vscode.TextDocument): boolean {
        return document?.uri?.scheme === "file" &&
            /\.(test|spec)\.(js|jsx|ts|tsx|cjs|mts)$/.test(document.uri.fsPath);
    }
    ...
    private parseTestBlocks(fileContent: string): TestNode[] {
        ...
        const testRegex = /\b(describe|test|it)(?:\.(?:skip|todo|failing|only))?(?:\.(?:if|todoIf|skipIf)\s*\([^)]*\))?(?:\.each\s*\([^)]*\))?\s*\(\s*(['"`])((?:\\\2|.)*?)\2\s*,/g;
            ...
            try {
                ...
                if (!isFileOnly && hasManyTests()) {
                    ...
                    if (testNames.length > 0) {
                        const testNamesRegex = testNames
                            .map(pattern => `(${pattern})`)
                            .join('|');
                ...
                const printArgs = args.map(e => (e === "test" || e.startsWith("\"") || e.startsWith("--") || e.startsWith("\'")) ? e : `"${e}"`).join(' ');
                ...
                await new Promise<void>((resolve, reject) => {
                    ...
                    proc.stdout?.on('data', (data) => {
                        ...
                        const filtered = chunk
                            .split(/\r?\n/)
                            .filter((line: string) => !line.startsWith("::group::") && !line.startsWith("::error ") && !line.startsWith("::endgroup::"))
                            .join("\r\n");
                        ...
                        try {
                            if (code === 0 || code === 1) {
                                ...
                                if (isFileOnly) {
                                    ...
                                    const fileResult = parsedOutput.find(
    ...
    private async debugTests(
        ...
        for (const test of tests) {
            let t = test.id.includes('#') ?
                test.id.slice(test.id.indexOf('#') + 1).split(' > ').join(" ") :
                test.label;
...
function getFileLocationFromError(error: string, _expectedFile: string): { file: string; line: number; column: number } | undefined {
...
function parseBunTestOutput(
    ...
    while (match !== null) {
        ...
        const errorLineIdx = lines.findIndex((l, idx) =>
            idx > lastErrorIndex &&
            l.includes(`::error file=${file},line=${line},col=${col},title=${title}::`)
        );
    ...
    if (lines && errors && errors.length > 0) {
        ...
        for (const testLine of testLineInfos) {
            ...
            if (bestError) {
                errorMsgMap.set(JSON.stringify(testLine.fullName), `${bestError.title.trim()}\n${bestError.message.trim()}`.trim());
            }
        }
    }
        ...
        if (suite.testsuite) {
            ...
            for (const childSuite of suites as Record<string, unknown>[]) {
                ...
                const file = childSuite.file
                    ...
                    : (suite.file
                        ? (path.isAbsolute(suite.file as string) ? suite.file as string : path.resolve(workspacePath, suite.file as string))
                        : undefined);
            ...
            for (const testcase of testcases as Record<string, unknown>[]) {
                ...
                const file = testcase.file
                    ...
                    : (suite.file
                        ? (path.isAbsolute(suite.file as string) ? suite.file as string : path.resolve(workspacePath, suite.file as string))
                        : undefined);
                const status = testcase.skipped !== undefined
                    ? "skipped"
...
function processErrorData({ testResult, testItem }: { testResult: BunTestResult; testItem: vscode.TestItem }): vscode.TestMessage | undefined {
    if (!testResult.message) {
        return undefined;
    }

    const messageLinesRaw = (testResult.message).split('\n');
    const stackTraceIndex = messageLinesRaw.findIndex(line =>
        line.trim().startsWith('at ') && line.includes('(') && line.includes(':')
    ...
    switch (errorType) {
        ...
        case 'expect(received).toBe(expected)': {
            ...
            if (!messageLines.match(regex)) {
                const code = messageLines
                    ...
                    .replace(/(?:\r?\n)+(- Expected\s+- \d+|\+ Received\s+\+ \d+)\s*$/g, "")
                    .trim();
                message = new vscode.TestMessage(new vscode.MarkdownString("Values did not match:\n")
        ...
        case 'expect(received).toBeTruthy()': {
            const message = vscode.TestMessage.diff(
                errorLine,
                "true",
                messageLines.replace("Received: ", "").trim(),
            );
            message.location = messageLocation;
            return message;
        }
        ...
        case 'expect(received).toBeGreaterThan(expected)': {
            ...
            const message = new vscode.TestMessage(`${messageLines.trim().match(regex)?.[2]?.trim()} isn't ${messageLines.trim().match(regex)?.[1]?.trim()}\n\n${messageLines}`);
            message.location = messageLocation;
            return message;
        }
