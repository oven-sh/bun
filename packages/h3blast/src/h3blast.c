...
static const char *C_RST = "\x1b[0m", *C_DIM = "\x1b[2m", *C_BLD = "\x1b[1m";
static const char *C_RED = "\x1b[31m", *C_GRN = "\x1b[32m", *C_YLW = "\x1b[33m";
static const char *C_BLU = "\x1b[34m", *C_MAG = "\x1b[35m", *C_CYN = "\x1b[36m";
static const char *C_GRY = "\x1b[90m";

static void disable_color(void) {
    C_RST = C_DIM = C_BLD = C_RED = C_GRN = C_YLW = C_BLU = C_MAG = C_CYN = C_GRY = "";
}
...
static void die(const char *fmt, ...) {
    va_list ap;
    va_start(ap, fmt);
    fprintf(stderr, "%serror%s ", g_isatty ? C_RED C_BLD : "", g_isatty ? C_RST : "");
...
static void draw_bar(FILE *f, double frac, int width) {
    ...
    fprintf(f, C_CYN);
    for (int i = 0; i < width; i++) fputs(i < filled ? "━" : C_GRY "━" C_CYN, f);
    fprintf(f, C_RST);
}
...
static void render_live(const struct config *cfg, struct worker *ws, int nw,
    ...
    fprintf(f, "\x1b[2K  %s%s%s  %s%12s%s %sreq/s%s    %speak %s%s\n",
            C_CYN, spinner_frames[spin % 10], C_RST,
            C_BLD C_GRN, b_rps, C_RST, C_DIM, C_RST,
            C_DIM, ({ static char p[32]; human_count(peak_rps, p, sizeof(p)); p; }), C_RST);

    fprintf(f, "\x1b[2K     %s↓%s %-10s %s↑%s %-10s %sconns%s %" PRIu64 "   %stotal%s %s\n",
            C_CYN, C_RST, b_rx, C_MAG, C_RST, b_tx,
            C_DIM, C_RST, cur->conns,
            C_DIM, C_RST, ({ static char t[32]; human_count((double)cur->done, t, sizeof(t)); t; }));

    fprintf(f, "\x1b[2K     ");
    if (cfg->duration_s > 0) {
        draw_bar(f, elapsed / cfg->duration_s, 34);
        fprintf(f, "  %s%.1fs / %.0fs%s\n", C_DIM, elapsed, cfg->duration_s, C_RST);
    } else {
    ...
    fprintf(f, "\x1b[2K     %s2xx%s %-9" PRIu64 " %s4xx%s %-7" PRIu64 " %s5xx%s %-7" PRIu64
            " %serr%s %-7" PRIu64 "\n",
            C_GRN, C_RST, cur->s2, C_YLW, C_RST, cur->s4,
            C_RED, C_RST, cur->s5, C_RED, C_RST, cur->err);
    ...
    if (show_workers) {
        ...
        for (int i = 0; i < show_workers; i++) {
            ...
            fprintf(f, "\x1b[2K     %sw%-2d%s ", C_DIM, i, C_RST);
...
static void render_final(const struct config *cfg, struct worker *ws, int nw,
    ...
    fprintf(f, "  %s%s┃%s %sh3blast%s  %s%s %s%s%s:%s%s\n",
            ...
            cfg->method, C_DIM, C_RST, cfg->host, C_DIM, C_RST, cfg->port);
    fprintf(f, "  %s%s┃%s %s%s%s\n", C_BLD, C_CYN, C_RST, C_DIM, cfg->path, C_RST);
    ...
    fprintf(f, "  ");
    hr(f, 50);
    fprintf(f, "    %s%s%s %sreq/s%s\n", C_BLD C_GRN, rps_full, C_RST, C_DIM, C_RST);
    fprintf(f, "  ");
    hr(f, 50);
    fprintf(f, "\n  %s%-10s%s %s in %.2fs\n", C_DIM, "requests", C_RST, b_total, elapsed);
    fprintf(f, "  %s%-10s%s ↓ %s (%s/s)   ↑ %s\n\n", C_DIM, "transfer", C_RST, b_rx, b_rxps, b_tx);
    ...
    for (size_t i = 0; i < sizeof(rows)/sizeof(rows[0]); i++) {
        ...
        fprintf(f, "  %s%-7s%s %s%-10s%s ", C_DIM, rows[i].label, C_RST, C_BLD, lat, C_RST);
        fputs(C_CYN, f);
    ...
    fprintf(f, "  %s%-7s%s %-10s %s± %s%s\n", C_DIM, "mean", C_RST, mean, C_DIM, stdd, C_RST);

    fprintf(f, "\n  %sStatus%s\n  ", C_BLD, C_RST);
    hr(f, 50);
    struct { const char *l; const char *c; uint64_t n; } st[] = {
        {"2xx", C_GRN, s->s2}, {"3xx", C_BLU, s->s3},
        {"4xx", C_YLW, s->s4}, {"5xx", C_RED, s->s5},
        {"other", C_GRY, s->so}, {"errors", C_RED, s->err},
    };
    ...
    for (size_t i = 0; i < sizeof(st)/sizeof(st[0]); i++) {
        ...
        fprintf(f, "  %s%-7s%s %-10" PRIu64 " %s%5.1f%%%s  %s",
                C_DIM, st[i].l, C_RST, st[i].n, C_DIM, pct, C_RST, st[i].c);
        for (int j = 0; j < barw; j++) fputs("▇", f);
        fputs(C_RST "\n", f);
    }
    if (s->hsk_fail)
        fprintf(f, "  %s%-7s%s %" PRIu64 "\n", C_RED, "hsk-fail", C_RST, s->hsk_fail);
...
static void usage(const char *argv0) {
    fprintf(stderr,
        ...
        C_BLD, C_RST, H3B_VERSION, C_BLD, argv0, C_RST,
...
int main(int argc, char **argv) {
    ...
    if (g_isatty && !cfg.quiet) {
        fprintf(stderr, "\n  %s┃%s %sh3blast%s → %s%s%s:%s%s%s%s   "
                ...
                C_CYN C_BLD, C_RST, C_BLD, C_RST,
                C_BLD, cfg.host, C_DIM, C_RST, cfg.port, C_DIM, C_RST,
                C_BLD, cfg.threads, C_RST, C_BLD, cfg.connections, C_RST,
                C_BLD, cfg.streams, C_RST);
    }
