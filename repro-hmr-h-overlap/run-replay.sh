...
run_one() {
  local seed="$1"
  BUN_DEBUG_QUIET_LOGS=1 timeout 900 "$BIN" "$(dirname "$0")/replay29.mjs" "$seed" "$ROUNDS" > "$OUT/seed-$seed.log" 2>&1
  echo "seed $seed exit $?" >> "$OUT/summary.txt"
}
export -f run_one
export BIN OUT ROUNDS PAUSE_MS EPHEMERAL BUN_ASSUME_PERFECT_INCREMENTAL EPH_DELAY_MS NO_H
