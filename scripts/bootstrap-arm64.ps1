# Version: 1
# A script that installs the dependencies needed to build and test Bun on Windows ARM64.
# This is separate from bootstrap.ps1 because Chocolatey does not support ARM64 packages.
# Uses Scoop for native ARM64 packages; manual installs for packages not in Scoop.
...
function Install-Ccache {
  ...
  $version = "4.12.2"
  Write-Output "Installing ccache $version (ARM64)..."
  $zip = Download-File "https://github.com/ccache/ccache/releases/download/v$version/ccache-$version-windows-aarch64.zip" -Name "ccache-arm64.zip"
  $extractDir = "$env:TEMP\ccache-extract"
  Expand-Archive $zip $extractDir -Force
  $installDir = "$env:ProgramFiles\ccache"
  New-Item -Path $installDir -ItemType Directory -Force | Out-Null
  # The zip contains a subdirectory
  Copy-Item "$extractDir\ccache-$version-windows-aarch64\*" $installDir -Recurse -Force
...
# Native ARM64 packages via Scoop
Install-Git
Install-NodeJs
Install-7zip
Install-CMake
...
# Manual ARM64 installs (not in Scoop or need special handling)
Install-Pwsh
Install-Bun
Install-Ccache
Install-Rust
...
# Perl (x64, runs under WoW64 emulation — no ARM64 build of Strawberry Perl)
