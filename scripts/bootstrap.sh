...
install_windows_sysroot() {
	...
	*) xwin_triple="x86_64-unknown-linux-musl" ;;
	...
	execute_sudo rm -rf "$sysroot"
	execute_sudo mkdir -p "$sysroot"
	# Cache dir must share a filesystem with --output: xwin splat rename()s
	# files from the cache into the output tree, and the download dir lives
	# under /tmp which can be tmpfs — a cross-device cache dies with
	# "Cross-device link (os error 18)". /opt/winsysroot-cache sits on the
	# same filesystem as the output; removed below.
