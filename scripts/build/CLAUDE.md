...
| `build-lock.ts`                | `lockBuildDir()` — one build at a time per build directory                                                                                                              |
| `proc.ts`                      | `processAlive()`/`processStartTime()` — telling a lock holder from a later process with the same pid                                                                    |
