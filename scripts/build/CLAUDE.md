...
**macOS machines** (`macosMachines` in the spec) get a generated `bootstrap.sh` from the same pins, but CI does not bake or name them: `scripts/darwin-ci` generates the script on the host and runs it in the Tart guest image it builds, or on a bare host. It runs as the machine's admin user, since Homebrew refuses root, and uses `sudo` for system paths. TODO for whoever sets up managed, ephemeral macOS machines: bake them like the others, named by hash.
