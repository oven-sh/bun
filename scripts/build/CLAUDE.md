...
- **Which ninja:** `resolveNinja()` (`ninja-release.ts`) — the oven-sh/ninja release pinned in `ci-images/spec.ts` (`pins.bunNinja`): the one a CI image has at `locations.bunNinja`, else fetched once into `cfg.cacheDir` and checked against the pinned sha256; `ninja` from PATH if there is no release for the host or it can't be fetched. Fetched by the driver because it is what runs the edges, and configure never fetches. Configure's `-t restat` and the helper scripts use `ninjaIfPresent()` — the same binary, never a fetch: ninja versions disagree on the `.ninja_log` format and rewrite or delete a log they don't recognise, so everything that touches a build directory must be one ninja.
...
| `proc.ts`                      | `processAlive()`/`processCommandLine()` — identity check behind the build-directory lock's pid file                                                                     |
