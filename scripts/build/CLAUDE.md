...
`bun run build --timings` builds as usual, then reports where the time went (`timings.ts`) and writes `<buildDir>/timings.html` (`timings-chart.ts`): every run of ninja as a Gantt chart, with the critical path in the top rows as a staircase and a strip of how many commands were running. Every CI build does the same: the report is the "Build timings" group of the step's log, and the chart and trace are uploaded as `timings-<target>-<mode>.html` / `-trace.json`, linked from a "build timings" annotation on the build page (`ci.ts` `publishTimings`). Nothing is measured for it and no second build runs: it reads what every build leaves behind.
