...
7. **Link** — `emitBunLink()`: `emitShims(n, cfg)` for platform workaround dylibs, then `link()`. A build that traces its own symbol order (`cfg.traceOrderFile`: CI sets it for releases and for a canary with nothing to inherit, `--traceOrderFile=on` for a local release) links twice in the one graph: `bun-profile-unordered` without the order file, an `order_file_trace` edge that runs that binary under the tracer and writes `linker.order`, and `bun-profile` against it. A link cannot read a file traced from its own output, so the two links are two edges with two outputs. The flags of the two are the same but for the order file and the names of the maps (`flags.ts` `LinkOf`). A failed trace writes an order file that orders nothing and annotates the build; it does not fail it.
...
| `trace-order-file.ts`          | Build-time CLI of the `order_file_trace` edge: traces the unordered binary (`scripts/orderfile/generate.ts`) and writes the symbol ordering file the final link reads   |
