...
| `toolchain-identity.ts`        | `<buildDir>/toolchain-identity.txt`: what each tool path resolves to; an input of every edge that runs one, so a replaced compiler rebuilds its objects                 |
