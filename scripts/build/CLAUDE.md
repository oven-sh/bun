...
- `none` — no build step: header-only deps, and the Rust crates (lolhtml, rust-argon2) that are only fetched into `vendor/` so the workspace cargo build in `rust.ts` can read them as path dependencies.
...
3. **Deps** — loop `allDeps`, call `resolveDep(n, cfg, dep)`. Each emits fetch → configure → build (nested-cmake), or fetch → direct cc+ar, or fetch only (none), or prebuilt download. Collects lib paths, include dirs, outputs.
