...
**rustc links nothing of bun.** `bun_runtime` is the final Rust crate, a `staticlib` to rustc, which keeps its generic instances private and folds single-caller code before the link; rustc is asked for its objects (`--emit=obj`), never the archive, and `run.ts` lists them in a response file the link reads beside every other crate's rlib. When the profile has rustc run an LTO (`lto = "fat"`: targets without cross-language LTO) the root is a library instead and the linker runs that LTO (`rust/units.ts` `UnitKind` says why). What rustc decides only when _it_ links is decided here: the allocator marker `__rust_no_alloc_shim_is_unstable_v2` is defined in `src/runtime/bin_entry/mod.rs` (the allocator itself comes from `#[global_allocator]` there, the error handler from std), and `rust.ts` leaves the panic runtime the profile does not use (`panic_unwind` under `panic = "abort"`) out of the link. Release Rust is bitcode in the rlibs (`-C linker-plugin-lto`) and lld optimises it, together with the C++ where cross-language LTO is on.
