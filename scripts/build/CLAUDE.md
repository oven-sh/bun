...
**macOS is not part of this yet.** The `test-darwin` machines are provisioned by `scripts/darwin-ci`, which runs the hand-written `scripts/bootstrap.sh` (in the Tart guest image it bakes, or on a bare host), so that script still carries its own copy of the versions for macOS, and the generator outputs nothing for macOS. TODO for whoever sets up managed, ephemeral macOS machines: give macOS an `Image` kind, tools and a generated `bootstrap.sh` like the Linux ones, and delete `scripts/bootstrap.sh`.
