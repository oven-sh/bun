...
| `timings.ts`                   | `--timings`: where a build directory's time went, from `.ninja_log`, `build.ninja` and output mtimes                                                                    |
| `timings-chart.ts`             | `timings.html`: every run as a Gantt chart, the critical path on top; one self-contained page                                                                           |
...
`bun run build --timings` builds as usual, then reports where the time went (`timings.ts`) and writes `<buildDir>/timings.html` (`timings-chart.ts`): every run of ninja as a Gantt chart with a lane per kind of command, the critical path outlined, and a strip of how many commands of each kind were running. Hovering a bar draws the chain of commands it waited on; clicking pins that and lights everything it held up. Every CI build does the same: the report is the "Build timings" group of the step's log, and the chart is uploaded as `timings-<target>-<mode>.html`, which Buildkite serves as a page, linked from a "build timings" annotation on the build page (`ci.ts` `publishTimings`). Nothing is measured for it and no second build runs: it reads what every build leaves behind.

- `.ninja_log` — each command's start and end on its ninja's clock, and a file timestamp: the command's start on the wall clock, or, for a `restat`/`generator` rule that changed its output, that output's mtime (ninja `Builder::FinishCommand`). One `build.ts` can be several ninja processes (CI calls ninja up to three times) and ninja compacts the log in no particular order, so processes are told apart by that timestamp, not by line order. ninja also starts over by itself, counting from zero again, when bringing `build.ninja` up to date rewrote it (every CI build: the Rust plan is new); a process that ended by writing `build.ninja` and the one that began where it ended are one run, on one clock. The `build.ninja` entry is the one whose timestamp is not its own: every configure stamps the file and has ninja record it (`-t restat`), so that command is placed by its start and end instead, in the process where one of its inputs ended right as it started.
- `build.ninja` — read back by `readManifest()` for each edge's rule, pool, inputs and outputs. No build statement names `build.ninja` as an input, but ninja brings it up to date before anything else, so the timings treat the edge that writes it as an input of every edge that is not upstream of it. An edge is named the way ninja prints it (the rule's `description`).
- Output mtimes — a library crate's dependents start at its `.rmeta`, long before the edge ends. `rust/run.ts` stamps the `.rmeta` when rustc reports it, so `mtime − the log's start stamp` is how far into the command that was. Any edge with `early_output_prefix` is read this way.

**The report describes the build directory, not one invocation**: for every edge, the last time it ran. It reads the same after a build that had nothing to do, and it is all ninja keeps once it compacts the log. Totals per rule, the slowest edges and the critical path come from that. The critical path is computed (longest chain through the graph, each edge taking what it last took and starting when its inputs exist), so it does not depend on which run built what; it is what a build of everything takes with every core free. What exists only within one run of ninja — stretches with few commands running, how long edges waited after their inputs existed (per pool) — is reported for the most recent run, the one run nothing can have overwritten part of. Locally, `<buildDir>/timings-trace.json` has the same runs as Chrome trace events (Perfetto, `chrome://tracing`).

**Why an edge is slow** is the compilers' to say, and costs a rebuild: `--time-trace=on` adds `-ftime-trace` to bun's C++ (a `.json` beside each `.o`) and `-Z time-passes` to every rustc (`run.ts` records the passes in `<output>.phases.json`; the flag is not a rustflag, so crates keep their hashes). The report shows the largest phases under each slow edge, the chart shows them on hover, and the trace nests them inside the edge.
