...
export function emitBun(n: Ninja, cfg: Config, sources: Sources): BunOutput {
  ...
  if (usePch) {
    n.comment("─── PCH ───");
    n.blank();
    // Dep outputs and codegen headers are IMPLICIT inputs (not order-only):
    // both are regenerated mid-run (local WebKit headers by dep_build, codegen
    // headers by their step), and the PCH's depfile records them by absolute
    // path, which is not the node the producing edge declares. At startup ninja
    // sees the old header → thinks the PCH is fresh → cxx compiles against a
    // stale PCH (or fails with "file modified since PCH was built"). As
    // implicit inputs, restat sees the regenerated file → PCH rebuilds in the
    // same run. See the pch() docstring.
    //
    // cppAll (not all) — bake/.rs outputs are rust-only; pulling them here
    // would run bake-codegen in cpp-only CI mode where it fails on the pinned
    // bun version (see cppAll docstring). cxx transitively waits: cxx → PCH →
    // deps+cppAll.
    pchOut = pch(n, cfg, "src/jsc/bindings/root-pch.h", {
      flags: cxxFlagsFull,
      implicitInputs: [...depHeaderSignal, ...codegen.cppAll],
    });
  }
