 ...
 */
export function glibcSysroot(pin: { gccDebsUrl: string }, sysroot: { x64: string; aarch64: string }): Tool {
  const debs = (arch: string) => `${pin.gccDebsUrl}/gcc-13-focal-${arch}.tar.gz`;
  return {
    name: "glibc-sysroot",
    script: "linux/glibc-sysroot.sh",
    variables: {
      GCC_DEBS_AMD64_URL: debs("amd64"),
      GCC_DEBS_ARM64_URL: debs("arm64"),
      GLIBC_SYSROOT_X64: sysroot.x64,
      GLIBC_SYSROOT_AARCH64: sysroot.aarch64,
    },
  };
}
