...
$installed = buildkite-agent --version
if ($installed -notmatch "version $([regex]::Escape($BUILDKITE_AGENT_VERSION))") { Fail "buildkite-agent --version is '$installed', expected $BUILDKITE_AGENT_VERSION" }
