...
import { builtinBytecodeEmbeddable, builtinBytecodePath, orderFilePath, usesOrderFile } from "./flags.ts";
...
export function packageAndUpload(cfg: Config, output: BunOutput): void {
  ...
  // Builtin bytecode: a canary lane running its own binary generates the blob
  // the NEXT build inherits (never this one — no relink). publishBuiltinBytecode
  // records the SHA256 pin in this build's meta-data before we upload.
  if (output.exe) {
    const blobArtifact = publishBuiltinBytecode(cfg, orderFileContext(), output.exe);
    if (blobArtifact) zipPaths.push(basename(blobArtifact));
  }
...
// ---------------------------------------------------------------------------
// Builtin bytecode blob — inherited from an earlier canary, never generated by
// the build that embeds it (that would need a second link). A canary lane that
// can run its own binary publishes the blob for the *next* build; every eligible
// build (canary or release) inherits the newest one. Same chain as the order file.
//
// Trust: the artifact is pinned by SHA256 recorded in the *publishing build's*
// Buildkite meta-data (`builtin-bytecode-sha256:<triplet>`); an inherited blob
// whose digest does not match that record is discarded, so artifact storage is
// not a trust root. Staleness is not a correctness concern: entries are keyed by
// module source hash + JSC bytecode-cache version, so mismatches fall back to
// parsing that module from source at runtime.
// ---------------------------------------------------------------------------

/** Artifact name for the standalone blob: `bun-linux-x64.builtin-bytecode`. */
function builtinBytecodeArtifact(cfg: Config): string {
  return `${computeBunTriplet(cfg)}.builtin-bytecode`;
}

/** Meta-data key holding the SHA256 of the blob a build published for a triplet. */
function builtinBytecodeShaKey(cfg: Config): string {
  return `builtin-bytecode-sha256:${computeBunTriplet(cfg)}`;
}
...
/** Same lanes as the order file: Buildkite, non-PR, link-producing modes. */
export function builtinBytecodeEligible(cfg: Config, ctx: OrderFileContext): boolean {
  if (!builtinBytecodeEmbeddable(cfg) || !ctx.buildkite || ctx.pullRequest) return false;
  return cfg.mode === "full" || cfg.mode === "link-only" || cfg.mode === "rust-and-link";
}

/**
 * Download the newest `<triplet>.builtin-bytecode` published by an earlier build
 * on this branch into codegen/, verifying its SHA256 against the digest that build
 * recorded in its meta-data. Returns true when a verified blob is in place; the
 * ninja pass then .incbin's it (see emitBun in bun.ts).
 */
export async function inheritBuiltinBytecode(cfg: Config, ctx: OrderFileContext): Promise<boolean> {
  if (!builtinBytecodeEligible(cfg, ctx)) return false;
  const start = Date.now();
  const artifact = builtinBytecodeArtifact(cfg);
  const destination = builtinBytecodePath(cfg);
  ...
  for await (const build of candidateBuilds(ctx)) {
    ...
    // Pin: the publishing build recorded the blob's digest in its meta-data.
    // An artifact that does not match its own build's record is not trusted.
    const expected = spawnSync(
      "buildkite-agent",
      ["meta-data", "get", builtinBytecodeShaKey(cfg), "--build", build.id],
      {
        cwd: cfg.buildDir,
        encoding: "utf8",
        timeout: ARTIFACT_DOWNLOAD_TIMEOUT_MS,
    ...
    const expectedSha = (expected.status === 0 ? expected.stdout : "").trim();
    const actualSha = sha256File(downloaded);
    if (!expectedSha || expectedSha !== actualSha) {
      console.log(
        `  #${build.number ?? "?"}: ${artifact} sha256 ${expectedSha ? "MISMATCH" : "not recorded"} ` +
          `(expected ${expectedSha || "<none>"}, got ${actualSha}) — discarding, looking further back`,
      );
      rmSync(downloaded, { force: true });
      continue;
    }

    mkdirSync(resolve(cfg.codegenDir), { recursive: true });
    cpSync(downloaded, destination);
    rmSync(downloaded, { force: true });
    console.log(
      `+ builtin bytecode: inherited ${artifact} (${statSync(destination).size} bytes, sha256 ${actualSha.slice(0, 12)}…) ` +
        `from #${build.number ?? "?"} in ${since(start)}`,
    );
    return true;
  }

  const what =
    tried === 0 ? "found no earlier build to inherit from" : `none of the ${tried} builds tried published a valid one`;
  console.log(`~ builtin bytecode: ${what}; builtin modules will parse from source (${since(start)})`);
  return false;
}
...
export function publishBuiltinBytecode(cfg: Config, ctx: OrderFileContext, exe: string): string | undefined {
  ...
  const start = Date.now();
  const artifact = builtinBytecodeArtifact(cfg);
  const out = resolve(cfg.buildDir, artifact);
  ...
  spawnSync("buildkite-agent", ["meta-data", "set", builtinBytecodeShaKey(cfg), sha], {
    cwd: cfg.buildDir,
    stdio: "inherit",
  });
  console.log(
    `+ builtin bytecode: generated ${artifact} (${statSync(out).size} bytes, sha256 ${sha.slice(0, 12)}…) ` +
      `for the next build in ${since(start)}`,
  );
  return out;
}
