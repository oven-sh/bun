...
| `machine/components/registry.ts`                 | name → component per platform, and the derivations that walk an image's `components` list: the ordered install steps and the download bundle, from one input. Also `commandSteps()`, which runs an entry's `systemSetup`/`systemCleanup` command strings.                                                                                                                                         |
...
| **Change host setup / cleanup** (limits, sysctls, services, cache clean) | the entry's `systemSetup` / `systemCleanup` command strings — literal `sh -c` (linux, as root) or PowerShell (windows) commands run first / last around the components; they are hashed, so an edited command re-bakes |
