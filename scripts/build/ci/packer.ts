...
//   1. upload scripts/build/ci, download the pinned node
...
export type PackerTemplateInput = {
  ...
  repoRef: string;
  /** Local directory holding scripts/build/ci (uploaded to the VM). */
  bootstrapDir: string;
...
export function windowsPackerTemplate(input: PackerTemplateInput): Record<string, unknown> {
  ...
  const bootstrapCommand = [
    `& '${REMOTE_NODE_DIR}\\${nodeFolder}\\node.exe' '${REMOTE_BOOTSTRAP_DIR}\\scripts\\build\\ci\\machine\\bootstrap.ts'`,
    `--image=${image.key}`,
    "--ci",
    `--repo-ref=${repoRef}`,
  ].join(" ");
  ...
  return {
    ...
    build: {
      ...
      provisioner: hclProvisioners([
        // Step 1: upload the CI sources (spec + bootstrap modules).
        {
          type: "file",
          source: `${bootstrapDir}/`,
