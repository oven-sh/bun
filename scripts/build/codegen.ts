...
/**
 * Codegen outputs that land in `src/` instead of `codegenDir`. The rust
 * compiler refuses to import files outside its source tree, so these two
 * generated `.rust` files live in `src/jsc/bindings/` (gitignored).
 *
 * Consumers of `sources.rust` (the `src/**\/*.rust` glob) must filter these
 ...
 */
export const rustFilesGeneratedIntoSrc = [
  "src/jsc/bindings/GeneratedBindings.rust",
  "src/jsc/bindings/GeneratedJS2Native.rust",
] as const;
...
export interface CodegenOutputs {
  ...
  /** Outputs that rust `@embedFile`s or imports. */
  rustInputs: string[];
   ...
   * bake outputs into BUN_RUST_GENERATED_SOURCES, never C++ deps — same here.
   *
  ...
  /** The bindgenv2 .rust outputs (legacy artifacts, retained for reference). */
  bindgenV2Rust: string[];
...
function emitErrorCode({ n, cfg, o, dirStamp }: Ctx): void {
  ...
  const outputs = [
    ...
    resolve(cfg.codegenDir, "ErrorCode.rust"),
  ];
...
function emitGeneratedClasses({ n, cfg, sources, o, dirStamp }: Ctx): void {
  ...
  const outputs = [
    resolve(cfg.codegenDir, "RustGeneratedClasses.h"),
    resolve(cfg.codegenDir, "RustGeneratedClasses.cpp"),
    resolve(cfg.codegenDir, "RustGeneratedClasses+lazyStructureHeader.h"),
    resolve(cfg.codegenDir, "RustGeneratedClasses+DOMClientIsoSubspaces.h"),
    resolve(cfg.codegenDir, "RustGeneratedClasses+DOMIsoSubspaces.h"),
    resolve(cfg.codegenDir, "RustGeneratedClasses+lazyStructureImpl.h"),
    resolve(cfg.codegenDir, "RustGeneratedClasses.rust"),
    resolve(cfg.codegenDir, "RustGeneratedClasses.lut.txt"),
  ...
  n.build({
    outputs,
    rule: "codegen",
    inputs: [script, ...sources.rustGeneratedClasses],
    orderOnlyInputs: [dirStamp],
    vars: {
      cwd: cfg.cwd,
      desc: "RustGeneratedClasses.{rust,cpp,h}",
      args: shJoin(cfg, ["run", script, ...sources.rustGeneratedClasses, cfg.codegenDir]),
    },
  });
...
function emitHostExports({ n, cfg, sources, o, dirStamp }: Ctx): void {
  ...
  // must wait on it — `rustInputs` is the implicit-dep list both rust and the
  // cargo edge consume.
...
function emitCppBind({ n, cfg, sources, o, dirStamp }: Ctx): void {
  ...
  const output = resolve(cfg.codegenDir, "cpp.rust");
...
function emitCiInfo({ n, cfg, o, dirStamp }: Ctx): void {
  ...
  const output = resolve(cfg.codegenDir, "ci_info.rust");
...
function emitJsModules({ n, cfg, sources, o, dirStamp }: Ctx): void {
  ...
  // Written into src/ (not codegenDir) — see rustFilesGeneratedIntoSrc at top.
  const js2nativeRust = resolve(cfg.cwd, rustFilesGeneratedIntoSrc[1]);
  ...
  const outputs = [
    ...
    resolve(cfg.codegenDir, "ResolvedSourceTag.rust"),
...
function emitBindgenV2({ n, cfg, sources, o, dirStamp }: Ctx): void {
  ...
  const rustOutputs = allOutputs.filter(p => p.endsWith(".rust"));
  const other = allOutputs.filter(p => !p.endsWith(".cpp") && !p.endsWith(".rust"));
  ...
  o.bindgenV2Rust.push(...rustOutputs);
...
function emitBindgen({ n, cfg, sources, o, dirStamp }: Ctx): void {
  ...
  // Written into src/ (not codegenDir) — see rustFilesGeneratedIntoSrc at top.
  const rustOut = resolve(cfg.cwd, rustFilesGeneratedIntoSrc[0]);
  ...
  n.build({
    outputs: [cppOut, rustOut],
    rule: "codegen",
    inputs: [script, ...sources.bindgen],
    orderOnlyInputs: [dirStamp],
    vars: {
      cwd: cfg.cwd,
      desc: ".bind.ts → GeneratedBindings.{cpp,rust}",
      args: shJoin(cfg, ["run", script, debugFlag(cfg), `--codegen-root=${cfg.codegenDir}`]),
    },
  });

  o.all.push(cppOut, rustOut);
  o.cppSources.push(cppOut);
  o.rustInputs.push(rustOut);
}
...
function emitObjectLuts({ n, cfg, o, dirStamp }: Ctx): void {
  ...
  // (source, output) pairs. RustGeneratedClasses.lut.txt is special: it's
  ...
  const pairs: [src: string, out: string][] = [
    ...
    [resolve(cfg.cwd, "src/jsc/bindings/RustGlobalObject.lut.txt"), resolve(cfg.codegenDir, "RustGlobalObject.lut.h")],
    ...
    [resolve(cfg.codegenDir, "RustGeneratedClasses.lut.txt"), resolve(cfg.codegenDir, "RustGeneratedClasses.lut.h")],
