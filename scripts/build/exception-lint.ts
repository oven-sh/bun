...
function lintSource(cfg: Config): string {
  return resolve(cfg.cwd, LINT_DIR, "jsc-exception-lint.cpp");
}

function lintDataFiles(cfg: Config): { nothrow: string; summaries: string[]; baseline: string } {
  return {
    nothrow: resolve(cfg.cwd, LINT_DIR, "nothrow.txt"),
    summaries: [resolve(cfg.cwd, LINT_DIR, "summaries/webkit.tsv"), resolve(cfg.cwd, LINT_DIR, "summaries/bun.tsv")],
    baseline: resolve(cfg.cwd, LINT_DIR, "baseline.tsv"),
  };
}
...
export function emitExceptionLint(n: Ninja, cfg: Config): ExceptionLintOpts | undefined {
  ...
  for (const file of [source, data.nothrow, ...data.summaries, data.baseline]) {
