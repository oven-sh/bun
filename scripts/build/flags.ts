...
/**
 * Whether this target can embed a precompiled builtin-bytecode blob. The blob
 * is JSC bytecode for the exact source in this build; it is inherited from an
 * earlier canary (ci.ts inheritBuiltinBytecode), never generated by the current
 * link. Windows is excluded until its embedding/generation lane exists.
 */
export function builtinBytecodeEmbeddable(cfg: Pick<Config, "release" | "asan" | "valgrind" | "windows">): boolean {
  return cfg.release && !cfg.asan && !cfg.valgrind && !cfg.windows;
}

/** Downloaded/generated into the codegen dir; .incbin'd by InternalBuiltinBytecode.S. */
export function builtinBytecodePath(cfg: Pick<Config, "codegenDir">): string {
  return join(cfg.codegenDir, "InternalBuiltinBytecode.bin");
}
