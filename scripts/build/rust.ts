...
export function cargoBuildInvocation(cfg: Config): CargoInvocation {
  ...
  // workspace code relies on — e.g. `src/collections/linear_fifo.rs` does
  // not compile under NLL. It is the default on nightlies since 2026-08-06
  // (rust-lang/rust#159343), so this is a no-op on the pinned toolchain;
  // it's passed explicitly because upstream reserves the right to flip the
  // nightly default back off while bugs get fixed ahead of stabilization,
  // and a toolchain bump must not change which borrow checker builds bun.
  // Must stay in sync with the generated `.cargo/config.toml`
  // (cargo-config.ts) so plain `cargo check`, rust-analyzer, and
  // `rust:check-all` accept the same code the ninja build does.
