...
export function emitRust(n: Ninja, cfg: Config, inputs: RustBuildInputs): string[] {
  ...
  // Socket syscall fault injection: the hooks in `bun_usockets::fault` and
  // the JS control surface in socket_body.rs, gated together by one cargo
  // feature (`bun_runtime/socket_fault_injection` forwards to bun_usockets).
  if (cfg.socketFaultInjection) {
    args.push("--features", "bun_runtime/socket_fault_injection");
  }
