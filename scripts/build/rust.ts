 ...
 * `Bun__*`/`Rust*` references pull every reachable member, and the release
...
export function emitRust(n: Ninja, cfg: Config, inputs: RustBuildInputs): string[] {
  ...
  // contributes ~561 KiB of `.data.rel.ro` that the Rust binary placed in
  ...
  // Keep frame pointers — matches Rust's `omit_frame_pointer = false`
  // (build.rust:319,841) and the C++ side's `-fno-omit-frame-pointer` / `/Oy-`
  // (flags.ts:293-301). Needed so profilers and crash backtraces walk Rust
  // frames the same as the Rust binary did.
  ...
  // (#53159: 33,162 extra `.pdata` entries vs Rust main on Windows, all from
  ...
  // of reading them from `BUN_CODEGEN_DIR` at runtime. Mirrors Rust
  ...
  // ─── Windows .bin/ shim PE ───
  // Replaces Rust's `mod.addAnonymousImport("bun_shim_impl.exe", ...)` (build.rust
  // built `src/install/windows-shim/bun_shim_impl.rust` as a freestanding
  // ReleaseFast PE and wired the artifact into `@embedFile`). The Rust port
  // dropped emitRust entirely, so without this step `include_bytes!` embeds the
  ...
  if (cfg.windows) {
    ...
    // profile — a debug bun should still write release shims (matches Rust's
