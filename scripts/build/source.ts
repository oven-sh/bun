...
/**
 * Compile sources directly into our ninja graph — no cmake sub-process.
 *
...
/**
 * What a dependency provides to bun's build: libraries to link and headers
 * to include. All paths are resolved to absolute during `resolveDep`.
 */
...
export interface ResolvedDep {
   ...
   * and prebuilt deps, and by `direct` deps when `cfg.archiveDeps` is on.
   */
  libs: string[];
  /**
...
export function registerDepRules(n: Ninja, cfg: Config): void {
  // Shell quoting: tool/script paths may contain spaces (e.g. cmake
  // in "C:\Program Files\CMake\..."). quote() passes through safe paths
 ...
 * emitPrebuilt — that's the contract between cpp-only (producer) and
 * link-only (consumer). If those emit-side paths change, change this too.
 */
...
/**
 * Compile a dep's sources directly — no cmake sub-process.
 *
