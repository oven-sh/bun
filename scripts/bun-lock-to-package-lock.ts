/**
 * Writes package-lock.json from bun.lock, for the `--root-package-manager=npm`
 * build path, which installs the repo root with `npm ci`. Run it after a
 * change to bun.lock:
 *
 *   bun run lock:npm   (or: node --experimental-strip-types scripts/bun-lock-to-package-lock.ts)
 *
 * It reads only built-ins, so it runs under bun or node. The output is a
 * lockfileVersion 3 file with the same resolved versions and integrity
 * hashes as bun.lock. Tarball URLs are the default registry's, because
 * bun.lock does not store them for that registry.
 */
...
console.log(`wrote ${outPath}: ${sortedKeys.length} packages`);
