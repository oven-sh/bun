export default [
  // 0
  [0, "https://github.com/maxogden/binary-split.git"], //binary-split
  [0, "https://github.com/mafintosh/blake2b-wasm.git"], //blake2b-wasm
  [0, "https://github.com/mafintosh/flush-write-stream.git"], //flush-write-stream
  [0, "https://github.com/hughsk/from2.git"], //from2
  [0, "https://github.com/unicode-org/full-icu-test.git"], //full-icu-test
  // 5
  [0, "https://github.com/mafintosh/pumpify.git"], //pumpify
  [0, "https://github.com/ForbesLindesay/thread-sleep.git"], //thread-sleep
  [0, "https://github.com/juliangruber/isarray.git"], //isarray
];
