...
const patterns = {
   ...
   * Files the sources embed (`include_bytes!` / `include_str!`: completions,
   * `bun init` templates, the dev error page, ...) are not listed here: the
   * cargo edge tracks them through cargo's dep-info depfile
   * (`registerRustRules()` in build/rust.ts), so a new embed needs no entry.
   */
  rust: {
