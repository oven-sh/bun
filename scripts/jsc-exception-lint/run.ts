...
//   --update-summaries   recompute the Bun callee summaries and rewrite summaries/bun.tsv
//                        (and summaries/webkit.tsv when the WebKit version changed)
//   --webkit             also recompute the JavaScriptCore summaries (about 30 minutes)
...
//   --no-summaries       ignore the committed summaries (faster, more false positives)
...
// from their call site. Export passes over the JavaScriptCore sources and over
// Bun's own sources record each function's effect on the exception state, and
// the result is committed under summaries/ so that plain runs, and the compiler
// plugin the build loads, need no extra passes. Refresh with --update-summaries
// after changing how a helper handles exceptions, and with --webkit after a
// WebKit bump.
...
// The tool compares real paths (symlinks resolved) with --only-under.
...
const updateSummaries = flag("--update-summaries") || recomputeWebKit || webkitOnly;
...
function summaryHeader(project: string, extraFlag: string): string[] {
  return [
    `${project} functions whose exception behavior differs from what their signature implies:`,
    "a function that takes a JSGlobalObject* or ThrowScope& but cannot throw, one with no scope",
    "of its own that passes the caller's state through, or the reverse. Calls across translation",
    "units are classified from these rows; the README explains the convention and the columns.",
    `Generated by scripts/jsc-exception-lint/run.ts --update-summaries${extraFlag ? " " + extraFlag : ""}. Do not edit.`,
    "mangled name, qualified name/arity, kind, exit states, verifies at entry",
  ];
}
...
async function main() {
  ...
  const committedWebKit = join(toolDir, "summaries/webkit.tsv");
  const committedBun = join(toolDir, "summaries/bun.tsv");
  const wkVersion =
  ...
  if (updateSummaries) {
    ...
    // Bun summaries: one pass seeded with the committed file (so helpers in
    // other translation units keep their refined classification), then the
    // analysis pass exports the next round, which becomes the committed file.
    const bunTmp = join(outDir, "bun-summaries");
    freshDir(bunTmp);
    const seed = existsSync(committedBun) ? [`--import-summaries=${committedBun}`] : [];
    ...
    const bunRound1 = join(outDir, "bun-summaries-round1.tsv");
    mergeTsv(bunTmp, bunRound1, "bun summaries");
    imports.push(bunRound1);
  } else if (!noSummaries) {
    for (const f of [committedWebKit, committedBun]) {
      if (existsSync(f)) imports.push(f);
      else console.error(`missing ${relative(repo, f)}; run with --update-summaries`);
    }
  }

  // The export of this pass becomes the committed file, so it has to cover
  // the generated sources too; they are only summarized (not under src/).
  const exportHere = updateSummaries && !onlyFiles.length;
  ...
  if (exportArgs.length) {
    const full = join(outDir, "bun-summaries.tsv");
    mergeTsv(bunTmp2, full, "analysis");
    writeFileSync(committedBun, trimSummaries(full, summaryHeader("Bun", "")));
    console.error(`wrote ${relative(repo, committedBun)}`);
  }
  type Finding = {
    file: string;
    line: number;
