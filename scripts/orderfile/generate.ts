 ...
 * as code moves; regenerate it when the win drifts.
 *
...
const bunProfile = join(buildDir, "bun-profile");
if (!existsSync(bunProfile)) {
  console.error(`error: ${bunProfile} not found — build it first (bun run build:release)`);
  process.exit(1);
}
...
try {
  ...
  for (const trace of traces) {
    ...
    console.log(`  ${trace.name.padEnd(16)} ${visited.size} pages touched, +${order.length - before} functions`);
  }
  ...
  const header = [
    ...
    "# Generated by `bun run orderfile` — see scripts/orderfile/generate.ts.",
    `# ${order.length} functions from ${workloads.length} workloads.`,
  ];
  writeFileSync(outPath, header.join("\n") + "\n" + order.join("\n") + "\n");
  console.log(`wrote ${outPath} (${order.length} functions)`);
} finally {
