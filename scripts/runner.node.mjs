...
// Directories whose bun-test files a shard batches into `bun test --parallel`
// buckets instead of one process per file: fast (<=15s median) with zero
// flaky/failed CI annotations across recent builds. Regenerated by
// scripts/update-parallel-allowlist.mjs; a missing/unreadable file just means
// every file runs one-at-a-time as before.
...
/**
 * @param {string} testPath relative to test/
 * @returns {boolean}
 */
...
// Docker-backed tests (test/docker/prestart-map.mjs prefixes). The
// coordinator prestarts their services; the runner orders them last.
...
async function runTests() {
  ...
  // JSC exception-check / leaksan validation applies on ASAN and local runs;
  // files opted out (test/no-validate-*.txt) need their own process env, so
  // they stay out of the shared-env parallel buckets when it applies.
...
async function spawnBunTest(execPath, testPath, opts = { cwd }) {
  ...
  // ASAN builds run 5-10x slower; without a wider per-test timeout,
  // install/git tests that spawn many subprocesses time out on
