...
// A utility to spawn a process.
// - Handles timeouts, retries, and capturing stdout/stderr
// - Emits annotations to Buildkite
// - Requires no dependencies and runs in both Bun and Node.js
...
/**
 * @typedef {object} SpawnAnnotation
 ...
 * @property {SpawnAnnotation[]} [annotations]
 */
...
export async function spawn(command, options = {}) {
  ...
  const { cwd, env: envMap, stdin, timeout, retry, inherit, attempt = 0 } = options;
  const env = envMap ?? { ...process.env };

  // Unless no color is set, force output to be colored
  if (!env["NO_COLOR"]) {
    env["FORCE_COLOR"] = "1";
  ...
  // Fake a Github environment, so annotations can be parsed
  env["GITHUB_ACTIONS"] = "true";

  const stdinMode = typeof stdin === "undefined" && isMain ? "inherit" : "pipe";
  const spawnOptions = { cwd, env, timeout, stdio: [stdinMode, "pipe", "pipe"] };
  ...
  let subprocess;
  try {
    subprocess = nodeSpawn(cmd, args, spawnOptions);
  } catch (error) {
    return { exitCode: 1, error, stdout: "", stderr: inspect(error) };
  }
  ...
  await Promise.all([
    ...
    Promise.all([
      Promise.race([
        new Promise(resolve => subprocess.on("exit", resolve)),
        new Promise(resolve => subprocess.on("error", resolve)),
      ]),
      new Promise(resolve => subprocess.stdout.on("end", resolve)),
      new Promise(resolve => subprocess.stderr.on("end", resolve)),
    ]),
  ]);
