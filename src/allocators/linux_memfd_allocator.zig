...
pub const LinuxMemFdAllocator = struct {
    ...
    pub fn alloc(this: *LinuxMemFdAllocator, len: usize, offset: usize, flags: std.posix.MAP) bun.JSC.Maybe(bun.webcore.Blob.ByteStore) {
        var size = len;
        ...
        switch (bun.sys.mmap(
            ...
            .result => |slice| {
                return .{
                    .result = bun.webcore.Blob.ByteStore{
    ...
    pub fn create(bytes: []const u8) bun.JSC.Maybe(bun.webcore.Blob.ByteStore) {
        if (comptime !bun.Environment.isLinux) {
            unreachable;
        }
