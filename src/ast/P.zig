...
pub fn NewParser_(
    ...
    return struct {
        ...
        inline fn nextScopeInOrderForVisitPass(p: *P) ?ScopeOrder {
            if (p.scope_order_to_visit.len == 0) return null;
        ...
        pub fn pushScopeForVisitPass(noalias p: *P, kind: js_ast.Scope.Kind, loc: logger.Loc) anyerror!void {
            const order = p.nextScopeInOrderForVisitPass() orelse {
                // This can happen with complex expressions that weren't properly scoped during parsing
                // Just return early to prevent crashes - the scope is likely not needed
                return;
            };
