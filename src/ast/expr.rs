...
impl Expr {
    ...
    /// Convert a JSON "simple" tree (`EObjectSimple` / `EArraySimple`) into
    /// the classic `E::Object` / `E::Array` tree: one `G::Property` per row
    /// (key = an `EString` at the row's `key_loc`), `EString` nodes for
    /// string leaves, recursively for nested containers. Container `loc`s
    /// (and `is_single_line` / closing-token locations) are preserved;
    /// materialized property values and array items carry the best location
    /// the tape stores — the row's `key_loc` (objects) or the container's
    /// `loc` (arrays) — since the simple AST keeps no per-value locations.
    /// Non-container expressions are returned unchanged.
    ///
    /// Cost: allocates a Store node per key/string/number/bool/null and a
    /// 112-byte `G::Property` per row — everything the simple AST avoids.
    /// Only for boundary consumers that splice parsed JSON into a JS AST or
    /// mutate it and print it back with formatting; everything that just
    /// *reads* JSON should use `ObjectSimple::properties()` /
    /// `ArraySimple::items()` (or the generic `Expr` accessors) directly.
    ...
    fn materialize_object(o: &E::ObjectSimple) -> E::Object {
        ...
        for row in rows {
            ...
            properties.push(G::Property {
                key: Some(key),
                value: Some(Self::materialize_json_value(&row.value, row.key_loc)),
    ...
    fn materialize_array(a: &E::ArraySimple, loc: Loc) -> E::Array {
        let rows = a.items();
        let mut items: crate::ExprNodeList = Vec::with_capacity_in(rows.len(), bun_alloc::AstAlloc);
        for item in rows {
            items.push(Self::materialize_json_value(item, loc));
        }
        E::Array {
            items,
    ...
    /// [`Expr::from_json_value`], except nested containers are deep-converted
    /// to `E::Object` / `E::Array` instead of staying simple.
    fn materialize_json_value(value: &E::JsonValue, loc: Loc) -> Expr {
        match value {
            E::JsonValue::Object(o) => Expr::init(Self::materialize_object(o.get()), loc),
            E::JsonValue::Array(a) => Expr::init(Self::materialize_array(a.get(), loc), loc),
