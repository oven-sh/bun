...
impl Scope {
    /// All-empty `Scope`. Used with struct-update syntax in the parser's
    /// per-scope allocation hot path (`push_scope_for_parse_pass` runs once
    /// per `{}` / function / class body).
    pub fn empty(alloc: AstAlloc) -> Self {
        Self {
            ...
            parent: None,
            children: alloc.vec(),
            members: MemberHashMap::new_in(alloc),
            generated: alloc.vec(),
            label_ref: Ref::NONE,
