...
impl Stmt {
    /// Reset the thread-local `Data::Store`. Associated wrapper so downstream
    /// crates can call `crate::Stmt::data_store_reset()` without naming the
    ...
    /// Create the thread-local `Data::Store`.
    #[inline]
    pub fn data_store_create() {
        data::Store::create();
    }

    /// Debug-only re-entrancy guard for the thread-local `Data::Store`.
    #[inline]
...
impl Default for Stmt {
    /// Empty statement — used to zero-init `loop_body` and bulk-fill stmt
    /// slices before population.
    #[inline]
    fn default() -> Self {
        Stmt {
            data: Data::SEmpty(NONE),
...
// PORT NOTE: plain mutable global counter, never read. Debug-only here so
// release doesn't pay a contended `lock xadd` per Stmt across the bundler
// worker pool.
#[cfg(debug_assertions)]
pub static ICOUNT: AtomicUsize = AtomicUsize::new(0);

/// Type-level dispatch for `init` / `alloc` / `allocate`. Each `S::*` payload
/// type implements this to map itself onto the corresponding `Data` variant.
///
/// The dispatch is the trait impl and the arm list is the
/// `impl_statement_data!` invocation below.
pub trait StatementData: Sized {
    /// Wrap an already-allocated payload (`Stmt.init` / `comptime_init`).
    fn wrap_ref(ptr: StoreRef<Self>) -> Data;
    /// Store-append `self` and wrap (`Stmt.alloc` / `comptime_alloc`).
    fn store_alloc(self) -> Data;
    /// Arena-allocate `self` and wrap (`Stmt.allocate` / `allocateData`).
...
impl Stmt {
    ...
    // `comptime_alloc` — folded into `StatementData::store_alloc`; kept as a
    ...
    // `allocateData` — folded into `StatementData::arena_alloc`.
    ...
    // `comptime_init` — in Rust the variant constructor IS the union-init;
    // this helper collapses to identity and is absorbed by
    // `StatementData::wrap_ref`.
...
// ── Layout guards ─────────────────────────────────────────────────────────
// `Stmt` must stay ≤ 24 bytes. Every payload
...
// visitStmt and the printer do unchecked union field reads. Rust callers
// `.unwrap()` (or pattern-match) — the `Option` is the cheapest sound encoding
// of UB-on-mismatch.
...
// `init/append/reset/destroy`. Type list is the set of payloads `Data` boxes.
crate::new_store!(
    stmt_store,
    [
        S::Block,
...
// There is no `StoredData(tag)` type-returning function. Rust has no
// type-returning fns.
