...
/// `BundleOptions.target` defaults to `.browser`; keep `Default` so resolver
/// can field-default it.
#[repr(u8)]
...
impl Target {
    ...
    pub fn default_conditions(self) -> &'static [&'static [u8]] {
        // PORT NOTE: callers (`ESMConditions::init`) take byte slices, so
        // surface bytes directly rather than `&str`.
        match self {
            Target::Node => &[b"node"],
