...
//! edge. The parser writes `input_hash` / `features_hash` /
//! `exports_kind` and calls `get()`; the on-disk encode/decode
//! (`Metadata` / `Entry`) lives here too.
...
/// The seed `hash_for_runtime_transpiler` callers must use so their
/// `features_hash` matches the one `to_file` / `from_file` persist.
pub const FEATURES_HASH_SEED: u64 = SEED;
...
impl RuntimeTranspilerCache {
    ...
    pub fn get(&mut self, source: &Source, features_hash: impl FnOnce() -> u64) -> bool {
