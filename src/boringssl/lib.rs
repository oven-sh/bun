...
// the translated file can be regenerated from a C-header bindings generator.
...
// MOVE_DOWN: `isSafeAltName`. Lives here so `boringssl` does not depend on
// `bun_runtime` (tier-6).
pub mod x509 {
...
std::thread_local! {
    // One pool per thread, lazily allocated on first `SSL_CTX.setup()` call
    // from that thread.
    static AUTO_CRYPTO_BUFFER_POOL: Cell<*mut CRYPTO_BUFFER_POOL> =
        const { Cell::new(ptr::null_mut()) };
}

/// `SSL_CTX.setup(ctx)` — install the per-thread `CRYPTO_BUFFER_POOL` and
/// set the cipher list to BoringSSL's `SSL_DEFAULT_CIPHER_LIST` (`"ALL"`).
///
/// # Safety
...
pub fn init_client() -> *mut boring::SSL {
    ...
    unsafe {
        ...
        // `SSL.init(ctx)` = `SSL_new(ctx)`
        let ssl = boring::SSL_new(ctx);
        // `setIsClient(true)` = `SSL_set_connect_state(ssl)`
...
// NOTE: `ERR_toJS` is intentionally absent here — the JS conversion lives in
// the `bun_runtime`/`*_jsc` crate as an extension method.
