...
impl<'a> BrotliReaderArrayList<'a> {
    ...
    pub fn init_with_options(
    ...
    ) -> crate::brotli::Result<Self> {
        if !BrotliDecoder::initialize_brotli() {
            return Err(crate::brotli::Error::BrotliFailedToLoad);
        }
        ...
        .ok_or(crate::brotli::Error::BrotliFailedToCreateInstance)?;
    ...
    pub fn read_all(&mut self, is_done: bool) -> crate::brotli::Result<()> {
        ...
        while self.state == ReaderState::Uninitialized || self.state == ReaderState::Inflating {
            ...
            if self.list_ptr.len() > self.max_output_size {
                ...
                return Err(crate::brotli::Error::BrotliDecompressionError);
            }
            ...
            match result {
                ...
                c::BrotliDecoderResult::needs_more_input => {
                    ...
                    return Err(crate::brotli::Error::ShortRead);
                }
...
impl StreamingDecoder {
    pub fn new(options: &DecoderOptions) -> crate::brotli::Result<Self> {
        if !BrotliDecoder::initialize_brotli() {
            return Err(crate::brotli::Error::BrotliFailedToLoad);
        }
        // SAFETY: brotli FFI constructor; alloc/free are valid extern "C"
...
impl BrotliCompressionStream {
    ...
    pub fn write_chunk(&mut self, input: &[u8], last: bool) -> crate::brotli::Result<&[u8]> {
        ...
        if !result.success {
            ...
            return Err(crate::brotli::Error::BrotliCompressionError);
        }

        Ok(result.output)
    }

    pub fn write(&mut self, input: &[u8], last: bool) -> crate::brotli::Result<&[u8]> {
    ...
    pub fn end(&mut self) -> crate::brotli::Result<&[u8]> {
    ...
    pub fn writer_context<W: bun_core::io::Write>(&mut self, writable: W) -> BrotliWriter<'_, W> {
        BrotliWriter::init(self, writable)
    }

    // The returned `BrotliWriter` implements `bun_core::io::Write` itself, so this
    // is just an alias for `writer_context()`.
    pub fn writer<W: bun_core::io::Write>(&mut self, writable: W) -> BrotliWriter<'_, W> {
...
impl<'a, W: bun_core::io::Write> BrotliWriter<'a, W> {
    ...
    pub fn write(&mut self, to_compress: &[u8]) -> crate::brotli::Result<usize> {
        let decompressed = self.compressor.write(to_compress, false)?;
        self.input_writer.write_all(decompressed)?;
        Ok(to_compress.len())
    }

    pub fn end(&mut self) -> crate::brotli::Result<()> {
...
impl<W: bun_core::io::Write> bun_core::io::Write for BrotliWriter<'_, W> {
    fn write_all(&mut self, buf: &[u8]) -> bun_core::io::Result<()> {
        self.write(buf)
            .map(|_| ())
            .map_err(|_| bun_core::Error::WriteFailed)
    }

    fn flush(&mut self) -> bun_core::io::Result<()> {
