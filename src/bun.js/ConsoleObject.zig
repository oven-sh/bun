...
fn messageWithTypeAndLevel_(
    ...
    if (level == .Warning or level == .Error or level == .Trace or message_type == .Assert) {
    ...
    const enable_colors = if (level == .Warning or level == .Error or level == .Trace)
        Output.enable_ansi_colors_stderr
    else
        Output.enable_ansi_colors_stdout;

    const writer = if (level == .Warning or level == .Error or level == .Trace)
