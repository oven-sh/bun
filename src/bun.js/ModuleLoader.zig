        ...
        .html => {
            ...
            const value = brk: {
                if (!jsc_vm.origin.isEmpty()) {
                    var buf = MutableString.init2048(jsc_vm.allocator) catch |oe| bun.outOfMemory(oe);
                    defer buf.deinit();
...
fn dumpSourceStringFailiable(vm: *VirtualMachine, specifier: string, written: []const u8) !void {
    ...
    if (std.fs.path.dirname(specifier)) |dir_path| {
        ...
        if (vm.source_mappings.get(specifier)) |mappings| {
            ...
            const map_path = std.mem.concat(bun.default_allocator, u8, &.{ std.fs.path.basename(specifier), ".map" }) catch |oe| bun.outOfMemory(oe);
    ...
    pub const TranspilerJob = struct {
        ...
        pub fn run(this: *TranspilerJob) void {
            ...
            if (ast_memory_store == null) {
                ast_memory_store = bun.default_allocator.create(js_ast.ASTMemoryAllocator) catch |oe| bun.outOfMemory(oe);
            ...
            defer {
                this.log = logger.Log.init(bun.default_allocator);
                log.cloneToWithRecycled(&this.log, true) catch |oe| bun.outOfMemory(oe);
            }
            var vm = this.vm;
