...
pub const JSBundler = struct {
    ...
    pub const Config = struct {
        ...
        hot: bool = false,
        watch: bool = false,
        /// @deprecated HMR is automatic with watch: true. Parsed but ignored.
        hmr: bool = false,
        ...
        pub fn fromJS(globalThis: *jsc.JSGlobalObject, config: jsc.JSValue, plugins: *?*Plugin, allocator: std.mem.Allocator) JSError!Config {
            ...
            if (try config.getBooleanLoose(globalThis, "watch")) |watch| {
                this.watch = watch;
            }

            if (try config.getBooleanLoose(globalThis, "hmr")) |hmr_val| {
                this.hmr = hmr_val;
            }
    ...
    fn build(
        ...
        var config = try Config.fromJS(globalThis, arguments[0], &plugins, bun.default_allocator);

        if (config.watch) {
            return buildWatch(globalThis, &config, plugins, arguments[0]);
        }
    ...
    /// Handle `Bun.build({ watch: true })` - returns a Promise that resolves
    /// to a WatchBuildResult object with .outputs, .on("rebuild", cb), .stop()
    fn buildWatch(
        globalThis: *jsc.JSGlobalObject,
        config: *Config,
        plugins: ?*Plugin,
        config_js: jsc.JSValue,
    ) bun.JSError!jsc.JSValue {
        const vm = globalThis.bunVM();

        // Create the WatchBuildResult object
        const watch_result = bun.default_allocator.create(WatchBuildResult) catch return globalThis.throwOutOfMemory();
        watch_result.* = .{
            .config = config.*,
            .plugins = plugins,
        ...
        // Store the JS config for later rebuilds
        watch_result.config_js.set(globalThis, config_js);

        // Keep the event loop alive
        watch_result.poll_ref.ref(vm);

        // Always use DevServer for incremental builds + HMR
        return buildWatchWithDevServer(globalThis, watch_result, config);
    }

    /// HMR mode: create a standalone DevServer for incremental builds
    fn buildWatchWithDevServer(
        globalThis: *jsc.JSGlobalObject,
        watch_result: *WatchBuildResult,
        config: *Config,
    ) bun.JSError!jsc.JSValue {
        const vm = globalThis.bunVM();
        const root: [:0]const u8 = root: {
            if (config.dir.list.items.len > 0) {
                break :root bun.default_allocator.dupeZ(u8, config.dir.slice()) catch
                    return globalThis.throwOutOfMemory();
            }
            break :root vm.transpiler.fs.top_level_dir;
        };

        const entry_points = config.entry_points.keys();
        // Dupe the entry point strings so DevServer owns them
        const duped_eps = bun.default_allocator.alloc([]const u8, entry_points.len) catch
            return globalThis.throwOutOfMemory();
        for (entry_points, 0..) |ep, i| {
            duped_eps[i] = bun.default_allocator.dupe(u8, ep) catch
                return globalThis.throwOutOfMemory();
        }

        const dev = bun.bake.DevServer.initStandalone(.{
            .root = root,
            .vm = vm,
            .entry_points = duped_eps,
        ...
        watch_result.dev_server = dev;

        // Set up the standalone callback: DevServer calls this after each build
        dev.standalone_callback_ctxs.append(bun.default_allocator, @ptrCast(watch_result)) catch bun.outOfMemory();
        dev.standalone_callback_fn = &onStandaloneBuildComplete;

        // Create outer promise
        const outer_promise = jsc.JSPromise.Strong.init(globalThis);
        watch_result.outer_promise = outer_promise;

        // Trigger the initial build
        dev.startStandaloneBuild() catch {
            watch_result.deinit();
            return globalThis.throwOutOfMemory();
        };

        return outer_promise.value();
    }

    /// Called by DevServer after each standalone build completes.
    fn onStandaloneBuildComplete(ctx: *anyopaque, dev: *bun.bake.DevServer, success: bool) void {
        const watch_result: *WatchBuildResult = @ptrCast(@alignCast(ctx));
        if (!watch_result.active.load(.acquire)) return;

        watch_result.last_success = success;
        const globalThis = watch_result.globalThis;

        // Build outputs array from the client graph's bundled files
        const outputs_array = blk: {
            const keys = dev.client_graph.bundled_files.keys();
            const arr = jsc.JSValue.createEmptyArray(globalThis, keys.len) catch break :blk jsc.JSValue.jsNull();
            for (keys, 0..) |path, i| {
                var str = bun.String.cloneUTF8(path);
                const js_str = str.toJS(globalThis) catch break :blk jsc.JSValue.jsNull();
                arr.putIndex(globalThis, @intCast(i), js_str) catch break :blk jsc.JSValue.jsNull();
            }
            break :blk arr;
        };
        watch_result.current_outputs.set(globalThis, outputs_array);

        // Build logs array from DevServer's log
        const logs_array = blk: {
            const log_msgs = dev.log.msgs.items;
            const arr = jsc.JSValue.createEmptyArray(globalThis, log_msgs.len) catch break :blk jsc.JSValue.jsNull();
            for (log_msgs, 0..) |*msg, i| {
                var msg_str = bun.String.cloneUTF8(msg.data.text);
                const js_str = msg_str.toJS(globalThis) catch break :blk jsc.JSValue.jsNull();
                arr.putIndex(globalThis, @intCast(i), js_str) catch break :blk jsc.JSValue.jsNull();
            }
            break :blk arr;
        };
        watch_result.current_logs.set(globalThis, logs_array);

        // If outer promise hasn't been resolved yet, this is the initial build
        if (watch_result.outer_promise.hasValue()) {
            const watch_js = WatchBuildResult.js.toJS(watch_result, globalThis);
            watch_result.outer_promise.resolve(globalThis, watch_js) catch {};
            return;
        }

        // Otherwise, fire the rebuild callback
        if (watch_result.on_rebuild_callback.get()) |callback| {
            // Create a result object matching the BuildOutput shape
            const result = jsc.JSValue.createEmptyObject(globalThis, 3);
            result.put(globalThis, "success", if (success) jsc.JSValue.true else jsc.JSValue.false);
            result.put(globalThis, "outputs", outputs_array);
            result.put(globalThis, "logs", logs_array);

            _ = callback.call(globalThis, .js_undefined, &.{result}) catch |e| {
...
pub const WatchBuildResult = struct {
    pub const js = jsc.Codegen.JSWatchBuildResult;
    pub const toJS = js.toJS;
    pub const fromJS = js.fromJS;
    pub const fromJSDirect = js.fromJSDirect;

    /// The original config (kept alive, not consumed)
    config: JSBundler.Config,
    /// Plugins from the original build
    plugins: ?*JSBundler.Plugin,
    /// Reference to the JS event loop for scheduling callbacks
    globalThis: *jsc.JSGlobalObject,
    /// Event loop reference (safe to access from watcher thread)
    event_loop: *jsc.EventLoop,
    /// The original JS config object, for re-parsing on rebuilds
    config_js: jsc.Strong.Optional = .empty,
    /// Promise for the initial build
    outer_promise: jsc.JSPromise.Strong = .{},
    /// JS callback for "rebuild" events
    on_rebuild_callback: jsc.Strong.Optional = .empty,
    /// Current build outputs as a JS array
    current_outputs: jsc.Strong.Optional = .empty,
    /// Current build logs as a JS array
    current_logs: jsc.Strong.Optional = .empty,
    /// Whether the last build was successful
    last_success: bool = false,
    /// The file watcher
    watcher: ?*bun.Watcher = null,
    /// Whether the watcher is still active
    active: std.atomic.Value(bool) = std.atomic.Value(bool).init(true),
    /// Keep the event loop alive while watching
    poll_ref: bun.Async.KeepAlive = .{},
    /// Debounce: only queue one rebuild at a time
    rebuild_pending: std.atomic.Value(bool) = std.atomic.Value(bool).init(false),
    /// DevServer handles incremental builds + HMR for watch mode
    dev_server: ?*bun.bake.DevServer = null,

    pub fn hasPendingActivity(this: *WatchBuildResult) callconv(.c) bool {
        return this.active.load(.acquire);
    }

    pub fn finalize(this: *WatchBuildResult) callconv(.c) void {
        this.deinit();
    }

    fn deinit(this: *WatchBuildResult) void {
        ...
        if (this.watcher) |w| {
            // Signal the watcher thread to stop. The watcher thread will
            // destroy its own Watcher struct when it exits. We can't
            // safely destroy WatchBuildResult here because the watcher
            // thread may still be in onFileUpdate. Instead we rely on
            // the active flag to make onFileUpdate a no-op, and accept
            // a small leak of the WatchBuildResult struct.
            w.deinit(false);
            this.watcher = null;
        }

        this.on_rebuild_callback.deinit();
        this.current_outputs.deinit();
        this.current_logs.deinit();
        this.config_js.deinit();
        this.outer_promise.deinit();

        // Note: We don't destroy(this) here because the watcher thread
        // may still reference this struct via onFileUpdate. The JS wrapper
        // object's GC handles the outer JS allocation.
    }

    pub fn stop(
        this: *WatchBuildResult,
    ...
    pub fn on(
        ...
        if (args.len < 2) {
            return globalObject.throwInvalidArguments("Expected 2 arguments: event name and callback", .{});
        }
        ...
        if (!event_name.isString()) {
            return globalObject.throwInvalidArguments("First argument must be a string", .{});
        }
        if (!callback.isCallable()) {
            return globalObject.throwInvalidArguments("Second argument must be a function", .{});
        }

        const name_str = try event_name.toBunString(globalObject);
        defer name_str.deref();
        const name_utf8 = name_str.toUTF8(bun.default_allocator);
        defer name_utf8.deinit();

        if (bun.strings.eqlComptime(name_utf8.slice(), "rebuild")) {
            this.on_rebuild_callback.set(globalObject, callback);
        } else {
            return globalObject.throwInvalidArguments("Unknown event: expected 'rebuild'", .{});
        }
    ...
    pub fn getOutputs(
        ...
        if (this.current_outputs.get()) |v| return v;
        return jsc.JSValue.jsNull();
    }
    ...
    pub fn getSuccess(
        ...
        return if (this.last_success) .true else .false;
    }
    ...
    pub fn getLogs(
        ...
        if (this.current_logs.get()) |v| return v;
        return jsc.JSValue.jsNull();
    }
    ...
    pub fn getUrl(
        ...
        const dev = this.dev_server orelse return .null;
        const port = dev.getStandaloneHmrPort() orelse return .null;

        // Build URL: http://localhost:PORT/_bun/client/ENTRY_BASENAME
        const entry_name: []const u8 = if (dev.standalone_entry_points.len > 0) blk: {
            const ep = dev.standalone_entry_points[0];
            break :blk bun.path.basename(ep);
        } else "bundle.js";

        var buf: [512]u8 = undefined;
        const url = std.fmt.bufPrint(&buf, "http://localhost:{d}/_bun/client/{s}", .{ port, entry_name }) catch return .null;
        return try bun.String.createUTF8ForJS(globalThis, url);
    }
    ...
    pub fn doImport(
        ...
        return globalThis.throwTODO("WatchBuildResult.import() is not yet implemented.");
    }

    /// Called when the initial build Promise resolves (JSHostFnZig callback)
    pub fn onInitialBuildResolved(_: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        const arguments = callframe.arguments_old(2);
        const result = arguments.ptr[0];
        const ctx = arguments.ptr[arguments.len - 1].asPromisePtr(WatchBuildResult);

        // Store the build results
        if (result.isObject()) {
            if (try result.getPropertyValue(ctx.globalThis, "outputs")) |outputs| {
                ctx.current_outputs.set(ctx.globalThis, outputs);
            }
            if (try result.getPropertyValue(ctx.globalThis, "logs")) |logs| {
                ctx.current_logs.set(ctx.globalThis, logs);
            }
            if (try result.getPropertyValue(ctx.globalThis, "success")) |success| {
                ctx.last_success = success.toBoolean();
            }
        }

        // Start the file watcher
        ctx.startWatcher() catch {
            // If watcher fails, still resolve with the build result
        };

        // Resolve the outer promise with the WatchBuildResult
        const watch_js = js.toJS(ctx, ctx.globalThis);
        ctx.outer_promise.resolve(ctx.globalThis, watch_js) catch {};

        return .js_undefined;
    }

    /// Called when the initial build Promise rejects (JSHostFnZig callback)
    pub fn onInitialBuildRejected(_: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        const arguments = callframe.arguments_old(2);
        ...
        ctx.outer_promise.reject(ctx.globalThis, err) catch {};

        // Clean up since we failed
        ctx.active.store(false, .release);
        ctx.poll_ref.unref(ctx.globalThis.bunVM());

        return .js_undefined;
    }

    fn startWatcher(this: *WatchBuildResult) !void {
        // The watcher was created before the initial build and BundleV2
        // already registered all resolved source files during bundling.
        // We just need to start the watcher thread.
        if (this.watcher) |watcher| {
            try watcher.start();
        }
    }

    /// Called from the watcher thread when files change
    pub fn onFileUpdate(ctx: *WatchBuildResult, _: []bun.Watcher.WatchEvent, _: []?[:0]u8, _: bun.Watcher.WatchList) void {
        if (!ctx.active.load(.acquire)) return;

        // Debounce: only queue one rebuild at a time
        if (ctx.rebuild_pending.cmpxchgStrong(false, true, .acq_rel, .acquire) == null) {
            // Successfully set pending - schedule rebuild on JS event loop
            // Use stored event_loop reference since this runs on the watcher thread
            ctx.event_loop.enqueueTaskConcurrent(
                jsc.ConcurrentTask.fromCallback(ctx, doRebuild),
            );
        }
    }

    pub fn onWatchError(_: *WatchBuildResult, _: bun.sys.Error) void {
        // Ignore watch errors for now
    }

    /// Runs on the JS event loop thread - triggers a rebuild
    fn doRebuild(this: *WatchBuildResult) void {
        if (!this.active.load(.acquire)) return;
        this.rebuild_pending.store(false, .release);

        // Re-parse config from the stored JS config object for each rebuild
        const config_value = this.config_js.get() orelse return;
        var rebuild_plugins: ?*JSBundler.Plugin = null;
        const rebuild_config = JSBundler.Config.fromJS(
            this.globalThis,
            config_value,
            &rebuild_plugins,
            bun.default_allocator,
        ...
        // Use createAndScheduleCompletionTask so we can pass the watcher
        // for newly discovered files during rebuild
        const completion = bun.BundleV2.createAndScheduleCompletionTask(
            rebuild_config,
            rebuild_plugins,
            this.globalThis,
        ...
        completion.bun_watcher = this.watcher;
        completion.promise = jsc.JSPromise.Strong.init(this.globalThis);
        const result = completion.promise.value();

        // The result is a Promise JSValue - attach .then() to call our rebuild callback
        if (result != .zero) {
            result.then(
                this.globalThis,
                this,
    ...
    /// Called when a rebuild Promise resolves (JSHostFnZig callback)
    fn onRebuildResolved(_: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        const arguments = callframe.arguments_old(2);
        const result = arguments.ptr[0];
        const ctx = arguments.ptr[arguments.len - 1].asPromisePtr(WatchBuildResult);

        if (!ctx.active.load(.acquire)) return .js_undefined;
        ...
        // Call the user's rebuild callback
        if (ctx.on_rebuild_callback.get()) |callback| {
            _ = callback.call(ctx.globalThis, .js_undefined, &.{result}) catch |e| {
                ctx.globalThis.reportActiveExceptionAsUnhandled(e);
            };
        }
    ...
    /// Called when a rebuild Promise rejects (JSHostFnZig callback)
    fn onRebuildRejected(_: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        const arguments = callframe.arguments_old(2);
        const err = arguments.ptr[0];
        const ctx = arguments.ptr[arguments.len - 1].asPromisePtr(WatchBuildResult);
        ...
        // Call the user's rebuild callback with an error result
        if (ctx.on_rebuild_callback.get()) |callback| {
            const error_result = jsc.JSValue.createEmptyObject(ctx.globalThis, 3);
            error_result.put(ctx.globalThis, "success", .false);
            const empty_array = jsc.JSValue.createEmptyArray(ctx.globalThis, 0) catch return .js_undefined;
            error_result.put(ctx.globalThis, "outputs", empty_array);
            error_result.put(ctx.globalThis, "error", err);

            _ = callback.call(ctx.globalThis, .js_undefined, &.{error_result}) catch |e| {
                ctx.globalThis.reportActiveExceptionAsUnhandled(e);
            };
        }
    ...
    comptime {
        if (bun.Environment.export_cpp_apis) {
            @export(&jsc.toJSHostFn(onInitialBuildResolved), .{ .name = "Bun__WatchBuildResult__onInitialBuildResolved" });
            @export(&jsc.toJSHostFn(onInitialBuildRejected), .{ .name = "Bun__WatchBuildResult__onInitialBuildRejected" });
            @export(&jsc.toJSHostFn(onRebuildResolved), .{ .name = "Bun__WatchBuildResult__onRebuildResolved" });
            @export(&jsc.toJSHostFn(onRebuildRejected), .{ .name = "Bun__WatchBuildResult__onRebuildRejected" });
        }
    }
};
