...
pub fn parse(
    ...
    const source = &logger.Source.initPathString("input.jsonc", input.slice());

    const root = json.parseTSConfig(source, &log, arena.allocator(), true) catch |err| return switch (err) {
        error.OutOfMemory => |oom| oom,
        error.StackOverflow => global.throwStackOverflow(),
        else => global.throwValue(try log.toJS(global, bun.default_allocator, "Failed to parse JSONC")),
    };
...
const ParserCtx = struct {
    ...
    pub fn toJS(ctx: *ParserCtx, args: *MarkedArgumentBuffer, expr: Expr) JSError!JSValue {
        ...
        switch (expr.data) {
            ...
            // unreachable. the jsonc AST does not use any other
            // expr types
