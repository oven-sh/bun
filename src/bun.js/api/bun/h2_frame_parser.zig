...
pub const H2FrameParser = struct {
    ...
    const Stream = struct {
        ...
        const PendingQueue = struct {
            ...
            pub fn enqueue(self: *PendingQueue, value: PendingFrame, allocator: Allocator) void {
                self.data.append(allocator, value) catch |oe| bun.outOfMemory(oe);
                self.len += 1;
        ...
        pub fn queueFrame(this: *Stream, client: *H2FrameParser, bytes: []const u8, callback: jsc.JSValue, end_stream: bool) void {
            ...
            if (this.dataFrameQueue.peekLast()) |last_frame| {
                ...
                if (last_frame.len == 0) {
                    ...
                    last_frame.buffer = client.allocator.alloc(u8, MAX_PAYLOAD_SIZE_WITHOUT_FRAME) catch |oe| bun.outOfMemory(oe);
                }
            ...
            const frame: PendingFrame = .{
                ...
                .buffer = if (bytes.len == 0) "" else client.allocator.alloc(u8, MAX_PAYLOAD_SIZE_WITHOUT_FRAME) catch |oe| bun.outOfMemory(oe),
        ...
        if (buffer.len > 0) {
            {
                ...
                if (written < buffer.len) {
                    ...
                    _ = this.writeBuffer.write(this.allocator, bytes) catch |oe| bun.outOfMemory(oe);
            ...
            this.writeBuffer.len = 0;
            {
                ...
                if (written < bytes.len) {
                    ...
                    _ = this.writeBuffer.write(this.allocator, pending) catch |oe| bun.outOfMemory(oe);
    ...
    pub fn handleIncommingPayload(this: *H2FrameParser, data: []const u8, streamIdentifier: u32) ?Payload {
        ...
        if (this.remainingLength > 0) {
            // buffer more data
            _ = this.readBuffer.appendSlice(payload) catch |oe| bun.outOfMemory(oe);
    ...
    fn handleReceivedStreamID(this: *H2FrameParser, streamIdentifier: u32) ?*Stream {
        ...
        // new stream open
        const entry = this.streams.getOrPut(streamIdentifier) catch |oe| bun.outOfMemory(oe);
    ...
    fn readBytes(this: *H2FrameParser, bytes: []const u8) bun.JSError!usize {
        ...
        if (buffered_data > 0) {
            ...
            if (total < FrameHeader.byteSize) {
                // buffer more data
                _ = this.readBuffer.appendSlice(bytes) catch |oe| bun.outOfMemory(oe);
                this.globalThis.vm().reportExtraMemory(bytes.len);
        ...
        if (bytes.len < FrameHeader.byteSize) {
            ...
            this.readBuffer.appendSlice(bytes) catch |oe| bun.outOfMemory(oe);
            this.globalThis.vm().reportExtraMemory(bytes.len);
        ...
        var this = brk: {
            if (ENABLE_ALLOCATOR_POOL) {
                if (H2FrameParser.pool == null) {
                    H2FrameParser.pool = bun.default_allocator.create(H2FrameParser.H2FrameParserHiveAllocator) catch |oe| bun.outOfMemory(oe);
                    H2FrameParser.pool.?.* = H2FrameParser.H2FrameParserHiveAllocator.init(bun.default_allocator);
                }
                const self = H2FrameParser.pool.?.tryGet() catch |oe| bun.outOfMemory(oe);
