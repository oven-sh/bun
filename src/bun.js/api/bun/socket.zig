...
fn x509GetNameObject(globalObject: *JSC.JSGlobalObject, name: ?*BoringSSL.X509_NAME) JSValue {
    const cnt = BoringSSL.X509_NAME_entry_count(name);
    if (cnt <= 0) {
        return JSValue.jsUndefined();
    }
    var result = JSC.JSValue.createEmptyObject(globalObject, 1);
    ...
    for (0..cnt) |i| {
        ...
        if (result.getTruthy(globalObject, name_slice)) |value| {
            if (value.jsType().isArray()) {
                ...
                const name = JSC.ZigString.fromUTF8(name_slice);
                ...
                result.put(globalObject, &name, array);
            }
        } else {
            const name = JSC.ZigString.fromUTF8(name_slice);
            result.put(globalObject, &name, JSC.ZigString.fromUTF8(value_slice).toValueGC(globalObject));
        }
    }
    return result;
}
...
fn x509SubjectAltNamePrint(out: *BoringSSL.BIO, ext: *BoringSSL.X509_EXTENSION) bool {
    ...
    if (BoringSSL.X509V3_EXT_d2i(ext)) |names_| {
        const names: *struct_stack_st_GENERAL_NAME = bun.cast(*struct_stack_st_GENERAL_NAME, names_);
...
fn x509GetSubjectAltNameString(bio: *BoringSSL.BIO, cert: *BoringSSL.X509) JSValue {
    const index = BoringSSL.X509_get_ext_by_NID(cert, BoringSSL.NID_subject_alt_name, -1);
    if (index < 0)
        return JSValue.jsUndefined();

    const ext = X509_get_ext(cert, index) orelse return JSValue.jsUndefined();
...
fn x509ToObject(globalObject: *JSC.JSGlobalObject, cert: *BoringSSL.X509) JSValue {
...
fn NewSocket(comptime ssl: bool) type {
    return struct {
        ...
        pub fn getCertificate(
            ...
            if (cert) |x509| {
                return x509ToObject(globalObject, x509);
            }
            return JSValue.jsUndefined();
        }
