    ...
    return struct {
                ...
                if (this.socket.ssl()) |ssl_ptr| {
                    if (!ssl_ptr.isInitFinished()) {
                        if (this.server_name) |server_name| {
                            ...
                            if (host.len > 0) {
                                const host__ = default_allocator.dupeZ(u8, host) catch |oe| bun.outOfMemory(oe);
        ...
        fn writeOrEndBuffered(this: *This, globalObject: *jsc.JSGlobalObject, data_value: jsc.JSValue, encoding_value: jsc.JSValue, comptime is_end: bool) WriteResult {
            ...
            const wrote: i32 = brk: {
                if (comptime !ssl and Environment.isPosix) {
                    ...
                    if (this.socket.socket == .connected and buffer.slice().len > 0) {
                        ...
                        if (remaining_in_input_data.len > 0) {
                            this.buffered_data_for_node_net.append(bun.default_allocator, remaining_in_input_data) catch |oe| bun.outOfMemory(oe);
                        }
                ...
                this.buffered_data_for_node_net.append(bun.default_allocator, buffer.slice()) catch |oe| bun.outOfMemory(oe);
        ...
        fn writeOrEnd(this: *This, globalObject: *jsc.JSGlobalObject, args: []jsc.JSValue, buffer_unwritten_data: bool, comptime is_end: bool) WriteResult {
            ...
            if (buffer_unwritten_data) {
                ...
                if (remaining.len > 0) {
                    this.buffered_data_for_node_net.append(bun.default_allocator, remaining) catch |oe| bun.outOfMemory(oe);
                }
            }
        ...
        pub fn upgradeTLS(this: *This, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
            ...
            var handlers_ptr = bun.default_allocator.create(Handlers) catch |oe| bun.outOfMemory(oe);
            ...
            var tls = bun.new(TLSSocket, .{
                ...
                .wrapped = .tls,
                .protos = if (protos) |p| (bun.default_allocator.dupe(u8, p[0..protos_len]) catch |oe| bun.outOfMemory(oe)) else null,
                .server_name = if (socket_config.server_name) |server_name| (bun.default_allocator.dupe(u8, server_name[0..bun.len(server_name)]) catch |oe| bun.outOfMemory(oe)) else null,
            ...
            var raw_handlers_ptr = bun.default_allocator.create(Handlers) catch |oe| bun.outOfMemory(oe);
...
pub fn jsUpgradeDuplexToTLS(globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    var handlers_ptr = handlers.vm.allocator.create(Handlers) catch |oe| bun.outOfMemory(oe);
    handlers_ptr.* = handlers;
