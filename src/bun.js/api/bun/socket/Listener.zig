...
pub const UnixOrHost = union(enum) {
    ...
    pub fn clone(this: UnixOrHost) UnixOrHost {
        switch (this) {
            .unix => |u| {
                return .{
                    .unix = (bun.default_allocator.dupe(u8, u) catch |oe| bun.outOfMemory(oe)),
                };
            },
            .host => |h| {
                return .{
                    .host = .{
                        .host = (bun.default_allocator.dupe(u8, h.host) catch |oe| bun.outOfMemory(oe)),
                        .port = this.host.port,
                    },
                };
            },
...
pub fn listen(globalObject: *jsc.JSGlobalObject, opts: JSValue) bun.JSError!JSValue {
    ...
    if (Environment.isWindows) {
        if (port == null) {
            ...
            if (normalizePipeName(slice, buf[0..])) |pipe_name| {
                const connection: Listener.UnixOrHost = .{ .unix = (hostname_or_unix.cloneIfNeeded(bun.default_allocator) catch |oe| bun.outOfMemory(oe)).slice() };
                if (ssl_enabled) {
                    if (ssl.?.protos) |p| {
                ...
                var socket = Listener{
                    ...
                    .socket_context = null,
                    .listener = .none,
                    .protos = if (protos) |p| (bun.default_allocator.dupe(u8, p) catch |oe| bun.outOfMemory(oe)) else null,
                };
                ...
                var this: *Listener = handlers.vm.allocator.create(Listener) catch |oe| bun.outOfMemory(oe);
                this.* = socket;
    ...
    var connection: Listener.UnixOrHost = if (port) |port_| .{
        .host = .{ .host = (hostname_or_unix.cloneIfNeeded(bun.default_allocator) catch |oe| bun.outOfMemory(oe)).slice(), .port = port_ },
    } else if (socket_config.fd) |fd| .{ .fd = fd } else .{
        .unix = (hostname_or_unix.cloneIfNeeded(bun.default_allocator) catch |oe| bun.outOfMemory(oe)).slice(),
    };
    var errno: c_int = 0;
    const listen_socket: *uws.ListenSocket = brk: {
        switch (connection) {
            .host => |c| {
                const host = bun.default_allocator.dupeZ(u8, c.host) catch |oe| bun.outOfMemory(oe);
            ...
            .unix => |u| {
                const host = bun.default_allocator.dupeZ(u8, u) catch |oe| bun.outOfMemory(oe);
                defer bun.default_allocator.free(host);
...
pub fn addServerName(this: *Listener, global: *jsc.JSGlobalObject, hostname: JSValue, tls: JSValue) bun.JSError!JSValue {
    ...
    const server_name = bun.default_allocator.dupeZ(u8, host_str.slice()) catch |oe| bun.outOfMemory(oe);
...
pub fn connectInner(globalObject: *jsc.JSGlobalObject, prev_maybe_tcp: ?*TCPSocket, prev_maybe_tls: ?*TLSSocket, opts: JSValue) bun.JSError!JSValue {
    ...
    var connection: Listener.UnixOrHost = blk: {
        ...
        if (port) |_| {
            break :blk .{ .host = .{ .host = (hostname_or_unix.cloneIfNeeded(bun.default_allocator) catch |oe| bun.outOfMemory(oe)).slice(), .port = port.? } };
        }

        break :blk .{ .unix = (hostname_or_unix.cloneIfNeeded(bun.default_allocator) catch |oe| bun.outOfMemory(oe)).slice() };
    };
        ...
        if (isNamedPipe) {
            ...
            if (ssl_enabled) {
                var tls = if (prev_maybe_tls) |prev| blk: {
                    ...
                    prev.protos = if (protos) |p| (bun.default_allocator.dupe(u8, p) catch |oe| bun.outOfMemory(oe)) else null;
    ...
    if (ssl_enabled) {
        ...
        if (ssl.?.server_name) |s| {
            server_name = bun.default_allocator.dupe(u8, s[0..bun.len(s)]) catch |oe| bun.outOfMemory(oe);
        }
