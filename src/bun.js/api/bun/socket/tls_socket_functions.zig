...
pub fn getSharedSigalgs(this: *This, globalObject: *jsc.JSGlobalObject, _: *jsc.CallFrame) bun.JSError!JSValue {
    ...
    for (0..@as(usize, @intCast(nsig))) |i| {
        ...
        if (hash_str != null) {
            ...
            const buffer = bun.default_allocator.alloc(u8, sig_with_md.len + hash_str_len + 1) catch |oe| bun.outOfMemory(oe);
            ...
            const buffer = bun.default_allocator.alloc(u8, sig_with_md.len + 6) catch |oe| bun.outOfMemory(oe);
            defer bun.default_allocator.free(buffer);
...
noinline fn getSSLException(globalThis: *jsc.JSGlobalObject, defaultMessage: []const u8) JSValue {
    ...
    if (written > 0) {
        ...
        zig_str = ZigString.init(std.fmt.allocPrint(bun.default_allocator, "OpenSSL {s}", .{message}) catch |oe| bun.outOfMemory(oe));
