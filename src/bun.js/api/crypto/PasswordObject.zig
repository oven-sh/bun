        ...
        pub const Result = struct {
            ...
            pub const Value = union(enum) {
                ...
                pub fn toErrorInstance(this: Value, globalObject: *jsc.JSGlobalObject) jsc.JSValue {
                    const error_code = std.fmt.allocPrint(bun.default_allocator, "PASSWORD{}", .{PascalToUpperUnderscoreCaseFormatter{ .input = @errorName(this.err) }}) catch |oe| bun.outOfMemory(oe);
