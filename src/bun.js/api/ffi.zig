...
pub const FFI = struct {
    ...
    const CompileC = struct {
        ...
        pub fn handleCompilationError(this_: ?*CompileC, message: ?[*:0]const u8) callconv(.C) void {
            ...
            this.deferred_errors.append(bun.default_allocator, bun.default_allocator.dupe(u8, msg) catch |oe| bun.outOfMemory(oe)) catch |oe| bun.outOfMemory(oe);
        }
        ...
        pub fn compile(this: *CompileC, globalThis: *JSGlobalObject) !struct { *TCC.State, []u8 } {
            ...
            for (this.symbols.map.keys(), this.symbols.map.values()) |symbol, *function| {
                ...
                // fallback allocator?
                const duped = bun.default_allocator.dupeZ(u8, symbol) catch |oe| bun.outOfMemory(oe);
    ...
    const StringArray = struct {
        ...
        pub fn fromJSArray(globalThis: *jsc.JSGlobalObject, value: jsc.JSValue, comptime property: []const u8) bun.JSError!StringArray {
            ...
            while (try iter.next()) |val| {
                ...
                items.append(str.toOwnedSliceZ(bun.default_allocator) catch |oe| bun.outOfMemory(oe)) catch |oe| bun.outOfMemory(oe);
            }
    ...
    pub fn Bun__FFI__cc(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        ...
        if (try object.getTruthy(globalThis, "flags")) |flags_value| {
            if (flags_value.isArray()) {
                ...
                defer flags.deinit();
                flags.appendSlice(CompileC.default_tcc_options) catch |oe| bun.outOfMemory(oe);
                ...
                while (try iter.next()) |value| {
                    ...
                    defer slice.deinit();
                    flags.append(' ') catch |oe| bun.outOfMemory(oe);
                    flags.appendSlice(slice.slice()) catch |oe| bun.outOfMemory(oe);
                }
                flags.append(0) catch |oe| bun.outOfMemory(oe);
                ...
                if (!str.isEmpty()) {
                    compile_c.flags = str.toOwnedSliceZ(allocator) catch |oe| bun.outOfMemory(oe);
                }
            }
        }
        ...
        if (try object.getTruthy(globalThis, "define")) |define_value| {
            if (define_value.getObject()) |define_obj| {
                ...
                while (try iter.next()) |entry| {
                    const key = entry.toOwnedSliceZ(allocator) catch |oe| bun.outOfMemory(oe);
                    var owned_value: [:0]const u8 = "";
                    if (!iter.value.isUndefinedOrNull()) {
                        if (iter.value.isString()) {
                            const value = try iter.value.getZigString(globalThis);
                            if (value.len > 0) {
                                owned_value = value.toOwnedSliceZ(allocator) catch |oe| bun.outOfMemory(oe);
                            }
                        }
                    }
                    if (globalThis.hasException()) {
                    ...
                    compile_c.define.append(allocator, .{ key, owned_value }) catch |oe| bun.outOfMemory(oe);
                }
            }
        }
        ...
        var tcc_state: ?*TCC.State, var bytes_to_free_on_error = compile_c.compile(globalThis) catch |err| {
            switch (err) {
                error.DeferredErrors => {
                    ...
                    writer.print("{d} errors while compiling {s}\n", .{ compile_c.deferred_errors.items.len, if (compile_c.current_file_for_errors.len > 0) compile_c.current_file_for_errors else compile_c.source.first() }) catch |oe| bun.outOfMemory(oe);

                    for (compile_c.deferred_errors.items) |deferred_error| {
                        writer.print("{s}\n", .{deferred_error}) catch |oe| bun.outOfMemory(oe);
                    }
        ...
        var lib = bun.default_allocator.create(FFI) catch |oe| bun.outOfMemory(oe);
        lib.* = .{
            .dylib = null,
    ...
    pub fn print(global: *JSGlobalObject, object: jsc.JSValue, is_callback_val: ?jsc.JSValue) bun.JSError!JSValue {
        ...
        jsc.markBinding(@src());
        var strs = std.ArrayList(bun.String).initCapacity(allocator, symbols.count()) catch |oe| bun.outOfMemory(oe);
        defer {
            for (strs.items) |str| {
                str.deref();
            }
...
const CompilerRT = struct {
    ...
    fn createCompilerRTDir() void {
        ...
        compiler_rt_dir = bun.default_allocator.dupeZ(u8, bun.getFdPath(.fromStdDir(bunCC), &path_buf) catch return) catch |oe| bun.outOfMemory(oe);
    }
