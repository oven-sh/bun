...
pub const AnyRoute = union(enum) {
    ...
    pub fn htmlRouteFromJS(argument: jsc.JSValue, init_ctx: *ServerInitContext) bun.JSError!?AnyRoute {
        if (argument.as(HTMLBundle)) |html_bundle| {
            const entry = init_ctx.dedupe_html_bundle_map.getOrPut(html_bundle) catch |oe| bun.outOfMemory(oe);
...
const ServePlugins = struct {
    ...
    fn loadAndResolvePlugins(this: *ServePlugins, global: *jsc.JSGlobalObject) bun.JSError!void {
        ...
        const alloc = sfb.get();
        const bunstring_array = alloc.alloc(bun.String, plugin_list.len) catch |oe| bun.outOfMemory(oe);
    ...
    pub fn handleOnResolve(this: *ServePlugins) void {
        ...
        for (html_bundle_routes.items) |route| {
            route.onPluginsResolved(plugin) catch |oe| bun.outOfMemory(oe);
            route.deref();
        }
        if (pending.dev_server) |server| {
            server.onPluginsResolved(plugin) catch |oe| bun.outOfMemory(oe);
        }
    }
    ...
    pub fn handleOnReject(this: *ServePlugins, global: *jsc.JSGlobalObject, err: JSValue) void {
        ...
        for (html_bundle_routes.items) |route| {
            route.onPluginsRejected() catch |oe| bun.outOfMemory(oe);
            route.deref();
        }
        if (pending.dev_server) |server| {
            server.onPluginsRejected() catch |oe| bun.outOfMemory(oe);
        }
...
pub fn NewServer(protocol_enum: enum { http, https }, development_kind: enum { debug, production }) type {
    return struct {
        ...
        pub fn getOrLoadPlugins(server: *ThisServer, callback: ServePlugins.Callback) ServePlugins.GetOrStartLoadResult {
            if (server.plugins) |p| {
                return p.getOrStartLoad(server.globalThis, callback) catch |oe| bun.outOfMemory(oe);
            }
            // no plugins
            return .{ .ready = null };
        }
        ...
        pub fn onFetch(
            ...
            if (first_arg.isString()) {
                ...
                existing_request = Request.init(
                    ...
                    headers,
                    this.vm.initRequestBodyValue(body) catch |oe| bun.outOfMemory(oe),
                    method,
                );
        ...
        pub fn prepareJsRequestContext(this: *ThisServer, req: *uws.Request, resp: *App.Response, should_deinit_context: ?*bool, create_js_request: bool, method: ?bun.http.Method) ?PreparedRequest {
            ...
            const ctx = this.request_pool_allocator.tryGet() catch |oe| bun.outOfMemory(oe);
        ...
        pub fn onWebSocketUpgrade(
            ...
            this.pending_requests += 1;
            req.setYield(false);
            var ctx = this.request_pool_allocator.tryGet() catch |oe| bun.outOfMemory(oe);
            var should_deinit_context = false;
        ...
        fn setRoutes(this: *ThisServer) jsc.JSValue {
                ...
                for (this.config.static_routes.items) |*entry| {
                    ...
                    switch (entry.route) {
                        ...
                        .html => |html_bundle_route| {
                            ...
                            if (dev_server) |dev| {
                                dev.html_router.put(dev.allocator, entry.path, html_bundle_route.data) catch |oe| bun.outOfMemory(oe);
                            }
                            needs_plugins = true;
                        },
                        .framework_router => {},
            ...
            if (dev_server) |dev| {
                ...
                has_dev_server_for_star_path = dev.setRoutes(this) catch |oe| bun.outOfMemory(oe);
