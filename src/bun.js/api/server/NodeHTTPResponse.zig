    ...
    pub fn preserveWebSocketHeadersIfNeeded(this: *UpgradeCTX) void {
        if (this.request) |request| {
            ...
            if (sec_websocket_key.len > 0) {
                this.sec_websocket_key = bun.default_allocator.dupe(u8, sec_websocket_key) catch |oe| bun.outOfMemory(oe);
            }
            if (sec_websocket_protocol.len > 0) {
                this.sec_websocket_protocol = bun.default_allocator.dupe(u8, sec_websocket_protocol) catch |oe| bun.outOfMemory(oe);
            }
            if (sec_websocket_extensions.len > 0) {
                this.sec_websocket_extensions = bun.default_allocator.dupe(u8, sec_websocket_extensions) catch |oe| bun.outOfMemory(oe);
            }
        }
    }
};
...
pub fn writeHead(this: *NodeHTTPResponse, globalObject: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    do_it: {
        ...
        const status_message = std.fmt.allocPrint(allocator, "{d} {s}", .{ status_code, message }) catch |oe| bun.outOfMemory(oe);
...
fn onBufferRequestBodyWhilePaused(this: *NodeHTTPResponse, chunk: []const u8, last: bool) void {
    ...
    this.buffered_request_body_data_during_pause.append(bun.default_allocator, chunk) catch |oe| bun.outOfMemory(oe);
    if (last) {
