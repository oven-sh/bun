...
pub fn initFromAnyBlob(blob: *const AnyBlob, options: InitFromBytesOptions) *StaticRoute {
    var headers = Headers.from(options.headers, bun.default_allocator, .{ .body = blob }) catch |oe| bun.outOfMemory(oe);
    if (options.mime_type) |mime_type| {
        if (headers.getContentType() == null) {
            headers.append("Content-Type", mime_type.value) catch |oe| bun.outOfMemory(oe);
        }
    }
    ...
    if (headers.get("etag") == null) {
        if (blob.slice().len > 0) {
            ETag.appendToHeaders(blob.slice(), &headers) catch |oe| bun.outOfMemory(oe);
        }
    }
