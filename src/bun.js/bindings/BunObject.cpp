...
static JSValue defaultBunSQLObject(VM& vm, JSObject* bunObject)
{
    ...
    // PropertyCallback handlers cannot safely return empty JSValue — JSC's
    // reifyStaticProperty passes the return value to putDirect() which calls
    // isGetterSetter() on it, causing a null deref if the value is empty.
...
{
    auto scope = DECLARE_THROW_SCOPE(vm);
    JSValue result = JSValue::decode(JSPasswordObject__create(bunObject->globalObject()));
    if (!result) [[unlikely]] {
        (void)scope.tryClearException();
        return jsUndefined();
    }
