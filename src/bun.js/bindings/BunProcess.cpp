...
JSC_DEFINE_HOST_FUNCTION(Process_functionThreadCpuUsage, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
...
#else
    // RUSAGE_THREAD is not available on macOS and other BSD systems
    // Fall back to RUSAGE_SELF for now
    struct rusage rusage;
    if (getrusage(RUSAGE_SELF, &rusage) != 0) {
        throwSystemError(throwScope, globalObject, "Failed to get thread CPU usage"_s, "getrusage"_s, errno);
        return {};
    ...
    int err = uv_getrusage(&rusage);
    if (err) {
        throwSystemError(throwScope, globalObject, "Failed to get thread CPU usage"_s, "uv_getrusage"_s, err);
        return {};
    }
#endif
