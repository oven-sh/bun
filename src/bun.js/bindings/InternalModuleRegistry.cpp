...
// Implemented in StandaloneModuleGraph.zig. The bool is set once at startup
// when a standalone executable was built with --bytecode; checking it lets
// normal Bun runs skip the extern lookup call.
extern "C" bool Bun__hasEmbeddedBuiltinBytecode;
extern "C" const uint8_t* Bun__findInternalModuleBytecode(uint32_t fieldId, size_t* outLen);
...
JSC::JSValue generateModule(JSC::JSGlobalObject* globalObject, JSC::VM& vm, const String& SOURCE, const String& moduleName, const String& urlString, uint32_t fieldId)
{
    ...
    UnlinkedFunctionExecutable* unlinked = nullptr;
    ...
    if (Bun__hasEmbeddedBuiltinBytecode) [[unlikely]] {
        size_t bytecodeLen = 0;
        if (const uint8_t* bytecode = Bun__findInternalModuleBytecode(fieldId, &bytecodeLen)) {
            auto cached = JSC::CachedBytecode::create(
                std::span<uint8_t>(const_cast<uint8_t*>(bytecode), bytecodeLen),
                [](const void*) {}, {});
            unlinked = JSC::decodeBuiltinFunctionExecutable(vm, WTF::move(cached));
        }
    }
...
JSValue initializeInternalModuleFromDisk(JSGlobalObject* globalObject, VM& vm, const WTF::String& moduleName, WTF::String fileBase, const WTF::String& urlString)
{
    ...
    if (auto contents = WTF::FileSystemImpl::readEntireFile(file)) {
        ...
        return generateModule(globalObject, vm, string, moduleName, urlString, UINT32_MAX);
    } else {
...
#define INTERNAL_MODULE_REGISTRY_GENERATE(globalObject, vm, moduleId, filename, SOURCE, urlString) \
    return generateModule(globalObject, vm, SOURCE, moduleId, urlString, (uint32_t)id)
#endif
...
#include "InternalModuleRegistry+sourceList.h"
