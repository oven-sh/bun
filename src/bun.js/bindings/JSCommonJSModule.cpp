...
static bool evaluateCommonJSModuleOnce(JSC::VM& vm, Zig::GlobalObject* globalObject, JSCommonJSModule* moduleObject, JSString* dirname, JSValue filename)
{
    ...
    // First, peek at what the code evaluates to by evaluating it in a temporary scope
    ...
    if (fn) {
        callData = JSC::getCallData(fn);
    }

    // If the evaluated code is not a callable function, it means the code is bare CommonJS
    // (e.g., `module.exports = [...]` without a function wrapper).
    // This can happen with code generated by bundlers like Turbopack.
    // In this case, we need to set up the module context and re-evaluate.
    if (!fn || callData.type == CallData::Type::None) [[unlikely]] {
        initializeModuleObject();
        RETURN_IF_EXCEPTION(scope, false);

        // Set up the CommonJS globals (module, exports, require, __filename, __dirname)
        // so that code like `module.exports = [...]` can execute properly
        auto exports = moduleObject->exportsObject();
        RETURN_IF_EXCEPTION(scope, false);
        globalObject->putDirect(vm, builtinNames(vm).exportsPublicName(), exports, 0);
        globalObject->putDirect(vm, builtinNames(vm).requirePublicName(), requireFunction, 0);
        ...
        // Now evaluate the code again with the proper context
        JSC::evaluate(globalObject, code, jsUndefined());
        RETURN_IF_EXCEPTION(scope, false);
