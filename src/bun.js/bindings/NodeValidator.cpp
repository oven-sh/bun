...
JSC_DEFINE_HOST_FUNCTION(jsFunction_validateInteger, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    if (!min.isNumber()) return Bun::ERR::INVALID_ARG_TYPE(scope, globalObject, "min"_s, "number"_s, value);
    if (!max.isNumber()) return Bun::ERR::INVALID_ARG_TYPE(scope, globalObject, "max"_s, "number"_s, value);
