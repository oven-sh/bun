    ...
    if (executionContextId > -1) {
        const auto initializeWorker = [&](WebCore::Worker& worker) -> void {
            ...
            if (options.env.has_value()) {
                HashMap<String, String> map = std::move(*std::exchange(options.env, std::nullopt));
                auto size = map.size();
                ...
                for (auto k : map) {
                    ...
                    env->putDirectMayBeIndex(globalObject, JSC::Identifier::fromString(vm, std::move(k.key)), strings.at(i++));
                }
    ...
    (JSC::JSGlobalObject * lexicalGlobalObject, JSC::CallFrame* callFrame))
{
    ...
    globalObject->vm().queueMicrotask(std::move(task));
...
// This code should pretty much only be called on Linux.
    ...
    if (type == JSC::Uint8ArrayType) {
        auto uint8array = JSC::JSUint8Array::create(globalObject, globalObject->m_typedArrayUint8.get(globalObject), std::move(buffer), 0, byteLength);
    ...
    if (type == JSC::ArrayBufferType) {
        ...
        return JSValue::encode(JSC::JSArrayBuffer::create(globalObject->vm(), structure, std::move(buffer)));
    } else {
        RELEASE_ASSERT_NOT_REACHED();
    }
#else
...
extern "C" JSC::EncodedJSValue Bun__createArrayBufferForCopy(JSC::JSGlobalObject* globalObject, const void* ptr, size_t len)
{
    ...
    RELEASE_AND_RETURN(scope, JSValue::encode(JSC::JSArrayBuffer::create(globalObject->vm(), globalObject->arrayBufferStructure(JSC::ArrayBufferSharingMode::Default), std::move(arrayBuffer))));
}
...
extern "C" JSC::EncodedJSValue Bun__makeArrayBufferWithBytesNoCopy(JSC::JSGlobalObject* globalObject, const void* ptr, size_t len, JSTypedArrayBytesDeallocator deallocator, void* deallocatorContext)
{
    ...
    JSArrayBuffer* jsBuffer = JSArrayBuffer::create(vm, globalObject->arrayBufferStructure(ArrayBufferSharingMode::Default), std::move(buffer));
    RETURN_IF_EXCEPTION(scope, {});
...
extern "C" JSC::EncodedJSValue Bun__makeTypedArrayWithBytesNoCopy(JSC::JSGlobalObject* globalObject, TypedArrayType ty, const void* ptr, size_t len, JSTypedArrayBytesDeallocator deallocator, void* deallocatorContext)
{
    ...
    RefPtr<ArrayBuffer>&& buffer = std::move(buffer_);
    if (!buffer) {
...
#define JSC_TYPED_ARRAY_FACTORY(type) \
    case Type##type:                  \
        RELEASE_AND_RETURN(scope, JSValue::encode(JS##type##Array::create(globalObject, globalObject->typedArrayStructure(Type##type, isResizableOrGrowableShared), std::move(buffer), offset, length)));
...
static inline JSC::EncodedJSValue jsFunctionAddEventListenerBody(JSC::JSGlobalObject* lexicalGlobalObject, JSC::CallFrame* callFrame, Zig::GlobalObject* castedThis)
{
    ...
    RETURN_IF_EXCEPTION(throwScope, {});
    auto result = JSValue::encode(WebCore::toJS<IDLUndefined>(*lexicalGlobalObject, throwScope, [&]() -> decltype(auto) { return impl->addEventListenerForBindings(std::move(type), std::move(listener), std::move(options)); }));
    RETURN_IF_EXCEPTION(throwScope, {});
...
static inline JSC::EncodedJSValue jsFunctionRemoveEventListenerBody(JSC::JSGlobalObject* lexicalGlobalObject, JSC::CallFrame* callFrame, Zig::GlobalObject* castedThis)
{
    ...
    RETURN_IF_EXCEPTION(throwScope, {});
    auto result = JSValue::encode(WebCore::toJS<IDLUndefined>(*lexicalGlobalObject, throwScope, [&]() -> decltype(auto) { return impl->removeEventListenerForBindings(std::move(type), std::move(listener), std::move(options)); }));
    RETURN_IF_EXCEPTION(throwScope, {});
    vm.writeBarrier(&static_cast<JSObject&>(*castedThis), argument1.value());
    return result;
}
...
JSC_DEFINE_HOST_FUNCTION(createWritableStreamFromInternal, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    return JSValue::encode(toJSNewlyCreated(globalObject, jsDOMGlobalObject, WritableStream::create(std::move(internalWritableStream))));
}
...
JSC_DEFINE_HOST_FUNCTION(addAbortAlgorithmToSignal, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    auto algorithmIdentifier = AbortSignal::addAbortAlgorithmToSignal(abortSignal->wrapped(), std::move(abortAlgorithm));
...
static JSC::JSPromise* handleResponseOnStreamingAction(JSGlobalObject* lexicalGlobalObject, JSC::JSValue source, JSC::Wasm::CompilerMode mode, JSC::JSObject* importObject)
{
    ...
    auto wrapper = WebCore::toJSNewlyCreated(globalObject, globalObject, std::move(compiler));
