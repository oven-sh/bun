...
{
    CheckPrimeJobCtx* ctx = new CheckPrimeJobCtx(std::move(candidate), checks);
...
JSC_DEFINE_HOST_FUNCTION(jsCheckPrime, (JSC::JSGlobalObject * lexicalGlobalObject, JSC::CallFrame* callFrame))
{
    ...
    CheckPrimeJob::createAndSchedule(lexicalGlobalObject, std::move(candidate), checks, callback);
...
{
    GeneratePrimeJobCtx* ctx = new GeneratePrimeJobCtx(size, safe, std::move(prime), std::move(add), std::move(rem), bigint);
...
JSValue GeneratePrimeJob::result(JSGlobalObject* globalObject, JSC::ThrowScope& scope, const ncrypto::BignumPointer& prime, bool bigint)
{
    ...
    return JSArrayBuffer::create(vm, globalObject->arrayBufferStructure(), std::move(buf));
}
...
JSC_DEFINE_HOST_FUNCTION(jsGeneratePrime, (JSC::JSGlobalObject * lexicalGlobalObject, JSC::CallFrame* callFrame))
{
    ...
    GeneratePrimeJob::createAndSchedule(lexicalGlobalObject, size, safe, std::move(prime), std::move(add), std::move(rem), bigint, callback);
