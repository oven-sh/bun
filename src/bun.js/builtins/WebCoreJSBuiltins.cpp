...
const char* const s_transformStreamInternalsCreateTransformStreamCode = "(function (_,I,T,c,j,q,v){\"use strict\";if(c===@undefined)c=1;if(j===@undefined)j=()=>1;if(q===@undefined)q=0;if(v===@undefined)v=()=>1;@assert(c>=0),@assert(q>=0);const x={};@putByIdDirectPrivate(x,\"TransformStream\",!0);const B=new @TransformStream(x),D=@newPromiseCapability(@Promise);@initializeTransformStream(B,D.@promise,c,j,q,v);const E=new @TransformStreamDefaultController;return @setUpTransformStreamDefaultController(B,E,I,T),_().@then(()=>{D.@resolve.@call()},(F)=>{D.@reject.@call(@undefined,F)}),B})\n";
...
const char* const s_transformStreamInternalsInitializeTransformStreamCode = "(function (_,I,f,j,q,v){\"use strict\";const x=()=>{return I},B=(N)=>{return @transformStreamDefaultSinkWriteAlgorithm(_,N)},C=(N)=>{return @transformStreamDefaultSinkAbortAlgorithm(_,N)},D=()=>{return @transformStreamDefaultSinkCloseAlgorithm(_)},E=@createWritableStream(x,B,D,C,f,j),F=()=>{return @transformStreamDefaultSourcePullAlgorithm(_)},G=(N)=>{return @transformStreamErrorWritableAndUnblockWrite(_,N),@Promise.@resolve()},J={};@putByIdDirectPrivate(J,\"start\",x),@putByIdDirectPrivate(J,\"pull\",F),@putByIdDirectPrivate(J,\"cancel\",G);const K={};@putByIdDirectPrivate(K,\"size\",v),@putByIdDirectPrivate(K,\"highWaterMark\",q);const L=new @ReadableStream(J,K);@putByIdDirectPrivate(_,\"writable\",E),@putByIdDirectPrivate(_,\"internalWritable\",@getInternalWritableStream(E)),@putByIdDirectPrivate(_,\"readable\",L),@putByIdDirectPrivate(_,\"backpressure\",@undefined),@putByIdDirectPrivate(_,\"backpressureChangePromise\",@undefined),@transformStreamSetBackpressure(_,!0),@putByIdDirectPrivate(_,\"controller\",@undefined)})\n";
...
const char* const s_transformStreamInternalsSetUpTransformStreamDefaultControllerFromTransformerCode = "(function (d,p,U){\"use strict\";const _=new @TransformStreamDefaultController;let b=(q)=>{try{@transformStreamDefaultControllerEnqueue(_,q)}catch(v){return @Promise.@reject(v)}return @Promise.@resolve()},j=()=>{return @Promise.@resolve()};if(\"transform\"in U)b=(q)=>{return @promiseInvokeOrNoopMethod(p,U[\"transform\"],[q,_])};if(\"flush\"in U)j=()=>{return @promiseInvokeOrNoopMethod(p,U[\"flush\"],[_])};@setUpTransformStreamDefaultController(d,_,b,j)})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToCode = "(function (_,c){\"use strict\";@assert(@isReadableStream(_));const f=new @ReadableStreamDefaultReader(_);@getByIdDirectPrivate(f,\"closedPromiseCapability\").@promise.@then(()=>{},(R)=>{c.error(R)});function g(){@readableStreamDefaultReaderRead(f).@then(function(R){if(R.done){c.close();return}try{c.enqueue(R.value)}catch(b){c.error(\"ReadableStream chunk enqueueing in the sink failed\");return}g()},function(R){c.error(R)})}g()})\n";
...
const char* const s_readableStreamInternalsAcquireReadableStreamDefaultReaderCode = "(function (_){\"use strict\";var c=@getByIdDirectPrivate(_,\"start\");if(c)c.@call(_);return new @ReadableStreamDefaultReader(_)})\n";
...
const char* const s_readableStreamInternalsSetupReadableStreamDefaultControllerCode = "(function (C,I,_,b,f,j,q){\"use strict\";const v=new @ReadableStreamDefaultController(C,I,_,b,@isReadableStream),w=()=>@promiseInvokeOrNoopMethod(I,j,[v]),x=(B)=>@promiseInvokeOrNoopMethod(I,q,[B]);@putByIdDirectPrivate(v,\"pullAlgorithm\",w),@putByIdDirectPrivate(v,\"cancelAlgorithm\",x),@putByIdDirectPrivate(v,\"pull\",@readableStreamDefaultControllerPull),@putByIdDirectPrivate(v,\"cancel\",@readableStreamDefaultControllerCancel),@putByIdDirectPrivate(C,\"readableStreamController\",v),@readableStreamDefaultControllerStart(v)})\n";
...
const char* const s_readableStreamInternalsCreateReadableStreamControllerCode = "(function (f,_,b){\"use strict\";const j=_.type,q=@toString(j);if(q===\"bytes\"){if(b.highWaterMark===@undefined)b.highWaterMark=0;if(b.size!==@undefined)@throwRangeError(\"Strategy for a ReadableByteStreamController cannot have a size\");@putByIdDirectPrivate(f,\"readableStreamController\",new @ReadableByteStreamController(f,_,b.highWaterMark,@isReadableStream))}else if(q===\"direct\"){var v=b\?.highWaterMark;@initializeArrayBufferStream.@call(f,_,v)}else if(j===@undefined){if(b.highWaterMark===@undefined)b.highWaterMark=1;@setupReadableStreamDefaultController(f,_,b.size,b.highWaterMark,_.start,_.pull,_.cancel)}else @throwRangeError(\"Invalid type for underlying source\")})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToWritableStreamCode = "(function (_,z,B,f,k,q){\"use strict\";const w=!!@getByIdDirectPrivate(_,\"start\");if(@assert(@isReadableStream(_)),@assert(@isWritableStream(z)),@assert(!@isReadableStreamLocked(_)),@assert(!@isWritableStreamLocked(z)),@assert(q===@undefined||@isAbortSignal(q)),@getByIdDirectPrivate(_,\"underlyingByteSource\")!==@undefined)return @Promise.@reject(\"Piping to a readable bytestream is not supported\");let x={source:_,destination:z,preventAbort:f,preventCancel:k,preventClose:B,signal:q};if(x.reader=@acquireReadableStreamDefaultReader(_),x.writer=@acquireWritableStreamDefaultWriter(z),@putByIdDirectPrivate(_,\"disturbed\",!0),x.finalized=!1,x.shuttingDown=!1,x.promiseCapability=@newPromiseCapability(@Promise),x.pendingReadPromiseCapability=@newPromiseCapability(@Promise),x.pendingReadPromiseCapability.@resolve.@call(),x.pendingWritePromise=@Promise.@resolve(),q!==@undefined){const E=(F)=>{if(x.finalized)return;@pipeToShutdownWithAction(x,()=>{const H=!x.preventAbort&&@getByIdDirectPrivate(x.destination,\"state\")===\"writable\"\?@writableStreamAbort(x.destination,F):@Promise.@resolve(),J=!x.preventCancel&&@getByIdDirectPrivate(x.source,\"state\")===@streamReadable\?@readableStreamCancel(x.source,F):@Promise.@resolve();let K=@newPromiseCapability(@Promise),L=!0,M=()=>{if(L){L=!1;return}K.@resolve.@call()},N=(O)=>{K.@reject.@call(@undefined,O)};return H.@then(M,N),J.@then(M,N),K.@promise},F)};if(@whenSignalAborted(q,E))return x.promiseCapability.@promise}return @pipeToErrorsMustBePropagatedForward(x),@pipeToErrorsMustBePropagatedBackward(x),@pipeToClosingMustBePropagatedForward(x),@pipeToClosingMustBePropagatedBackward(x),@pipeToLoop(x),x.promiseCapability.@promise})\n";
...
const char* const s_readableStreamInternalsPipeToClosingMustBePropagatedForwardCode = "(function (d){\"use strict\";const r=()=>{d.pendingReadPromiseCapability.@resolve.@call(@undefined,!1);const s=@getByIdDirectPrivate(d.source,\"storedError\");if(!d.preventClose){@pipeToShutdownWithAction(d,()=>@writableStreamDefaultWriterCloseWithErrorPropagation(d.writer));return}@pipeToShutdown(d)};if(@getByIdDirectPrivate(d.source,\"state\")===@streamClosed){r();return}@getByIdDirectPrivate(d.reader,\"closedPromiseCapability\").@promise.@then(r,@undefined)})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const _=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!_)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const D=@getByIdDirectPrivate(_,\"state\");if(@putByIdDirectPrivate(_,\"disturbed\",!0),D===@streamClosed)return{value:[],size:0,done:!0};else if(D===@streamErrored)throw @getByIdDirectPrivate(_,\"storedError\");var B=@getByIdDirectPrivate(_,\"readableStreamController\"),d=@getByIdDirectPrivate(B,\"queue\");if(!d)return B.@pull(B).@then(function({done:F,value:G}){return F\?{done:!0,value:[],size:0}:{value:[G],size:1,done:!1}});const W=d.content;var j=d.size,k=W.toArray(!1),w=k.length;if(w>0){var x=@newArrayWithSize(w);if(@isReadableByteStreamController(B)){{const F=k[0];if(!(@ArrayBuffer.@isView(F)||F instanceof @ArrayBuffer))@putByValDirect(x,0,new @Uint8Array(F.buffer,F.byteOffset,F.byteLength));else @putByValDirect(x,0,F)}for(var A=1;A<w;A++){const F=k[A];if(!(@ArrayBuffer.@isView(F)||F instanceof @ArrayBuffer))@putByValDirect(x,A,new @Uint8Array(F.buffer,F.byteOffset,F.byteLength));else @putByValDirect(x,A,F)}}else{@putByValDirect(x,0,k[0].value);for(var A=1;A<w;A++)@putByValDirect(x,A,k[A].value)}if(@resetQueue(@getByIdDirectPrivate(B,\"queue\")),@getByIdDirectPrivate(B,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(B,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(B))@readableStreamDefaultControllerCallPullIfNeeded(B);else if(@isReadableByteStreamController(B))@readableByteStreamControllerCallPullIfNeeded(B);return{value:x,size:j,done:!1}}var C=(F)=>{if(F.done)return{value:[],size:0,done:!0};var G=@getByIdDirectPrivate(_,\"readableStreamController\"),H=@getByIdDirectPrivate(G,\"queue\"),I=[F.value].concat(H.content.toArray(!1)),J=I.length;if(@isReadableByteStreamController(G))for(var K=0;K<J;K++){const Q=I[K];if(!(@ArrayBuffer.@isView(Q)||Q instanceof @ArrayBuffer)){const{buffer:S,byteOffset:T,byteLength:U}=Q;@putByValDirect(I,K,new @Uint8Array(S,T,U))}}else for(var K=1;K<J;K++)@putByValDirect(I,K,I[K].value);var N=H.size;if(@resetQueue(H),@getByIdDirectPrivate(G,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(G,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(G))@readableStreamDefaultControllerCallPullIfNeeded(G);else if(@isReadableByteStreamController(G))@readableByteStreamControllerCallPullIfNeeded(G);return{value:I,size:N,done:!1}},E=B.@pull(B);if(E&&@isPromise(E))return E.@then(C);return C(E)})\n";
...
const char* const s_importMetaObjectRequireCode = "(function (r){\"use strict\";const t=this\?.path\?\?arguments.callee.path;if(typeof r!==\"string\")@throwTypeError(\"require(name) must be a string\");return @internalRequire(@resolveSync(r,t))})\n";
