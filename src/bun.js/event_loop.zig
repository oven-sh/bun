...
pub fn tickImmediateTasks(this: *EventLoop, virtual_machine: *VirtualMachine) void {
    ...
    if (this.next_immediate_tasks.capacity > 0) {
        ...
        @branchHint(.unlikely);
        this.immediate_tasks.appendSlice(bun.default_allocator, this.next_immediate_tasks.items) catch |oe| bun.outOfMemory(oe);
...
pub fn enqueueImmediateTask(this: *EventLoop, task: *Timer.ImmediateObject) void {
    this.immediate_tasks.append(bun.default_allocator, task) catch |oe| bun.outOfMemory(oe);
}
