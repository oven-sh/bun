    ...
    pub fn decodeIPCMessage(data: []const u8, globalThis: *jsc.JSGlobalObject) IPCDecodeError!DecodeIPCMessageResult {
        ...
        if (bun.strings.indexOfChar(data, '\n')) |idx| {
            ...
            const deserialized = str.toJSByParseJSON(globalThis) catch |e| switch (e) {
                ...
                error.OutOfMemory => return bun.outOfMemory(error.OutOfMemory),
            };
...
pub const SendQueue = struct {
    ...
    pub fn startMessage(self: *SendQueue, global: *jsc.JSGlobalObject, callback: jsc.JSValue, handle: ?Handle) bun.JSError!*SendHandle {
        ...
        self.queue.append(.{ .handle = handle, .callbacks = .init(callback) }) catch |oe| bun.outOfMemory(oe);
    ...
    pub fn insertMessage(this: *SendQueue, message: SendHandle) void {
        ...
        if ((this.queue.items.len == 0 or this.queue.items[0].data.cursor == 0) and !this.write_in_progress) {
            ...
            this.queue.insert(0, message) catch |oe| bun.outOfMemory(oe);
        } else {
            ...
            this.queue.insert(1, message) catch |oe| bun.outOfMemory(oe);
        }
    }
    ...
    pub fn writeVersionPacket(this: *SendQueue, global: *JSGlobalObject) void {
        ...
        if (bytes.len > 0) {
            this.queue.append(.{ .handle = null, .callbacks = .none }) catch |oe| bun.outOfMemory(oe);
            this.queue.items[this.queue.items.len - 1].data.write(bytes) catch |oe| bun.outOfMemory(oe);
    ...
    fn _write(this: *SendQueue, data: []const u8, fd: ?bun.FileDescriptor) void {
        ...
        return switch (Environment.isWindows) {
            true => {
                ...
                const write_req_slice = bun.default_allocator.dupe(u8, data[0..write_len]) catch |oe| bun.outOfMemory(oe);
    ...
    pub fn windowsConfigureClient(this: *SendQueue, pipe_fd: bun.FileDescriptor) !void {
        log("configureClient", .{});
        const ipc_pipe = bun.default_allocator.create(uv.Pipe) catch |oe| bun.outOfMemory(oe);
    ...
    if (internal_command) |icmd| {
        switch (icmd) {
            .handle => |msg_data| {
                ...
                handle.data.write(packet) catch |oe| bun.outOfMemory(oe);
        ...
        while (true) {
            const result = decodeIPCMessage(send_queue.mode, data, globalThis) catch |e| switch (e) {
                error.NotEnoughBytes => {
                    _ = send_queue.incoming.write(bun.default_allocator, data) catch |oe| bun.outOfMemory(oe);
    ...
    pub const WindowsNamedPipe = struct {
        fn onReadAlloc(send_queue: *SendQueue, suggested_size: usize) []u8 {
            ...
            if (available.len < suggested_size) {
                send_queue.incoming.ensureUnusedCapacity(bun.default_allocator, suggested_size) catch |oe| bun.outOfMemory(oe);
