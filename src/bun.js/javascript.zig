...
pub const VirtualMachine = struct {
    ...
    fn configureDebugger(this: *VirtualMachine, cli_flag: bun.CLI.Command.Debugger) void {
        ...
        const set_breakpoint_on_first_line = unix.len > 0 and strings.includes(unix, "?break=1"); // If we should set a breakpoint on the first line
        const wait_for_debugger = unix.len > 0 and strings.includes(unix, "?wait=1"); // If we should wait (either 30ms if report is passed, forever otherwise) for the debugger to connect
        const report = unix.len > 0 and strings.endsWith(unix, "&report=1"); // If either `break=1` or `wait=1` are specified, passing this will make the wait be 30ms and act like it's reporting to clients like the VSCode extension
