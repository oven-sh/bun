  ...
  if (value.isCell()) {
    ...
    if (value.isCallable()) {
      ...
      if (callData.type == JSC::CallData::Type::Native) {
        if (callData.native.function.untaggedPtr() ==
...
JSC_DEFINE_HOST_FUNCTION(jsFunctionResolveLookupPaths,
  ...
  if (request.characterAt(0) != '.' ||
      (request.length() > 1 && request.characterAt(1) != '.' &&
...
JSC_DEFINE_HOST_FUNCTION(jsFunctionFindPath, (JSGlobalObject * globalObject,
                                              JSC::CallFrame *callFrame)) {
  JSC::VM &vm = globalObject->vm();
  auto scope = DECLARE_THROW_SCOPE(vm);
  ...
  JSArray *paths =
      paths_value.isCell() ? jsDynamicCast<JSArray *>(paths_value) : nullptr;
...
JSC_DEFINE_HOST_FUNCTION(jsFunctionInitPaths, (JSGlobalObject * globalObject,
                                               JSC::CallFrame *callFrame)) {
  return JSC::JSValue::encode(JSC::jsUndefined());
}
...
JSC_DEFINE_HOST_FUNCTION(jsFunctionLoad, (JSGlobalObject * globalObject,
                                          JSC::CallFrame *callFrame)) {
  return JSC::JSValue::encode(JSC::jsUndefined());
}

JSC_DEFINE_HOST_FUNCTION(jsFunctionRunMain, (JSGlobalObject * globalObject,
                                             JSC::CallFrame *callFrame)) {
  return JSC::JSValue::encode(JSC::jsUndefined());
}
...
public:
  ...
  static constexpr unsigned StructureFlags =
      Base::StructureFlags | HasStaticPropertyTable;
  ...
  static JSModuleConstructor *create(JSC::VM &vm,
    ...
    auto *moduleConstructor =
        new (NotNull, JSC::allocateCell<JSModuleConstructor>(vm))
