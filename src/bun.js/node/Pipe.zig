...
//!
//! GC liveness: JSRef starts weak, upgrades to strong on readStart and back to
//! weak on readStop, so the JS wrapper survives while the poll is live.
...
fn closeInternal(this: *Pipe) void {
    ...
    if (!this.flags.finalized) this.this_value.downgrade();
}
...
pub fn onReadChunk(this: *Pipe, chunk: []const u8, has_more: bun.io.ReadState) bool {
    ...
    if (this.flags.finalized) return false;
