...
pub fn sendHelperChild(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    if (callback.isFunction()) {
        ...
        child_singleton.callbacks.put(bun.default_allocator, child_singleton.seq, jsc.Strong.Optional.create(callback, globalThis)) catch |oe| bun.outOfMemory(oe);
    }
...
pub const InternalMsgHolder = struct {
    ...
    pub fn enqueue(this: *InternalMsgHolder, message: jsc.JSValue, globalThis: *jsc.JSGlobalObject) void {
        ...
        const new_item_ptr = this.messages.addOne(bun.default_allocator) catch |oe| bun.outOfMemory(oe);
...
pub fn sendHelperPrimary(globalThis: *jsc.JSGlobalObject, callframe: *jsc.CallFrame) bun.JSError!jsc.JSValue {
    ...
    if (callback.isFunction()) {
        ipc_data.internal_msg_queue.callbacks.put(bun.default_allocator, ipc_data.internal_msg_queue.seq, jsc.Strong.Optional.create(callback, globalThis)) catch |oe| bun.outOfMemory(oe);
    }
