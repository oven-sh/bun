    ...
    switch (ctype) {
        .ERRNO => {
            if (comptime getErrnoConstant(name)) |constant| {
                object.put(globalObject, bun.String.static("E" ++ name), jsc.JSValue.jsNumber(constant));
            }
        },
        .ERRNO_WIN => {
            if (comptime getWindowsErrnoConstant(name)) |constant| {
                object.put(globalObject, bun.String.static(name), jsc.JSValue.jsNumber(constant));
            }
        },
        .SIG => {
            if (comptime getSignalsConstant(name)) |constant| {
                object.put(globalObject, bun.String.static("SIG" ++ name), jsc.JSValue.jsNumber(constant));
            }
        },
        .DLOPEN => {
            if (comptime getDlopenConstant(name)) |constant| {
                object.put(globalObject, bun.String.static("RTLD_" ++ name), jsc.JSValue.jsNumber(constant));
            }
        },
        .OTHER => {
            object.put(globalObject, bun.String.static(name), jsc.JSValue.jsNumberFromInt32(value.?));
        },
    }
}
...
pub fn create(globalObject: *jsc.JSGlobalObject) jsc.JSValue {
    ...
    object.put(globalObject, bun.String.static("errno"), createErrno(globalObject));
    object.put(globalObject, bun.String.static("signals"), createSignals(globalObject));
    object.put(globalObject, bun.String.static("priority"), createPriority(globalObject));
    object.put(globalObject, bun.String.static("dlopen"), createDlopen(globalObject));
