...
pub const PathWatcher = struct {
        ...
        for (this.handlers.values(), 0..) |*event, i| {
            if (event.emit(hash, timestamp, event_type)) {
                ...
                onPathUpdateFn(ctx, event_type.toEvent(switch (ctx.encoding) {
                    ...
                    else => .{ .bytes_to_free = bun.default_allocator.dupeZ(u8, path) catch |oe| bun.outOfMemory(oe) },
                }), is_file);
                if (comptime bun.Environment.isDebug)
                    debug_count += 1;
                onUpdateEndFn(ctx);
    ...
    pub fn init(manager: *PathWatcherManager, path: [:0]const u8, recursive: bool) bun.sys.Maybe(*PathWatcher) {
        ...
        const watchers_entry = manager.watchers.getOrPut(bun.default_allocator, @as([]const u8, event_path)) catch |oe| bun.outOfMemory(oe);
        ...
        watchers_entry.key_ptr.* = bun.default_allocator.dupeZ(u8, event_path) catch |oe| bun.outOfMemory(oe);
...
pub fn watch(
    ...
    watcher.handlers.put(bun.default_allocator, ctx, .{}) catch |oe| bun.outOfMemory(oe);
