...
fn spin(this: *WebWorker) void {
    ...
    evaluate_entry_point: {
        var promise = vm.loadEntryPointForWebWorker(path) catch {
            // Snapshot after preloads ran so we preserve any `process.exitCode`
            // they set.
            ...
            // A preload's `error` listener may cancel the dispatch; keep the
            // worker alive in that case. Restore only our sentinel `1` —
            // honor a new exitCode set by the listener itself.
            if (this.flushLogs(vm)) {
        ...
        if (promise.status() == .rejected) {
            ...
            // Cancelled: keep the worker running. Reset so later errors are
            // re-evaluated afresh rather than inheriting this dispatch's state.
...
/// callers that would normally terminate must keep the worker running in that
/// case.
fn flushLogs(this: *WebWorker, vm: *jsc.VirtualMachine) bool {
    jsc.markBinding(@src());
    ...
    const prevented = bun.jsc.fromJSHostCallGeneric(vm.global, @src(), WebWorker__dispatchError, .{ vm.global, this.cpp_worker, str, err }) catch |e| {
        ...
        // Clear here too — `reportUncaughtException` doesn't touch `vm.log`,
        // and leaving `msgs` non-empty would cause a later `flushLogs()` call
        // to re-dispatch the same error event (possibly without the listener
        // cancelling it the second time).
        vm.log.reset();
        return false;
    };
    // `Log.toJS` doesn't clear `msgs`; reset to avoid re-dispatch if called
    // again (e.g. the keep-alive path where spin() stays in the event loop).
    vm.log.reset();
    return prevented;
}
...
fn onUnhandledRejection(vm: *jsc.VirtualMachine, globalObject: *jsc.JSGlobalObject, error_instance_or_exception: jsc.JSValue) void {
    ...
    // Reset so callers see a fresh value, not a stale `true` from a prior
    // dispatch (e.g. another `uncaughtException` that was cancelled).
