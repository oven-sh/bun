...
pub fn getFormDataEncoding(this: *Blob) ?*bun.FormData.AsyncFormData {
    ...
    return bun.FormData.AsyncFormData.init(this.allocator orelse bun.default_allocator, encoding) catch |oe| bun.outOfMemory(oe);
}
...
pub fn doReadFile(this: *Blob, comptime Function: anytype, global: *JSGlobalObject) JSValue {
    ...
    var read_file_task = read_file.ReadFileTask.createOnJSThread(bun.default_allocator, global, file_read) catch |oe| bun.outOfMemory(oe);
...
const URLSearchParamsConverter = struct {
    ...
    pub fn convert(this: *URLSearchParamsConverter, str: ZigString) void {
        var out = str.toSlice(this.allocator).cloneIfNeeded(this.allocator) catch |oe| bun.outOfMemory(oe);
...
pub fn fromDOMFormData(
    ...
    const store = Blob.Store.init(context.joiner.done(allocator) catch |oe| bun.outOfMemory(oe), allocator);
    var blob = Blob.initWithStore(store, globalThis);
    blob.content_type = std.fmt.allocPrint(allocator, "multipart/form-data; boundary={s}", .{boundary}) catch |oe| bun.outOfMemory(oe);
    blob.content_type_allocated = true;
    blob.content_type_was_set = true;
...
pub fn writeFileWithSourceDestination(ctx: *jsc.JSGlobalObject, source_blob: *Blob, destination_blob: *Blob, options: WriteFileOptions) bun.JSError!jsc.JSValue {
    ...
    if (destination_type == .file and source_type == .bytes) {
        ...
        var task = write_file.WriteFileTask.createOnJSThread(bun.default_allocator, ctx, file_copier) catch |oe| bun.outOfMemory(oe);
            ...
            switch (store_.data) {
                .bytes => |*bytes| {
                    bytes.stored_name = bun.PathString.init(
                        (name_value_str.toUTF8WithoutRef(bun.default_allocator).cloneIfNeeded(bun.default_allocator) catch |oe| bun.outOfMemory(oe)).slice(),
                    );
                },
                .s3, .file => {
...
pub fn findOrCreateFileFromPath(path_or_fd: *jsc.Node.PathOrFileDescriptor, globalThis: *JSGlobalObject, comptime check_s3: bool) Blob {
    ...
    if (check_s3) {
        if (path_or_fd.* == .path) {
            if (strings.startsWith(path_or_fd.path.slice(), "s3://")) {
                ...
                return Blob.initWithStore(Blob.Store.initS3(copy.path, null, credentials, allocator) catch |oe| bun.outOfMemory(oe), globalThis);
            }
        }
    }
                ...
                if (Environment.isWindows and bun.strings.eqlComptime(slice, "/dev/null")) {
                    ...
                    path_or_fd.* = .{
                        .path = .{
                            ...
                            .string = bun.PathString.init(allocator.dupe(u8, "\\\\.\\NUL") catch |oe| bun.outOfMemory(oe)),
                        },
                    };
    ...
    return Blob.initWithStore(Blob.Store.initFile(path, null, allocator) catch |oe| bun.outOfMemory(oe), globalThis);
}
            ...
            if (try options_object.getTruthy(globalThis, "type")) |content_type| {
                ...
                if (strings.isAllASCII(slice)) {
                    ...
                    if (globalThis.bunVM().mimeType(slice)) |mime| {
                        ...
                        const content_type_buf = bun.default_allocator.alloc(u8, slice.len) catch |oe| bun.outOfMemory(oe);
...
export fn Blob__fromBytes(globalThis: *jsc.JSGlobalObject, ptr: ?[*]const u8, len: usize) callconv(.C) *Blob {
    ...
    const bytes = bun.default_allocator.dupe(u8, ptr.?[0..len]) catch |oe| bun.outOfMemory(oe);
...
pub fn create(
    ...
    return tryCreate(bytes_, allocator_, globalThis, was_string) catch |oe| bun.outOfMemory(oe);
}
...
pub fn dupeWithContentType(this: *const Blob, include_content_type: bool) Blob {
    ...
    if (duped.content_type_allocated and duped.allocator != null and !include_content_type) {
        ...
        duped.content_type = bun.default_allocator.dupe(u8, this.content_type) catch |oe| bun.outOfMemory(oe);
    }
