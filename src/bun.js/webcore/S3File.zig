...
pub fn constructS3FileWithS3CredentialsAndOptions(
    ...
    const store = brk: {
        if (aws_options.changed_credentials) {
            break :brk Blob.Store.initS3(path, null, aws_options.credentials, bun.default_allocator) catch |oe| bun.outOfMemory(oe);
        } else {
            break :brk Blob.Store.initS3WithReferencedCredentials(path, null, default_credentials, bun.default_allocator) catch |oe| bun.outOfMemory(oe);
        }
    };
            ...
            if (try opts.getTruthyComptime(globalObject, "type")) |file_type| {
                inner: {
                    if (file_type.isString()) {
                        ...
                        const content_type_buf = allocator.alloc(u8, slice.len) catch |oe| bun.outOfMemory(oe);
...
pub fn constructS3FileWithS3Credentials(
    ...
    defer aws_options.deinit();
    const store = Blob.Store.initS3(path, null, aws_options.credentials, bun.default_allocator) catch |oe| bun.outOfMemory(oe);
    errdefer store.deinit();
    store.data.s3.options = aws_options.options;
    store.data.s3.acl = aws_options.acl;
    store.data.s3.storage_class = aws_options.storage_class;
