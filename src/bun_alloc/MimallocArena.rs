...
// recycled a destroyed `mi_heap_t*` slot. Plain `mi_heap_malloc` is correct.
...
// check on the `mi_heap_*` allocation paths: a per-
...
pub struct MimallocArena {
    ...
    /// is then a no-op and allocations live for the process lifetime — for
    /// callers that just need an `&Arena`
    /// without paying `mi_heap_new` + `mi_heap_destroy`.
    owns: bool,
    /// Debug-only thread-ownership stamp. Stamped on
...
// multiple threads, but `mi_free` may be called from any thread, and
// same-thread allocation is enforced by `assert_owning_thread()` in debug
// builds. We expose `Send` so an arena can be moved into a worker thread
// (e.g. passed to thread-pool workers); concurrent `&self`
// allocation across threads is the caller's responsibility.
...
impl MimallocArena {
    ...
    ///
    /// Use this where a `default_allocator`-shaped parameter needs an
    /// `&Arena` but
    ...
    /// Debug-only check that the calling
    ...
    /// to mimalloc (which may purge/decommit them). So the policy is
    ...
    ///
    /// (An earlier iteration made this an unconditional `reset()` on the theory
    ...
    /// re-commits and re-zeroes it. Hence the cap-gated retain.)
    ///
    ...
    #[inline]
    pub(crate) fn resize_in_place(&self, ptr: NonNull<u8>, _old_len: usize, new_len: usize) -> bool {
