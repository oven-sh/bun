...
fn mimalloc_free(_: *mut c_void, buf: &mut [u8], alignment: Alignment, _: usize) {
    bun_output::scoped_log!(mimalloc, "mi_free({})", buf.len());
...
impl MimallocAllocator {
    fn aligned_alloc(len: usize, alignment: Alignment) -> *mut u8 {
        bun_output::scoped_log!(mimalloc, "mi_alloc({}, {})", len, alignment.to_byte_units());
...
impl ZAllocator {
    fn aligned_alloc(len: usize, alignment: Alignment) -> *mut u8 {
        bun_output::scoped_log!(mimalloc, "ZAllocator.alignedAlloc: {}\n", len);
