...
//!
//! Every symbol that used to be stubbed here now has a real home
//! inside `bun_jsc` / `bun_runtime` /
...
//! unimplemented anywhere (no Rust body, no C++ body) — those are
...
// PHASE-C: C++ callback — `Bun__panic(msg, len) -> !`
...
// REAL: now provided by bun_jsc (src/jsc/UnsafeStringView.rs).
// UnsafeStringView__freeGlobal
...
// ════════════════════════════════════════════════════════════════════════════
// Genuinely unimplemented — no Rust body, no C++ body. Kept so the
...
pub extern "C" fn JSC__JSValue__parseJSON(
    ...
    unreachable!("JSC__JSValue__parseJSON: not implemented (CPP_DECL with no C++ body)")
}

// Imported by bun_jsc/bun_sys_jsc as extern but no provider in C++ or Rust.
#[unsafe(no_mangle)]
pub extern "C" fn BunString__toErrorInstance(
    this: *const c_void,
    global: *mut JSGlobalObject,
) -> JSValue {
    unreachable!("BunString__toErrorInstance: not implemented (no C++ body)")
}

// Declared `extern` in InspectorLifecycleAgent.cpp:47-48 but never defined in
// C++ nor Rust. The agent's
...
// but `BunGeneratedClasses.cpp` only defines them for classes whose `.classes.ts`
