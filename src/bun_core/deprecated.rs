...
// Plain storage for a buffered reader. The in-tree consumers
// (`pack_command::BufferedFileReader`, `bun_sys::stdio::BufferedStdin`) supply
// their own read shims over `bun_sys::read`, so this stays a bare struct: no
// reader trait, no methods.
