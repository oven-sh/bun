...
// Where `bun:internal-for-testing` resolves: a build with `--bytecode-order`, and a compiled executable that writes an
// order file (`%p` as in its path), write what they call each function here. The tests compare the two.
