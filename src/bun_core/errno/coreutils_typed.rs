...
// `crate::coreutils_error_map` (a `comptime_string_map!` keyed by errno
// *name*); here
...
// the name-keyed table fall back to `UNKNOWN`.
pub static COREUTILS_ERROR_MAP: LazyLock<EnumMap<SystemErrno, &'static str>> =
    LazyLock::new(|| {
        let map = EnumMap::from_fn(|errno: SystemErrno| {
            crate::coreutils_error_map::get_by_name(<&'static str>::from(errno)).unwrap_or(UNKNOWN)
        });
