...
macro_rules! impl_get_errno_libc {
    ($($t:ty),+ $(,)?) => {$(
        impl $crate::errno::GetErrno for $t {
            #[inline]
            fn get_errno(self) -> $crate::errno::E {
...
// `-bun_uv_errno::UV_E*` on Windows / for codes the host OS lacks). Rather
...
// match — e.g. `(CHARSET, $e:tt, $uv:tt) => { -::bun_uv_errno::$uv }` — while
...
pub mod coreutils_typed;

// ──────────────────────────────────────────────────────────────────────────
// posix — mode_t, the errno enum, S_* mode bits, and the C errno accessor
//
// Landed here so the errno
// module stays leaf (T0) and bun_sys (T≥1) imports forward. Windows keeps its
...
pub mod posix {
    ...
    pub type E = crate::errno::SystemErrno;
...
impl crate::output::ErrName for SystemErrno {
    fn name(&self) -> &[u8] {
...
// duplicate exists anywhere. `result.rs` consumes this directly — the enum and
// its consumer now live in the same crate.
// ──────────────────────────────────────────────────────────────────────────

/// Platform errno integer → its typed `SystemErrno`.
...
#[inline]
pub fn system_errno_from_i32(errno: i32) -> Option<SystemErrno> {
    #[cfg(not(windows))]
    {
        if errno <= 0 {
            return None;
        }
        SystemErrno::init(i64::from(errno))
    }
    #[cfg(windows)]
    {
        ...
        u16::try_from(n).ok().and_then(SystemErrno::from_repr)
    }
}

/// Platform errno integer → its `SystemErrno` tag name.
/// Same contract as [`system_errno_from_i32`], projected onto the strum table.
#[inline]
pub(crate) fn system_errno_name(errno: i32) -> Option<&'static str> {
    system_errno_from_i32(errno).map(<&'static str>::from)
}

/// Length of the dense `0..MAX` prefix of `SystemErrno` (on Windows, the
/// dense head before the sparse UV_* range). Exposed so `result.rs` can pre-seed
/// its interned `ERRNO_MAP` without a second hand-written per-OS length table.
#[inline]
...
/// platforms so `result.rs` can call it unconditionally; always `None` off
/// Windows.
#[inline]
...
mod errno_name_tests {
    use super::*;
    use crate::{Error, coreutils_error_map};
