...
// `uv::UV_E*` constants come from `bun_uv_errno` (leaf);
...
// lives in `windows::uv_err_to_e` below.)
//
// Entry shape:
...
/// Re-export so `bun_core::errno::S::IFDIR` compiles cross-platform.
pub use crate::S;
...
impl SystemErrno {
    ...
    /// Routes through `crate::Error::from_errno`, which interns tag names
    /// from the per-OS `SystemErrno` strum table.
    #[inline]
    pub fn to_error(self) -> crate::Error {
        crate::Error::from_errno(self as u16 as i32)
    }
    ...
    fn init_numeric(code: u16) -> Option<SystemErrno> {
        ...
        if cfg!(debug_assertions) {
            crate::debug_warn!("Unknown error code: {}\n", code);
        }
        None
    }
...
/// Thin typed adapter over the canonical row table in [`windows::uv_err_to_e`].
pub fn translate_uv_error_to_e(code: c_int) -> E {
    windows::uv_err_to_e(code)
        .or_else(|| {
...
// Thin adapter over the canonical row table in `windows::uv_err_to_e`.
#[inline]
fn uv_code_to_system_errno(mag: u16) -> Option<SystemErrno> {
    let d = windows::uv_err_to_e(-c_int::from(mag))? as u16;
...
pub mod windows {
    ...
    pub trait Win32ErrorExt: Copy {
        ...
        /// `Ok(())` for SUCCESS, otherwise the interned `bun_core::Error` for the
        /// mapped errno (EUNKNOWN when unmapped).
        fn to_result(self) -> core::result::Result<(), crate::Error>;
    }
    ...
    /// Negative `UV_E*` code -> typed `E`. Replaces the integer-discriminant
    /// table formerly inlined in `bun_libuv_sys::uv_err_to_e_discriminant`.
    pub const fn uv_err_to_e(code: core::ffi::c_int) -> Option<super::E> {
        use bun_uv_errno as uv;
        Some(match code {
            uv::UV_EPERM => E::PERM,
            uv::UV_ENOENT => E::NOENT,
            uv::UV_ESRCH => E::SRCH,
            uv::UV_EINTR => E::INTR,
            uv::UV_EIO => E::IO,
            uv::UV_ENXIO => E::NXIO,
            uv::UV_E2BIG => E::_2BIG,
            uv::UV_ENOEXEC => E::NOEXEC,
            uv::UV_EBADF => E::BADF,
            uv::UV_EAGAIN => E::AGAIN,
            uv::UV_ENOMEM => E::NOMEM,
            uv::UV_EACCES => E::ACCES,
            uv::UV_EFAULT => E::FAULT,
            uv::UV_EBUSY => E::BUSY,
            uv::UV_EEXIST => E::EXIST,
            uv::UV_EXDEV => E::XDEV,
            uv::UV_ENODEV => E::NODEV,
            uv::UV_ENOTDIR => E::NOTDIR,
            uv::UV_EISDIR => E::ISDIR,
            uv::UV_EINVAL => E::INVAL,
            uv::UV_ENFILE => E::NFILE,
            uv::UV_EMFILE => E::MFILE,
            uv::UV_ENOTTY => E::NOTTY,
            uv::UV_EFTYPE => E::FTYPE,
            uv::UV_ETXTBSY => E::TXTBSY,
            uv::UV_EFBIG => E::FBIG,
            uv::UV_ENOSPC => E::NOSPC,
            uv::UV_ESPIPE => E::SPIPE,
            uv::UV_EROFS => E::ROFS,
            uv::UV_EMLINK => E::MLINK,
            uv::UV_EPIPE => E::PIPE,
            uv::UV_ERANGE => E::RANGE,
            uv::UV_ENAMETOOLONG => E::NAMETOOLONG,
            uv::UV_ENOSYS => E::NOSYS,
            uv::UV_ENOTEMPTY => E::NOTEMPTY,
            uv::UV_ELOOP => E::LOOP,
            uv::UV_EUNATCH => E::UNATCH,
            uv::UV_ENODATA => E::NODATA,
            uv::UV_ENONET => E::NONET,
            uv::UV_EPROTO => E::PROTO,
            uv::UV_EOVERFLOW => E::OVERFLOW,
            uv::UV_EILSEQ => E::ILSEQ,
            uv::UV_ENOTSOCK => E::NOTSOCK,
            uv::UV_EDESTADDRREQ => E::DESTADDRREQ,
            uv::UV_EMSGSIZE => E::MSGSIZE,
            uv::UV_EPROTOTYPE => E::PROTOTYPE,
            uv::UV_ENOPROTOOPT => E::NOPROTOOPT,
            uv::UV_EPROTONOSUPPORT => E::PROTONOSUPPORT,
            uv::UV_ESOCKTNOSUPPORT => E::SOCKTNOSUPPORT,
            uv::UV_ENOTSUP => E::NOTSUP,
            uv::UV_EAFNOSUPPORT => E::AFNOSUPPORT,
            uv::UV_EADDRINUSE => E::ADDRINUSE,
            uv::UV_EADDRNOTAVAIL => E::ADDRNOTAVAIL,
            uv::UV_ENETDOWN => E::NETDOWN,
            uv::UV_ENETUNREACH => E::NETUNREACH,
            uv::UV_ECONNABORTED => E::CONNABORTED,
            uv::UV_ECONNRESET => E::CONNRESET,
            uv::UV_ENOBUFS => E::NOBUFS,
            uv::UV_EISCONN => E::ISCONN,
            uv::UV_ENOTCONN => E::NOTCONN,
            uv::UV_ESHUTDOWN => E::SHUTDOWN,
            uv::UV_ETIMEDOUT => E::TIMEDOUT,
            uv::UV_ECONNREFUSED => E::CONNREFUSED,
            uv::UV_EHOSTDOWN => E::HOSTDOWN,
            uv::UV_EHOSTUNREACH => E::HOSTUNREACH,
            uv::UV_EALREADY => E::ALREADY,
            uv::UV_EREMOTEIO => E::REMOTEIO,
            uv::UV_ECANCELED => E::CANCELED,
            uv::UV_ECHARSET => E::CHARSET,
            uv::UV_EOF => E::EOF,
            uv::UV_UNKNOWN => E::UNKNOWN,
            uv::UV_EAI_ADDRFAMILY => E::UV_EAI_ADDRFAMILY,
            uv::UV_EAI_AGAIN => E::UV_EAI_AGAIN,
            uv::UV_EAI_BADFLAGS => E::UV_EAI_BADFLAGS,
            uv::UV_EAI_BADHINTS => E::UV_EAI_BADHINTS,
            uv::UV_EAI_CANCELED => E::UV_EAI_CANCELED,
            uv::UV_EAI_FAIL => E::UV_EAI_FAIL,
            uv::UV_EAI_FAMILY => E::UV_EAI_FAMILY,
            uv::UV_EAI_MEMORY => E::UV_EAI_MEMORY,
            uv::UV_EAI_NODATA => E::UV_EAI_NODATA,
            uv::UV_EAI_NONAME => E::UV_EAI_NONAME,
            uv::UV_EAI_OVERFLOW => E::UV_EAI_OVERFLOW,
            uv::UV_EAI_PROTOCOL => E::UV_EAI_PROTOCOL,
            uv::UV_EAI_SERVICE => E::UV_EAI_SERVICE,
            uv::UV_EAI_SOCKTYPE => E::UV_EAI_SOCKTYPE,
            _ => return None,
        })
    }
    ...
    /// Moved DOWN so `bun_core::errno` owns the only NTSTATUS→`E` mapping (cycle-break).
