...
///
/// Matches the behavior of the npm `ms` package: accepts an optional
/// leading `+`/`-`, a number (integer or decimal), optional whitespace,
/// and an optional unit. When no unit is provided the number is
/// interpreted as milliseconds. Returns `null` for invalid input.
///
...
pub fn parseMs(input: []const u8) ?f64 {
    const ms_per_year: f64 = 365.25 * @as(f64, std.time.ms_per_day);

    var remaining = strings.trim(input, " \t\r\n");
    if (remaining.len == 0) return null;
    // The npm `ms` package caps input at 100 characters.
    if (remaining.len > 100) return null;

    // Split the leading number from the unit.
    var i: usize = 0;
    if (remaining[0] == '+' or remaining[0] == '-') i += 1;
    var saw_digit = false;
    while (i < remaining.len and remaining[i] >= '0' and remaining[i] <= '9') : (i += 1) {
        saw_digit = true;
    }
    ...
    const value = std.fmt.parseFloat(f64, remaining[0..i]) catch return null;

    remaining = strings.trim(remaining[i..], " \t\r\n");

    // Lowercase the unit into a small stack buffer for comparison.
    var unit_buf: [16]u8 = undefined;
    if (remaining.len > unit_buf.len) return null;
    for (remaining, 0..) |c, j| {
        unit_buf[j] = switch (c) {
            'A'...'Z' => c | 0x20,
            'a'...'z' => c,
    ...
    const unit = unit_buf[0..remaining.len];

    const multiplier: f64 = multiplier: {
        if (unit.len == 0) break :multiplier 1;

        if (strings.eqlComptime(unit, "ms") or
            strings.eqlComptime(unit, "msec") or
            strings.eqlComptime(unit, "msecs") or
            strings.eqlComptime(unit, "millisecond") or
            strings.eqlComptime(unit, "milliseconds")) break :multiplier 1;

        if (strings.eqlComptime(unit, "s") or
            strings.eqlComptime(unit, "sec") or
            strings.eqlComptime(unit, "secs") or
            strings.eqlComptime(unit, "second") or
            strings.eqlComptime(unit, "seconds")) break :multiplier std.time.ms_per_s;

        if (strings.eqlComptime(unit, "m") or
            strings.eqlComptime(unit, "min") or
            strings.eqlComptime(unit, "mins") or
            strings.eqlComptime(unit, "minute") or
            strings.eqlComptime(unit, "minutes")) break :multiplier std.time.ms_per_min;

        if (strings.eqlComptime(unit, "h") or
            strings.eqlComptime(unit, "hr") or
            strings.eqlComptime(unit, "hrs") or
            strings.eqlComptime(unit, "hour") or
            strings.eqlComptime(unit, "hours")) break :multiplier std.time.ms_per_hour;

        if (strings.eqlComptime(unit, "d") or
            strings.eqlComptime(unit, "day") or
            strings.eqlComptime(unit, "days")) break :multiplier std.time.ms_per_day;

        if (strings.eqlComptime(unit, "w") or
            strings.eqlComptime(unit, "week") or
            strings.eqlComptime(unit, "weeks")) break :multiplier std.time.ms_per_week;

        if (strings.eqlComptime(unit, "y") or
            strings.eqlComptime(unit, "yr") or
            strings.eqlComptime(unit, "yrs") or
            strings.eqlComptime(unit, "year") or
            strings.eqlComptime(unit, "years")) break :multiplier ms_per_year;
