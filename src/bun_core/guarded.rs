...
use crate::ThreadLock;
use bun_sync::Mutex;

/// A wrapper around a mutex, and a value protected by the mutex.
/// This type uses `bun_sync::Mutex` internally.
///
...
impl<'a, Value> GuardedLock<'a, Value, Mutex> {
    ...
    /// `bun_threading::Condition::wait_guarded` to unlock /
