...
/// OOM-narrowing variant of [`crate::handle_oom`]: only `error.OutOfMemory`
/// aborts; other errors pass through (see the `HandleOom` impls).
///
...
/// explicitly.
pub fn handle_oom_strict<A: HandleOom>(error_union_or_set: A) -> A::Output {
    error_union_or_set.handle_oom()
}

/// Output-type selection for [`handle_oom_strict`]: each impl below is one arm of
