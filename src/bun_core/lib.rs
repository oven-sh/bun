...
/// nested in each reader/compressor struct.
pub mod compress {
...
pub use string::{
    ByteString, STRING_ALLOCATION_LIMIT, UnsafeStringViewGithubActionFormatter,
    cheap_prefix_normalizer, escape_reg_exp, identifier, lexer, lexer_tables, parse_double,
    printer, quote_for_json, string_joiner, unsafe_string_view, write,
};
pub use string::{
    HashedString, MutableString, NodeEncoding, OwnedString, OwnedStringCell, PathString,
    SliceWithUnderlyingString, SmolStr, String, StringBuilder, UTF8Slice, UnsafeStringView,
...
/// Bit-cast between fn-pointer types. Used when erasing only the *pointee
/// type* of one or more thin-pointer
...
pub mod os {
    ...
    // hands out a shared `&[_]`) never aliases a live `&mut`. A plain slice
    // global has no exclusivity guarantee; mirroring it with `&'static mut`
    // would be UB the moment a reader borrows while a writer holds the
    // swapped-out `&mut`.
...
pub mod path_sep {
    ...
    /// Cross-platform separator predicate — `/` **or** `\` on every target.
...
// ─── libm shims ───────────────────────────────────────────────────────────────
// Canonical extern for libm's `powf`/`pow`. Hot CSS color-space conversion
// paths (gam_srgb, lab, prophoto) call the safe wrapper below; keep
// `#[inline]` so cross-crate use stays a direct libm call.
unsafe extern "C" {
...
pub mod vec {
    ...
    /// or length bump occurs in the hot loop. Replaces the open-coded
    ...
    ///
    /// Replaces the open-coded `reserve(n); set_len(len+n); &mut v[len..]`
    ...
    /// shifting a consumed prefix down after a partial write/parse, previously
    /// open-coded as a forward memmove + length decrement at every site.
    #[inline]
    ...
    // ── Spare-capacity-as-`&mut [u8]` family ──────────────────────────────
    // The codebase had grown 11 hand-rolled
    // `spare_capacity_mut().as_mut_ptr().cast::<u8>()` + `set_len(len+n)`
    // copies because `spare_capacity_mut` returns `MaybeUninit<u8>` and every
    // C-ABI fill site (read/recv/pread, simdutf, zlib, zstd, libdeflate,
    // base64) needs `*mut u8` / `&mut [u8]`.
    ...
    /// View the **entire** allocation `v[0..capacity]` as `&mut [u8]`. For the
    /// spare-only `[len..capacity]` view use [`spare_bytes_mut`].
    ///
    /// # Safety
...
/// Build-time configuration constants. Real values are emitted by
/// `build.rs` via `env!()` in Phase C (link). Placeholder values let env.rs
/// const-evaluate cleanly.
pub mod build_options {
    /// `option_env!` with a fallback literal.
    ...
    /// `true` for the `release-assertions` profile.
    ...
    /// Repo root. The build system always provides this normalized with
    /// native separators — there is *no* fallback in the spec.
    ...
    /// match — so require the env var there.
    ...
    /// Hex-string hash of the fallback HTML bundle, injected by the build
    /// system. Placeholder until Phase C wires the real value via `env!()`
    /// in `build.rs`.
...
//
// Parent-from-field recovery. Intrusive data structures (task
...
/// [`OFFSET`](IntrusiveField::OFFSET). This is the single canonical
/// container-of helper: every per-module `const X_OFFSET: usize`
...
pub unsafe trait IntrusiveField<F>: Sized {
    ...
    /// Recover `*mut Self` from a pointer to its embedded `F`. Thin wrapper
    /// over [`container_of`].
    ///
    /// # Safety
...
/// `bun.JSError` — the canonical JS error union (`{Thrown, OutOfMemory, Terminated}`).
/// Tier-0 so every layer of the runtime can name it directly; `bun_jsc` re-exports
...
impl From<crate::Error> for JsError {
    fn from(_: crate::Error) -> Self {
        // PORT NOTE: an arbitrary `Error` is coerced into the JS error union by
...
impl From<JsError> for crate::Error {
    /// Widen a `bun.JSError` value back into the `Error` newtype. Preserves
    ...
    fn from(e: JsError) -> Self {
        match e {
            ...
            // `Terminated` (worker shutdown) has no
            // distinct `error.` tag, so collapse into `JSError` like every
...
/// Write `parts` consecutively into `dest` and return the written prefix as a
/// mutable slice. Panics if `sum(parts.len()) > dest.len()`.
#[inline]
...
/// Allocate a fresh `Box<[T]>` holding all `parts` joined. No zero-init:
/// `extend_from_slice` is `memcpy`-specialized for `T: Copy`, so no `Default`
/// bound is required.
#[inline]
...
/// that panics at runtime if the wrong variant is active. The codebase
/// hand-wrote ~20 identical
/// `match self { Self::V(x) => x, _ => unreachable!() }` accessors across
/// `jsc` / `bundler` / `ini` / `resolver` / `ast` / `install`; this macro is
/// the canonical replacement. Invoke it *inside* an `impl Enum { ... }` block:
///
/// ```ignore
/// impl Data {
...
/// Unwrap a `Result`, calling `outOfMemory()` on `Err`. The full multi-arm
/// version (which narrows mixed error sets) lives in
...
/// `Result<T, E>`. This is the *loose* idiom that panics on **any** `Err`, not
/// just OOM-only error sets. For the narrowing version that whitelists the
/// error set, see `bun_crash_handler::HandleOom`.
///
...
/// Capture an error-return trace for crash reporting. Rust panics already
/// carry a backtrace, so this tier-0 shim is a no-op that keeps call-site
/// shape; the real reporter lives above in
...
/// Intentional *runtime* "feature not yet implemented" path that the runtime
/// ships with — distinct from a porting placeholder. Captures file/line via
/// `file!()`/`line!()` and routes through `Output::panic`.
...
// `err!(Name)` / `err!("Name")` — produce the named `Error` literal.
//
...
/// CamelCase alias for the [`feature_flags`] module.
...
/// `bun.Timer` — minimal monotonic stopwatch with `{start,read}` methods so
/// callers (e.g. `Lockfile::clean_with_logger`, `LifecycleScriptSubprocess`)
/// compile against the tier-0 surface without pulling in `bun_perf`.
pub mod time {
...
/// `bun.schema` namespace stub. The full generated API
...
pub(crate) mod strings_impl {
    ...
    // Re-exported as `strings.u16_lead`/`strings.u16_trail`. Defined here in
    ...
    /// Byte length of `input` after replacing every occurrence of `needle`
    /// with `replacement`. Empty `needle` ⇒ `input.len()` (lenient; matches
    /// every existing caller's expectation).
    ...
    /// Naive O(n·m) windowed case-insensitive ASCII substring search.
    ...
    pub fn is_windows_absolute_path_missing_drive_letter<T: crate::strings::PathByte>(
        ...
        // Spec asserts non-empty + windows-absolute; release-mode callers may
        ...
        // Equivalent to `windows_filesystem_root(chars).len == 1`. With
    ...
    /// Slice equality where the caller has already checked `a.len() ==
    /// b.len()` (the "ignore len" means "don't re-check"). PERF(port): a
    /// length-specialized SWAR variant could be generated; this scalar
    ...
    /// loop. The per-crate duplication of this helper was an accident, not a
    /// design choice. Runtime callers should prefer plain `==` (lowers to
    /// `memcmp`).
    #[inline]
    ...
    // ──────────────────────────────────────────────────────────────────────
    // Transcoding helpers. Lives in T0 so collections::Vec<u8> can call it
    // without depending on bun_string. Allocator params dropped per
    ...
    /// PERF(port): a hand-rolled SWAR/vector ASCII-span scanner is unnecessary; we use
    ...
    /// Port of `copyLatin1IntoUTF8StopOnNonASCII`. Finding each non-ASCII byte
    /// is exactly [`first_non_ascii`] (simdutf `validate_ascii_with_errors`,
    /// with a ≤32B scalar fast path), so the body reduces to "scan the next
    /// ASCII span, memcpy it, encode one high byte, repeat". Speculative
    /// 8-byte over-write is dropped (callers only read `buf[..written]`).
    ...
    /// `.len()` excludes the trailing NUL sentinel.
    ...
    /// + `copyU16IntoU8`, replacing the open-coded per-site combination.
    #[inline]
    ...
    // `strings::has_prefix_t(a, b)` stay 1:1 with the spec name. Rust already
    // lowers slice `==` on integer T to memcmp, so the
    // `eql_long`/`reinterpret_to_u8` perf path from immutable.rs is
    // unnecessary.
    #[inline]
    ...
    /// Replaces open-coded `or`-chains over `eql_case_insensitive_ascii` at
    /// every call site. Haystacks are 6-12 const literals; `#[inline]` lets
    /// LLVM unroll back to the original short-circuit chain. For key→value
    /// dispatch use `in_map_case_insensitive`.
    #[inline]
    ...
    // ──────────────────────────────────────────────────────────────────────
    // IP-literal predicates. The spec calls
    ...
    /// `bun.strings.isIPAddress` — `ares_inet_pton(AF_INET || AF_INET6) > 0`.
    ...
    /// `bun.strings.isIPV6Address` — `ares_inet_pton(AF_INET6, …) > 0`.
    ...
    pub fn starts_with_npm_secret(s: &[u8]) -> usize {
        // `bun.strings.startsWithNpmSecret`: case-insensitive
    ...
    pub fn find_url_password(s: &[u8]) -> Option<(usize, usize)> {
        // Case-sensitive prefix match, then truncate the search
        // region at the first '\n' before scanning for '@'/':'.
    ...
    /// `bun.strings.utf8ByteSequenceLength`.
    ...
    /// `bun.strings.wtf8ByteSequenceLength`.
    ...
    /// `bun.strings.wtf8ByteSequenceLengthWithInvalid` — alias of
    ...
    /// the written sub-slice. Infallible. The caller is responsible for sizing
    /// `out` for the worst case (≤ 3× input code units).
    ///
    /// PORT NOTE: the spec passes `out` straight to simdutf with no bounds check
    ...
    pub fn remove_leading_dot_slash(slice: &[u8]) -> &[u8] {
        if slice.len() >= 2 {
            // PERF(port): direct 2-byte slice comparison compiles to a single
            // u16 compare.
...
pub mod strings {
    ...
    // `index_of_any{,_t}` keep the scalar `Option<usize>` form (the
    // `immutable` form returns `Option<OptionalUsize>` which no caller wants here).
...
/// `bun.linuxKernelVersion()` → `analytics.GeneratePlatform.kernelVersion()`.
...
/// `bun.assertWithLocation` — `bun.assert` plus
/// the caller's source location for the failure message. Panics under
/// `debug_assertions` and is a no-op otherwise (matching `bun.assert`'s
/// release-safe behaviour).
#[track_caller]
#[inline]
...
/// user body directly; an internal panic still routes through `bun.crash_handler`.
pub mod ffi {
    ...
    /// and callers always assume a valid sentinel pointer, so a null here is
    /// always a caller bug.
    ///
    /// # Safety
...
pub mod asan {
    //! Low-tier ASAN/LSAN runtime hooks. `bun_safety` depends on
...
// ────────────────────────────────────────────────────────────────────────────
// PHASE-C: glibc-compat / link wraps for missing symbols.
...
/// Windows: `RtlCaptureStackBackTrace` (kernel32/ntdll). No DbgHelp dependency
...
/// Return-address placeholder. Rust has no stable equivalent; `0` tells
...
/// `SourceLocation`/`SymbolInfo` — pure data structs shared by
/// crash_handler's stub `debug` mod and btjs's symbolication module. Neither
/// of those crates can depend on the other, so the canonical home is here
/// (alongside `capture_stack_trace`/`return_address`) pending a dedicated
/// `bun_debug` crate.
pub mod debug {
    /// Source-file location for a symbolized frame.
    #[derive(Clone)]
    ...
    /// Symbol metadata for a stack frame.
