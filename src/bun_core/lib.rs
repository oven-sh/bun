...
pub(crate) mod strings_impl {
    ...
    /// `&[u8]` is UB unless the pointer is 2-byte-aligned.
    fn utf16_units_from_bytes(bytes: &[u8]) -> Vec<u16> {
        let n_u16 = bytes.len() / 2;
        let mut aligned: Vec<u16> = Vec::with_capacity(n_u16);
        // SAFETY: the fresh allocation is aligned with `n_u16 * 2` writable
        // bytes; copy exactly that many bytes before exposing initialized units.
        unsafe {
    ...
    /// Transcode raw UTF-16-LE bytes (no alignment requirement) to UTF-8.
    pub fn to_utf8_alloc_from_le_bytes(le_bytes: &[u8]) -> Vec<u8> {
        to_utf8_alloc(&utf16_units_from_bytes(le_bytes))
    }

    /// Transcode raw UTF-16-BE bytes (no alignment requirement) to UTF-8.
