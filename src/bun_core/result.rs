// ─── bun_core::Error — global named error set ─────────────────────────────
//
// `Error` models a process-wide error set: every distinct `error.Foo` name in
// the program is assigned a unique non-zero u16, `as_u16()` returns that code,
// `name()` recovers the string, and two `Error`s from different modules
// compare equal because the *name* is the identity.
//
// Names are interned at runtime: a process-wide append-only `&'static str`
// table guarded by an RwLock. The `err!()` macro caches each call-site's code
// in a 2-byte `AtomicU16` slot (zero-init → `.bss`), so the lock is touched
// once per *name-site*, not once per call — `e == err!(Foo)` is a u16 compare
// after first use.
//
// Layout is `#[repr(transparent)] NonZeroU16`, so `Option<Error>` is one u16
// and FFI/packed-struct slots that hold an `Error` stay 2 bytes wide.
...
const SEED: &[&str] = &[
    // — well-known error-set members the runtime matches on by value —
...
impl Error {
    ...
    /// string (by value) always yields the same `Error`.
    ///
    ...
    /// Recover the interned tag name. Never allocates; the table only stores `'static`.
    ...
    /// Return the interned u16 code.
    #[inline]
    pub const fn as_u16(self) -> u16 {
        self.0.get()
    }

    /// Reconstruct an `Error` from a previously-interned u16 code. `0` (the
    /// reserved "no error" value) maps to `Unexpected` rather than panicking,
    /// since callers feed untrusted ints.
    #[inline]
    ...
    /// Map a raw OS errno to its named error. Unknown errnos collapse to
    /// `Unexpected`.
    pub fn from_errno(errno: i32) -> Self {
        // Build a dense `errno_map: [max+1]Error` once at first use by
        // interning every platform `SystemErrno` tag name. After init, lookup
        // is a plain bounds-checked array index.
        static ERRNO_MAP: crate::Once<Box<[Error]>> = crate::Once::new();
        let map = ERRNO_MAP.get_or_init(|| {
            // Index 0 ("SUCCESS") is the no-error hole → default to Unexpected.
        ...
        // Windows libuv errnos are negative; normalise to a non-negative index.
        ...
        // `from_errno(-4058)` → `error.UV_ENOENT`. The dense map above only
        // covers the contiguous low errnos.
        #[cfg(windows)]
...
impl From<std::io::Error> for Error {
    fn from(e: std::io::Error) -> Self {
        match e.raw_os_error() {
            ...
            // alias garbage (5→EIO, 32→EPIPE). The expected pipeline first
            // converts via `Win32Error::toSystemErrno()` before any
...
/// Writer-style I/O failures surface as `error.WriteFailed`. Formatted output
/// here routes through `core::fmt::Write`, whose only error value is the unit
/// `fmt::Error`; map it to the same tag so `?`-propagation matches the
/// expected error name.
...
/// [`From<std::io::Error>`] → errno/`Unexpected`, which diverges from the
/// `error.WriteFailed` contract. Replaces the open-coded
...
/// exactly the variant tag. Replaces the hand-rolled 5-line
/// `f.write_str(<&'static str>::from(self))` boilerplate.
///
...
// ─── coreutils_error_map ─────────────────────────────────────────────────
// The full `EnumMap<SystemErrno, &str>` lives in
// `bun_sys::coreutils_error_map`; that crate is tier-above `bun_core`, so for
// `output.rs`'s integer-errno hot path we keep a parallel table here, keyed by
// `SystemErrno` *name* and resolved through the per-OS `ErrnoNames` hook —
// i.e. the same `errno → SystemErrno → message` composition, just without the
// cross-crate enum.
//
...
/// Two-variant ok/err result used at FFI boundaries that pass a tagged union by value.
pub enum Result<T, E> {
    Ok(T),
    Err(E),
}
