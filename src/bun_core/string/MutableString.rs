...
/// Layout-identical to POSIX `iovec_const`
/// (`{ base: *const u8, len: usize }`), which we define unconditionally for
/// every target — it does NOT alias `uv_buf_t`/`WSABUF` on Windows (those
/// have reversed field order and a `u32` len). `MutableString::to_socket_buffers`
/// returns this shape on all platforms, so there is no `cfg(windows)` split.
#[repr(C)]
#[derive(Clone, Copy)]
...
/// A growable byte buffer. The global mimalloc allocator is implicit, so this
...
// The `bun_collections::pool::ObjectPoolType` impl lives in `bun_collections`
// (trait owner) to avoid a `bun_core → bun_collections` dep cycle now that
// `MutableString` is in `bun_core`.
...
impl MutableString {
    ...
    /// Set `len = capacity` so callers can index into the spare region (e.g.
    /// `read()` into `&mut list[n..]`).
    ///
    /// The new tail must be treated as write-only — callers must not read
    /// `list[old_len..]` until they have overwritten it (typically via
    /// `read()`). An earlier port zero-filled here, which memset the entire
    /// pooled scratch buffer before every `package.json` read.
    #[inline]
    ...
    pub fn owns(&self, items: &[u8]) -> bool {
        // bun.isSliceInBuffer(items, &list[0..capacity]).
    ...
    pub fn writable_n_bytes_assume_capacity(&mut self, amount: usize) -> &mut [u8] {
        // SAFETY: caller fully writes the returned slice (caller contract).
    ...
    pub fn buffered_writer(&mut self) -> BufferedWriter<'_> {
        BufferedWriter {
            context: self,
            buffer: [0u8; BufferedWriter::MAX], // PERF(port): could be left uninitialized
            pos: 0,
        }
    }
    ...
    pub fn ensure_unused_capacity(&mut self, amount: usize) -> Result<(), AllocError> {
        // ensureUnusedCapacity is an alias for growIfNeeded.
        self.grow_if_needed(amount)
    }
    ...
    pub fn ensure_valid_identifier(str: &[u8]) -> Result<Box<[u8]>, AllocError> {
        // TODO(port): the result could be either the input borrow or a fresh
        // allocation. Rust cannot express that without a lifetime + Cow; for
        // now we always return owned `Box<[u8]>` and copy on the borrow paths.
        // Phase B: consider `Cow<'a, [u8]>`.
        if str.is_empty() {
    ...
    pub fn copy(&mut self, str: impl AsRef<[u8]>) -> Result<(), AllocError> {
        ...
        if self.list.is_empty() {
            ...
            // Replace `list[0..str.len]` with `str`.
            // TODO(port): verify this matches ArrayList.replaceRange semantics
    ...
    pub fn append_slice_exact(&mut self, items: &[u8]) -> Result<(), AllocError> {
        ...
        // ensureTotalCapacityPrecise(len + items.len) → reserve_exact(items.len())
    ...
    pub fn reset_to(&mut self, index: usize) {
        ...
        // uninitialized. Callers must have previously written those bytes
        // (e.g. via writable_n_bytes).
        unsafe { self.list.set_len(index) };
    }

    pub fn inflate(&mut self, amount: usize) -> Result<(), AllocError> {
        // `list.resize(amount)` leaves new bytes uninitialized — callers
        // always overwrite the inflated region (it's a printer buffer pre-size).
...
/// Growable string sink — backs `MutableString::writer()`.
...
impl MutableString {
    ...
    pub fn to_dynamic_owned(&mut self) -> Box<[u8]> {
        // TODO(port): `DynamicOwned([]u8)` carried its allocator; with the
        // global allocator this collapses to `Box<[u8]>`. Revisit if a distinct
        // `bun_ptr::DynamicOwned` type is introduced.
        self.to_owned_slice()
    }

    /// Original asserted that the allocator was `bun.default_allocator`; the
    /// allocator field is gone, so this is just `to_owned_slice`.
    ...
    pub fn index_of(&self, str: u8) -> Option<usize> {
        // TODO(port): the original signature took `str: u8` but the body
        // called a slice-search routine — looks like a latent bug in the
        // pre-port source. Implemented here as a single-byte search.
    ...
    /// Returns an array of `SocketBuffer` (POSIX `iovec_const` layout) — that
    /// struct is `{ base: *const u8, len: usize }` on every target (including
    /// Windows; it is NOT `uv_buf_t`). Single implementation, no
    /// `cfg(windows)` split.
    pub fn to_socket_buffers<const COUNT: usize>(
        &self,
        ranges: [(usize, usize); COUNT],
    ) -> [SocketBuffer; COUNT] {
        // PERF(port): could be an unrolled loop; plain loop here.
...
// `MutableString::init2048` is the default `Init` fn passed to the
// `ObjectPoolType` impl (uses init2048() per the npm registry body pool spec).
...
impl<'a> BufferedWriter<'a> {
    ...
    pub fn write_all16(&mut self, bytes: &[u16]) -> Result<usize, AllocError> {
        ...
        if pending.len() >= Self::MAX {
            self.flush()?;
            // PORT NOTE: an earlier implementation wrote into
            // `self.remain()[0..bytes.len*2]` here, which after `flush()` is
            // `self.buffer[0..bytes.len*2]` — but `bytes.len*2 > MAX`, so that
            // indexes past the stack buffer. That was a latent bug (should
            // write into `context.list`). Implementing the apparent intent:
