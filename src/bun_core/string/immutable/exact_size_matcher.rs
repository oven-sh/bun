...
// Only the supported sizes (1, 2, 4, 8, 12, 16) get `ExactSizeInt` impls;
...
// MAX_BYTES=12 has no native 96-bit integer; back it with `u128` and zero-pad
// the high 4 bytes. `MAX` is the true 96-bit max so the "too long" sentinel
// is distinct from any 12-byte payload.
    ...
    Self: ExactSizeInt<MAX_BYTES>,
{
    ...
    /// Like [`r#match`](Self::r#match), lowercasing ASCII bytes first.
    #[inline]
    pub fn match_lower(str: &[u8]) -> <Self as ExactSizeInt<MAX_BYTES>>::T {
        match str.len() {
            ...
            n if n == MAX_BYTES => {
                // PORT NOTE: the `== max_bytes` arm intentionally does NOT
                // lowercase (preserves upstream behavior; likely a latent bug).
                let arr: &[u8; MAX_BYTES] = str.try_into().expect("len == MAX_BYTES");
                Self::read_le(arr)
            }
            0 => Self::ZERO,
    ...
    /// Runtime variant. For const-position use (e.g. `match` arm patterns),
    /// use the [`exact_case!`] macro below — `const fn` cannot call the
    /// non-const trait method `read_le` on stable.
    #[inline(always)]
    pub fn case(str: &'static [u8]) -> <Self as ExactSizeInt<MAX_BYTES>>::T {
        // if (str.len < max_bytes) { zero-pad } else if (== max_bytes) { read } else { panic }
        assert!(
            str.len() <= MAX_BYTES,
