...
impl Utf8WithString {
    ...
    /// For a holder that may be dropped on any thread (a `Blob` store):
    /// `string` becomes a [`String::thread_isolated_copy`] never handed to JS.
    pub fn thread_isolated_copy(self) -> Self {
        Self {
            string: self.string.thread_isolated_copy(),
            utf8: self.utf8,
        }
    }
}
