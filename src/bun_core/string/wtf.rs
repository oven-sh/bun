...
// `UTF8Slice` lives alongside `UnsafeStringView` in `crate::string`.
use crate::string::UTF8Slice;
...
/// `bun_string` types ([`UTF8Slice`], `crate::ZBox`) or
...
pub trait WTFStringImplExt {
    fn to_latin1_slice(&self) -> UTF8Slice;
    fn to_utf8(&self) -> UTF8Slice;
    fn to_utf8_without_ref(&self) -> UTF8Slice;
    fn to_utf8_borrowed(&self) -> UTF8Slice;
    fn to_owned_slice_z(&self) -> crate::ZBox;
    fn to_utf8_if_needed(&self) -> Option<UTF8Slice>;
...
impl WTFStringImplExt for WTFStringImplStruct {
    ...
    fn to_latin1_slice(&self) -> UTF8Slice {
        ...
        // UTF8Slice::WTF derefs `self` on Drop — explicit ownership
        // replaces the legacy StringImplAllocator vtable trick.
    ...
    fn to_utf8(&self) -> UTF8Slice {
        if self.is_8bit() {
            if let Some(utf8) = strings::to_utf8_from_latin1(self.latin1_slice()) {
                return UTF8Slice::init_owned(utf8);
            }

            return self.to_latin1_slice();
        }

        UTF8Slice::init_owned(strings::to_utf8_alloc(self.utf16_slice()))
    }
    ...
    fn to_utf8_without_ref(&self) -> UTF8Slice {
        if self.is_8bit() {
            ...
            return UTF8Slice::from_utf8_never_free(self.latin1_slice());
        }
    ...
    /// [`UTF8Slice::WtfBorrowed`] view (no `r#ref`/`deref` pair) instead of
    /// the ref-holding [`UTF8Slice::WTF`]. The caller MUST keep this impl
    ...
    fn to_utf8_if_needed(&self) -> Option<UTF8Slice> {
        if self.is_8bit() {
            if let Some(utf8) = strings::to_utf8_from_latin1(self.latin1_slice()) {
                return Some(UTF8Slice::init_owned(utf8));
            }

            return None;
        }

        Some(UTF8Slice::init_owned(strings::to_utf8_alloc(
            self.utf16_slice(),
        )))
    }
    ...
    fn utf8_byte_length(&self) -> usize {
        if self.is_8bit() {
            ...
            if !input.is_empty() {
                // latin1→utf8 length is just elementLengthLatin1IntoUTF8
                // (each high byte becomes 2 utf8 bytes). Equivalent to
                // jsc.WebCore.encoding.byteLengthU8 for the Utf8 target.
...
// PORT NOTE: the legacy `StringImplAllocator` was an allocator vtable trick
// (alloc() bumped ref, free() dropped it) so a slice would deref the
// WTFStringImpl when freed. Replaced by `UTF8Slice::WTF { .. }` explicit
...
// any string/jsc dep. Re-exported here to keep the historical namespace shape.
