...
//!
//! `bun_core` is tier-0 and may not depend on `bun_sys` (cycle). Win32 POD
//! typedefs/structs, kernel32 externs, and the TEB→PEB chain are owned by the
//! tier-0 leaf `bun_windows_sys` (which has zero `bun_*` deps, so no cycle);
//! only the `bun_core`-specific console consts/helpers live here. All
//! declarations are zero-cost FFI (`extern "system"` = `__stdcall`, which on
//! x64 is the same as `extern "C"`).
#![cfg(windows)]
