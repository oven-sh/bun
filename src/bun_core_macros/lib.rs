...
//!
//! `pretty_fmt!(FMT, true|false)` is the compile-time form of
//! `Output.prettyFmt` (`src/bun_core/output.rs`). It rewrites Bun's `<tag>`
...
/// Compile-time `prettyFmt`, plus legacy format-spec rewrites
/// (`{s}`/`{d}` → `{}`, `{any}`/`{?}` → `{:?}`).
///
...
// `RefCount` contract (`src/ptr/ref_count.rs`) requires
...
// ──────────────────────────────────────────────────────────────────────────
//
// Third sibling of CellRefCounted / ThreadSafeRefCounted. Replaces the
// `bun.ptr.RefCount(.., "ref_count", destructor, .{ .debug_name = … })`
// mixin pattern — the form taken by ~17 Rust
...
// ──────────────────────────────────────────────────────────────────────────
//
// Generates the data→discriminant projection a tagged union gets for free.
// Every tagged-union ported to a Rust `enum` lost the implicit
// projection and grew a hand-written
