...
pub struct Chunk {
    ...
    /// `__chunks(...)` registration for a browser entry chunk of a split
    /// bundle (see `module_preload_registration`); built on the bundle thread
    /// before post-processing, which reads sibling chunks, and consumed by
    /// `post_process_js_chunk`.
    pub(crate) preload_registration: Vec<u8>,
