...
pub struct DeferredBatchTask {
    // Debug-only running flag — zero-sized in release.
...
impl DeferredBatchTask {
    ...
    pub fn get_bundle_v2(&mut self) -> &mut BundleV2<'static> {
        ...
        // (raw `*BundleV2` semantics); callers must not outlive the owning bundle.
        unsafe {
    ...
    pub fn schedule(&mut self) {
        ...
        // `getBundleV2().jsLoopForPlugins().enqueueTaskConcurrent(task)`.
        self.get_bundle_v2().enqueue_on_js_loop_for_plugins(task);
    }

    pub fn run_on_js_thread(&mut self) {
        // PORT NOTE: reshaped for borrowck — the deferred `this.deinit()` only resets
        ...
        // its error and resetting the flag afterwards is equivalent on both paths.
        {
            ...
            // `if (bv2.completion) |c| c.result == .err else false`
            let rejected = bv2.completion.map(|c| c.result_is_err()).unwrap_or(false);
            // `bv2.plugins.?.drainDeferred(rejected) catch return;`
