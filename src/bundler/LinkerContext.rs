...
impl<'a> LinkerContext<'a> {
    ...
    pub(crate) fn load(
        ...
        self.options
            .compile_entry_point_name
            .clone_from(&transpiler.options.compile_entry_point_name);
        self.options
            .entry_naming
    ...
    pub(crate) fn generate_js_renamer(
        ...
        if let crate::chunk::Content::Javascript(js) = &chunk.content {
            let (renamer, nested_scopes) =
                rename_symbols_in_chunk(ctx.c, &*chunk, &js.files_in_chunk_order)
...
pub struct LinkerOptions {
    ...
    /// `options.compile_entry_point_name` of the bundle's transpiler.
    pub(crate) compile_entry_point_name: Box<[u8]>,
...
impl LinkerOptions {
    /// The runtime helper that defines a namespace object's exports.
    pub(crate) fn export_runtime_function(&self) -> &'static [u8] {
        if self.deprecated_namespace_object_setters {
            b"__export"
        } else {
            b"__exportGetters"
        }
...
impl<'a> LinkerContext<'a> {
    ...
    pub(crate) fn print_code_for_file_in_chunk_js(
        ...
        let import_member_bindings: &bun_ast::ast_result::ImportMemberBindings =
            &self.graph.import_member_bindings;
...
impl<'a> LinkerContext<'a> {
    ...
    pub(crate) fn match_import_with_export(
        ...
        self.match_import_with_export_inner(pg, init_tracker, None, re_exports)
    }
    ...
    fn match_import_with_export_inner(
        ...
        'loop_: loop {
            ...
            let advanced = self.advance_import_tracker(pg, &tracker, first_hop.take());
    ...
    pub(crate) fn match_imports_with_exports_for_file(
        ...
        for import_ref in refs {
            ...
            if is_call_item && !self.call_record_binds(source_index, import_record_index) {
                continue;
            }
            ...
            match result.kind {
                ...
                MatchImportKind::Cycle => {
                    ...
                    let alias = alias.expect("infallible: alias present").slice();
                    self.log.add_range_error_fmt(
                        Some(source),
                        r,
                        format_args!(
        ...
        self.bind_import_property_accesses(pg, source_index, imports_to_bind, member_resolutions);
