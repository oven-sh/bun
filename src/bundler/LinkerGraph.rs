...
use crate::entry_point::{EntryPointField, EntryPointListExt as _};
use crate::{
...
// `#[derive(MultiArrayElement)]` as `*ListExt` traits with
    ...
    pub fn load(
        ...
        let entry_point_kinds: &mut [entry_point::Kind] = unsafe {
            core::slice::from_raw_parts_mut(
                files_slice.items_raw::<entry_point::Kind>(FileField::entry_point_kind),
                files_len,
            )
        };
        {
        ...
        // Setup entry points
        {
            ...
            let source_indices: &mut [index::Int] = unsafe {
                core::slice::from_raw_parts_mut(
                    ep_slice.items_raw::<index::Int>(EntryPointField::source_index),
                    ep_len,
                )
            };
            let path_strings: &mut [PathString] = unsafe {
                core::slice::from_raw_parts_mut(
                    ep_slice.items_raw::<PathString>(EntryPointField::output_path),
                    ep_len,
                )
            };
            {
                let output_was_auto_generated: &mut [bool] = unsafe {
                    core::slice::from_raw_parts_mut(
                        ep_slice.items_raw::<bool>(
                            EntryPointField::output_path_was_auto_generated,
                        ),
                        ep_len,
                    )
                };
            ...
            if scb.list.len() > 0 {
                ...
                debug_assert_eq!(
                    scb.list.use_directive().len(),
                    scb.list.source_index().len()
                );
                debug_assert_eq!(
                    scb.list.use_directive().len(),
                    scb.list.reference_source_index().len()
                );
                for ((use_, original_id), ref_id) in scb
                    .list
                    .use_directive()
                    .iter()
                    .zip(scb.list.source_index().iter())
                    .zip(scb.list.reference_source_index().iter())
                {
                    match use_ {
                        UseDirective::None => {}
                        UseDirective::Client => {
        ...
        // Setup files
        {
            ...
            let distances: &mut [u32] = unsafe {
                core::slice::from_raw_parts_mut(
                    files_slice.items_raw::<u32>(FileField::distance_from_entry_point),
                    files_len,
                )
            };
