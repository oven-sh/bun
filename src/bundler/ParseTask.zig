...
fn runWithSourceCode(
    ...
    const target = (if (task.source_index.get() == 1) bundler.targetFromHashbang(entry.contents) else null) orelse
...
pub fn runFromThreadPool(this: *ParseTask) void {
    var worker = bundler.ThreadPool.Worker.get(this.ctx);
    defer worker.unget();
...
const ThreadPool = bundler.ThreadPool;
const Expr = js_ast.Expr;
const G = js_ast.G;
const S = js_ast.S;
const HTMLScanner = bundler.HTMLScanner;
const E = js_ast.E;
...
const NodeFallbackModules = JSC.ModuleLoader.node_fallbacks;
