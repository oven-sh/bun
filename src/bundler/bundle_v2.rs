...
pub mod bv2_impl {
    ...
    impl<'a> BundleV2<'a> {
        ...
        fn append_module_federation_runtime_init(
            ...
            code.append_slice(b"const __bunMFImportShared = (specifier) => import(specifier);\nglobalThis.__bunModuleFederationRuntimeInit ??= init;\nconst __bunMFCompatPlugin = {\n  name: \"bun-module-federation-compat\",\n  afterResolve(args) {\n    if (args.remoteInfo?.type === \"script\") args.remoteInfo.type = \"global\";\n    return args;\n  },\n")?;
            code.append_slice(
                b"  async loadEntry({ remoteInfo }) {\n    if (!remoteInfo || remoteInfo.type === \"module\" || remoteInfo.type === \"esm\") return;\n    if (typeof document !== \"undefined\" && document?.createElement) return;\n    const entry = remoteInfo.entry;\n    const response = await fetch(entry);\n    if (!response.ok) throw new Error(`Module Federation remote \"${remoteInfo.name}\" failed to fetch script \"${entry}\": ${response.status} ${response.statusText}`);\n    const source = await response.text();\n    const currentScriptUrlKey = \"__bunModuleFederationCurrentScriptUrl\";\n    const previousScriptUrl = globalThis[currentScriptUrlKey];\n    globalThis[currentScriptUrlKey] = entry;\n    try {\n      (0, eval)(`${source}\\n//# sourceURL=${entry}`);\n      const container = globalThis[remoteInfo.entryGlobalName || remoteInfo.name];\n      if (!container || container.__bunModuleFederationRemoteEntry || typeof container.init !== \"function\") return container;\n      return {\n        ...container,\n        get: typeof container.get === \"function\" ? container.get.bind(container) : container.get,\n        init(shareScope, initScope, initOptions) {\n          const compatShareScope = {};\n          if (shareScope && typeof shareScope === \"object\") {\n            for (const shareName of Object.keys(shareScope)) {\n              const shareValue = shareScope[shareName];\n              if (shareValue && typeof shareValue === \"object\" && !(\"version\" in shareValue)) {\n                const versions = Object.keys(shareValue);\n                const selected = versions.length ? shareValue[versions[0]] : undefined;\n                compatShareScope[shareName] = selected && typeof selected === \"object\" ? { ...shareValue, ...selected } : shareValue;\n              } else {\n                compatShareScope[shareName] = shareValue;\n              }\n            }\n          }\n          return container.init(compatShareScope, initScope, initOptions);\n        },\n      };\n    } finally {\n      if (previousScriptUrl === undefined) delete globalThis[currentScriptUrlKey];\n      else globalThis[currentScriptUrlKey] = previousScriptUrl;\n    }\n  },\n",
            )?;
            code.append_slice(b"};\ninit({\n  name: ")?;
