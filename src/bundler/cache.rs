...
/// `expected_version` in `RuntimeTranspilerCache`.
pub const RUNTIME_TRANSPILER_CACHE_VERSION: u32 = 20;

/// Mirrors the `pub var is_disabled` mutable global — written by T6
/// (CLI argument parsing, `VirtualMachine`) and
...
pub trait RuntimeTranspilerCacheExt {
    /// Mirrors the `pub var is_disabled` namespaced const — kept as an
...
/// Mirrors `RuntimeTranspilerCache.Encoding`.
///
...
/// into a `bun.String` (`RuntimeTranspilerCache`) and only feeds
...
/// Mirrors `RuntimeTranspilerCache.ModuleType`.
///
...
impl Default for RuntimeTranspilerCacheMetadata {
    /// `RuntimeTranspilerCache` defaults
...
impl Set {
    /// `Set::init`. PORT NOTE: `arena` is unused —
    /// `MutableString::init`/`JavaScript::init` source from the global heap in
    /// the Rust port; param kept so callers match the original signature
...
/// (mirrors the `bun.transpiler.cache.Fs.Entry` qualified name).
...
impl Fs {
    ...
    ///
    /// Ownership transfer: historically the field is overwritten WITHOUT freeing
    /// the old buffer, because the suspended parse keeps pointers into it (see ModuleLoader,
    ...
    // TODO(port): the legacy `Fs.deinit` references `c.entries` which is not a field on `Fs` —
    // dead code (lazy compilation never instantiated it). No Drop impl needed beyond
...
impl Fs {
    ...
    ///
    /// `arena` restores the legacy `allocator` param: when
    ...
    /// `None` keeps the global-heap `Contents::Owned(Vec<u8>)` path. The transpiler
    /// passes `if (use_shared_buffer)
    ...
    pub fn read_file_with_allocator(
        ...
        // PORT NOTE: reshaped — the original declared `file_handle = undefined` then assigned on each
        ...
        let contents = match (use_shared_buffer, arena) {
            // `readFileWithHandleAndAllocator(this_parse.allocator, …)` —
...
impl JavaScript {
    ...
    pub fn parse<'a>(
        ...
        let result = match parser.parse() {
            Ok(r) => r,
            Err(err) => {
                // PORT NOTE: `Parser::parse` consumes `self` (the original took `*Parser`
                ...
                // range is lost; fall back to `Range::None` (the original used
                // `parser.lexer.range()`).
    ...
    pub fn scan<'a>(
        ...
        let mut temp_log = bun_ast::Log::init();
        // PORT NOTE: reshaped for borrowck — `defer temp_log.appendToMaybeRecycled(log, source)`;
