...
pub fn generate(
    ...
    if (include_refresh_runtime and refresh_runtime_module_id > 0) {
        ...
        const hex_bytes = std.mem.asBytes(&refresh_runtime_module_id);
    ...
    const hex_bytes = std.mem.asBytes(&etag_u64);
    ...
    javascript_bundle.etag = try std.fmt.allocPrint(allocator, "{any}", .{std.fmt.fmtSliceHexLower(hex_bytes)});
