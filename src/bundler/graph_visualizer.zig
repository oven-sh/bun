...
pub const GraphVisualizer = struct {
    ...
    fn buildGraphData(
                ...
                if (parts.len > 0) {
                    ...
                    for (parts, 0..) |part, j| {
                        ...
                        while (use_iter.next()) |entry| : (use_idx += 1) {
                            symbol_uses[use_idx] = .{
                                .ref = try std.fmt.allocPrint(allocator, "{any}", .{entry.key_ptr.*}),
                        ...
                        for (decl_entries.items(.ref), decl_entries.items(.is_top_level), 0..) |ref, is_top_level, k| {
                            declared_symbols[k] = .{
                                .ref = try std.fmt.allocPrint(allocator, "{any}", .{ref}),
        ...
        for (ast_named_exports, 0..) |exports, source_idx| {
            ...
            while (iter.next()) |entry| {
                ...
                if (export_ref.isValid()) {
                    const src_idx = export_ref.sourceIndex();
                    if (src_idx < ctx.graph.symbols.symbols_for_source.len) {
                        const symbols = ctx.graph.symbols.symbols_for_source.at(src_idx);
                        const inner_idx = export_ref.innerIndex();
                        if (inner_idx < symbols.len) {
                            original_symbol_name = symbols.at(inner_idx).original_name;
                        }
                    }
                }

                try exports_list.append(allocator, .{
        ...
        for (meta_resolved_exports, 0..) |resolved, source_idx| {
            ...
            while (iter.next()) |entry| {
                ...
                try resolved_exports_list.append(allocator, .{
                    .source = @intCast(source_idx),
    ...
    fn buildSymbolChains(
        ...
        // Note: This function uses an arena allocator from the caller, so all allocations
        // are freed when the arena is destroyed. No manual cleanup is needed.
        var chains: std.ArrayList(SymbolChain) = .empty;
        ...
        for (resolved_exports) |resolved| {
            ...
            var chain_links: std.ArrayList(ChainLink) = .empty;

            // Add export link
            const is_reexport = resolved.target_source != null and resolved.target_source.? != resolved.source;
            try chain_links.append(allocator, .{
                .file_index = resolved.source,
                .symbol_name = resolved.export_alias,
                .symbol_ref = resolved.target_ref orelse "unresolved",
                .link_type = if (is_reexport) "re-export" else "export",
            });
            ...
            if (is_reexport) {
                if (resolved.target_ref) |ref| {
                    try chain_links.append(allocator, .{
                        .file_index = resolved.target_source.?,
                        .symbol_name = resolved.export_alias,
                        .symbol_ref = ref,
                        .link_type = "import",
                    });
            ...
            try chains.append(allocator, .{
                ...
                .chain = try chain_links.toOwnedSlice(allocator),
        ...
        return try chains.toOwnedSlice(allocator);
    }
