...
pub const LinkerOptions = struct {
    ...
    output_format: bun.options.Format = .esm,
    ...
    source_maps: bun.options.SourceMapOption = .none,
...
pub const SourceMapData = struct {
    ...
    pub fn computeLineOffsets(this: *LinkerContext, allocator: std.mem.Allocator, source_index: Index.Int) void {
        ...
        const loader: bun.options.Loader = this.parse_graph.input_files.items(.loader)[source_index];
...
inline fn shouldCallRuntimeRequire(format: bun.options.Format) bool {
    return format != .cjs;
}
...
fn postProcessJSChunk(ctx: GenerateChunkCtx, worker: *ThreadPool.Worker, chunk: *Chunk, chunk_index: usize) !void {
    ...
    const sources: []const Logger.Source = c.parse_graph.input_files.items(.source);
    const targets: []const bun.options.Target = c.parse_graph.ast.items(.target);
    for (compile_results) |compile_result| {
        const source_index = compile_result.sourceIndex();
...
pub fn generateChunksInParallel(c: *LinkerContext, chunks: []Chunk, comptime is_dev_server: bool) !if (is_dev_server) void else std.ArrayList(OutputFile) {
    ...
    var output_files = std.ArrayList(OutputFile).initCapacity(
    ...
    if (root_path.len > 0) {
        ...
        for (chunks) |*chunk| {
            ...
            var sourcemap_output_file: ?OutputFile = null;
            ...
            const bytecode_output_file: ?OutputFile = brk: {
            ...
            const output_kind = if (chunk.content == .css)
                ...
                .chunk;
            try output_files.append(OutputFile.init(.{
                .data = .{
                    .buffer = .{
...
fn writeOutputFilesToDisk(
    ...
    root_path: string,
    chunks: []Chunk,
    output_files: *std.ArrayList(OutputFile),
) !void {
    ...
    for (chunks) |*chunk| {
        ...
        var source_map_output_file: ?OutputFile = null;
                ...
                if (symbol.import_item_status == .generated) {
                    ...
                    if (c.resolver.opts.target == .browser and JSC.ModuleLoader.HardcodedModule.Alias.has(next_source.path.pretty, .bun)) {
                        c.log.addRangeWarningFmtWithNote(
                            ...
                            "Bun's bundler defaults to browser builds instead of node or bun builds. If you want to use node or bun builds, you can set the target to \"node\" or \"bun\" in the transpiler bun.options.",
                            .{},
                            r,
                        ) catch unreachable;
                    } else {
...
const JSAst = bun.JSAst.BundledAst;
...
const TopLevelSymbolToParts = bundler.TopLevelSymbolToParts;
...
const PathTemplate = bun.options.PathTemplate;
const Binding = js_ast.Binding;
const CrossChunkImport = bundler.CrossChunkImport;
const default_allocator = bun.default_allocator;
const DataURL = bun.resolver.DataURL;
const lol = bun.LOLHTML;
const StableSymbolCount = bun.renamer.StableSymbolCount;
const B = js_ast.B;
...
const NodeFallbackModules = bun.JSC.ModuleLoader.node_fallbacks;
