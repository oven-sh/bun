...
- Leaves an entry point's own chunk alone when the entry point has exports, with `--compile`, and for a browser entry point without `[hash]` in its name (a page can load it as `entry.js?v=1`, so no chunk may import it)
