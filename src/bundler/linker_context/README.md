...
- Moves the files beside such an entry point's own file into the parent chunk of its class, if there is one: that chunk is evaluated before the entry point's chunk, and a file that the entry point imports first must still run first. Files that import the entry point's file stay with it
