...
### 3. `generateChunksInParallel()` - generateChunksInParallel.rs:1
...
- Moves the files beside such an entry point's own file into the parent chunk of its class, if that chunk runs anything at load: it is evaluated before the entry point's chunk, and a file that the entry point imports first must still run first. Files that lead back to the entry point's file stay with it. The entry point's chunk imports the parent last (`left_entry_chunk`), so these files still run after every other chunk that the entry point loads
