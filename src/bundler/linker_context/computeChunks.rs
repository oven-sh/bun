...
use crate::{BundleV2, Chunk, Index, IndexInt, LinkerContext, UseDirective};
...
pub(crate) fn compute_chunks(
    ...
    // See `find_imported_css_files_in_js_order`: lets a route collect the stylesheets its client components import.
    let client_of_proxy: &[Index] = {
        let boundaries = &parse_graph.server_component_boundaries.list;
        if boundaries.len() == 0 {
            &[]
        } else {
            let map = temp.alloc_slice_fill_copy(this.graph.files.len(), Index::INVALID);
            for ((&proxy, &client), &use_directive) in boundaries
                .items_reference_source_index()
                .iter()
                .zip(boundaries.items_source_index())
                .zip(boundaries.items_use_directive())
            {
                if use_directive == UseDirective::Client {
                    map[proxy as usize] = Index::init(client);
                }
            }
            map
        }
