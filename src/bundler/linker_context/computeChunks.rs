...
use crate::{BundleV2, Chunk, Index, IndexInt, LinkerContext, WrapKind};
...
pub(crate) fn compute_chunks(
    ...
    // Files with at least one live part that prints something. A file with
    // none (nothing survived tree shaking, or only imports and re-exports of
    // unwrapped files did, which print nothing) gets no code-splitting chunk:
    // it would be an empty file, and two such chunks share a content hash. A
    // wrapped file's wrapper part prints the `__esm` / `__commonJS` closure.
    let contributes_code = {
        ...
        let wrapper_part_indices = this.graph.meta.items_wrapper_part_index();
        ...
        for source_index in reachable {
            ...
            let wrapper_part_index = if flags[i].wrap != WrapKind::None {
                wrapper_part_indices[i].get()
            } else {
                Index::INVALID.get()
            };
            if parts[i].as_slice().iter().enumerate().any(|(p, part)| {
                parts_live.is_set(p)
                    && (p as u32 == wrapper_part_index
                        || part
                            .stmts
                            .slice()
                            .iter()
                            .any(|stmt| !this.stmt_prints_nothing(source_index.get(), stmt)))
            }) {
                bits.set(i);
            }
        }
