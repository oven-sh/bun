...
use crate::bun_css;
use crate::bun_fs;
use crate::options::{PathTemplate, Placeholder, PlaceholderField};
...
pub(crate) fn compute_chunks<'a>(
    ...
    if code_splitting {
        ...
        merge_small_chunks(this, parse_graph, temp, min_chunk_size)?;
    }
    ...
    find_all_imported_parts_in_js_order(this, parse_graph, pool, chunks)?;
    ...
    for (chunk_id, chunk) in chunks.iter_mut().enumerate() {
        ...
        chunk.id_key = bun_ast::StoreStr::new(
            parse_graph
                .heap
                .alloc_str(&format!(
                    "{}",
            ...
            && this.chunk_side(chunk) == Side::Server
        {
            ...
            chunk.template = PathTemplate {
                ...
                placeholder: Placeholder {
                    name: this.options.compile_entry_point_name.clone(),
                    ..Default::default()
                },
            };
            continue;
...
use crate::chunk;
use crate::options::{Loader, OutputKind, Side, Target};
