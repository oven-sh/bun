...
use crate::{BundleV2, Chunk, Index, IndexInt, LinkerContext, Ref};
...
fn entry_point_keeps_own_module(this: &LinkerContext, source_index: u32) -> bool {
    ...
    let resolved_exports = &this.graph.meta.items_resolved_exports()[source_index as usize];
    let mut exported: bun_collections::HashMap<Ref, ()> = Default::default();
    for alias in
        this.graph.meta.items_sorted_and_filtered_export_aliases()[source_index as usize].iter()
    {
        if let Some(export) = resolved_exports.get(alias) {
            exported.insert(export.data.import_ref, ());
        }
    }
    ...
    for other in this.graph.reachable_files.iter() {
        ...
        if other == source_index
            || !this.graph.files_live.is_set(other as usize)
            || css_asts[other as usize].is_some()
        {
            continue;
        }
        for import in imports_to_bind[other as usize].values() {
            let ref_ = import.data.import_ref;
    ...
    if code_splitting {
        ...
        for (entry_id, &source_index) in entry_source_indices.iter().enumerate() {
            ...
            if chunk_index == u32::MAX
                || kinds[source_index as usize] != crate::EntryPoint::Kind::UserSpecified
                || own_module_chunk[source_index as usize] != u32::MAX
                || !contributes_code.is_set(source_index as usize)
                || file_entry_bits[source_index as usize].count() == 1
                || !entry_point_keeps_own_module(this, source_index)
            {
                continue;
            }
            own_module_chunk[source_index as usize] = chunk_index;
        }
    }
