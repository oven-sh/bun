...
pub fn compute_chunks(
    ...
    // Backref copy (not `this.parse_graph()`) because the loop below needs
    // disjoint `&mut this.graph.*` borrows while `parse_graph` is held.
    ...
    // immutable borrows into `this.graph` while the BundleV2 back-pointer
    // recovery still wants `*mut LinkerContext`. Split borrows
    ...
    for (entry_id_, &source_index) in entry_source_indices.iter().enumerate() {
        ...
        if css_asts[source_index as usize].is_some() {
            let order = find_imported_files_in_css_order(this, temp, &[Index::init(source_index)]);
    ...
    // SAFETY: `this` is the `linker` field of a live `BundleV2` that owns it and
    // outlives the link step. `from_raw_mut` keeps write provenance for the two
    // `assume_mut` sites below; every other use projects `&BundleV2` via `Deref`.
    let bv2 = unsafe { bun_ptr::ParentRef::from_raw_mut(LinkerContext::bundle_v2_ptr(this_ptr)) };
                    ...
                    && !chunk.flags.contains(chunk::Flags::HAS_HTML_CHUNK)))
        {
            ...
            if has_server_html_imports && bv2.transpiler().options.entry_naming.is_empty() {
                ...
                if chunk
                    ...
                    .contains(chunk::Flags::IS_BROWSER_CHUNK_FROM_SERVER_BUILD)
                {
                    // SAFETY: the borrow ends with this statement; nothing else
                    // reads through `bv2` or `this` while it is live.
                    let bv2_mut = unsafe { bv2.assume_mut() };
