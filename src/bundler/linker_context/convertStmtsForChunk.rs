            ...
            match stmt.data {
                        ...
                        && c.options.output_format.keep_es6_import_export_syntax()
                    {
                            ...
                            .contains(ImportRecordFlags::CALLS_RUNTIME_RE_EXPORT_FN)
                        {
                            ...
                            if let Some(mod_) = module_exports_for_export {
                                // TODO(port): original wrote args[3] which is out-of-bounds (len is 3); preserved as args[2] — verify intent in Phase B
                                args.push(mod_);
                            }
                ...
                bun_ast::StmtData::SFunction(s) => {
                    ...
                    if should_strip_exports && s.func.flags.contains(G::FnFlags::IsExport) {
                        ...
                        stmt = Stmt::alloc(
                            S::Function {
                                // SAFETY: shallow bitwise copy of arena-backed G::Fn (`s.func`).
                ...
                bun_ast::StmtData::SClass(s) => {
                    ...
                    if should_strip_exports && s.is_export {
                        ...
                        stmt = Stmt::alloc(
                            S::Class {
                                // SAFETY: shallow bitwise copy of arena-backed E::Class (`s.class`).
                            ...
                            bun_ast::StmtOrExpr::Stmt(stmt2) => {
                                match stmt2.data {
                                    ...
                                    bun_ast::StmtData::SFunction(s2) => {
                                        ...
                                        stmt = Stmt::alloc(
                                            S::Function {
                                                // SAFETY: shallow bitwise copy of arena-backed G::Fn (`s2.func`).
                                                func: unsafe {
                                    ...
                                    bun_ast::StmtData::SClass(s2) => {
                                        ...
                                        stmt = Stmt::alloc(
                                            S::Class {
                                                // SAFETY: shallow bitwise copy of arena-backed E::Class (`s2.class`).
                                                class: unsafe {
