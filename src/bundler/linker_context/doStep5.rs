...
use crate::perf;
use crate::{Index, LinkerContext, LinkerGraph, RefImportData, ResolvedExports, WrapKind, js_meta};
...
pub(crate) struct Step5Shared<'r, 'a> {
    ...
    entry_point_kinds: &'r [crate::EntryPoint::Kind],
    dyn_ref_aliases: &'r [js_meta::DynamicImportReferencedAliases],
    lifted_setter_params: &'r [Ref],
}
...
impl Step5Shared<'_, '_> {
    ...
    #[inline]
    fn export_runtime_function(&self) -> &'static [u8] {
        self.options.export_runtime_function()
    }
}
...
pub(crate) fn do_step5<'a>(
    ...
    let shared = Step5Shared {
        ...
        entry_point_kinds: files.items_entry_point_kind(),
        dyn_ref_aliases: &*meta.dynamic_import_referenced_aliases,
        lifted_setter_params: &*meta.lifted_setter_param,
    };
...
fn step5_for_file(c: &Step5Shared<'_, '_>, row: &mut Step5Row<'_, '_>) {
    ...
    let referenced_filter: Option<&js_meta::DynamicImportReferencedAliases> =
        match &c.dyn_ref_aliases[id as usize] {
            js_meta::DynamicImportReferencedAliases::Partial(_)
                if c.entry_point_kinds[id as usize] != crate::EntryPoint::Kind::UserSpecified =>
            {
                Some(&c.dyn_ref_aliases[id as usize])
            }
            _ => None,
        };

    {
        ...
        let is_lifted_commonjs = row.ast_flags.contains(AstFlags::COMMONJS_LIFTED_TO_ESM);
        ...
        let is_wrapped_commonjs = is_lifted_commonjs && row.meta_flags.wrap == WrapKind::Cjs;
    ...
    create_exports_for_file(
        ...
        export_aliases,
        re_exports_count,
        c.lifted_setter_params[id as usize],
        row.meta_flags,
        row.ast_flags,
        row.parts,
    );
...
fn create_exports_for_file(
...
) {
    {
        ...
        if !properties.is_empty() {
            ...
            export_ref = c.runtime_function(if is_lifted {
                b"__exportCjs"
            } else {
                c.export_runtime_function()
            });
