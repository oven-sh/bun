...
pub fn findImportedFilesInCSSOrder(this: *LinkerContext, temp_allocator: std.mem.Allocator, entry_points: []const Index) BabyList(Chunk.CssImportOrder) {
                ...
                if (rule.* == .import) {
                    ...
                    if (record.source_index.isValid()) {
                        ...
                        if (rule.import.hasConditions()) {
                            ...
                            var nested_import_records = wrapping_import_records.clone(visitor.allocator) catch |oe| bun.outOfMemory(oe);

                            // Clone these import conditions and append them to the state
                            nested_conditions.push(visitor.allocator, rule.import.conditionsWithImportRecords(visitor.allocator, &nested_import_records)) catch |oe| bun.outOfMemory(oe);
                    ...
                    if (!record.is_internal) {
                        ...
                        var all_import_records = wrapping_import_records.clone(visitor.allocator) catch |oe| bun.outOfMemory(oe);
                        ...
                        if (rule.import.hasConditions()) {
                            all_conditions.push(visitor.allocator, rule.import.conditionsWithImportRecords(visitor.allocator, &all_import_records)) catch |oe| bun.outOfMemory(oe);
                            visitor.order.push(
                                visitor.allocator,
    ...
    var visitor = Visitor{
        ...
        .visited = BabyList(Index).initCapacity(temp_allocator, 16) catch |oe| bun.outOfMemory(oe),
    ...
    var wip_order = BabyList(Chunk.CssImportOrder).initCapacity(temp_allocator, order.len) catch |oe| bun.outOfMemory(oe);
    ...
    if (visitor.has_external_import) {
        ...
        for (order.slice()) |*entry| {
            if ((entry.kind == .layers and is_at_layer_prefix) or entry.kind == .external_path) {
                wip_order.push(temp_allocator, entry.*) catch |oe| bun.outOfMemory(oe);
            }
    ...
    // overrides all previous instances of that declaration.
    {
        ...
        next_backward: while (i != 0) {
            ...
            switch (entry.kind) {
                .source_index => |idx| {
                    const gop = source_index_duplicates.getOrPut(idx.get()) catch |oe| bun.outOfMemory(oe);
                    ...
                    gop.value_ptr.push(temp_allocator, i) catch |oe| bun.outOfMemory(oe);
                },
                .external_path => |p| {
                    const gop = external_path_duplicates.getOrPut(p.text) catch |oe| bun.outOfMemory(oe);
                    if (!gop.found_existing) {
                        gop.value_ptr.* = BabyList(u32){};
                    }
                    for (gop.value_ptr.slice()) |j| {
