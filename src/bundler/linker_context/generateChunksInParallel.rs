...

    {
        ...
        if c.graph.code_splitting {
            if c.options.minify_identifiers {
                ...
                pool.worker_pool()
                    .each_mut(ctx, LinkerContext::finish_js_renamer, chunks);
            }
...
pub(crate) fn finalize_chunks<'a, const IS_DEV_SERVER: bool>(
    ...
    }
    {
        ...
        if !IS_DEV_SERVER || chunks.len() > first {
            ...
            if c.module_preload() {
                for i in first..chunks.len() {
                    if matches!(chunks[i].content, crate::chunk::Content::Javascript(_)) {
                        let registration =
                            crate::linker_context::post_process_js_chunk::module_preload_registration(
                                c, &chunks[i], i, chunks,
                            )?;
                        chunks[i].preload_registration = registration;
                    }
                }
            }
    ...
    // TODO: enforceNoCyclicChunkImports()
    {
        ...
        // paths of each chunk.
        let hashes = c.final_chunk_hashes(pg, chunks)?;
            ...
            if claimed.found_existing {
                ...
                if !dup.found_existing {
                    *dup.value_ptr = DuplicateEntry::default();
                    dup.value_ptr.sources.push(first);
                }
