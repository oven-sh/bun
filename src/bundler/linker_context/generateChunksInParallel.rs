...
pub(crate) fn generate_chunks_in_parallel<const IS_DEV_SERVER: bool>(
    ...
    // TODO: enforceNoCyclicChunkImports()
    {
        ...
        for index in 0..chunks.len() {
            ...
            if entry.found_existing {
                ...
                // same path for both. That collision is benign: emit one file
                // and alias the duplicate to it, matching esbuild.
    ...
    if chunks.iter().any(Chunk::is_duplicate_output) {
        let is_removed =
            |i: usize| i < chunks.len() && chunks[i].is_duplicate_output();
