pub fn generateChunksInParallel(
...

    {
        ...
        if (c.parse_graph.css_file_count > 0) {
            ...
            const tasks = c.allocator.alloc(LinkerContext.PrepareCssAstTask, total_count) catch |oe| bun.outOfMemory(oe);
            var i: usize = 0;
            for (chunks) |*chunk| {
    ...
    {
        const chunk_contexts = c.allocator.alloc(GenerateChunkCtx, chunks.len) catch |oe| bun.outOfMemory(oe);
...

        {
            ...
            for (chunks, chunk_contexts) |*chunk, *chunk_ctx| {
                switch (chunk.content) {
                    .javascript => {
                        ...
                        chunk.compile_results_for_chunk = c.allocator.alloc(CompileResult, chunk.content.javascript.parts_in_chunk_in_order.len) catch |oe| bun.outOfMemory(oe);
                        has_js_chunk = true;
                    },
                    .css => {
                        has_css_chunk = true;
                        ...
                        chunk.compile_results_for_chunk = c.allocator.alloc(CompileResult, chunk.content.css.imports_in_chunk_in_order.len) catch |oe| bun.outOfMemory(oe);
                    },
                    .html => {
                        has_html_chunk = true;
                        ...
                        chunk_ctx.* = .{ .c = c, .chunks = chunks, .chunk = chunk };
                        total_count += 1;
                        chunk.compile_results_for_chunk = c.allocator.alloc(CompileResult, 1) catch |oe| bun.outOfMemory(oe);
                    },
                }
            }
            ...
            const combined_part_ranges = c.allocator.alloc(PendingPartRange, total_count) catch |oe| bun.outOfMemory(oe);
    ...
    // TODO: enforceNoCyclicChunkImports()
    {
        ...
        for (chunks, 0..) |*chunk, index| {
            ...
            const rel_path = std.fmt.allocPrint(c.allocator, "{any}", .{chunk.template}) catch |oe| bun.outOfMemory(oe);
            ...
            if (Environment.isWindows and std.mem.indexOf(u8, rel_path, "/./") != null) {
                ...
                const rel_path_fixed = c.allocator.dupe(u8, bun.path.normalizeBuf(rel_path, &buf, .posix)) catch |oe| bun.outOfMemory(oe);
    ...
    var static_route_visitor = StaticRouteVisitor{ .c = c, .visited = bun.bit_set.AutoBitSet.initEmpty(bun.default_allocator, c.graph.files.len) catch |oe| bun.outOfMemory(oe) };
        ...
        for (chunks, 0..) |*chunk, chunk_index_in_chunks_list| {
            ...
            switch (chunk.content.sourcemap(c.options.source_maps)) {
                .external, .linked => |tag| {
                    ...
                    var source_map_final_rel_path = bun.default_allocator.alloc(u8, chunk.final_rel_path.len + ".map".len) catch |oe| bun.outOfMemory(oe);
                    ...
                    if (loader.isJavaScriptLike()) {
                        ...
                        if (jsc.CachedBytecode.generate(c.options.output_format, code_result.buffer, &source_provider_url)) |result| {
                            ...
                            break :brk options.OutputFile.init(.{
                                .output_path = bun.default_allocator.dupe(u8, source_provider_url_str.slice()) catch |oe| bun.outOfMemory(oe),
                                .input_path = std.fmt.allocPrint(bun.default_allocator, "{s}" ++ bun.bytecode_extension, .{chunk.final_rel_path}) catch |oe| bun.outOfMemory(oe),
                                .input_loader = .js,
