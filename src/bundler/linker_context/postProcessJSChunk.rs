...
pub(crate) fn module_preload_registration(
    c: &LinkerContext,
    chunk: &Chunk,
    chunk_index: usize,
    chunks: &[Chunk],
    ...
    let renamer = chunk.renamer.as_renamer();
    write!(
        &mut code,
        "{}(import.meta.url,[",
...
pub(crate) fn post_process_js_chunk(
    ...
    // Built by `finalize_chunks` before this phase: it reads sibling chunks.
    let mut preload_registration = core::mem::take(&mut chunk.preload_registration);
