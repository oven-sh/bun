...
// multiple chunks — the original implementation has the same overlap; the writes are
...
pub unsafe fn rename_symbols_in_chunk(
    ...
    // PORT NOTE: `symbol::Map` is not `Clone`/`Copy`; the original passed the
    // struct (slice header) by value. Build a non-owning shallow view via
    ...
    let sorted_imports_from_other_chunks: Vec<StableRef> = {
        ...
        // PERF(port): original pre-set len and filled via slice writes; using push() here
    ...
    // PORT NOTE: original used `r.temp_arena` for this list; arena param dropped
