...
/// The nested scopes a chunk still has to name after `rename_symbols_in_chunk`
/// (number renamer only): `(source_index, module-scope child)` grouped by file.
pub(crate) type NestedScopes = Vec<(u32, *const bun_ast::Scope)>;
...
pub(crate) fn rename_symbols_in_chunk(
    ...
    chunk: &Chunk,
    files_in_order: &[u32],
) -> Result<(ChunkRenamer, NestedScopes), crate::Error> {
    ...
    if c.options.minify_identifiers {
        ...
        return Ok((ChunkRenamer::Minify(minify_renamer), Vec::new()));
    }
    ...
    Ok((ChunkRenamer::Number(r), nested_scopes))
}
...
// CONCURRENCY: `each_mut` callback, one task per (chunk, file). Reads the
...
pub(crate) fn run_nested_rename_task(
    ...
    let ast = c.graph.ast.slice();
    let parts = &ast.items_parts()[source_index as usize];
    let scope_uses = &ast.items_scope_uses()[source_index as usize];
