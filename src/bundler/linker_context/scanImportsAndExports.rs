...
pub(crate) fn scan_imports_and_exports<'a>(
...

    {
        ...
        for source_index_ in &reachable {
            ...
            for ni in named_imports[id].values() {
                let Some(record) = import_records[id]
                    .as_slice()
                ...
                if other >= exports_kind.len() || exports_kind[other] != ExportsKind::Esm {
                    continue;
                }
                // The default import of a lifted CommonJS module is its namespace.
                if ni.alias_is_star
                    || (ast_flags[other].contains(AstFlags::COMMONJS_LIFTED_TO_ESM)
                        && ni.alias.is_some_and(|alias| alias.slice() == b"default"))
                {
                    dyn_ref_aliases[other].merge_all();
                } else if let Some(alias) = ni.alias {
                    dyn_ref_aliases[other].insert(alias.slice());
                }
            }
            for &star in ast.export_star_import_records[id].iter() {
                if let Some(record) = import_records[id].as_slice().get(star as usize)
                    && record.source_index.is_valid()
                {
                    dyn_ref_aliases[record.source_index.get() as usize].merge_all();
                }
            }
            ...
            for (import_record_index, record) in import_records[id].as_slice().iter().enumerate() {
                ...
                let is_external_dynamic_import = code_splitting
                    && other_file != id
                    && match record.kind {
                ...
                if other_kind != ExportsKind::Esm {
                    dyn_ref_aliases[other_file].merge_all();
                } else {
                    match record.kind {
                        ...
                        ImportKind::Dynamic | ImportKind::Require => {
                            match dynamic_import_aliases[id].get(&(import_record_index as u32)) {
                                None => dyn_ref_aliases[other_file].merge_all(),
                                ...
                                Some(dynamic_use) => {
                                    ...
                                    dyn_ref_aliases[other_file].insert(
                        ...
                        _ => dyn_ref_aliases[other_file].merge_all(),
                    }
                }
                ...
                match record.kind {
                    ...
                    ImportKind::Dynamic => {
                        if !code_splitting {
                            ...
                            && is_external_dynamic_import
                        {
                            let exports = &named_exports[other_file];
                            let user_entry =
                                entry_point_kinds[other_file] == EntryPoint::Kind::UserSpecified;
                            ...
                            if default_is_module_exports && default_is_read {
                                meta_flags[other_file].needs_synthetic_default_export = true;
                            }
                        }
                    }
                    _ => {}
            ...
            for source_index_ in &reachable {
                ...
                if dependency_wrapper.export_star_records[id].len() > 0 {
                    ...
                    if has_dynamic_exports
                        ...
                        && ast_flags[id].contains(AstFlags::COMMONJS_LIFTED_TO_ESM)
                    {
                        dependency_wrapper.exports_kind[id] = ExportsKind::Cjs;
        ...
        // export stars because imports can bind to export star re-exports.
        {
            ...
            for source_index_ in &reachable {
                ...
                let is_lifted_commonjs = this.graph.ast.items_flags()[source_index]
                    .contains(AstFlags::COMMONJS_LIFTED_TO_ESM);
                if is_lifted_commonjs && flag.wrap == WrapKind::Cjs {
                    ...
                    this.graph.meta.items_lifted_setter_param_mut()[source_index] = param;
                }
    ...
    // generate wrapper parts for wrapped files.
    {
        ...
        this.entry_point_part_indices = vec![u32::MAX; this.graph.ast.len()];
        ...
        for source_index_ in &reachable {
            ...
            if flag.needs_export_symbol_from_runtime {
                let export_symbol_ref =
                    if this.graph.meta.items_lifted_setter_param()[id].is_valid() {
                ...
                for star_index in 0..this.graph.ast.items_export_star_import_records()[id].len() {
                        ...
                        && !(rec_source_index.is_valid() && rec_source_index.get() == source_index)
                    {
                        if rec_source_index.is_valid() {
                            ...
                            if this.graph.ast.items_exports_kind()[other_id] != ExportsKind::Cjs {
                                this.graph.generate_symbol_import_and_use(
                                    source_index,
                                    part_index as u32,
                                    this.graph.ast.items_exports_ref()[other_id],
...
impl ExportStarContext<'_, '_> {
    ...
    fn add_exports(
        ...
        let reexports_default = self.source_index_stack[..stack_end_pos]
            .iter()
            .all(|i| self.ast_flags[*i as usize].contains(AstFlags::COMMONJS_LIFTED_TO_ESM));
