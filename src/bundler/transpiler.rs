...
fn parse_data_loader<'a>(
    ...
    let parts: Box<[bun_ast::Part]> = 'parts: {
        ...
        return export_default_parse_result(expr, source, loader, source_backing, arena);
    };
    let mut ast = bun_ast::Ast::from_parts(parts, arena);
    ast.symbols = bun_alloc::vec_from_iter_in(symbols, arena);

    return Some(ParseResult::with_ast(ast, source, loader, source_backing));
}
...
fn parse_text_loader<'a>(
    ...
    export_default_parse_result(expr, source, loader, source_backing, arena)
}
