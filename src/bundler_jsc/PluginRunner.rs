...
//! the bundler transpiler so `bundler/` is free of `JSValue`/`JSGlobalObject`.

pub use bun_resolver::fs::Path as FsPath;

/// `MacroJSCtx` — re-export of the canonical newtype
...
/// `PluginRunner` — re-export of the concrete struct.
