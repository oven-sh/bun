...
pub const Bunfig = struct {
    ...
    pub const Parser = struct {
        ...
        ctx: Command.Context,
        /// Lazy-loaded. Borrowed from `source.path`.
        ...
        fn loadParentDir(this: *Parser) ![]const u8 {
            ...
            if (!bun.path.isAbsoluteCrossPlatform(source)) {
        ...
        fn loadPreload(
            ...
            if (expr.asArray()) |array_| {
                var array = array_;
                this.ctx.preloads.clearRetainingCapacity();
                try this.ctx.preloads.ensureTotalCapacityPrecise(allocator, array.array.items.len);
                while (array.next()) |item| {
                    try this.expectString(item);
                    if (item.data.e_string.len() == 0) continue;
                    const target = try item.data.e_string.string(allocator);
                    this.ctx.preloads.appendAssumeCapacity(Preload.fromDir(parent_dir, target));
                }
            } else if (expr.data == .e_string) {
                if (expr.data.e_string.len() > 0) {
                    this.ctx.preloads.clearRetainingCapacity();
                    const preload = try expr.data.e_string.string(allocator);
                    errdefer allocator.free(preload);
                    try this.ctx.preloads.append(allocator, Preload.fromDir(parent_dir, preload));
                }
