...
fn get_system_config_path(buf: &mut PathBuffer) -> SystemConfigResult<'_> {
    ...
    // `ALLUSERSPROFILE` is declared `posix = None`; attribute-`#[cfg]` removes
    // the accessor call before type-check on POSIX, like other `posix = None`
    // users (SYSTEMROOT, WINDIR).
    #[cfg(windows)]
    {
...
fn load_bunfig(
    ...
    // Intern `config_path` in the process-lifetime `FilenameStore`:
    // `Bunfig::parse` stores `Location.file` borrows of `source.path.text` in
    // the process-global `ctx.log`, and those diagnostics can print after the
    // caller's `PathBuffer` frame is gone — borrowing the stack buffer would be
    // a stack-use-after-return. The interned slice is genuinely `'static`
    // (never-freed BSS singleton, reachable for LeakSanitizer); the trailing
    // NUL lets it back a `ZStr`.
    ...
    // Only project configs mark loaded_bunfig; run_command/standalone/repl use
    // it to decide whether project bunfig.toml still needs loading.
    if is_project {
...
pub fn load_system_bunfig(cmd: CommandTag, ctx: Context<'_>) -> Result<(), crate::Error> {
    ...
    if let Some(path) = result.path {
        ...
        // Not project-level; explicit paths must fail loudly on a missing file.
...
pub fn load_config(
    ...
    // An explicit BUN_SYSTEM_CONFIG is an administrator override: load it
    // before the standalone DISABLE_AUTOLOAD_BUNFIG check so compiled binaries
    // honor it, while the flag still blocks home/project autoload below.
