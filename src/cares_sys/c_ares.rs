...
/// Address-family constants — only the address families c-ares
...
/// `EAI_*` getaddrinfo error constants. The `libc` crate is missing
...
// Modeled as a non-exhaustive C-int enum. Values are only ever
...
// Each reply type defines a callback signature and a wrapper that
// monomorphizes a unique `extern "C"` thunk per (Type, fn) pair via the
...
pub trait HostentWithTtlsHandler: Sized {
    ...
    /// parser for [`hostent_with_ttls::callback_wrapper`]. Mirrors the original
...
impl hostent_with_ttls {
    ...
    pub unsafe extern "C" fn host_callback_wrapper<T: HostentWithTtlsHandler>(
        ...
        // TODO(port): originally declared as `ares_host_callback` (4th arg
...
// Per-record-type newtype aliases. The original instantiated the resolve machinery over
...
impl AddrInfo {
    ...
    pub fn cnames(&self) -> &[AddrInfo_node] {
        // TODO(port): the original spanned a [*c]AddrInfo_cname (sentinel-
...
/// state callbacks via `Container.onDNSSocketState` + `this.channel = ch`.
///
...
/// TODO(port): the original dispatched per record-type name and
...
// valid bit pattern (zero-initialized) (S021).
...
impl Drop for struct_any_reply {
    fn drop(&mut self) {
        // Field-by-field write, unrolled by hand.
...
// Note: this aliases the full sockaddr_in (not the 4-byte in_addr).
// Preserved for ABI parity in `Options.servers`.
// TODO(port): verify against the c-ares header; this looks like an upstream misnomer.
