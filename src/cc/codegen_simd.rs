//! Code generation for 128-bit vectors and atomics.
//!
...
impl FnGen<'_, '_> {
    fn lane_of(&self, ty: &Type, loc: Loc) -> Res<Lane> {
        match self.tcx.lane_of(ty) {
            Some(lane) => Ok(lane),
            None => internal(loc, "operation on a vector that is not 16 bytes"),
        }
    }
    ...
    /// `base + (index mod lanes) * lane size`, for a lane chosen at run time.
    fn lane_address(&mut self, base: V, offset: i64, index: V, lane: Lane) -> V {
        let mask = self.b.const_i32(i32::from(lane.count()) - 1);
    ...
    pub(super) fn gen_lane_place(&mut self, e: &Expr, base: &Expr, index: &Expr) -> Res<Place> {
        ...
        let constant = match index.kind {
            ExprKind::IntLit(i) => Some((i as u64 % u64::from(lane.count())) as u8),
            _ => None,
        };
        match (place, constant) {
            ...
            (Place::Mem { base, offset }, None) => {
                ...
                Ok(Place::Mem {
                    base: self.lane_address(base, offset, i, lane),
                    offset: 0,
                })
            }
    ...
    fn gen_lane_value(&mut self, e: &Expr, base: &Expr, index: &Expr) -> Res<V> {
        ...
        if let ExprKind::IntLit(i) = index.kind {
            let index = (i as u64 % u64::from(lane.count())) as u8;
        ...
        let address = self.lane_address(scratch, 0, i, lane);
    ...
    pub(super) fn gen_vec_neg(&mut self, ty: &Type, v: V, loc: Loc) -> Res<V> {
        let lane = self.lane_of(ty, loc)?;
        Ok(self.b.vlane(VLaneOp::Neg, lane, false, vec![v]))
    }

    pub(super) fn gen_vec_not(&mut self, v: V) -> V {
        self.b.vbits(VBitsOp::Not, vec![v])
    }

    pub(super) fn gen_vec_splat(&mut self, e: &Expr, scalar: &Expr) -> Res<V> {
        if let Some(bytes) = vector_bytes(e, self.tcx) {
            return Ok(self.const_v128(bytes));
        }
        let lane = self.lane_of(&e.ty, e.loc)?;
        let v = self.gen_value(scalar)?;
        Ok(self.b.vlane(VLaneOp::Splat, lane, false, vec![v]))
    }
    ...
    pub(super) fn gen_vec_init(&mut self, e: &Expr, lanes: &[Expr]) -> Res<V> {
        ...
        let lane = self.lane_of(&e.ty, e.loc)?;
        if lanes.len() != lane.count() as usize {
        ...
        if let Some((first, rest)) = lanes.split_first() {
            if rest.iter().all(|other| same_pure(first, other)) {
                ...
                return Ok(self.b.vlane(VLaneOp::Splat, lane, false, vec![v]));
            }
        }
    ...
    fn gen_operands(&mut self, args: &[Expr]) -> Res<Vec<V>> {
        ...
        Ok(held.into_iter().map(|h| self.release(h)).collect())
    }
    ...
    fn convert_lanes(&mut self, v: V, from: &Type, to: &Type, loc: Loc) -> Res<V> {
        ...
        for i in 0..to_lane.count().min(from_lane.count()) {
    ...
    pub(super) fn gen_vec_builtin(&mut self, e: &Expr, op: VecBuiltin, args: &[Expr]) -> Res<V> {
        ...
        Ok(match op {
            ...
            VecBuiltin::ShuffleDynamic { two_inputs } if lane == Lane::I8x16 => {
            ...
            VecBuiltin::ShuffleDynamic { two_inputs } => {
                ...
                self.b.effect(Inst::Store(MemKind::V128, b, scratch, 16));
                let count = i64::from(lane.count());
            ...
            VecBuiltin::Bitmask => {
                ...
                self.b.vlane(VLaneOp::Bitmask, integer, false, vec![v(0)?])
            }
