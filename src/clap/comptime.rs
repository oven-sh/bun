...
//
// `ComptimeClap(Id, params)` originally was a comptime type-generator: it iterated
...
// when the work is done at compile time.
//
...
//     `&'static ConvertedTable` — the analogue of a comptime
...
/// Compile-time conversion loop: re-indexes each param's `id` to its slot
/// within its category.
...
/// Compile-time name → converted-param index. This is the const-eval form
/// of `findParam` and is intended to be called inside a
...
impl ConvertedTable {
    ...
    /// index, and short-name direct index all land in rodata — full
...
pub struct ComptimeClap<Id> {
    // Inner `&'static [u8]` slices borrow argv (process-lifetime); never freed.
    ...
    // The original captured `converted_params` as a comptime const on the returned type.
    // Here it is carried as a `&'static` table — either rodata (`comptime_table!`) or
...
impl<Id> ComptimeClap<Id> {
        ...
        // TODO(port): narrow error set
    {
        ...
        // The second type arg is the iterator type `I`.
        ...
        while let Some(arg) = stream.next()? {
            ...
            if param.names.long.is_none() && param.names.short.is_none() {
                ...
                if opt.stop_after_positional_at > 0 && pos.len() >= opt.stop_after_positional_at {
                    ...
                    // PORT NOTE: `ArgIter` already yields sized `&[u8]`, so the
                    // original NUL-scan over argv entries is a no-op and is dropped.
        ...
        Ok(Self {
            single_options,
            // PORT NOTE: the original left these uninitialized and filled them post-loop.
    ...
    // The original `deinit` only freed `multi_options[*]` and `pos` (not
    // `passthrough_positionals` — likely a leak). All are owned here, so `Drop` handles it;
    ...
    /// `hasFlag` is a const-eval-only predicate over the captured table.
