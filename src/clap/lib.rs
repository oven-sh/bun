...
pub enum Values {
    ...
    OneNoDashValue,
    /// Like [`Values::OneOptional`], but a separate following argument is consumed
    /// only when it does not start with '-' (Node's `-p`), and in a short cluster
    /// the attached remainder stays part of the cluster (`-pe` is `-p` then `-e`).
