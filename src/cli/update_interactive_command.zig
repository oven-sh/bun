    ...
    fn getOutdatedPackages(
        ...
        for (workspace_pkg_ids) |workspace_pkg_id| {
            ...
            for (pkg_deps.begin()..pkg_deps.end()) |dep_id| {
                ...
                const latest = manifest.findByDistTagWithFilter("latest", manager.options.minimum_release_age, manager.options.minimum_release_age_exclusions).unwrap() orelse continue;
                ...
                const update_version = if (resolved_version.tag == .npm)
                    manifest.findBestVersionWithFilter(resolved_version.value.npm.version, string_buf, manager.options.minimum_release_age, manager.options.minimum_release_age_exclusions).unwrap() orelse latest
                else
                    manifest.findByDistTagWithFilter(resolved_version.value.dist_tag.tag.slice(string_buf), manager.options.minimum_release_age, manager.options.minimum_release_age_exclusions).unwrap() orelse latest;
