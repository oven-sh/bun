...
export class TypeImpl<K extends TypeKind = TypeKind> {
  ...
  isVirtualArgument() {
    return this.kind === "globalObject" || this.kind === "rustVirtualMachine";
  }
  ...
  cppName() {
    ...
    const cAbiType = this.canDirectlyMapToCAbi();
    const namespace = typeHashToNamespace.get(this.hash());
    if (cAbiType && typeof cAbiType === "string" && this.kind !== "rustEnum" && this.kind !== "stringEnum") {
 ...
 *
 * For simplicity, the possibility of any Rust binding returning an error/calling
...
export function registerFunction(opts: FuncOptions) {
  ...
  const func: Func = {
    [isFunc]: true,
    name: "",
    rustPrefix: opts.implNamespace ? `${opts.implNamespace}.` : "",
    snapshot,
    rustFile,
    variants,
  };
...
export class Struct {
  ...
  emitRust(rust: CodeWriter, semi: "with-semi" | "no-semi") {
    rust.line("extern struct {");
    rust.indent();
    for (const field of this.fields) {
      rust.line(`${snake(field.name)}: ${field.type},`);
    }
    rust.dedent();
    rust.line("}" + (semi === "with-semi" ? ";" : ""));
  }
