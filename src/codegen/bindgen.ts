...
/** Rust mirrors of `t.stringEnum` types, accumulated by emitCppEnumHeader. */
const rustStringEnums: { ns: string; name: string; values: string[]; bits: number }[] = [];
const rustStringEnumsSeen = new Set<string>();
...
function emitCppEnumHeader(w: CodeWriter, type: TypeImpl) {
  ...
  const ns = typeHashToNamespace.get(type.hash()) ?? type.ownerFile;
  if (ns && !rustStringEnumsSeen.has(`${ns}::${type.name()}`)) {
    rustStringEnumsSeen.add(`${ns}::${type.name()}`);
    rustStringEnums.push({ ns, name: type.name(), values: [...type.data], bits: intBits });
  }
...
{
  let out = "// Generated by bindgen.ts — Rust mirrors of `t.stringEnum` types.\n";
  out += "// Discriminants match the generated C++ `enum class` (values are sorted\n";
  out += "// alphabetically at construction in bindgen-lib.ts).\n";
  const byNamespace = new Map<string, { name: string; values: string[]; bits: number }[]>();
  for (const e of rustStringEnums.sort((a, b) => (a.ns + a.name).localeCompare(b.ns + b.name))) {
    let list = byNamespace.get(e.ns);
    if (!list) byNamespace.set(e.ns, (list = []));
    list.push(e);
  }
  for (const [ns, enums] of byNamespace) {
    out += `pub mod ${ns} {\n`;
    for (const e of enums) {
      out += `    #[repr(u${e.bits})]\n    #[derive(Copy, Clone, Eq, PartialEq, Debug)]\n`;
      out += `    pub enum ${e.name} {\n`;
      e.values.forEach((v, i) => (out += `        ${pascal(v)} = ${i},\n`));
      out += `    }\n`;
    }
    out += `}\n`;
  }
  writeIfNotChanged(path.join(codegenRoot, "bindgen_string_enums.rs"), out);
}
