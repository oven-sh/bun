 ...
 * that successfully converts determines the active field in the corresponding Rust tagged union.
 *
  ...
  if (typeof alternativesOrName !== "string") {
    ...
    return new (class extends AnonymousUnionType {
      ...
      rustType(style?: CodeStyle) {
        if (style !== "pretty") {
          return `bun.meta.TaggedUnion(&.{ ${alternatives.map(a => a.rustType()).join(", ")} })`;
        }
        return dedent(`bun.meta.TaggedUnion(&.{
          ${joinIndented(
            10,
            alternatives.map(a => a.rustType("pretty") + ","),
          )}
        })`);
      }
      get dependencies() {
        ...
        pub const ${name} = union(enum) {
          ${joinIndented(
            ...
            Object.entries(namedAlternatives).map(([altName, altType]) => {
              return `${altName}: ${altType.rustType("pretty")},`;
            }),
          )}
