...
import { cap, checkAscii, low, writeIfNotChanged } from "./helpers.ts";
...
`,
    ...
    let output = build.outputFiles[0].text.replace(
      /[^\x00-\x7f]/g,
      c => "\\u" + c.charCodeAt(0).toString(16).padStart(4, "0"),
    );
    let usesDebug = output.includes("$debug_log");
    let usesAssert = output.includes("$assert");
    // esbuild hoists array and object `define` values into variables before `$$capture_start$$`.
    // Move them into the function body. The rest of that prologue is comments and the `$;` line.
    const [, prologue, body] = output.match(/^([\s\S]*?)\$\$capture_start\$\$([\s\S]+)\.\$\$capture_end\$\$/)!;
    const hoisted = prologue.replace(/^\/\/.*$|^\$;$/gm, "").trim();
    const captured = hoisted ? body.replace(/function\s*\(.*?\)\s*{/, match => match + hoisted + "\n") : body;
