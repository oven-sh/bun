...
export async function bundleBuiltinFunctions({ requireTransformer }: BundleBuiltinFunctionsArgs) {
  ...
  let globalBuiltinId = 0;
  // Compute source offsets and global IDs
  {
    for (const { basename, functions } of files) {
      for (const fn of functions) {
        fn.globalId = globalBuiltinId++;
...
JSBuiltinInternalFunctions::JSBuiltinInternalFunctions(JSC::VM& vm) : m_vm(vm)
  ...
  // Source accessor for bytecode generation at build time
  {
    const entries: string[] = [];
    for (const { basename, functions } of files) {
      for (const fn of functions) {
        const inlineAttr = fn.directives.alwaysInline ? "Always" : "None";
        entries.push(
          `{ ${fn.sourceOffset}, ${fn.source.length}, "${low(basename)}${cap(fn.name)}", ` +
            `(uint8_t)JSC::ImplementationVisibility::${fn.visibility}, ` +
            `(uint8_t)JSC::ConstructorKind::${fn.constructKind}, ` +
            `(uint8_t)JSC::ConstructAbility::${fn.constructAbility}, ` +
            `(uint8_t)JSC::InlineAttribute::${inlineAttr} }`,
        );
      }
    }
    bundledCPP += `
extern "C" const char* Bun__getBuiltinFunctionSource(uint32_t id, size_t* len, const char** name, uint8_t* vis, uint8_t* ctorKind, uint8_t* ctorAbility, uint8_t* inlineAttr)
{
    struct Entry { uint32_t offset; uint32_t length; const char* name; uint8_t vis, ck, ca, ia; };
    static const Entry table[] = {
        ${entries.join(",\n        ")}
    };
    if (id >= ${globalBuiltinId}) return nullptr;
    const auto& e = table[id];
    *len = e.length; *name = e.name; *vis = e.vis; *ctorKind = e.ck; *ctorAbility = e.ca; *inlineAttr = e.ia;
    return (const char*)WebCore::combinedSourceCodeBuffer + e.offset;
}

extern "C" uint32_t Bun__getBuiltinFunctionSourceCount() { return ${globalBuiltinId}; }
    `;
  }
  ...
  let bundledHeader = `// Generated by ${import.meta.path}
    ...
    namespace Bun {
    JSC::UnlinkedFunctionExecutable* tryDecodeBuiltinFunctionBytecode(JSC::VM&, uint32_t globalId);
    }
  ...
  for (const { basename, functions, internal } of files) {
    ...
    static constexpr uint32_t s_${name}GlobalId = ${fn.globalId};
    ...
    {\\
        if (!m_##name##Executable) {\\
            ...
            auto* unlinked = Bun::tryDecodeBuiltinFunctionBytecode(m_vm, s_##name##GlobalId);\\
            if (!unlinked)\\
                unlinked = JSC::createBuiltinExecutable(m_vm, m_##name##Source, executableName, s_##name##ImplementationVisibility, s_##name##ConstructorKind, s_##name##ConstructAbility, s_##name##InlineAttribute);\\
            m_##name##Executable = JSC::Weak<JSC::UnlinkedFunctionExecutable>(unlinked, this, &m_##name##Executable);\\
        }\\
