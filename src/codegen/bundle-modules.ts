...
// InternalBuiltinBytecode.S — embeds the precompiled builtin bytecode blob
// (produced by the stage-1 binary during the build). Only assembled when the
// build embeds bytecode; the weak fallbacks live in InternalBuiltinBytecode.cpp.
writeIfNotChanged(
  path.join(CODEGEN_DIR, "InternalBuiltinBytecode.S"),
  `// Generated by src/codegen/bundle-modules.ts
#if defined(__APPLE__)
.section __TEXT,__const
#define BUN_SYM(x) _##x
...
.globl BUN_SYM(bun_builtin_bytecode_blob)
.p2align 7
BUN_SYM(bun_builtin_bytecode_blob):
.incbin "InternalBuiltinBytecode.bin"
BUN_SYM(bun_builtin_bytecode_blob_end):

.globl BUN_SYM(bun_builtin_bytecode_blob_size)
.p2align 3
BUN_SYM(bun_builtin_bytecode_blob_size):
.quad BUN_SYM(bun_builtin_bytecode_blob_end) - BUN_SYM(bun_builtin_bytecode_blob)
`,
);
