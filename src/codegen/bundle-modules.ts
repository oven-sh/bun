...
import { declareASCIILiteral, writeIfNotChanged } from "./helpers";
import {
  createInternalModuleRegistry,
  idToEnumName,
  idToPublicSpecifierOrEnumName,
} from "./internal-module-registry-scanner";
...
// Rust sibling of ResolvedSourceTag.zig: the body of `bun_jsc::resolved_source_tag`,
// include!()'d by src/jsc/lib.rs. Lookup keys are the same names as the Zig enum
// fields; `HardcodedModule` (src/resolve_builtins/HardcodedModule.rs) strum names
// must match them — see `ResolvedSourceTag::from_name`.
writeIfNotChanged(
  path.join(CODEGEN_DIR, "ResolvedSourceTag.rs"),
  `// Auto-generated by src/codegen/bundle-modules.ts — DO NOT EDIT.
//
// Rust sibling of ResolvedSourceTag.zig / SyntheticModuleType.h. This is the module
// body of \`bun_jsc::resolved_source_tag\` (src/jsc/lib.rs), pulled in with
// \`include!(concat!(env!("BUN_CODEGEN_DIR"), "/ResolvedSourceTag.rs"))\`.

/// \`ResolvedSource.Tag\` — \`enum(u32)\` in Zig, plain \`uint32_t\` in C++
/// (\`headers-handwritten.h\`). Modelled as a transparent \`u32\` newtype so the
/// generated InternalModuleRegistry IDs (\`(1 << 9) | id\`) round-trip without an
...
impl ResolvedSourceTag {
    ...
    /// Map a builtin-module registry name (e.g. \`b"node:fs"\`) to its
    ...
    /// callers feed only \`HardcodedModule\` strum names, so a miss means that
    /// enum has drifted from the module registry.
...
// Built in modules are loaded through InternalModuleRegistry by numerical ID.
// In this map they are represented as \`(1 << 9) | id\`.
static INTERNAL_MODULE_TAG: phf::Map<&'static [u8], ResolvedSourceTag> = phf::phf_map! {
${moduleList
  .slice(0, nativeStartIndex)
  .map((id, n) => `    b"${idToPublicSpecifierOrEnumName(id)}" => ResolvedSourceTag(${(1 << 9) | n}),`)
  .join("\n")}
    // Native modules come after the JS modules
${Object.entries(nativeModuleEnumToId)
  .map(
