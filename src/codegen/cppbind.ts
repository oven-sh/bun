...
function generateRustFn(fn: CppFn, rustRaw: string[], rustWrap: string[]): void {
  ...
  if (fn.tag === "nothrow") {
    ...
    if (needsUnsafe) {
      ...
      rustWrap.push(
        ...
        `    // SAFETY: \`[[ZIG_EXPORT(nothrow)]]\` extern; ref args (opaque-ZST handles, \`&Str\`, \`&mut String\`) are valid for the call.`,
  ...
  if (fn.tag === "check_slow") {
    ...
    rustWrap.push(
      ...
      `    // SAFETY: \`[[ZIG_EXPORT(check_slow)]]\` extern; ref args (opaque-ZST handles, \`&Str\`, \`&mut String\`) are valid for the call;`,
  ...
  rustWrap.push(
    ...
    `    // SAFETY: \`[[ZIG_EXPORT(${fn.tag})]]\` extern; ref args (opaque-ZST handles, \`&Str\`, \`&mut String\`) are valid for the call;`,
    `    // any raw-pointer args are forwarded under the wrapper's own \`unsafe fn\` contract.`,
