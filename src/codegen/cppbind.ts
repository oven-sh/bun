...
1. **nothrow** - Function that never throws exceptions:
   ...
   ```
   Rust usage: `bun.cpp.hello_world();`
...
2. **zero_is_throw** - Function returns JSValue, where .zero indicates an exception:
   ...
   ```
   Rust usage: `try bun.cpp.create_object(globalThis);`
...
3. **check_slow** - Function that may throw, performs runtime exception checking:
   ...
   ```
   Rust usage: `try bun.cpp.process_data(globalThis);`
...
// ─────────────────────────── Rust output (cpp.rs) ───────────────────────────
//
// Mirror of `generateRustFn` for the Rust port: each `[[RUST_EXPORT(mode)]]` C++
...
const rustSharedTypes: Record<string, string> = {
  ...
  // unsigned on aarch64); match Rust's `c_char` so a future by-value return
  ...
  "Rust::GlobalObject": "crate::JSGlobalObject",
  "RustException": "crate::rust_exception::RustException",
  "RustString": "bun_core::RustString",
  "JSC::VM": "crate::VM",
...
function generateRustType(type: CppType, parent: CppType | null): string {
  ...
  if (type.type === "named") {
    ...
    // so match the Rust generator and fail loudly at the C++ source location.
    if (parent?.type === "pointer") return "core::ffi::c_void";
    throwError(
      type.position,
...
function isGlobalObjectPtr(t: CppType): boolean {
  return (
    t.type === "pointer" &&
    t.child.type === "named" &&
    (t.child.name === "JSC::JSGlobalObject" || t.child.name === "Rust::GlobalObject")
  );
}
...
function generateRustFn(fn: CppFn, rustRaw: string[], rustWrap: string[]): void {
  ...
  if (!globalArg) {
    // Same constraint as the Rust generator; emit a stub so the module still
  ...
  if (fn.tag === "check_slow") {
    ...
    // Rust's `cpp.rust` (`@src()` inside the wrapper); `#[track_caller]` would be a no-op
  ...
  // file/line — parity with Rust's `@src()` inside cpp.rust wrappers). `#[track_caller]`
...
function generateRustType(type: CppType, parent: CppType | null) {
  if (type.type === "pointer") {
    const optionalChar = type.isNonNull ? "" : "?";
    const ptrChar = type.isMany ? "[*]" : "*";
    const constChar = type.isConst ? "const " : "";
    return `${optionalChar}${ptrChar}${constChar}${generateRustType(type.child, type)}`;
  }
  if (type.type === "fn") {
    return `fn(${type.parameters.map(p => formatRustName(p.name) + ": " + generateRustType(p.type, null)).join(", ")}) callconv(.c) ${generateRustType(type.returnType, null)}`;
  }
  if (type.type === "named" && type.name === "void") {
...
function formatRustName(name: string): string {
  if (name.match(/^[a-zA-Z_][a-zA-Z0-9_]*$/)) return name;
  return "@" + JSON.stringify(name);
}
function generateRustParameterList(parameters: CppParameter[], globalThisArg?: CppParameter): string {
  return parameters
    .map(p => {
      if (p === globalThisArg) {
        return `${formatRustName(p.name)}: *jsc.JSGlobalObject`;
      } else {
        return `${formatRustName(p.name)}: ${generateRustType(p.type, null)}`;
      }
    })
    .join(", ");
}
function generateRustSourceComment(cfg: Cfg, resultSourceLinks: string[], fn: CppFn): string {
...
function generateRustFn(
  ...
  let returnType = generateRustType(fn.returnType, null);
  if (resultBindings.length) resultBindings.push("");
  resultBindings.push(generateRustSourceComment(cfg, resultSourceLinks, fn));
  if (fn.tag === "nothrow") {
    resultBindings.push(
      `pub extern fn ${formatRustName(fn.name)}(${generateRustParameterList(fn.parameters)}) ${returnType};`,
    );
    return;
  }

  resultRaw.push(`    extern fn ${formatRustName(fn.name)}(${generateRustParameterList(fn.parameters)}) ${returnType};`);
  let globalThisArg: CppParameter | undefined;
  for (const param of fn.parameters) {
    const type = generateRustType(param.type, null);
  ...
  if (fn.tag === "check_slow") {
    ...
    resultBindings.push(
      `pub fn ${formatRustName(fn.name)}(${generateRustParameterList(fn.parameters, globalThisArg)}) error{JSError}!${returnType} {`,
      `    if (comptime Environment.ci_assert) {`,
      `        var scope: jsc.TopExceptionScope = undefined;`,
      `        scope.init(${formatRustName(globalThisArg.name)}, @src());`,
      `        defer scope.deinit();`,
      ``,
      `        const result = raw.${formatRustName(fn.name)}(${fn.parameters.map(p => formatRustName(p.name)).join(", ")});`,
      `        try scope.returnIfException();`,
      `        return result;`,
      `    } else {`,
      `        const result = raw.${formatRustName(fn.name)}(${fn.parameters.map(p => formatRustName(p.name)).join(", ")});`,
      `        if (Bun__RETURN_IF_EXCEPTION(${formatRustName(globalThisArg.name)})) return error.JSError;`,
      `        return result;`,
      `    }`,
      `}`,
    );
  ...
  resultBindings.push(
    ...
    `        defer scope.deinit();`,
    ``,
    `        const value = raw.${formatRustName(fn.name)}(${fn.parameters.map(p => formatRustName(p.name)).join(", ")});`,
...
`.slice(1),
  ...
  for (const fn of allFunctions) {
    try {
      generateRustFn(fn, resultRaw, resultBindings, resultSourceLinks, { dstDir });
    } catch (e) {
  ...
  const resultFilePath = join(dstDir, "cpp.rust");
