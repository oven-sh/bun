...
function generateRustFn(fn: CppFn, rustRaw: string[], rustWrap: string[]): void {
  ...
  if (fn.tag === "nothrow") {
    ...
    if (needsUnsafe) {
      rustWrap.push(RUST_ALLOW, `pub use self::raw::${fn.name};`);
    } else {
      rustWrap.push(
        RUST_ALLOW,
        `#[inline]`,
...
function generateScopedFn(fn: CppFn, rustScoped: string[]): void {
  ...
  // `null_is_throw` returns a GC cell pointer; a scoped variant would hand it
  // back unbranded (no `Local` lifetime), which is the escape the layer exists
  // to prevent. Reach these through their `Local` methods instead.
  if (fn.tag === "null_is_throw") return;
  const globalArg = fn.parameters.find(p => isGlobalObjectPtr(p.type));
...
`.slice(1),
  ...
  const rustContents =
    ...
    rustWrap.join("\n") +
    "\n\n" +
    "/// `Scope`-branded variants of the explicitly classified throwing wrappers\n" +
    "/// above (`reenters_js` → `&mut Scope`, `no_user_js` → `&Scope`; unclassified\n" +
    "/// and `null_is_throw` functions get no variant). By-value `EncodedJSValue`s become\n" +
    "/// `Local`; the return is branded verbatim, so functions that encode sentinels in\n" +
    "/// their return (e.g. `ValueDeleted`) must be consumed through their filtering Rust wrapper.\n" +
    "pub mod scoped {\n" +
    "    #[allow(unused_imports)] use super::*;\n" +
    rustScoped.join("\n") +
    "\n}\n";
