  ...
  for (const file of files) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (abi === "rust") shape = "rust";
        ...
        shape = "lazy";
        // Lazy property creators are direct C++ calls (e.g. RustGlobalObject.cpp
...
function emitThunk(e: Export): string {
  ...
  // `callconv(jsc.conv)` bodies in the .rust spec — wrap them and you trip
...
use bun_jsc::{JSInternalPromise, JSObject, JSPromise, RustStackFrame};
