...
// Only emit imports for types the generated thunks actually reference, so the
// workspace's \`unused_imports = "deny"\` is satisfied regardless of which
// HOST_EXPORTs survive dead-code passes.
const importCandidates: Array<[string, string]> = [
  ["core::ffi", "c_char"],
  ["core::ffi", "c_int"],
  ["core::ffi", "c_void"],
  ["bun_jsc", "host_fn"],
  ...
  ["bun_jsc::frontend_dev_server_agent", "InspectorBunFrontendDevServerAgentHandle"],
...
writeIfNotChanged(path.join(outBase, "generated_host_exports.rs"), header + imports + body + "\n" + externAudit);
