...
// The js2native dispatch table was originally implemented in Zig and exposed
// to C++ via `extern "C"` symbols spelled `JS2Zig__<src-path>_zig__<symbol>`.
// The Rust port replaced the implementer of every symbol but kept the symbol
// *names* unchanged: `GeneratedJS2Native.h` still declares `JS2Zig__*` externs,
// hand-exported Rust thunks still carry `#[unsafe(export_name = "JS2Zig__...")]`,
// and tooling/snapshots/crash reports key on the same strings. Renaming them
// would be a no-op churn at best and an ABI/debuggability break at worst, so
// we deliberately keep the historical `Zig`/`_zig_` markers and only swap the
// *file extension* fed into the mangler.
function normalizeSymbolPathPrefix(input: string) {
  ...
  return input.replaceAll(".rs", "_zig_").replace(/[^A-Za-z]/g, "_");
}
