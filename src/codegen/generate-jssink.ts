...
// Each thunk dispatches to a method on the per-sink `${name}SinkImpl` trait
// (default body = `unimplemented!`); porting a sink means providing a real
// `impl ${name}SinkImpl for ${name}SinkNative` in `src/runtime/webcore/`.
//
// Calling convention: `BUN_DECLARE_HOST_FUNCTION` is `SYSV_ABI` which is plain
// `extern "C"` on every target except Windows-x64; the win-x64 path is handled
// by the proc-macro layer in a later phase, so this file emits `extern "C"`
// directly (matching `generate-classes.ts::generateRust()`).
function rustSink() {
  const symbols: string[] = [];
  let templ = `// Auto-generated by src/codegen/generate-jssink.ts — DO NOT EDIT.
//
// Per-sink \`#[unsafe(no_mangle)] extern "C"\` thunks satisfying the externs
// declared by JSSink.cpp / headers.h. Each thunk dispatches to a method on
// the per-sink \`\${name}SinkImpl\` trait (default body = \`unimplemented!\`);
// porting a sink means providing a real \`impl \${name}SinkImpl for
// \${name}SinkNative\` and (eventually) replacing the opaque placeholder with
// the real native layout.
...
`;
  ...
  for (const name of classes) {
    // `name` already ends in "Sink" (e.g. ArrayBufferSink) — don't double it.
...
/// Native backing type for \`JS${name}.m_sinkPtr\`. Opaque placeholder until the
/// real Rust struct is wired; \`impl ${impl} for ${native}\` overrides any of
/// the defaulted methods below.
#[repr(C)]
pub struct ${native} {
    _opaque: [u8; 0],
...
pub trait ${impl} {
    fn construct(global: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> { unimplemented!("${name}::construct not yet ported to Rust") }
    fn write(global: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> { unimplemented!("${name}::write not yet ported to Rust") }
    fn end(global: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> { unimplemented!("${name}::end not yet ported to Rust") }
    fn flush(global: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> { unimplemented!("${name}::flush not yet ported to Rust") }
    fn start(global: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> { unimplemented!("${name}::start not yet ported to Rust") }
    fn getInternalFd(this: &mut Self) -> JSValue { JSValue::NULL }
    fn memoryCost(this: &Self) -> usize { 0 }
}
impl ${impl} for ${native} {}
...
pub unsafe extern "C" fn ${sym}(global: *mut JSGlobalObject, callframe: *mut CallFrame) -> JSValue {
    ...
    bun_jsc::host_fn::host_fn_result(global, || <${native} as ${impl}>::${fn}(global, callframe))
}
...
`;
    ...
    // The C++ wrapper is opaque on the Rust side; recover m_sinkPtr via the
    // generated `${name}__fromJS`-style accessor once wired. For now treat the
    // argument as an opaque cell pointer and dispatch on the trait.
...
pub unsafe extern "C" fn ${name}__getInternalFd(this: *mut c_void) -> JSValue {
    ...
    unsafe { <${native} as ${impl}>::getInternalFd(&mut *this.cast::<${native}>()) }
}
...
pub unsafe extern "C" fn ${name}__memoryCost(this: *mut c_void) -> usize {
    ...
    unsafe { <${native} as ${impl}>::memoryCost(&*this.cast::<${native}>()) }
}
