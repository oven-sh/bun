...
function rustSink() {
  ...
  // calling). No `unsafe` block or `unsafe fn` appears in the generated file
  // (the `#[unsafe(no_mangle)]` attribute is mandatory Rust 2024 syntax, not an
  // unsafe block) — same shape as `generate-classes.ts::generateRust()`.
...
// declared by JSSink.cpp / headers.h. Each thunk calls an inherent method on
// the real sink struct (re-exported here as \`\${name}\`). No trait, no opaque
// placeholder — a missing method is a hard compile error.
//
...
`;
    ...
    templ += `#[unsafe(no_mangle)]
pub extern "C" fn ${name}__endWithSink(this: &mut ${name}, global: &JSGlobalObject) -> JSValue {
