...
pub const js_bindings = struct {
    ...
    pub fn jsGetFeatureData(global: *jsc.JSGlobalObject, _: *jsc.CallFrame) bun.JSError!jsc.JSValue {
        ...
        obj.putDirect(global, jsc.ZigString.static("features"), array);
        obj.putDirect(global, jsc.ZigString.static("version"), bun.String.init(Global.package_json_version).toJS(global));
        ...
        obj.putDirect(global, jsc.ZigString.static("revision"), bun.String.init(bun.Environment.git_sha).toJS(global));

        obj.putDirect(global, jsc.ZigString.static("generated_at"), JSValue.jsNumberFromInt64(@max(std.time.milliTimestamp(), 0)));
        return obj;
    }
};
