...
let field_len = 0;
let flagsRust = `pub const Features = packed struct(u32) {
    ${flags
      .map((flag, i) => {
...
let targets = fs
  .readFileSync("src/css/targets.rust", "utf8")
  .replace(/pub const Features = packed struct\(u32\) \{((?:.|\n)+?)\n\}/, flagsRust);

console.log("TARGETS", targets);
fs.writeFileSync("src/css/targets.rust", targets);
await Bun.$`rust fmt src/css/targets.rust`;
...
fs.writeFileSync("src/css/prefixes.rust", s);
await Bun.$`rust fmt src/css/prefixes.rust`;
...
fs.writeFileSync("src/css/compat.rust", c);
await Bun.$`rust fmt src/css/compat.rust`;
