...
// ─── compat.rs ────────────────────────────────────────────────────────────
// One row per Feature variant, in enum-declaration order (indexed by
// `Feature as usize`), instead of a match arm of version checks per feature.
{
  const entries = [...compat]
    .flatMap(([features, supported]) => features.map(name => [enumify(name), supported]))
    .sort(([a], [b]) => (a < b ? -1 : 1));

  const variants = entries.map(([name]) => `    ${name},`).join("\n");
  const structFields = allBrowsers.map(b => `    ${b}: u32,`).join("\n");
  const rows = entries
    .map(([name, supported]) => {
      if (Object.keys(supported).length === 0) {
        return `    None, // ${name}`;
      }
      const fields = allBrowsers.map(b => `${b}: ${supported[b] ?? "UNSUPPORTED"}`).join(", ");
      return `    Some(MinVersions { ${fields} }), // ${name}`;
    })
    .join("\n");
  const checks = allBrowsers.map(b => `        check!(${b});`).join("\n");
  const lastVariant = entries[entries.length - 1][0];
