...
fs.writeFileSync("src/css/compat.rs", generateCompatRs(compatRsEntries, allBrowsers));
await Bun.$`rustfmt src/css/compat.rs`;
...
// The shipping Rust implementation of the compat data. Instead of one match
// arm of version checks per feature (the compat.zig form), emit a
// per-feature minimum-version table consumed by a generic check.
function generateCompatRs(entries, browserFields) {
  let variants = entries.map(([name]) => `    ${name},`).join("\n");
  let structFields = browserFields.map(browser => `    ${browser}: u32,`).join("\n");
  let rows = entries
    .map(([name, supportedBrowsers]) => {
      if (Object.keys(supportedBrowsers).length === 0) {
        return `    None, // ${name}`;
      }
      let fields = browserFields
        .map(browser => `${browser}: ${supportedBrowsers[browser] ?? "UNSUPPORTED"}`)
        .join(", ");
      return `    Some(MinVersions { ${fields} }), // ${name}`;
    })
    .join("\n");
  let checks = browserFields.map(browser => `        check!(${browser});`).join("\n");
  let lastVariant = entries[entries.length - 1][0];

  return `// This file is autogenerated by build-prefixes.js (ported from lightningcss).
// DO NOT EDIT!
//
...
const FEATURE_COUNT: usize = Feature::${lastVariant} as usize + 1;
