// Data derived from the output of build-prefixes.js (which generates the
// sibling compat.zig, ported from lightningcss). Unlike the generated code,
// which emits one match arm of version checks per feature, the data here is
// stored as a per-feature minimum-version table consumed by a generic check.
