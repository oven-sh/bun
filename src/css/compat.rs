...
impl Feature {
    ...
    pub fn is_compatible(self, _targets: Browsers) -> bool {
        // 215-arm per-browser min-version match (compat.zig:220-5325) — blocked
        // on build-prefixes.js growing a --rs emitter. Loud failure instead of
        // a silent `false` so callers don't quietly take the wrong lowering
        // path (§Forbidden-patterns: silent-no-op).
        todo!("compat::Feature::is_compatible — per-browser min-version tables (build-prefixes.js --rs)")
    }
    ...
    pub fn is_partially_compatible(self, targets: Browsers) -> bool {
        ...
        // Stays correct automatically once `is_compatible` is filled in.
...
//   confidence: high
//   todos:      1
//   notes:      215-variant Feature enum is real; is_compatible body is a
//               loud todo!() until build-prefixes.js emits the per-browser
//               version tables. is_partially_compatible is implemented
//               generically (probes one browser at a time) per zig:5327-5394.
// ──────────────────────────────────────────────────────────────────────────
