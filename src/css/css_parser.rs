...
//! arena-backed. Phase A keeps `&'bump Bump` threading
...
// ───────────────────────────── re-exports ─────────────────────────────
//
// B-2 un-gate: the css_parser hub re-exports the entire crate surface.
...
// `Maybe` was a tagged-result type. In Rust we use
...
pub fn oom(_e: bun_core::Error) -> ! {
    if cfg!(debug_assertions) {
        // assert the error is out-of-memory
    }
...
impl SourceLocation {
    ...
    // PORT NOTE: originally a duck-typed dispatch on
    ...
    // (the `BasicParseError` arm is dead/ill-typed — see note below).
...
/// Dispatch trait for `SourceLocation::new_custom_error` (replaces a
/// duck-typed type switch).
...
// PORT NOTE: `newCustomError` originally had a third type-switch arm for
...
// `BasicParseError`, so it was never instantiated. We
...
// Rust callers use `CssResult<T>` directly (originally a generic `Result(T)` type ctor).
...
//
// The original file defines `DefineListShorthand`, `DefineShorthand`,
// `DefineRectShorthand`, `DefineSizeShorthand`, `DeriveParse`, `DeriveToCss`,
// `DefineEnumProperty`, `DeriveValueType` — all of which use compile-time type
// reflection to generate `parse`/`toCss`/etc. for arbitrary
...
///
/// PORT NOTE: the original `DefineShorthand` bodies were unconditional compile
/// errors — i.e. instantiating the codegen and reaching any method failed to
/// compile. The faithful Rust mapping is a trait with **no default bodies**: any
/// `impl DefineShorthand for T` that omits a method fails at compile time.
/// Per-type bodies are emitted by `#[derive(DefineShorthand)]` using the
/// (currently commented-out) `PropertyFieldMap`/`VendorPrefixMap`
/// reflection algorithm.
pub trait DefineShorthand: Sized {
    /// The shorthand's own `PropertyIdTag` (compile-time `property_name`).
...
// PORT NOTE: the original `DefineListShorthand` / `DefineRectShorthand` /
// `DefineSizeShorthand` / `DeriveParse` / `DeriveToCss` codegen helpers became
...
pub mod enum_property_util {
    ...
    // TODO(port): `as_str` / `parse` / `to_css` originally used a compile-time
    // enum-name map. In Rust this is
...
// (`css.DefineEnumProperty(...)` historically lived in this file).
pub use bun_css_derive::{DefineEnumProperty, Parse, ToCss};

/// Replaces the original `DefineEnumProperty` codegen.
...
fn parse_until_before<T, C>(
    ...
    // PORT NOTE: reshaped for borrowck — `parser.input` was originally aliased
    ...
    // `at_start_of` is *moved into* the inner parse (originally moved into the
    // delimited Parser, leaving the outer null) — since we reuse the same
    ...
    let result = {
        ...
        if matches!(error_behavior, ParseUntilErrorBehavior::Stop) && result.is_err() {
            ...
            // The delimited parser *moved* `at_start_of` out of the
...
//
// `ValidQualifiedRuleParser(T)` etc. were compile-time duck-type checks.
// PORTING.md: trait bounds ARE that check.
...
pub trait CustomAtRuleParser {
    ...
    /// of the parser. The spec only populates `StyleSheet.layer_names`
    /// when `P == BundlerAtRuleParser`; Rust can't
...
pub struct BundlerAtRuleParser<'a> {
    ...
    /// points to (one `*Vec` is passed to both — see `parse_bundler`).
    /// Both views are raw pointers sharing a single
...
pub struct TopLevelRuleParser<'a, AtRuleParserT: CustomAtRuleParser> {
    // PORT NOTE: `input.arena()` was originally threaded at every call site; the Rust
...
/// Dispatch trait for `parse_declaration_impl` (replaces a duck-typed
/// `composes_ctx` parameter). Implemented by `NestedRuleParser`.
...
mod rule_parsers {
    ...
    // PORT NOTE: a duck-typed `composes_ctx` (pointer to the
    // `NestedRuleParser`) was originally threaded directly into `parse_declaration`. Rust's borrow checker
    ...
    impl<'a, T: CustomAtRuleParser> NestedRuleParser<'a, T> {
        pub fn parse_nested(
            ...
            // PORT NOTE: reshaped for borrowck — `self` was originally aliased. Spell
    ...
    impl<'a, T: CustomAtRuleParser> AtRuleParser for NestedRuleParser<'a, T> {
        ...
        fn parse_prelude(
            ...
            let result: Self::Prelude = 'brk: {
                // ASCII-CI dispatch on at-rule prelude name.
                crate::match_ignore_ascii_case! { name, {
                    ...
                    b"page" => {
                        // tryParse(parseCommaSeparated(PageSelector.parse)) → on
                        // error returns empty list. EOF inside `PageSelector::parse`
                        // (e.g. `@page foo` with nothing after) propagates here and is
                        // swallowed by `try_parse`.
            ...
            match prelude {
                ...
                AtRulePrelude::Layer(mut layer) => {
                    ...
                    let name = if layer.len() == 0 {
                        ...
                        // `&[u8]`, so this is the same field-walk a struct copy did).
    ...
    impl<'a, T: CustomAtRuleParser> QualifiedRuleParser for NestedRuleParser<'a, T> {
        ...
        fn parse_block(
            ...
            let loc = this.get_loc(start);
            // PORT NOTE: `composes_refs` is cleared (retaining capacity) on every exit.
    ...
    /// `MediaList::parse` thunk. The body belongs in `media_query.rs`; the
...
impl CssRef {
    ...
    pub fn to_real_ref(self, source_index: u32) -> bun_ast::Ref {
        // Spec constructs a symbol-tagged `Ref`.
...
impl PropertyUsage {
    ...
    pub fn fill(&mut self, used: &PropertyBitset, custom_properties: &[&'static [u8]]) {
        ...
        // TODO(port): lifetime — should store a borrowed slice; box for now.
        self.custom_properties = custom_properties.to_vec().into_boxed_slice();
    }
}

// TODO(port): originally sized via `ceilPow2(EnumFields(PropertyIdTag).len)`.
...
pub struct StyleSheet<AtRule> {
    ...
    // Originally a conditional field present only when AtRule == BundlerAtRule.
...
// `// PORT NOTE:` borrowck reshapes where the original aliased pointers.
mod stylesheet_impl {
    use super::*;
    ...
    impl<AtRule> StyleSheet<AtRule> {
        ...
        /// (originally an `Allocator` parameter). It is threaded into `MinifyContext` so
        ...
        pub fn to_css<'a>(
            ...
            // TODO(port): writer adapter — originally an allocating writer; here we
        ...
        pub fn parse(
            ...
            // PORT NOTE: originally a generic `StyleSheet(DefaultAtRule).parse`; Rust
        ...
        pub fn parse_with<P: CustomAtRuleParser<AtRule = AtRule>>(
            ...
            // The caller owns the arena (originally an `Allocator`
            ...
            // at_rule_parser.layer_names else .{}`. Rust
        ...
        pub fn contains_tailwind_directives(&self) -> bool {
            // TODO(port): originally a compile error if AtRule != BundlerAtRule.
        ...
        pub fn pluck_imports(
            ...
            // PORT NOTE: the original fn took a `*const` receiver but wrote
            // through it (no const-transitivity in the source language).
            ...
            //
            // Originally a compile-time two-pass loop (count, exec). Unroll.
            let mut count: u32 = 0;
            {
            ...
            for rule in self.rules.v.iter_mut() {
                match rule {
                    ...
                    CssRule::Import(import_rule) => {
                        ...
                        // PORT NOTE: reshaped for borrowck — originally a
                        // bitwise `appendAssumeCapacity(rule.*)` copy then
    ...
    impl StyleAttribute {
        ...
        pub fn to_css<'a>(
            ...
            // TODO(port): writer adapter — originally an allocating writer; route
    ...
    impl StyleSheet<BundlerAtRule> {
        pub fn parse_bundler(
            ...
            // PORT NOTE: `import_records` was originally aliased into both
            // `BundlerAtRuleParser` *and* the inner `Parser`, and `&options` was
            // aliased into the at-rule parser while also passing `options` by value
            // (struct copy) to `parseWith`. Rust forbids both overlaps directly:
            ...
            // - `options`: bitwise-duplicate via `ptr::read` (mirroring the original
...
pub struct ParserOptions<'a> {
    ...
    ///
    /// Stored as a raw `NonNull<Log>` (mirrors the original `*Log`) so `warn(&self)`
...
impl<'a> ParserOptions<'a> {
    pub fn warn(&self, warning: ParseError<ParserError>) {
        if let Some(lg) = self.logger {
            ...
            // Log exists for the duration of parsing; mutate through the raw pointer.
...
// PORT NOTE: originally a packed struct with `css_modules: bool`. Expose accessor:
impl ParserOpts {
    #[inline]
...
pub struct Parser<'a> {
    ...
    /// Stored as a raw `NonNull` (mirrors the original `?*Vec(ImportRecord)`)
    /// because `BundlerAtRuleParser` holds an aliasing
...
impl<'a> Parser<'a> {
    ...
    pub fn add_import_record(
        ...
        if let Some(ptr) = self.import_records {
            ...
            // the original stores the same borrowed slice; Phase-A
    ...
    // `tryParseImpl` is the same as `tryParse` with manual args tuple;
...
pub struct Tokenizer<'a> {
    ...
    pub current_line_start_position: usize,
    pub current_line_number: u32,
    // TODO(port): AST crate — keep arena. Originally an `Allocator`; in Rust
    // this is `&'a Bump`.
    pub arena: &'a Bump,
...
/// UTF-8 encoding of U+0FFD — used by `serializer`. The original
/// literal is `0xFFD` (sic — likely a typo for `0xFFFD`), but the spec is
...
// TODO(port): verify upstream — the original wrote 0xFFD, comment says
// "replacement character" which is U+FFFD. Byte-matching the spec (0x0FFD) for now.
...
/// so `slice_from`/`to_slice` return `&'a [u8]` (matching the original
/// plain byte-slice borrows). That change is blocked
...
impl<'a> Tokenizer<'a> {
    ...
    pub fn see_function(&mut self, name: &[u8]) {
        if self.var_or_env_functions == SeenStatus::LookingForThem {
            // PORT NOTE: the original had `and` here (always false); preserved.
    ...
    pub fn consume_escape_and_write(&mut self, bytes: &mut CopyOnWriteStr<'a>) {
        ...
        // Route through char's UTF-8 encoder (val is guaranteed a valid
        // scalar by consume_escape).
...
pub fn split_source_map(contents: &[u8]) -> Option<&[u8]> {
    ...
    // TODO(port): originally a UTF-8 codepoint iterator. Approximate with byte
    // scan since the delimiters are all ASCII.
    for (i, &c) in contents.iter().enumerate() {
        match c {
            b' ' | b'\t' | FORM_FEED_BYTE | b'\r' | b'\n' => {
                return Some(&contents[0..i + 1]);
                // PORT NOTE: the original returned `[0..iter.i]` where `i` is *after*
...
impl TokenKind {
    pub fn to_string(self) -> &'static str {
        // TODO(port): the original switch had stale variant names (close_bracket,
        // hash, string) and pattern-matched a `delim` payload — which TokenKind
        // has none of. Preserved best-effort; Phase B revisits.
        match self {
...
impl Token {
    ...
    pub fn eql(lhs: &Token, rhs: &Token) -> bool {
        // TODO(port): originally implemented via reflective field-walk.
...
// for error messages. The full `Token.format` (CSS-serialization-correct)
// is `Token::to_css_generic` above; switch lib.rs's impl to delegate once
// dependents stop relying on the simple form.
// TODO(port): `format` had subtle differences from `to_css_generic`
// (quoted_string→serialize_string, idhash→serialize_identifier). Phase B
// specializes.

/// Byte-writer trait for `serializer` and `to_css_generic` (replaces a
/// duck-typed writer). Aliased to the canonical `bun_io::Write`; the associated
...
pub mod to_css {
    ...
    pub fn string<'a, T: generic::ToCss>(
        ...
        // PORT NOTE: originally `T == CSSString` was special-cased → `CSSStringFns.toCss`;
...
pub mod signfns {
    /// Spec-faithful — note the ±0.0 sign FLIP is
