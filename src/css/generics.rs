...
=======
        // `BabyList::deep_clone_with` takes an arena-aware per-element closure;
        // delegate to *this* trait's `deep_clone` so the bump lifetime threads.
        self.deep_clone_with(|e| e.deep_clone(bump))
>>>>>>> Stashed changes
    }
}
