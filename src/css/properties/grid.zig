...
pub const TrackList = struct {
    ...
    pub fn parse(input: *css.Parser) css.Result(@This()) {
        ...
        while (true) {
            ...
            line_names.append(input.allocator(), line_name) catch |oe| bun.outOfMemory(oe);

            if (input.tryParse(TrackSize.parse, .{}).asValue()) |track_size| {
                // TODO: error handling
                items.append(.{ .track_size = track_size }) catch |oe| bun.outOfMemory(oe);
            } else if (input.tryParse(TrackRepeat.parse, .{}).asValue()) |repeat| {
                // TODO: error handling
                items.append(.{ .track_repeat = repeat }) catch |oe| bun.outOfMemory(oe);
            } else {
                break;
            }
        }
...
pub const TrackSizeList = struct {
    ...
    pub fn parse(input: *css.Parser) css.Result(@This()) {
        ...
        while (input.tryParse(TrackSize.parse, .{}).asValue()) |size| {
            res.append(input.allocator(), size) catch |oe| bun.outOfMemory(oe);
        }
        ...
        return input.parseNestedBlock(struct {
            fn parse(i: *css.Parser) css.Result(TrackRepeat) {
                ...
                while (true) {
                    ...
                    line_names.append(i.allocator(), line_name) catch |oe| bun.outOfMemory(oe);

                    if (input.tryParse(TrackSize.parse, .{}).asValue()) |track_size| {
                        // TODO: error handling
                        track_sizes.append(i.allocator(), track_size) catch |oe| bun.outOfMemory(oe);
                    } else {
                        break;
                    }
                }
    ...
    return input.parseNestedBlock(struct {
        fn parse(i: *css.Parser) css.Result(CustomIdentList) {
            ...
            while (input.tryParse(CustomIdent.parse, .{}).asValue()) |ident| {
                values.append(i.allocator(), ident) catch |oe| bun.outOfMemory(oe);
            }
...
pub const GridTemplateAreas = union(enum) {
    ...
    fn parseString(allocator: Allocator, s: []const u8, tokens: *SmallList(?[]const u8, 1)) bun.Maybe(u32, void) {
        ...
        while (true) {
            ...
            tokens.append(allocator, token) catch |oe| bun.outOfMemory(oe);
