...
/// Discriminant-only tag for [`Property`] / [`PropertyId`] (`u16` discriminant).
#[repr(u16)]
...
impl PropertyIdTag {
    ...
    /// (i.e. is a `(T, VendorPrefix)` tuple in the original union).
    ...
    /// Kebab-case CSS property name (derived from `PropertyIdTag`).
...
// equality (`PropertyId.eql`) ignores the
...
// PORT NOTE: `PropertyId.eql()` compares the
...
// `rules/style.rs:is_duplicate`). `prefix()` already returns the `VendorPrefix`
...
// equality is exactly the original semantics.
...
// PORT NOTE: `PropertyId.hash()` hashes only
...
impl PropertyId {
    ...
    ///
    /// Mirrors `PropertyId.name()`, which is
    ...
    pub fn from_name_and_prefix(name: &[u8], pre: VendorPrefix) -> Option<PropertyId> {
        ...
        // PORT NOTE: the original used a compile-time perfect-hash map; linear scan here is
...
impl Property {
    ...
    /// routed through a no-op `lh!` that mirrors the original fallthrough
    /// (`return null`) until the `DefineShorthand` derive is ported. There
    /// are no callers.
    // blocked_on: shorthand_handler_port — leaf shorthand types lack `.longhand()` (original body is `@compileError(todo_stuff.depth)`)
    pub fn longhand(&self, property_id: &PropertyId) -> Option<Property> {
        ...
        fn lh<T: ?Sized>(_v: &T, _id: &PropertyId) -> Option<Property> {
            ...
            // `@compileError(todo_stuff.depth)` and never instantiated.
...
// originally. Expose it via `Default` so the un-gated stub branch in
