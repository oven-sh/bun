// Hand-maintained Rust port, originally derived from
// src/css/properties/properties_generated.zig. Note: generate_properties.ts
// (`bun run css-properties`) regenerates only the .zig reference file, never
// this .rs file — edits here are safe and will not be clobbered.
//
...
// Zig `PropertyId.eql()` (properties_generated.zig:9195) compares the
...
// Zig `PropertyId.hash()` (properties_generated.zig:9209) hashes only
...
impl PropertyId {
    ...
    pub fn from_name_and_prefix(name: &[u8], pre: VendorPrefix) -> Option<PropertyId> {
        ...
        // Zig used a comptime perfect-hash map; the linear scan here is
        // correct but slower — a phf / match-on-len would be an optimization.
...
impl Property {
    ...
    ///
    /// In Zig (`properties_generated.zig:7087-7160`) each arm
    ...
    pub fn longhand(&self, property_id: &PropertyId) -> Option<Property> {
        ...
        fn lh<T: ?Sized>(_v: &T, _id: &PropertyId) -> Option<Property> {
            // Per-type `v.longhand(property_id)` is
