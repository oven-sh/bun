...
// PERF(port): Phase-A shim. Phase B threads `'bump` and replaces this with
// `crate::generics::ArrayList<'bump, T>` (= `bun_alloc::ArenaVec`) crate-wide
// in one pass.
...
// rule (matching the arena-backed AST). That cascades into
        ...
        impl<R> CssRule<R> {
            pub fn to_css(&self, dest: &mut Printer) -> Result<(), PrintErr> {
                match self {
                    ...
                    // `DefaultAtRule` (whose `to_css` errors unconditionally) and
                    // `TailwindAtRule` (`src/css/rules/tailwind.rs`, used via
                    // `BundlerAtRule` when `ENABLE_TAILWIND_PARSING`), whose `to_css`
...
// shared read-only across the bundler thread pool (the arena-backed AST is
// freely handed between threads). Thread-safety therefore follows
// `R`'s auto-traits.
...
pub(super) mod dc {
    ...
    ///
    /// Exists so call-sites that need an empty block (e.g.
    /// `nested_rule.declarations = Default::default()`) route through ONE
...
/// Port of `DeclarationBlock::to_css_block`. The real impl is gated in
...
pub(super) fn decl_block_to_css(
    ...
    let mut i: usize = 0;
    // Iterate `declarations` then `important_declarations` (manually unrolled).
...
/// Port of `VendorPrefix::to_css`. Lives here because the canonical
/// `impl VendorPrefix` block in lib.rs hasn't grown a `to_css` yet
...
/// so call it here to match the spec.
...
impl<R> CssRuleList<R> {
        ...
        R: for<'b> css::generics::DeepClone<'b>,
    {
        ...
        for rule in self.v.iter_mut() {
            ...
            // so the source slot is replaced with `Ignored` after the arm.
            ...
            'arm: {
                match rule {
                    CssRule::Keyframes(_keyframez) => {
                        ...
                        // Currently a debug-TODO fallthrough.
                    }
                    ...
                    CssRule::Container(_cont) => {
                        // TODO(port): ContainerRule minify — fallthrough.
                    }
                    ...
                    CssRule::LayerStatement(_lay) => {
                        // TODO(port): LayerStatementRule minify — fallthrough.
                    }
                    CssRule::MozDocument(_doc) => {
                        // TODO(port): MozDocumentRule minify — fallthrough.
                    }
                    CssRule::Style(_sty) => {
                    ...
                    CssRule::CounterStyle(_) => { /* TODO(port): fallthrough */ }
                    CssRule::Scope(_) => { /* TODO(port): fallthrough */ }
                    CssRule::Nesting(_) => { /* TODO(port): fallthrough */ }
                    CssRule::StartingStyle(_) => { /* TODO(port): fallthrough */ }
                    CssRule::FontPaletteValues(_) => { /* TODO(port): fallthrough */ }
                    CssRule::Property(_) => { /* TODO(port): fallthrough */ }
                    _ => {}
                }
...
fn minify_style_arm<R: for<'b> css::generics::DeepClone<'b>>(
        ...
        && (!logical.is_empty() || !supps.is_empty() || !incompatible_rules.is_empty())
    {
        ...
        // Empty declaration block — route through the centralized `'bump`-erasure
        // helper instead of fabricating `&'static Arena` here (PORTING.md §Forbidden).
...
///
/// PORT NOTE: a previous design additionally stored a raw
/// `*const Vec<CssRule<R>>` and dereferenced it inside `eql()`.
/// That pattern is unsound under Stacked/Tree Borrows — keys persist
...
pub fn merge_style_rules<R>(
        ...
        && last_style_rule.rules.v.is_empty()
    {
        ...
        if sty.is_compatible(*context.targets) && last_style_rule.is_compatible(*context.targets) {
            ...
            // super-linearly across repeated merges — keeps the N-way merge
            // amortized O(N).
...
// `Location` is a single nominal type. Re-export the crate-root struct so
// `css_rules::Location` and `crate::Location` are interchangeable.
...
///
/// PORT NOTE: `arena` is the AST arena (`&'a Arena`, bumpalo). All
/// sub-allocations during minify go through it so the whole transformed tree
/// is bulk-freed with the arena.
...
pub struct MinifyContext<'a, 'bump> {
    ...
    // PORT NOTE: `selector::is_unused` currently borrows `&ArrayHashMap<&[u8], ()>`; the
