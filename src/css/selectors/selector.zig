        ...
        .negation => |selectors| {
            ...
            if (selectors.len > 1 and css.targets.Targets.shouldCompileSame(&targets, .not_selector_list)) {
                const is: Selector = Selector.fromComponent(allocator, Component{ .is = selectors: {
                    const new_selectors = allocator.alloc(Selector, selectors.len) catch |oe| bun.outOfMemory(oe);
                    for (new_selectors, selectors) |*new, *sel| {
                        new.* = sel.deepClone(allocator);
                    }
                    break :selectors new_selectors;
                } });
                var list = ArrayList(Selector).initCapacity(allocator, 1) catch |oe| bun.outOfMemory(oe);
    ...
    if (!targets.shouldCompileSame(.lang_selector_list)) {
        const c = Component{
            .non_ts_pseudo_class = PseudoClass{
                .lang = .{ .languages = lang: {
                    var list = ArrayList([]const u8).initCapacity(allocator, RTL_LANGS.len) catch |oe| bun.outOfMemory(oe);
        ...
        if (dir == .ltr) return Component{
            .negation = negation: {
                var list = allocator.alloc(Selector, 1) catch |oe| bun.outOfMemory(oe);
...
fn langListToSelectors(allocator: Allocator, langs: []const []const u8) []Selector {
    var selectors = allocator.alloc(Selector, langs.len) catch |oe| bun.outOfMemory(oe);
    for (langs, selectors[0..]) |lang, *sel| {
        sel.* = Selector.fromComponent(allocator, Component{
            .non_ts_pseudo_class = PseudoClass{
                .lang = .{ .languages = langs: {
                    var list = ArrayList([]const u8).initCapacity(allocator, 1) catch |oe| bun.outOfMemory(oe);
