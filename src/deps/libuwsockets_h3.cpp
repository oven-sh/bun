...
bool uws_h3_res_try_end(uws_h3_res_t *res, const char *bytes, size_t len, size_t total_len, bool close) {
  Http3Response *r = (Http3Response *) res;
  auto pair = r->tryEnd(sv(bytes, len), total_len, close);
  if (pair.first) r->clearOnWritableAndAborted();
  return pair.first;
}
...
void uws_h3_res_on_aborted(uws_h3_res_t *res, void (*h)(uws_h3_res_t *, void *), void *opt) {
  if (h) ((Http3Response *) res)->onAborted(opt, (Http3ResponseData::OnAbortedCallback) h);
  else ((Http3Response *) res)->clearOnAborted();
}
void uws_h3_res_on_timeout(uws_h3_res_t *res, void (*h)(uws_h3_res_t *, void *), void *opt) {
  if (h) ((Http3Response *) res)->onTimeout(opt, (Http3ResponseData::OnTimeoutCallback) h);
  else ((Http3Response *) res)->clearOnTimeout();
}
