...
pub const Timer = opaque {
    pub fn create(loop: *Loop, ptr: anytype) *Timer {
        ...
        return us_create_timer(loop, 0, @sizeOf(Type)) orelse std.debug.panic("us_create_timer: returned null: {d} -- limit: {d}", .{ std.c._errno(), bun.fs.FileSystem.RealFS.Limit.handles });
    }
    ...
    pub fn createFallthrough(loop: *Loop, ptr: anytype) *Timer {
        ...
        // the problem is uSockets hardcodes it on the other end
        // so we can never free non-fallthrough polls
        return us_create_timer(loop, 1, @sizeOf(Type)) orelse std.debug.panic("us_create_timer: returned null: {d} -- limit: {d}", .{ std.c._errno(), bun.fs.FileSystem.RealFS.Limit.handles });
    }
