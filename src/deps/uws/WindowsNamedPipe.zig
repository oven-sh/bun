...
fn onReadAlloc(this: *WindowsNamedPipe, suggested_size: usize) []u8 {
    ...
    if (available.len < suggested_size) {
        this.incoming.ensureUnusedCapacity(bun.default_allocator, suggested_size) catch |oe| bun.outOfMemory(oe);
    ...
    if (data) |bytes| {
        if (bytes.len > 0) {
            ...
            if (this.flags.disconnected) {
                ...
                this.writer.outgoing.write(bytes) catch |oe| bun.outOfMemory(oe);
            } else {
