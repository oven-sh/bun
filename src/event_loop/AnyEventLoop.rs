...
/// `jsc::EventLoop` / `jsc::VirtualMachine` directly. The original implementation
/// had no crate boundaries and just called `this.js.tick()` etc. inline. To match
...
// PORT NOTE: variant order/discriminant must match `crate::EventLoopKind`.
...
// PORT NOTE: originally an associated decl `Task = AnyTaskWithExtraContext`.
...
impl<'a> AnyEventLoop<'a> {
    ...
    /// [`EventLoopHandle::from_any`] kept for shape parity — callers that
    /// originally spelled `jsc.EventLoopHandle.init(any_loop)` now spell it
    /// `AnyEventLoop::as_handle(any_loop)`.
    #[inline]
    ...
    pub fn init() -> AnyEventLoop<'a> {
        // PORT NOTE: original took an allocator param; dropped per §Allocators (non-AST crate).
        AnyEventLoop::Mini(MiniEventLoop::init())
    }

    /// Construct the `Js` variant wrapping a specific erased
    /// `*mut jsc::EventLoop`. Mirrors the `.{ .js = vm.eventLoop() }`
    ...
    pub fn js(js_event_loop: *mut ()) -> AnyEventLoop<'static> {
        // SAFETY: caller passes a live erased `*mut jsc::EventLoop`
    ...
    // PORT NOTE: original took a generic `context` and erased the fn-ptr
    // type at the call into `mini.tick(ctx, fn(*anyopaque) -> bool)`. All
    ...
    /// while the callback runs. The original `jsc.EventLoop.tick` had no such
    /// constraint because its raw pointers were non-exclusive.
    ///
    /// # Safety
        ...
        while !is_done(context) {
            ...
            match unsafe { &mut *this } {
                ...
                AnyEventLoop::Mini(mini) => {
                    ...
                    // next `is_done` reborrow. Spec: `MiniEventLoop::tick`
                    // loop body.
    ...
    pub fn enqueue_task_concurrent<Context, ParentContext>(
        ...
        // Original param was a compile-time field selector — replaced per
        ...
        match self {
            AnyEventLoop::Js { .. } => {
                ...
                // Intentionally unreachable: the JS arm has never implemented this.
...
pub enum EventLoopHandle {
    ...
    // a raw `*MiniEventLoop`. The pointee is the per-thread singleton
...
/// `EventLoopHandle` discriminant — `EventLoopTaskPtr` is an untagged union.
#[derive(Copy, Clone)]
...
/// Owned storage for either kind of concurrent task (`EventLoopTask`).
...
impl EventLoopHandle {
    ...
    // PORT NOTE: original generic `init` dispatched by argument type over five input
    ...
    /// (`parent_tag` / `parent_ptr`). Tag 1 = JS, tag 2 = mini — matches
    /// `setParentEventLoop`.
    #[inline]
    ...
    /// in `uws::InternalLoopData` (`loop.internal_loop_data.getParent()`).
    ///
...
impl EventLoopHandle {
    /// Equivalent of `loop.internal_loop_data.setParentEventLoop(EventLoopHandle::init(..))`.
    ...
    pub fn env(self) -> *mut DotEnvLoader<'static> {
        match self {
            ...
            // Caller invariant: `mini.env` must be set. `env_ptr()` takes
            // `&self` and returns `Option<NonNull<DotEnvLoader>>` (mutable
            // provenance; the field is a nullable `*DotEnvLoader`). Safe via
            // `BackRef: Deref`.
    ...
    pub fn create_null_delimited_env_map(
        ...
        match self {
            ...
            EventLoopHandle::Mini(mini) => {
                ...
                // Caller invariant: `mini.env` must be set.
    ...
    // PORT NOTE: original `cast(tag)` returned a type at compile time — no Rust
    // equivalent. Callers should pattern-match the enum directly.
    // PORT NOTE: original `allocator()` dropped per §Allocators (non-AST crate).
}
