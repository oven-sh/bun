...
pub struct AnyTask {
    ...
    /// worker-terminate drain (JS thread, VM alive, so plain drop suffices).
...
impl AnyTask {
    ...
    /// [`Self::from_typed`] plus a release for `ctx` if the task is reclaimed
    /// unrun by the worker-terminate drain (see the [`Self::dispose`] field).
    #[inline]
