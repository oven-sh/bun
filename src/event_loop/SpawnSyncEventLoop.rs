...
// `jsc::EventLoop` / `jsc::VirtualMachine`. The original implementation did
...
/// and restores the prior value on drop (mirrors the original
...
pub struct SpawnSyncEventLoop {
    ...
    // `&mut self` (LLVM `noalias`). The original raw pointers freely aliased; in Rust the field
    // must be interior-mutable so the re-entrant write is sound under Stacked Borrows.
...
mod handler {
    ...
    /// Adapter for `uws::Loop::create<H: LoopHandler>()` — the original
    /// compile-time `Handler` with `wakeup`/`pre`/`post` decls maps to a trait
...
impl SpawnSyncEventLoop {
    ...
    // The caller passes uninitialized storage, hence `MaybeUninit<Self>` (out-param ctor exception).
    ...
    pub fn init(
        ...
        // PORT NOTE: original passed a compile-time `Handler` type with wakeup/pre/post decls.
        ...
        this.write(Self {
            uws_loop: loop_,
            original_event_loop_handle: None, // originally uninitialized; overwritten in `prepare`
            #[cfg(windows)]
            uv_timer: None,
...
impl Drop for SpawnSyncEventLoop {
    fn drop(&mut self) {
        ...
        // PORT NOTE: original order was `event_loop.deinit()` then `uws_loop.deinit()`.
...
extern "C" fn on_uv_timer(timer_: *mut libuv::Timer) {
    ...
    // `&mut uws::Loop` (Loop::tick_with_timeout takes `&mut self`). The original raw pointers
    // freely aliased, but in Rust we must not:
...
impl SpawnSyncEventLoop {
    ...
    pub fn tick_with_timeout(&mut self, timeout: Option<&Timespec>) -> TickState {
        ...
        // PORT NOTE: original used `defer` to restore at scope exit; RAII Drop mirrors that.
