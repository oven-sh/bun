...
//!
//! `HashObject` exposes this via `hashWrap(Adler32)`, which calls the
//! single-argument `hash(input)` (no seed) — the JS-side seed argument is
//! ignored for Adler32.
    ...
    pub fn permute(state: u32, input: &[u8]) -> u32 {
        ...
        if input.len() == 1 {
            ...
            while i + Self::NMAX <= input.len() {
                ...
                while rounds < N {
                    // Inner 16-step block — rely on the optimizer to unroll.
                    for j in 0..16usize {
