        ...
        if (BoringSSL.SSL_get_peer_cert_chain(sslPtr)) |cert_chain| {
            if (BoringSSL.sk_X509_value(cert_chain, 0)) |x509| {
                ...
                if (client.signals.get(.cert_errors)) {
                    ...
                    const cert = bun.default_allocator.alloc(u8, @intCast(cert_size)) catch |oe| bun.outOfMemory(oe);
                    ...
                    client.state.certificate_info = .{
                        .cert = cert,
                        .hostname = bun.default_allocator.dupe(u8, hostname) catch |oe| bun.outOfMemory(oe),
                        .cert_error = .{
                            .error_no = certError.error_no,
                            .code = bun.default_allocator.dupeZ(u8, certError.code) catch |oe| bun.outOfMemory(oe),
                            .reason = bun.default_allocator.dupeZ(u8, certError.reason) catch |oe| bun.outOfMemory(oe),
                        },
                    };
...
fn writeToSocketWithBufferFallback(comptime is_ssl: bool, socket: NewHTTPContext(is_ssl).HTTPSocket, buffer: *bun.io.StreamBuffer, data: []const u8) !usize {
    ...
    if (amount < data.len) {
        buffer.write(data[@intCast(amount)..]) catch |oe| bun.outOfMemory(oe);
    }
    return amount;
}
...
fn writeToStreamUsingBuffer(this: *HTTPClient, comptime is_ssl: bool, socket: NewHTTPContext(is_ssl).HTTPSocket, buffer: *bun.io.StreamBuffer, data: []const u8) !bool {
    if (buffer.isNotEmpty()) {
        ...
        if (amount < to_send.len) {
            ...
            if (data.len > 0) {
                buffer.write(data) catch |oe| bun.outOfMemory(oe);
            }
...
inline fn handleShortRead(
    ...
    if (needs_move) {
        ...
        if (to_copy.len > 0) {
            ...
            this.state.response_message_buffer.append(to_copy) catch |oe| bun.outOfMemory(oe);
        }
    }
...
pub fn handleOnDataHeaders(
    ...
    if (this.state.response_message_buffer.list.items.len > 0) {
        ...
        this.state.response_message_buffer.appendSliceExact(incoming_data) catch |oe| bun.outOfMemory(oe);
