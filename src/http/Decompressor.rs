...
// of the output buffer (and a `&'a [u8]` of the input). We erase the borrow to
// `'static` and uphold the invariant that the reader never outlives the
...
impl Decompressor {
    // PORT NOTE: the boxed readers' `Drop` impls call `end()`, so an explicit
    // `Drop` is unnecessary. Callers that want a mid-lifecycle reset assign
    ...
    pub fn update_buffers(
            ...
            match encoding {
                Encoding::Gzip | Encoding::Deflate => {
                    let reader = ZlibReaderArrayList::init_with_options_and_list_allocator(
                        input,
                        out,
                        // PORT NOTE: explicit allocator args dropped per §Allocators.
                        bun_zlib::Options {
                            // zlib.MAX_WBITS = 15
                ...
                Encoding::Brotli => {
                    let reader = BrotliReaderArrayList::new_with_options(
                        input,
                        out,
                        // PORT NOTE: explicit allocator arg dropped per §Allocators.
                        Default::default(),
                    )?;
        ...
        match self {
            Decompressor::Zlib(reader) => {
                ...
                // PORT NOTE: the spare region must be writable so the zlib
                // output pointers can write into it while `read_all` later
                // truncates back to `total_out`. `read_all`'s
                ...
                // PORT NOTE: the reader keeps a `&mut Vec<u8>` rather than
                // aliasing the buffer header by value; re-seat it in case the
                // response buffer
            ...
            Decompressor::Brotli(reader) => {
                ...
                reader.input = input;
                reader.total_in = 0;
                // PORT NOTE: re-seat list_ptr instead of aliasing the buffer header.
                reader.list_ptr = out;
