...
pub enum Decompressor {
    Zlib(bun_sys::zlib::InflateDecoder),
    Brotli(Box<bun_sys::brotli::StreamingDecoder>),
    Zstd(Box<bun_sys::zstd::StreamingDecoder>),
    #[default]
    None,
}
        ...
        match encoding {
            Encoding::Gzip | Encoding::Deflate => {
                ...
                let window_bits = if encoding == Encoding::Gzip {
                    bun_sys::zlib::MAX_WBITS | 16
                } else if first_chunk.len() > 1 && first_chunk[0] == 120 {
                    0
                } else {
                    -bun_sys::zlib::MAX_WBITS
                };
                *self = Decompressor::Zlib(bun_sys::zlib::InflateDecoder::new(window_bits)?);
            }
            Encoding::Brotli => {
                *self = Decompressor::Brotli(Box::new(bun_sys::brotli::StreamingDecoder::new(
                    &Default::default(),
                )?));
            }
            Encoding::Zstd => {
                *self = Decompressor::Zstd(Box::new(bun_sys::zstd::StreamingDecoder::new()?));
            }
