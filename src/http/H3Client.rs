...
// Single `Output.scoped(.h3_client, .hidden)` for the whole module tree. The
// four submodules share one `really_disable`/`is_visible_once`/lock; declaring
// it once here and importing in each child preserves that behavior.
...
// PORT NOTE: keep `live_sessions`/`live_streams` snake_case so cross-crate
// readers (`bun_http_jsc`) and the gated submodules see the same identifier;
// SCREAMING_SNAKE aliases preserved for existing internal references.
...
// PORT NOTE: a `TestingAPIs` re-export of `H3TestingAPIs` was deleted per
// PORTING.md — *_jsc aliases are dropped; H3TestingAPIs lives in
