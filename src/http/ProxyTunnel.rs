...
// PORT NOTE: inherent associated types are unstable in Rust, so the free
// `HTTPSocket<SSL>` alias from http_context is used instead.
// `HTTPSocket<B>` = `uws::SocketHandler<B>` = `NewSocketHandler<B>`,
...
impl Drop for ProxyTunnel {
    fn drop(&mut self) {
        // wrapper / write_buffer cleanup is handled by their own Drop impls;
...
fn on_open(ctx: *mut HTTPClient) {
    ...
    if let Some(ssl_ptr) = ProxyTunnel::wrapper_ssl(proxy_nn) {
        ...
        // PORT NOTE: `configureHTTPClient` is `configureHTTPClientWithALPN(ssl, host, .h1)`;
        // the ALPN form is exposed in `crate::configure_http_client_with_alpn`.
...
fn on_handshake(
    ...
    if handshake_success {
        ...
        if this.flags.reject_unauthorized {
            ...
            // Assert wrapper-is-Some, but silently bail if ssl is None — the
            // two cases are intentionally split.
...
impl ProxyTunnel {
    pub fn start<const IS_SSL: bool>(
        ...
        // into the client field; no bump — `RefPtr::adopt_ref` takes ownership
        // of the existing ref.
    ...
    pub fn on_writable<const IS_SSL: bool>(&mut self, socket: HTTPSocket<IS_SSL>) {
        ...
        // PORT NOTE: `wrapper.flush()` must run AFTER the body but BEFORE the
        // ref-guard deref. We sequence that explicitly at the single
        ...
        // _guard derefs here (last in, first out).
    }
    ...
    pub fn receive(&mut self, buf: &[u8]) {
        ...
        // _guard derefs here (last in, first out); `self_ptr` provenance
    ...
    pub fn detach_and_deref(&mut self) {
        // detachSocket() must run BEFORE deref() — if refcount > 1 the tunnel
    ...
    pub fn adopt<const IS_SSL: bool>(
        ...
        // HTTPContext which moves the parked ref into the new client.
