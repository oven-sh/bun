...
impl Callback {
    pub fn init<T>(callback: fn(*mut T), context: *mut T) -> Self {
        Self {
            ...
            // `context` is only ever passed back to this callback, which knows
            // its real type.
...
impl ThreadSafeStreamBuffer {
    ...
    pub fn acquire(&mut self) -> &mut StreamBuffer {
        self.mutex.lock();
        // PORT NOTE: returns &mut self.buffer while the mutex stays locked
        // until `release()`. Prefer `lock()` (RAII guard) for simple critical
        // sections; this split form remains for callers that interleave
        // release with disjoint `self` access.
        &mut self.buffer
    }
