...
//! See `src/http/H2Client.rs` for the module-level overview.
...
// PORT NOTE: 31/24-bit fields widened to u32; range asserts at use sites.
...
impl ClientSession {
    ...
    fn replenish_window(&mut self) {
        ...
        // PERF(port): could iterate and write in one pass; profile if extra Vec matters.
    }
    ...
    fn deliver_stream(&mut self, stream_ptr: *mut Stream) -> bool {
        ...
        if stream.headers_ready {
            ...
            client.h2_clone_metadata();
            // Mirror the h1 path (handleOnDataHeaders): deliver headers
