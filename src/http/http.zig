...
pub const TestingAPIs = @import("../http_jsc/headers_jsc.zig").HTTPTestingAPIs;
...
pub fn onData(
    ...
    switch (this.state.response_stage) {
        ...
        .body => {
            ...
            if (!this.state.flags.receive_paused) this.setTimeout(socket);
...
fn maybePauseReceive(this: *HTTPClient, comptime is_ssl: bool, socket: NewHTTPContext(is_ssl).HTTPSocket, n: usize) void {
    ...
    // `socket.setTimeout(0)` clears both the short-tick and long-minute
    // timers (whichever `idle_timeout_seconds` selected).
    socket.setTimeout(0);
