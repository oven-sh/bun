...
impl<'a> HTTPClient<'a> {
    ...
    fn send_progress_update_without_stage_check<const IS_SSL: bool>(
        ...
        let has_more = self.send_progress_update_inner(|this| {
            ...
            if core::mem::take(&mut this.state.flags.receive_paused)
                && !socket.is_closed_or_has_error()
            {
                let _ = socket.resume_stream();
            }
    ...
    /// result to the callback. Returns `has_more` so the HTTP/1.1 caller can
    /// apply receive backpressure after the callback has consumed the chunk.
    fn send_progress_update_inner(&mut self, release_transport: impl FnOnce(&mut Self)) -> bool {
    ...
    fn apply_redirect_url(&mut self, new_href: Vec<u8>) -> Result<bool, bun_core::Error> {
