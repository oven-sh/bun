...
// Conceptually a 4-bit enum. Rust has no u4 repr; values are 0x0..=0xF so u8 is layout-safe.
#[repr(u8)]
...
impl Opcode {
    ...
    /// Convert a raw nibble to an `Opcode`. Caller must guarantee `n <= 0xF`;
...
// Bit-packed u16 with non-bool fields → #[repr(transparent)] u16 + manual shift accessors.
...
impl WebsocketHeader {
    ...
    /// Construct with default reserved bits
    ...
    pub const fn compressed(self) -> bool {
        (self.0 >> Self::COMPRESSED_SHIFT) & 1 != 0
    }
    #[inline]
    pub fn set_compressed(&mut self, v: bool) {
        self.0 =
            (self.0 & !(1u16 << Self::COMPRESSED_SHIFT)) | ((v as u16) << Self::COMPRESSED_SHIFT);
    }
