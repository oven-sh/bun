...
/// `Method.fromJS` (the `ComptimeStringMap` JSC bridge). Converts a JS string
...
pub fn from_js(global_this: &JSGlobalObject, input: JSValue) -> JsResult<Option<Method>> {
    ...
    // PORT NOTE: the phf table is keyed on `&[u8]`, so materialize UTF-8 and
    // call the existing `Method::which` (which wraps the same map lookup).
