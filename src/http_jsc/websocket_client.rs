...
pub struct WebSocket<const SSL: bool> {
    ...
    /// `pause()` is in effect: no frame is parsed, and bytes that are already
    /// in memory wait in `held_data` for `resume()`.
    paused: Cell<bool>,
    /// Received bytes the frame parser has not seen yet: the handshake
    /// overflow, and the rest of a read whose handler called `pause()`.
    held_data: RefCell<HeldData>,
...
impl<const SSL: bool> WebSocket<SSL> {
    ...
    pub fn handle_data(this: ThisPtr<Self>, data_: &[u8]) {
        ...
        if !this.held_data.borrow().is_empty() {
        ...
        let unparsed = if this.paused.get() {
            data_
        } else {
            this.handle_data_loop(data_)
        };
        if !unparsed.is_empty() {
            this.held_data.borrow_mut().push(unparsed);
        }
    }

    /// Parse `held_data` unless `pause()` is in effect. A handler that pauses
    /// again leaves the rest held.
    fn parse_held_data(&self) {
        ...
        if self.paused.get() || self.close_received.get() {
            return;
        }
        let mut held = self.held_data.take();
        if held.is_empty() {
            return;
        }
        ...
        if self.cpp_websocket().is_none() || !is_connected {
            return;
        }

        let unparsed_len = self.handle_data_loop(held.unparsed()).len();
        if unparsed_len == 0 {
            return;
        }
        held.offset = held.bytes.len() - unparsed_len;
        // Bytes that arrived while the handlers ran are newer than the rest of `held`.
        let newer = self.held_data.take();
        held.bytes.extend_from_slice(newer.unparsed());
        self.held_data.replace(held);
    }
    ...
    /// Returns the bytes left unparsed because a handler called `pause()`.
    fn handle_data_loop<'a>(&self, data: &'a [u8]) -> &'a [u8] {
        ...
        let terminated = loop {
            ...
            match step {
                // A handler called `pause()`: the rest of this read waits for `resume()`.
                Step::Continue if self.paused.get() => break false,
                Step::Continue => {}
                Step::NeedMoreData => {
                    cursor.data = &[];
                    break false;
    ...
    pub(crate) fn resume(this: ThisPtr<Self>) -> bool {
        this.paused.set(false);
        Self::schedule_held_data(this);
        match this.tunnel() {
            Some(tunnel) => tunnel.resume_stream(),
            None => this.tcp.get().resume_stream(),
        }
    }
    ...
    fn new_raw(
        ...
        let ws = RefPtr::new(WebSocket::<SSL> {
            ...
            held_data: RefCell::new(HeldData::default()),
    ...
    fn finish_init(
        ...
        if let Some(buffered_data) = buffered_data {
            ws.held_data.replace(HeldData {
                bytes: buffered_data.0,
                offset: 0,
            });
    ...
    pub(crate) fn memory_cost(&self) -> usize {
        ...
        cost += self
            .held_data
            .try_borrow()
            .map_or(0, |held| held.bytes.capacity());
...
pub fn bun__websocketclienttls__resume(
    ...
    WebSocketClientTLS::resume(this)
}
...
/// `bytes[offset..]` waits for the frame parser. A handler that pauses on
/// every message moves `offset` instead of copying the rest of the read.
#[derive(Default)]
struct HeldData {
    bytes: Vec<u8>,
    offset: usize,
