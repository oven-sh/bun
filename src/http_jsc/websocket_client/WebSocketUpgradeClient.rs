...
impl<const SSL: bool> HTTPClient<SSL> {
    ...
    /// Forward the parsed handshake response (status, headers, and optional
    /// body) to the C++ `WebSocket` so the `ws` shim can emit `upgrade` (on a
    /// 101) or `unexpected-response` (otherwise). The C++ side is a no-op when
    /// no `handshake` listener is registered, so the browser-style
    /// `new WebSocket()` path pays nothing.
    ///
    /// The header name/value slices and `response.status` borrow the same
    /// buffer `response` was parsed from; the caller keeps that buffer alive
    /// for the duration of this synchronous dispatch. `body` is copied into a
    /// JS Uint8Array by the C++ side before any user JS runs.
    ...
    pub unsafe fn process_response(
        ...
        if response.status_code != 101 {
            // Forward the parsed status/headers/body to JS so the `ws` shim can
            // emit `unexpected-response` before the connection is torn down.
            // The dispatch runs user JS synchronously; a `handshake` listener
            // that calls `ws.close()`/`ws.terminate()` reenters `cancel()`,
            // which takes `outgoing_websocket` and detaches `tcp`, so
            // `terminate()` below becomes a no-op (it finds `outgoing_websocket
            // == None` and `tcp` detached). `handle_data` holds a ref around
            // this call, keeping `this` alive for the trailing `terminate`.
            // SAFETY: short-lived read of `outgoing_websocket`.
            if let Some(ws) = unsafe { (*this).outgoing_websocket } {
                Self::dispatch_handshake_response(ws, &response, remain_buf);
            }
            // SAFETY: no `&mut Self` is live across this call.
        ...
        // SAFETY: short-lived read of `outgoing_websocket`.
        if let Some(ws) = unsafe { (*this).outgoing_websocket } {
            Self::dispatch_handshake_response(ws, &response, &[]);
        }
