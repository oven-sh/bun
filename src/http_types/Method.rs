...
// PORT NOTE: the private `with_body`/`with_request_body` EnumSet consts are
...
impl Method {
    /// Uppercase HTTP method token. `M_SEARCH`
    /// renders as `"M-SEARCH"` (the wire form).
    ...
    /// Length-gated lookup, then a
    /// flat byte-pattern match on the entries of that exact length. A previous
    /// implementation used a `phf::Map`,
    ...
    ///
    /// `#[inline]`: the lookup should be fully
...
mod tests {
    ...
    fn which_roundtrip() {
        ...
        // Mixed case must reject (the table has only all-upper / all-lower).
