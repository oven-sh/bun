...
pub const Pixel = struct {
    ...
    pub fn toGray(self: Pixel) u8 {
        ...
        return @intFromFloat(
            0.2126 * @as(f32, @floatFromInt(self.r)) + 
...
pub fn convertPortion(
    ...
    while (row < end_row) : (row += 1) {
        ...
        try convertRow(
            src[src_offset..src_offset+src_row_size],
            src_format,
            dest[dest_offset..dest_offset+dest_row_size],
            dest_format,
            width
        );
    }
...
pub fn convertSIMD(
    ...
    if ((src_format == .RGB or src_format == .BGR) and dest_format == .Gray) {
        ...
        while (i < pixels) : (i += 1) {
            ...
            const gray_value = @as(u8, @intCast(
                (r_weight * @as(i32, @intCast(r)) + 
...
pub fn unpremultiplyAlpha(
    ...
    while (i < width * height) : (i += 1) {
        ...
        if (alpha > 0) {
            ...
            if (format == .GrayAlpha) {
                // Special case for grayscale+alpha
                const value = @as(u8, @intFromFloat(@min(
                    @as(f32, @floatFromInt(src[offset + positions.r])) * alpha_f,
                    255.0
                )));
                dest[offset + positions.r] = value;
            } else {
                // Regular case for color with alpha
                dest[offset + positions.r] = @as(u8, @intFromFloat(@min(
                    @as(f32, @floatFromInt(src[offset + positions.r])) * alpha_f,
                    255.0
                )));
                dest[offset + positions.g] = @as(u8, @intFromFloat(@min(
                    @as(f32, @floatFromInt(src[offset + positions.g])) * alpha_f,
                    255.0
                )));
                dest[offset + positions.b] = @as(u8, @intFromFloat(@min(
                    @as(f32, @floatFromInt(src[offset + positions.b])) * alpha_f,
                    255.0
                )));
            }
        }
