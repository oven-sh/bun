...
pub fn initializeMiniStore() void {
    const MiniStore = struct {
        heap: bun.heap.MimallocArena,
        memory_allocator: JSAst.ASTMemoryAllocator,

        pub threadlocal var instance: ?*@This() = null;
    };
    if (MiniStore.instance == null) {
        var mini_store = bun.heap.default_allocator.create(MiniStore) catch bun.outOfMemory();
        mini_store.* = .{
            .heap = bun.heap.MimallocArena.init() catch bun.outOfMemory(),
    ...
    pub fn callback(task: *ThreadPool.Task) void {
            ...
            if (this.status == .success) {
                if (this.apply_patch_task) |pt| {
                    ...
                    if (pt.callback.apply.logger.errors > 0) {
                        ...
                        // this.log.addErrorFmt(null, logger.Loc.Empty, bun.heap.default_allocator, "failed to apply patch: {}", .{e}) catch unreachable;
        ...
        switch (this.tag) {
            .package_manifest => {
                ...
                defer {
                    bun.heap.default_allocator.free(body);
                }
...
pub fn NewPackageInstall(comptime kind: PkgInstallKind) type {
    ...
    return struct {
        ...
        fn verifyGitResolution(
            ...
            var git_tag_stack_fallback = std.heap.stackFallback(2048, bun.heap.default_allocator);
        ...
        const HardLinkWindowsInstallTask = struct {
            ...
            pub fn init(src: []const bun.OSPathChar, dest: []const bun.OSPathChar, basename: []const bun.OSPathChar) *@This() {
                ...
                const combined = bun.heap.default_allocator.alloc(u16, allocation_size) catch bun.outOfMemory();
            ...
            pub fn deinit(task: *@This()) void {
                bun.heap.default_allocator.free(task.bytes);
                task.destroy();
            }
                ...
                .result => {
                    ...
                    const UninstallTask = struct {
                        ...
                        pub fn deinit(uninstall_task: *@This()) void {
                            bun.heap.default_allocator.free(uninstall_task.absolute_path);
                    ...
                    var task = UninstallTask.new(.{
                        .absolute_path = bun.heap.default_allocator.dupeZ(u8, bun.path.joinAbsString(FileSystem.instance.top_level_dir, &.{ this.node_modules.path.items, temp_path }, .auto)) catch bun.outOfMemory(),
                    });
...
const DependencyInstallContext = struct {
    ...
    path: std.ArrayList(u8) = std.ArrayList(u8).init(bun.heap.default_allocator),
    dependency_id: DependencyID,
};
...
const PackageManifestMap = struct {
    ...
    pub fn insert(this: *PackageManifestMap, name_hash: PackageNameHash, manifest: *const Npm.PackageManifest) !void {
        try this.hash_map.put(bun.heap.default_allocator, name_hash, .{ .manifest = manifest.* });
    }
    ...
    pub fn byNameHashAllowExpired(
        ...
        const entry = this.hash_map.getOrPut(bun.heap.default_allocator, name_hash) catch bun.outOfMemory();
...
pub const PackageManager = struct {
    ...
    network_dedupe_map: NetworkTask.DedupeMap = NetworkTask.DedupeMap.init(bun.heap.default_allocator),
    ...
    total_tasks: u32 = 0,
    preallocated_network_tasks: PreallocatedNetworkTasks = PreallocatedNetworkTasks.init(bun.heap.default_allocator),
    preallocated_resolve_tasks: PreallocatedTaskStore = PreallocatedTaskStore.init(bun.heap.default_allocator),
    ...
    pub const WorkspacePackageJSONCache = struct {
        ...
        pub fn getWithPath(
            ...
            entry.value_ptr.* = .{
                .root = json.root.deepClone(bun.heap.default_allocator) catch bun.outOfMemory(),
                .source = source,
    ...
    pub fn configureEnvForScripts(this: *PackageManager, ctx: Command.Context, log_level: Options.LogLevel) !*transpiler.Transpiler {
...

        {
            ...
            if (this.env.getNodePath(this_transpiler.fs, &node_path)) |node_pathZ| {
                _ = try this.env.loadNodeJSConfig(this_transpiler.fs, bun.heap.default_allocator.dupe(u8, node_pathZ) catch bun.outOfMemory());
            } else brk: {
                const current_path = this.env.get("PATH") orelse "";
                var PATH = try std.ArrayList(u8).initCapacity(bun.heap.default_allocator, current_path.len);
                ...
                _ = try this.env.loadNodeJSConfig(this_transpiler.fs, bun.heap.default_allocator.dupe(u8, bun_path) catch bun.outOfMemory());
            }
        }
    ...
    pub inline fn getTemporaryDirectory(this: *PackageManager) std.fs.Dir {
        return this.temp_dir_ orelse brk: {
            ...
            this.temp_dir_path = bun.heap.default_allocator.dupeZ(u8, temp_dir_path) catch bun.outOfMemory();
    ...
    pub fn ensureTempNodeGypScript(this: *PackageManager) !void {
        ...
        var PATH = try std.ArrayList(u8).initCapacity(bun.heap.default_allocator, existing_path.len + 1 + this.temp_dir_name.len + 1 + this.node_gyp_tempdir_name.len);
    ...
    pub fn allocatePackageManager() void {
        Holder.ptr = bun.heap.default_allocator.create(PackageManager) catch bun.outOfMemory();
    }
                    ...
                    if (comptime @TypeOf(callbacks.onExtract) != void) {
                        if (ptask.callback.apply.task_id) |task_id| {
                            ...
                            if (ptask.callback.apply.install_context) |*ctx| {
                                ...
                                const path = ctx.path;
                                ctx.path = std.ArrayList(u8).init(bun.heap.default_allocator);
    ...
    pub fn init(
        ...
        const root_package_json_file = root_package_json_file: {
                        ...
                        if (json.asProperty("workspaces")) |prop| {
                            ...
                            for (workspace_names.keys(), workspace_names.values()) |path, entry| {
                                ...
                                if (strings.eqlLong(maybe_workspace_path, path, true)) {
                                    fs.top_level_dir = try bun.heap.default_allocator.dupeZ(u8, parent);
                                    found = true;
                                    child_json.close();
                                    if (comptime Environment.isWindows) {
            ...
            fs.top_level_dir = try bun.heap.default_allocator.dupeZ(u8, child_cwd);
        ...
        manager.* = PackageManager{
            ...
            // .progress
            .event_loop = .{
                .mini = JSC.MiniEventLoop.init(bun.heap.default_allocator),
            },
    ...
    pub const UpdateRequest = struct {
        ...
        pub fn fromJS(globalThis: *JSC.JSGlobalObject, input: JSC.JSValue) bun.JSError!JSC.JSValue {
            ...
            const update_requests = parseWithError(allocator, null, &log, all_positionals.items, &array, .add, false) catch {
                return globalThis.throwValue(log.toJS(globalThis, bun.heap.default_allocator, "Failed to parse dependencies"));
            };
        ...
        fn parseWithError(
            ...
            outer: for (positionals) |positional| {
                var input: []u8 = bun.heap.default_allocator.dupe(u8, std.mem.trim(u8, positional, " \n\r\t")) catch bun.outOfMemory();
                {
    ...
    fn pathArgumentRelativeToRootWorkspacePackage(manager: *PackageManager, lockfile: *const Lockfile, argument: []const u8) ?[]const u8 {
        ...
        return bun.heap.default_allocator.dupe(u8, bun.path.join(&[_][]const u8{ rel_path, argument }, .posix)) catch bun.outOfMemory();
    }
    ...
    fn doPatchCommit(
        ...
        const patchfile_contents = brk: {
            ...
            const paths = bun.patch.gitDiffPreprocessPaths(bun.heap.default_allocator, old_folder, new_folder, false);
    ...
    pub const PackageInstaller = struct {
        ...
        pub fn fixCachedLockfilePackageSlices(this: *PackageInstaller) void {
            ...
            if (this.successfully_installed.bit_length < this.lockfile.packages.len) {
                const new = Bitset.initEmpty(bun.heap.default_allocator, this.lockfile.packages.len) catch bun.outOfMemory();
                var old = this.successfully_installed;
                defer old.deinit(bun.heap.default_allocator);
                old.copyInto(new);
                ...
                differ: {
                    ...
                    if (had_any_diffs) {
                        ...
                        const all_name_hashes: []PackageNameHash = brk: {
                            ...
                            var all_name_hashes = try bun.heap.default_allocator.alloc(PackageNameHash, hashes_len);
    ...
    pub fn spawnPackageLifecycleScripts(
        ...
        var PATH = try std.ArrayList(u8).initCapacity(bun.heap.default_allocator, original_path.len + 1 + "node_modules/.bin".len + cwd.len + 1);
