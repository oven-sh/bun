...
pub const InstallResult = union(enum) {
    ...
    /// paths in src/runtime/cli/*.zig before manager init).
    ...
    /// ONLY for use in src/runtime/cli/*.zig — never call from src/install/.
    ...
    /// the first error's exit code. ONLY for use in src/runtime/cli/*.zig.
