pub const PackageInstall = struct {
    ...
    const HardLinkWindowsInstallTask = struct {
        ...
        pub fn init(src: []const bun.OSPathChar, dest: []const bun.OSPathChar, basename: []const bun.OSPathChar) *@This() {
            ...
            const combined = bun.default_allocator.alloc(u16, allocation_size) catch |oe| bun.outOfMemory(oe);
        ...
        switch (bun.sys.renameat(
            ...
            .result => {
                ...
                var task = UninstallTask.new(.{
                    .absolute_path = bun.default_allocator.dupeZ(u8, bun.path.joinAbsString(FileSystem.instance.top_level_dir, &.{ this.node_modules.path.items, temp_path }, .auto)) catch |oe| bun.outOfMemory(oe),
                });
