...
pub fn enforce_block_exotic_subdeps(manager: &PackageManager) -> usize {
    ...
    for _parent_id in 0..pkgs.len() {
        ...
        for _dep_id in parent_deps.begin()..parent_deps.end() {
            ...
            // lookup would have hit with the SAME key. `PackageManagerEnqueue`
            // looks up `overrides.get(hash(realname()))` and skips the lookup
            // entirely for workspace-behavior deps and npm aliases. Our lookup
            // below keys on `dep.name_hash` (i.e. `hash(dep.name)`), so it
            // only matches the resolver when `realname() == dep.name` — which
            // is exactly `!dep.is_aliased()`. `is_aliased()` is true for:
            //   * npm / dist_tag aliases (`"foo": "npm:bar@..."` →
            //     `realname() == "bar" != "foo"`), where the resolver keys on
            //     the alias target (and for `.npm` skips overrides outright),
            //   * git / github / tarball specifiers, whose `package_name`
            //     `parse_with_tag` leaves empty until `runTasks` backfills it
            //     after the fetch, so `realname()` differs from `dep.name`.
            // For every other tag `realname() == dep.name`, so the resolver's
            // key equals ours and the override IS applied.
            //
            // Using the specifier (not `dep_res_tag`) matters for overrides
            // whose *target* is exotic: a parent that wrote `"foo":
            // "file:../bad"` with a root override `{foo: "git+https://..."}`
            // has a non-aliased `.folder` specifier but `dep_res_tag == .git`;
            // gating on `dep_res_tag` would wrongly suppress the applied
            // override literal. It also matters for override-to-`catalog:` —
            // `classify`'s catalog short-circuit only fires when `literal_raw
            // == "catalog:"`, which requires propagating the override.
            let override_applied = !dep.behavior.is_workspace() && !dep.is_aliased(string_buf);
