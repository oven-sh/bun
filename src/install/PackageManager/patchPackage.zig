...
pub fn doPatchCommit(
    ...
    const patch_key = std.fmt.allocPrint(manager.allocator, "{s}", .{resolution_label}) catch |oe| bun.outOfMemory(oe);
    const patchfile_path = manager.allocator.dupe(u8, path_in_patches_dir) catch |oe| bun.outOfMemory(oe);
...
fn escapePatchFilename(allocator: std.mem.Allocator, name: []const u8) ?[]const u8 {
    ...
    var buf = allocator.alloc(u8, count) catch |oe| bun.outOfMemory(oe);
    var i: usize = 0;
    for (name) |c| {
...
fn overwritePackageInNodeModulesFolder(
    ...
    var walker = Walker.walk(.fromStdDir(pkg_in_cache_dir), manager.allocator, &.{}, IGNORED_PATHS) catch |oe| bun.outOfMemory(oe);
    defer walker.deinit();
...
fn pkgInfoForNameAndVersion(
    ...
    var pairs = std.ArrayList(IdPair).initCapacity(sfb.get(), 8) catch |oe| bun.outOfMemory(oe);
    defer pairs.deinit();
    ...
    for (dependencies, 0..) |dep, dep_id| {
        ...
        if (version) |v| {
            ...
            if (std.mem.eql(u8, label, v)) {
                pairs.append(.{ @intCast(dep_id), pkg_id }) catch |oe| bun.outOfMemory(oe);
            }
        } else {
            pairs.append(.{ @intCast(dep_id), pkg_id }) catch |oe| bun.outOfMemory(oe);
        }
...
fn pathArgumentRelativeToRootWorkspacePackage(manager: *PackageManager, lockfile: *const Lockfile, argument: []const u8) ?[]const u8 {
    ...
    return bun.default_allocator.dupe(u8, bun.path.join(&[_][]const u8{ rel_path, argument }, .posix)) catch |oe| bun.outOfMemory(oe);
}
