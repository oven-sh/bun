...
impl GitSubprocess {
    ...
    /// A clone or commit lookup that only optional dependencies wait for fails with warnings.
    fn is_required(&self) -> bool {
        self.tag == Tag::GitCheckout || self.manager().is_network_task_required(self.task().id)
    }
