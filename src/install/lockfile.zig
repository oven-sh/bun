...
pub const Yarn = struct {
    /// Toggles which comments we should allow besides the headers
    ...
    // followed by: "{}-{}-{}-{}" where each {} is 8 bytes represented as 16 characters of hex
            ...
            .SWAR => while (true) {
                ...
                inline for (chars) |chr| {
                    comptime if (chr >= 0x80)
                        @compileError("advanceUntilAny can only search for ascii characters in its SWAR implementation.");
    ...
    fn parseYarnFile(this: *Lockfile, allocator: Allocator, file_buffer: []u8, sentinels_start: usize) !void {
        ...
        NEXT_DEPENDENCY: while (true) {
            ...
            while (true) {
                ...
                while (true) {
                    cur = cur[1..];
                    if (cur[0] != ' ') break; // this branch should be 100% predictable (besides first run)
                }
            }
            ...
            inline for (@typeInfo(YarnFields).Enum.fields) |field| {
                if (advanceOver(&cur, field.name ++ " ")) {
                    ...
                    std.debug.print("{}: <{s}>\n", .{ @intToEnum(YarnFields, field.value), data });
            ...
            const UnsupportedFields = enum { name, uid, registry, bundledDependencies };
            inline for (@typeInfo(UnsupportedFields).Enum.fields) |field| {
                if (advanceOver(&cur, field.name)) {
                    return switch (@intToEnum(UnsupportedFields, field.value)) {
