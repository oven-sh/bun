...
const BytesInInstructionCache = string_immutable.BytesInInstructionCache;
...
pub const Package = extern struct {
    ...
    const DependencyAlphabetizer = struct {
        dependencies: []const Dependency,
        buf: []const u8,
...
pub const Yarn = struct {
    ...
    const ALLOW_COMMENTS_BETWEEN_LINES = true;
    const ALLOW_INLINE_COMMENTS = false;
    const ALLOW_CARRIAGE_RETURNS = true;

    const ADVANCE_IMPL: enum { naive, SWAR, SIMD } = if (Environment.enableSIMD) .SIMD else .SWAR;
    ...
    pub inline fn sentinelCmp(x: anytype, sentinels_start: anytype, comptime sentinel: ?u8) bool {
        return if (Environment.isDebug and sentinel != null)
    ...
    inline fn advanceUntilAnyInternal(cur: *string, comptime chars_: string, comptime advance_over_current: bool, impl: @TypeOf(ADVANCE_IMPL)) !void {
        ...
        comptime if (impl == .SWAR) for (chars_) |chr|
            if (chr >= 0x80) return advanceUntilAnyInternal(cur, chars_, advance_over_current, .naive);
        ...
        switch (impl) {
            ...
            .SIMD => while (true) {
                ...
                var found_mask: std.meta.Int(.unsigned, width) = 0;
                inline for (chars) |chr| found_mask |= @bitCast(@TypeOf(found_mask), vec == @splat(width, chr));
                cur.* = cur.*[@ctz(found_mask)..]; // do a ctz no matter what, avoid the branch
                if (found_mask != 0) break;
            },
        }
    ...
    pub fn checkOverAllocatedEnough(comptime delim: anytype) @TypeOf(delim) {
        switch (@typeInfo(@TypeOf(delim))) {
            .Array => inline for (delim) |item| {
                _ = checkOverAllocatedEnough(item);
            },
            else => comptime if (!std.mem.eql(u8, delim, YARN_LOCK_HEADER_0) and
    ...
    pub inline fn advanceOverComptime(cur: *[]u8, comptime delim: string) bool {
        const has_prefix = strings.hasPrefixComptimeIgnoreLen(cur.*, checkOverAllocatedEnough(delim));
        if (has_prefix) cur.* = cur.*[delim.len..];
        return has_prefix;
    }

    pub inline fn advanceOver(cur: *[]u8, delim: string) bool {
        const has_prefix = strings.startsWithIgnoreLen(cur.*, delim);
        if (has_prefix) cur.* = cur.*[delim.len..];
        return has_prefix;
    }
    ...
    fn parse(this: *Lockfile, allocator: Allocator, list: std.ArrayListUnmanaged(u8)) !DependencyOverrides {
        ...
        if (advanceOverComptime(&cur, YARN_LOCK_HEADER_1)) {
        ...
        parse_dependency: while (true) {
            while (true) {
                switch (cur[0]) {
                    ...
                    if (ALLOW_COMMENTS_BETWEEN_LINES) '#' => try advanceUntilAnyNaive(&cur, "\n", true),
                    else => break,
                }
                cur = cur[1..];
                found_newline = true;
            ...
            if (advanceOverComptime(&cur, "integrity ")) {
                if (!advanceOverComptime(&cur, "sha")) return error.@"Unknown integrity type";

                // Used to match 512, 384, and 256
                const matchers = BytesInInstructionCache(u16)
                    .init(&[_]u8{ '1', '2', 0, 0, '8', '4', '5', '6' });

                const i = cur[0] -% '1';
                // error for any sha that isn't "1", "256", "384", or "512"
                if (i > 4 or i == 3 or (i != 0 and matchers.get(4 - i) != @bitCast(u16, cur[1..3].*)))
                    return error.@"Unknown integrity type";

                cur = cur[if (i == 0) 1 else 3..];
                comptime std.debug.assert(std.mem.indexOfAny(u8, &SENTINELS, "-") == null); //
                if (cur[0] != '-') return error.@"Missing integrity hash after sha prefix.";
                cur = cur[1..];

                const hash_len = BytesInInstructionCache(u8)
                    .init(&[_]u8{ (160 + 5) / 6, (256 + 5) / 6, (384 + 5) / 6, 0, (512 + 5) / 6 })
                    .get(i);

                cur = cur[hash_len..];

                if (sentinelCmp(@ptrToInt(&cur[0]), sentinels_start, null) or cur[0] != '=')
                    return error.@"Invalid integrity hash";
            ...
            for (checkOverAllocatedEnough([_]string{ "dependencies", "optionalDependencies" })) |field| {
                if (!advanceOver(&cur, field)) continue;
                ...
                while (true) {
                    ...
                    var at_place = cur;
                    cur = cur[" ".len..]; // skip ' ', it's already checked
    ...
    fn needsQuoteVersionSubstring(name: string) !bool {
        var checker: IncrementalShouldWrapInQuotesChecker = .{ .match_char = 0 };
        try checker.check(name);
        return checker.needs_quote;
    }
