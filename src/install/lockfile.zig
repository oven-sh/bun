...
const MetaHash = [META_HASH_LEN]u8;
const META_HASH_CHUNK_LEN = 8;
const META_HASH_LEN = std.crypto.hash.sha2.Sha512256.digest_length;
const META_HASH_LEN_IN_FILE = ((META_HASH_LEN - 1) / META_HASH_CHUNK_LEN) + 2 * META_HASH_LEN;
...
pub const MetaHashFormatter = struct {
    ...
    pub fn format(this: MetaHashFormatter, comptime _: []const u8, _: std.fmt.FormatOptions, writer: anytype) !void {
        const meta_hash = this.meta_hash;
        try std.fmt.format(
            writer,
            "{}-{}-{}-{}",
            .{
                std.fmt.fmtSliceHexUpper(meta_hash[META_HASH_CHUNK_LEN * 0 .. META_HASH_CHUNK_LEN * 1]),
                std.fmt.fmtSliceHexLower(meta_hash[META_HASH_CHUNK_LEN * 1 .. META_HASH_CHUNK_LEN * 2]),
                std.fmt.fmtSliceHexUpper(meta_hash[META_HASH_CHUNK_LEN * 2 .. META_HASH_CHUNK_LEN * 3]),
                std.fmt.fmtSliceHexLower(meta_hash[META_HASH_CHUNK_LEN * 3 .. META_HASH_CHUNK_LEN * 4]),
            },
        );
    }
};
...
pub const Yarn = struct {
    /// Whether we allow comments in yarn.lock files besides the headers below.
    const ALLOW_INLINE_COMMENTS = false;
    const ALLOW_CARRIAGE_RETURNS = true;
    const ADVANCE_IMPL: enum { naive, SWAR, SIMD } = .naive;
    ...
    // followed by: {}-{}-{}-{}
    // Where:
    // {} 8 bytes represented as 16 characters of upper hex 0123456789ABCDEF
    // {} 8 bytes represented as 16 characters of lower hex 0123456789abcdef
    // {} 8 bytes represented as 16 characters of upper hex 0123456789ABCDEF
    // {} 8 bytes represented as 16 characters of lower hex 0123456789abcdef

    pub const min_alloc_size = YARN_LOCK_HEADER_0.len +
        NEWLINES.len +
        YARN_LOCK_HEADER_1.len +
        "v1"[1..].len +
        NEWLINES.len +
        YARN_LOCK_HEADER_META_HASH.len +
        META_HASH_LEN_IN_FILE;

    pub const over_allocated_space = @max("optionalDependencies".len, switch (ADVANCE_IMPL) {
        .naive => 1,
    ...
    // inline fn advanceUntilNotAny(cur: *[]u8, comptime chars: []const u8, comptime advance_over_current: bool) !void {
    //     OUTER: while (true) {
    ...
    inline fn getLittleEndianBytes(bytes: anytype) @TypeOf(bytes) {
    ...
    pub inline fn advanceUntilAny(cur: *[]u8, comptime chars: []const u8, comptime advance_over_current: bool) !void {
        comptime std.debug.assert(chars.len > 0);
        ...
        switch (ADVANCE_IMPL) {
            ...
            .SWAR => {
                if (advance_over_current) cur.* = cur.*[1..];
                const width = @typeInfo(usize).Int.bits / 8;
                comptime std.debug.assert(over_allocated_space >= width);

                while (true) : (cur = cur[width..]) {
                    const word = getLittleEndianBytes(@bitCast(u64, cur[0..width].*));

                    // Algorithm from here: http://0x80.pl/notesen/2023-03-06-swar-find-any.html
                    comptime var one_in_each_byte = 0;
                    ...
                    var t0: usize = std.math.maxInt(usize); // LLVM, pls optimize this away

                    inline for (chars) |chr| {
                        comptime std.debug.assert(chr < 0x80);
                        t0 &= (lo7bits ^ (comptime chr * one_in_each_byte)) + mask;
                    }

                    const match_mask = ((t0 | word) & mask2) ^ mask2;
                    if (match_mask != 0) {
                        cur = cur[@ctz(match_mask) / 8 ..];
                        break;
                    }
                }
            },
            ...
            .SIMD => {
                if (advance_over_current) cur.* = cur.*[1..];
                const width = ascii_vector_size;
                comptime std.debug.assert(over_allocated_space >= width);

                while (true) : (cur = cur[width..]) {
                    const vec: @Vector(width, u8) = cur[0..width].*;
                    const found_mask: std.meta.Int(.unsigned, width) = 0;
                    inline for (chars) |chr| found_mask |= vec == @splat(width, chr);
                    if (found_mask != 0) {
                        cur = cur[@ctz(found_mask)..];
                        break;
                    }
                }
            },
    ...
    pub inline fn advanceOver(cur: *[]u8, comptime delim: []const u8) bool {
        comptime if (!std.mem.eql(u8, delim, YARN_LOCK_HEADER_0) and
            ...
            over_allocated_space < delim.len)
            @compileError(std.fmt.comptimePrint("Did not overallocate enough space! Should have overallocated {} bytes for \"{s}\", got {}.\nPlease update `over_allocated_space` to be \"{s}\".len\n", .{ delim.len, delim, over_allocated_space, delim }));

        const has_prefix = strings.hasPrefixComptimeIgnoreLen(cur.*, delim);
        if (has_prefix) cur.* = cur.*[delim.len..];
        return has_prefix;
    }
    ...
    pub inline fn advanceOverSkipInsignificant(cur: *[]u8, comptime delim: []const u8) bool {
        ...
        return advanceOver(cur, delim);
    }

    /// Parses a yarn file and consumes the data.
    // If we find a backslash, we error. I doubt anyone needs this functionality, and if they do,
    // they don't need it everywhere. We can add it to a specific place.
    // The yarn parser calls JSON.parse on the resulting string to support escapes.
    ...
    fn parseYarnFile(this: *Lockfile, allocator: Allocator, file_buffer: []u8, equals_sentinel: usize) !void {
        ...
        if (advanceOver(&cur, YARN_LOCK_HEADER_0)) {
            if (ALLOW_CARRIAGE_RETURNS and cur[0] == '\r') cur = cur[1..];
            if (cur[0] == '\n') cur = cur[1..];
        }

        if (advanceOver(&cur, YARN_LOCK_HEADER_1)) {
            if (cur[0] != '1') return error.@"Only yarn v1 lockfiles are supported";
            cur = cur[1..];
            if (ALLOW_CARRIAGE_RETURNS and cur[0] == '\r') cur = cur[1..];
            if (cur[0] == '\n') cur = cur[1..];
        }

        if (advanceOver(&cur, YARN_LOCK_HEADER_META_HASH)) try parseMetaHash(&this.meta_hash, &cur);
        ...
        NEXT_DEPENDENCY: while (true) {
            ...
            inline for ([_]enum { version, resolved }{ .version, .resolved }, 0..) |enumeration, i| {
                if (advanceOver(&cur, @typeInfo(@TypeOf(enumeration)).Enum.fields[i].name ++ " ")) {
                    const is_quote = cur[0] == '"';
                    var string_start = cur;
                    ...
                    if (is_quote) {
                        ...
                        // Do it the dumb way
                        else => return switch (enumeration) {
            ...
            // Are these in the right order? I can't find or produce a file with
            // `devDependencies` or `peerDependencies`
            inline for ([_]string{ "dependencies", "devDependencies", "optionalDependencies", "peerDependencies" }) |field| {
                if (advanceOver(&cur, field)) {
                    std.debug.print("Entering {s}\n", .{field});
                    if (cur[0] != ':') return error.@"Missing colon after dependencies list declaration";
                    cur = cur[1..];

                    if (!advanceOverSkipInsignificant(&cur, "\n" ++ " " ** 4))
                        return error.@"Missing data after dependencies list declaration";

                    while (true) {
                        const is_quoted = cur[0] == '"';
                        var string_start = cur;
                        ...
                        if (is_quoted) {
                            ...
                            string_start = cur;
                            try advanceUntilAny(&cur, "\"", false);
                            if (cur[1] != ' ') return error.@"Missing version for dependency";
                        } else switch (cur[0]) {
                            'A'...'Z', 'a'...'z' => {
                        ...
                        // this boolToInt is the price we pay so that we can combine the branches.
                        cur = cur[if (is_quoted) 2 else 1..]; // skip ' ', it's already checked

                        const is_quoted2 = cur[0] == '"';
                        var ver_start = cur;
                        ...
                        const dependency_ver = ver_start[0 .. @ptrToInt(&cur[0]) - @ptrToInt(ver_start.ptr)];
                        std.debug.print("{s}\n", .{dependency_ver});
            ...
            inline for ([_]enum { name, uid, registry, bundledDependencies }{ .name, .uid, .registry, .bundledDependencies }, 0..) |enumeration, i| {
                if (advanceOver(&cur, @typeInfo(@TypeOf(enumeration)).Enum.fields[i].name)) {
                    return switch (enumeration) {
                        .name => error.@"Parsing `name` fields in yarn.lock is not implemented",
                        .uid => error.@"Parsing `uid` fields in yarn.lock is not implemented",
                        .registry => error.@"Parsing `registry` fields in yarn.lock is not implemented",
                        .bundledDependencies => error.@"Parsing `bundledDependencies` fields in yarn.lock is not implemented",
                    };
                }
            }
    ...
    inline fn parseMetaHash(meta_hash: *MetaHash, cur: *[]u8) !void {
        comptime std.debug.assert(std.math.isPowerOfTwo(META_HASH_CHUNK_LEN));
        var meta: []u8 = meta_hash.*[0..];
        if (Environment.allow_assert) std.debug.assert(std.mem.allEqual(u8, meta, 0));
        var bytes_left: u8 = META_HASH_LEN;

        while (true) {
            // 8 bytes represented across 16 characters means we should iterate 16 times
            var chars_to_match: u8 = META_HASH_CHUNK_LEN * 2; // I hate giving types to values like this.
            var shift: std.math.Log2Int(u8) = 4; // This will be used to shift by a half byte or 0 each time

            while (chars_to_match > 0) : (chars_to_match -= 1) {
                const c = cur.*[0];
                meta[0] |= ((c + switch (c) {
                    '0'...'9' => @as(u8, 0),
                    'A'...'F', 'a'...'f' => @as(u8, 9),
                    else => return error.@"Invalid meta_hash",
                }) & 0xF) << shift;
                cur.* = cur.*[1..];
                shift ^= 4; // toggle between shifting by half a byte or not
                meta = meta[shift >> 2 ..];
            }

            bytes_left -= META_HASH_CHUNK_LEN;
            if (bytes_left == 0) break;
            if (cur.*[0] != '-') return error.@"Invalid meta_hash";
            cur.* = cur.*[1..];
        }
    }
    ...
    pub fn print(
        ...
        comptime std.debug.assert(YARN_LOCK_HEADER_1[YARN_LOCK_HEADER_1.len - 1] != '1');

        try writer.writeAll(YARN_LOCK_HEADER_0 ++ "\n" ++
            YARN_LOCK_HEADER_1 ++ "1\n" ++
            YARN_LOCK_HEADER_META_HASH);
