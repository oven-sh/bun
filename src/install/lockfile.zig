...
allocator: Allocator,
scratch: Scratch = .{},
/// An object which maps keys of the form a@^1.0.1 to how they should resolve, e.g. a@1.0.3
/// Used for when we retrieve this information from lockfiles generated by other tools like yarn
/// Owns all of its own memory. All of the strings located within it are in its string_buffer,
/// Everything else is stored in-situ at the moment.
...
pub fn loadOverridesFromNonBunLockfile(this: *Lockfile, allocator: Allocator) !void {
    ...
    if (dir.openFileZ("yarn.lock", .{})) |file| {
        defer file.close();
        this.dependency_overrides = try Yarn.loadFromFile(allocator, file);
    } else |_| {}
...
pub const Package = extern struct {
    ...
    pub fn fromNPM(
        ...
        const version: Semver.Version = find_result.version;
        const package_version_ptr: *const Npm.PackageVersion = find_result.package;
        var package = Lockfile.Package{};
        ...
        string_builder.count(find_result.tarball_override orelse manifest.str(&package_version_ptr.tarball_url));
            ...
            package.resolution = Resolution{
                .value = .{
                    .npm = .{
                        ...
                        .url = string_builder.append(String, find_result.tarball_override orelse manifest.str(&package_version_ptr.tarball_url)),
                    },
                },
                .tag = .npm,
            };
...
const DependencyOverrideMap = std.ArrayHashMapUnmanaged(string, DependencyOverrides.Override, DependencyOverrides.KeyContext, false);
pub const DependencyOverrides = struct {
    pub const Override = struct { version: Semver.Version, url: string };
    ...
    pub const KeyContext2 = struct {
        ...
        pub fn hash(self: @This(), key: name_and_version) HASH_SIZE {
            _ = self;
            if (Environment.allow_assert) std.debug.assert(std.hash_map.hashString(key.name) == key.name_hash);
            return @truncate(HASH_SIZE, key.name_hash ^ std.hash_map.hashString(key.version));
        }
    ...
    pub fn getOverride(self: Self, ctx: KeyContext2.name_and_version) ?Override {
        return self.map.getAdapted(ctx, KeyContext2{});
    }

    /// Moves all the string data into a separate buffer and returns the map with the newly owned buffer.
    fn initAsOwned(allocator: Allocator, buf: string, map: DependencyOverrideMap) !DependencyOverrides {
        var needed_buffer_size: u32 = 0;
        var prev_off: u32 = 0; // a valid value for this can never be 0
        var iter = map.iterator();
        var previous_version_string: String = .{};

        while (iter.next()) |entry| {
            const url = entry.value_ptr.url;
            needed_buffer_size += @intCast(u32, entry.key_ptr.len);
            needed_buffer_size += @intCast(u32, url.len);

            if (!entry.value_ptr.version.tag.pre.value.isInline()) {
                var ptr = entry.value_ptr.version.tag.pre.value.ptr();

                if (prev_off != ptr.off) { // we offsets as a unique identifier of each resolved version string. E.g. a@x and a@^1.0 might both resolve to the same version, e.g. 1.0.1
                    if (bun.C.memmem(url.ptr, url.len, buf.ptr + ptr.off, ptr.len)) |found_start| { // if the version string exists in the url, no need to store separately
                        // e.g. if our url is "https://registry.npmjs.org/pkg/-/pkg-10.2.156316-beta.tgz", our version slice can share the same space
                        previous_version_string = String.init(buf, found_start[0..ptr.len]);
                        // Note, we store it relative to `buf` rather than the `url` so that offsets are not relative to the `url` because we use the offsets as unique identifiers of each string
                    } else {
                        previous_version_string = entry.value_ptr.version.tag.pre.value;
                        needed_buffer_size += ptr.len;
                    }
                }

                prev_off = ptr.off; // intentionally stale after the update below
                entry.value_ptr.version.tag.pre.value = previous_version_string;
            }
        }
        ...
        while (iter.next()) |entry| {
            ...
            const old_url = entry.value_ptr.url;
            const url_len = old_url.len;
            bun.copy(u8, cur, entry.value_ptr.url);
            const url = cur[0..url_len];
            entry.value_ptr.url = url;
            cur = cur[url_len..];

            if (!entry.value_ptr.version.tag.pre.value.isInline()) {
                const ptr = entry.value_ptr.version.tag.pre.value.ptr();

                if (prev_off != ptr.off) {
                    const old_version_str = buf[ptr.off..][0..ptr.len];
                    if (bun.isSliceInBuffer(old_version_str, old_url)) {
                        previous_version_string = String.init(string_buffer, url[ptr.off - @intCast(u32, @ptrToInt(old_url.ptr) - @ptrToInt(buf.ptr)) ..][0..ptr.len]);
                    } else {
                        bun.copy(u8, cur, old_version_str); // we have to store it separately (rare?)
                        previous_version_string = String.init(string_buffer, cur[0..ptr.len]);
                        cur = cur[ptr.len..];
                    }
                }
                entry.value_ptr.version.tag.pre.value = previous_version_string;
    ...
    fn deinit(self: *Self, allocator: Allocator) !void {
        allocator.free(self.string_buffer);
...
pub const Yarn = struct {
    ...
    pub inline fn sentinelCmp(cur: []u8, sentinels_start: usize, comptime sentinel: ?u8) bool {
        const x = @ptrToInt(cur.ptr);
    ...
    pub inline fn advanceUntilAnyNaive(cur: *string, comptime chars_: string, comptime advance_over_current: bool) @TypeOf(advanceUntilAnyInternal(cur, chars_, advance_over_current, ADVANCE_IMPL)) {
        return advanceUntilAnyInternal(cur, chars_, advance_over_current, .naive);
    }

    pub fn advanceUntilAny(cur: *string, comptime chars_: string, comptime advance_over_current: bool) @TypeOf(advanceUntilAnyInternal(cur, chars_, advance_over_current, ADVANCE_IMPL)) {
    ...
    inline fn advanceUntilAnyInternal(cur: *string, comptime chars_: string, comptime advance_over_current: bool, impl: @TypeOf(ADVANCE_IMPL)) if (std.mem.indexOfAny(u8, chars_, "\"") != null) (anyerror!void) else void {
    ...
    fn parse(allocator: Allocator, buffer_list: std.ArrayListUnmanaged(u8)) !DependencyOverrides {
        ...
        errdefer map.deinit(allocator); // caller owns this

        try map.ensureTotalCapacity(allocator, buffer_list.capacity / 256); // guesstimate how much memory we will need
        var cur = buffer_list.allocatedSlice();
        const sentinels_start: usize = @ptrToInt(&buffer_list.items.ptr[buffer_list.items.len]);
        ...
        // We could parse the MetaHash but have no reason to at the moment.
        ...
        parse_dependency: while (true) {
            ...
            while (true) {
                switch (cur[0]) {
                    ...
                    if (ALLOW_COMMENTS_BETWEEN_LINES) '#' => advanceUntilAnyNaive(&cur, "\n", true),
                    else => break,
                }
                cur = cur[1..];
                found_newline = true;
            ...
            if (@ptrToInt(cur.ptr) >= sentinels_start)
                return DependencyOverrides.initAsOwned(allocator, buffer_list.items, map);

            // Otherwise, start matching dependencies
            if (!found_newline) return error.@"No newline before dependency name in yarn.lock";
            ...
            parse_comma_separated_titles: while (true) {
                ...
                if (cur[0] == '@') return error.@"Unable to find package name in yarn.lock";
                advanceUntilAny(&cur, "@", false); // find the @?<name>@version

                if (sentinelCmp(cur, sentinels_start, '@'))
                ...
                if (is_quote)
                    ...
                    advanceUntilAny(&cur, ":,", false);

                const package_title = name_start[0 .. @ptrToInt(cur.ptr) - @ptrToInt(name_start.ptr)];

                const get_result = try map.getOrPut(allocator, package_title);
                if (get_result.found_existing)
                    return error.@"Duplicate package titles found in yarn.lock";

                get_result.value_ptr.*.url = "";
            ...
            if (cur[0] != ':')
                return error.@"Lacking colon after a dependency name or invalid characters were detected in yarn.lock";

            cur = cur[1..];

            if (sentinelCmp(cur, sentinels_start, '='))
                return error.@"Found unterminated string or dependency name in yarn.lock";

            if (advanceOverComptime(&cur, "\n  name ")) advanceUntilAnyNaive(&cur, NEWLINES, false);

            if (!advanceOverSkipInsignificant(&cur, "\n  version \""))
                return error.@"Missing (quoted) version field in yarn.lock";

            {
                ...
                while (true) {
                    ...
                    cur_subver = @intToPtr(*u32, @ptrToInt(cur_subver) + @sizeOf(u32));
                }

                if (num_dots != 2) return error.@"Invalid version in yarn.lock";

                if (cur[0] == '-') { // Note: There is never any build info in yarn files.
                    cur = cur[1..];
                    const prerelease_start = cur;
                    comptime std.debug.assert(std.mem.indexOfAny(u8, &SENTINELS, "\"").? != 0);
                    try advanceUntilAnyNaive(&cur, "\"", true);

                    if (prerelease_start[0] == '"' or sentinelCmp(cur, sentinels_start, '"'))
                        return error.@"empty release tag in yarn.lock"; // error on a@1.0.0-

                    const prerelease = prerelease_start[0 .. @ptrToInt(cur.ptr) - @ptrToInt(prerelease_start.ptr)];
                    if (Environment.allow_assert) std.debug.assert(std.mem.indexOfAny(u8, prerelease, "+") == null);
                    version.tag.pre = SlicedString.init(buffer_list.items, prerelease).external();
                }

                for (map.values()[map_start_index..]) |*value| value.*.version = version;
            ...
            if (advanceOverComptime(&cur, "uid ")) {
                advanceUntilAnyNaive(&cur, NEWLINES, false);
                if (!advanceOverSkipInsignificant(&cur, "\n" ++ " " ** 2)) continue :parse_dependency;
            }
            ...
            if (advanceOverComptime(&cur, "resolved ")) {
                const is_quote = cur[0] == '"';
                cur = cur[@boolToInt(is_quote)..];
                const resolved_link_start = cur;

                if (is_quote) {
                    try advanceUntilAny(&cur, "\"", false);
                } else switch (cur[0]) {
                    'A'...'Z', 'a'...'z' => advanceUntilAnyNaive(&cur, NEWLINES, true),
                    else => return error.@"Unable to parse `resolved` field in yarn.lock",
                }

                if (sentinelCmp(cur, sentinels_start, null))
                    return error.@"Missing resolved field in yarn.lock";

                const resolved_link = resolved_link_start[0 .. @ptrToInt(cur.ptr) - @ptrToInt(resolved_link_start.ptr)];
                for (map.values()[map_start_index..]) |*value| value.*.url = resolved_link;
            ...
            if (advanceOverComptime(&cur, "integrity ")) {
                if (!advanceOverComptime(&cur, "sha")) return error.@"Unknown integrity type in yarn.lock";
                ...
                if (i > 4 or i == 3 or (i != 0 and matchers.get(4 - i) != @bitCast(u16, cur[1..3].*)))
                    return error.@"Unknown integrity type in yarn.lock";

                cur = cur[if (i == 0) 1 else 3..];
                comptime std.debug.assert(std.mem.indexOfAny(u8, &SENTINELS, "-") == null);
                if (cur[0] != '-') return error.@"Missing integrity hash after sha prefix in yarn.lock";
                cur = cur[1..];
                ...
                if (sentinelCmp(cur, sentinels_start, null) or cur[0] != '=')
                    return error.@"Invalid integrity hash in yarn.lock";
            ...
            if (advanceOverComptime(&cur, "registry ")) {
                advanceUntilAnyNaive(&cur, NEWLINES, false);
                if (!advanceOverSkipInsignificant(&cur, "\n" ++ " " ** 2)) continue :parse_dependency;
            }
            ...
            for (checkOverAllocatedEnough([_]string{ "dependencies:", "optionalDependencies:" })) |field| {
                ...
                if (!advanceOverSkipInsignificant(&cur, "\n" ++ " " ** 4))
                    return error.@"Missing data after dependencies list declaration in yarn.lock";

                parse_sub_dependencies: while (true) {
                    ...
                    if (is_quoted) {
                        ...
                        try advanceUntilAny(&cur, "\"", false);
                        cur = cur[1..];
                        comptime std.debug.assert(SENTINELS[std.mem.indexOfAny(u8, &SENTINELS, "\"").? + 1] != ' ');
                        if (cur[0] != ' ') return error.@"Missing version for dependency in yarn.lock";
                    } else switch (cur[0]) {
                        'A'...'Z', 'a'...'z' => blk: {
                            comptime std.debug.assert(switch (SENTINELS[std.mem.indexOfAny(u8, &SENTINELS, " ").? + 1]) {
                                '"', 'A'...'Z', 'a'...'z' => false,
                                else => true,
                            }); // prove it is safe to avoid checking if we hit a sentinel by guaranteeing we fail later
                            break :blk advanceUntilAny(&cur, " ", true);
                        },
                        else => return error.@"Unexpected dependency character in yarn.lock",
                    }

                    // const dep_name_len = @ptrToInt(cur.ptr) - @ptrToInt(string_start.ptr);
                    ...
                    if (is_quoted2) {
                        ...
                        try advanceUntilAny(&cur, "\"", false);
                        if (sentinelCmp(cur, sentinels_start, '"'))
                            return error.@"Missing close quote for dependency version in yarn.lock";
                    } else switch (cur[0]) {
                        'A'...'Z', 'a'...'z' => advanceUntilAnyNaive(&cur, NEWLINES, true),
                        else => return error.@"Unexpected dependency version character in yarn.lock",
                    }
                    ...
                    // at_place[0] = '@';
                    // const dependency_ver = new_string_start[0 .. @ptrToInt(cur.ptr) - @ptrToInt(new_string_start.ptr)];

                    cur = cur[@boolToInt(is_quoted2)..]; // skip last '"'

                    if (!advanceOverSkipInsignificant(&cur, "\n" ++ " " ** 4)) break :parse_sub_dependencies;
                } // : parse_sub_dependencies

                if (!advanceOverComptime(&cur, "\n" ++ " " ** 2)) continue :parse_dependency;
            }

            return error.@"Missing or invalid dependency field after indentation in yarn.lock";
        }
    }

    /// Takes in a file owned by the caller to parse as a yarn.lock file
    pub fn loadFromFile(allocator: Allocator, file: std.fs.File) !DependencyOverrides {
        ...
        return parse(allocator, body_pool.data.list);
    }
    ...
    const IncrementalShouldWrapInQuotesChecker = struct {
        ...
        pub fn check(self: *Self, name: string) !void {
            for (name) |chr| switch (chr) {
                ...
                0...31, '\\', '"' => return error.@"Unable to print characters that must be escaped in yarn.lock",
    ...
    pub fn print(
        ...
        comptime std.debug.assert(ALLOW_COMMENTS_BETWEEN_LINES);
        try writer.print(
            "{}\n\n",
        ...
        for (alphabetized_names) |i| {
            ...
            const version_formatter: ?Semver.Version.Formatter = if (resolution.tag == .npm) null else version_formatter: {
                ...
                break :version_formatter if (ver.stopped_at < version_writer.pos or !ver.valid or ver.wildcard != .none)
                    null
                else
                    .{
