...
// PORT NOTE: the original `deinit` only freed owned ArrayListUnmanaged fields; in Rust
// the Vec-backed field types drop automatically, so no explicit `Drop` impl is needed.
...
//
// PORT NOTE: the original computed this with reflection + an insertion sort
// by descending field alignment. Every field is an `ArrayListUnmanaged`,
...
// only for `Aligner.write`) was `Tree.List.Slice` i.e. `[]Tree`; the
// alignment of `[]Tree` is the alignment of the SLICE fat-pointer (ptr+len), i.e.
...
mod sizes {
    ...
    /// Alignment used by `Aligner::write` for every array payload (`sizes.types[0]`).
    ///
    /// `sizes.types[0]` is `[]Tree` (a slice type), and `@alignOf([]Tree)` is the
    ...
    // `sizes.bytes` was never read; omitted.
...
pub fn read_array<T: Copy>(stream: &mut Stream) -> Result<Vec<T>, bun_core::Error> {
    ...
    // PORT NOTE: the original went through `stream.reader()`; `FixedBufferStream` exposes
    ...
    if byte_len == 0 {
        ...
        // be aligned. `readArray` returns the empty slice
    ...
    // The original used an unchecked alignment cast here with the same precondition.
...
where
    // PORT NOTE: the original threaded a separate `stream` and `writer` over the
    ...
    // TODO(port): narrow error set
{
    ...
    // SAFETY: `T` has no uninitialized padding (asserted above); reading
    // its bytes is sound.
    let bytes: &[u8] =
    ...
    // PORT NOTE: the original built this at comptime from the type name, size,
    // and alignment of the array element. The reader skips
    // this prefix by absolute offset so it is semantically inert, but we emit the
    // exact same bytes so that re-saving an unchanged lockfile is a byte
    // no-op across the migration. Call sites pass the verbatim
...
where
    // PORT NOTE: see `write_array` — the original separate stream/writer aliased one
    // buffer; collapsed to a single bound to avoid two `&mut` to the same object.
    S: lockfile::PositionalStream + bun_io::Write,
{
    let buffers = &lockfile.buffers;

    // PORT NOTE: the original looped over `sizes.names` via field reflection.
    ...
    macro_rules! save_generic_field {
        ($field:ident, $name:literal, $elem:ty, $prefix:literal) => {{
            ...
            // PORT NOTE: the original had a `Type == Tree` branch here, but `Type`
            // was the slice element type, never `Tree`, so that arm
    ...
    // -- trees --
    {
        ...
        // PORT NOTE: the original `Type == Tree` arm
        // never fires because `Type` is `[]Tree`, so the original wrote raw `Tree`
        // bytes — which worked only because the auto-layout for `Tree` happened
        ...
        // `repr(Rust)` field order. This is byte-identical to what the original emitted
        ...
        write_array(
            stream,
            clone.as_slice(),
            // Verbatim original type-name output. The original wrote raw `Tree` (the
            // `Type == Tree` branch is dead — see PORT NOTE above), so it reports
            // `4 alignof` even though we serialize `tree::External` (`[u8;20]`,
            // align 1). The reader ignores this string; we match the original bytes.
    ...
    // -- dependencies --
    {
        ...
        write_array(
            stream,
            to_clone.as_slice(),
            // Type-name string for `Dependency.External` where `External = [26]u8`.
...
pub fn load(
    ...
    // PORT NOTE: the original `inline for (sizes.names)` is unrolled — see `sizes` module note.
    ...
    // -- trees --
    {
        ...
        // PORT NOTE: the original did `initCapacity` + `items.len = N` + write each slot.
    ...
    // PORT NOTE: the original did `expandToCapacity` + `items.len = N` then wrote each
