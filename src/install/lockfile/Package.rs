        ...
        'trusted_dependencies: {
            ...
            if let (Some(from_trusted_dependencies), Some(to_trusted_dependencies)) = (
                ...
                for from_name in from_trusted_dependencies.names() {
                    if !to_trusted_dependencies.contains(from_name) {
                        summary.removed_trusted_dependencies.insert(from_name)?;
                    }
                }
                for &from_hash in from_trusted_dependencies.legacy_hashes() {
                    if !to_trusted_dependencies.contains_truncated_hash(from_hash) {
                        summary
