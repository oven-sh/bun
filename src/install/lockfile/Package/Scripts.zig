        ...
        pub fn appendToLockfile(this: Package.Scripts.List, lockfile: *Lockfile) void {
            inline for (this.items, 0..) |maybe_script, i| {
                if (maybe_script) |script| {
                    ...
                    @field(lockfile.scripts, Lockfile.Scripts.names[i]).append(lockfile.allocator, script) catch |oe| bun.outOfMemory(oe);
                }
            }
        }
    };
    ...
    pub fn createList(
        ...
        if (first_index != -1) {
            ...
            return .{
                ...
                .total = total,
                .cwd = allocator.dupeZ(u8, cwd) catch |oe| bun.outOfMemory(oe),
                .package_name = lockfile.allocator.dupe(u8, package_name) catch |oe| bun.outOfMemory(oe),
            };
        }
