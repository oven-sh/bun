...
pub const Stringifier = struct {
    ...
    fn writePackageInfoObject(
        ...
        inline for (workspace_dependency_groups) |group| {
            ...
            for (pkg_dep_ids) |dep_id| {
                ...
                // Check all dep entries for this package, not just the peer entry,
                // because the same package name may be resolved through a prod dep.
                if (dep.behavior.peer and !dep.behavior.optional and
                    !isResolvedByAnyEntry(dep.name, pkg_dep_ids, deps_buf, resolution_buf, buf))
                {
                    try optional_peers_buf.append(dep.name);
                }
            }
    ...
    fn writeWorkspaceDeps(
        ...
        inline for (workspace_dependency_groups) |group| {
            ...
            for (dep_slice.begin()..dep_slice.end()) |_dep_id| {
                ...
                // because the same package name may be resolved through a prod dep.
                if (dep.behavior.peer and !dep.behavior.optional and
                    !isResolvedByAnyEntrySlice(dep.name, dep_slice, deps_buf, resolution_buf, buf))
                {
                    try optional_peers_buf.append(dep.name);
                }
            }
    ...
    /// Check if a dependency name is resolved by any entry in the given dep ID list.
    /// Used to avoid adding a peer dep to optionalPeers when the same package is
    /// resolved through another dep entry (e.g., as a prod dependency).
    fn isResolvedByAnyEntry(
        name: String,
        pkg_dep_ids: []const DependencyID,
        deps_buf: []const Dependency,
        resolution_buf: []const PackageID,
        ...
        const peer_name = name.slice(buf);
        for (pkg_dep_ids) |other_id| {
            if (resolution_buf[other_id] != invalid_package_id and
                bun.strings.eql(deps_buf[other_id].name.slice(buf), peer_name))
            {
                return true;
            }
        }
    ...
    /// Same as isResolvedByAnyEntry but takes a DependencySlice instead of a list.
    fn isResolvedByAnyEntrySlice(
        name: String,
        dep_slice: DependencySlice,
        deps_buf: []const Dependency,
        resolution_buf: []const PackageID,
        buf: string,
    ) bool {
        const peer_name = name.slice(buf);
        for (dep_slice.begin()..dep_slice.end()) |other_id| {
            if (resolution_buf[other_id] != invalid_package_id and
                bun.strings.eql(deps_buf[other_id].name.slice(buf), peer_name))
            {
                return true;
