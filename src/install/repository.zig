...
pub const Repository = extern struct {
    ...
    pub var shared_env: struct {
        ...
        pub fn get(this: *@This(), allocator: std.mem.Allocator, other: *DotEnv.Loader) DotEnv.Map {
            return this.env orelse brk: {
                ...
                var cloned = other.map.cloneWithAllocator(allocator) catch |oe| bun.outOfMemory(oe);

                if (cloned.get("GIT_ASKPASS") == null) {
                    const config = SloppyGlobalGitConfig.get();
                    if (!config.has_askpass) {
                        cloned.put("GIT_ASKPASS", "echo") catch |oe| bun.outOfMemory(oe);
                    }
                }
                ...
                if (cloned.get("GIT_SSH_COMMAND") == null) {
                    ...
                    if (!config.has_ssh_command) {
                        cloned.put("GIT_SSH_COMMAND", "ssh -oStrictHostKeyChecking=accept-new") catch |oe| bun.outOfMemory(oe);
                    }
                }
    ...
    pub fn createDependencyNameFromVersionLiteral(
        ...
        if (name.len == 0) {
            ...
            const name_buf = allocator.alloc(u8, bun.sha.EVP.SHA1.digest) catch |oe| bun.outOfMemory(oe);
        ...
        return allocator.dupe(u8, name) catch |oe| bun.outOfMemory(oe);
    }
