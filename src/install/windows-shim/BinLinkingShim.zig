...
pub const Shebang = struct {
    ...
    pub fn parse(contents_maybe_overflow: []const u8, bin_path: []const u16) !?Shebang {
        ...
        if (eqlComptime(first, "/usr/bin/env") or eqlComptime(first, "/bin/env")) {
            ...
            // Reconstruct the launcher as "program rest_args..."
            // `program` and `rest_after_program` are slices into the same `line` buffer,
            // so we can compute the combined slice from program start to rest end.
