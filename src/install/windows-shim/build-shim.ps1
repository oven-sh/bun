...
try {
  if($Debug) {
    zig build-exe .\bun_shim_debug.zig -O Debug -fsingle-threaded `
      -fomit-frame-pointer -fno-valgrind "-femit-bin=bun_shim_debug.exe"
  } else {
    ...
    clang-cl.exe /c bun_shim_impl.ll /GS- /Gs999999 /O2 /DEBUG
    lld-link.exe bun_shim_impl.obj /subsystem:console /LTCG `
      'C:\Program Files (x86)\Windows Kits\10\Lib\10.0.22621.0\um\x64\ntdll.lib' `
      'C:\Program Files (x86)\Windows Kits\10\Lib\10.0.22621.0\um\x64\kernel32.lib' /DEBUG

    dumpbin bun_shim_impl.exe /imports
