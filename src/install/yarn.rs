...
pub(crate) fn migrate_yarn_lockfile<'a>(
    ...
    // Like every other lockfile loader, index each package (root included) under the
    // name it was appended with: resolution looks packages up by real name, so an
    // entry keyed by an alias spec must not be indexed under the alias.
