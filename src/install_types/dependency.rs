...
use crate::resolver_hooks::PackageNameHash;
use crate::resolver_hooks::Repository;
...
// Owning crate for the dependency-parsing surface: these are the canonical
// short spellings (`Version`/`Tag`/`Value`) for the `resolver_hooks` types.
pub use crate::resolver_hooks::{
...
impl Dependency {
    ...
    ///
    /// `alias_hash` and `log` accept either the bare value
    /// (`u64` / `&mut Log`) or `Option<_>` via `impl Into<Option<_>>`.
    #[inline]
    pub fn parse_with_optional_tag<'a>(
        ...
        package_manager: Option<&mut (dyn NpmAliasRegistry + '_)>,
    ) -> Option<Version> {
...
pub struct Context<'a> {
    ...
    pub buffer: &'a [u8],
    pub package_manager: Option<&'a mut dyn NpmAliasRegistry>,
}
...
pub fn parse<'a>(
    ...
    sliced: &SlicedString,
    log: impl Into<Option<&'a mut bun_ast::Log>>,
    manager: Option<&mut dyn NpmAliasRegistry>,
) -> Option<Version> {
