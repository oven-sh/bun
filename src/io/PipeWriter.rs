...
pub trait BaseWindowsPipeWriter: Sized {
    ...
    /// Ownership of `pipe` transfers to `self.source` (later freed via
    /// `close_and_destroy`).
    fn start_with_pipe(&mut self, pipe: Box<uv::Pipe>) -> sys::Result<()> {
...
// `borrow = ptr`    → bodies call `Self::method(this, ..)` with the raw
//                     `*mut Self` (legacy; new parents use `this`).
...
// `expr` is pasted into an `unsafe` block with `this: *mut Self` in scope; for
