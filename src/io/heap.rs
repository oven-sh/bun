...
//
// PORT NOTE: the comparator is folded into a trait on `Context`
// (`HeapContext<T>::less`). This preserves monomorphization (no indirect call) at
// the cost of requiring the caller to impl the trait instead of passing a free fn.
// PERF(port): keeps the comparator monomorphized — profile in Phase B.
