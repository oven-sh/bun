...
pub const errno_map: [94]Errno = brk: {
    var errors: [94]Errno = undefined;
    ...
    errors[45] = error.Unexpected;
    errors[46] = error.EPFNOSUPPORT;
    errors[47] = error.Unexpected;
    ...
    errors[59] = error.ETIMEDOUT;
    errors[60] = error.ECONNREFUSED;
    errors[61] = error.ELOOP;
    errors[62] = error.ENAMETOOLONG;
    errors[63] = error.EHOSTDOWN;
    errors[64] = error.EHOSTUNREACH;
    errors[65] = error.ENOTEMPTY;
    errors[66] = error.EPROCLIM;
    errors[67] = error.EUSERS;
    errors[68] = error.EDQUOT;
    errors[69] = error.ESTALE;
    errors[70] = error.EBADRPC;
    errors[71] = error.ERPCMISMATCH;
    errors[72] = error.EPROGUNAVAIL;
    errors[73] = error.EPROGMISMATCH;
    errors[74] = error.EPROCUNAVAIL;
    errors[75] = error.ENOLCK;
    errors[76] = error.ENOSYS;
    errors[77] = error.EFTYPE;
    errors[78] = error.EAUTH;
    errors[79] = error.ENEEDAUTH;
    errors[80] = error.EOVERFLOW;
    errors[81] = error.ECANCELED;
    errors[82] = error.EIDRM;
    errors[83] = error.ENOMSG;
    errors[84] = error.EILSEQ;
    errors[85] = error.ENOATTR;
    errors[86] = error.EBADMSG;
    errors[87] = error.EMULTIHOP;
    errors[88] = error.ENOLINK;
    errors[89] = error.EPROTO;
    errors[90] = error.EOPNOTSUPP;
    errors[91] = error.ENOTRECOVERABLE;
    errors[92] = error.EOWNERDEAD;
    break :brk errors;
};

const socket_t = os.socket_t;
const sockaddr = freebsd.sockaddr;
const socklen_t = freebsd.socklen_t;
pub const system = freebsd;
...
pub const Syscall = struct {
    pub fn close(fd: std.os.fd_t) CloseError!void {
        return switch (freebsd.getErrno(freebsd.@"close"(fd))) {
            .SUCCESS => {},
    ...
    pub fn open(path: [*:0]const u8, oflag: c_uint) OpenError!fd_t {
        const fd = freebsd.@"open$NOCANCEL"(path, oflag);
        return switch (freebsd.getErrno(fd)) {
            .SUCCESS => fd,
    ...
    pub fn fcntl(fd: fd_t, cmd: i32, arg: usize) Errno!usize {
        const rc = freebsd.@"fcntl$NOCANCEL"(fd, cmd, arg);
        return switch (freebsd.getErrno(rc)) {
    ...
    pub fn setsockopt(fd: socket_t, level: u32, optname: u32, opt: []const u8) SetSockOptError!void {
        switch (freebsd.getErrno(freebsd.setsockopt(fd, level, optname, opt.ptr, @as(socklen_t, @intCast(opt.len))))) {
            .SUCCESS => {},
    ...
    pub fn socket(domain: u32, socket_type: u32, protocol: u32) SocketError!socket_t {
        ...
        const rc = freebsd.socket(domain, filtered_sock_type, protocol);
        switch (freebsd.getErrno(rc)) {
            .SUCCESS => {
...
pub const Waker = struct {
    ...
    const zeroed = std.mem.zeroes([16]Kevent);
...
pub const UserFilterWaker = struct {
    ...
    pub fn wake(this: UserFilterWaker) !void {
        ...
        events[0].udata = 0;
        const errno = std.os.system.kevent(
            this.kq,
            &events,
            1,
            &events,
    ...
    pub fn init(_: std.mem.Allocator) !UserFilterWaker {
        ...
        var events = [1]Kevent{std.mem.zeroes(Kevent)};
        events[0].ident = 123;
        events[0].filter = c.EVFILT_USER;
        events[0].flags = c.EV_ADD | c.EV_ENABLE;
        events[0].data = 0;
...
const Kevent = std.os.system.Kevent;
...
fn flush(self: *IO, comptime _: @Type(.EnumLiteral)) !void {
    ...
    var events: [2048]Kevent = undefined;
    ...
    const new_events_ = std.os.system.kevent(
        self.waker.kq,
        &events,
...
fn flush_io(_: *IO, events: []Kevent, io_pending_top: *?*Completion) usize {
    ...
    self.submit(
        ...
        struct {
            fn doOperation(op: anytype) AcceptError!os.socket_t {
                ...
                // freebsd doesn't support os.MSG.NOSIGNAL,
            ...
            fn doOperation(op: anytype) IO.ConnectError!void {
                ...
                const result = switch (op.initiated) {
                    true => brk: {
                        ...
                        assert(size == 4);
                        break :brk switch (freebsd.getErrno(rc)) {
                    ...
                    else => switch (freebsd.getErrno(freebsd.@"connect$NOCANCEL"(op.socket, &op.address.any, op.address.getOsSockLen()))) {
                        .SUCCESS => {},
                        .ACCES => error.PermissionDenied,
                        .PERM => error.PermissionDenied,
                        .ADDRINUSE => error.AddressInUse,
...
pub fn openSocket(family: u32, sock_type: u32, protocol: u32) !os.socket_t {
    ...
    // freebsd doesn't support os.MSG.NOSIGNAL, but instead a socket option to avoid SIGPIPE.
