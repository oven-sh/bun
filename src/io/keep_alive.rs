...
//!
//! Historically there were two near-identical per-platform copies; 9 of 14
//! methods were byte-identical. The 5 that diverge keep their per-platform
//! behaviour via `#[cfg]` arms inline below — no caller-visible contract
//! changes (all external users go through `bun_io::KeepAlive` and only touch
//! the identical-signature methods).
...
impl KeepAlive {
    ...
    #[cfg(windows)]
    pub fn activate(&mut self, loop_: &mut crate::Loop) {
        // Windows guards on `!= Active` (vs posix `!= Inactive`); preserved
        // verbatim.
        if self.status != Status::Active {
            return;
        }
    ...
    pub fn unref_on_next_tick_concurrently(&mut self, vm: EventLoopCtx) {
        ...
        self.status = Status::Inactive;
        // TODO(port): vm.pending_unref_counter must be Atomic (relaxed fetch-add)
        #[cfg(not(windows))]
    ...
    /// Allow a poll to keep the process alive.
    ///
    /// Raw-identifier alias of [`KeepAlive::ref_`] matching the historical
    /// method name `ref` exactly. Downstream code calls both spellings; this
    /// keeps them source-compatible.
    #[inline]
