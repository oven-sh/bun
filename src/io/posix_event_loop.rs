...
impl FilePoll {
    ...
    fn unregister_with_fd_impl(
        ...
        // borrowed fd), and on epoll the next EPOLL_CTL_ADD for it then fails
        // with EEXIST, so a forced unregister (teardown) deletes it after all.
        // Its direction is no longer recorded: epoll deletes by fd; kqueue gets a
        // delete for both filters and the missing one is not an error.
