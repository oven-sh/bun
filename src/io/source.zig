...
pub const Source = union(enum) {
    ...
    pub fn openPipe(loop: *uv.Loop, fd: bun.FileDescriptor) bun.sys.Maybe(*Source.Pipe) {
        log("openPipe (fd = {})", .{fd});
        const pipe = bun.default_allocator.create(Source.Pipe) catch |oe| bun.outOfMemory(oe);
    ...
    pub fn openTty(loop: *uv.Loop, fd: bun.FileDescriptor) bun.sys.Maybe(*Source.Tty) {
        ...
        const tty = bun.default_allocator.create(Source.Tty) catch |oe| bun.outOfMemory(oe);
    ...
    pub fn openFile(fd: bun.FileDescriptor) *Source.File {
        ...
        log("openFile (fd = {})", .{fd});
        const file = bun.default_allocator.create(Source.File) catch |oe| bun.outOfMemory(oe);
