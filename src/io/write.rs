...
//!
//! Streaming-writer surface that the rest of the codebase actually
//! touches (`write_all`, `write_byte`, `print`, `splat_byte_all`, `write_int`,
...
impl<B: AsRef<[u8]>> FixedBufferStream<B> {
    ...
    /// The read methods here live directly on `FixedBufferStream`, so this
    /// just returns `self` to keep call-site shape
...
impl<W: fmt::Write> Write for FmtAdapter<'_, W> {
    fn write_all(&mut self, buf: &[u8]) -> Result<()> {
        ...
        let r = match bun_core::str_utf8(buf) {
            ...
            // PERF(port): lossy alloc only on invalid UTF-8; the writer trait
            // here has no text/bytes split so this branch is the price of bridging.
