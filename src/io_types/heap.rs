...
impl<T: HeapNode, Context: HeapContext<T>> Intrusive<T, Context> {
    ...
    ///
    /// # Safety
    /// `v` must be a valid, uniquely accessible heap node that is not already
    /// linked into any intrusive heap.
    ...
    ///
    /// # Safety
    /// Every node reachable from `self.root` must be valid for shared traversal
    /// and must belong to this heap.
    ...
    ///
    /// # Safety
    /// Every node reachable from `self.root` must be valid and uniquely owned by
    /// this heap for mutation.
    ...
    ///
    /// # Safety
    /// `v` must be a valid node currently linked into this heap, and all heap
    /// nodes reachable during relinking must be valid for unique mutation.
...
mod tests {
    ...
    struct Node {
        value: u8,
        heap: IntrusiveField<Node>,
    }

    impl HeapNode for Node {
        fn heap(&mut self) -> &mut IntrusiveField<Self> {
            &mut self.heap
        }
    }
    ...
    impl HeapContext<Node> for MinHeap {
        fn less(&self, a: *mut Node, b: *mut Node) -> bool {
            unsafe { (*a).value < (*b).value }
        }
    }

    #[test]
    fn intrusive_heap_orders_nodes_without_allocating_container() {
        let mut one = Node { value: 1, ..Node::default() };
        let mut two = Node { value: 2, ..Node::default() };
        let mut three = Node { value: 3, ..Node::default() };
        let mut heap = Intrusive::<Node, MinHeap>::default();

        unsafe {
            heap.insert(&mut three);
            heap.insert(&mut one);
            heap.insert(&mut two);

            assert_eq!((*heap.peek()).value, 1);
            assert_eq!((*heap.delete_min()).value, 1);
            assert_eq!((*heap.delete_min()).value, 2);
            assert_eq!((*heap.delete_min()).value, 3);
            assert!(heap.peek().is_null());
        }
    }
}
