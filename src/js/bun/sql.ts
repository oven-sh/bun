      ...
      if (values.length > i) {
        ...
        if (value instanceof Query) {
          ...
          if (command === SQLCommand.none || command === SQLCommand.where) {
            throw new SyntaxError("INSERT, UPDATE and WHERE IN commands are allowed when using helpers");
          }
          if (i > 0 && command !== SQLCommand.whereIn) {
            throw new SyntaxError(`${commandToString(command)} helper must be the first value`);
          }
          ...
          if (command === SQLCommand.insert) {
            ...
            if (!$isArray(items)) {
              throw new SyntaxError("An array of values is required for WHERE IN");
            }
            const itemsCount = items.length;
            const lastItemIndex = itemsCount - 1;
            query += "(";
            for (let j = 0; j < itemsCount; j++) {
              ...
              if (columnCount > 0) {
                ...
                if (columnCount > 1) {
                  ...
                  throw new SyntaxError("Cannot use WHERE IN with multiple columns");
                }
                const value = items[j];
