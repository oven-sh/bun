...
export const packageJsonInternals = {
  /**
   * Drives `SideEffects.buildAbsolutePattern` + `hasSideEffects` on a
   * synthetic `(package_dir, patterns, runtime_path)` triple so tests can
   * exercise the Windows-path matching fix for #30320 on any platform.
   *
   ...
   *
   * `usePreFix: true` routes through the pre-#30320 `r.fs.join` code path
