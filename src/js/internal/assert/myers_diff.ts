...
export function myersDiff(actual, expected, checkCommaDisparity = false) {
...
function backtrack(trace, actual, expected, checkCommaDisparity) {
  ...
  for (let diffLevel = trace.length - 1; diffLevel >= 0; diffLevel--) {
    ...
    while (x > prevX && y > prevY) {
      const value =
        !checkCommaDisparity || StringPrototypeEndsWith(actual[x - 1], ",") ? actual[x - 1] : expected[y - 1];
...
export function printSimpleMyersDiff(diff) {
  let message = "";
...
export function printMyersDiff(diff, simple = false) {
  let message = "";
  let skipped = false;
  let nopCount = 0;
