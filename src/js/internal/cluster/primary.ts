...
class ReusePortHandle {
  ...
  constructor(key, message, owned) {
    ...
    server.once("error", err => {
      this.errno = typeof err.errno === "number" && err.errno !== 0 ? -Math.abs(err.errno) : -1;
      this.#settle();
    });
...
function shareListenFd(worker, message) {
  ...
  try {
    send(worker, { errno: 0, ack: message.seq }, { fd }, undefined, "net.Native");
  } catch {
...
function send(worker, message, handle?, cb?, handleType?) {
  if (handle) {
    ...
    message = { cmd: "NODE_HANDLE", type: handleType || "dgram.Native", message: { ...message, cmd: "NODE_CLUSTER" } };
  } else {
