// Port of Node.js lib/internal/debugger/inspect.js (v26.3.0) — the entry
// point for the `bun inspect` / `node inspect` CLI debugger.
"use strict";
...
const { launchChildProcess, writeInspectUsageAndExit } = require("internal/debugger/inspect_helpers");
const { parseProbeTokens, runProbeMode } = require("internal/debugger/inspect_probe");
const createRepl = require("internal/debugger/inspect_repl");

const debuglog = util.debuglog("inspect");

// Node's process exit codes (internalBinding('errors').exitCodes upstream).
  ...
  function handleUnexpectedError(e) {
    if (e.code !== "ERR_DEBUGGER_STARTUP_ERROR") {
      process.stderr.write(
        "There was an internal error in Bun's debugger. Please report this bug.\n" + `${e.message}\n${e.stack}\n`,
      );
    } else {
...
export default { start: startInspect };
