// Port of Node.js lib/internal/debugger/inspect_client.js (v26.3.0).
"use strict";
...
function unpackError({ code, message }) {
  const err = $ERR_DEBUGGER_ERROR(`${message}`);
  err.code = code;
...
function validateHandshake(requestKey, responseKey) {
  ...
  if (shabuf.toString("base64") !== responseKey) {
    throw $ERR_DEBUGGER_ERROR(`WebSocket secret mismatch: ${requestKey} did not match ${responseKey}`);
  }
}
...
function encodeFrameHybi17(payload) {
  ...
  const header = Buffer.from([kFinalBit | kOpCodeText, kMaskBit | singleByteLength]);
...
function decodeFrameHybi17(data) {
  ...
  if (compressed) {
    throw $ERR_DEBUGGER_ERROR("Compressed frames not supported");
  }
  if (!final || reserved2 || reserved3) {
    throw $ERR_DEBUGGER_ERROR("Only compression extension is supported");
  }

  if (masked) {
    throw $ERR_DEBUGGER_ERROR("Masked server frame - not supported");
  }
  ...
  switch (opCode) {
    ...
    case kOpCodeText:
      break;
    default:
      throw $ERR_DEBUGGER_ERROR(`Unsupported op code ${opCode}`);
  }
...
class Client extends EventEmitter {
  ...
  _handleChunk(chunk) {
    ...
    while (this._unprocessed.length > 2) {
      ...
      if (payloadStr[0] !== "{" || lastChar !== "}") {
        throw $ERR_DEBUGGER_ERROR(`Payload does not look like JSON: ${payloadStr}`);
      }
      let payload;
      try {
      ...
      if (id) {
        ...
        throw $ERR_DEBUGGER_ERROR(`Unsupported response: ${payloadStr}`);
      }
    }
  }
  ...
  callMethod(method, params) {
    return new Promise((resolve, reject) => {
      if (!this._socket) {
        reject($ERR_DEBUGGER_ERROR("Use `run` to start the app again."));
        return;
      }
      ...
      function onResponse(httpRes) {
        function parseChunks() {
          ...
          try {
            ...
            reject($ERR_DEBUGGER_ERROR(`Response didn't contain JSON: ${resBody}`));
          }
        }
