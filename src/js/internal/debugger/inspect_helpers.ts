// Port of Node.js lib/internal/debugger/inspect_helpers.js (v26.3.0) for the
...
const { setInterval: pSetInterval, setTimeout: pSetTimeout } = require("node:timers/promises");

// Node's exit code for an invalid command line argument.
...
async function portIsFree(host, port, timeout = 3000) {
  ...
  while (true) {
    ...
    if (signal.aborted) {
      throw ERR_DEBUGGER_STARTUP_ERROR(`Timeout (${timeout}) waiting for ${host}:${port} to be free`);
    }
...
async function launchChildProcess(childArgs, inspectHost, inspectPort, childOutput, options = { __proto__: null }) {
  if (!options.skipPortPreflight) {
    await portIsFree(inspectHost, inspectPort);
  }

  // Bun's --inspect-brk injects a `debugger;`, making interactive sessions
  // stop twice; `deferBreakToClient` uses --inspect-wait and arms the break in
  // NodeInspector.run. Probe mode keeps --inspect-brk so the entry is parsed.
  ...
  return new Promise((resolve, reject) => {
    function rejectLaunch(message) {
      reject(ERR_DEBUGGER_STARTUP_ERROR(message, { childStderr: stderrOutput }));
    }
