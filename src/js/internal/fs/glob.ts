// @ts-nocheck -- the bottom of this file embeds a vendored copy of minimatch
...
class Glob {
  ...
  constructor(pattern, options = kEmptyObject) {
    ...
    this.matchers = patterns.map(pattern => createMatcher(pattern));
    this.#patterns = this.matchers.flatMap(matcher =>
      matcher.set.map((pattern, i) => new Pattern(pattern, matcher.globParts[i], new Set().add(0), new Set())),
    );
  }
...
function lazyMinimatch() {
  if (_minimatch) return _minimatch;
  const exports: any = {};
      ...
      var AST = class {
        ...
        toRegExpSource(allowDot) {
          ...
          if (!isExtglobAST(this)) {
            ...
            const src = this.#parts
              .map(p => {
                const [re, _, hasMagic, uflag] =
                  typeof p === "string" ? _a.#parseGlob(p, this.#hasMagic, noEmpty) : p.toRegExpSource(allowDot);
            ...
            return [final2, (0, unescape_js_12.unescape)(src), (this.#hasMagic = !!this.#hasMagic), this.#uflag];
          }
          ...
          return [final, (0, unescape_js_12.unescape)(body), (this.#hasMagic = !!this.#hasMagic), this.#uflag];
        }
        #flatten() {
          if (!isExtglobAST(this)) {
            for (const p of this.#parts) {
  ...
  var qmarksRE = /^\?+([^+@!?*[(]*)?$/;
  var qmarksTestNocase = ([m0, ext2 = ""]) => {
    const noext = qmarksTestNoExt([m0]);
    if (!ext2) return noext;
    ext2 = ext2.toLowerCase();
    return f => noext(f) && f.toLowerCase().endsWith(ext2);
  };
  var qmarksTestNocaseDot = ([m0, ext2 = ""]) => {
    const noext = qmarksTestNoExtDot([m0]);
    if (!ext2) return noext;
    ext2 = ext2.toLowerCase();
    return f => noext(f) && f.toLowerCase().endsWith(ext2);
  };
  var qmarksTestDot = ([m0, ext2 = ""]) => {
    const noext = qmarksTestNoExtDot([m0]);
    return !ext2 ? noext : f => noext(f) && f.endsWith(ext2);
  };
  var qmarksTest = ([m0, ext2 = ""]) => {
    const noext = qmarksTestNoExt([m0]);
    return !ext2 ? noext : f => noext(f) && f.endsWith(ext2);
  };
  var qmarksTestNoExt = ([m0]) => {
    const len = m0.length;
    return f => f.length === len && !f.startsWith(".");
  };
  var qmarksTestNoExtDot = ([m0]) => {
    const len = m0.length;
