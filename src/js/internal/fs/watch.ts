...
function createIgnoreMatcher(ignore) {
  ...
  for (const matcher of matchers) {
    if (typeof matcher === "string") {
      ...
      if (matcher.includes("/")) {
        compiled.push(filename => glob.match(filename));
      } else {
        // matchBase: patterns without slashes match against the basename
        compiled.push(filename => glob.match(filename) || glob.match(basename(filename)));
      }
    } else if (matcher instanceof RegExp) {
      compiled.push(filename => matcher.exec(filename) !== null);
...
class FSWatcher extends EventEmitter {
  ...
  static {
    closeNativeWatcher = watcher => {
      watcher.#watcher?.close();
      watcher.#watcher = null;
    };
    refNativeWatcher = watcher => {
      watcher.#watcher?.ref();
    };
    unrefNativeWatcher = watcher => {
