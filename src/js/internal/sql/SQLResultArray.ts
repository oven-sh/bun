export class SQLResultArray<T = any> extends Array<T> {
  ...
  constructor(...args: any[]) {
    super(...(args.length > 1 || (args.length === 1 && typeof args[0] === "number") ? args : [0])); // Handles Array(len) vs Array(...items)
    if (args.length === 1 && Array.isArray(args[0])) {
      args[0].forEach(item => this.push(item));
    } else if (args.length > 1 || (args.length === 1 && typeof args[0] !== "number")) {
