...
const {
  ...
  init: initPostgres,
} = $rust("postgres.rust", "createBinding") as PostgresDotRust;
...
export interface PostgresDotRust {
  init: (
    onResolveQuery: (
      query: Query<any, any>,
  ...
  createConnection: (
    ...
    query: string,
    path: string,
    onConnected: (err: Error | null, connection: $RustGeneratedClasses.PostgresSQLConnection) => void,
    onDisconnected: (err: Error | null, connection: $RustGeneratedClasses.PostgresSQLConnection) => void,
    idleTimeout: number,
    connectionTimeout: number,
    maxLifetime: number,
    useUnnamedPreparedStatements: boolean,
  ) => $RustGeneratedClasses.PostgresSQLConnection;
  createQuery: (
    sql: string,
    values: unknown[],
    ...
    bigint: boolean,
    simple: boolean,
  ) => $RustGeneratedClasses.PostgresSQLQuery;
}
...
class PooledPostgresConnection {
  ...
  ): Promise<$RustGeneratedClasses.PostgresSQLConnection | null> {
    const {
      hostname,
      port,
      username,
  ...
  connection: $RustGeneratedClasses.PostgresSQLConnection | null = null;
