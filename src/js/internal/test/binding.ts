...
// node answers this from nghttp2_strerror(); Bun's HTTP/2 stack does not link
// nghttp2, so the strings live here, built on first use (test-http2-binding).
let nghttp2ErrorMessages: Record<number, string> | undefined;
function nghttp2ErrorString(code: number) {
  nghttp2ErrorMessages ??= {
    [0]: "Success",
