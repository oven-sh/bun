...
function copyArrayBuffer(src, dst, byteOffset, byteLength) {
  dst.set(new Uint8Array(src.slice(byteOffset, byteOffset + byteLength)));
}
...
function isPromisePending(promise) {
  ...
  return promise.toString().includes("[pending]"); // FIXME: this is horrible
}
