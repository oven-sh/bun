...
// instead of making console.log() throw.
function makeConsoleWriter(stdout: Writable, stderr: Writable) {
  return (chunk: Buffer, fd: number) => {
    const stream = fd === 2 ? stderr : stdout;
    if (stream.writable === false) return;
    try {
      stream.write(chunk);
    } catch {}
  };
