...
// ---------------------------------------------------------------------------
// Socket callbacks — called by Rust with `this` = native handle (not useful).
...
//   rawSocket (TCP) → upgradeDuplexToTLS (Rust TLS layer) → socket callbacks
//     → tlsSocket.push() → H2 session reads
//   H2 session writes → tlsSocket._write() → handle.$write() → Rust TLS layer → rawSocket
//
...
// Rust level, completely bypassing our JS data callback and Duplex.push() path.
...
function upgradeRawSocketToH2(
  ...
  // socket: callbacks — bind to tlsSocket since Rust calls them with native handle as `this`
  ...
  try {
    ...
    // for ["h2"]). The Rust SSLConfig expects an ArrayBuffer, so we slice the underlying buffer.
  ...
  // intercept data at the Rust level and bypass our Duplex push path.
  ...
  // the Rust TLS engine expects to receive data/end/drain/close through.
