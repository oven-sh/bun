import { ConnResetException, controllerSymbol, deferredSymbol, drainMicrotasks, eofInProgress, firstWriteSymbol, hasServerResponseFinished, headersSymbol, isTlsSymbol, kCloseCallback, kHandle, kHeaderState, kInternalSocketData, kPendingCallbacks, kRealListen, kRequest, NodeHTTPBodyReadState, NodeHTTPHeaderState, NodeHTTPResponseAbortEvent, NodeHTTPResponseFlags, optionsSymbol, runSymbol, serverSymbol, setServerIdleTimeout, STATUS_CODES, swapIsNextIncomingMessageHTTPS, tlsSymbol } from "internal/http/share";
import { isValidTLSArray } from "internal/tls";
import { validateBoolean, validateInteger, validateLinkHeaderValue, validateObject } from "internal/validators";
...
type Server = any;
type NodeHTTPServerSocket = any;

const kConnectionsCheckingInterval = Symbol('kConnectionsCheckingInterval');

// TODO: why this instead of $Promise
...
export function ServerResponse(req, options): void {
...
const ServerResponsePrototype = {
  ...
  get headersSent() {
    return (
      this[kHeaderState] === NodeHTTPHeaderState.sent || this[kHeaderState] === NodeHTTPHeaderState.assigned
    );
  },
  set headersSent(value) {
    this[kHeaderState] = value ? NodeHTTPHeaderState.sent : NodeHTTPHeaderState.none;
  },
  ...
  // But we don't want it for the fetch() response version.
    ...
    const headerState = this[kHeaderState];
    ...
    if (headerState !== NodeHTTPHeaderState.sent) {
      handle.cork(() => {
        ...
        // So we set it here.
        this[kHeaderState] = NodeHTTPHeaderState.sent;
  ...
  get writable() {
    return !hasServerResponseFinished(this, undefined, undefined);
  },
  ...
  _send(data, encoding, callback, byteLength) {
    const handle = this[kHandle];
    if (!handle) {
      return OutgoingMessage.prototype._send.$apply(this, arguments);
    }
  ...
  writeHead(statusCode, statusMessage, headers) {
    if (this[kHeaderState] === NodeHTTPHeaderState.none) {
      _writeHead(statusCode, statusMessage, headers, this);
      updateHasBody(this, statusCode);
      this[kHeaderState] = NodeHTTPHeaderState.assigned;
    }
...
export function Server(options, callback): void   {
  ...
  [kRealListen](tls, port, host, socketPath, reusePort, onListen) {
    {
      ...
      this[serverSymbol] = Bun.serve<any>({
        ...
        onNodeHTTPRequest(
          ...
          const prevIsNextIncomingMessageHTTPS = swapIsNextIncomingMessageHTTPS(isHTTPS);
          if (!socket) {
          ...
          swapIsNextIncomingMessageHTTPS(prevIsNextIncomingMessageHTTPS);
...
function storeHTTPOptions(options) {
  ...
  if (headersTimeout !== undefined) {
    ...
    this.headersTimeout = $min(60_000, this.requestTimeout); // Minimum between 60 seconds or requestTimeout
  }
