...
const { ConnectionsList } = process.binding("http_parser");
const { kUniqueHeaders, parseUniqueHeadersOption, OutgoingMessage } = require("node:_http_outgoing");
const { kOutHeaders, kNeedDrain, STATUS_CODES } = require("internal/http");
const { IncomingMessage } = require("node:_http_incoming");
const { ConnResetException, kEmptyObject } = require("internal/shared");
...
$toClass(Server, "Server", net.Server);
...
function connectionListenerInternal(server, socket) {
  ...
  const lenient = server.insecureHTTPParser === undefined ? isLenient() : server.insecureHTTPParser;
...
function socketSetEncoding() {
  throw $ERR_HTTP_SOCKET_ENCODING();
}
...
function onRequestTimeout(socket) {
  // socketOnError has additional logic and will call socket.destroy(err).
  socketOnError.$call(socket, $ERR_HTTP_REQUEST_TIMEOUT());
}
