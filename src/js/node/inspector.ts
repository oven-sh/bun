...
const startCPUProfiler = $newCppFunction(
  'JSInspectorProfiler.cpp',
  'jsFunction_startCPUProfiler',
  0,
)
const stopCPUProfiler = $newCppFunction(
  'JSInspectorProfiler.cpp',
  'jsFunction_stopCPUProfiler',
  0,
)
const setCPUSamplingInterval = $newCppFunction(
  'JSInspectorProfiler.cpp',
  'jsFunction_setCPUSamplingInterval',
  1,
)
const isCPUProfilerRunning = $newCppFunction(
  'JSInspectorProfiler.cpp',
  'jsFunction_isCPUProfilerRunning',
  0,
)
    ...
    switch (method) {
      ...
      case 'Profiler.start':
        if (!this.#profilerEnabled)
          return new Error(
            'Profiler is not enabled. Call Profiler.enable first.',
          )
      ...
      case 'Profiler.stop':
        if (!isCPUProfilerRunning())
          return new Error(
            'Profiler is not started. Call Profiler.start first.',
          )
        try {
