...
// (the native `SSLContextCache` underneath is shared with every Rust consumer
...
const NativeSecureContext = $rust("SecureContext.rust", "js.getConstructor");
...
TLSSocket.prototype.getCertificate = function getCertificate() {
  // need to implement certificate on socket.rust
...
function Server(options, secureConnectionListener): void {
  ...
  this.addContext = function (hostname, context) {
    ...
    if (this._handle) {
      ...
      // the Rust side detects it via SecureContext.fromJS and up_refs.
...
const getUseSystemCA = $newRustFunction("bun.rust", "getUseSystemCA", 0);
