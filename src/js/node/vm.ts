...
// alive; dead entries are pruned on each measurement.
...
class SourceTextModule extends Module {
  ...
  linkRequests(modules) {
    ...
    const moduleRequests = this.#moduleRequests;
    if (modules.length !== moduleRequests.length) {
      throw $ERR_MODULE_LINK_MISMATCH(`Expected ${moduleRequests.length} modules, got ${modules.length}`);
    }
    const specifiers = Array(modules.length);
    const moduleNatives = Array(modules.length);
