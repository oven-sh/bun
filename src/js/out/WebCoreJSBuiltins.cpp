...
const int s_moduleRequireCodeLength = 871;
...
const char* const s_readableStreamInternalsOnPullDirectStreamCode = "(function (h){\"use strict\";var i=h.@controlledReadableStream;if(!i||@getByIdDirectPrivate(i,\"state\")!==@streamReadable)return;if(h._deferClose===-1)return;h._deferClose=-1,h._deferFlush=-1;var b,g;try{var y=h.@underlyingSource.pull(h);if(y&&@isPromise(y)){if(h._handleError===@undefined)h._handleError=@handleDirectStreamErrorReject.bind(h);@Promise.prototype.catch.@call(y,h._handleError)}}catch(k){return @handleDirectStreamErrorReject.@call(h,k)}finally{b=h._deferClose,g=h._deferFlush,h._deferFlush=h._deferClose=0}var j;if(h._pendingRead===@undefined)h._pendingRead=j=@newPromise();else j=@readableStreamAddReadRequest(i);if(b===1){var _=h._deferCloseReason;return h._deferCloseReason=@undefined,@onCloseDirectStream.@call(h,_),j}if(g===1)@onFlushDirectStream.@call(h);return j})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const _=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!_)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const x=@getByIdDirectPrivate(_,\"state\");if(@putByIdDirectPrivate(_,\"disturbed\",!0),x===@streamClosed)return{value:[],size:0,done:!0};else if(x===@streamErrored)throw @getByIdDirectPrivate(_,\"storedError\");var F=@getByIdDirectPrivate(_,\"readableStreamController\"),G=@getByIdDirectPrivate(F,\"queue\");if(!G)return F.@pull(F).@then(function({done:w,value:C}){return w\?{done:!0,value:[],size:0}:{value:[C],size:1,done:!1}});const H=G.content;var I=G.size,E=H.toArray(!1),J=E.length;if(J>0){var K=@newArrayWithSize(J);if(@isReadableByteStreamController(F)){{const w=E[0];if(!(@ArrayBuffer.@isView(w)||w instanceof @ArrayBuffer))@putByValDirect(K,0,new @Uint8Array(w.buffer,w.byteOffset,w.byteLength));else @putByValDirect(K,0,w)}for(var d=1;d<J;d++){const w=E[d];if(!(@ArrayBuffer.@isView(w)||w instanceof @ArrayBuffer))@putByValDirect(K,d,new @Uint8Array(w.buffer,w.byteOffset,w.byteLength));else @putByValDirect(K,d,w)}}else{@putByValDirect(K,0,E[0].value);for(var d=1;d<J;d++)@putByValDirect(K,d,E[d].value)}if(@resetQueue(@getByIdDirectPrivate(F,\"queue\")),@getByIdDirectPrivate(F,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(F,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(F))@readableStreamDefaultControllerCallPullIfNeeded(F);else if(@isReadableByteStreamController(F))@readableByteStreamControllerCallPullIfNeeded(F);return{value:K,size:I,done:!1}}var B=(w)=>{if(w.done)return{value:[],size:0,done:!0};var C=@getByIdDirectPrivate(_,\"readableStreamController\"),N=@getByIdDirectPrivate(C,\"queue\"),Q=[w.value].concat(N.content.toArray(!1)),S=Q.length;if(@isReadableByteStreamController(C))for(var T=0;T<S;T++){const U=Q[T];if(!(@ArrayBuffer.@isView(U)||U instanceof @ArrayBuffer)){const{buffer:W,byteOffset:A,byteLength:D}=U;@putByValDirect(Q,T,new @Uint8Array(W,A,D))}}else for(var T=1;T<S;T++)@putByValDirect(Q,T,Q[T].value);var j=N.size;if(@resetQueue(N),@getByIdDirectPrivate(C,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(C,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(C))@readableStreamDefaultControllerCallPullIfNeeded(C);else if(@isReadableByteStreamController(C))@readableByteStreamControllerCallPullIfNeeded(C);return{value:Q,size:j,done:!1}},k=F.@pull(F);if(k&&@isPromise(k))return k.@then(B);return B(k)})\n";
