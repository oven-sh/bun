...
const int s_importMetaObjectCreateRequireCacheCodeLength = 1048;
static const JSC::Intrinsic s_importMetaObjectCreateRequireCacheCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";class _{id;parent;filename;children=[];paths=[];require;constructor(g){this.id=g;const j=g.lastIndexOf(\"/\");if(j!==-1&&g.length>j+1)this.filename=g.substring(j+1);else this.filename=g;this.require=this.#$.bind(this),this.require.resolve=this.#_.bind(this),this.require.cache=b,this.require.main=@undefined}get loaded(){return!0}#_(g,j){return @resolveSync(g,this.id)}#$(g){return @internalRequire(this.#_(g,@undefined))}get exports(){return @requireMap.@get(this.id)\?\?{}}set exports(g){@requireMap.@set(this.id,g)}}var c=new Map,b=new Proxy({},{get(g,j){if(@requireMap.@get(j)){var x=c.@get(j);if(!x)x=new _(j),c.@set(j,x);return x}},set(g,j,w){if(!c.@has(j))c.@set(j,new _(j));return @requireMap.@set(j,w\?.exports),!0},has(g,j){return @requireMap.@has(j)},deleteProperty(g,j){return c.@delete(j),@requireMap.@delete(j),@Loader.registry.@delete(j),!0},ownKeys(g){return[...@requireMap.@keys()]},getPrototypeOf(g){return null},getOwnPropertyDescriptor(g,j){if(@requireMap.@has(j))return{configurable:!0,enumerable:!0}}})})\n";
