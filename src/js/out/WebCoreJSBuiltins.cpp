...
const char* const s_readableStreamInternalsLazyLoadStreamCode = "(function (b,f){\"use strict\";var j=@getByIdDirectPrivate(b,\"bunNativeType\"),m=@getByIdDirectPrivate(b,\"bunNativePtr\"),q=@lazyStreamPrototypeMap.@get(j);if(q===@undefined){let Q=function(Y){var{c:Z,v:_}=this;this.c=@undefined,this.v=@undefined,I(Y,Z,_)},U=function(Y){try{Y.close()}catch(Z){globalThis.reportError(Z)}},W=function(Y,Z,_,p){p[0]=!1;var z;try{z=x(Y,_,p)}catch(A){return Z.error(A)}return I(z,Z,_)};var O=Q,P=U,N=W,[x,y,B,D,E,F,G]=@lazyLoad(j),H=[!1],I;I=function Y(Z,_,p){if(Z&&@isPromise(Z))return Z.then(Q.bind({c:_,v:p}),(z)=>_.error(z));else if(typeof Z===\"number\")if(p&&p.byteLength===Z&&p.buffer===_.byobRequest\?.view\?.buffer)_.byobRequest.respondWithNewView(p);else _.byobRequest.respond(Z);else if(Z.constructor===@Uint8Array)_.enqueue(Z);if(H[0]||Z===!1)@enqueueJob(U,_),H[0]=!1};const X=E\?new FinalizationRegistry(E):null;q=class Y{constructor(Z,_,p){if(this.#f=Z,this.#b={},this.pull=this.#j.bind(this),this.cancel=this.#m.bind(this),this.autoAllocateChunkSize=_,p!==@undefined)this.start=(z)=>{z.enqueue(p)};if(X)X.register(this,Z,this.#b)}#b;pull;cancel;start;#f;type=\"bytes\";autoAllocateChunkSize=0;static startSync=y;#j(Z){var _=this.#f;if(!_){Z.close();return}W(_,Z,Z.byobRequest.view,H)}#m(Z){var _=this.#f;X&&X.unregister(this.#b),F&&F(_,!1),B(_,Z)}static deinit=E;static drain=G},@lazyStreamPrototypeMap.@set(j,q)}const J=q.startSync(m,f);var K;const{drain:L,deinit:M}=q;if(L)K=L(m);if(J===0){if(E&&m&&@enqueueJob(E,m),(K\?.byteLength\?\?0)>0)return{start(Q){Q.enqueue(K),Q.close()},type:\"bytes\"};return{start(Q){Q.close()},type:\"bytes\"}}return new q(m,J,K)})\n";
