...
const char* const s_bundlerPluginRunOnLoadPluginsCode = "(function (T,b,Q,w){\"use strict\";const H={jsx:0,js:1,ts:2,tsx:3,css:4,file:5,json:6,toml:7,wasm:8,napi:9,base64:10,dataurl:11,text:12},G=[\"jsx\",\"js\",\"ts\",\"tsx\",\"css\",\"file\",\"json\",\"toml\",\"wasm\",\"napi\",\"base64\",\"dataurl\",\"text\"][w];var J=(async(j,v,q,y)=>{var g=this.onLoad.@get(q);if(!g)return this.onLoadAsync(j,null,null),null;for(let[x,F]of g)if(x.test(v)){var B=F({path:v,namespace:q,loader:y});while(B&&@isPromise(B)&&(@getPromiseInternalField(B,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)B=@getPromiseInternalField(B,@promiseFieldReactionsOrResult);if(B&&@isPromise(B))B=await B;if(!B||!@isObject(B))continue;var{contents:K,loader:z=y}=B;if(typeof K!==\"string\"&&!@isTypedArrayView(K))@throwTypeError('onLoad plugins must return an object with \"contents\" as a string or Uint8Array');if(typeof z!==\"string\")@throwTypeError('onLoad plugins must return an object with \"loader\" as a string');const C=H[z];if(C===@undefined)@throwTypeError(`Loader ${z} is not supported.`);return this.onLoadAsync(j,K,C),null}return this.onLoadAsync(j,null,null),null})(T,b,Q,G);while(J&&@isPromise(J)&&(@getPromiseInternalField(J,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)J=@getPromiseInternalField(J,@promiseFieldReactionsOrResult);if(J&&@isPromise(J))J.then(()=>{},(j)=>{this.addError(T,j,1)})})\n";
...
const char* const s_writableStreamInternalsWritableStreamUpdateBackpressureCode = "(function (d,h){\"use strict\";@assert(@getByIdDirectPrivate(d,\"state\")===\"writable\"),@assert(!@writableStreamCloseQueuedOrInFlight(d));const n=@getByIdDirectPrivate(d,\"writer\");if(n!==@undefined&&h!==@getByIdDirectPrivate(d,\"backpressure\"))if(h)@putByIdDirectPrivate(n,\"readyPromise\",@newPromiseCapability(@Promise));else @getByIdDirectPrivate(n,\"readyPromise\").@resolve.@call();@putByIdDirectPrivate(d,\"backpressure\",h)})\n";
...
const char* const s_writableStreamInternalsWritableStreamDefaultWriterEnsureReadyPromiseRejectedCode = "(function (c,n){\"use strict\";let h=@getByIdDirectPrivate(c,\"readyPromise\"),g=h.@promise;if((@getPromiseInternalField(g,@promiseFieldFlags)&@promiseStateMask)!==@promiseStatePending)h=@newPromiseCapability(@Promise),g=h.@promise,@putByIdDirectPrivate(c,\"readyPromise\",h);h.@reject.@call(@undefined,n),@markPromiseAsHandled(g)})\n";
...
const char* const s_writableStreamInternalsSetUpWritableStreamDefaultControllerCode = "(function (_,U,B,d,v,q,P,f){\"use strict\";@assert(@isWritableStream(_)),@assert(@getByIdDirectPrivate(_,\"controller\")===@undefined),@putByIdDirectPrivate(U,\"stream\",_),@putByIdDirectPrivate(_,\"controller\",U),@resetQueue(@getByIdDirectPrivate(U,\"queue\")),@putByIdDirectPrivate(U,\"started\",-1),@putByIdDirectPrivate(U,\"startAlgorithm\",B),@putByIdDirectPrivate(U,\"strategySizeAlgorithm\",f),@putByIdDirectPrivate(U,\"strategyHWM\",P),@putByIdDirectPrivate(U,\"writeAlgorithm\",d),@putByIdDirectPrivate(U,\"closeAlgorithm\",v),@putByIdDirectPrivate(U,\"abortAlgorithm\",q);const j=@writableStreamDefaultControllerGetBackpressure(U);@writableStreamUpdateBackpressure(_,j),@writableStreamDefaultControllerStart(U)})\n";
...
const char* const s_writableStreamInternalsSetUpWritableStreamDefaultControllerFromUnderlyingSinkCode = "(function (_,f,j,p,q){\"use strict\";const x=new @WritableStreamDefaultController;let B=()=>{},C=()=>{return @Promise.@resolve()},E=()=>{return @Promise.@resolve()},F=()=>{return @Promise.@resolve()};if(\"start\"in j){const v=j[\"start\"];B=()=>@promiseInvokeOrNoopMethodNoCatch(f,v,[x])}if(\"write\"in j){const v=j[\"write\"];C=(P)=>@promiseInvokeOrNoopMethod(f,v,[P,x])}if(\"close\"in j){const v=j[\"close\"];E=()=>@promiseInvokeOrNoopMethod(f,v,[])}if(\"abort\"in j){const v=j[\"abort\"];F=(P)=>@promiseInvokeOrNoopMethod(f,v,[P])}@setUpWritableStreamDefaultController(_,x,B,C,E,F,p,q)})\n";
...
const char* const s_transformStreamInitializeTransformStreamCode = "(function (){\"use strict\";let _=arguments[0];if(@isObject(_)&&@getByIdDirectPrivate(_,\"TransformStream\"))return this;let v=arguments[1],B=arguments[2];if(_===@undefined)_=null;if(B===@undefined)B={};if(v===@undefined)v={};let E={};if(_!==null){if(\"start\"in _){if(E[\"start\"]=_[\"start\"],typeof E[\"start\"]!==\"function\")@throwTypeError(\"transformer.start should be a function\")}if(\"transform\"in _){if(E[\"transform\"]=_[\"transform\"],typeof E[\"transform\"]!==\"function\")@throwTypeError(\"transformer.transform should be a function\")}if(\"flush\"in _){if(E[\"flush\"]=_[\"flush\"],typeof E[\"flush\"]!==\"function\")@throwTypeError(\"transformer.flush should be a function\")}if(\"readableType\"in _)@throwRangeError(\"TransformStream transformer has a readableType\");if(\"writableType\"in _)@throwRangeError(\"TransformStream transformer has a writableType\")}const F=@extractHighWaterMark(B,0),G=@extractSizeAlgorithm(B),I=@extractHighWaterMark(v,1),x=@extractSizeAlgorithm(v),J=@newPromiseCapability(@Promise);if(@initializeTransformStream(this,J.@promise,I,x,F,G),@setUpTransformStreamDefaultControllerFromTransformer(this,_,E),(\"start\"in E)){const u=@getByIdDirectPrivate(this,\"controller\");(()=>@promiseInvokeOrNoopMethodNoCatch(_,E[\"start\"],[u]))().@then(()=>{J.@resolve.@call()},(q)=>{J.@reject.@call(@undefined,q)})}else J.@resolve.@call();return this})\n";
...
const char* const s_jsBufferPrototypeWriteIntLECode = "(function (d,j,c){\"use strict\";const r=this.@dataView||=new DataView(this.buffer,this.byteOffset,this.byteLength);switch(c){case 1:{r.setInt8(j,d);break}case 2:{r.setInt16(j,d,!0);break}case 3:{r.setUint16(j,d&65535,!0),r.setInt8(j+2,Math.floor(d*0.0000152587890625));break}case 4:{r.setInt32(j,d,!0);break}case 5:{r.setUint32(j,d|0,!0),r.setInt8(j+4,Math.floor(d*0.00000000023283064365386964));break}case 6:{r.setUint32(j,d|0,!0),r.setInt16(j+4,Math.floor(d*0.00000000023283064365386964),!0);break}default:@throwRangeError(\"byteLength must be >= 1 and <= 6\")}return j+c})\n";
...
const char* const s_readableStreamInternalsReadableStreamTeeCode = "(function (f,j){\"use strict\";@assert(@isReadableStream(f)),@assert(typeof j===\"boolean\");var k=@getByIdDirectPrivate(f,\"start\");if(k)@putByIdDirectPrivate(f,\"start\",@undefined),k();const q=new @ReadableStreamDefaultReader(f),i={closedOrErrored:!1,canceled1:!1,canceled2:!1,reason1:@undefined,reason2:@undefined};i.cancelPromiseCapability=@newPromiseCapability(@Promise);const w=@readableStreamTeePullFunction(i,q,j),_={};@putByIdDirectPrivate(_,\"pull\",w),@putByIdDirectPrivate(_,\"cancel\",@readableStreamTeeBranch1CancelFunction(i,f));const x={};@putByIdDirectPrivate(x,\"pull\",w),@putByIdDirectPrivate(x,\"cancel\",@readableStreamTeeBranch2CancelFunction(i,f));const y=new @ReadableStream(_),g=new @ReadableStream(x);return @getByIdDirectPrivate(q,\"closedPromiseCapability\").@promise.@then(@undefined,function(v){if(i.closedOrErrored)return;if(@readableStreamDefaultControllerError(y.@readableStreamController,v),@readableStreamDefaultControllerError(g.@readableStreamController,v),i.closedOrErrored=!0,!i.canceled1||!i.canceled2)i.cancelPromiseCapability.@resolve.@call()}),i.branch1=y,i.branch2=g,[y,g]})\n";
...
const char* const s_readableStreamInternalsOnPullDirectStreamCode = "(function (g){\"use strict\";var y=g.@controlledReadableStream;if(!y||@getByIdDirectPrivate(y,\"state\")!==@streamReadable)return;if(g._deferClose===-1)return;g._deferClose=-1,g._deferFlush=-1;var h,_;try{var b=g.@underlyingSource.pull(g);if(b&&@isPromise(b)){if(g._handleError===@undefined)g._handleError=@handleDirectStreamErrorReject.bind(g);@Promise.prototype.catch.@call(b,g._handleError)}}catch(k){return @handleDirectStreamErrorReject.@call(g,k)}finally{h=g._deferClose,_=g._deferFlush,g._deferFlush=g._deferClose=0}var i;if(g._pendingRead===@undefined)g._pendingRead=i=@newPromise();else i=@readableStreamAddReadRequest(y);if(h===1){var j=g._deferCloseReason;return g._deferCloseReason=@undefined,@onCloseDirectStream.@call(g,j),i}if(_===1)@onFlushDirectStream.@call(g);return i})\n";
...
const char* const s_readableStreamInternalsCreateTextStreamCode = "(function (_){\"use strict\";var v,x=[],q=!1,z=!1,w=\"\",A=@toLength(0),C=@newPromiseCapability(@Promise),E=!1;return v={start(){},write(j){if(typeof j===\"string\"){var F=@toLength(j.length);if(F>0)w+=j,q=!0,A+=F;return F}if(!j||!(@ArrayBuffer.@isView(j)||j instanceof @ArrayBuffer))@throwTypeError(\"Expected text, ArrayBuffer or ArrayBufferView\");const G=@toLength(j.byteLength);if(G>0)if(z=!0,w.length>0)@arrayPush(x,w,j),w=\"\";else @arrayPush(x,j);return A+=G,G},flush(){return 0},end(){if(E)return\"\";return v.fulfill()},fulfill(){E=!0;const j=v.finishInternal();return @fulfillPromise(C.@promise,j),j},finishInternal(){if(!q&&!z)return\"\";if(q&&!z)return w;if(z&&!q)return new globalThis.TextDecoder().decode(@Bun.concatArrayBuffers(x));var j=new @Bun.ArrayBufferSink;j.start({highWaterMark:A,asUint8Array:!0});for(let F of x)j.write(F);if(x.length=0,w.length>0)j.write(w),w=\"\";return new globalThis.TextDecoder().decode(j.end())},close(){try{if(!E)E=!0,v.fulfill()}catch(j){}}},[v,C]})\n";
...
const char* const s_readableStreamInternalsInitializeArrayStreamCode = "(function (_,t){\"use strict\";var p=[],d=@newPromiseCapability(@Promise),b=!1;function j(){return b=!0,d.@resolve.@call(@undefined,p),p}var m={start(){},write(v){return @arrayPush(p,v),v.byteLength||v.length},flush(){return 0},end(){if(b)return[];return j()},close(){if(!b)j()}},q={@underlyingSource:_,@pull:@onPullDirectStream,@controlledReadableStream:this,@sink:m,close:@onCloseDirectStream,write:m.write,error:@handleDirectStreamError,end:@onCloseDirectStream,@close:@onCloseDirectStream,flush:@onFlushDirectStream,_pendingRead:@undefined,_deferClose:0,_deferFlush:0,_deferCloseReason:@undefined,_handleError:@undefined};return @putByIdDirectPrivate(this,\"readableStreamController\",q),@putByIdDirectPrivate(this,\"underlyingSource\",@undefined),@putByIdDirectPrivate(this,\"start\",@undefined),d})\n";
...
const char* const s_readableStreamInternalsReadableStreamDefaultControllerPullCode = "(function (a){\"use strict\";var f=@getByIdDirectPrivate(a,\"queue\");if(f.content.isNotEmpty()){const d=@dequeueValue(f);if(@getByIdDirectPrivate(a,\"closeRequested\")&&f.content.isEmpty())@readableStreamClose(@getByIdDirectPrivate(a,\"controlledReadableStream\"));else @readableStreamDefaultControllerCallPullIfNeeded(a);return @createFulfilledPromise({value:d,done:!1})}const _=@readableStreamAddReadRequest(@getByIdDirectPrivate(a,\"controlledReadableStream\"));return @readableStreamDefaultControllerCallPullIfNeeded(a),_})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const k=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!k)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const d=@getByIdDirectPrivate(k,\"state\");if(@putByIdDirectPrivate(k,\"disturbed\",!0),d===@streamClosed)return{value:[],size:0,done:!0};else if(d===@streamErrored)throw @getByIdDirectPrivate(k,\"storedError\");var A=@getByIdDirectPrivate(k,\"readableStreamController\"),B=@getByIdDirectPrivate(A,\"queue\");if(!B)return A.@pull(A).@then(function({done:G,value:H}){return G\?{done:!0,value:[],size:0}:{value:[H],size:1,done:!1}});const w=B.content;var x=B.size,C=w.toArray(!1),E=C.length;if(E>0){var _=@newArrayWithSize(E);if(@isReadableByteStreamController(A)){{const G=C[0];if(!(@ArrayBuffer.@isView(G)||G instanceof @ArrayBuffer))@putByValDirect(_,0,new @Uint8Array(G.buffer,G.byteOffset,G.byteLength));else @putByValDirect(_,0,G)}for(var D=1;D<E;D++){const G=C[D];if(!(@ArrayBuffer.@isView(G)||G instanceof @ArrayBuffer))@putByValDirect(_,D,new @Uint8Array(G.buffer,G.byteOffset,G.byteLength));else @putByValDirect(_,D,G)}}else{@putByValDirect(_,0,C[0].value);for(var D=1;D<E;D++)@putByValDirect(_,D,C[D].value)}if(@resetQueue(@getByIdDirectPrivate(A,\"queue\")),@getByIdDirectPrivate(A,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(A,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(A))@readableStreamDefaultControllerCallPullIfNeeded(A);else if(@isReadableByteStreamController(A))@readableByteStreamControllerCallPullIfNeeded(A);return{value:_,size:x,done:!1}}var j=(G)=>{if(G.done)return{value:[],size:0,done:!0};var H=@getByIdDirectPrivate(k,\"readableStreamController\"),I=@getByIdDirectPrivate(H,\"queue\"),J=[G.value].concat(I.content.toArray(!1)),K=J.length;if(@isReadableByteStreamController(H))for(var N=0;N<K;N++){const S=J[N];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer)){const{buffer:T,byteOffset:U,byteLength:W}=S;@putByValDirect(J,N,new @Uint8Array(T,U,W))}}else for(var N=1;N<K;N++)@putByValDirect(J,N,J[N].value);var Q=I.size;if(@resetQueue(I),@getByIdDirectPrivate(H,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(H,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(H))@readableStreamDefaultControllerCallPullIfNeeded(H);else if(@isReadableByteStreamController(H))@readableByteStreamControllerCallPullIfNeeded(H);return{value:J,size:Q,done:!1}},F=A.@pull(A);if(F&&@isPromise(F))return F.@then(j);return j(F)})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (x){\"use strict\";var z=@Loader,B=@createFIFO(),F=x;while(F){var w=z.registry.@get(F);if((w\?.state\?\?0)<=@ModuleFetch)@fulfillModuleSync(F),w=z.registry.@get(F);var T=@getPromiseInternalField(w.fetch,@promiseFieldReactionsOrResult),D=z.parseModule(F,T),U=w.module;if(D&&@isPromise(D)){var V=@getPromiseInternalField(D,@promiseFieldReactionsOrResult),W=@getPromiseInternalField(D,@promiseFieldFlags),L=W&@promiseStateMask;if(L===@promiseStatePending||V&&@isPromise(V))@throwTypeError(`require() async module \"${F}\" is unsupported. use \"await import()\" instead.`);else if(L===@promiseStateRejected){if(!V\?.message)@throwTypeError(`${V+\"\"\?V:\"An error occurred\"} occurred while parsing module \\\"${F}\\\"`);throw V}w.module=U=V}else if(D&&!U)w.module=U=D;@setStateToMax(w,@ModuleLink);var _=U.dependenciesMap,G=z.requestedModules(U),Q=@newArrayWithSize(G.length);for(var H=0,X=G.length;H<X;++H){var J=G[H],Y=J[0]===\"/\"\?J:z.resolve(J,F),Z=z.ensureRegistered(Y);if(Z.state<@ModuleLink)B.push(Y);@putByValDirect(Q,H,Z),_.@set(J,Z)}w.dependencies=Q,w.instantiate=@Promise.resolve(w),w.satisfy=@Promise.resolve(w),F=B.shift();while(F&&(z.registry.@get(F)\?.state\?\?@ModuleFetch)>=@ModuleLink)F=B.shift()}var I=z.linkAndEvaluateModule(x,@undefined);if(I&&@isPromise(I))@throwTypeError(`require() async module \\\"${x}\\\" is unsupported. use \"await import()\" instead.`);return z.registry.@get(x)})\n";
...
const char* const s_readableStreamInitializeReadableStreamCode = "(function (_,b){\"use strict\";if(_===@undefined)_={@bunNativeType:0,@bunNativePtr:0,@lazy:!1};if(b===@undefined)b={};if(!@isObject(_))@throwTypeError(\"ReadableStream constructor takes an object as first argument\");if(b!==@undefined&&!@isObject(b))@throwTypeError(\"ReadableStream constructor takes an object as second argument, if any\");@putByIdDirectPrivate(this,\"state\",@streamReadable),@putByIdDirectPrivate(this,\"reader\",@undefined),@putByIdDirectPrivate(this,\"storedError\",@undefined),@putByIdDirectPrivate(this,\"disturbed\",!1),@putByIdDirectPrivate(this,\"readableStreamController\",null),@putByIdDirectPrivate(this,\"bunNativeType\",@getByIdDirectPrivate(_,\"bunNativeType\")\?\?0),@putByIdDirectPrivate(this,\"bunNativePtr\",@getByIdDirectPrivate(_,\"bunNativePtr\")\?\?0);const f=_.type===\"direct\",m=!!_.@lazy,v=f||m;if(@getByIdDirectPrivate(_,\"pull\")!==@undefined&&!v){const B=@getByIdDirectPrivate(b,\"size\"),I=@getByIdDirectPrivate(b,\"highWaterMark\");return @putByIdDirectPrivate(this,\"highWaterMark\",I),@putByIdDirectPrivate(this,\"underlyingSource\",@undefined),@setupReadableStreamDefaultController(this,_,B,I!==@undefined\?I:1,@getByIdDirectPrivate(_,\"start\"),@getByIdDirectPrivate(_,\"pull\"),@getByIdDirectPrivate(_,\"cancel\")),this}if(f)@putByIdDirectPrivate(this,\"underlyingSource\",_),@putByIdDirectPrivate(this,\"highWaterMark\",@getByIdDirectPrivate(b,\"highWaterMark\")),@putByIdDirectPrivate(this,\"start\",()=>@createReadableStreamController(this,_,b));else if(v){const B=_.autoAllocateChunkSize;@putByIdDirectPrivate(this,\"highWaterMark\",@undefined),@putByIdDirectPrivate(this,\"underlyingSource\",@undefined),@putByIdDirectPrivate(this,\"highWaterMark\",B||@getByIdDirectPrivate(b,\"highWaterMark\")),@putByIdDirectPrivate(this,\"start\",()=>{const I=@lazyLoadStream(this,B);if(I)@createReadableStreamController(this,I,b)})}else @putByIdDirectPrivate(this,\"underlyingSource\",@undefined),@putByIdDirectPrivate(this,\"highWaterMark\",@getByIdDirectPrivate(b,\"highWaterMark\")),@putByIdDirectPrivate(this,\"start\",@undefined),@createReadableStreamController(this,_,b);return this})\n";
...
const char* const s_readableStreamConsumeReadableStreamCode = "(function (j,A,B){\"use strict\";const D=globalThis.Symbol.for(\"Bun.consumeReadableStreamPrototype\");var G=globalThis[D];if(!G)G=globalThis[D]=[];var H=G[A];if(H===@undefined){var[I,_,F,J,k,q]=globalThis[globalThis.Symbol.for(\"Bun.lazy\")](A);H=class w{handleError;handleClosed;processResult;constructor(L,x){this.#_=x,this.#F=L,this.#$=!1,this.handleError=this._handleError.bind(this),this.handleClosed=this._handleClosed.bind(this),this.processResult=this._processResult.bind(this),L.closed.then(this.handleClosed,this.handleError)}_handleClosed(){if(this.#$)return;this.#$=!0;var L=this.#_;this.#_=0,J(L),q(L)}_handleError(L){if(this.#$)return;this.#$=!0;var x=this.#_;this.#_=0,_(x,L),q(x)}#_;#$=!1;#F;_handleReadMany({value:L,done:x,size:N}){if(x){this.handleClosed();return}if(this.#$)return;F(this.#_,L,x,N)}read(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.read())}_processResult(L){if(L&&@isPromise(L)){if(@getPromiseInternalField(L,@promiseFieldFlags)&@promiseStateFulfilled){const N=@getPromiseInternalField(L,@promiseFieldReactionsOrResult);if(N)L=N}}if(L&&@isPromise(L))return L.then(this.processResult,this.handleError),null;if(L.done)return this.handleClosed(),0;else if(L.value)return L.value;else return-1}readMany(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.readMany())}};const K=A+1;if(G.length<K)G.length=K;@putByValDirect(G,A,H)}if(@isReadableStreamLocked(B))@throwTypeError(\"Cannot start reading from a locked stream\");return new H(B.getReader(),j)})\n";
...
const char* const s_readableStreamPipeToCode = "(function (m){\"use strict\";if(!@isReadableStream(this))return @Promise.@reject(@makeThisTypeError(\"ReadableStream\",\"pipeTo\"));if(@isReadableStreamLocked(this))return @Promise.@reject(@makeTypeError(\"ReadableStream is locked\"));let b=@argument(1),R=!1,S=!1,u=!1,_;if(!@isUndefinedOrNull(b)){if(!@isObject(b))return @Promise.@reject(@makeTypeError(\"options must be an object\"));try{S=!!b[\"preventAbort\"],u=!!b[\"preventCancel\"],R=!!b[\"preventClose\"],_=b[\"signal\"]}catch(E){return @Promise.@reject(E)}if(_!==@undefined&&!@isAbortSignal(_))return @Promise.@reject(@makeTypeError(\"options.signal must be AbortSignal\"))}const j=@getInternalWritableStream(m);if(!@isWritableStream(j))return @Promise.@reject(@makeTypeError(\"ReadableStream pipeTo requires a WritableStream\"));if(@isWritableStreamLocked(j))return @Promise.@reject(@makeTypeError(\"WritableStream is locked\"));return @readableStreamPipeToWritableStream(this,j,R,S,u,_)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerRespondInternalCode = "(function (d,_){\"use strict\";let k=@getByIdDirectPrivate(d,\"pendingPullIntos\").peek(),u=@getByIdDirectPrivate(d,\"controlledReadableStream\");if(@getByIdDirectPrivate(u,\"state\")===@streamClosed){if(_!==0)@throwTypeError(\"bytesWritten is different from 0 even though stream is closed\");@readableByteStreamControllerRespondInClosedState(d,k)}else @assert(@getByIdDirectPrivate(u,\"state\")===@streamReadable),@readableByteStreamControllerRespondInReadableState(d,_,k)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerRespondInReadableStateCode = "(function (R,f,g){\"use strict\";if(g.bytesFilled+f>g.byteLength)@throwRangeError(\"bytesWritten value is too great\");if(@assert(@getByIdDirectPrivate(R,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(R,\"pendingPullIntos\").peek()===g),@readableByteStreamControllerInvalidateBYOBRequest(R),g.bytesFilled+=f,g.bytesFilled<g.elementSize)return;@readableByteStreamControllerShiftPendingDescriptor(R);const h=g.bytesFilled%g.elementSize;if(h>0){const _=g.byteOffset+g.bytesFilled,w=@cloneArrayBuffer(g.buffer,_-h,h);@readableByteStreamControllerEnqueueChunk(R,w,0,w.byteLength)}g.buffer=@transferBufferToCurrentRealm(g.buffer),g.bytesFilled-=h,@readableByteStreamControllerCommitDescriptor(@getByIdDirectPrivate(R,\"controlledReadableStream\"),g),@readableByteStreamControllerProcessPullDescriptors(R)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerFillDescriptorFromQueueCode = "(function (j,w){\"use strict\";const _=w.bytesFilled-w.bytesFilled%w.elementSize,z=@getByIdDirectPrivate(j,\"queue\").size<w.byteLength-w.bytesFilled\?@getByIdDirectPrivate(j,\"queue\").size:w.byteLength-w.bytesFilled,q=w.bytesFilled+z,E=q-q%w.elementSize;let k=z,G=!1;if(E>_)k=E-w.bytesFilled,G=!0;while(k>0){let H=@getByIdDirectPrivate(j,\"queue\").content.peek();const v=k<H.byteLength\?k:H.byteLength,J=w.byteOffset+w.bytesFilled;if(new @Uint8Array(w.buffer).set(new @Uint8Array(H.buffer,H.byteOffset,v),J),H.byteLength===v)@getByIdDirectPrivate(j,\"queue\").content.shift();else H.byteOffset+=v,H.byteLength-=v;@getByIdDirectPrivate(j,\"queue\").size-=v,@assert(@getByIdDirectPrivate(j,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(j,\"pendingPullIntos\").peek()===w),@readableByteStreamControllerInvalidateBYOBRequest(j),w.bytesFilled+=v,k-=v}if(!G)@assert(@getByIdDirectPrivate(j,\"queue\").size===0),@assert(w.bytesFilled>0),@assert(w.bytesFilled<w.elementSize);return G})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerInvalidateBYOBRequestCode = "(function (P){\"use strict\";if(@getByIdDirectPrivate(P,\"byobRequest\")===@undefined)return;const d=@getByIdDirectPrivate(P,\"byobRequest\");@putByIdDirectPrivate(d,\"associatedReadableByteStreamController\",@undefined),@putByIdDirectPrivate(d,\"view\",@undefined),@putByIdDirectPrivate(P,\"byobRequest\",@undefined)})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class A extends EventTarget{#X;#G;#L;#J;#K;#Y=!1;#Z=null;#z=\"\";#W=\"\";#A=\"\";#M=!0;#O=0;#B=0;#Q=0;#U=null;static#V(M){M.#H()}static#F(M,j){const O=M.data,w=O.#A\?`Last-Event-ID: ${O.#A}\\r\\n`:\"\",Q=`GET ${j.pathname}${j.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${w}\\r\\n`,B=M.write(Q);if(B!==Q.length)O.#W=Q.substring(B)}static#j(M,j,O){for(;;){if(O>=j.length)return;let w=-1,Q=j.indexOf(\"\\r\\n\",O);const B=Q+2;if(Q>0)if(M.#O===0){const X=parseInt(j.substring(O,Q),16);if(X===0){M.#G=2,M.#Z\?.end();return}w=B+X}else w=j.length;else{if(M.#z.length===0){M.#z+=j.substring(O);return}w=j.length}let U=j.substring(B,w);O=w+2;let V=0,F=U.indexOf(\"\\n\\n\");if(F==-1){M.#z+=j.substring(B);return}if(M.#z.length)M.#z+=U,U=M.#z,M.#z=\"\";let W=!0;while(W){const X=U.substring(V,F);let Y,Z=\"\",z,H=0,G=-1;for(;;){let K=X.indexOf(\"\\n\",H);if(K===-1){if(H>=X.length)break;K=X.length}const L=X.substring(H,K);if(L.startsWith(\"data:\"))if(Z.length)Z+=`\\n${L.substring(5).trim()}`;else Z=L.substring(5).trim();else if(L.startsWith(\"event:\"))Y=L.substring(6).trim();else if(L.startsWith(\"id:\"))z=L.substring(3).trim();else if(L.startsWith(\"retry:\")){if(G=parseInt(L.substring(6).trim(),10),@isNaN(G))G=-1}H=K+1}if(M.#A=z||\"\",G>=0)M.#Q=G;if(Z||z||Y)M.dispatchEvent(new MessageEvent(Y||\"message\",{data:Z||\"\",origin:M.#X.origin,source:M,lastEventId:z}));if(U.length===F+2){W=!1;break}const J=U.indexOf(\"\\n\\n\",F+1);if(J===-1)break;V=F,F=J}}}static#w={open(M){const j=M.data;if(j.#Z=M,!j.#Y)A.#F(M,j.#X)},handshake(M,j,O){const w=M.data;if(j)A.#F(M,w.#X);else w.#G=2,w.dispatchEvent(new ErrorEvent(\"error\",{error:O})),M.end()},data(M,j){const O=M.data;switch(O.#G){case 0:{let w=j.toString();const Q=w.indexOf(\"\\r\\n\\r\\n\");if(Q===-1){O.#z+=w;return}if(O.#z.length)O.#z+=w,w=O.#z,O.#z=\"\";const B=w.substring(0,Q),U=B.indexOf(\"\\r\\n\");if(U===-1){O.#G=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),M.end();return}const V=B.substring(0,U);if(V!==\"HTTP/1.1 200 OK\"){O.#G=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(V)})),M.end();return}let F=U+1,W=!1,X=-1;for(;;){let Z=B.indexOf(\"\\r\\n\",F);if(Z===-1){if(F>=B.length){if(!W)O.#G=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),M.end();return}Z=B.length}const z=B.substring(F+1,Z),H=z.indexOf(\":\"),G=z.substring(0,H),J=G.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(F=Z+1,J)if(z.endsWith(\" text/event-stream\"))W=!0;else{O.#G=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),M.end();return}else if(G.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(X=parseInt(z.substring(H+1).trim(),10),@isNaN(X)||X<=0){O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),M.end();return}if(W)break}else if(G.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(z.substring(H+1).trim()!==\"chunked\"){O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),M.end();return}if(X=0,W)break}}O.#O=X,O.#G=1,O.dispatchEvent(new Event(\"open\"));const Y=w.substring(Q+4);if(A.#j(O,Y,0),O.#O>0){if(O.#B+=Y.length,O.#B>=O.#O)O.#G=2,M.end()}return}case 1:if(A.#j(O,j.toString(),2),O.#O>0){if(O.#B+=j.byteLength,O.#B>=O.#O)O.#G=2,M.end()}return;default:break}},drain(M){const j=M.data;if(j.#G===0){const O=j.#z;if(O.length){const w=M.write(O);if(w!==O.length)M.data.#W=O.substring(w);else M.data.#W=\"\"}}},close:A.#$,end(M){A.#$(M).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(M){A.#$(M).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#$(M){const j=M.data;if(j.#Z=null,j.#B=0,j.#G=2,j.#M){if(j.#U)clearTimeout(j.#U);j.#U=setTimeout(A.#V,j.#Q,j)}return j}constructor(M,j=@undefined){super();const O=new URL(M);this.#Y=O.protocol===\"https:\",this.#X=O,this.#G=2,process.nextTick(A.#V,this)}ref(){this.#U\?.ref(),this.#Z\?.ref()}unref(){this.#U\?.unref(),this.#Z\?.unref()}#H(){if(this.#G!==2)return;const M=this.#X,j=this.#Y;this.#G=0,@Bun.connect({data:this,socket:A.#w,hostname:M.hostname,port:parseInt(M.port||(j\?\"443\":\"80\"),10),tls:j\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((O)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:O})),this.#M){if(this.#U)this.#U.unref\?.();this.#U=setTimeout(A.#V,1000,this)}})}get url(){return this.#X.href}get readyState(){return this.#G}close(){this.#M=!1,this.#G=2,this.#Z\?.unref(),this.#Z\?.end()}get onopen(){return this.#K}get onerror(){return this.#L}get onmessage(){return this.#J}set onopen(M){if(this.#K)super.removeEventListener(\"close\",this.#K);super.addEventListener(\"open\",M),this.#K=M}set onerror(M){if(this.#L)super.removeEventListener(\"error\",this.#L);super.addEventListener(\"error\",M),this.#L=M}set onmessage(M){if(this.#J)super.removeEventListener(\"message\",this.#J);super.addEventListener(\"message\",M),this.#J=M}}return Object.defineProperty(A.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(A.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(A.prototype,\"CLOSED\",{enumerable:!0,value:2}),A[Symbol.for(\"CommonJS\")]=0,A})\n";
