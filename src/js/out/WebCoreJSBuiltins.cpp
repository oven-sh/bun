...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const O=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!O)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const A=@getByIdDirectPrivate(O,\"state\");if(@putByIdDirectPrivate(O,\"disturbed\",!0),A===@streamClosed)return{value:[],size:0,done:!0};else if(A===@streamErrored)throw @getByIdDirectPrivate(O,\"storedError\");var H=@getByIdDirectPrivate(O,\"readableStreamController\"),C=@getByIdDirectPrivate(H,\"queue\");if(!C)return H.@pull(H).@then(function({done:j,value:J}){return j\?{done:!0,value:[],size:0}:{value:[J],size:1,done:!1}});const D=C.content;var d=C.size,B=D.toArray(!1),Q=B.length;if(Q>0){var I=@newArrayWithSize(Q);if(@isReadableByteStreamController(H)){{const j=B[0];if(!(@ArrayBuffer.@isView(j)||j instanceof @ArrayBuffer))@putByValDirect(I,0,new @Uint8Array(j.buffer,j.byteOffset,j.byteLength));else @putByValDirect(I,0,j)}for(var _=1;_<Q;_++){const j=B[_];if(!(@ArrayBuffer.@isView(j)||j instanceof @ArrayBuffer))@putByValDirect(I,_,new @Uint8Array(j.buffer,j.byteOffset,j.byteLength));else @putByValDirect(I,_,j)}}else{@putByValDirect(I,0,B[0].value);for(var _=1;_<Q;_++)@putByValDirect(I,_,B[_].value)}if(@resetQueue(@getByIdDirectPrivate(H,\"queue\")),@getByIdDirectPrivate(H,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(H,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(H))@readableStreamDefaultControllerCallPullIfNeeded(H);else if(@isReadableByteStreamController(H))@readableByteStreamControllerCallPullIfNeeded(H);return{value:I,size:d,done:!1}}var E=(j)=>{if(j.done)return{value:[],size:0,done:!0};var J=@getByIdDirectPrivate(O,\"readableStreamController\"),K=@getByIdDirectPrivate(J,\"queue\"),N=[j.value].concat(K.content.toArray(!1)),k=N.length;if(@isReadableByteStreamController(J))for(var G=0;G<k;G++){const T=N[G];if(!(@ArrayBuffer.@isView(T)||T instanceof @ArrayBuffer)){const{buffer:U,byteOffset:w,byteLength:x}=T;@putByValDirect(N,G,new @Uint8Array(U,w,x))}}else for(var G=1;G<k;G++)@putByValDirect(N,G,N[G].value);var S=K.size;if(@resetQueue(K),@getByIdDirectPrivate(J,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(J,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(J))@readableStreamDefaultControllerCallPullIfNeeded(J);else if(@isReadableByteStreamController(J))@readableByteStreamControllerCallPullIfNeeded(J);return{value:N,size:S,done:!1}},F=H.@pull(H);if(F&&@isPromise(F))return F.@then(E);return E(F)})\n";
...
const char* const s_streamInternalsCreateFIFOCode = "(function (){\"use strict\";var b=@Array.prototype.slice;class k{constructor(){this._head=0,this._tail=0,this._capacityMask=3,this._list=@newArrayWithSize(4)}_head;_tail;_capacityMask;_list;size(){if(this._head===this._tail)return 0;if(this._head<this._tail)return this._tail-this._head;else return this._capacityMask+1-(this._head-this._tail)}isEmpty(){return this.size()==0}isNotEmpty(){return this.size()>0}shift(){var{_head:E,_tail:z,_list:v,_capacityMask:F}=this;if(E===z)return @undefined;var g=v[E];if(@putByValDirect(v,E,@undefined),E=this._head=E+1&F,E<2&&z>1e4&&z<=v.length>>>2)this._shrinkArray();return g}peek(){if(this._head===this._tail)return @undefined;return this._list[this._head]}push(E){var z=this._tail;if(@putByValDirect(this._list,z,E),this._tail=z+1&this._capacityMask,this._tail===this._head)this._growArray()}toArray(E){var z=this._list,v=@toLength(z.length);if(E||this._head>this._tail){var F=@toLength(this._head),g=@toLength(this._tail),A=@toLength(v-F+g),x=@newArrayWithSize(A),w=0;for(var G=F;G<v;G++)@putByValDirect(x,w++,z[G]);for(var G=0;G<g;G++)@putByValDirect(x,w++,z[G]);return x}else return b.@call(z,this._head,this._tail)}clear(){this._head=0,this._tail=0,this._list.fill(@undefined)}_growArray(){if(this._head)this._list=this.toArray(!0),this._head=0;this._tail=@toLength(this._list.length),this._list.length<<=1,this._capacityMask=this._capacityMask<<1|1}shrinkArray(){this._list.length>>>=1,this._capacityMask>>>=1}}return new k})\n";
