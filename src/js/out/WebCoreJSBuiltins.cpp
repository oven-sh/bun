...
const char* const s_writableStreamInternalsWritableStreamCloseCode = "(function (n){\"use strict\";const u=@getByIdDirectPrivate(n,\"state\");if(u===\"closed\"||u===\"errored\")return @Promise.@reject(@makeTypeError(\"Cannot close a writable stream that is closed or errored\"));@assert(u===\"writable\"||u===\"erroring\"),@assert(!@writableStreamCloseQueuedOrInFlight(n));const d=@newPromiseCapability(@Promise);@putByIdDirectPrivate(n,\"closeRequest\",d);const _=@getByIdDirectPrivate(n,\"writer\");if(_!==@undefined&&@getByIdDirectPrivate(n,\"backpressure\")&&u===\"writable\")@getByIdDirectPrivate(_,\"readyPromise\").@resolve.@call();return @writableStreamDefaultControllerClose(@getByIdDirectPrivate(n,\"controller\")),d.@promise})\n";
...
const char* const s_writableStreamInternalsWritableStreamDefaultControllerGetChunkSizeCode = "(function (i,d){\"use strict\";try{return @getByIdDirectPrivate(i,\"strategySizeAlgorithm\").@call(@undefined,d)}catch(a){return @writableStreamDefaultControllerErrorIfNeeded(i,a),1}})\n";
...
const char* const s_moduleRequireCode = "(function (_){\"use strict\";const S=@requireMap.@get(_)||@requireMap.@get(_=@resolveSync(_,this.path,!1));if(S)return @evaluateCommonJSModule(S),S.exports;if(_.endsWith(\".json\")||_.endsWith(\".toml\")||_.endsWith(\".node\"))return @internalRequire(_);let f=@Loader.registry.@get(_);if(f\?.evaluated&&(f.state\?\?0)>=@ModuleReady){const r=f.module,M=@Loader.getModuleNamespaceObject(r),b=M\?.[@commonJSSymbol]===0||M\?.default\?.[@commonJSSymbol]===0\?M.default:M.__esModule\?M:Object.create(M,{__esModule:{value:!0}});return @requireMap.@set(_,@createCommonJSModule(_,b,!0)),b}const L=@createCommonJSModule(_,{},!1);@requireMap.@set(_,L);var h=this.@require(_,L);if(h===-1){try{h=@requireESM(_)}catch(r){throw @requireMap.@delete(_),r}if(f=@Loader.registry.@get(_),f\?.evaluated&&(f.state\?\?0)>=@ModuleReady){const r=@Loader.getModuleNamespaceObject(f.module);return L.exports=r\?.[@commonJSSymbol]===0||r\?.default\?.[@commonJSSymbol]===0\?r.default:r.__esModule\?r:Object.create(r,{__esModule:{value:!0}})}}return @evaluateCommonJSModule(L),L.exports})\n";
...
const char* const s_jsBufferPrototypeReadIntBECode = "(function (r,u){\"use strict\";const c=this.@dataView||=new DataView(this.buffer,this.byteOffset,this.byteLength);switch(u){case 1:return c.getInt8(r);case 2:return c.getInt16(r,!1);case 3:{const d=c.getUint16(r+1,!1)+c.getUint8(r)*65536;return d|(d&8388608)*510}case 4:return c.getInt32(r,!1);case 5:{const d=c.getUint8(r);return(d|(d&128)*33554430)*4294967296+c.getUint32(r+1,!1)}case 6:{const d=c.getUint16(r,!1);return(d|(d&32768)*131070)*4294967296+c.getUint32(r+2,!1)}}@throwRangeError(\"byteLength must be >= 1 and <= 6\")})\n";
...
const char* const s_jsBufferPrototypeSliceCode = "(function (c,m){\"use strict\";var{buffer:p,byteOffset:k,byteLength:v}=this;function q(z,i){if(z=@trunc(z),z===0||@isNaN(z))return 0;else if(z<0)return z+=i,z>0\?z:0;else return z<i\?z:i}var w=q(c,v),x=m!==@undefined\?q(m,v):v;return new @Buffer(p,k+w,x>w\?x-w:0)})\n";
...
const char* const s_readableStreamInternalsReadableStreamTeeCode = "(function (_,g){\"use strict\";@assert(@isReadableStream(_)),@assert(typeof g===\"boolean\");var j=@getByIdDirectPrivate(_,\"start\");if(j)@putByIdDirectPrivate(_,\"start\",@undefined),j();const k=new @ReadableStreamDefaultReader(_),v={closedOrErrored:!1,canceled1:!1,canceled2:!1,reason1:@undefined,reason2:@undefined};v.cancelPromiseCapability=@newPromiseCapability(@Promise);const w=@readableStreamTeePullFunction(v,k,g),i={};@putByIdDirectPrivate(i,\"pull\",w),@putByIdDirectPrivate(i,\"cancel\",@readableStreamTeeBranch1CancelFunction(v,_));const x={};@putByIdDirectPrivate(x,\"pull\",w),@putByIdDirectPrivate(x,\"cancel\",@readableStreamTeeBranch2CancelFunction(v,_));const y=new @ReadableStream(i),f=new @ReadableStream(x);return @getByIdDirectPrivate(k,\"closedPromiseCapability\").@promise.@then(@undefined,function(q){if(v.closedOrErrored)return;if(@readableStreamDefaultControllerError(y.@readableStreamController,q),@readableStreamDefaultControllerError(f.@readableStreamController,q),v.closedOrErrored=!0,!v.canceled1||!v.canceled2)v.cancelPromiseCapability.@resolve.@call()}),v.branch1=y,v.branch2=f,[y,f]})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (_){\"use strict\";var w=@Loader,D=@createFIFO(),F=_;while(F){var L=w.registry.@get(F);if((L\?.state\?\?0)<=@ModuleFetch)@fulfillModuleSync(F),L=w.registry.@get(F);var Q=@getPromiseInternalField(L.fetch,@promiseFieldReactionsOrResult),T=w.parseModule(F,Q),G=L.module;if(T&&@isPromise(T)){var U=@getPromiseInternalField(T,@promiseFieldReactionsOrResult),V=@getPromiseInternalField(T,@promiseFieldFlags),J=V&@promiseStateMask;if(J===@promiseStatePending||U&&@isPromise(U))@throwTypeError(`require() async module \"${F}\" is unsupported. use \"await import()\" instead.`);else if(J===@promiseStateRejected){if(!U\?.message)@throwTypeError(`${U+\"\"\?U:\"An error occurred\"} occurred while parsing module \\\"${F}\\\"`);throw U}L.module=G=U}else if(T&&!G)L.module=G=T;@setStateToMax(L,@ModuleLink);var W=G.dependenciesMap,X=w.requestedModules(G),H=@newArrayWithSize(X.length);for(var I=0,z=X.length;I<z;++I){var x=X[I],B=x[0]===\"/\"\?x:w.resolve(x,F),Y=w.ensureRegistered(B);if(Y.state<@ModuleLink)D.push(B);@putByValDirect(H,I,Y),W.@set(x,Y)}L.dependencies=H,L.instantiate=@Promise.resolve(L),L.satisfy=@Promise.resolve(L),F=D.shift();while(F&&(w.registry.@get(F)\?.state\?\?@ModuleFetch)>=@ModuleLink)F=D.shift()}var Z=w.linkAndEvaluateModule(_,@undefined);if(Z&&@isPromise(Z))@throwTypeError(`require() async module \\\"${_}\\\" is unsupported. use \"await import()\" instead.`);return w.registry.@get(_)})\n";
...
const char* const s_readableStreamConsumeReadableStreamCode = "(function (_,q,w){\"use strict\";const k=globalThis.Symbol.for(\"Bun.consumeReadableStreamPrototype\");var B=globalThis[k];if(!B)B=globalThis[k]=[];var x=B[q];if(x===@undefined){var[A,D,G,H,I,J]=globalThis[globalThis.Symbol.for(\"Bun.lazy\")](q);x=class K{handleError;handleClosed;processResult;constructor(L,N){this.#_=N,this.#F=L,this.#$=!1,this.handleError=this._handleError.bind(this),this.handleClosed=this._handleClosed.bind(this),this.processResult=this._processResult.bind(this),L.closed.then(this.handleClosed,this.handleError)}_handleClosed(){if(this.#$)return;this.#$=!0;var L=this.#_;this.#_=0,H(L),J(L)}_handleError(L){if(this.#$)return;this.#$=!0;var N=this.#_;this.#_=0,D(N,L),J(N)}#_;#$=!1;#F;_handleReadMany({value:L,done:N,size:j}){if(N){this.handleClosed();return}if(this.#$)return;G(this.#_,L,N,j)}read(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.read())}_processResult(L){if(L&&@isPromise(L)){if(@getPromiseInternalField(L,@promiseFieldFlags)&@promiseStateFulfilled){const j=@getPromiseInternalField(L,@promiseFieldReactionsOrResult);if(j)L=j}}if(L&&@isPromise(L))return L.then(this.processResult,this.handleError),null;if(L.done)return this.handleClosed(),0;else if(L.value)return L.value;else return-1}readMany(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.readMany())}};const F=q+1;if(B.length<F)B.length=F;@putByValDirect(B,q,x)}if(@isReadableStreamLocked(w))@throwTypeError(\"Cannot start reading from a locked stream\");return new x(w.getReader(),_)})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class j extends EventTarget{#$;#j;#w;#A;#B;#F=!1;#G=null;#J=\"\";#K=\"\";#L=\"\";#M=!0;#O=0;#Q=0;#U=0;#V=null;static#W(Z){Z.#H()}static#X(Z,z){const H=Z.data,w=H.#L\?`Last-Event-ID: ${H.#L}\\r\\n`:\"\",Y=`GET ${z.pathname}${z.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${w}\\r\\n`,A=Z.write(Y);if(A!==Y.length)H.#K=Y.substring(A)}static#Y(Z,z,H){for(;;){if(H>=z.length)return;let w=-1,Y=z.indexOf(\"\\r\\n\",H);const A=Y+2;if(Y>0)if(Z.#O===0){const K=parseInt(z.substring(H,Y),16);if(K===0){Z.#j=2,Z.#G\?.end();return}w=A+K}else w=z.length;else{if(Z.#J.length===0){Z.#J+=z.substring(H);return}w=z.length}let B=z.substring(A,w);H=w+2;let F=0,G=B.indexOf(\"\\n\\n\");if(G==-1){Z.#J+=z.substring(A);return}if(Z.#J.length)Z.#J+=B,B=Z.#J,Z.#J=\"\";let J=!0;while(J){const K=B.substring(F,G);let L,M=\"\",O,Q=0,U=-1;for(;;){let W=K.indexOf(\"\\n\",Q);if(W===-1){if(Q>=K.length)break;W=K.length}const X=K.substring(Q,W);if(X.startsWith(\"data:\"))if(M.length)M+=`\\n${X.substring(5).trim()}`;else M=X.substring(5).trim();else if(X.startsWith(\"event:\"))L=X.substring(6).trim();else if(X.startsWith(\"id:\"))O=X.substring(3).trim();else if(X.startsWith(\"retry:\")){if(U=parseInt(X.substring(6).trim(),10),@isNaN(U))U=-1}Q=W+1}if(Z.#L=O||\"\",U>=0)Z.#U=U;if(M||O||L)Z.dispatchEvent(new MessageEvent(L||\"message\",{data:M||\"\",origin:Z.#$.origin,source:Z,lastEventId:O}));if(B.length===G+2){J=!1;break}const V=B.indexOf(\"\\n\\n\",G+1);if(V===-1)break;F=G,G=V}}}static#Z={open(Z){const z=Z.data;if(z.#G=Z,!z.#F)j.#X(Z,z.#$)},handshake(Z,z,H){const w=Z.data;if(z)j.#X(Z,w.#$);else w.#j=2,w.dispatchEvent(new ErrorEvent(\"error\",{error:H})),Z.end()},data(Z,z){const H=Z.data;switch(H.#j){case 0:{let w=z.toString();const Y=w.indexOf(\"\\r\\n\\r\\n\");if(Y===-1){H.#J+=w;return}if(H.#J.length)H.#J+=w,w=H.#J,H.#J=\"\";const A=w.substring(0,Y),B=A.indexOf(\"\\r\\n\");if(B===-1){H.#j=2,H.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),Z.end();return}const F=A.substring(0,B);if(F!==\"HTTP/1.1 200 OK\"){H.#j=2,H.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(F)})),Z.end();return}let G=B+1,J=!1,K=-1;for(;;){let M=A.indexOf(\"\\r\\n\",G);if(M===-1){if(G>=A.length){if(!J)H.#j=2,H.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),Z.end();return}M=A.length}const O=A.substring(G+1,M),Q=O.indexOf(\":\"),U=O.substring(0,Q),V=U.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(G=M+1,V)if(O.endsWith(\" text/event-stream\"))J=!0;else{H.#j=2,H.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),Z.end();return}else if(U.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(K=parseInt(O.substring(Q+1).trim(),10),@isNaN(K)||K<=0){H.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),Z.end();return}if(J)break}else if(U.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(O.substring(Q+1).trim()!==\"chunked\"){H.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),Z.end();return}if(K=0,J)break}}H.#O=K,H.#j=1,H.dispatchEvent(new Event(\"open\"));const L=w.substring(Y+4);if(j.#Y(H,L,0),H.#O>0){if(H.#Q+=L.length,H.#Q>=H.#O)H.#j=2,Z.end()}return}case 1:if(j.#Y(H,z.toString(),2),H.#O>0){if(H.#Q+=z.byteLength,H.#Q>=H.#O)H.#j=2,Z.end()}return;default:break}},drain(Z){const z=Z.data;if(z.#j===0){const H=z.#J;if(H.length){const w=Z.write(H);if(w!==H.length)Z.data.#K=H.substring(w);else Z.data.#K=\"\"}}},close:j.#z,end(Z){j.#z(Z).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(Z){j.#z(Z).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#z(Z){const z=Z.data;if(z.#G=null,z.#Q=0,z.#j=2,z.#M){if(z.#V)clearTimeout(z.#V);z.#V=setTimeout(j.#W,z.#U,z)}return z}constructor(Z,z=@undefined){super();const H=new URL(Z);this.#F=H.protocol===\"https:\",this.#$=H,this.#j=2,process.nextTick(j.#W,this)}ref(){this.#V\?.ref(),this.#G\?.ref()}unref(){this.#V\?.unref(),this.#G\?.unref()}#H(){if(this.#j!==2)return;const Z=this.#$,z=this.#F;this.#j=0,@Bun.connect({data:this,socket:j.#Z,hostname:Z.hostname,port:parseInt(Z.port||(z\?\"443\":\"80\"),10),tls:z\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((H)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:H})),this.#M){if(this.#V)this.#V.unref\?.();this.#V=setTimeout(j.#W,1000,this)}})}get url(){return this.#$.href}get readyState(){return this.#j}close(){this.#M=!1,this.#j=2,this.#G\?.unref(),this.#G\?.end()}get onopen(){return this.#B}get onerror(){return this.#w}get onmessage(){return this.#A}set onopen(Z){if(this.#B)super.removeEventListener(\"close\",this.#B);super.addEventListener(\"open\",Z),this.#B=Z}set onerror(Z){if(this.#w)super.removeEventListener(\"error\",this.#w);super.addEventListener(\"error\",Z),this.#w=Z}set onmessage(Z){if(this.#A)super.removeEventListener(\"message\",this.#A);super.addEventListener(\"message\",Z),this.#A=Z}}return Object.defineProperty(j.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(j.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(j.prototype,\"CLOSED\",{enumerable:!0,value:2}),j[Symbol.for(\"CommonJS\")]=0,j})\n";
