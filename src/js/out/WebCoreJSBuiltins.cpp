...
const char* const s_writableStreamInternalsWritableStreamFinishInFlightCloseWithErrorCode = "(function (_,d){\"use strict\";const n=@getByIdDirectPrivate(_,\"inFlightCloseRequest\");@assert(n!==@undefined),n.@reject.@call(@undefined,d),@putByIdDirectPrivate(_,\"inFlightCloseRequest\",@undefined);const c=@getByIdDirectPrivate(_,\"state\");@assert(c===\"writable\"||c===\"erroring\");const p=@getByIdDirectPrivate(_,\"pendingAbortRequest\");if(p!==@undefined)p.promise.@reject.@call(@undefined,d),@putByIdDirectPrivate(_,\"pendingAbortRequest\",@undefined);@writableStreamDealWithRejection(_,d)})\n";
...
const char* const s_transformStreamInternalsSetUpTransformStreamDefaultControllerFromTransformerCode = "(function (_,b,j){\"use strict\";const p=new @TransformStreamDefaultController;let q=(d)=>{try{@transformStreamDefaultControllerEnqueue(p,d)}catch(v){return @Promise.@reject(v)}return @Promise.@resolve()},w=()=>{return @Promise.@resolve()};if(\"transform\"in j)q=(d)=>{return @promiseInvokeOrNoopMethod(b,j[\"transform\"],[d,p])};if(\"flush\"in j)w=()=>{return @promiseInvokeOrNoopMethod(b,j[\"flush\"],[p])};@setUpTransformStreamDefaultController(_,p,q,w)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultSinkCloseAlgorithmCode = "(function (q){\"use strict\";const I=@getByIdDirectPrivate(q,\"readable\"),f=@getByIdDirectPrivate(q,\"controller\"),j=@getByIdDirectPrivate(I,\"readableStreamController\"),u=@getByIdDirectPrivate(f,\"flushAlgorithm\");@assert(u!==@undefined);const _=@getByIdDirectPrivate(f,\"flushAlgorithm\").@call();@transformStreamDefaultControllerClearAlgorithms(f);const v=@newPromiseCapability(@Promise);return _.@then(()=>{if(@getByIdDirectPrivate(I,\"state\")===@streamErrored){v.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"));return}if(@readableStreamDefaultControllerCanCloseOrEnqueue(j))@readableStreamDefaultControllerClose(j);v.@resolve()},(k)=>{@transformStreamError(@getByIdDirectPrivate(f,\"stream\"),k),v.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"))}),v.@promise})\n";
...
const char* const s_transformStreamInitializeTransformStreamCode = "(function (){\"use strict\";let u=arguments[0];if(@isObject(u)&&@getByIdDirectPrivate(u,\"TransformStream\"))return this;let E=arguments[1],j=arguments[2];if(u===@undefined)u=null;if(j===@undefined)j={};if(E===@undefined)E={};let F={};if(u!==null){if(\"start\"in u){if(F[\"start\"]=u[\"start\"],typeof F[\"start\"]!==\"function\")@throwTypeError(\"transformer.start should be a function\")}if(\"transform\"in u){if(F[\"transform\"]=u[\"transform\"],typeof F[\"transform\"]!==\"function\")@throwTypeError(\"transformer.transform should be a function\")}if(\"flush\"in u){if(F[\"flush\"]=u[\"flush\"],typeof F[\"flush\"]!==\"function\")@throwTypeError(\"transformer.flush should be a function\")}if(\"readableType\"in u)@throwRangeError(\"TransformStream transformer has a readableType\");if(\"writableType\"in u)@throwRangeError(\"TransformStream transformer has a writableType\")}const q=@extractHighWaterMark(j,0),G=@extractSizeAlgorithm(j),v=@extractHighWaterMark(E,1),I=@extractSizeAlgorithm(E),x=@newPromiseCapability(@Promise);if(@initializeTransformStream(this,x.@promise,v,I,q,G),@setUpTransformStreamDefaultControllerFromTransformer(this,u,F),(\"start\"in F)){const J=@getByIdDirectPrivate(this,\"controller\");(()=>@promiseInvokeOrNoopMethodNoCatch(u,F[\"start\"],[J]))().@then(()=>{x.@resolve.@call()},(_)=>{x.@reject.@call(@undefined,_)})}else x.@resolve.@call();return this})\n";
...
const char* const s_moduleRequireCode = "(function (h){\"use strict\";const M=@requireMap.@get(h)||@requireMap.@get(h=@resolveSync(h,this.path,!1));if(M)return M.exports;if(h.endsWith(\".json\")||h.endsWith(\".toml\")||h.endsWith(\".node\"))return @internalRequire(h);let l=@Loader.registry.@get(h);if(l\?.evaluated){const _=l.module,y=@Loader.getModuleNamespaceObject(_),q=y\?.[@commonJSSymbol]===0||y\?.default\?.[@commonJSSymbol]===0\?y.default:y;return @requireMap.@set(h,@createCommonJSModule(h,q)),q}let f=this.@require(h);if(f===-1){const _=@createCommonJSModule(h,{});@requireMap.@set(h,_);try{f=@requireESM(h)}catch(y){throw @requireMap.@delete(h),y}if(l=@Loader.registry.@get(h),l\?.evaluated){const y=@Loader.getModuleNamespaceObject(l.module);return _.exports=y\?.[@commonJSSymbol]===0||y\?.default\?.[@commonJSSymbol]===0\?y.default:y}}const r=@requireMap.@get(h);if(r)return r.exports;return f})\n";
...
const char* const s_jsBufferPrototypeWriteUIntBECode = "(function (d,p,_){\"use strict\";const r=this.@dataView||=new DataView(this.buffer,this.byteOffset,this.byteLength);switch(_){case 1:{r.setUint8(p,d);break}case 2:{r.setUint16(p,d,!1);break}case 3:{r.setUint16(p+1,d&65535,!1),r.setUint8(p,Math.floor(d*0.0000152587890625));break}case 4:{r.setUint32(p,d,!1);break}case 5:{r.setUint32(p+1,d|0,!1),r.setUint8(p,Math.floor(d*0.00000000023283064365386964));break}case 6:{r.setUint32(p+2,d|0,!1),r.setUint16(p,Math.floor(d*0.00000000023283064365386964),!1);break}default:@throwRangeError(\"byteLength must be >= 1 and <= 6\")}return p+_})\n";
...
const char* const s_consoleObjectWriteCode = "(function (a){\"use strict\";var _=@getByIdDirectPrivate(this,\"writer\");if(!_){var d=@toLength(a\?.length\?\?0);_=@Bun.stdout.writer({highWaterMark:d>65536\?d:65536}),@putByIdDirectPrivate(this,\"writer\",_)}var b=_.write(a);const c=@argumentCount();for(var f=1;f<c;f++)b+=_.write(@argument(f));return _.flush(!0),b})\n";
...
const char* const s_readableStreamInternalsReadStreamIntoSinkCode = "(async function (f,A,I){\"use strict\";var B=!1,D=!1;try{var E=f.getReader(),F=E.readMany();if(F&&@isPromise(F))F=await F;if(F.done)return B=!0,A.end();var G=F.value.length;const K=@getByIdDirectPrivate(f,\"highWaterMark\");if(I)@startDirectStream.@call(A,f,@undefined,()=>!D&&@markPromiseAsHandled(f.cancel()));A.start({highWaterMark:K||0});for(var H=0,J=F.value,_=F.value.length;H<_;H++)A.write(J[H]);var c=@getByIdDirectPrivate(f,\"state\");if(c===@streamClosed)return B=!0,A.end();while(!0){var{value:P,done:x}=await E.read();if(x)return B=!0,A.end();A.write(P)}}catch(K){D=!0;try{E=@undefined;const q=f.cancel(K);@markPromiseAsHandled(q)}catch(q){}if(A&&!B){B=!0;try{A.close(K)}catch(q){throw new globalThis.AggregateError([K,q])}}throw K}finally{if(E){try{E.releaseLock()}catch(q){}E=@undefined}A=@undefined;var c=@getByIdDirectPrivate(f,\"state\");if(f){var z=@getByIdDirectPrivate(f,\"readableStreamController\");if(z){if(@getByIdDirectPrivate(z,\"underlyingSource\"))@putByIdDirectPrivate(z,\"underlyingSource\",@undefined);if(@getByIdDirectPrivate(z,\"controlledReadableStream\"))@putByIdDirectPrivate(z,\"controlledReadableStream\",@undefined);if(@putByIdDirectPrivate(f,\"readableStreamController\",null),@getByIdDirectPrivate(f,\"underlyingSource\"))@putByIdDirectPrivate(f,\"underlyingSource\",@undefined);z=@undefined}if(!D&&c!==@streamClosed&&c!==@streamErrored)@readableStreamClose(f);f=@undefined}}})\n";
...
const char* const s_readableStreamInternalsOnPullDirectStreamCode = "(function (h){\"use strict\";var _=h.@controlledReadableStream;if(!_||@getByIdDirectPrivate(_,\"state\")!==@streamReadable)return;if(h._deferClose===-1)return;h._deferClose=-1,h._deferFlush=-1;var b,g;try{var i=h.@underlyingSource.pull(h);if(i&&@isPromise(i)){if(h._handleError===@undefined)h._handleError=@handleDirectStreamErrorReject.bind(h);@Promise.prototype.catch.@call(i,h._handleError)}}catch(k){return @handleDirectStreamErrorReject.@call(h,k)}finally{b=h._deferClose,g=h._deferFlush,h._deferFlush=h._deferClose=0}var j;if(h._pendingRead===@undefined)h._pendingRead=j=@newPromise();else j=@readableStreamAddReadRequest(_);if(b===1){var y=h._deferCloseReason;return h._deferCloseReason=@undefined,@onCloseDirectStream.@call(h,y),j}if(g===1)@onFlushDirectStream.@call(h);return j})\n";
...
const char* const s_readableStreamInternalsCreateTextStreamCode = "(function (j){\"use strict\";var v,_=[],x=!1,q=!1,z=\"\",w=@toLength(0),A=@newPromiseCapability(@Promise),C=!1;return v={start(){},write(E){if(typeof E===\"string\"){var F=@toLength(E.length);if(F>0)z+=E,x=!0,w+=F;return F}if(!E||!(@ArrayBuffer.@isView(E)||E instanceof @ArrayBuffer))@throwTypeError(\"Expected text, ArrayBuffer or ArrayBufferView\");const G=@toLength(E.byteLength);if(G>0)if(q=!0,z.length>0)@arrayPush(_,z,E),z=\"\";else @arrayPush(_,E);return w+=G,G},flush(){return 0},end(){if(C)return\"\";return v.fulfill()},fulfill(){C=!0;const E=v.finishInternal();return @fulfillPromise(A.@promise,E),E},finishInternal(){if(!x&&!q)return\"\";if(x&&!q)return z;if(q&&!x)return new globalThis.TextDecoder().decode(@Bun.concatArrayBuffers(_));var E=new @Bun.ArrayBufferSink;E.start({highWaterMark:w,asUint8Array:!0});for(let F of _)E.write(F);if(_.length=0,z.length>0)E.write(z),z=\"\";return new globalThis.TextDecoder().decode(E.end())},close(){try{if(!C)C=!0,v.fulfill()}catch(E){}}},[v,A]})\n";
...
const char* const s_readableStreamInternalsReadableStreamCancelCode = "(function (i,u){\"use strict\";@putByIdDirectPrivate(i,\"disturbed\",!0);const d=@getByIdDirectPrivate(i,\"state\");if(d===@streamClosed)return @Promise.@resolve();if(d===@streamErrored)return @Promise.@reject(@getByIdDirectPrivate(i,\"storedError\"));@readableStreamClose(i);var h=@getByIdDirectPrivate(i,\"readableStreamController\"),_=h.@cancel;if(_)return _(h,u).@then(function(){});var p=h.close;if(p)return @Promise.@resolve(h.close(u));@throwTypeError(\"ReadableStreamController has no cancel or close method\")})\n";
...
const char* const s_readableStreamInternalsReadableStreamDefineLazyIteratorsCode = "(function (k){\"use strict\";var z=globalThis.Symbol.asyncIterator,w=async function*B(D,F){var G=D.getReader(),i;try{while(!0){var q,h;const j=G.readMany();if(@isPromise(j))({done:q,value:h}=await j);else({done:q,value:h}=j);if(q)return;yield*h}}catch(j){i=j}finally{if(G.releaseLock(),!F)D.cancel(i);if(i)throw i}},g=function B(){return w(this,!1)},x=function B({preventCancel:D=!1}={preventCancel:!1}){return w(this,D)};return @Object.@defineProperty(k,z,{value:g}),@Object.@defineProperty(k,\"values\",{value:x}),k})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (x){\"use strict\";var J=@Loader,L=@createFIFO(),Q=x;while(Q){var T=J.registry.@get(Q);if(!T||!T.state||T.state<=@ModuleFetch)@fulfillModuleSync(Q),T=J.registry.@get(Q);var U=@getPromiseInternalField(T.fetch,@promiseFieldReactionsOrResult),h=J.parseModule(Q,U),V=T.module;if(h&&@isPromise(h)){var W=@getPromiseInternalField(h,@promiseFieldReactionsOrResult),z=@getPromiseInternalField(h,@promiseFieldFlags),B=z&@promiseStateMask;if(B===@promiseStatePending||W&&@isPromise(W))@throwTypeError(`require() async module \"${Q}\" is unsupported. use \"await import()\" instead.`);else if(B===@promiseStateRejected){if(!W\?.message)@throwTypeError(`${W+\"\"\?W:\"An error occurred\"} occurred while parsing module \\\"${Q}\\\"`);throw W}T.module=V=W}else if(h&&!V)T.module=V=h;@setStateToMax(T,@ModuleLink);var _=V.dependenciesMap,G=J.requestedModules(V),H=@newArrayWithSize(G.length);for(var I=0,X=G.length;I<X;++I){var w=G[I],Y=w[0]===\"/\"\?w:J.resolve(w,Q),D=J.ensureRegistered(Y);if(D.state<@ModuleLink)L.push(Y);@putByValDirect(H,I,D),_.@set(w,D)}T.dependencies=H,T.instantiate=@Promise.resolve(T),T.satisfy=@Promise.resolve(T),Q=L.shift();while(Q&&(J.registry.@get(Q)\?.state\?\?@ModuleFetch)>=@ModuleLink)Q=L.shift()}var F=J.linkAndEvaluateModule(x,@undefined);if(F&&@isPromise(F))@throwTypeError(`require() async module \\\"${x}\\\" is unsupported. use \"await import()\" instead.`);return J.registry.@get(x)})\n";
...
const int s_importMetaObjectInternalRequireCodeLength = 774;
static const JSC::Intrinsic s_importMetaObjectInternalRequireCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_importMetaObjectInternalRequireCode = "(function (_){\"use strict\";var F=@requireMap.@get(_);const M=_.substring(_.length-5);if(F)return F.exports;if(M===\".json\"){var b=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),j=JSON.parse(b.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,j)),j}else if(M===\".node\"){const k=@createCommonJSModule(_,{});return process.dlopen(k,_),@requireMap.@set(_,k),k.exports}else if(M===\".toml\"){var b=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),j=@Bun.TOML.parse(b.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,j)),j}else{var j=@requireESM(_);const g=@requireMap.@get(_);if(g)return g.exports;var n=@createCommonJSModule(_,j);if(@requireMap.@set(_,n),n.exports\?.default\?.[@commonJSSymbol]===0)return n.exports=n.exports.default;return j}})\n";
...
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";var _=new Map,t={};return new Proxy(t,{get(L,b){const c=@requireMap.@get(b);if(c)return c;const f=@Loader.registry.@get(b);if(f&&f.evaluated){const h=@Loader.getModuleNamespaceObject(f.module),u=h[@commonJSSymbol]===0||h.default\?.[@commonJSSymbol]\?h.default:h,g=@createCommonJSModule(b,u);return @requireMap.@set(b,g),g}return t[b]},set(L,b,c){return @requireMap.@set(b,c),!0},has(L,b){return @requireMap.@has(b)||@Loader.registry.@has(b)},deleteProperty(L,b){return _.@delete(b),@requireMap.@delete(b),@Loader.registry.@delete(b),!0},ownKeys(L){var b=[...@requireMap.@keys()];const c=[...@Loader.registry.@keys()];for(let f of c)if(!b.includes(f))@arrayPush(b,f);return b},getPrototypeOf(L){return null},getOwnPropertyDescriptor(L,b){if(@requireMap.@has(b)||@Loader.registry.@has(b))return{configurable:!0,enumerable:!0}}})})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerCommitDescriptorCode = "(function (_,g){\"use strict\";@assert(@getByIdDirectPrivate(_,\"state\")!==@streamErrored);let h=!1;if(@getByIdDirectPrivate(_,\"state\")===@streamClosed)@assert(!g.bytesFilled),h=!0;let b=@readableByteStreamControllerConvertDescriptor(g);if(g.readerType===\"default\")@readableStreamFulfillReadRequest(_,b,h);else @assert(g.readerType===\"byob\"),@readableStreamFulfillReadIntoRequest(_,b,h)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullIntoCode = "(function (_,E){\"use strict\";const b=@getByIdDirectPrivate(_,\"controlledReadableStream\");let L=1;if(E.BYTES_PER_ELEMENT!==@undefined)L=E.BYTES_PER_ELEMENT;const d=E.constructor,N={buffer:E.buffer,byteOffset:E.byteOffset,byteLength:E.byteLength,bytesFilled:0,elementSize:L,ctor:d,readerType:\"byob\"};var k=@getByIdDirectPrivate(_,\"pendingPullIntos\");if(k\?.isNotEmpty())return N.buffer=@transferBufferToCurrentRealm(N.buffer),k.push(N),@readableStreamAddReadIntoRequest(b);if(@getByIdDirectPrivate(b,\"state\")===@streamClosed){const A=new d(N.buffer,N.byteOffset,0);return @createFulfilledPromise({value:A,done:!0})}if(@getByIdDirectPrivate(_,\"queue\").size>0){if(@readableByteStreamControllerFillDescriptorFromQueue(_,N)){const A=@readableByteStreamControllerConvertDescriptor(N);return @readableByteStreamControllerHandleQueueDrain(_),@createFulfilledPromise({value:A,done:!1})}if(@getByIdDirectPrivate(_,\"closeRequested\")){const A=@makeTypeError(\"Closing stream has been requested\");return @readableByteStreamControllerError(_,A),@Promise.@reject(A)}}N.buffer=@transferBufferToCurrentRealm(N.buffer),@getByIdDirectPrivate(_,\"pendingPullIntos\").push(N);const R=@readableStreamAddReadIntoRequest(b);return @readableByteStreamControllerCallPullIfNeeded(_),R})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class H extends EventTarget{#w;#O;#Q;#U;#V;#A=!1;#W=null;#B=\"\";#J=\"\";#M=\"\";#X=!0;#Y=0;#Z=0;#K=0;#z=null;static#$(O){O.#G()}static#F(O,A){const K=O.data,Z=K.#M\?`Last-Event-ID: ${K.#M}\\r\\n`:\"\",z=`GET ${A.pathname}${A.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${Z}\\r\\n`,G=O.write(z);if(G!==z.length)K.#J=z.substring(G)}static#L(O,A,K){for(;;){if(K>=A.length)return;let Z=-1,z=A.indexOf(\"\\r\\n\",K);const G=z+2;if(z>0)if(O.#Y===0){const U=parseInt(A.substring(K,z),16);if(U===0){O.#O=2,O.#W\?.end();return}Z=G+U}else Z=A.length;else{if(O.#B.length===0){O.#B+=A.substring(K);return}Z=A.length}let Q=A.substring(G,Z);K=Z+2;let X=0,B=Q.indexOf(\"\\n\\n\");if(B==-1){O.#B+=A.substring(G);return}if(O.#B.length)O.#B+=Q,Q=O.#B,O.#B=\"\";let V=!0;while(V){const U=Q.substring(X,B);let W,Y=\"\",F,j=0,L=-1;for(;;){let J=U.indexOf(\"\\n\",j);if(J===-1){if(j>=U.length)break;J=U.length}const M=U.substring(j,J);if(M.startsWith(\"data:\"))if(Y.length)Y+=`\\n${M.substring(5).trim()}`;else Y=M.substring(5).trim();else if(M.startsWith(\"event:\"))W=M.substring(6).trim();else if(M.startsWith(\"id:\"))F=M.substring(3).trim();else if(M.startsWith(\"retry:\")){if(L=parseInt(M.substring(6).trim(),10),@isNaN(L))L=-1}j=J+1}if(O.#M=F||\"\",L>=0)O.#K=L;if(Y||F||W)O.dispatchEvent(new MessageEvent(W||\"message\",{data:Y||\"\",origin:O.#w.origin,source:O,lastEventId:F}));if(Q.length===B+2){V=!1;break}const w=Q.indexOf(\"\\n\\n\",B+1);if(w===-1)break;X=B,B=w}}}static#H={open(O){const A=O.data;if(A.#W=O,!A.#A)H.#F(O,A.#w)},handshake(O,A,K){const Z=O.data;if(A)H.#F(O,Z.#w);else Z.#O=2,Z.dispatchEvent(new ErrorEvent(\"error\",{error:K})),O.end()},data(O,A){const K=O.data;switch(K.#O){case 0:{let Z=A.toString();const z=Z.indexOf(\"\\r\\n\\r\\n\");if(z===-1){K.#B+=Z;return}if(K.#B.length)K.#B+=Z,Z=K.#B,K.#B=\"\";const G=Z.substring(0,z),Q=G.indexOf(\"\\r\\n\");if(Q===-1){K.#O=2,K.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),O.end();return}const X=G.substring(0,Q);if(X!==\"HTTP/1.1 200 OK\"){K.#O=2,K.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(X)})),O.end();return}let B=Q+1,V=!1,U=-1;for(;;){let Y=G.indexOf(\"\\r\\n\",B);if(Y===-1){if(B>=G.length){if(!V)K.#O=2,K.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),O.end();return}Y=G.length}const F=G.substring(B+1,Y),j=F.indexOf(\":\"),L=F.substring(0,j),w=L.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(B=Y+1,w)if(F.endsWith(\" text/event-stream\"))V=!0;else{K.#O=2,K.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),O.end();return}else if(L.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(U=parseInt(F.substring(j+1).trim(),10),@isNaN(U)||U<=0){K.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),O.end();return}if(V)break}else if(L.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(F.substring(j+1).trim()!==\"chunked\"){K.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),O.end();return}if(U=0,V)break}}K.#Y=U,K.#O=1,K.dispatchEvent(new Event(\"open\"));const W=Z.substring(z+4);if(H.#L(K,W,0),K.#Y>0){if(K.#Z+=W.length,K.#Z>=K.#Y)K.#O=2,O.end()}return}case 1:if(H.#L(K,A.toString(),2),K.#Y>0){if(K.#Z+=A.byteLength,K.#Z>=K.#Y)K.#O=2,O.end()}return;default:break}},drain(O){const A=O.data;if(A.#O===0){const K=A.#B;if(K.length){const Z=O.write(K);if(Z!==K.length)O.data.#J=K.substring(Z);else O.data.#J=\"\"}}},close:H.#j,end(O){H.#j(O).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(O){H.#j(O).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#j(O){const A=O.data;if(A.#W=null,A.#Z=0,A.#O=2,A.#X){if(A.#z)clearTimeout(A.#z);A.#z=setTimeout(H.#$,A.#K,A)}return A}constructor(O,A=@undefined){super();const K=new URL(O);this.#A=K.protocol===\"https:\",this.#w=K,this.#O=2,process.nextTick(H.#$,this)}ref(){this.#z\?.ref(),this.#W\?.ref()}unref(){this.#z\?.unref(),this.#W\?.unref()}#G(){if(this.#O!==2)return;const O=this.#w,A=this.#A;this.#O=0,@Bun.connect({data:this,socket:H.#H,hostname:O.hostname,port:parseInt(O.port||(A\?\"443\":\"80\"),10),tls:A\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((K)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:K})),this.#X){if(this.#z)this.#z.unref\?.();this.#z=setTimeout(H.#$,1000,this)}})}get url(){return this.#w.href}get readyState(){return this.#O}close(){this.#X=!1,this.#O=2,this.#W\?.unref(),this.#W\?.end()}get onopen(){return this.#V}get onerror(){return this.#Q}get onmessage(){return this.#U}set onopen(O){if(this.#V)super.removeEventListener(\"close\",this.#V);super.addEventListener(\"open\",O),this.#V=O}set onerror(O){if(this.#Q)super.removeEventListener(\"error\",this.#Q);super.addEventListener(\"error\",O),this.#Q=O}set onmessage(O){if(this.#U)super.removeEventListener(\"message\",this.#U);super.addEventListener(\"message\",O),this.#U=O}}return Object.defineProperty(H.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(H.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(H.prototype,\"CLOSED\",{enumerable:!0,value:2}),H[Symbol.for(\"CommonJS\")]=0,H})\n";
