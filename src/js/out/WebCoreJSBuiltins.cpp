...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class O extends EventTarget{#O;#$;#w;#L;#M;#j=!1;#A=null;#B=\"\";#F=\"\";#G=\"\";#J=!0;#K=0;#Q=0;#U=0;static#V(w){w.#z()}static#W(w,L){const M=w.data,j=M.#G\?`Last-Event-ID: ${M.#G}\\r\\n`:\"\",A=`GET ${L.pathname}${L.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${j}\\r\\n`,B=w.write(A);if(B!==A.length)M.#F=A.substring(B)}static#X(w,L,M){for(;;){if(M>=L.length)return;let j=-1,A=L.indexOf(\"\\r\\n\",M);const B=A+2;if(A>0)if(w.#K===0){const Q=parseInt(L.substring(M,A),16);if(Q===0){w.#$=2,w.#A\?.end();return}j=B+Q}else j=L.length;else{if(w.#B.length===0){w.#B+=L.substring(M);return}j=L.length}let F=L.substring(B,j);M=j+2;let G=0,J=F.indexOf(\"\\n\\n\");if(J==-1){w.#B+=L.substring(B);return}if(w.#B.length)w.#B+=F,F=w.#B,w.#B=\"\";let K=!0;while(K){const Q=F.substring(G,J);let U,V=\"\",W,X=0,Y=-1;for(;;){let z=Q.indexOf(\"\\n\",X);if(z===-1){if(X>=Q.length)break;z=Q.length}const H=Q.substring(X,z);if(H.startsWith(\"data:\"))if(V.length)V+=`\\n${H.substring(5).trim()}`;else V=H.substring(5).trim();else if(H.startsWith(\"event:\"))U=H.substring(6).trim();else if(H.startsWith(\"id:\"))W=H.substring(3).trim();else if(H.startsWith(\"retry:\")){if(Y=parseInt(H.substring(6).trim(),10),@isNaN(Y))Y=-1}X=z+1}if(w.#G=W||\"\",Y>=0)w.#U=Y;if(V||W||U)w.dispatchEvent(new MessageEvent(U||\"message\",{data:V||\"\",origin:w.#O.origin,source:w,lastEventId:W}));if(F.length===J+2){K=!1;break}const Z=F.indexOf(\"\\n\\n\",J+1);if(Z===-1)break;G=J,J=Z}}}static#Y={open(w){const L=w.data;if(L.#A=w,!L.#j)O.#W(w,L.#O)},handshake(w,L,M){const j=w.data;if(L)O.#W(w,j.#O);else j.#$=2,j.dispatchEvent(new ErrorEvent(\"error\",{error:M})),w.end()},data(w,L){const M=w.data;switch(M.#$){case 0:{let j=L.toString();const A=j.indexOf(\"\\r\\n\\r\\n\");if(A===-1){M.#B+=j;return}if(M.#B.length)M.#B+=j,j=M.#B,M.#B=\"\";const B=j.substring(0,A),F=B.indexOf(\"\\r\\n\");if(F===-1){M.#$=2,M.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),w.end();return}const G=B.substring(0,F);if(G!==\"HTTP/1.1 200 OK\"){M.#$=2,M.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(G)})),w.end();return}let J=F+1,K=!1,Q=-1;for(;;){let V=B.indexOf(\"\\r\\n\",J);if(V===-1){if(J>=B.length){if(!K)M.#$=2,M.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),w.end();return}V=B.length}const W=B.substring(J+1,V),X=W.indexOf(\":\"),Y=W.substring(0,X),Z=Y.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(J=V+1,Z)if(W.endsWith(\" text/event-stream\"))K=!0;else{M.#$=2,M.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),w.end();return}else if(Y.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(Q=parseInt(W.substring(X+1).trim(),10),@isNaN(Q)||Q<=0){M.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),w.end();return}if(K)break}else if(Y.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(W.substring(X+1).trim()!==\"chunked\"){M.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),w.end();return}if(Q=0,K)break}}M.#K=Q,M.#$=1,M.dispatchEvent(new Event(\"open\"));const U=j.substring(A+4);if(O.#X(M,U,0),M.#K>0){if(M.#Q+=U.length,M.#Q>=M.#K)M.#$=2,w.end()}return}case 1:if(O.#X(M,L.toString(),2),M.#K>0){if(M.#Q+=L.byteLength,M.#Q>=M.#K)M.#$=2,w.end()}return;default:break}},drain(w){const L=w.data;if(L.#$===0){const M=L.#B;if(M.length){const j=w.write(M);if(j!==M.length)w.data.#F=M.substring(j);else w.data.#F=\"\"}}},close:O.#Z,end(w){O.#Z(w).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(w){O.#Z(w).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#Z(w){const L=w.data;if(L.#A=null,L.#Q=0,L.#$=2,L.#J)setTimeout(O.#V,L.#U,L);return L}constructor(w,L=@undefined){super();const M=new URL(w);this.#j=M.protocol===\"https:\",this.#O=M,this.#$=2,process.nextTick(O.#V,this)}#z(){if(this.#$!==2)return;const w=this.#O,L=this.#j;this.#$=0,@Bun.connect({data:this,socket:O.#Y,hostname:w.hostname,port:parseInt(w.port||(L\?\"443\":\"80\"),10),tls:L\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((M)=>{if(this.dispatchEvent(new ErrorEvent(\"error\",{error:M})),this.#J)setTimeout(O.#V,1000,this)})}get url(){return this.#O.href}get readyState(){return this.#$}close(){this.#J=!1,this.#$=2,this.#A\?.end()}get onopen(){return this.#M}get onerror(){return this.#w}get onmessage(){return this.#L}set onopen(w){if(this.#M)this.removeEventListener(\"close\",this.#M);this.addEventListener(\"open\",w),this.#M=w}set onerror(w){if(this.#w)this.removeEventListener(\"error\",this.#w);this.addEventListener(\"error\",w),this.#w=w}set onmessage(w){if(this.#L)this.removeEventListener(\"message\",this.#L);this.addEventListener(\"message\",w),this.#L=w}}return Object.defineProperty(O.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(O.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(O.prototype,\"CLOSED\",{enumerable:!0,value:2}),O[Symbol.for(\"CommonJS\")]=0,O})\n";
