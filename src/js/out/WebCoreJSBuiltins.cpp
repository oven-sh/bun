...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class w extends EventTarget{#w;#$;#L;#M;#O;#j=!1;#A=null;#B=\"\";#F=\"\";#G=\"\";#J=!0;#K=0;#Q=0;#U=0;static#V(L){L.#H()}static#W(L,M){const O=L.data,j=O.#G\?`Last-Event-ID: ${O.#G}\\r\\n`:\"\",A=`GET ${M.pathname}${M.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${j}\\r\\n`,B=L.write(A);if(B!==A.length)O.#F=A.substring(B)}static#X(L,M,O){for(;;){if(O>=M.length)return;let j=-1,A=M.indexOf(\"\\r\\n\",O);const B=A+2;if(A>0)if(L.#K===0){const Q=parseInt(M.substring(O,A),16);if(Q===0){L.#$=2,L.#A\?.end();return}j=B+Q}else j=M.length;else{if(L.#B.length===0){L.#B+=M.substring(O);return}j=M.length}let F=M.substring(B,j);O=j+2;let G=0,J=F.indexOf(\"\\n\\n\");if(J==-1){L.#B+=M.substring(B);return}if(L.#B.length)L.#B+=F,F=L.#B,L.#B=\"\";let K=!0;while(K){const Q=F.substring(G,J);let U,V=\"\",W,X=0,Y=-1;for(;;){let H=Q.indexOf(\"\\n\",X);if(H===-1){if(X>=Q.length)break;H=Q.length}const z=Q.substring(X,H);if(z.startsWith(\"data:\"))if(V.length)V+=`\\n${z.substring(5).trim()}`;else V=z.substring(5).trim();else if(z.startsWith(\"event:\"))U=z.substring(6).trim();else if(z.startsWith(\"id:\"))W=z.substring(3).trim();else if(z.startsWith(\"retry:\")){if(Y=parseInt(z.substring(6).trim(),10),@isNaN(Y))Y=-1}X=H+1}if(L.#G=W||\"\",Y>=0)L.#U=Y;if(V||W||U)L.dispatchEvent(new MessageEvent(U||\"message\",{data:V||\"\",origin:L.#w.origin,source:L,lastEventId:W}));if(F.length===J+2){K=!1;break}const Z=F.indexOf(\"\\n\\n\",J+1);if(Z===-1)break;G=J,J=Z}}}static#Y={open(L){const M=L.data;if(M.#A=L,!M.#j)w.#W(L,M.#w)},handshake(L,M,O){const j=L.data;if(M)w.#W(L,j.#w);else j.#$=2,j.dispatchEvent(new ErrorEvent(\"error\",{error:O})),L.end()},data(L,M){const O=L.data;switch(O.#$){case 0:{let j=M.toString();const A=j.indexOf(\"\\r\\n\\r\\n\");if(A===-1){O.#B+=j;return}if(O.#B.length)O.#B+=j,j=O.#B,O.#B=\"\";const B=j.substring(0,A),F=B.indexOf(\"\\r\\n\");if(F===-1){O.#$=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),L.end();return}const G=B.substring(0,F);if(G!==\"HTTP/1.1 200 OK\"){O.#$=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(G)})),L.end();return}let J=F+1,K=!1,Q=-1;for(;;){let V=B.indexOf(\"\\r\\n\",J);if(V===-1){if(J>=B.length){if(!K)O.#$=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),L.end();return}V=B.length}const W=B.substring(J+1,V),X=W.indexOf(\":\"),Y=W.substring(0,X),Z=Y.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(J=V+1,Z)if(W.endsWith(\" text/event-stream\"))K=!0;else{O.#$=2,O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),L.end();return}else if(Y.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(Q=parseInt(W.substring(X+1).trim(),10),@isNaN(Q)||Q<=0){O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),L.end();return}if(K)break}else if(Y.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(W.substring(X+1).trim()!==\"chunked\"){O.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),L.end();return}if(Q=0,K)break}}O.#K=Q,O.#$=1,O.dispatchEvent(new Event(\"open\"));const U=j.substring(A+4);if(w.#X(O,U,0),O.#K>0){if(O.#Q+=U.length,O.#Q>=O.#K)O.#$=2,L.end()}return}case 1:if(w.#X(O,M.toString(),2),O.#K>0){if(O.#Q+=M.byteLength,O.#Q>=O.#K)O.#$=2,L.end()}return;default:break}},drain(L){const M=L.data;if(M.#$===0){const O=M.#B;if(O.length){const j=L.write(O);if(j!==O.length)L.data.#F=O.substring(j);else L.data.#F=\"\"}}},close:w.#Z,end(L){w.#Z(L).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(L){w.#Z(L).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#Z(L){const M=L.data;if(M.#A=null,M.#Q=0,M.#$=2,M.#J)setTimeout(w.#V,M.#U,M);return M}constructor(L,M=@undefined){super();const O=new URL(L);this.#j=O.protocol===\"https:\",this.#w=O,this.#$=2,process.nextTick(w.#V,this)}#H(){if(this.#$!==2)return;const L=this.#w,M=this.#j;this.#$=0,@Bun.connect({data:this,socket:w.#Y,hostname:L.hostname,port:parseInt(L.port||(M\?\"443\":\"80\"),10),tls:M\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((O)=>{if(this.dispatchEvent(new ErrorEvent(\"error\",{error:O})),this.#J)setTimeout(w.#V,1000,this)})}get url(){return this.#w.href}get readyState(){return this.#$}close(){this.#J=!1,this.#$=2,this.#A\?.end()}get onopen(){return this.#O}get onerror(){return this.#L}get onmessage(){return this.#M}set onopen(L){if(this.#O)this.removeEventListener(\"close\",this.#O);this.addEventListener(\"open\",L),this.#O=L}set onerror(L){if(this.#L)this.removeEventListener(\"error\",this.#L);this.addEventListener(\"error\",L),this.#L=L}set onmessage(L){if(this.#M)this.removeEventListener(\"message\",this.#M);this.addEventListener(\"message\",L),this.#M=L}}return Object.defineProperty(w.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(w.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(w.prototype,\"CLOSED\",{enumerable:!0,value:2}),w[Symbol.for(\"CommonJS\")]=0,w})\n";
