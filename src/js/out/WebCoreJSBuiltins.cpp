...
const char* const s_bundlerPluginRunOnResolvePluginsCode = "(function (_,z,A,B,w){\"use strict\";const C=[\"entry-point\",\"import-statement\",\"require-call\",\"dynamic-import\",\"require-resolve\",\"import-rule\",\"url-token\",\"internal\"][w];var E=(async(F,G,H,g)=>{var{onResolve:y,onLoad:q}=this,J=y.@get(G);if(!J)return this.onResolveAsync(B,null,null,null),null;for(let[O,Q]of J)if(O.test(F)){var b=Q({path:F,importer:H,namespace:G,kind:g});while(b&&@isPromise(b)&&(@getPromiseInternalField(b,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)b=@getPromiseInternalField(b,@promiseFieldReactionsOrResult);if(b&&@isPromise(b))b=await b;if(!b||!@isObject(b))continue;var{path:K,namespace:j=G,external:M}=b;if(typeof K!==\"string\"||typeof j!==\"string\")@throwTypeError(\"onResolve plugins must return an object with a string 'path' and string 'loader' field\");if(!K)continue;if(!j)j=G;if(typeof M!==\"boolean\"&&!@isUndefinedOrNull(M))@throwTypeError('onResolve plugins \"external\" field must be boolean or unspecified');if(!M){if(j===\"file\"){if(darwin!==\"win32\"){if(K[0]!==\"/\"||K.includes(\"..\"))@throwTypeError('onResolve plugin \"path\" must be absolute when the namespace is \"file\"')}}if(j===\"dataurl\"){if(!K.startsWith(\"data:\"))@throwTypeError('onResolve plugin \"path\" must start with \"data:\" when the namespace is \"dataurl\"')}if(j&&j!==\"file\"&&(!q||!q.@has(j)))@throwTypeError(`Expected onLoad plugin for namespace ${j} to exist`)}return this.onResolveAsync(B,K,j,M),null}return this.onResolveAsync(B,null,null,null),null})(_,z,A,C);while(E&&@isPromise(E)&&(@getPromiseInternalField(E,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)E=@getPromiseInternalField(E,@promiseFieldReactionsOrResult);if(E&&@isPromise(E))E.then(()=>{},(F)=>{this.addError(B,F,0)})})\n";
...
const char* const s_transformStreamInternalsCreateTransformStreamCode = "(function (_,B,c,D,j,E,q){\"use strict\";if(D===@undefined)D=1;if(j===@undefined)j=()=>1;if(E===@undefined)E=0;if(q===@undefined)q=()=>1;@assert(D>=0),@assert(E>=0);const F={};@putByIdDirectPrivate(F,\"TransformStream\",!0);const v=new @TransformStream(F),G=@newPromiseCapability(@Promise);@initializeTransformStream(v,G.@promise,D,j,E,q);const x=new @TransformStreamDefaultController;return @setUpTransformStreamDefaultController(v,x,B,c),_().@then(()=>{G.@resolve.@call()},(I)=>{G.@reject.@call(@undefined,I)}),v})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultControllerTerminateCode = "(function (i){\"use strict\";const _=@getByIdDirectPrivate(i,\"stream\"),k=@getByIdDirectPrivate(_,\"readable\"),f=@getByIdDirectPrivate(k,\"readableStreamController\");if(@readableStreamDefaultControllerCanCloseOrEnqueue(f))@readableStreamDefaultControllerClose(f);const u=@makeTypeError(\"the stream has been terminated\");@transformStreamErrorWritableAndUnblockWrite(_,u)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultSinkCloseAlgorithmCode = "(function (_){\"use strict\";const I=@getByIdDirectPrivate(_,\"readable\"),j=@getByIdDirectPrivate(_,\"controller\"),k=@getByIdDirectPrivate(I,\"readableStreamController\"),u=@getByIdDirectPrivate(j,\"flushAlgorithm\");@assert(u!==@undefined);const f=@getByIdDirectPrivate(j,\"flushAlgorithm\").@call();@transformStreamDefaultControllerClearAlgorithms(j);const q=@newPromiseCapability(@Promise);return f.@then(()=>{if(@getByIdDirectPrivate(I,\"state\")===@streamErrored){q.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"));return}if(@readableStreamDefaultControllerCanCloseOrEnqueue(k))@readableStreamDefaultControllerClose(k);q.@resolve()},(v)=>{@transformStreamError(@getByIdDirectPrivate(j,\"stream\"),v),q.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"))}),q.@promise})\n";
...
const char* const s_processObjectInternalsGetStdioWriteStreamCode = "(function (Z,K){\"use strict\";var L={path:\"node:process\",require:K},M=(Q)=>L.require(Q);function N(Q){var{Duplex:V,eos:Y,destroy:P}=M(\"node:stream\"),A=class T extends V{#$;#B;#j=!0;#z=!0;#G;#H;#J;#K;#L;#M;get isTTY(){return this.#M\?\?=M(\"node:tty\").isatty(Q)}get fd(){return Q}constructor(x){super({readable:!0,writable:!0});this.#G=`/dev/fd/${x}`}#N(x){const J=this.#H;if(this.#H=null,J)J(x);else if(x)this.destroy(x);else if(!this.#j&&!this.#z)this.destroy()}_destroy(x,J){if(!x&&this.#H!==null){var U=class G extends Error{code;name;constructor(j=\"The operation was aborted\",X=void 0){if(X!==void 0&&typeof X!==\"object\")throw new Error(`Invalid AbortError options:\\n\\n${JSON.stringify(X,null,2)}`);super(j,X);this.code=\"ABORT_ERR\",this.name=\"AbortError\"}};x=new U}if(this.#J=null,this.#K=null,this.#H===null)J(x);else{if(this.#H=J,this.#$)P(this.#$,x);if(this.#B)P(this.#B,x)}}_write(x,J,U){if(!this.#$){var{createWriteStream:G}=M(\"node:fs\"),j=this.#$=G(this.#G);j.on(\"finish\",()=>{if(this.#K){const X=this.#K;this.#K=null,X()}}),j.on(\"drain\",()=>{if(this.#J){const X=this.#J;this.#J=null,X()}}),Y(j,(X)=>{if(this.#z=!1,X)P(j,X);this.#N(X)})}if(j.write(x,J))U();else this.#J=U}_final(x){this.#$&&this.#$.end(),this.#K=x}#O(){var{createReadStream:x}=M(\"node:fs\"),J=this.#B=x(this.#G);return J.on(\"readable\",()=>{if(this.#L){const U=this.#L;this.#L=null,U()}else this.read()}),J.on(\"end\",()=>{this.push(null)}),Y(J,(U)=>{if(this.#j=!1,U)P(J,U);this.#N(U)}),J}_read(){var x=this.#B;if(!x)x=this.#O();while(!0){const J=x.read();if(J===null||!this.push(J))return}}};return new A(Q)}var{EventEmitter:B}=M(\"node:events\");function z(Q){if(!Q)return!0;var V=Q.toLowerCase();return V===\"utf8\"||V===\"utf-8\"||V===\"buffer\"||V===\"binary\"}var O,H=class Q extends B{#$;#B;#j;#z;bytesWritten=0;setDefaultEncoding(V){if(this.#B||!z(V))return this.#J(),this.#B.setDefaultEncoding(V)}#G(){switch(this.#$){case 1:{var V=@Bun.stdout.writer({highWaterMark:0});return V.unref(),V}case 2:{var V=@Bun.stderr.writer({highWaterMark:0});return V.unref(),V}default:throw new Error(\"Unsupported writer\")}}#H(){return this.#j\?\?=this.#G()}constructor(V){super();this.#$=V}get fd(){return this.#$}get isTTY(){return this.#z\?\?=M(\"node:tty\").isatty(this.#$)}cursorTo(V,Y,P){return(O\?\?=M(\"readline\")).cursorTo(this,V,Y,P)}moveCursor(V,Y,P){return(O\?\?=M(\"readline\")).moveCursor(this,V,Y,P)}clearLine(V,Y){return(O\?\?=M(\"readline\")).clearLine(this,V,Y)}clearScreenDown(V){return(O\?\?=M(\"readline\")).clearScreenDown(this,V)}ref(){this.#H().ref()}unref(){this.#H().unref()}on(V,Y){if(V===\"close\"||V===\"finish\")return this.#J(),this.#B.on(V,Y);if(V===\"drain\")return super.on(\"drain\",Y);if(V===\"error\")return super.on(\"error\",Y);return super.on(V,Y)}get _writableState(){return this.#J(),this.#B._writableState}get _readableState(){return this.#J(),this.#B._readableState}pipe(V){return this.#J(),this.#B.pipe(V)}unpipe(V){return this.#J(),this.#B.unpipe(V)}#J(){if(this.#B)return;this.#B=N(this.#$);const V=this.eventNames();for(let Y of V)this.#B.on(Y,(...P)=>{this.emit(Y,...P)})}#K(V){var Y=this.#H();const P=Y.write(V);this.bytesWritten+=P;const A=Y.flush(!1);return!!(P||A)}#L(V,Y){if(!z(Y))return this.#J(),this.#B.write(V,Y);return this.#K(V)}#M(V,Y){if(Y)this.emit(\"error\",Y);try{V(Y\?Y:null)}catch(P){this.emit(\"error\",P)}}#N(V,Y,P){if(!z(Y))return this.#J(),this.#B.write(V,Y,P);var A=this.#H();const T=A.write(V),x=A.flush(!0);if(x\?.then)return x.then(()=>{this.#M(P),this.emit(\"drain\")},(J)=>this.#M(P,J)),!1;return queueMicrotask(()=>{this.#M(P)}),!!(T||x)}write(V,Y,P){const A=this._write(V,Y,P);if(A)this.emit(\"drain\");return A}get hasColors(){return @Bun.tty[this.#$].hasColors}_write(V,Y,P){var A=this.#B;if(A)return A.write(V,Y,P);switch(arguments.length){case 0:{var T=new Error(\"Invalid arguments\");throw T.code=\"ERR_INVALID_ARG_TYPE\",T}case 1:return this.#K(V);case 2:if(typeof Y===\"function\")return this.#N(V,\"\",Y);else if(typeof Y===\"string\")return this.#L(V,Y);default:{if(typeof Y!==\"undefined\"&&typeof Y!==\"string\"||typeof P!==\"undefined\"&&typeof P!==\"function\"){var T=new Error(\"Invalid arguments\");throw T.code=\"ERR_INVALID_ARG_TYPE\",T}if(typeof P===\"undefined\")return this.#L(V,Y);return this.#N(V,Y,P)}}}destroy(){return this}end(){return this}};return new H(Z)})\n";
...
const char* const s_transformStreamInitializeTransformStreamCode = "(function (){\"use strict\";let _=arguments[0];if(@isObject(_)&&@getByIdDirectPrivate(_,\"TransformStream\"))return this;let v=arguments[1],B=arguments[2];if(_===@undefined)_=null;if(B===@undefined)B={};if(v===@undefined)v={};let u={};if(_!==null){if(\"start\"in _){if(u[\"start\"]=_[\"start\"],typeof u[\"start\"]!==\"function\")@throwTypeError(\"transformer.start should be a function\")}if(\"transform\"in _){if(u[\"transform\"]=_[\"transform\"],typeof u[\"transform\"]!==\"function\")@throwTypeError(\"transformer.transform should be a function\")}if(\"flush\"in _){if(u[\"flush\"]=_[\"flush\"],typeof u[\"flush\"]!==\"function\")@throwTypeError(\"transformer.flush should be a function\")}if(\"readableType\"in _)@throwRangeError(\"TransformStream transformer has a readableType\");if(\"writableType\"in _)@throwRangeError(\"TransformStream transformer has a writableType\")}const q=@extractHighWaterMark(B,0),x=@extractSizeAlgorithm(B),E=@extractHighWaterMark(v,1),F=@extractSizeAlgorithm(v),G=@newPromiseCapability(@Promise);if(@initializeTransformStream(this,G.@promise,E,F,q,x),@setUpTransformStreamDefaultControllerFromTransformer(this,_,u),(\"start\"in u)){const j=@getByIdDirectPrivate(this,\"controller\");(()=>@promiseInvokeOrNoopMethodNoCatch(_,u[\"start\"],[j]))().@then(()=>{G.@resolve.@call()},(J)=>{G.@reject.@call(@undefined,J)})}else G.@resolve.@call();return this})\n";
...
const char* const s_jsBufferPrototypeSliceCode = "(function (c,m){\"use strict\";var{buffer:q,byteOffset:v,byteLength:w}=this;function z(k,x){if(k=@trunc(k),k===0||@isNaN(k))return 0;else if(k<0)return k+=x,k>0\?k:0;else return k<x\?k:x}var p=z(c,w),i=m!==@undefined\?z(m,w):w;return new @Buffer(q,v+p,i>p\?i-p:0)})\n";
...
const char* const s_consoleObjectAsyncIteratorCode = "(function (){\"use strict\";const m=async function*z(){var w=@Bun.stdin.stream().getReader(),H=new globalThis.TextDecoder(\"utf-8\",{fatal:!1}),j,J=@Bun.indexOfLine;try{while(!0){var K,L,F;const B=w.readMany();if(@isPromise(B))({done:K,value:L}=await B);else({done:K,value:L}=B);if(K){if(F)yield H.decode(F);return}var _;for(let M of L){if(_=M,F)_=@Buffer.concat([F,M]),F=null;var A=0,G=J(_,A);while(G!==-1)yield H.decode(_.subarray(A,G)),A=G+1,G=J(_,A);F=_.subarray(A)}}}catch(B){j=B}finally{if(w.releaseLock(),j)throw j}},q=globalThis.Symbol.asyncIterator;return this[q]=m,m()})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToCode = "(function (c,h){\"use strict\";@assert(@isReadableStream(c));const g=new @ReadableStreamDefaultReader(c);@getByIdDirectPrivate(g,\"closedPromiseCapability\").@promise.@then(()=>{},(S)=>{h.error(S)});function y(){@readableStreamDefaultReaderRead(g).@then(function(S){if(S.done){h.close();return}try{h.enqueue(S.value)}catch(_){h.error(\"ReadableStream chunk enqueueing in the sink failed\");return}y()},function(S){h.error(S)})}y()})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToWritableStreamCode = "(function (f,H,J,D,E,K){\"use strict\";if(@assert(@isReadableStream(f)),@assert(@isWritableStream(H)),@assert(!@isReadableStreamLocked(f)),@assert(!@isWritableStreamLocked(H)),@assert(K===@undefined||@isAbortSignal(K)),@getByIdDirectPrivate(f,\"underlyingByteSource\")!==@undefined)return @Promise.@reject(\"Piping to a readable bytestream is not supported\");let _={source:f,destination:H,preventAbort:D,preventCancel:E,preventClose:J,signal:K};if(_.reader=@acquireReadableStreamDefaultReader(f),_.writer=@acquireWritableStreamDefaultWriter(H),@putByIdDirectPrivate(f,\"disturbed\",!0),_.finalized=!1,_.shuttingDown=!1,_.promiseCapability=@newPromiseCapability(@Promise),_.pendingReadPromiseCapability=@newPromiseCapability(@Promise),_.pendingReadPromiseCapability.@resolve.@call(),_.pendingWritePromise=@Promise.@resolve(),K!==@undefined){const T=(k)=>{if(_.finalized)return;@pipeToShutdownWithAction(_,()=>{const L=!_.preventAbort&&@getByIdDirectPrivate(_.destination,\"state\")===\"writable\"\?@writableStreamAbort(_.destination,k):@Promise.@resolve(),w=!_.preventCancel&&@getByIdDirectPrivate(_.source,\"state\")===@streamReadable\?@readableStreamCancel(_.source,k):@Promise.@resolve();let x=@newPromiseCapability(@Promise),z=!0,B=()=>{if(z){z=!1;return}x.@resolve.@call()},F=(G)=>{x.@reject.@call(@undefined,G)};return L.@then(B,F),w.@then(B,F),x.@promise},k)};if(@whenSignalAborted(K,T))return _.promiseCapability.@promise}return @pipeToErrorsMustBePropagatedForward(_),@pipeToErrorsMustBePropagatedBackward(_),@pipeToClosingMustBePropagatedForward(_),@pipeToClosingMustBePropagatedBackward(_),@pipeToLoop(_),_.promiseCapability.@promise})\n";
...
const char* const s_readableStreamInternalsAssignToStreamCode = "(function (h,f){\"use strict\";var _=@getByIdDirectPrivate(h,\"underlyingSource\");if(_)try{return @readDirectStream(h,f,_)}catch(b){throw b}finally{_=@undefined,h=@undefined,f=@undefined}return @readStreamIntoSink(h,f,!0)})\n";
...
const char* const s_readableStreamInternalsHandleDirectStreamErrorCode = "(function (u){\"use strict\";var h=this,R=h.@sink;if(R){@putByIdDirectPrivate(h,\"sink\",@undefined);try{R.close(u)}catch(a){}}if(this.error=this.flush=this.write=this.close=this.end=@onReadableStreamDirectControllerClosed,typeof this.@underlyingSource.close===\"function\")try{this.@underlyingSource.close.@call(this.@underlyingSource,u)}catch(a){}try{var i=h._pendingRead;if(i)h._pendingRead=@undefined,@rejectPromise(i,u)}catch(a){}var _=h.@controlledReadableStream;if(_)@readableStreamError(_,u)})\n";
...
const char* const s_readableStreamInternalsReadableStreamFulfillReadRequestCode = "(function (i,y,p){\"use strict\";const _=@getByIdDirectPrivate(@getByIdDirectPrivate(i,\"reader\"),\"readRequests\").shift();@fulfillPromise(_,{value:y,done:p})})\n";
...
const char* const s_readableStreamInternalsLazyLoadStreamCode = "(function (Q,G){\"use strict\";var K=@getByIdDirectPrivate(Q,\"bunNativeType\"),H=@getByIdDirectPrivate(Q,\"bunNativePtr\"),I=@lazyStreamPrototypeMap.@get(K);if(I===@undefined){let m=function(D){var{c:p,v:z}=this;this.c=@undefined,this.v=@undefined,X(D,p,z)},J=function(D){try{D.close()}catch(p){globalThis.reportError(p)}},_=function(D,p,z,A){A[0]=!1;var E;try{E=f(D,z,A)}catch(F){return p.error(F)}return X(E,p,z)};var j=m,P=J,q=_,[f,b,L,x,M,O,U]=@lazyLoad(K),W=[!1],X;X=function D(p,z,A){if(p&&@isPromise(p))return p.then(m.bind({c:z,v:A}),(E)=>z.error(E));else if(typeof p===\"number\")if(A&&A.byteLength===p&&A.buffer===z.byobRequest\?.view\?.buffer)z.byobRequest.respondWithNewView(A);else z.byobRequest.respond(p);else if(p.constructor===@Uint8Array)z.enqueue(p);if(W[0]||p===!1)@enqueueJob(J,z),W[0]=!1};const B=M\?new FinalizationRegistry(M):null;I=class D{constructor(p,z,A){if(this.#f=p,this.#b={},this.pull=this.#j.bind(this),this.cancel=this.#m.bind(this),this.autoAllocateChunkSize=z,A!==@undefined)this.start=(E)=>{E.enqueue(A)};if(B)B.register(this,p,this.#b)}#b;pull;cancel;start;#f;type=\"bytes\";autoAllocateChunkSize=0;static startSync=b;#j(p){var z=this.#f;if(!z){p.close();return}_(z,p,p.byobRequest.view,W)}#m(p){var z=this.#f;B&&B.unregister(this.#b),O&&O(z,!1),L(z,p)}static deinit=M;static drain=U},@lazyStreamPrototypeMap.@set(K,I)}const Y=I.startSync(H,G);var Z;const{drain:y,deinit:N}=I;if(y)Z=y(H);if(Y===0){if(M&&H&&@enqueueJob(M,H),(Z\?.byteLength\?\?0)>0)return{start(m){m.enqueue(Z),m.close()},type:\"bytes\"};return{start(m){m.close()},type:\"bytes\"}}return new I(H,Y,Z)})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const U=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!U)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const A=@getByIdDirectPrivate(U,\"state\");if(@putByIdDirectPrivate(U,\"disturbed\",!0),A===@streamClosed)return{value:[],size:0,done:!0};else if(A===@streamErrored)throw @getByIdDirectPrivate(U,\"storedError\");var k=@getByIdDirectPrivate(U,\"readableStreamController\"),D=@getByIdDirectPrivate(k,\"queue\");if(!D)return k.@pull(k).@then(function({done:H,value:I}){return H\?{done:!0,value:[],size:0}:{value:[I],size:1,done:!1}});const j=D.content;var _=D.size,x=j.toArray(!1),C=x.length;if(C>0){var E=@newArrayWithSize(C);if(@isReadableByteStreamController(k)){{const H=x[0];if(!(@ArrayBuffer.@isView(H)||H instanceof @ArrayBuffer))@putByValDirect(E,0,new @Uint8Array(H.buffer,H.byteOffset,H.byteLength));else @putByValDirect(E,0,H)}for(var F=1;F<C;F++){const H=x[F];if(!(@ArrayBuffer.@isView(H)||H instanceof @ArrayBuffer))@putByValDirect(E,F,new @Uint8Array(H.buffer,H.byteOffset,H.byteLength));else @putByValDirect(E,F,H)}}else{@putByValDirect(E,0,x[0].value);for(var F=1;F<C;F++)@putByValDirect(E,F,x[F].value)}if(@resetQueue(@getByIdDirectPrivate(k,\"queue\")),@getByIdDirectPrivate(k,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(k,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(k))@readableStreamDefaultControllerCallPullIfNeeded(k);else if(@isReadableByteStreamController(k))@readableByteStreamControllerCallPullIfNeeded(k);return{value:E,size:_,done:!1}}var d=(H)=>{if(H.done)return{value:[],size:0,done:!0};var I=@getByIdDirectPrivate(U,\"readableStreamController\"),W=@getByIdDirectPrivate(I,\"queue\"),B=[H.value].concat(W.content.toArray(!1)),J=B.length;if(@isReadableByteStreamController(I))for(var K=0;K<J;K++){const Q=B[K];if(!(@ArrayBuffer.@isView(Q)||Q instanceof @ArrayBuffer)){const{buffer:w,byteOffset:S,byteLength:T}=Q;@putByValDirect(B,K,new @Uint8Array(w,S,T))}}else for(var K=1;K<J;K++)@putByValDirect(B,K,B[K].value);var N=W.size;if(@resetQueue(W),@getByIdDirectPrivate(I,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(I,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(I))@readableStreamDefaultControllerCallPullIfNeeded(I);else if(@isReadableByteStreamController(I))@readableByteStreamControllerCallPullIfNeeded(I);return{value:B,size:N,done:!1}},G=k.@pull(k);if(G&&@isPromise(G))return G.@then(d);return d(G)})\n";
...
const char* const s_streamInternalsPromiseInvokeOrNoopCode = "(function (n,l,m){\"use strict\";try{return @promiseInvokeOrNoopNoCatch(n,l,m)}catch(d){return @Promise.@reject(d)}})\n";
...
const char* const s_streamInternalsCreateFIFOCode = "(function (){\"use strict\";var b=@Array.prototype.slice;class v{constructor(){this._head=0,this._tail=0,this._capacityMask=3,this._list=@newArrayWithSize(4)}_head;_tail;_capacityMask;_list;size(){if(this._head===this._tail)return 0;if(this._head<this._tail)return this._tail-this._head;else return this._capacityMask+1-(this._head-this._tail)}isEmpty(){return this.size()==0}isNotEmpty(){return this.size()>0}shift(){var{_head:x,_tail:w,_list:g,_capacityMask:z}=this;if(x===w)return @undefined;var B=g[x];if(@putByValDirect(g,x,@undefined),x=this._head=x+1&z,x<2&&w>1e4&&w<=g.length>>>2)this._shrinkArray();return B}peek(){if(this._head===this._tail)return @undefined;return this._list[this._head]}push(x){var w=this._tail;if(@putByValDirect(this._list,w,x),this._tail=w+1&this._capacityMask,this._tail===this._head)this._growArray()}toArray(x){var w=this._list,g=@toLength(w.length);if(x||this._head>this._tail){var z=@toLength(this._head),B=@toLength(this._tail),E=@toLength(g-z+B),k=@newArrayWithSize(E),F=0;for(var A=z;A<g;A++)@putByValDirect(k,F++,w[A]);for(var A=0;A<B;A++)@putByValDirect(k,F++,w[A]);return k}else return b.@call(w,this._head,this._tail)}clear(){this._head=0,this._tail=0,this._list.fill(@undefined)}_growArray(){if(this._head)this._list=this.toArray(!0),this._head=0;this._tail=@toLength(this._list.length),this._list.length<<=1,this._capacityMask=this._capacityMask<<1|1}shrinkArray(){this._list.length>>>=1,this._capacityMask>>>=1}}return new v})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (D){\"use strict\";var J=@Loader,z=@createFIFO(),F=D;while(F){var G=J.registry.@get(F);if((G\?.state\?\?0)<=@ModuleFetch)@fulfillModuleSync(F),G=J.registry.@get(F);var H=@getPromiseInternalField(G.fetch,@promiseFieldReactionsOrResult),I=J.parseModule(F,H),L=G.module;if(I&&@isPromise(I)){var Q=@getPromiseInternalField(I,@promiseFieldReactionsOrResult),T=@getPromiseInternalField(I,@promiseFieldFlags),_=T&@promiseStateMask;if(_===@promiseStatePending||Q&&@isPromise(Q))@throwTypeError(`require() async module \"${F}\" is unsupported. use \"await import()\" instead.`);else if(_===@promiseStateRejected){if(!Q\?.message)@throwTypeError(`${Q+\"\"\?Q:\"An error occurred\"} occurred while parsing module \\\"${F}\\\"`);throw Q}G.module=L=Q}else if(I&&!L)G.module=L=I;@setStateToMax(G,@ModuleLink);var w=L.dependenciesMap,x=J.requestedModules(L),B=@newArrayWithSize(x.length);for(var U=0,V=x.length;U<V;++U){var W=x[U],X=W[0]===\"/\"\?W:J.resolve(W,F),Y=J.ensureRegistered(X);if(Y.state<@ModuleLink)z.push(X);@putByValDirect(B,U,Y),w.@set(W,Y)}G.dependencies=B,G.instantiate=@Promise.resolve(G),G.satisfy=@Promise.resolve(G),F=z.shift();while(F&&(J.registry.@get(F)\?.state\?\?@ModuleFetch)>=@ModuleLink)F=z.shift()}var Z=J.linkAndEvaluateModule(D,@undefined);if(Z&&@isPromise(Z))@throwTypeError(`require() async module \\\"${D}\\\" is unsupported. use \"await import()\" instead.`);return J.registry.@get(D)})\n";
...
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";var f=new Map,_={};return new Proxy(_,{get(h,u){const t=@requireMap.@get(u);if(t)return t;const b=@Loader.registry.@get(u);if(b\?.evaluated){const L=@Loader.getModuleNamespaceObject(b.module),c=L[@commonJSSymbol]===0||L.default\?.[@commonJSSymbol]\?L.default:L,g=@createCommonJSModule(u,c,!0);return @requireMap.@set(u,g),g}return _[u]},set(h,u,t){return @requireMap.@set(u,t),!0},has(h,u){return @requireMap.@has(u)||@Loader.registry.@has(u)},deleteProperty(h,u){return f.@delete(u),@requireMap.@delete(u),@Loader.registry.@delete(u),!0},ownKeys(h){var u=[...@requireMap.@keys()];const t=[...@Loader.registry.@keys()];for(let b of t)if(!u.includes(b))@arrayPush(u,b);return u},getPrototypeOf(h){return null},getOwnPropertyDescriptor(h,u){if(@requireMap.@has(u)||@Loader.registry.@has(u))return{configurable:!0,enumerable:!0}}})})\n";
...
const char* const s_readableStreamPipeToCode = "(function (u){\"use strict\";if(!@isReadableStream(this))return @Promise.@reject(@makeThisTypeError(\"ReadableStream\",\"pipeTo\"));if(@isReadableStreamLocked(this))return @Promise.@reject(@makeTypeError(\"ReadableStream is locked\"));let R=@argument(1),S=!1,j=!1,b=!1,m;if(!@isUndefinedOrNull(R)){if(!@isObject(R))return @Promise.@reject(@makeTypeError(\"options must be an object\"));try{j=!!R[\"preventAbort\"],b=!!R[\"preventCancel\"],S=!!R[\"preventClose\"],m=R[\"signal\"]}catch(E){return @Promise.@reject(E)}if(m!==@undefined&&!@isAbortSignal(m))return @Promise.@reject(@makeTypeError(\"options.signal must be AbortSignal\"))}const _=@getInternalWritableStream(u);if(!@isWritableStream(_))return @Promise.@reject(@makeTypeError(\"ReadableStream pipeTo requires a WritableStream\"));if(@isWritableStreamLocked(_))return @Promise.@reject(@makeTypeError(\"WritableStream is locked\"));return @readableStreamPipeToWritableStream(this,_,S,j,b,m)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullIntoCode = "(function (b,L){\"use strict\";const N=@getByIdDirectPrivate(b,\"controlledReadableStream\");let d=1;if(L.BYTES_PER_ELEMENT!==@undefined)d=L.BYTES_PER_ELEMENT;const k=L.constructor,R={buffer:L.buffer,byteOffset:L.byteOffset,byteLength:L.byteLength,bytesFilled:0,elementSize:d,ctor:k,readerType:\"byob\"};var _=@getByIdDirectPrivate(b,\"pendingPullIntos\");if(_\?.isNotEmpty())return R.buffer=@transferBufferToCurrentRealm(R.buffer),_.push(R),@readableStreamAddReadIntoRequest(N);if(@getByIdDirectPrivate(N,\"state\")===@streamClosed){const E=new k(R.buffer,R.byteOffset,0);return @createFulfilledPromise({value:E,done:!0})}if(@getByIdDirectPrivate(b,\"queue\").size>0){if(@readableByteStreamControllerFillDescriptorFromQueue(b,R)){const E=@readableByteStreamControllerConvertDescriptor(R);return @readableByteStreamControllerHandleQueueDrain(b),@createFulfilledPromise({value:E,done:!1})}if(@getByIdDirectPrivate(b,\"closeRequested\")){const E=@makeTypeError(\"Closing stream has been requested\");return @readableByteStreamControllerError(b,E),@Promise.@reject(E)}}R.buffer=@transferBufferToCurrentRealm(R.buffer),@getByIdDirectPrivate(b,\"pendingPullIntos\").push(R);const A=@readableStreamAddReadIntoRequest(N);return @readableByteStreamControllerCallPullIfNeeded(b),A})\n";
