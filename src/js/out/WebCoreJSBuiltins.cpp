...
const char* const s_processObjectInternalsBindingCode = "(function (p){\"use strict\";if(p!==\"constants\")@throwTypeError(\"process.binding() is not supported in Bun. If that breaks something, please file an issue and include a reproducible code sample.\");var o=globalThis.Symbol.for(\"process.bindings.constants\"),r=globalThis[o];if(!r){const{constants:u}=globalThis[globalThis.Symbol.for('Bun.lazy')](\"createImportMeta\",\"node:process\").require(\"node:fs\");r={fs:u,zlib:{},crypto:{},os:@Bun._Os().constants},globalThis[o]=r}return r})\n";
