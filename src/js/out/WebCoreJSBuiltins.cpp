...
const char* const s_transformStreamInternalsInitializeTransformStreamCode = "(function (f,B,T,j,x,E){\"use strict\";const G=()=>{return B},I=(F)=>{return @transformStreamDefaultSinkWriteAlgorithm(f,F)},J=(F)=>{return @transformStreamDefaultSinkAbortAlgorithm(f,F)},K=()=>{return @transformStreamDefaultSinkCloseAlgorithm(f)},L=@createWritableStream(G,I,K,J,T,j),N=()=>{return @transformStreamDefaultSourcePullAlgorithm(f)},q=(F)=>{return @transformStreamErrorWritableAndUnblockWrite(f,F),@Promise.@resolve()},C={};@putByIdDirectPrivate(C,\"start\",G),@putByIdDirectPrivate(C,\"pull\",N),@putByIdDirectPrivate(C,\"cancel\",q);const D={};@putByIdDirectPrivate(D,\"size\",E),@putByIdDirectPrivate(D,\"highWaterMark\",x);const v=new @ReadableStream(C,D);@putByIdDirectPrivate(f,\"writable\",L),@putByIdDirectPrivate(f,\"internalWritable\",@getInternalWritableStream(L)),@putByIdDirectPrivate(f,\"readable\",v),@putByIdDirectPrivate(f,\"backpressure\",@undefined),@putByIdDirectPrivate(f,\"backpressureChangePromise\",@undefined),@transformStreamSetBackpressure(f,!0),@putByIdDirectPrivate(f,\"controller\",@undefined)})\n";
...
const char* const s_transformStreamInternalsSetUpTransformStreamDefaultControllerCode = "(function (P,b,d,_){\"use strict\";@assert(@isTransformStream(P)),@assert(@getByIdDirectPrivate(P,\"controller\")===@undefined),@putByIdDirectPrivate(b,\"stream\",P),@putByIdDirectPrivate(P,\"controller\",b),@putByIdDirectPrivate(b,\"transformAlgorithm\",d),@putByIdDirectPrivate(b,\"flushAlgorithm\",_)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultControllerTerminateCode = "(function (i){\"use strict\";const f=@getByIdDirectPrivate(i,\"stream\"),k=@getByIdDirectPrivate(f,\"readable\"),_=@getByIdDirectPrivate(k,\"readableStreamController\");if(@readableStreamDefaultControllerCanCloseOrEnqueue(_))@readableStreamDefaultControllerClose(_);const u=@makeTypeError(\"the stream has been terminated\");@transformStreamErrorWritableAndUnblockWrite(f,u)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultSinkWriteAlgorithmCode = "(function (d,v){\"use strict\";const f=@getByIdDirectPrivate(d,\"internalWritable\");@assert(@getByIdDirectPrivate(f,\"state\")===\"writable\");const j=@getByIdDirectPrivate(d,\"controller\");if(@getByIdDirectPrivate(d,\"backpressure\")){const q=@newPromiseCapability(@Promise),x=@getByIdDirectPrivate(d,\"backpressureChangePromise\");return @assert(x!==@undefined),x.@promise.@then(()=>{const z=@getByIdDirectPrivate(f,\"state\");if(z===\"erroring\"){q.@reject.@call(@undefined,@getByIdDirectPrivate(f,\"storedError\"));return}@assert(z===\"writable\"),@transformStreamDefaultControllerPerformTransform(j,v).@then(()=>{q.@resolve()},(_)=>{q.@reject.@call(@undefined,_)})},(z)=>{q.@reject.@call(@undefined,z)}),q.@promise}return @transformStreamDefaultControllerPerformTransform(j,v)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultSinkCloseAlgorithmCode = "(function (q){\"use strict\";const I=@getByIdDirectPrivate(q,\"readable\"),f=@getByIdDirectPrivate(q,\"controller\"),_=@getByIdDirectPrivate(I,\"readableStreamController\"),j=@getByIdDirectPrivate(f,\"flushAlgorithm\");@assert(j!==@undefined);const k=@getByIdDirectPrivate(f,\"flushAlgorithm\").@call();@transformStreamDefaultControllerClearAlgorithms(f);const u=@newPromiseCapability(@Promise);return k.@then(()=>{if(@getByIdDirectPrivate(I,\"state\")===@streamErrored){u.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"));return}if(@readableStreamDefaultControllerCanCloseOrEnqueue(_))@readableStreamDefaultControllerClose(_);u.@resolve()},(v)=>{@transformStreamError(@getByIdDirectPrivate(f,\"stream\"),v),u.@reject.@call(@undefined,@getByIdDirectPrivate(I,\"storedError\"))}),u.@promise})\n";
...
const char* const s_processObjectInternalsGetStdioWriteStreamCode = "(function (X,K){\"use strict\";var N={path:\"node:process\",require:K},L=(O)=>N.require(O);function M(O){var{Duplex:U,eos:Y,destroy:P}=L(\"node:stream\"),A=class T extends U{#$;#B;#j=!0;#z=!0;#G;#H;#J;#K;#L;#M;get isTTY(){return this.#M\?\?=L(\"node:tty\").isatty(O)}get fd(){return O}constructor(x){super({readable:!0,writable:!0});this.#G=`/dev/fd/${x}`}#N(x){const J=this.#H;if(this.#H=null,J)J(x);else if(x)this.destroy(x);else if(!this.#j&&!this.#z)this.destroy()}_destroy(x,J){if(!x&&this.#H!==null){var Q=class z extends Error{code;name;constructor(j=\"The operation was aborted\",V=void 0){if(V!==void 0&&typeof V!==\"object\")throw new Error(`Invalid AbortError options:\\n\\n${JSON.stringify(V,null,2)}`);super(j,V);this.code=\"ABORT_ERR\",this.name=\"AbortError\"}};x=new Q}if(this.#J=null,this.#K=null,this.#H===null)J(x);else{if(this.#H=J,this.#$)P(this.#$,x);if(this.#B)P(this.#B,x)}}_write(x,J,Q){if(!this.#$){var{createWriteStream:z}=L(\"node:fs\"),j=this.#$=z(this.#G);j.on(\"finish\",()=>{if(this.#K){const V=this.#K;this.#K=null,V()}}),j.on(\"drain\",()=>{if(this.#J){const V=this.#J;this.#J=null,V()}}),Y(j,(V)=>{if(this.#z=!1,V)P(j,V);this.#N(V)})}if(j.write(x,J))Q();else this.#J=Q}_final(x){this.#$&&this.#$.end(),this.#K=x}#O(){var{createReadStream:x}=L(\"node:fs\"),J=this.#B=x(this.#G);return J.on(\"readable\",()=>{if(this.#L){const Q=this.#L;this.#L=null,Q()}else this.read()}),J.on(\"end\",()=>{this.push(null)}),Y(J,(Q)=>{if(this.#j=!1,Q)P(J,Q);this.#N(Q)}),J}_read(){var x=this.#B;if(!x)x=this.#O();while(!0){const J=x.read();if(J===null||!this.push(J))return}}};return new A(O)}var{EventEmitter:B}=L(\"node:events\");function G(O){if(!O)return!0;var U=O.toLowerCase();return U===\"utf8\"||U===\"utf-8\"||U===\"buffer\"||U===\"binary\"}var Z,H=class O extends B{#$;#B;#j;#z;bytesWritten=0;setDefaultEncoding(U){if(this.#B||!G(U))return this.#J(),this.#B.setDefaultEncoding(U)}#G(){switch(this.#$){case 1:{var U=@Bun.stdout.writer({highWaterMark:0});return U.unref(),U}case 2:{var U=@Bun.stderr.writer({highWaterMark:0});return U.unref(),U}default:throw new Error(\"Unsupported writer\")}}#H(){return this.#j\?\?=this.#G()}constructor(U){super();this.#$=U}get fd(){return this.#$}get isTTY(){return this.#z\?\?=L(\"node:tty\").isatty(this.#$)}cursorTo(U,Y,P){return(Z\?\?=L(\"readline\")).cursorTo(this,U,Y,P)}moveCursor(U,Y,P){return(Z\?\?=L(\"readline\")).moveCursor(this,U,Y,P)}clearLine(U,Y){return(Z\?\?=L(\"readline\")).clearLine(this,U,Y)}clearScreenDown(U){return(Z\?\?=L(\"readline\")).clearScreenDown(this,U)}ref(){this.#H().ref()}unref(){this.#H().unref()}on(U,Y){if(U===\"close\"||U===\"finish\")return this.#J(),this.#B.on(U,Y);if(U===\"drain\")return super.on(\"drain\",Y);if(U===\"error\")return super.on(\"error\",Y);return super.on(U,Y)}get _writableState(){return this.#J(),this.#B._writableState}get _readableState(){return this.#J(),this.#B._readableState}pipe(U){return this.#J(),this.#B.pipe(U)}unpipe(U){return this.#J(),this.#B.unpipe(U)}#J(){if(this.#B)return;this.#B=M(this.#$);const U=this.eventNames();for(let Y of U)this.#B.on(Y,(...P)=>{this.emit(Y,...P)})}#K(U){var Y=this.#H();const P=Y.write(U);this.bytesWritten+=P;const A=Y.flush(!1);return!!(P||A)}#L(U,Y){if(!G(Y))return this.#J(),this.#B.write(U,Y);return this.#K(U)}#M(U,Y){if(Y)this.emit(\"error\",Y);try{U(Y\?Y:null)}catch(P){this.emit(\"error\",P)}}#N(U,Y,P){if(!G(Y))return this.#J(),this.#B.write(U,Y,P);var A=this.#H();const T=A.write(U),x=A.flush(!0);if(x\?.then)return x.then(()=>{this.#M(P),this.emit(\"drain\")},(J)=>this.#M(P,J)),!1;return queueMicrotask(()=>{this.#M(P)}),!!(T||x)}write(U,Y,P){const A=this._write(U,Y,P);if(A)this.emit(\"drain\");return A}get hasColors(){return @Bun.tty[this.#$].hasColors}_write(U,Y,P){var A=this.#B;if(A)return A.write(U,Y,P);switch(arguments.length){case 0:{var T=new Error(\"Invalid arguments\");throw T.code=\"ERR_INVALID_ARG_TYPE\",T}case 1:return this.#K(U);case 2:if(typeof Y===\"function\")return this.#N(U,\"\",Y);else if(typeof Y===\"string\")return this.#L(U,Y);default:{if(typeof Y!==\"undefined\"&&typeof Y!==\"string\"||typeof P!==\"undefined\"&&typeof P!==\"function\"){var T=new Error(\"Invalid arguments\");throw T.code=\"ERR_INVALID_ARG_TYPE\",T}if(typeof P===\"undefined\")return this.#L(U,Y);return this.#N(U,Y,P)}}}destroy(){return this}end(){return this}};return new H(X)})\n";
...
const char* const s_moduleRequireCode = "(function (_){\"use strict\";const f=@requireMap.@get(_)||@requireMap.@get(_=@resolveSync(_,this.path,!1));if(f)return @evaluateCommonJSModule(f),f.exports;if(_.endsWith(\".json\")||_.endsWith(\".toml\")||_.endsWith(\".node\"))return @internalRequire(_);let l=@Loader.registry.@get(_);if(l\?.evaluated){const h=l.module,r=@Loader.getModuleNamespaceObject(h),v=r\?.[@commonJSSymbol]===0||r\?.default\?.[@commonJSSymbol]===0\?r.default:r;return @requireMap.@set(_,@createCommonJSModule(_,v,!0)),v}let w=this.@require(_);if(w===-1){const h=@createCommonJSModule(_,{},!1);@requireMap.@set(_,h);try{w=@requireESM(_)}catch(r){throw @requireMap.@delete(_),r}if(l=@Loader.registry.@get(_),l\?.evaluated){const r=@Loader.getModuleNamespaceObject(l.module);return h.exports=r\?.[@commonJSSymbol]===0||r\?.default\?.[@commonJSSymbol]===0\?r.default:r}}const y=@requireMap.@get(_);if(y)return @evaluateCommonJSModule(y),y.exports;return w})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToWritableStreamCode = "(function (E,f,T,x,B,F){\"use strict\";if(@assert(@isReadableStream(E)),@assert(@isWritableStream(f)),@assert(!@isReadableStreamLocked(E)),@assert(!@isWritableStreamLocked(f)),@assert(F===@undefined||@isAbortSignal(F)),@getByIdDirectPrivate(E,\"underlyingByteSource\")!==@undefined)return @Promise.@reject(\"Piping to a readable bytestream is not supported\");let G={source:E,destination:f,preventAbort:x,preventCancel:B,preventClose:T,signal:F};if(G.reader=@acquireReadableStreamDefaultReader(E),G.writer=@acquireWritableStreamDefaultWriter(f),@putByIdDirectPrivate(E,\"disturbed\",!0),G.finalized=!1,G.shuttingDown=!1,G.promiseCapability=@newPromiseCapability(@Promise),G.pendingReadPromiseCapability=@newPromiseCapability(@Promise),G.pendingReadPromiseCapability.@resolve.@call(),G.pendingWritePromise=@Promise.@resolve(),F!==@undefined){const H=(I)=>{if(G.finalized)return;@pipeToShutdownWithAction(G,()=>{const z=!G.preventAbort&&@getByIdDirectPrivate(G.destination,\"state\")===\"writable\"\?@writableStreamAbort(G.destination,I):@Promise.@resolve(),k=!G.preventCancel&&@getByIdDirectPrivate(G.source,\"state\")===@streamReadable\?@readableStreamCancel(G.source,I):@Promise.@resolve();let q=@newPromiseCapability(@Promise),w=!0,K=()=>{if(w){w=!1;return}q.@resolve.@call()},D=(L)=>{q.@reject.@call(@undefined,L)};return z.@then(K,D),k.@then(K,D),q.@promise},I)};if(@whenSignalAborted(F,H))return G.promiseCapability.@promise}return @pipeToErrorsMustBePropagatedForward(G),@pipeToErrorsMustBePropagatedBackward(G),@pipeToClosingMustBePropagatedForward(G),@pipeToClosingMustBePropagatedBackward(G),@pipeToLoop(G),G.promiseCapability.@promise})\n";
...
const char* const s_readableStreamInternalsOnCloseDirectStreamCode = "(function (v){\"use strict\";var b=this.@controlledReadableStream;if(!b||@getByIdDirectPrivate(b,\"state\")!==@streamReadable)return;if(this._deferClose!==0){this._deferClose=1,this._deferCloseReason=v;return}if(@putByIdDirectPrivate(b,\"state\",@streamClosing),typeof this.@underlyingSource.close===\"function\")try{this.@underlyingSource.close.@call(this.@underlyingSource,v)}catch(C){}var y;try{y=this.@sink.end(),@putByIdDirectPrivate(this,\"sink\",@undefined)}catch(C){if(this._pendingRead){var B=this._pendingRead;this._pendingRead=@undefined,@rejectPromise(B,C)}@readableStreamError(b,C);return}this.error=this.flush=this.write=this.close=this.end=@onReadableStreamDirectControllerClosed;var S=@getByIdDirectPrivate(b,\"reader\");if(S&&@isReadableStreamDefaultReader(S)){var c=this._pendingRead;if(c&&@isPromise(c)&&y\?.byteLength){this._pendingRead=@undefined,@fulfillPromise(c,{value:y,done:!1}),@readableStreamClose(b);return}}if(y\?.byteLength){var j=@getByIdDirectPrivate(S,\"readRequests\");if(j\?.isNotEmpty()){@readableStreamFulfillReadRequest(b,y,!1),@readableStreamClose(b);return}@putByIdDirectPrivate(b,\"state\",@streamReadable),this.@pull=()=>{var C=@createFulfilledPromise({value:y,done:!1});return y=@undefined,@readableStreamClose(b),b=@undefined,C}}else if(this._pendingRead){var B=this._pendingRead;this._pendingRead=@undefined,@putByIdDirectPrivate(this,\"pull\",@noopDoneFunction),@fulfillPromise(B,{value:@undefined,done:!0})}@readableStreamClose(b)})\n";
...
const char* const s_readableStreamInternalsCreateTextStreamCode = "(function (q){\"use strict\";var v,w=[],A=!1,x=!1,C=\"\",z=@toLength(0),E=@newPromiseCapability(@Promise),F=!1;return v={start(){},write(G){if(typeof G===\"string\"){var _=@toLength(G.length);if(_>0)C+=G,A=!0,z+=_;return _}if(!G||!(@ArrayBuffer.@isView(G)||G instanceof @ArrayBuffer))@throwTypeError(\"Expected text, ArrayBuffer or ArrayBufferView\");const j=@toLength(G.byteLength);if(j>0)if(x=!0,C.length>0)@arrayPush(w,C,G),C=\"\";else @arrayPush(w,G);return z+=j,j},flush(){return 0},end(){if(F)return\"\";return v.fulfill()},fulfill(){F=!0;const G=v.finishInternal();return @fulfillPromise(E.@promise,G),G},finishInternal(){if(!A&&!x)return\"\";if(A&&!x)return C;if(x&&!A)return new globalThis.TextDecoder().decode(@Bun.concatArrayBuffers(w));var G=new @Bun.ArrayBufferSink;G.start({highWaterMark:z,asUint8Array:!0});for(let _ of w)G.write(_);if(w.length=0,C.length>0)G.write(C),C=\"\";return new globalThis.TextDecoder().decode(G.end())},close(){try{if(!F)F=!0,v.fulfill()}catch(G){}}},[v,E]})\n";
...
const char* const s_readableStreamInternalsReadableStreamFulfillReadRequestCode = "(function (i,y,_){\"use strict\";const p=@getByIdDirectPrivate(@getByIdDirectPrivate(i,\"reader\"),\"readRequests\").shift();@fulfillPromise(p,{value:y,done:_})})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayBufferDirectCode = "(function (_,z){\"use strict\";var A=new @Bun.ArrayBufferSink;@putByIdDirectPrivate(_,\"underlyingSource\",@undefined);var B=@getByIdDirectPrivate(_,\"highWaterMark\");A.start(B\?{highWaterMark:B}:{});var C=@newPromiseCapability(@Promise),O=!1,D=z.pull,j=z.close,v={start(){},close(F){if(!O){if(O=!0,j)j();@fulfillPromise(C.@promise,A.end())}},end(){if(!O){if(O=!0,j)j();@fulfillPromise(C.@promise,A.end())}},flush(){return 0},write:A.write.bind(A)},x=!1;try{const F=D(v);if(F&&@isObject(F)&&@isPromise(F))return async function(G,q,w){while(!O)await w(G);return await q}(v,promise,D);return C.@promise}catch(F){return x=!0,@readableStreamError(_,F),@Promise.@reject(F)}finally{if(!x&&_)@readableStreamClose(_);v=j=A=D=_=@undefined}})\n";
...
const char* const s_readableStreamInternalsReadableStreamDefineLazyIteratorsCode = "(function (g){\"use strict\";var i=globalThis.Symbol.asyncIterator,j=async function*z(B,D){var F=B.getReader(),G;try{while(!0){var h,k;const q=F.readMany();if(@isPromise(q))({done:h,value:k}=await q);else({done:h,value:k}=q);if(h)return;yield*k}}catch(q){G=q}finally{if(F.releaseLock(),!D)B.cancel(G);if(G)throw G}},w=function z(){return j(this,!1)},x=function z({preventCancel:B=!1}={preventCancel:!1}){return j(this,B)};return @Object.@defineProperty(g,i,{value:w}),@Object.@defineProperty(g,\"values\",{value:x}),g})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const x=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!x)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const B=@getByIdDirectPrivate(x,\"state\");if(@putByIdDirectPrivate(x,\"disturbed\",!0),B===@streamClosed)return{value:[],size:0,done:!0};else if(B===@streamErrored)throw @getByIdDirectPrivate(x,\"storedError\");var D=@getByIdDirectPrivate(x,\"readableStreamController\"),w=@getByIdDirectPrivate(D,\"queue\");if(!w)return D.@pull(D).@then(function({done:H,value:I}){return H\?{done:!0,value:[],size:0}:{value:[I],size:1,done:!1}});const A=w.content;var C=w.size,E=A.toArray(!1),F=E.length;if(F>0){var _=@newArrayWithSize(F);if(@isReadableByteStreamController(D)){{const H=E[0];if(!(@ArrayBuffer.@isView(H)||H instanceof @ArrayBuffer))@putByValDirect(_,0,new @Uint8Array(H.buffer,H.byteOffset,H.byteLength));else @putByValDirect(_,0,H)}for(var j=1;j<F;j++){const H=E[j];if(!(@ArrayBuffer.@isView(H)||H instanceof @ArrayBuffer))@putByValDirect(_,j,new @Uint8Array(H.buffer,H.byteOffset,H.byteLength));else @putByValDirect(_,j,H)}}else{@putByValDirect(_,0,E[0].value);for(var j=1;j<F;j++)@putByValDirect(_,j,E[j].value)}if(@resetQueue(@getByIdDirectPrivate(D,\"queue\")),@getByIdDirectPrivate(D,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(D,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(D))@readableStreamDefaultControllerCallPullIfNeeded(D);else if(@isReadableByteStreamController(D))@readableByteStreamControllerCallPullIfNeeded(D);return{value:_,size:C,done:!1}}var k=(H)=>{if(H.done)return{value:[],size:0,done:!0};var I=@getByIdDirectPrivate(x,\"readableStreamController\"),J=@getByIdDirectPrivate(I,\"queue\"),K=[H.value].concat(J.content.toArray(!1)),d=K.length;if(@isReadableByteStreamController(I))for(var N=0;N<d;N++){const S=K[N];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer)){const{buffer:T,byteOffset:U,byteLength:W}=S;@putByValDirect(K,N,new @Uint8Array(T,U,W))}}else for(var N=1;N<d;N++)@putByValDirect(K,N,K[N].value);var Q=J.size;if(@resetQueue(J),@getByIdDirectPrivate(I,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(I,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(I))@readableStreamDefaultControllerCallPullIfNeeded(I);else if(@isReadableByteStreamController(I))@readableByteStreamControllerCallPullIfNeeded(I);return{value:K,size:Q,done:!1}},G=D.@pull(D);if(G&&@isPromise(G))return G.@then(k);return k(G)})\n";
...
const char* const s_streamInternalsCreateFIFOCode = "(function (){\"use strict\";var b=@Array.prototype.slice;class x{constructor(){this._head=0,this._tail=0,this._capacityMask=3,this._list=@newArrayWithSize(4)}_head;_tail;_capacityMask;_list;size(){if(this._head===this._tail)return 0;if(this._head<this._tail)return this._tail-this._head;else return this._capacityMask+1-(this._head-this._tail)}isEmpty(){return this.size()==0}isNotEmpty(){return this.size()>0}shift(){var{_head:k,_tail:w,_list:v,_capacityMask:z}=this;if(k===w)return @undefined;var g=v[k];if(@putByValDirect(v,k,@undefined),k=this._head=k+1&z,k<2&&w>1e4&&w<=v.length>>>2)this._shrinkArray();return g}peek(){if(this._head===this._tail)return @undefined;return this._list[this._head]}push(k){var w=this._tail;if(@putByValDirect(this._list,w,k),this._tail=w+1&this._capacityMask,this._tail===this._head)this._growArray()}toArray(k){var w=this._list,v=@toLength(w.length);if(k||this._head>this._tail){var z=@toLength(this._head),g=@toLength(this._tail),A=@toLength(v-z+g),B=@newArrayWithSize(A),E=0;for(var F=z;F<v;F++)@putByValDirect(B,E++,w[F]);for(var F=0;F<g;F++)@putByValDirect(B,E++,w[F]);return B}else return b.@call(w,this._head,this._tail)}clear(){this._head=0,this._tail=0,this._list.fill(@undefined)}_growArray(){if(this._head)this._list=this.toArray(!0),this._head=0;this._tail=@toLength(this._list.length),this._list.length<<=1,this._capacityMask=this._capacityMask<<1|1}shrinkArray(){this._list.length>>>=1,this._capacityMask>>>=1}}return new x})\n";
...
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";var _=new Map,t={};return new Proxy(t,{get(u,f){const h=@requireMap.@get(f);if(h)return h;const L=@Loader.registry.@get(f);if(L\?.evaluated){const b=@Loader.getModuleNamespaceObject(L.module),c=b[@commonJSSymbol]===0||b.default\?.[@commonJSSymbol]\?b.default:b,g=@createCommonJSModule(f,c,!0);return @requireMap.@set(f,g),g}return t[f]},set(u,f,h){return @requireMap.@set(f,h),!0},has(u,f){return @requireMap.@has(f)||@Loader.registry.@has(f)},deleteProperty(u,f){return _.@delete(f),@requireMap.@delete(f),@Loader.registry.@delete(f),!0},ownKeys(u){var f=[...@requireMap.@keys()];const h=[...@Loader.registry.@keys()];for(let L of h)if(!f.includes(L))@arrayPush(f,L);return f},getPrototypeOf(u){return null},getOwnPropertyDescriptor(u,f){if(@requireMap.@has(f)||@Loader.registry.@has(f))return{configurable:!0,enumerable:!0}}})})\n";
...
const char* const s_readableStreamConsumeReadableStreamCode = "(function (q,D,H){\"use strict\";const I=globalThis.Symbol.for(\"Bun.consumeReadableStreamPrototype\");var F=globalThis[I];if(!F)F=globalThis[I]=[];var J=F[D];if(J===@undefined){var[j,K,L,w,G,_]=globalThis[globalThis.Symbol.for(\"Bun.lazy\")](D);J=class A{handleError;handleClosed;processResult;constructor(B,N){this.#_=N,this.#F=B,this.#$=!1,this.handleError=this._handleError.bind(this),this.handleClosed=this._handleClosed.bind(this),this.processResult=this._processResult.bind(this),B.closed.then(this.handleClosed,this.handleError)}_handleClosed(){if(this.#$)return;this.#$=!0;var B=this.#_;this.#_=0,w(B),_(B)}_handleError(B){if(this.#$)return;this.#$=!0;var N=this.#_;this.#_=0,K(N,B),_(N)}#_;#$=!1;#F;_handleReadMany({value:B,done:N,size:k}){if(N){this.handleClosed();return}if(this.#$)return;L(this.#_,B,N,k)}read(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.read())}_processResult(B){if(B&&@isPromise(B)){if(@getPromiseInternalField(B,@promiseFieldFlags)&@promiseStateFulfilled){const k=@getPromiseInternalField(B,@promiseFieldReactionsOrResult);if(k)B=k}}if(B&&@isPromise(B))return B.then(this.processResult,this.handleError),null;if(B.done)return this.handleClosed(),0;else if(B.value)return B.value;else return-1}readMany(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.readMany())}};const x=D+1;if(F.length<x)F.length=x;@putByValDirect(F,D,J)}if(@isReadableStreamLocked(H))@throwTypeError(\"Cannot start reading from a locked stream\");return new J(H.getReader(),q)})\n";
...
const char* const s_readableStreamGetReaderCode = "(function (e){\"use strict\";if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"getReader\");const b=@toDictionary(e,{},\"ReadableStream.getReader takes an object as first argument\").mode;if(b===@undefined){var n=@getByIdDirectPrivate(this,\"start\");if(n)@putByIdDirectPrivate(this,\"start\",@undefined),n();return new @ReadableStreamDefaultReader(this)}if(b==\"byob\")return new @ReadableStreamBYOBReader(this);@throwTypeError(\"Invalid mode is specified\")})\n";
...
const char* const s_readableStreamPipeThroughCode = "(function (_,c){\"use strict\";const j=_,d=j[\"readable\"];if(!@isReadableStream(d))throw @makeTypeError(\"readable should be ReadableStream\");const y=j[\"writable\"],h=@getInternalWritableStream(y);if(!@isWritableStream(h))throw @makeTypeError(\"writable should be WritableStream\");let k=!1,q=!1,x=!1,u;if(!@isUndefinedOrNull(c)){if(!@isObject(c))throw @makeTypeError(\"options must be an object\");if(q=!!c[\"preventAbort\"],x=!!c[\"preventCancel\"],k=!!c[\"preventClose\"],u=c[\"signal\"],u!==@undefined&&!@isAbortSignal(u))throw @makeTypeError(\"options.signal must be AbortSignal\")}if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"pipeThrough\");if(@isReadableStreamLocked(this))throw @makeTypeError(\"ReadableStream is locked\");if(@isWritableStreamLocked(h))throw @makeTypeError(\"WritableStream is locked\");return @readableStreamPipeToWritableStream(this,h,k,q,x,u),d})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullCode = "(function (_){\"use strict\";const d=@getByIdDirectPrivate(_,\"controlledReadableStream\");if(@assert(@readableStreamHasDefaultReader(d)),@getByIdDirectPrivate(_,\"queue\").content\?.isNotEmpty()){const F=@getByIdDirectPrivate(_,\"queue\").content.shift();@getByIdDirectPrivate(_,\"queue\").size-=F.byteLength,@readableByteStreamControllerHandleQueueDrain(_);let R;try{R=new @Uint8Array(F.buffer,F.byteOffset,F.byteLength)}catch(P){return @Promise.@reject(P)}return @createFulfilledPromise({value:R,done:!1})}if(@getByIdDirectPrivate(_,\"autoAllocateChunkSize\")!==@undefined){let F;try{F=@createUninitializedArrayBuffer(@getByIdDirectPrivate(_,\"autoAllocateChunkSize\"))}catch(P){return @Promise.@reject(P)}const R={buffer:F,byteOffset:0,byteLength:@getByIdDirectPrivate(_,\"autoAllocateChunkSize\"),bytesFilled:0,elementSize:1,ctor:@Uint8Array,readerType:\"default\"};@getByIdDirectPrivate(_,\"pendingPullIntos\").push(R)}const h=@readableStreamAddReadRequest(d);return @readableByteStreamControllerCallPullIfNeeded(_),h})\n";
