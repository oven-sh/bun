...
const char* const s_jsBufferPrototypeSliceCode = "(function (p,q){\"use strict\";var{buffer:m,byteOffset:v,byteLength:c}=this;function w(z,k){if(z=@trunc(z),z===0||@isNaN(z))return 0;else if(z<0)return z+=k,z>0\?z:0;else return z<k\?z:k}var i=w(p,c),x=q!==@undefined\?w(q,c):c;return new @Buffer(m,v+i,x>i\?x-i:0)})\n";
...
const char* const s_readableStreamPipeThroughCode = "(function (d,j){\"use strict\";const k=d,q=k[\"readable\"];if(!@isReadableStream(q))throw @makeTypeError(\"readable should be ReadableStream\");const u=k[\"writable\"],_=@getInternalWritableStream(u);if(!@isWritableStream(_))throw @makeTypeError(\"writable should be WritableStream\");let h=!1,c=!1,y=!1,x;if(!@isUndefinedOrNull(j)){if(!@isObject(j))throw @makeTypeError(\"options must be an object\");if(c=!!j[\"preventAbort\"],y=!!j[\"preventCancel\"],h=!!j[\"preventClose\"],x=j[\"signal\"],x!==@undefined&&!@isAbortSignal(x))throw @makeTypeError(\"options.signal must be AbortSignal\")}if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"pipeThrough\");if(@isReadableStreamLocked(this))throw @makeTypeError(\"ReadableStream is locked\");if(@isWritableStreamLocked(_))throw @makeTypeError(\"WritableStream is locked\");return @readableStreamPipeToWritableStream(this,_,h,c,y,x),q})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerFillDescriptorFromQueueCode = "(function (q,_){\"use strict\";const j=_.bytesFilled-_.bytesFilled%_.elementSize,v=@getByIdDirectPrivate(q,\"queue\").size<_.byteLength-_.bytesFilled\?@getByIdDirectPrivate(q,\"queue\").size:_.byteLength-_.bytesFilled,k=_.bytesFilled+v,w=k-k%_.elementSize;let z=v,E=!1;if(w>j)z=w-_.bytesFilled,E=!0;while(z>0){let G=@getByIdDirectPrivate(q,\"queue\").content.peek();const H=z<G.byteLength\?z:G.byteLength,J=_.byteOffset+_.bytesFilled;if(new @Uint8Array(_.buffer).set(new @Uint8Array(G.buffer,G.byteOffset,H),J),G.byteLength===H)@getByIdDirectPrivate(q,\"queue\").content.shift();else G.byteOffset+=H,G.byteLength-=H;@getByIdDirectPrivate(q,\"queue\").size-=H,@assert(@getByIdDirectPrivate(q,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(q,\"pendingPullIntos\").peek()===_),@readableByteStreamControllerInvalidateBYOBRequest(q),_.bytesFilled+=H,z-=H}if(!E)@assert(@getByIdDirectPrivate(q,\"queue\").size===0),@assert(_.bytesFilled>0),@assert(_.bytesFilled<_.elementSize);return E})\n";
