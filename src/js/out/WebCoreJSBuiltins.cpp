...
const char* const s_writableStreamInternalsSetUpWritableStreamDefaultControllerCode = "(function (B,U,d,f,j,v,_,q){\"use strict\";@assert(@isWritableStream(B)),@assert(@getByIdDirectPrivate(B,\"controller\")===@undefined),@putByIdDirectPrivate(U,\"stream\",B),@putByIdDirectPrivate(B,\"controller\",U),@resetQueue(@getByIdDirectPrivate(U,\"queue\")),@putByIdDirectPrivate(U,\"started\",-1),@putByIdDirectPrivate(U,\"startAlgorithm\",d),@putByIdDirectPrivate(U,\"strategySizeAlgorithm\",q),@putByIdDirectPrivate(U,\"strategyHWM\",_),@putByIdDirectPrivate(U,\"writeAlgorithm\",f),@putByIdDirectPrivate(U,\"closeAlgorithm\",j),@putByIdDirectPrivate(U,\"abortAlgorithm\",v);const P=@writableStreamDefaultControllerGetBackpressure(U);@writableStreamUpdateBackpressure(B,P),@writableStreamDefaultControllerStart(U)})\n";
...
const char* const s_transformStreamInternalsInitializeTransformStreamCode = "(function (D,I,E,J,F,K){\"use strict\";const G=()=>{return I},L=(x)=>{return @transformStreamDefaultSinkWriteAlgorithm(D,x)},N=(x)=>{return @transformStreamDefaultSinkAbortAlgorithm(D,x)},f=()=>{return @transformStreamDefaultSinkCloseAlgorithm(D)},q=@createWritableStream(G,L,f,N,E,J),T=()=>{return @transformStreamDefaultSourcePullAlgorithm(D)},j=(x)=>{return @transformStreamErrorWritableAndUnblockWrite(D,x),@Promise.@resolve()},B={};@putByIdDirectPrivate(B,\"start\",G),@putByIdDirectPrivate(B,\"pull\",T),@putByIdDirectPrivate(B,\"cancel\",j);const v={};@putByIdDirectPrivate(v,\"size\",K),@putByIdDirectPrivate(v,\"highWaterMark\",F);const C=new @ReadableStream(B,v);@putByIdDirectPrivate(D,\"writable\",q),@putByIdDirectPrivate(D,\"internalWritable\",@getInternalWritableStream(q)),@putByIdDirectPrivate(D,\"readable\",C),@putByIdDirectPrivate(D,\"backpressure\",@undefined),@putByIdDirectPrivate(D,\"backpressureChangePromise\",@undefined),@transformStreamSetBackpressure(D,!0),@putByIdDirectPrivate(D,\"controller\",@undefined)})\n";
...
const char* const s_transformStreamInternalsTransformStreamDefaultControllerEnqueueCode = "(function (W,f){\"use strict\";const i=@getByIdDirectPrivate(W,\"stream\"),_=@getByIdDirectPrivate(i,\"readable\"),S=@getByIdDirectPrivate(_,\"readableStreamController\");if(@assert(S!==@undefined),!@readableStreamDefaultControllerCanCloseOrEnqueue(S))@throwTypeError(\"TransformStream.readable cannot close or enqueue\");try{@readableStreamDefaultControllerEnqueue(S,f)}catch(g){throw @transformStreamErrorWritableAndUnblockWrite(i,g),@getByIdDirectPrivate(_,\"storedError\")}const j=!@readableStreamDefaultControllerShouldCallPull(S);if(j!==@getByIdDirectPrivate(i,\"backpressure\"))@assert(j),@transformStreamSetBackpressure(i,!0)})\n";
...
const char* const s_jsBufferPrototypeReadIntLECode = "(function (u,d){\"use strict\";const _=this.@dataView||=new DataView(this.buffer,this.byteOffset,this.byteLength);switch(d){case 1:return _.getInt8(u);case 2:return _.getInt16(u,!0);case 3:{const r=_.getUint16(u,!0)+_.getUint8(u+2)*65536;return r|(r&8388608)*510}case 4:return _.getInt32(u,!0);case 5:{const r=_.getUint8(u+4);return(r|(r&128)*33554430)*4294967296+_.getUint32(u,!0)}case 6:{const r=_.getUint16(u+4,!0);return(r|(r&32768)*131070)*4294967296+_.getUint32(u,!0)}}@throwRangeError(\"byteLength must be >= 1 and <= 6\")})\n";
...
const char* const s_consoleObjectAsyncIteratorCode = "(function (){\"use strict\";const j=async function*B(){var F=@Bun.stdin.stream().getReader(),G=new globalThis.TextDecoder(\"utf-8\",{fatal:!1}),H,J=@Bun.indexOfLine;try{while(!0){var m,K,w;const L=F.readMany();if(@isPromise(L))({done:m,value:K}=await L);else({done:m,value:K}=L);if(m){if(w)yield G.decode(w);return}var _;for(let M of K){if(_=M,w)_=@Buffer.concat([w,M]),w=null;var q=0,A=J(_,q);while(A!==-1)yield G.decode(_.subarray(q,A)),q=A+1,A=J(_,q);w=_.subarray(q)}}}catch(L){H=L}finally{if(F.releaseLock(),H)throw H}},z=globalThis.Symbol.asyncIterator;return this[z]=j,j()})\n";
...
const char* const s_readableStreamInternalsSetupReadableStreamDefaultControllerCode = "(function (b,q,v,w,_,x,f){\"use strict\";const B=new @ReadableStreamDefaultController(b,q,v,w,@isReadableStream),C=()=>@promiseInvokeOrNoopMethod(q,x,[B]),D=(j)=>@promiseInvokeOrNoopMethod(q,f,[j]);@putByIdDirectPrivate(B,\"pullAlgorithm\",C),@putByIdDirectPrivate(B,\"cancelAlgorithm\",D),@putByIdDirectPrivate(B,\"pull\",@readableStreamDefaultControllerPull),@putByIdDirectPrivate(B,\"cancel\",@readableStreamDefaultControllerCancel),@putByIdDirectPrivate(b,\"readableStreamController\",B),@readableStreamDefaultControllerStart(B)})\n";
...
const char* const s_readableStreamInternalsReadStreamIntoSinkCode = "(async function (_,I,P){\"use strict\";var z=!1,A=!1;try{var B=_.getReader(),D=B.readMany();if(D&&@isPromise(D))D=await D;if(D.done)return z=!0,I.end();var c=D.value.length;const J=@getByIdDirectPrivate(_,\"highWaterMark\");if(P)@startDirectStream.@call(I,_,@undefined,()=>!A&&@markPromiseAsHandled(_.cancel()));I.start({highWaterMark:J||0});for(var q=0,E=D.value,F=D.value.length;q<F;q++)I.write(E[q]);var G=@getByIdDirectPrivate(_,\"state\");if(G===@streamClosed)return z=!0,I.end();while(!0){var{value:H,done:f}=await B.read();if(f)return z=!0,I.end();I.write(H)}}catch(J){A=!0;try{B=@undefined;const K=_.cancel(J);@markPromiseAsHandled(K)}catch(K){}if(I&&!z){z=!0;try{I.close(J)}catch(K){throw new globalThis.AggregateError([J,K])}}throw J}finally{if(B){try{B.releaseLock()}catch(K){}B=@undefined}I=@undefined;var G=@getByIdDirectPrivate(_,\"state\");if(_){var x=@getByIdDirectPrivate(_,\"readableStreamController\");if(x){if(@getByIdDirectPrivate(x,\"underlyingSource\"))@putByIdDirectPrivate(x,\"underlyingSource\",@undefined);if(@getByIdDirectPrivate(x,\"controlledReadableStream\"))@putByIdDirectPrivate(x,\"controlledReadableStream\",@undefined);if(@putByIdDirectPrivate(_,\"readableStreamController\",null),@getByIdDirectPrivate(_,\"underlyingSource\"))@putByIdDirectPrivate(_,\"underlyingSource\",@undefined);x=@undefined}if(!A&&G!==@streamClosed&&G!==@streamErrored)@readableStreamClose(_);_=@undefined}}})\n";
...
const char* const s_readableStreamInternalsCreateTextStreamCode = "(function (E){\"use strict\";var G,F=[],A=!1,_=!1,v=\"\",z=@toLength(0),x=@newPromiseCapability(@Promise),w=!1;return G={start(){},write(q){if(typeof q===\"string\"){var C=@toLength(q.length);if(C>0)v+=q,A=!0,z+=C;return C}if(!q||!(@ArrayBuffer.@isView(q)||q instanceof @ArrayBuffer))@throwTypeError(\"Expected text, ArrayBuffer or ArrayBufferView\");const j=@toLength(q.byteLength);if(j>0)if(_=!0,v.length>0)@arrayPush(F,v,q),v=\"\";else @arrayPush(F,q);return z+=j,j},flush(){return 0},end(){if(w)return\"\";return G.fulfill()},fulfill(){w=!0;const q=G.finishInternal();return @fulfillPromise(x.@promise,q),q},finishInternal(){if(!A&&!_)return\"\";if(A&&!_)return v;if(_&&!A)return new globalThis.TextDecoder().decode(@Bun.concatArrayBuffers(F));var q=new @Bun.ArrayBufferSink;q.start({highWaterMark:z,asUint8Array:!0});for(let C of F)q.write(C);if(F.length=0,v.length>0)q.write(v),v=\"\";return new globalThis.TextDecoder().decode(q.end())},close(){try{if(!w)w=!0,G.fulfill()}catch(q){}}},[G,x]})\n";
...
const char* const s_readableStreamInternalsInitializeArrayStreamCode = "(function (_,t){\"use strict\";var b=[],p=@newPromiseCapability(@Promise),j=!1;function d(){return j=!0,p.@resolve.@call(@undefined,b),b}var m={start(){},write(v){return @arrayPush(b,v),v.byteLength||v.length},flush(){return 0},end(){if(j)return[];return d()},close(){if(!j)d()}},q={@underlyingSource:_,@pull:@onPullDirectStream,@controlledReadableStream:this,@sink:m,close:@onCloseDirectStream,write:m.write,error:@handleDirectStreamError,end:@onCloseDirectStream,@close:@onCloseDirectStream,flush:@onFlushDirectStream,_pendingRead:@undefined,_deferClose:0,_deferFlush:0,_deferCloseReason:@undefined,_handleError:@undefined};return @putByIdDirectPrivate(this,\"readableStreamController\",q),@putByIdDirectPrivate(this,\"underlyingSource\",@undefined),@putByIdDirectPrivate(this,\"start\",@undefined),p})\n";
...
const char* const s_readableStreamInternalsLazyLoadStreamCode = "(function (Z,H){\"use strict\";var L=@getByIdDirectPrivate(Z,\"bunNativeType\"),I=@getByIdDirectPrivate(Z,\"bunNativePtr\"),J=@lazyStreamPrototypeMap.@get(L);if(J===@undefined){let q=function(E){var{c:p,v:z}=this;this.c=@undefined,this.v=@undefined,P(E,p,z)},K=function(E){try{E.close()}catch(p){globalThis.reportError(p)}},_=function(E,p,z,A){A[0]=!1;var F;try{F=f(E,z,A)}catch(G){return p.error(G)}return P(F,p,z)};var m=q,Y=K,x=_,[f,b,M,X,j,N,O]=@lazyLoad(L),y=[!1],P;P=function E(p,z,A){if(p&&@isPromise(p))return p.then(q.bind({c:z,v:A}),(F)=>z.error(F));else if(typeof p===\"number\")if(A&&A.byteLength===p&&A.buffer===z.byobRequest\?.view\?.buffer)z.byobRequest.respondWithNewView(A);else z.byobRequest.respond(p);else if(p.constructor===@Uint8Array)z.enqueue(p);if(y[0]||p===!1)@enqueueJob(K,z),y[0]=!1};const D=j\?new FinalizationRegistry(j):null;J=class E{constructor(p,z,A){if(this.#f=p,this.#b={},this.pull=this.#j.bind(this),this.cancel=this.#m.bind(this),this.autoAllocateChunkSize=z,A!==@undefined)this.start=(F)=>{F.enqueue(A)};if(D)D.register(this,p,this.#b)}#b;pull;cancel;start;#f;type=\"bytes\";autoAllocateChunkSize=0;static startSync=b;#j(p){var z=this.#f;if(!z){p.close();return}_(z,p,p.byobRequest.view,y)}#m(p){var z=this.#f;D&&D.unregister(this.#b),N&&N(z,!1),M(z,p)}static deinit=j;static drain=O},@lazyStreamPrototypeMap.@set(L,J)}const Q=J.startSync(I,H);var U;const{drain:B,deinit:W}=J;if(B)U=B(I);if(Q===0){if(j&&I&&@enqueueJob(j,I),(U\?.byteLength\?\?0)>0)return{start(q){q.enqueue(U),q.close()},type:\"bytes\"};return{start(q){q.close()},type:\"bytes\"}}return new J(I,Q,U)})\n";
...
const char* const s_readableStreamInternalsReadableStreamIntoTextCode = "(function (i){\"use strict\";const[d,_]=@createTextStream(@getByIdDirectPrivate(i,\"highWaterMark\")),h=@readStreamIntoSink(i,d,!1);if(h&&@isPromise(h))return @Promise.@resolve(h).@then(_.@promise);return _.@promise})\n";
...
const char* const s_jsBufferConstructorFromCode = "(function (n){\"use strict\";if(@isUndefinedOrNull(n))@throwTypeError(\"The first argument must be one of type string, Buffer, ArrayBuffer, Array, or Array-like Object.\");if(typeof n===\"string\"||typeof n===\"object\"&&(@isTypedArrayView(n)||n instanceof @ArrayBuffer||n instanceof SharedArrayBuffer||n instanceof String))switch(@argumentCount()){case 1:return new @Buffer(n);case 2:return new @Buffer(n,@argument(1));default:return new @Buffer(n,@argument(1),@argument(2))}var u=@toObject(n,\"The first argument must be of type string or an instance of Buffer, ArrayBuffer, or Array or an Array-like Object.\");if(!@isJSArray(u)){const d=@tryGetByIdWithWellKnownSymbol(n,\"toPrimitive\");if(d){const f=d.@call(n,\"string\");if(typeof f===\"string\")switch(@argumentCount()){case 1:return new @Buffer(f);case 2:return new @Buffer(f,@argument(1));default:return new @Buffer(f,@argument(1),@argument(2))}}if(!(\"length\"in u)||@isCallable(u))@throwTypeError(\"The first argument must be of type string or an instance of Buffer, ArrayBuffer, or Array or an Array-like Object.\")}return new @Buffer(@Uint8Array.from(u).buffer)})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const w=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!w)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const D=@getByIdDirectPrivate(w,\"state\");if(@putByIdDirectPrivate(w,\"disturbed\",!0),D===@streamClosed)return{value:[],size:0,done:!0};else if(D===@streamErrored)throw @getByIdDirectPrivate(w,\"storedError\");var E=@getByIdDirectPrivate(w,\"readableStreamController\"),_=@getByIdDirectPrivate(E,\"queue\");if(!_)return E.@pull(E).@then(function({done:j,value:H}){return j\?{done:!0,value:[],size:0}:{value:[H],size:1,done:!1}});const K=_.content;var N=_.size,J=K.toArray(!1),F=J.length;if(F>0){var d=@newArrayWithSize(F);if(@isReadableByteStreamController(E)){{const j=J[0];if(!(@ArrayBuffer.@isView(j)||j instanceof @ArrayBuffer))@putByValDirect(d,0,new @Uint8Array(j.buffer,j.byteOffset,j.byteLength));else @putByValDirect(d,0,j)}for(var B=1;B<F;B++){const j=J[B];if(!(@ArrayBuffer.@isView(j)||j instanceof @ArrayBuffer))@putByValDirect(d,B,new @Uint8Array(j.buffer,j.byteOffset,j.byteLength));else @putByValDirect(d,B,j)}}else{@putByValDirect(d,0,J[0].value);for(var B=1;B<F;B++)@putByValDirect(d,B,J[B].value)}if(@resetQueue(@getByIdDirectPrivate(E,\"queue\")),@getByIdDirectPrivate(E,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(E,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(E))@readableStreamDefaultControllerCallPullIfNeeded(E);else if(@isReadableByteStreamController(E))@readableByteStreamControllerCallPullIfNeeded(E);return{value:d,size:N,done:!1}}var I=(j)=>{if(j.done)return{value:[],size:0,done:!0};var H=@getByIdDirectPrivate(w,\"readableStreamController\"),O=@getByIdDirectPrivate(H,\"queue\"),Q=[j.value].concat(O.content.toArray(!1)),k=Q.length;if(@isReadableByteStreamController(H))for(var S=0;S<k;S++){const T=Q[S];if(!(@ArrayBuffer.@isView(T)||T instanceof @ArrayBuffer)){const{buffer:U,byteOffset:x,byteLength:A}=T;@putByValDirect(Q,S,new @Uint8Array(U,x,A))}}else for(var S=1;S<k;S++)@putByValDirect(Q,S,Q[S].value);var C=O.size;if(@resetQueue(O),@getByIdDirectPrivate(H,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(H,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(H))@readableStreamDefaultControllerCallPullIfNeeded(H);else if(@isReadableByteStreamController(H))@readableByteStreamControllerCallPullIfNeeded(H);return{value:Q,size:C,done:!1}},G=E.@pull(E);if(G&&@isPromise(G))return G.@then(I);return I(G)})\n";
...
const char* const s_streamInternalsPromiseInvokeOrFallbackOrNoopCode = "(function (n,u,p,_,v){\"use strict\";try{const l=n[u];if(l===@undefined)return @promiseInvokeOrNoopNoCatch(n,_,v);return @shieldingPromiseResolve(l.@apply(n,p))}catch(l){return @Promise.@reject(l)}})\n";
...
const char* const s_streamInternalsValidateAndNormalizeQueuingStrategyCode = "(function (f,l){\"use strict\";if(f!==@undefined&&typeof f!==\"function\")@throwTypeError(\"size parameter must be a function\");const d=@toNumber(l);if(@isNaN(d)||d<0)@throwRangeError(\"highWaterMark value is negative or not a number\");return{size:f,highWaterMark:d}})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (_){\"use strict\";var Q=@Loader,w=@createFIFO(),D=_;while(D){var F=Q.registry.@get(D);if((F\?.state\?\?0)<=@ModuleFetch)@fulfillModuleSync(D),F=Q.registry.@get(D);var L=@getPromiseInternalField(F.fetch,@promiseFieldReactionsOrResult),T=Q.parseModule(D,L),U=F.module;if(T&&@isPromise(T)){var V=@getPromiseInternalField(T,@promiseFieldReactionsOrResult),J=@getPromiseInternalField(T,@promiseFieldFlags),W=J&@promiseStateMask;if(W===@promiseStatePending||V&&@isPromise(V))@throwTypeError(`require() async module \"${D}\" is unsupported. use \"await import()\" instead.`);else if(W===@promiseStateRejected){if(!V\?.message)@throwTypeError(`${V+\"\"\?V:\"An error occurred\"} occurred while parsing module \\\"${D}\\\"`);throw V}F.module=U=V}else if(T&&!U)F.module=U=T;@setStateToMax(F,@ModuleLink);var X=U.dependenciesMap,x=Q.requestedModules(U),G=@newArrayWithSize(x.length);for(var H=0,z=x.length;H<z;++H){var Y=x[H],Z=Y[0]===\"/\"\?Y:Q.resolve(Y,D),B=Q.ensureRegistered(Z);if(B.state<@ModuleLink)w.push(Z);@putByValDirect(G,H,B),X.@set(Y,B)}F.dependencies=G,F.instantiate=@Promise.resolve(F),F.satisfy=@Promise.resolve(F),D=w.shift();while(D&&(Q.registry.@get(D)\?.state\?\?@ModuleFetch)>=@ModuleLink)D=w.shift()}var I=Q.linkAndEvaluateModule(_,@undefined);if(I&&@isPromise(I))@throwTypeError(`require() async module \\\"${_}\\\" is unsupported. use \"await import()\" instead.`);return Q.registry.@get(_)})\n";
...
const char* const s_importMetaObjectInternalRequireCode = "(function (_){\"use strict\";var b=@requireMap.@get(_);const q=_.substring(_.length-5);if(b)return b.exports;if(q===\".json\"){var k=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),g=JSON.parse(k.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,g,!0)),g}else if(q===\".node\"){const v=@createCommonJSModule(_,{},!0);return process.dlopen(v,_),@requireMap.@set(_,v),v.exports}else if(q===\".toml\"){var k=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),g=@Bun.TOML.parse(k.readFileSync(_,\"utf8\"));return @requireMap.@set(_,@createCommonJSModule(_,g,!0)),g}else{var g=@requireESM(_);const n=@requireMap.@get(_);if(n)return n.exports;var j=g\?.default;if(j\?.[@commonJSSymbol]===0)g=j;return @requireMap.@set(_,@createCommonJSModule(_,g,!0)),g}})\n";
...
const char* const s_readableStreamConsumeReadableStreamCode = "(function (j,x,_){\"use strict\";const A=globalThis.Symbol.for(\"Bun.consumeReadableStreamPrototype\");var F=globalThis[A];if(!F)F=globalThis[A]=[];var B=F[x];if(B===@undefined){var[k,D,G,q,H,I]=globalThis[globalThis.Symbol.for(\"Bun.lazy\")](x);B=class K{handleError;handleClosed;processResult;constructor(L,N){this.#_=N,this.#F=L,this.#$=!1,this.handleError=this._handleError.bind(this),this.handleClosed=this._handleClosed.bind(this),this.processResult=this._processResult.bind(this),L.closed.then(this.handleClosed,this.handleError)}_handleClosed(){if(this.#$)return;this.#$=!0;var L=this.#_;this.#_=0,q(L),I(L)}_handleError(L){if(this.#$)return;this.#$=!0;var N=this.#_;this.#_=0,D(N,L),I(N)}#_;#$=!1;#F;_handleReadMany({value:L,done:N,size:w}){if(N){this.handleClosed();return}if(this.#$)return;G(this.#_,L,N,w)}read(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.read())}_processResult(L){if(L&&@isPromise(L)){if(@getPromiseInternalField(L,@promiseFieldFlags)&@promiseStateFulfilled){const w=@getPromiseInternalField(L,@promiseFieldReactionsOrResult);if(w)L=w}}if(L&&@isPromise(L))return L.then(this.processResult,this.handleError),null;if(L.done)return this.handleClosed(),0;else if(L.value)return L.value;else return-1}readMany(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.readMany())}};const J=x+1;if(F.length<J)F.length=J;@putByValDirect(F,x,B)}if(@isReadableStreamLocked(_))@throwTypeError(\"Cannot start reading from a locked stream\");return new B(_.getReader(),j)})\n";
...
const char* const s_readableStreamPipeThroughCode = "(function (d,_){\"use strict\";const u=d,c=u[\"readable\"];if(!@isReadableStream(c))throw @makeTypeError(\"readable should be ReadableStream\");const y=u[\"writable\"],j=@getInternalWritableStream(y);if(!@isWritableStream(j))throw @makeTypeError(\"writable should be WritableStream\");let h=!1,k=!1,q=!1,x;if(!@isUndefinedOrNull(_)){if(!@isObject(_))throw @makeTypeError(\"options must be an object\");if(k=!!_[\"preventAbort\"],q=!!_[\"preventCancel\"],h=!!_[\"preventClose\"],x=_[\"signal\"],x!==@undefined&&!@isAbortSignal(x))throw @makeTypeError(\"options.signal must be AbortSignal\")}if(!@isReadableStream(this))throw @makeThisTypeError(\"ReadableStream\",\"pipeThrough\");if(@isReadableStreamLocked(this))throw @makeTypeError(\"ReadableStream is locked\");if(@isWritableStreamLocked(j))throw @makeTypeError(\"WritableStream is locked\");return @readableStreamPipeToWritableStream(this,j,h,k,q,x),c})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullCode = "(function (_){\"use strict\";const P=@getByIdDirectPrivate(_,\"controlledReadableStream\");if(@assert(@readableStreamHasDefaultReader(P)),@getByIdDirectPrivate(_,\"queue\").content\?.isNotEmpty()){const F=@getByIdDirectPrivate(_,\"queue\").content.shift();@getByIdDirectPrivate(_,\"queue\").size-=F.byteLength,@readableByteStreamControllerHandleQueueDrain(_);let h;try{h=new @Uint8Array(F.buffer,F.byteOffset,F.byteLength)}catch(d){return @Promise.@reject(d)}return @createFulfilledPromise({value:h,done:!1})}if(@getByIdDirectPrivate(_,\"autoAllocateChunkSize\")!==@undefined){let F;try{F=@createUninitializedArrayBuffer(@getByIdDirectPrivate(_,\"autoAllocateChunkSize\"))}catch(d){return @Promise.@reject(d)}const h={buffer:F,byteOffset:0,byteLength:@getByIdDirectPrivate(_,\"autoAllocateChunkSize\"),bytesFilled:0,elementSize:1,ctor:@Uint8Array,readerType:\"default\"};@getByIdDirectPrivate(_,\"pendingPullIntos\").push(h)}const R=@readableStreamAddReadRequest(P);return @readableByteStreamControllerCallPullIfNeeded(_),R})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerRespondInternalCode = "(function (u,d){\"use strict\";let _=@getByIdDirectPrivate(u,\"pendingPullIntos\").peek(),k=@getByIdDirectPrivate(u,\"controlledReadableStream\");if(@getByIdDirectPrivate(k,\"state\")===@streamClosed){if(d!==0)@throwTypeError(\"bytesWritten is different from 0 even though stream is closed\");@readableByteStreamControllerRespondInClosedState(u,_)}else @assert(@getByIdDirectPrivate(k,\"state\")===@streamReadable),@readableByteStreamControllerRespondInReadableState(u,d,_)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerFillDescriptorFromQueueCode = "(function (q,v){\"use strict\";const z=v.bytesFilled-v.bytesFilled%v.elementSize,E=@getByIdDirectPrivate(q,\"queue\").size<v.byteLength-v.bytesFilled\?@getByIdDirectPrivate(q,\"queue\").size:v.byteLength-v.bytesFilled,j=v.bytesFilled+E,G=j-j%v.elementSize;let _=E,H=!1;if(G>z)_=G-v.bytesFilled,H=!0;while(_>0){let k=@getByIdDirectPrivate(q,\"queue\").content.peek();const w=_<k.byteLength\?_:k.byteLength,J=v.byteOffset+v.bytesFilled;if(new @Uint8Array(v.buffer).set(new @Uint8Array(k.buffer,k.byteOffset,w),J),k.byteLength===w)@getByIdDirectPrivate(q,\"queue\").content.shift();else k.byteOffset+=w,k.byteLength-=w;@getByIdDirectPrivate(q,\"queue\").size-=w,@assert(@getByIdDirectPrivate(q,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(q,\"pendingPullIntos\").peek()===v),@readableByteStreamControllerInvalidateBYOBRequest(q),v.bytesFilled+=w,_-=w}if(!H)@assert(@getByIdDirectPrivate(q,\"queue\").size===0),@assert(v.bytesFilled>0),@assert(v.bytesFilled<v.elementSize);return H})\n";
...
const char* const s_readableByteStreamInternalsReadableStreamFulfillReadIntoRequestCode = "(function (_,i,l){\"use strict\";const p=@getByIdDirectPrivate(@getByIdDirectPrivate(_,\"reader\"),\"readIntoRequests\").shift();@fulfillPromise(p,{value:i,done:l})})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class F extends EventTarget{#$;#j;#w;#A;#B;#F=!1;#G=null;#J=\"\";#K=\"\";#L=\"\";#M=!0;#O=0;#Q=0;#U=0;#V=null;static#W(M){M.#H()}static#X(M,j){const A=M.data,G=A.#L\?`Last-Event-ID: ${A.#L}\\r\\n`:\"\",K=`GET ${j.pathname}${j.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${G}\\r\\n`,O=M.write(K);if(O!==K.length)A.#K=K.substring(O)}static#Y(M,j,A){for(;;){if(A>=j.length)return;let G=-1,K=j.indexOf(\"\\r\\n\",A);const O=K+2;if(K>0)if(M.#O===0){const X=parseInt(j.substring(A,K),16);if(X===0){M.#j=2,M.#G\?.end();return}G=O+X}else G=j.length;else{if(M.#J.length===0){M.#J+=j.substring(A);return}G=j.length}let Q=j.substring(O,G);A=G+2;let U=0,V=Q.indexOf(\"\\n\\n\");if(V==-1){M.#J+=j.substring(O);return}if(M.#J.length)M.#J+=Q,Q=M.#J,M.#J=\"\";let W=!0;while(W){const X=Q.substring(U,V);let J,w=\"\",B,Y=0,L=-1;for(;;){let z=X.indexOf(\"\\n\",Y);if(z===-1){if(Y>=X.length)break;z=X.length}const H=X.substring(Y,z);if(H.startsWith(\"data:\"))if(w.length)w+=`\\n${H.substring(5).trim()}`;else w=H.substring(5).trim();else if(H.startsWith(\"event:\"))J=H.substring(6).trim();else if(H.startsWith(\"id:\"))B=H.substring(3).trim();else if(H.startsWith(\"retry:\")){if(L=parseInt(H.substring(6).trim(),10),@isNaN(L))L=-1}Y=z+1}if(M.#L=B||\"\",L>=0)M.#U=L;if(w||B||J)M.dispatchEvent(new MessageEvent(J||\"message\",{data:w||\"\",origin:M.#$.origin,source:M,lastEventId:B}));if(Q.length===V+2){W=!1;break}const Z=Q.indexOf(\"\\n\\n\",V+1);if(Z===-1)break;U=V,V=Z}}}static#Z={open(M){const j=M.data;if(j.#G=M,!j.#F)F.#X(M,j.#$)},handshake(M,j,A){const G=M.data;if(j)F.#X(M,G.#$);else G.#j=2,G.dispatchEvent(new ErrorEvent(\"error\",{error:A})),M.end()},data(M,j){const A=M.data;switch(A.#j){case 0:{let G=j.toString();const K=G.indexOf(\"\\r\\n\\r\\n\");if(K===-1){A.#J+=G;return}if(A.#J.length)A.#J+=G,G=A.#J,A.#J=\"\";const O=G.substring(0,K),Q=O.indexOf(\"\\r\\n\");if(Q===-1){A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),M.end();return}const U=O.substring(0,Q);if(U!==\"HTTP/1.1 200 OK\"){A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(U)})),M.end();return}let V=Q+1,W=!1,X=-1;for(;;){let w=O.indexOf(\"\\r\\n\",V);if(w===-1){if(V>=O.length){if(!W)A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),M.end();return}w=O.length}const B=O.substring(V+1,w),Y=B.indexOf(\":\"),L=B.substring(0,Y),Z=L.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(V=w+1,Z)if(B.endsWith(\" text/event-stream\"))W=!0;else{A.#j=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),M.end();return}else if(L.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(X=parseInt(B.substring(Y+1).trim(),10),@isNaN(X)||X<=0){A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),M.end();return}if(W)break}else if(L.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(B.substring(Y+1).trim()!==\"chunked\"){A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),M.end();return}if(X=0,W)break}}A.#O=X,A.#j=1,A.dispatchEvent(new Event(\"open\"));const J=G.substring(K+4);if(F.#Y(A,J,0),A.#O>0){if(A.#Q+=J.length,A.#Q>=A.#O)A.#j=2,M.end()}return}case 1:if(F.#Y(A,j.toString(),2),A.#O>0){if(A.#Q+=j.byteLength,A.#Q>=A.#O)A.#j=2,M.end()}return;default:break}},drain(M){const j=M.data;if(j.#j===0){const A=j.#J;if(A.length){const G=M.write(A);if(G!==A.length)M.data.#K=A.substring(G);else M.data.#K=\"\"}}},close:F.#z,end(M){F.#z(M).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(M){F.#z(M).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#z(M){const j=M.data;if(j.#G=null,j.#Q=0,j.#j=2,j.#M){if(j.#V)clearTimeout(j.#V);j.#V=setTimeout(F.#W,j.#U,j)}return j}constructor(M,j=@undefined){super();const A=new URL(M);this.#F=A.protocol===\"https:\",this.#$=A,this.#j=2,process.nextTick(F.#W,this)}ref(){this.#V\?.ref(),this.#G\?.ref()}unref(){this.#V\?.unref(),this.#G\?.unref()}#H(){if(this.#j!==2)return;const M=this.#$,j=this.#F;this.#j=0,@Bun.connect({data:this,socket:F.#Z,hostname:M.hostname,port:parseInt(M.port||(j\?\"443\":\"80\"),10),tls:j\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((A)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:A})),this.#M){if(this.#V)this.#V.unref\?.();this.#V=setTimeout(F.#W,1000,this)}})}get url(){return this.#$.href}get readyState(){return this.#j}close(){this.#M=!1,this.#j=2,this.#G\?.unref(),this.#G\?.end()}get onopen(){return this.#B}get onerror(){return this.#w}get onmessage(){return this.#A}set onopen(M){if(this.#B)super.removeEventListener(\"close\",this.#B);super.addEventListener(\"open\",M),this.#B=M}set onerror(M){if(this.#w)super.removeEventListener(\"error\",this.#w);super.addEventListener(\"error\",M),this.#w=M}set onmessage(M){if(this.#A)super.removeEventListener(\"message\",this.#A);super.addEventListener(\"message\",M),this.#A=M}}return Object.defineProperty(F.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(F.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(F.prototype,\"CLOSED\",{enumerable:!0,value:2}),F[Symbol.for(\"CommonJS\")]=0,F})\n";
