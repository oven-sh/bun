...
const char* const s_writableStreamInternalsWritableStreamDefaultControllerWriteCode = "(function (d,y,D){\"use strict\";try{@enqueueValueWithSize(@getByIdDirectPrivate(d,\"queue\"),y,D);const B=@getByIdDirectPrivate(d,\"stream\"),I=@getByIdDirectPrivate(B,\"state\");if(!@writableStreamCloseQueuedOrInFlight(B)&&I===\"writable\"){const _=@writableStreamDefaultControllerGetBackpressure(d);@writableStreamUpdateBackpressure(B,_)}@writableStreamDefaultControllerAdvanceQueueIfNeeded(d)}catch(B){@writableStreamDefaultControllerErrorIfNeeded(d,B)}})\n";
...
const int s_moduleRequireCodeLength = 879;
static const JSC::Intrinsic s_moduleRequireCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_moduleRequireCode = "(function (_){\"use strict\";const M=@requireMap.@get(_)||@requireMap.@get(_=@resolveSync(_,this.path,!1));if(M)return M.exports;if(_.endsWith(\".json\")||_.endsWith(\".toml\")||_.endsWith(\".node\"))return @internalRequire(_);let f=@Loader.registry.@get(_);if((f\?.state\?\?0)>=@ModuleLink){const r=f.module,h=@Loader.getModuleNamespaceObject(r),y=h\?.[@commonJSSymbol]===0||h\?.default\?.[@commonJSSymbol]===0\?h.default:h;return @requireMap.@set(_,@createCommonJSModule(_,y)),y}let l=this.@require(_);if(l===-1){const r=@createCommonJSModule(_,{});@requireMap.@set(_,r);try{l=@requireESM(_)}catch(h){throw @requireMap.@delete(_),h}if(f=@Loader.registry.@get(_),(f\?.state\?\?0)>=@ModuleLink){const h=@Loader.getModuleNamespaceObject(f.module);return r.exports=h\?.[@commonJSSymbol]===0||h\?.default\?.[@commonJSSymbol]===0\?h.default:h}}const q=@requireMap.@get(_);if(q)return q.exports;return l})\n";
...
const char* const s_readableStreamInternalsReadableStreamDefaultControllerErrorCode = "(function (u,d){\"use strict\";const i=@getByIdDirectPrivate(u,\"controlledReadableStream\");if(@getByIdDirectPrivate(i,\"state\")!==@streamReadable)return;@putByIdDirectPrivate(u,\"queue\",@newQueue()),@readableStreamError(i,d)})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToCode = "(function (_,g){\"use strict\";@assert(@isReadableStream(_));const h=new @ReadableStreamDefaultReader(_);@getByIdDirectPrivate(h,\"closedPromiseCapability\").@promise.@then(()=>{},(y)=>{g.error(y)});function c(){@readableStreamDefaultReaderRead(h).@then(function(y){if(y.done){g.close();return}try{g.enqueue(y.value)}catch(S){g.error(\"ReadableStream chunk enqueueing in the sink failed\");return}c()},function(y){g.error(y)})}c()})\n";
...
const char* const s_readableStreamInternalsSetupReadableStreamDefaultControllerCode = "(function (_,x,f,B,b,C,j){\"use strict\";const D=new @ReadableStreamDefaultController(_,x,f,B,@isReadableStream),q=()=>@promiseInvokeOrNoopMethod(x,C,[D]),v=(w)=>@promiseInvokeOrNoopMethod(x,j,[w]);@putByIdDirectPrivate(D,\"pullAlgorithm\",q),@putByIdDirectPrivate(D,\"cancelAlgorithm\",v),@putByIdDirectPrivate(D,\"pull\",@readableStreamDefaultControllerPull),@putByIdDirectPrivate(D,\"cancel\",@readableStreamDefaultControllerCancel),@putByIdDirectPrivate(_,\"readableStreamController\",D),@readableStreamDefaultControllerStart(D)})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToWritableStreamCode = "(function (_,f,B,D,F,E){\"use strict\";if(@assert(@isReadableStream(_)),@assert(@isWritableStream(f)),@assert(!@isReadableStreamLocked(_)),@assert(!@isWritableStreamLocked(f)),@assert(E===@undefined||@isAbortSignal(E)),@getByIdDirectPrivate(_,\"underlyingByteSource\")!==@undefined)return @Promise.@reject(\"Piping to a readable bytestream is not supported\");let G={source:_,destination:f,preventAbort:D,preventCancel:F,preventClose:B,signal:E};if(G.reader=@acquireReadableStreamDefaultReader(_),G.writer=@acquireWritableStreamDefaultWriter(f),@putByIdDirectPrivate(_,\"disturbed\",!0),G.finalized=!1,G.shuttingDown=!1,G.promiseCapability=@newPromiseCapability(@Promise),G.pendingReadPromiseCapability=@newPromiseCapability(@Promise),G.pendingReadPromiseCapability.@resolve.@call(),G.pendingWritePromise=@Promise.@resolve(),E!==@undefined){const T=(H)=>{if(G.finalized)return;@pipeToShutdownWithAction(G,()=>{const I=!G.preventAbort&&@getByIdDirectPrivate(G.destination,\"state\")===\"writable\"\?@writableStreamAbort(G.destination,H):@Promise.@resolve(),J=!G.preventCancel&&@getByIdDirectPrivate(G.source,\"state\")===@streamReadable\?@readableStreamCancel(G.source,H):@Promise.@resolve();let w=@newPromiseCapability(@Promise),K=!0,x=()=>{if(K){K=!1;return}w.@resolve.@call()},L=(z)=>{w.@reject.@call(@undefined,z)};return I.@then(x,L),J.@then(x,L),w.@promise},H)};if(@whenSignalAborted(E,T))return G.promiseCapability.@promise}return @pipeToErrorsMustBePropagatedForward(G),@pipeToErrorsMustBePropagatedBackward(G),@pipeToClosingMustBePropagatedForward(G),@pipeToClosingMustBePropagatedBackward(G),@pipeToLoop(G),G.promiseCapability.@promise})\n";
...
const char* const s_readableStreamInternalsReadStreamIntoSinkCode = "(async function (f,A,I){\"use strict\";var B=!1,D=!1;try{var E=f.getReader(),F=E.readMany();if(F&&@isPromise(F))F=await F;if(F.done)return B=!0,A.end();var G=F.value.length;const P=@getByIdDirectPrivate(f,\"highWaterMark\");if(I)@startDirectStream.@call(A,f,@undefined,()=>!D&&@markPromiseAsHandled(f.cancel()));A.start({highWaterMark:P||0});for(var H=0,J=F.value,_=F.value.length;H<_;H++)A.write(J[H]);var c=@getByIdDirectPrivate(f,\"state\");if(c===@streamClosed)return B=!0,A.end();while(!0){var{value:K,done:q}=await E.read();if(q)return B=!0,A.end();A.write(K)}}catch(P){D=!0;try{E=@undefined;const z=f.cancel(P);@markPromiseAsHandled(z)}catch(z){}if(A&&!B){B=!0;try{A.close(P)}catch(z){throw new globalThis.AggregateError([P,z])}}throw P}finally{if(E){try{E.releaseLock()}catch(z){}E=@undefined}A=@undefined;var c=@getByIdDirectPrivate(f,\"state\");if(f){var x=@getByIdDirectPrivate(f,\"readableStreamController\");if(x){if(@getByIdDirectPrivate(x,\"underlyingSource\"))@putByIdDirectPrivate(x,\"underlyingSource\",@undefined);if(@getByIdDirectPrivate(x,\"controlledReadableStream\"))@putByIdDirectPrivate(x,\"controlledReadableStream\",@undefined);if(@putByIdDirectPrivate(f,\"readableStreamController\",null),@getByIdDirectPrivate(f,\"underlyingSource\"))@putByIdDirectPrivate(f,\"underlyingSource\",@undefined);x=@undefined}if(!D&&c!==@streamClosed&&c!==@streamErrored)@readableStreamClose(f);f=@undefined}}})\n";
...
const char* const s_readableStreamInternalsCreateTextStreamCode = "(function (v){\"use strict\";var x,A=[],C=!1,j=!1,E=\"\",q=@toLength(0),F=@newPromiseCapability(@Promise),G=!1;return x={start(){},write(w){if(typeof w===\"string\"){var z=@toLength(w.length);if(z>0)E+=w,C=!0,q+=z;return z}if(!w||!(@ArrayBuffer.@isView(w)||w instanceof @ArrayBuffer))@throwTypeError(\"Expected text, ArrayBuffer or ArrayBufferView\");const _=@toLength(w.byteLength);if(_>0)if(j=!0,E.length>0)@arrayPush(A,E,w),E=\"\";else @arrayPush(A,w);return q+=_,_},flush(){return 0},end(){if(G)return\"\";return x.fulfill()},fulfill(){G=!0;const w=x.finishInternal();return @fulfillPromise(F.@promise,w),w},finishInternal(){if(!C&&!j)return\"\";if(C&&!j)return E;if(j&&!C)return new globalThis.TextDecoder().decode(@Bun.concatArrayBuffers(A));var w=new @Bun.ArrayBufferSink;w.start({highWaterMark:q,asUint8Array:!0});for(let z of A)w.write(z);if(A.length=0,E.length>0)w.write(E),E=\"\";return new globalThis.TextDecoder().decode(w.end())},close(){try{if(!G)G=!0,x.fulfill()}catch(w){}}},[x,F]})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayBufferDirectCode = "(function (O,j){\"use strict\";var w=new @Bun.ArrayBufferSink;@putByIdDirectPrivate(O,\"underlyingSource\",@undefined);var v=@getByIdDirectPrivate(O,\"highWaterMark\");w.start(v\?{highWaterMark:v}:{});var q=@newPromiseCapability(@Promise),x=!1,_=j.pull,z=j.close,A={start(){},close(C){if(!x){if(x=!0,z)z();@fulfillPromise(q.@promise,w.end())}},end(){if(!x){if(x=!0,z)z();@fulfillPromise(q.@promise,w.end())}},flush(){return 0},write:w.write.bind(w)},B=!1;try{const C=_(A);if(C&&@isObject(C)&&@isPromise(C))return async function(D,F,G){while(!x)await G(D);return await F}(A,promise,_);return q.@promise}catch(C){return B=!0,@readableStreamError(O,C),@Promise.@reject(C)}finally{if(!B&&O)@readableStreamClose(O);A=z=w=_=O=@undefined}})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const Q=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!Q)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const C=@getByIdDirectPrivate(Q,\"state\");if(@putByIdDirectPrivate(Q,\"disturbed\",!0),C===@streamClosed)return{value:[],size:0,done:!0};else if(C===@streamErrored)throw @getByIdDirectPrivate(Q,\"storedError\");var E=@getByIdDirectPrivate(Q,\"readableStreamController\"),F=@getByIdDirectPrivate(E,\"queue\");if(!F)return E.@pull(E).@then(function({done:J,value:d}){return J\?{done:!0,value:[],size:0}:{value:[d],size:1,done:!1}});const G=F.content;var _=F.size,D=G.toArray(!1),S=D.length;if(S>0){var j=@newArrayWithSize(S);if(@isReadableByteStreamController(E)){{const J=D[0];if(!(@ArrayBuffer.@isView(J)||J instanceof @ArrayBuffer))@putByValDirect(j,0,new @Uint8Array(J.buffer,J.byteOffset,J.byteLength));else @putByValDirect(j,0,J)}for(var I=1;I<S;I++){const J=D[I];if(!(@ArrayBuffer.@isView(J)||J instanceof @ArrayBuffer))@putByValDirect(j,I,new @Uint8Array(J.buffer,J.byteOffset,J.byteLength));else @putByValDirect(j,I,J)}}else{@putByValDirect(j,0,D[0].value);for(var I=1;I<S;I++)@putByValDirect(j,I,D[I].value)}if(@resetQueue(@getByIdDirectPrivate(E,\"queue\")),@getByIdDirectPrivate(E,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(E,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(E))@readableStreamDefaultControllerCallPullIfNeeded(E);else if(@isReadableByteStreamController(E))@readableByteStreamControllerCallPullIfNeeded(E);return{value:j,size:_,done:!1}}var T=(J)=>{if(J.done)return{value:[],size:0,done:!0};var d=@getByIdDirectPrivate(Q,\"readableStreamController\"),k=@getByIdDirectPrivate(d,\"queue\"),W=[J.value].concat(k.content.toArray(!1)),x=W.length;if(@isReadableByteStreamController(d))for(var H=0;H<x;H++){const B=W[H];if(!(@ArrayBuffer.@isView(B)||B instanceof @ArrayBuffer)){const{buffer:K,byteOffset:w,byteLength:N}=B;@putByValDirect(W,H,new @Uint8Array(K,w,N))}}else for(var H=1;H<x;H++)@putByValDirect(W,H,W[H].value);var A=k.size;if(@resetQueue(k),@getByIdDirectPrivate(d,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(d,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(d))@readableStreamDefaultControllerCallPullIfNeeded(d);else if(@isReadableByteStreamController(d))@readableByteStreamControllerCallPullIfNeeded(d);return{value:W,size:A,done:!1}},U=E.@pull(E);if(U&&@isPromise(U))return U.@then(T);return T(U)})\n";
...
const int s_importMetaObjectRequireESMCodeLength = 327;
static const JSC::Intrinsic s_importMetaObjectRequireESMCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_importMetaObjectRequireESMCode = "(function (u,_){\"use strict\";var a=@Loader.registry.@get(u);if(!a||!a.evaluated)a=@loadCJS2ESM(u);if(!a||!a.evaluated||!a.module)@throwTypeError(`require() failed to evaluate module \"${u}\". This is an internal consistentency error.`);var b=@Loader.getModuleNamespaceObject(a.module);if(b[@commonJSSymbol]===0)return;return b})\n";
...
const char* const s_importMetaObjectInternalRequireCode = "(function (F){\"use strict\";var M=@requireMap.@get(F);const _=F.substring(F.length-5);if(M)return M.exports;if(_===\".json\"){var b=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),g=JSON.parse(b.readFileSync(F,\"utf8\"));return @requireMap.@set(F,@createCommonJSModule(F,g)),g}else if(_===\".node\"){const n=@createCommonJSModule(F,{});return process.dlopen(n,F),@requireMap.@set(F,n),n.exports}else if(_===\".toml\"){var b=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),g=@Bun.TOML.parse(b.readFileSync(F,\"utf8\"));return @requireMap.@set(F,@createCommonJSModule(F,g)),g}else{var g=@requireESM(F);const j=@requireMap.@get(F);if(j)return j.exports;return @requireMap.@set(F,@createCommonJSModule(F,g)),g}})\n";
...
const int s_importMetaObjectCreateRequireCacheCodeLength = 1208;
static const JSC::Intrinsic s_importMetaObjectCreateRequireCacheCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";class _{id;parent;filename;children=[];paths=[];constructor(q){this.id=q;const P=q.lastIndexOf(\"/\");if(P!==-1&&q.length>P+1)this.filename=q.substring(P+1);else this.filename=q}get loaded(){return!0}require(q){return @internalRequire(@resolveSync(q,this.id))}get exports(){return @requireMap.@get(this.id)\?\?{}}set exports(q){@requireMap.@set(this.id,q)}}var d=new Map;return new Proxy({},{get(q,P){const b=@requireMap.@get(P);if(b)return b;const j=@Loader.registry.@get(P);if(j&&j.evaluated){const w=@Loader.getModuleNamespaceObject(j.module),z=w[@commonJSSymbol]===0||w.default\?.[@commonJSSymbol]\?w.default:w,A=@createCommonJSModule(P,z);return @requireMap.@set(P,A),A}return @undefined},set(q,P,b){return @requireMap.@set(P,b),!0},has(q,P){return @requireMap.@has(P)||@Loader.registry.@has(P)},deleteProperty(q,P){return d.@delete(P),@requireMap.@delete(P),@Loader.registry.@delete(P),!0},ownKeys(q){var P=[...@requireMap.@keys()];const b=[...@Loader.registry.@keys()];for(let j of b)if(!P.includes(j))@arrayPush(P,j);return P},getPrototypeOf(q){return null},getOwnPropertyDescriptor(q,P){if(@requireMap.@has(P)||@Loader.registry.@has(P))return{configurable:!0,enumerable:!0}}})})\n";
...
const char* const s_readableStreamConsumeReadableStreamCode = "(function (_,D,w){\"use strict\";const F=globalThis.Symbol.for(\"Bun.consumeReadableStreamPrototype\");var x=globalThis[F];if(!x)x=globalThis[F]=[];var G=x[D];if(G===@undefined){var[I,J,K,j,L,N]=globalThis[globalThis.Symbol.for(\"Bun.lazy\")](D);G=class A{handleError;handleClosed;processResult;constructor(B,q){this.#_=q,this.#F=B,this.#$=!1,this.handleError=this._handleError.bind(this),this.handleClosed=this._handleClosed.bind(this),this.processResult=this._processResult.bind(this),B.closed.then(this.handleClosed,this.handleError)}_handleClosed(){if(this.#$)return;this.#$=!0;var B=this.#_;this.#_=0,j(B),N(B)}_handleError(B){if(this.#$)return;this.#$=!0;var q=this.#_;this.#_=0,J(q,B),N(q)}#_;#$=!1;#F;_handleReadMany({value:B,done:q,size:H}){if(q){this.handleClosed();return}if(this.#$)return;K(this.#_,B,q,H)}read(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.read())}_processResult(B){if(B&&@isPromise(B)){if(@getPromiseInternalField(B,@promiseFieldFlags)&@promiseStateFulfilled){const H=@getPromiseInternalField(B,@promiseFieldReactionsOrResult);if(H)B=H}}if(B&&@isPromise(B))return B.then(this.processResult,this.handleError),null;if(B.done)return this.handleClosed(),0;else if(B.value)return B.value;else return-1}readMany(){if(!this.#_)return @throwTypeError(\"ReadableStreamSink is already closed\");return this.processResult(this.#F.readMany())}};const k=D+1;if(x.length<k)x.length=k;@putByValDirect(x,D,G)}if(@isReadableStreamLocked(w))@throwTypeError(\"Cannot start reading from a locked stream\");return new G(w.getReader(),_)})\n";
...
const char* const s_readableStreamPipeToCode = "(function (m){\"use strict\";if(!@isReadableStream(this))return @Promise.@reject(@makeThisTypeError(\"ReadableStream\",\"pipeTo\"));if(@isReadableStreamLocked(this))return @Promise.@reject(@makeTypeError(\"ReadableStream is locked\"));let _=@argument(1),R=!1,j=!1,S=!1,b;if(!@isUndefinedOrNull(_)){if(!@isObject(_))return @Promise.@reject(@makeTypeError(\"options must be an object\"));try{j=!!_[\"preventAbort\"],S=!!_[\"preventCancel\"],R=!!_[\"preventClose\"],b=_[\"signal\"]}catch(E){return @Promise.@reject(E)}if(b!==@undefined&&!@isAbortSignal(b))return @Promise.@reject(@makeTypeError(\"options.signal must be AbortSignal\"))}const u=@getInternalWritableStream(m);if(!@isWritableStream(u))return @Promise.@reject(@makeTypeError(\"ReadableStream pipeTo requires a WritableStream\"));if(@isWritableStreamLocked(u))return @Promise.@reject(@makeTypeError(\"WritableStream is locked\"));return @readableStreamPipeToWritableStream(this,u,R,j,S,b)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerFillDescriptorFromQueueCode = "(function (_,q){\"use strict\";const j=q.bytesFilled-q.bytesFilled%q.elementSize,k=@getByIdDirectPrivate(_,\"queue\").size<q.byteLength-q.bytesFilled\?@getByIdDirectPrivate(_,\"queue\").size:q.byteLength-q.bytesFilled,z=q.bytesFilled+k,v=z-z%q.elementSize;let E=k,G=!1;if(v>j)E=v-q.bytesFilled,G=!0;while(E>0){let H=@getByIdDirectPrivate(_,\"queue\").content.peek();const w=E<H.byteLength\?E:H.byteLength,J=q.byteOffset+q.bytesFilled;if(new @Uint8Array(q.buffer).set(new @Uint8Array(H.buffer,H.byteOffset,w),J),H.byteLength===w)@getByIdDirectPrivate(_,\"queue\").content.shift();else H.byteOffset+=w,H.byteLength-=w;@getByIdDirectPrivate(_,\"queue\").size-=w,@assert(@getByIdDirectPrivate(_,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(_,\"pendingPullIntos\").peek()===q),@readableByteStreamControllerInvalidateBYOBRequest(_),q.bytesFilled+=w,E-=w}if(!G)@assert(@getByIdDirectPrivate(_,\"queue\").size===0),@assert(q.bytesFilled>0),@assert(q.bytesFilled<q.elementSize);return G})\n";
...
const char* const s_writableStreamDefaultControllerInitializeWritableStreamDefaultControllerCode = "(function (){\"use strict\";return @putByIdDirectPrivate(this,\"queue\",@newQueue()),@putByIdDirectPrivate(this,\"abortSteps\",(u)=>{const _=@getByIdDirectPrivate(this,\"abortAlgorithm\").@call(@undefined,u);return @writableStreamDefaultControllerClearAlgorithms(this),_}),@putByIdDirectPrivate(this,\"errorSteps\",()=>{@resetQueue(@getByIdDirectPrivate(this,\"queue\"))}),this})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class j extends EventTarget{#$;#j;#w;#A;#B;#F=!1;#G=null;#J=\"\";#K=\"\";#L=\"\";#M=!0;#O=0;#Q=0;#U=0;#V=null;static#W(w){w.#H()}static#X(w,A){const B=w.data,F=B.#L\?`Last-Event-ID: ${B.#L}\\r\\n`:\"\",U=`GET ${A.pathname}${A.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${F}\\r\\n`,G=w.write(U);if(G!==U.length)B.#K=U.substring(G)}static#Y(w,A,B){for(;;){if(B>=A.length)return;let F=-1,U=A.indexOf(\"\\r\\n\",B);const G=U+2;if(U>0)if(w.#O===0){const X=parseInt(A.substring(B,U),16);if(X===0){w.#j=2,w.#G\?.end();return}F=G+X}else F=A.length;else{if(w.#J.length===0){w.#J+=A.substring(B);return}F=A.length}let V=A.substring(G,F);B=F+2;let J=0,W=V.indexOf(\"\\n\\n\");if(W==-1){w.#J+=A.substring(G);return}if(w.#J.length)w.#J+=V,V=w.#J,w.#J=\"\";let K=!0;while(K){const X=V.substring(J,W);let L,Y=\"\",M,Z=0,z=-1;for(;;){let H=X.indexOf(\"\\n\",Z);if(H===-1){if(Z>=X.length)break;H=X.length}const Q=X.substring(Z,H);if(Q.startsWith(\"data:\"))if(Y.length)Y+=`\\n${Q.substring(5).trim()}`;else Y=Q.substring(5).trim();else if(Q.startsWith(\"event:\"))L=Q.substring(6).trim();else if(Q.startsWith(\"id:\"))M=Q.substring(3).trim();else if(Q.startsWith(\"retry:\")){if(z=parseInt(Q.substring(6).trim(),10),@isNaN(z))z=-1}Z=H+1}if(w.#L=M||\"\",z>=0)w.#U=z;if(Y||M||L)w.dispatchEvent(new MessageEvent(L||\"message\",{data:Y||\"\",origin:w.#$.origin,source:w,lastEventId:M}));if(V.length===W+2){K=!1;break}const O=V.indexOf(\"\\n\\n\",W+1);if(O===-1)break;J=W,W=O}}}static#Z={open(w){const A=w.data;if(A.#G=w,!A.#F)j.#X(w,A.#$)},handshake(w,A,B){const F=w.data;if(A)j.#X(w,F.#$);else F.#j=2,F.dispatchEvent(new ErrorEvent(\"error\",{error:B})),w.end()},data(w,A){const B=w.data;switch(B.#j){case 0:{let F=A.toString();const U=F.indexOf(\"\\r\\n\\r\\n\");if(U===-1){B.#J+=F;return}if(B.#J.length)B.#J+=F,F=B.#J,B.#J=\"\";const G=F.substring(0,U),V=G.indexOf(\"\\r\\n\");if(V===-1){B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),w.end();return}const J=G.substring(0,V);if(J!==\"HTTP/1.1 200 OK\"){B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(J)})),w.end();return}let W=V+1,K=!1,X=-1;for(;;){let Y=G.indexOf(\"\\r\\n\",W);if(Y===-1){if(W>=G.length){if(!K)B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),w.end();return}Y=G.length}const M=G.substring(W+1,Y),Z=M.indexOf(\":\"),z=M.substring(0,Z),O=z.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(W=Y+1,O)if(M.endsWith(\" text/event-stream\"))K=!0;else{B.#j=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),w.end();return}else if(z.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(X=parseInt(M.substring(Z+1).trim(),10),@isNaN(X)||X<=0){B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),w.end();return}if(K)break}else if(z.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(M.substring(Z+1).trim()!==\"chunked\"){B.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),w.end();return}if(X=0,K)break}}B.#O=X,B.#j=1,B.dispatchEvent(new Event(\"open\"));const L=F.substring(U+4);if(j.#Y(B,L,0),B.#O>0){if(B.#Q+=L.length,B.#Q>=B.#O)B.#j=2,w.end()}return}case 1:if(j.#Y(B,A.toString(),2),B.#O>0){if(B.#Q+=A.byteLength,B.#Q>=B.#O)B.#j=2,w.end()}return;default:break}},drain(w){const A=w.data;if(A.#j===0){const B=A.#J;if(B.length){const F=w.write(B);if(F!==B.length)w.data.#K=B.substring(F);else w.data.#K=\"\"}}},close:j.#z,end(w){j.#z(w).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(w){j.#z(w).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#z(w){const A=w.data;if(A.#G=null,A.#Q=0,A.#j=2,A.#M){if(A.#V)clearTimeout(A.#V);A.#V=setTimeout(j.#W,A.#U,A)}return A}constructor(w,A=@undefined){super();const B=new URL(w);this.#F=B.protocol===\"https:\",this.#$=B,this.#j=2,process.nextTick(j.#W,this)}ref(){this.#V\?.ref(),this.#G\?.ref()}unref(){this.#V\?.unref(),this.#G\?.unref()}#H(){if(this.#j!==2)return;const w=this.#$,A=this.#F;this.#j=0,@Bun.connect({data:this,socket:j.#Z,hostname:w.hostname,port:parseInt(w.port||(A\?\"443\":\"80\"),10),tls:A\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((B)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:B})),this.#M){if(this.#V)this.#V.unref\?.();this.#V=setTimeout(j.#W,1000,this)}})}get url(){return this.#$.href}get readyState(){return this.#j}close(){this.#M=!1,this.#j=2,this.#G\?.unref(),this.#G\?.end()}get onopen(){return this.#B}get onerror(){return this.#w}get onmessage(){return this.#A}set onopen(w){if(this.#B)super.removeEventListener(\"close\",this.#B);super.addEventListener(\"open\",w),this.#B=w}set onerror(w){if(this.#w)super.removeEventListener(\"error\",this.#w);super.addEventListener(\"error\",w),this.#w=w}set onmessage(w){if(this.#A)super.removeEventListener(\"message\",this.#A);super.addEventListener(\"message\",w),this.#A=w}}return Object.defineProperty(j.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(j.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(j.prototype,\"CLOSED\",{enumerable:!0,value:2}),j[Symbol.for(\"CommonJS\")]=0,j})\n";
