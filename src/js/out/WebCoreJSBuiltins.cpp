...
const char* const s_bundlerPluginRunOnResolvePluginsCode = "(function (_,z,A,B,w){\"use strict\";const C=[\"entry-point\",\"import-statement\",\"require-call\",\"dynamic-import\",\"require-resolve\",\"import-rule\",\"url-token\",\"internal\"][w];var E=(async(F,g,y,q)=>{var{onResolve:b,onLoad:j}=this,G=b.@get(g);if(!G)return this.onResolveAsync(B,null,null,null),null;for(let[O,Q]of G)if(O.test(F)){var H=Q({path:F,importer:y,namespace:g,kind:q});while(H&&@isPromise(H)&&(@getPromiseInternalField(H,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)H=@getPromiseInternalField(H,@promiseFieldReactionsOrResult);if(H&&@isPromise(H))H=await H;if(!H||!@isObject(H))continue;var{path:J,namespace:K=g,external:M}=H;if(typeof J!==\"string\"||typeof K!==\"string\")@throwTypeError(\"onResolve plugins must return an object with a string 'path' and string 'loader' field\");if(!J)continue;if(!K)K=g;if(typeof M!==\"boolean\"&&!@isUndefinedOrNull(M))@throwTypeError('onResolve plugins \"external\" field must be boolean or unspecified');if(!M){if(K===\"file\"){if(darwin!==\"win32\"){if(J[0]!==\"/\"||J.includes(\"..\"))@throwTypeError('onResolve plugin \"path\" must be absolute when the namespace is \"file\"')}}if(K===\"dataurl\"){if(!J.startsWith(\"data:\"))@throwTypeError('onResolve plugin \"path\" must start with \"data:\" when the namespace is \"dataurl\"')}if(K&&K!==\"file\"&&(!j||!j.@has(K)))@throwTypeError(`Expected onLoad plugin for namespace ${K} to exist`)}return this.onResolveAsync(B,J,K,M),null}return this.onResolveAsync(B,null,null,null),null})(_,z,A,C);while(E&&@isPromise(E)&&(@getPromiseInternalField(E,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)E=@getPromiseInternalField(E,@promiseFieldReactionsOrResult);if(E&&@isPromise(E))E.then(()=>{},(F)=>{this.addError(B,F,0)})})\n";
...
const char* const s_bundlerPluginRunOnLoadPluginsCode = "(function (b,j,q,v){\"use strict\";const w={jsx:0,js:1,ts:2,tsx:3,css:4,file:5,json:6,toml:7,wasm:8,napi:9,base64:10,dataurl:11,text:12},x=[\"jsx\",\"js\",\"ts\",\"tsx\",\"css\",\"file\",\"json\",\"toml\",\"wasm\",\"napi\",\"base64\",\"dataurl\",\"text\"][v];var y=(async(z,B,C,F)=>{var G=this.onLoad.@get(C);if(!G)return this.onLoadAsync(z,null,null),null;for(let[K,Q]of G)if(K.test(B)){var g=Q({path:B,namespace:C,loader:F});while(g&&@isPromise(g)&&(@getPromiseInternalField(g,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)g=@getPromiseInternalField(g,@promiseFieldReactionsOrResult);if(g&&@isPromise(g))g=await g;if(!g||!@isObject(g))continue;var{contents:H,loader:J=F}=g;if(typeof H!==\"string\"&&!@isTypedArrayView(H))@throwTypeError('onLoad plugins must return an object with \"contents\" as a string or Uint8Array');if(typeof J!==\"string\")@throwTypeError('onLoad plugins must return an object with \"loader\" as a string');const T=w[J];if(T===@undefined)@throwTypeError(`Loader ${J} is not supported.`);return this.onLoadAsync(z,H,T),null}return this.onLoadAsync(z,null,null),null})(b,j,q,x);while(y&&@isPromise(y)&&(@getPromiseInternalField(y,@promiseFieldFlags)&@promiseStateMask)===@promiseStateFulfilled)y=@getPromiseInternalField(y,@promiseFieldReactionsOrResult);if(y&&@isPromise(y))y.then(()=>{},(z)=>{this.addError(b,z,1)})})\n";
...
const char* const s_writableStreamInternalsWritableStreamAbortCode = "(function (B,_){\"use strict\";const f=@getByIdDirectPrivate(B,\"state\");if(f===\"closed\"||f===\"errored\")return @Promise.@resolve();const c=@getByIdDirectPrivate(B,\"pendingAbortRequest\");if(c!==@undefined)return c.promise.@promise;@assert(f===\"writable\"||f===\"erroring\");let h=!1;if(f===\"erroring\")h=!0,_=@undefined;const j=@newPromiseCapability(@Promise);if(@putByIdDirectPrivate(B,\"pendingAbortRequest\",{promise:j,reason:_,wasAlreadyErroring:h}),!h)@writableStreamStartErroring(B,_);return j.@promise})\n";
...
const char* const s_writableStreamInternalsWritableStreamDefaultControllerProcessWriteCode = "(function (i,_){\"use strict\";const d=@getByIdDirectPrivate(i,\"stream\");@writableStreamMarkFirstWriteRequestInFlight(d),@getByIdDirectPrivate(i,\"writeAlgorithm\").@call(@undefined,_).@then(()=>{@writableStreamFinishInFlightWrite(d);const v=@getByIdDirectPrivate(d,\"state\");if(@assert(v===\"writable\"||v===\"erroring\"),@dequeueValue(@getByIdDirectPrivate(i,\"queue\")),!@writableStreamCloseQueuedOrInFlight(d)&&v===\"writable\"){const M=@writableStreamDefaultControllerGetBackpressure(i);@writableStreamUpdateBackpressure(d,M)}@writableStreamDefaultControllerAdvanceQueueIfNeeded(i)},(v)=>{if(@getByIdDirectPrivate(d,\"state\")===\"writable\")@writableStreamDefaultControllerClearAlgorithms(i);@writableStreamFinishInFlightWriteWithError(d,v)})})\n";
...
const char* const s_transformStreamInternalsInitializeTransformStreamCode = "(function (f,I,B,T,j,F){\"use strict\";const J=()=>{return I},K=(G)=>{return @transformStreamDefaultSinkWriteAlgorithm(f,G)},q=(G)=>{return @transformStreamDefaultSinkAbortAlgorithm(f,G)},v=()=>{return @transformStreamDefaultSinkCloseAlgorithm(f)},L=@createWritableStream(J,K,v,q,B,T),N=()=>{return @transformStreamDefaultSourcePullAlgorithm(f)},x=(G)=>{return @transformStreamErrorWritableAndUnblockWrite(f,G),@Promise.@resolve()},D={};@putByIdDirectPrivate(D,\"start\",J),@putByIdDirectPrivate(D,\"pull\",N),@putByIdDirectPrivate(D,\"cancel\",x);const E={};@putByIdDirectPrivate(E,\"size\",F),@putByIdDirectPrivate(E,\"highWaterMark\",j);const C=new @ReadableStream(D,E);@putByIdDirectPrivate(f,\"writable\",L),@putByIdDirectPrivate(f,\"internalWritable\",@getInternalWritableStream(L)),@putByIdDirectPrivate(f,\"readable\",C),@putByIdDirectPrivate(f,\"backpressure\",@undefined),@putByIdDirectPrivate(f,\"backpressureChangePromise\",@undefined),@transformStreamSetBackpressure(f,!0),@putByIdDirectPrivate(f,\"controller\",@undefined)})\n";
...
const char* const s_processObjectInternalsGetStdioWriteStreamCode = "(function (M,G){\"use strict\";var H={path:\"node:process\",require:G},J=(V)=>H.require(V);function K(V){var{Duplex:B,eos:X,destroy:Z}=J(\"node:stream\"),Y=class P extends B{#$;#B;#j=!0;#z=!0;#G;#H;#J;#K;#L;#M;get isTTY(){return this.#M\?\?=J(\"node:tty\").isatty(V)}get fd(){return V}constructor(A){super({readable:!0,writable:!0});this.#G=`/dev/fd/${A}`}#N(A){const j=this.#H;if(this.#H=null,j)j(A);else if(A)this.destroy(A);else if(!this.#j&&!this.#z)this.destroy()}_destroy(A,j){if(!A&&this.#H!==null){var L=class T extends Error{code;name;constructor(x=\"The operation was aborted\",z=void 0){if(z!==void 0&&typeof z!==\"object\")throw new Error(`Invalid AbortError options:\\n\\n${JSON.stringify(z,null,2)}`);super(x,z);this.code=\"ABORT_ERR\",this.name=\"AbortError\"}};A=new L}if(this.#J=null,this.#K=null,this.#H===null)j(A);else{if(this.#H=j,this.#$)Z(this.#$,A);if(this.#B)Z(this.#B,A)}}_write(A,j,L){if(!this.#$){var{createWriteStream:T}=J(\"node:fs\"),x=this.#$=T(this.#G);x.on(\"finish\",()=>{if(this.#K){const z=this.#K;this.#K=null,z()}}),x.on(\"drain\",()=>{if(this.#J){const z=this.#J;this.#J=null,z()}}),X(x,(z)=>{if(this.#z=!1,z)Z(x,z);this.#N(z)})}if(x.write(A,j))L();else this.#J=L}_final(A){this.#$&&this.#$.end(),this.#K=A}#O(){var{createReadStream:A}=J(\"node:fs\"),j=this.#B=A(this.#G);return j.on(\"readable\",()=>{if(this.#L){const L=this.#L;this.#L=null,L()}else this.read()}),j.on(\"end\",()=>{this.push(null)}),X(j,(L)=>{if(this.#j=!1,L)Z(j,L);this.#N(L)}),j}_read(){var A=this.#B;if(!A)A=this.#O();while(!0){const j=A.read();if(j===null||!this.push(j))return}}};return new Y(V)}var{EventEmitter:N}=J(\"node:events\");function O(V){if(!V)return!0;var B=V.toLowerCase();return B===\"utf8\"||B===\"utf-8\"||B===\"buffer\"||B===\"binary\"}var Q,U=class V extends N{#$;#B;#j;#z;bytesWritten=0;setDefaultEncoding(B){if(this.#B||!O(B))return this.#J(),this.#B.setDefaultEncoding(B)}#G(){switch(this.#$){case 1:{var B=@Bun.stdout.writer({highWaterMark:0});return B.unref(),B}case 2:{var B=@Bun.stderr.writer({highWaterMark:0});return B.unref(),B}default:throw new Error(\"Unsupported writer\")}}#H(){return this.#j\?\?=this.#G()}constructor(B){super();this.#$=B}get fd(){return this.#$}get isTTY(){return this.#z\?\?=J(\"node:tty\").isatty(this.#$)}cursorTo(B,X,Z){return(Q\?\?=J(\"readline\")).cursorTo(this,B,X,Z)}moveCursor(B,X,Z){return(Q\?\?=J(\"readline\")).moveCursor(this,B,X,Z)}clearLine(B,X){return(Q\?\?=J(\"readline\")).clearLine(this,B,X)}clearScreenDown(B){return(Q\?\?=J(\"readline\")).clearScreenDown(this,B)}ref(){this.#H().ref()}unref(){this.#H().unref()}on(B,X){if(B===\"close\"||B===\"finish\")return this.#J(),this.#B.on(B,X);if(B===\"drain\")return super.on(\"drain\",X);if(B===\"error\")return super.on(\"error\",X);return super.on(B,X)}get _writableState(){return this.#J(),this.#B._writableState}get _readableState(){return this.#J(),this.#B._readableState}pipe(B){return this.#J(),this.#B.pipe(B)}unpipe(B){return this.#J(),this.#B.unpipe(B)}#J(){if(this.#B)return;this.#B=K(this.#$);const B=this.eventNames();for(let X of B)this.#B.on(X,(...Z)=>{this.emit(X,...Z)})}#K(B){var X=this.#H();const Z=X.write(B);this.bytesWritten+=Z;const Y=X.flush(!1);return!!(Z||Y)}#L(B,X){if(!O(X))return this.#J(),this.#B.write(B,X);return this.#K(B)}#M(B,X){if(X)this.emit(\"error\",X);try{B(X\?X:null)}catch(Z){this.emit(\"error\",Z)}}#N(B,X,Z){if(!O(X))return this.#J(),this.#B.write(B,X,Z);var Y=this.#H();const P=Y.write(B),A=Y.flush(!0);if(A\?.then)return A.then(()=>{this.#M(Z),this.emit(\"drain\")},(j)=>this.#M(Z,j)),!1;return queueMicrotask(()=>{this.#M(Z)}),!!(P||A)}write(B,X,Z){const Y=this._write(B,X,Z);if(Y)this.emit(\"drain\");return Y}get hasColors(){return @Bun.tty[this.#$].hasColors}_write(B,X,Z){var Y=this.#B;if(Y)return Y.write(B,X,Z);switch(arguments.length){case 0:{var P=new Error(\"Invalid arguments\");throw P.code=\"ERR_INVALID_ARG_TYPE\",P}case 1:return this.#K(B);case 2:if(typeof X===\"function\")return this.#N(B,\"\",X);else if(typeof X===\"string\")return this.#L(B,X);default:{if(typeof X!==\"undefined\"&&typeof X!==\"string\"||typeof Z!==\"undefined\"&&typeof Z!==\"function\"){var P=new Error(\"Invalid arguments\");throw P.code=\"ERR_INVALID_ARG_TYPE\",P}if(typeof Z===\"undefined\")return this.#L(B,X);return this.#N(B,X,Z)}}}destroy(){return this}end(){return this}};return new U(M)})\n";
...
const char* const s_transformStreamInitializeTransformStreamCode = "(function (){\"use strict\";let _=arguments[0];if(@isObject(_)&&@getByIdDirectPrivate(_,\"TransformStream\"))return this;let j=arguments[1],q=arguments[2];if(_===@undefined)_=null;if(q===@undefined)q={};if(j===@undefined)j={};let v={};if(_!==null){if(\"start\"in _){if(v[\"start\"]=_[\"start\"],typeof v[\"start\"]!==\"function\")@throwTypeError(\"transformer.start should be a function\")}if(\"transform\"in _){if(v[\"transform\"]=_[\"transform\"],typeof v[\"transform\"]!==\"function\")@throwTypeError(\"transformer.transform should be a function\")}if(\"flush\"in _){if(v[\"flush\"]=_[\"flush\"],typeof v[\"flush\"]!==\"function\")@throwTypeError(\"transformer.flush should be a function\")}if(\"readableType\"in _)@throwRangeError(\"TransformStream transformer has a readableType\");if(\"writableType\"in _)@throwRangeError(\"TransformStream transformer has a writableType\")}const u=@extractHighWaterMark(q,0),F=@extractSizeAlgorithm(q),x=@extractHighWaterMark(j,1),B=@extractSizeAlgorithm(j),E=@newPromiseCapability(@Promise);if(@initializeTransformStream(this,E.@promise,x,B,u,F),@setUpTransformStreamDefaultControllerFromTransformer(this,_,v),(\"start\"in v)){const G=@getByIdDirectPrivate(this,\"controller\");(()=>@promiseInvokeOrNoopMethodNoCatch(_,v[\"start\"],[G]))().@then(()=>{E.@resolve.@call()},(J)=>{E.@reject.@call(@undefined,J)})}else E.@resolve.@call();return this})\n";
...
const int s_moduleRequireCodeLength = 851;
static const JSC::Intrinsic s_moduleRequireCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_moduleRequireCode = "(function (r){\"use strict\";const _=@requireMap.@get(r)||@requireMap.@get(r=@resolveSync(r,this.path,!1));if(_)return _.exports;if(r.endsWith(\".json\")||r.endsWith(\".toml\")||r.endsWith(\".node\"))return @internalRequire(r);let l=@Loader.registry.@get(r);if(l\?.evaluated){const f=l.module,b=@Loader.getModuleNamespaceObject(f),h=b\?.[@commonJSSymbol]===0||b\?.default\?.[@commonJSSymbol]===0\?b.default:b;return @requireMap.@set(r,@createCommonJSModule(r,h)),h}let y=this.@require(r);if(y===-1){const f=@createCommonJSModule(r,{});@requireMap.@set(r,f);try{y=@requireESM(r)}catch(b){throw @requireMap.@delete(r),b}if(l=@Loader.registry.@get(r),l\?.evaluated){const b=@Loader.getModuleNamespaceObject(l.module);return f.exports=b\?.[@commonJSSymbol]===0||b\?.default\?.[@commonJSSymbol]===0\?b.default:b}}const M=@requireMap.@get(r);if(M)return M.exports;return y})\n";
...
const char* const s_jsBufferPrototypeSliceCode = "(function (c,k){\"use strict\";var{buffer:m,byteOffset:q,byteLength:v}=this;function w(i,z){if(i=@trunc(i),i===0||@isNaN(i))return 0;else if(i<0)return i+=z,i>0\?i:0;else return i<z\?i:z}var p=w(c,v),x=k!==@undefined\?w(k,v):v;return new @Buffer(m,q+p,x>p\?x-p:0)})\n";
...
const char* const s_consoleObjectAsyncIteratorCode = "(function (){\"use strict\";const _=async function*m(){var B=@Bun.stdin.stream().getReader(),F=new globalThis.TextDecoder(\"utf-8\",{fatal:!1}),G,H=@Bun.indexOfLine;try{while(!0){var J,q,K;const D=B.readMany();if(@isPromise(D))({done:J,value:q}=await D);else({done:J,value:q}=D);if(J){if(K)yield F.decode(K);return}var L;for(let M of q){if(L=M,K)L=@Buffer.concat([K,M]),K=null;var z=0,A=H(L,z);while(A!==-1)yield F.decode(L.subarray(z,A)),z=A+1,A=H(L,z);K=L.subarray(z)}}}catch(D){G=D}finally{if(B.releaseLock(),G)throw G}},j=globalThis.Symbol.asyncIterator;return this[j]=_,_()})\n";
...
const char* const s_readableStreamInternalsCreateReadableStreamControllerCode = "(function (v,w,C){\"use strict\";const f=w.type,A=@toString(f);if(A===\"bytes\"){if(C.highWaterMark===@undefined)C.highWaterMark=0;if(C.size!==@undefined)@throwRangeError(\"Strategy for a ReadableByteStreamController cannot have a size\");@putByIdDirectPrivate(v,\"readableStreamController\",new @ReadableByteStreamController(v,w,C.highWaterMark,@isReadableStream))}else if(A===\"direct\"){var b=C\?.highWaterMark;@initializeArrayBufferStream.@call(v,w,b)}else if(f===@undefined){if(C.highWaterMark===@undefined)C.highWaterMark=1;@setupReadableStreamDefaultController(v,w,C.size,C.highWaterMark,w.start,w.pull,w.cancel)}else @throwRangeError(\"Invalid type for underlying source\")})\n";
...
const char* const s_readableStreamInternalsReadableStreamPipeToWritableStreamCode = "(function (f,z,H,B,D,I){\"use strict\";if(@assert(@isReadableStream(f)),@assert(@isWritableStream(z)),@assert(!@isReadableStreamLocked(f)),@assert(!@isWritableStreamLocked(z)),@assert(I===@undefined||@isAbortSignal(I)),@getByIdDirectPrivate(f,\"underlyingByteSource\")!==@undefined)return @Promise.@reject(\"Piping to a readable bytestream is not supported\");let _={source:f,destination:z,preventAbort:B,preventCancel:D,preventClose:H,signal:I};if(_.reader=@acquireReadableStreamDefaultReader(f),_.writer=@acquireWritableStreamDefaultWriter(z),@putByIdDirectPrivate(f,\"disturbed\",!0),_.finalized=!1,_.shuttingDown=!1,_.promiseCapability=@newPromiseCapability(@Promise),_.pendingReadPromiseCapability=@newPromiseCapability(@Promise),_.pendingReadPromiseCapability.@resolve.@call(),_.pendingWritePromise=@Promise.@resolve(),I!==@undefined){const J=(E)=>{if(_.finalized)return;@pipeToShutdownWithAction(_,()=>{const K=!_.preventAbort&&@getByIdDirectPrivate(_.destination,\"state\")===\"writable\"\?@writableStreamAbort(_.destination,E):@Promise.@resolve(),k=!_.preventCancel&&@getByIdDirectPrivate(_.source,\"state\")===@streamReadable\?@readableStreamCancel(_.source,E):@Promise.@resolve();let q=@newPromiseCapability(@Promise),w=!0,G=()=>{if(w){w=!1;return}q.@resolve.@call()},L=(x)=>{q.@reject.@call(@undefined,x)};return K.@then(G,L),k.@then(G,L),q.@promise},E)};if(@whenSignalAborted(I,J))return _.promiseCapability.@promise}return @pipeToErrorsMustBePropagatedForward(_),@pipeToErrorsMustBePropagatedBackward(_),@pipeToClosingMustBePropagatedForward(_),@pipeToClosingMustBePropagatedBackward(_),@pipeToLoop(_),_.promiseCapability.@promise})\n";
...
const char* const s_readableStreamInternalsPipeToShutdownWithActionCode = "(function (b,j){\"use strict\";if(b.shuttingDown)return;b.shuttingDown=!0;const d=arguments.length>2,g=arguments[2],_=()=>{j().@then(()=>{if(d)@pipeToFinalize(b,g);else @pipeToFinalize(b)},(m)=>{@pipeToFinalize(b,m)})};if(@getByIdDirectPrivate(b.destination,\"state\")===\"writable\"&&!@writableStreamCloseQueuedOrInFlight(b.destination)){b.pendingReadPromiseCapability.@promise.@then(()=>{b.pendingWritePromise.@then(_,_)},(h)=>@pipeToFinalize(b,h));return}_()})\n";
...
const char* const s_readableStreamInternalsReadStreamIntoSinkCode = "(async function (c,D,E){\"use strict\";var f=!1,x=!1;try{var F=c.getReader(),H=F.readMany();if(H&&@isPromise(H))H=await H;if(H.done)return f=!0,D.end();var J=H.value.length;const q=@getByIdDirectPrivate(c,\"highWaterMark\");if(E)@startDirectStream.@call(D,c,@undefined,()=>!x&&@markPromiseAsHandled(c.cancel()));D.start({highWaterMark:q||0});for(var K=0,I=H.value,z=H.value.length;K<z;K++)D.write(I[K]);var _=@getByIdDirectPrivate(c,\"state\");if(_===@streamClosed)return f=!0,D.end();while(!0){var{value:P,done:A}=await F.read();if(A)return f=!0,D.end();D.write(P)}}catch(q){x=!0;try{F=@undefined;const G=c.cancel(q);@markPromiseAsHandled(G)}catch(G){}if(D&&!f){f=!0;try{D.close(q)}catch(G){throw new globalThis.AggregateError([q,G])}}throw q}finally{if(F){try{F.releaseLock()}catch(G){}F=@undefined}D=@undefined;var _=@getByIdDirectPrivate(c,\"state\");if(c){var B=@getByIdDirectPrivate(c,\"readableStreamController\");if(B){if(@getByIdDirectPrivate(B,\"underlyingSource\"))@putByIdDirectPrivate(B,\"underlyingSource\",@undefined);if(@getByIdDirectPrivate(B,\"controlledReadableStream\"))@putByIdDirectPrivate(B,\"controlledReadableStream\",@undefined);if(@putByIdDirectPrivate(c,\"readableStreamController\",null),@getByIdDirectPrivate(c,\"underlyingSource\"))@putByIdDirectPrivate(c,\"underlyingSource\",@undefined);B=@undefined}if(!x&&_!==@streamClosed&&_!==@streamErrored)@readableStreamClose(c);c=@undefined}}})\n";
...
const char* const s_readableStreamInternalsOnPullDirectStreamCode = "(function (g){\"use strict\";var _=g.@controlledReadableStream;if(!_||@getByIdDirectPrivate(_,\"state\")!==@streamReadable)return;if(g._deferClose===-1)return;g._deferClose=-1,g._deferFlush=-1;var b,i;try{var j=g.@underlyingSource.pull(g);if(j&&@isPromise(j)){if(g._handleError===@undefined)g._handleError=@handleDirectStreamErrorReject.bind(g);@Promise.prototype.catch.@call(j,g._handleError)}}catch(y){return @handleDirectStreamErrorReject.@call(g,y)}finally{b=g._deferClose,i=g._deferFlush,g._deferFlush=g._deferClose=0}var k;if(g._pendingRead===@undefined)g._pendingRead=k=@newPromise();else k=@readableStreamAddReadRequest(_);if(b===1){var h=g._deferCloseReason;return g._deferCloseReason=@undefined,@onCloseDirectStream.@call(g,h),k}if(i===1)@onFlushDirectStream.@call(g);return k})\n";
...
const char* const s_readableStreamInternalsInitializeArrayStreamCode = "(function (t,_){\"use strict\";var p=[],j=@newPromiseCapability(@Promise),b=!1;function m(){return b=!0,j.@resolve.@call(@undefined,p),p}var d={start(){},write(v){return @arrayPush(p,v),v.byteLength||v.length},flush(){return 0},end(){if(b)return[];return m()},close(){if(!b)m()}},q={@underlyingSource:t,@pull:@onPullDirectStream,@controlledReadableStream:this,@sink:d,close:@onCloseDirectStream,write:d.write,error:@handleDirectStreamError,end:@onCloseDirectStream,@close:@onCloseDirectStream,flush:@onFlushDirectStream,_pendingRead:@undefined,_deferClose:0,_deferFlush:0,_deferCloseReason:@undefined,_handleError:@undefined};return @putByIdDirectPrivate(this,\"readableStreamController\",q),@putByIdDirectPrivate(this,\"underlyingSource\",@undefined),@putByIdDirectPrivate(this,\"start\",@undefined),j})\n";
...
const char* const s_readableStreamInternalsReadableStreamErrorCode = "(function (i,f){\"use strict\";@assert(@isReadableStream(i)),@assert(@getByIdDirectPrivate(i,\"state\")===@streamReadable),@putByIdDirectPrivate(i,\"state\",@streamErrored),@putByIdDirectPrivate(i,\"storedError\",f);const c=@getByIdDirectPrivate(i,\"reader\");if(!c)return;if(@isReadableStreamDefaultReader(c)){const _=@getByIdDirectPrivate(c,\"readRequests\");@putByIdDirectPrivate(c,\"readRequests\",@createFIFO());for(var h=_.shift();h;h=_.shift())@rejectPromise(h,f)}else{@assert(@isReadableStreamBYOBReader(c));const _=@getByIdDirectPrivate(c,\"readIntoRequests\");@putByIdDirectPrivate(c,\"readIntoRequests\",@createFIFO());for(var h=_.shift();h;h=_.shift())@rejectPromise(h,f)}@getByIdDirectPrivate(c,\"closedPromiseCapability\").@reject.@call(@undefined,f);const v=@getByIdDirectPrivate(c,\"closedPromiseCapability\").@promise;@markPromiseAsHandled(v)})\n";
...
const char* const s_readableStreamInternalsReadableStreamFulfillReadRequestCode = "(function (i,p,_){\"use strict\";const y=@getByIdDirectPrivate(@getByIdDirectPrivate(i,\"reader\"),\"readRequests\").shift();@fulfillPromise(y,{value:p,done:_})})\n";
...
const char* const s_readableStreamInternalsLazyLoadStreamCode = "(function (W,I){\"use strict\";var M=@getByIdDirectPrivate(W,\"bunNativeType\"),J=@getByIdDirectPrivate(W,\"bunNativePtr\"),K=@lazyStreamPrototypeMap.@get(M);if(K===@undefined){let m=function(F){var{c:p,v:z}=this;this.c=@undefined,this.v=@undefined,f(F,p,z)},U=function(F){try{F.close()}catch(p){globalThis.reportError(p)}},L=function(F,p,z,A){A[0]=!1;var G;try{G=b(F,z,A)}catch(H){return p.error(H)}return f(G,p,z)};var j=m,Q=U,q=L,[b,x,X,y,O,Y,Z]=@lazyLoad(M),P=[!1],f;f=function F(p,z,A){if(p&&@isPromise(p))return p.then(m.bind({c:z,v:A}),(G)=>z.error(G));else if(typeof p===\"number\")if(A&&A.byteLength===p&&A.buffer===z.byobRequest\?.view\?.buffer)z.byobRequest.respondWithNewView(A);else z.byobRequest.respond(p);else if(p.constructor===@Uint8Array)z.enqueue(p);if(P[0]||p===!1)@enqueueJob(U,z),P[0]=!1};const E=O\?new FinalizationRegistry(O):null;K=class F{constructor(p,z,A){if(this.#f=p,this.#b={},this.pull=this.#j.bind(this),this.cancel=this.#m.bind(this),this.autoAllocateChunkSize=z,A!==@undefined)this.start=(G)=>{G.enqueue(A)};if(E)E.register(this,p,this.#b)}#b;pull;cancel;start;#f;type=\"bytes\";autoAllocateChunkSize=0;static startSync=x;#j(p){var z=this.#f;if(!z){p.close();return}L(z,p,p.byobRequest.view,P)}#m(p){var z=this.#f;E&&E.unregister(this.#b),Y&&Y(z,!1),X(z,p)}static deinit=O;static drain=Z},@lazyStreamPrototypeMap.@set(M,K)}const B=K.startSync(J,I);var _;const{drain:D,deinit:N}=K;if(D)_=D(J);if(B===0){if(O&&J&&@enqueueJob(O,J),(_\?.byteLength\?\?0)>0)return{start(m){m.enqueue(_),m.close()},type:\"bytes\"};return{start(m){m.close()},type:\"bytes\"}}return new K(J,B,_)})\n";
...
const char* const s_readableStreamInternalsReadableStreamToArrayBufferDirectCode = "(function (_,B){\"use strict\";var O=new @Bun.ArrayBufferSink;@putByIdDirectPrivate(_,\"underlyingSource\",@undefined);var j=@getByIdDirectPrivate(_,\"highWaterMark\");O.start(j\?{highWaterMark:j}:{});var x=@newPromiseCapability(@Promise),A=!1,C=B.pull,D=B.close,q={start(){},close(F){if(!A){if(A=!0,D)D();@fulfillPromise(x.@promise,O.end())}},end(){if(!A){if(A=!0,D)D();@fulfillPromise(x.@promise,O.end())}},flush(){return 0},write:O.write.bind(O)},v=!1;try{const F=C(q);if(F&&@isObject(F)&&@isPromise(F))return async function(G,w,z){while(!A)await z(G);return await w}(q,promise,C);return x.@promise}catch(F){return v=!0,@readableStreamError(_,F),@Promise.@reject(F)}finally{if(!v&&_)@readableStreamClose(_);q=D=O=C=_=@undefined}})\n";
...
const char* const s_readableStreamDefaultReaderReadManyCode = "(function (){\"use strict\";if(!@isReadableStreamDefaultReader(this))@throwTypeError(\"ReadableStreamDefaultReader.readMany() should not be called directly\");const K=@getByIdDirectPrivate(this,\"ownerReadableStream\");if(!K)@throwTypeError(\"readMany() called on a reader owned by no readable stream\");const k=@getByIdDirectPrivate(K,\"state\");if(@putByIdDirectPrivate(K,\"disturbed\",!0),k===@streamClosed)return{value:[],size:0,done:!0};else if(k===@streamErrored)throw @getByIdDirectPrivate(K,\"storedError\");var G=@getByIdDirectPrivate(K,\"readableStreamController\"),x=@getByIdDirectPrivate(G,\"queue\");if(!x)return G.@pull(G).@then(function({done:S,value:T}){return S\?{done:!0,value:[],size:0}:{value:[T],size:1,done:!1}});const C=x.content;var _=x.size,A=C.toArray(!1),N=A.length;if(N>0){var D=@newArrayWithSize(N);if(@isReadableByteStreamController(G)){{const S=A[0];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer))@putByValDirect(D,0,new @Uint8Array(S.buffer,S.byteOffset,S.byteLength));else @putByValDirect(D,0,S)}for(var E=1;E<N;E++){const S=A[E];if(!(@ArrayBuffer.@isView(S)||S instanceof @ArrayBuffer))@putByValDirect(D,E,new @Uint8Array(S.buffer,S.byteOffset,S.byteLength));else @putByValDirect(D,E,S)}}else{@putByValDirect(D,0,A[0].value);for(var E=1;E<N;E++)@putByValDirect(D,E,A[E].value)}if(@resetQueue(@getByIdDirectPrivate(G,\"queue\")),@getByIdDirectPrivate(G,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(G,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(G))@readableStreamDefaultControllerCallPullIfNeeded(G);else if(@isReadableByteStreamController(G))@readableByteStreamControllerCallPullIfNeeded(G);return{value:D,size:_,done:!1}}var H=(S)=>{if(S.done)return{value:[],size:0,done:!0};var T=@getByIdDirectPrivate(K,\"readableStreamController\"),U=@getByIdDirectPrivate(T,\"queue\"),W=[S.value].concat(U.content.toArray(!1)),j=W.length;if(@isReadableByteStreamController(T))for(var F=0;F<j;F++){const d=W[F];if(!(@ArrayBuffer.@isView(d)||d instanceof @ArrayBuffer)){const{buffer:I,byteOffset:B,byteLength:J}=d;@putByValDirect(W,F,new @Uint8Array(I,B,J))}}else for(var F=1;F<j;F++)@putByValDirect(W,F,W[F].value);var w=U.size;if(@resetQueue(U),@getByIdDirectPrivate(T,\"closeRequested\"))@readableStreamClose(@getByIdDirectPrivate(T,\"controlledReadableStream\"));else if(@isReadableStreamDefaultController(T))@readableStreamDefaultControllerCallPullIfNeeded(T);else if(@isReadableByteStreamController(T))@readableByteStreamControllerCallPullIfNeeded(T);return{value:W,size:w,done:!1}},Q=G.@pull(G);if(Q&&@isPromise(Q))return Q.@then(H);return H(Q)})\n";
...
const char* const s_streamInternalsCreateFIFOCode = "(function (){\"use strict\";var b=@Array.prototype.slice;class w{constructor(){this._head=0,this._tail=0,this._capacityMask=3,this._list=@newArrayWithSize(4)}_head;_tail;_capacityMask;_list;size(){if(this._head===this._tail)return 0;if(this._head<this._tail)return this._tail-this._head;else return this._capacityMask+1-(this._head-this._tail)}isEmpty(){return this.size()==0}isNotEmpty(){return this.size()>0}shift(){var{_head:g,_tail:v,_list:k,_capacityMask:z}=this;if(g===v)return @undefined;var A=k[g];if(@putByValDirect(k,g,@undefined),g=this._head=g+1&z,g<2&&v>1e4&&v<=k.length>>>2)this._shrinkArray();return A}peek(){if(this._head===this._tail)return @undefined;return this._list[this._head]}push(g){var v=this._tail;if(@putByValDirect(this._list,v,g),this._tail=v+1&this._capacityMask,this._tail===this._head)this._growArray()}toArray(g){var v=this._list,k=@toLength(v.length);if(g||this._head>this._tail){var z=@toLength(this._head),A=@toLength(this._tail),B=@toLength(k-z+A),E=@newArrayWithSize(B),x=0;for(var F=z;F<k;F++)@putByValDirect(E,x++,v[F]);for(var F=0;F<A;F++)@putByValDirect(E,x++,v[F]);return E}else return b.@call(v,this._head,this._tail)}clear(){this._head=0,this._tail=0,this._list.fill(@undefined)}_growArray(){if(this._head)this._list=this.toArray(!0),this._head=0;this._tail=@toLength(this._list.length),this._list.length<<=1,this._capacityMask=this._capacityMask<<1|1}shrinkArray(){this._list.length>>>=1,this._capacityMask>>>=1}}return new w})\n";
...
const char* const s_importMetaObjectLoadCJS2ESMCode = "(function (x){\"use strict\";var w=@Loader,z=@createFIFO(),F=x;while(F){var G=w.registry.@get(F);if(!G||!G.state||G.state<=@ModuleFetch)@fulfillModuleSync(F),G=w.registry.@get(F);var H=@getPromiseInternalField(G.fetch,@promiseFieldReactionsOrResult),I=w.parseModule(F,H),J=G.module;if(I&&@isPromise(I)){var L=@getPromiseInternalField(I,@promiseFieldReactionsOrResult),Q=@getPromiseInternalField(I,@promiseFieldFlags),_=Q&@promiseStateMask;if(_===@promiseStatePending||L&&@isPromise(L))@throwTypeError(`require() async module \"${F}\" is unsupported`);else if(_===@promiseStateRejected){if(!L\?.message)@throwTypeError(`${L+\"\"\?L:\"An error occurred\"} occurred while parsing module \\\"${F}\\\"`);throw L}G.module=J=L}else if(I&&!J)G.module=J=I;@setStateToMax(G,@ModuleLink);var h=J.dependenciesMap,B=w.requestedModules(J),D=@newArrayWithSize(B.length);for(var T=0,U=B.length;T<U;++T){var V=B[T],W=V[0]===\"/\"\?V:w.resolve(V,F),X=w.ensureRegistered(W);if(X.state<@ModuleLink)z.push(W);@putByValDirect(D,T,X),h.@set(V,X)}G.dependencies=D,G.instantiate=@Promise.resolve(G),G.satisfy=@Promise.resolve(G),F=z.shift();while(F&&(w.registry.@get(F)\?.state\?\?@ModuleFetch)>=@ModuleLink)F=z.shift()}var Y=w.linkAndEvaluateModule(x,@undefined);if(Y&&@isPromise(Y))@throwTypeError(`require() async module \\\"${x}\\\" is unsupported`);return w.registry.@get(x)})\n";
...
const int s_importMetaObjectInternalRequireCodeLength = 686;
static const JSC::Intrinsic s_importMetaObjectInternalRequireCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_importMetaObjectInternalRequireCode = "(function (n){\"use strict\";var _=@requireMap.@get(n);const b=n.substring(n.length-5);if(_)return _.exports;if(b===\".json\"){var M=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),g=JSON.parse(M.readFileSync(n,\"utf8\"));return @requireMap.@set(n,@createCommonJSModule(n,g)),g}else if(b===\".node\"){const j=@createCommonJSModule(n,{});return process.dlopen(j,n),@requireMap.@set(n,j),j.exports}else if(b===\".toml\"){var M=globalThis[Symbol.for(\"_fs\")]||=@Bun.fs(),g=@Bun.TOML.parse(M.readFileSync(n,\"utf8\"));return @requireMap.@set(n,@createCommonJSModule(n,g)),g}else{var g=@requireESM(n);const F=@requireMap.@get(n);if(F)return F.exports;return @requireMap.@set(n,@createCommonJSModule(n,g)),g}})\n";
...
const int s_importMetaObjectCreateRequireCacheCodeLength = 1206;
static const JSC::Intrinsic s_importMetaObjectCreateRequireCacheCodeIntrinsic = JSC::NoIntrinsic;
const char* const s_importMetaObjectCreateRequireCacheCode = "(function (){\"use strict\";class _{id;parent;filename;children=[];paths=[];constructor(q){this.id=q;const O=q.lastIndexOf(\"/\");if(O!==-1&&q.length>O+1)this.filename=q.substring(O+1);else this.filename=q}get loaded(){return!0}require(q){return @internalRequire(@resolveSync(q,this.id))}get exports(){return @requireMap.@get(this.id)\?\?{}}set exports(q){@requireMap.@set(this.id,q)}}var g=new Map,d={};return new Proxy(d,{get(q,O){const b=@requireMap.@get(O);if(b)return b;const j=@Loader.registry.@get(O);if(j&&j.evaluated){const w=@Loader.getModuleNamespaceObject(j.module),z=w[@commonJSSymbol]===0||w.default\?.[@commonJSSymbol]\?w.default:w,L=@createCommonJSModule(O,z);return @requireMap.@set(O,L),L}return d[O]},set(q,O,b){return @requireMap.@set(O,b),!0},has(q,O){return @requireMap.@has(O)||@Loader.registry.@has(O)},deleteProperty(q,O){return g.@delete(O),@requireMap.@delete(O),@Loader.registry.@delete(O),!0},ownKeys(q){var O=[...@requireMap.@keys()];const b=[...@Loader.registry.@keys()];for(let j of b)if(!O.includes(j))@arrayPush(O,j);return O},getPrototypeOf(q){return null},getOwnPropertyDescriptor(q,O){if(@requireMap.@has(O)||@Loader.registry.@has(O))return{configurable:!0,enumerable:!0}}})})\n";
...
const char* const s_readableStreamPipeToCode = "(function (m){\"use strict\";if(!@isReadableStream(this))return @Promise.@reject(@makeThisTypeError(\"ReadableStream\",\"pipeTo\"));if(@isReadableStreamLocked(this))return @Promise.@reject(@makeTypeError(\"ReadableStream is locked\"));let E=@argument(1),R=!1,_=!1,S=!1,b;if(!@isUndefinedOrNull(E)){if(!@isObject(E))return @Promise.@reject(@makeTypeError(\"options must be an object\"));try{_=!!E[\"preventAbort\"],S=!!E[\"preventCancel\"],R=!!E[\"preventClose\"],b=E[\"signal\"]}catch(j){return @Promise.@reject(j)}if(b!==@undefined&&!@isAbortSignal(b))return @Promise.@reject(@makeTypeError(\"options.signal must be AbortSignal\"))}const u=@getInternalWritableStream(m);if(!@isWritableStream(u))return @Promise.@reject(@makeTypeError(\"ReadableStream pipeTo requires a WritableStream\"));if(@isWritableStreamLocked(u))return @Promise.@reject(@makeTypeError(\"WritableStream is locked\"));return @readableStreamPipeToWritableStream(this,u,R,_,S,b)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerRespondInReadableStateCode = "(function (f,R,_){\"use strict\";if(_.bytesFilled+R>_.byteLength)@throwRangeError(\"bytesWritten value is too great\");if(@assert(@getByIdDirectPrivate(f,\"pendingPullIntos\").isEmpty()||@getByIdDirectPrivate(f,\"pendingPullIntos\").peek()===_),@readableByteStreamControllerInvalidateBYOBRequest(f),_.bytesFilled+=R,_.bytesFilled<_.elementSize)return;@readableByteStreamControllerShiftPendingDescriptor(f);const g=_.bytesFilled%_.elementSize;if(g>0){const w=_.byteOffset+_.bytesFilled,h=@cloneArrayBuffer(_.buffer,w-g,g);@readableByteStreamControllerEnqueueChunk(f,h,0,h.byteLength)}_.buffer=@transferBufferToCurrentRealm(_.buffer),_.bytesFilled-=g,@readableByteStreamControllerCommitDescriptor(@getByIdDirectPrivate(f,\"controlledReadableStream\"),_),@readableByteStreamControllerProcessPullDescriptors(f)})\n";
...
const char* const s_readableByteStreamInternalsReadableByteStreamControllerPullIntoCode = "(function (R,E){\"use strict\";const A=@getByIdDirectPrivate(R,\"controlledReadableStream\");let d=1;if(E.BYTES_PER_ELEMENT!==@undefined)d=E.BYTES_PER_ELEMENT;const N=E.constructor,k={buffer:E.buffer,byteOffset:E.byteOffset,byteLength:E.byteLength,bytesFilled:0,elementSize:d,ctor:N,readerType:\"byob\"};var b=@getByIdDirectPrivate(R,\"pendingPullIntos\");if(b\?.isNotEmpty())return k.buffer=@transferBufferToCurrentRealm(k.buffer),b.push(k),@readableStreamAddReadIntoRequest(A);if(@getByIdDirectPrivate(A,\"state\")===@streamClosed){const L=new N(k.buffer,k.byteOffset,0);return @createFulfilledPromise({value:L,done:!0})}if(@getByIdDirectPrivate(R,\"queue\").size>0){if(@readableByteStreamControllerFillDescriptorFromQueue(R,k)){const L=@readableByteStreamControllerConvertDescriptor(k);return @readableByteStreamControllerHandleQueueDrain(R),@createFulfilledPromise({value:L,done:!1})}if(@getByIdDirectPrivate(R,\"closeRequested\")){const L=@makeTypeError(\"Closing stream has been requested\");return @readableByteStreamControllerError(R,L),@Promise.@reject(L)}}k.buffer=@transferBufferToCurrentRealm(k.buffer),@getByIdDirectPrivate(R,\"pendingPullIntos\").push(k);const _=@readableStreamAddReadIntoRequest(A);return @readableByteStreamControllerCallPullIfNeeded(R),_})\n";
...
const char* const s_eventSourceGetEventSourceCode = "(function (){\"use strict\";class H extends EventTarget{#j;#Q;#V;#W;#w;#A=!1;#X=null;#B=\"\";#M=\"\";#U=\"\";#L=!0;#F=0;#Y=0;#O=0;#Z=null;static#$(z){z.#H()}static#G(z,w){const A=z.data,B=A.#U\?`Last-Event-ID: ${A.#U}\\r\\n`:\"\",j=`GET ${w.pathname}${w.search} HTTP/1.1\\r\\nHost: bun\\r\\nContent-type: text/event-stream\\r\\nContent-length: 0\\r\\n${B}\\r\\n`,Q=z.write(j);if(Q!==j.length)A.#M=j.substring(Q)}static#J(z,w,A){for(;;){if(A>=w.length)return;let B=-1,j=w.indexOf(\"\\r\\n\",A);const Q=j+2;if(j>0)if(z.#F===0){const Y=parseInt(w.substring(A,j),16);if(Y===0){z.#Q=2,z.#X\?.end();return}B=Q+Y}else B=w.length;else{if(z.#B.length===0){z.#B+=w.substring(A);return}B=w.length}let F=w.substring(Q,B);A=B+2;let W=0,M=F.indexOf(\"\\n\\n\");if(M==-1){z.#B+=w.substring(Q);return}if(z.#B.length)z.#B+=F,F=z.#B,z.#B=\"\";let K=!0;while(K){const Y=F.substring(W,M);let L,X=\"\",G,Z=0,O=-1;for(;;){let U=Y.indexOf(\"\\n\",Z);if(U===-1){if(Z>=Y.length)break;U=Y.length}const V=Y.substring(Z,U);if(V.startsWith(\"data:\"))if(X.length)X+=`\\n${V.substring(5).trim()}`;else X=V.substring(5).trim();else if(V.startsWith(\"event:\"))L=V.substring(6).trim();else if(V.startsWith(\"id:\"))G=V.substring(3).trim();else if(V.startsWith(\"retry:\")){if(O=parseInt(V.substring(6).trim(),10),@isNaN(O))O=-1}Z=U+1}if(z.#U=G||\"\",O>=0)z.#O=O;if(X||G||L)z.dispatchEvent(new MessageEvent(L||\"message\",{data:X||\"\",origin:z.#j.origin,source:z,lastEventId:G}));if(F.length===M+2){K=!1;break}const J=F.indexOf(\"\\n\\n\",M+1);if(J===-1)break;W=M,M=J}}}static#z={open(z){const w=z.data;if(w.#X=z,!w.#A)H.#G(z,w.#j)},handshake(z,w,A){const B=z.data;if(w)H.#G(z,B.#j);else B.#Q=2,B.dispatchEvent(new ErrorEvent(\"error\",{error:A})),z.end()},data(z,w){const A=z.data;switch(A.#Q){case 0:{let B=w.toString();const j=B.indexOf(\"\\r\\n\\r\\n\");if(j===-1){A.#B+=B;return}if(A.#B.length)A.#B+=B,B=A.#B,A.#B=\"\";const Q=B.substring(0,j),F=Q.indexOf(\"\\r\\n\");if(F===-1){A.#Q=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Invalid HTTP request\")})),z.end();return}const W=Q.substring(0,F);if(W!==\"HTTP/1.1 200 OK\"){A.#Q=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(W)})),z.end();return}let M=F+1,K=!1,Y=-1;for(;;){let X=Q.indexOf(\"\\r\\n\",M);if(X===-1){if(M>=Q.length){if(!K)A.#Q=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has no MIME type and \"text/event-stream\" is required. Aborting the connection.`)})),z.end();return}X=Q.length}const G=Q.substring(M+1,X),Z=G.indexOf(\":\"),O=G.substring(0,Z),J=O.localeCompare(\"content-type\",@undefined,{sensitivity:\"accent\"})===0;if(M=X+1,J)if(G.endsWith(\" text/event-stream\"))K=!0;else{A.#Q=2,A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's response has a MIME type that is not \"text/event-stream\". Aborting the connection.`)})),z.end();return}else if(O.localeCompare(\"content-length\",@undefined,{sensitivity:\"accent\"})===0){if(Y=parseInt(G.substring(Z+1).trim(),10),@isNaN(Y)||Y<=0){A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Content-Length is invalid. Aborting the connection.`)})),z.end();return}if(K)break}else if(O.localeCompare(\"transfer-encoding\",@undefined,{sensitivity:\"accent\"})===0){if(G.substring(Z+1).trim()!==\"chunked\"){A.dispatchEvent(new ErrorEvent(\"error\",{error:new Error(`EventSource's Transfer-Encoding is invalid. Aborting the connection.`)})),z.end();return}if(Y=0,K)break}}A.#F=Y,A.#Q=1,A.dispatchEvent(new Event(\"open\"));const L=B.substring(j+4);if(H.#J(A,L,0),A.#F>0){if(A.#Y+=L.length,A.#Y>=A.#F)A.#Q=2,z.end()}return}case 1:if(H.#J(A,w.toString(),2),A.#F>0){if(A.#Y+=w.byteLength,A.#Y>=A.#F)A.#Q=2,z.end()}return;default:break}},drain(z){const w=z.data;if(w.#Q===0){const A=w.#B;if(A.length){const B=z.write(A);if(B!==A.length)z.data.#M=A.substring(B);else z.data.#M=\"\"}}},close:H.#K,end(z){H.#K(z).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Connection closed by server\")}))},timeout(z){H.#K(z).dispatchEvent(new ErrorEvent(\"error\",{error:new Error(\"Timeout\")}))},binaryType:\"buffer\"};static#K(z){const w=z.data;if(w.#X=null,w.#Y=0,w.#Q=2,w.#L){if(w.#Z)clearTimeout(w.#Z);w.#Z=setTimeout(H.#$,w.#O,w)}return w}constructor(z,w=@undefined){super();const A=new URL(z);this.#A=A.protocol===\"https:\",this.#j=A,this.#Q=2,process.nextTick(H.#$,this)}ref(){this.#Z\?.ref(),this.#X\?.ref()}unref(){this.#Z\?.unref(),this.#X\?.unref()}#H(){if(this.#Q!==2)return;const z=this.#j,w=this.#A;this.#Q=0,@Bun.connect({data:this,socket:H.#z,hostname:z.hostname,port:parseInt(z.port||(w\?\"443\":\"80\"),10),tls:w\?{requestCert:!0,rejectUnauthorized:!1}:!1}).catch((A)=>{if(super.dispatchEvent(new ErrorEvent(\"error\",{error:A})),this.#L){if(this.#Z)this.#Z.unref\?.();this.#Z=setTimeout(H.#$,1000,this)}})}get url(){return this.#j.href}get readyState(){return this.#Q}close(){this.#L=!1,this.#Q=2,this.#X\?.unref(),this.#X\?.end()}get onopen(){return this.#w}get onerror(){return this.#V}get onmessage(){return this.#W}set onopen(z){if(this.#w)super.removeEventListener(\"close\",this.#w);super.addEventListener(\"open\",z),this.#w=z}set onerror(z){if(this.#V)super.removeEventListener(\"error\",this.#V);super.addEventListener(\"error\",z),this.#V=z}set onmessage(z){if(this.#W)super.removeEventListener(\"message\",this.#W);super.addEventListener(\"message\",z),this.#W=z}}return Object.defineProperty(H.prototype,\"CONNECTING\",{enumerable:!0,value:0}),Object.defineProperty(H.prototype,\"OPEN\",{enumerable:!0,value:1}),Object.defineProperty(H.prototype,\"CLOSED\",{enumerable:!0,value:2}),H[Symbol.for(\"CommonJS\")]=0,H})\n";
