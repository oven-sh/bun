 ...
 *
 * @see {@link $native} for native Rust bindings.
 ...
 * @see `src/codegen/replacements.ts` for the script that performs replacement of this funciton.
 *
 * @param filename name of the `.rs` file containing the function: the bare
 *                 basename, or a path relative to `src/` when the basename is
 *                 ambiguous.
 ...
 * @returns whatever the binding function returns.
 */
declare function $native<T = any>(filename: NativeFilenameRust, symbol: string): T;
declare function $newCppFunction<T = (...args: any) => any>(
  filename: NativeFilenameCPP,
  symbol: string,
  argCount: number,
): T;
declare function $newNativeFunction<T = (...args: any) => any>(
