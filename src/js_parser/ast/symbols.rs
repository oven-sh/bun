...
impl<'a, const TYPESCRIPT: bool, J: JsxT, const SCAN_ONLY: bool> P<'a, TYPESCRIPT, J, SCAN_ONLY> {
    ...
    pub fn find_symbol_with_record_usage<const RECORD_USAGE: bool>(
        ...
        #[cfg(any())] // TODO(b2-blocked): Scope::get_member_hash/get_member_with_hash (round-A prehashed adapter), get_or_put_member_with_hash
        {
            ...
            let ref_: Ref = 'brk: {
                // TODO(port): lifetime — Scope.parent is a backref raw pointer; using *mut Scope.
                ...
                while let Some(scope) = unsafe { current.as_ref() } {
                    ...
                    if let Some(ts_namespace) = scope.ts_namespace.as_ref() {
                        // SAFETY: ARENA-class backref per LIFETIMES.tsv
                        let ts = unsafe { ts_namespace.as_ref() };
                        if let Some(member) = ts.exported_members.get(name) {
                            if member.data.is_enum() == ts.is_enum_scope {
                                ...
                                // SAFETY: re-borrow ts_namespace after &mut self.
                                let ts_mut = unsafe { ts_namespace.as_ptr().as_mut().unwrap() };
            ...
            return Ok(FindSymbolResult { r#ref: ref_, declare_loc, is_inside_with_scope });
        }
        let _ = (loc, name);
        todo!("b2-blocked: Scope prehashed-adapter API")
    }
}
