...
//! struct field here points into either the source text or the
...
// Full Macro impl lives in *_jsc; this stub re-exposes the JSC-free constants and a
...
pub mod Macro {
    ...
    /// Equivalent to `bundler_jsc::PluginRunner::MacroJSCtx` (= `JSC.JSValue`).
    ///
    ...
    /// faithful port of the original `visitExpr` macro hooks without an upward
    /// import. `javascript_object` is surfaced here so `Transpiler::parse` can
    /// thread `this_parse.macro_js_ctx` through without this crate depending
    /// on `bun_jsc::JSValue`.
    pub struct MacroContext {
        ...
        /// (matches the original process-lifetime `default_allocator`);
...
// (the original sidestepped this via result-location semantics; Rust does not). Boxing the
...
pub mod defines {
    ...
    pub struct DotDefine {
        // The original stored borrowed slices into static tables / user-define
    ...
    /// Packed `u8` — `_padding: u3, valueless: bool,
    ...
    /// Packed LSB-first → bit positions below match the original layout exactly.
    #[repr(transparent)]
    ...
    pub struct DefineData {
        pub value: ExprData,
        // The original stored a separate name pointer + length pair
        // borrowing into caller-owned strings (a 48→40-byte
    ...
    /// Named-init shim (mirrors anonymous-struct init).
    ...
    impl DefineData {
        ...
        pub fn init_static_string(str: &'static E::EString) -> DefineData {
            ...
            flags.set_can_be_removed_if_unused(true);
            DefineData {
                // Expr.Data.e_string stores a non-const `E.String` pointer; the static is never mutated.
    ...
    impl Define {
        ...
        // PORT NOTE: original took the iterator type as a compile-time param; dropped here.
...
pub mod defines_full_draft {
    ...
    #[derive(Clone)]
    pub struct DotDefine {
        // The original stored borrowed slices into the user-define key strings;
    ...
    bitflags::bitflags! {
        // Packed `u8`: `_padding: u3, valueless, can_be_removed_if_unused,
    ...
    impl DefineData {
        ...
        pub fn init_static_string(str_: &'static E::String) -> DefineData {
            DefineData {
                // Expr.Data stores StoreRef (NonNull); the static is never mutated.
        ...
        pub fn parse(
            ...
            // The original deep-cloned then checked `isPrimitiveLiteral()`.
    ...
    impl Define {
        // Rust callers import `DefineData` directly.
        ...
        pub fn insert(
            ...
            if let Some(last_dot) = strings::last_index_of_char(key, b'.') {
                ...
                // The original allocated slices borrowing the input key; the Rust
        ...
        pub fn init(
            ...
            // (Iterate over `StringHashMap` — consume the inner map.)
            ...
            // (`ArrayHashMap` has no `IntoIterator`; walk insertion-order entries.)
...
// Only the slot-assignment helpers the parser calls live here;
