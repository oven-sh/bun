...
// Note: Zig used `std.ArrayListUnmanaged` with `p.arena` for the four
...
impl<'a> ConvertESMExportsForHmr<'a> {
    // Note: `<P>` unbounded generic → concrete `P<'p, TS, SCAN>`. Zig's
    // `p: anytype` also accepted `AstBuilder`; the Rust `AstBuilder` instead
        ...
        if gop.found_existing {
            ...
            if items_len > 0 {
                if stmt.items.is_empty() {
                    ...
                    // Note: Zig `std.mem.concat` — allocate concatenated slice in arena.
            ...
            if namespace_ref.is_valid() {
                if !stmt.namespace_ref.is_valid() {
                    ...
                    // Note: Zig `@hasField(@typeInfo(@TypeOf(p)).pointer.child, "symbol_uses")`
    ...
    pub(crate) fn finalize<'p, const TS: bool, const SCAN: bool>(
        &mut self,
        p: &mut P<'p, TS, SCAN>,
        // Note: Zig took `all_parts: []Part` and freely re-derived
        ...
        for part in head_parts.iter_mut() {
            ...
            // Note: reshaped for borrowck — Zig zipped keys()/values(); index loop avoids
        ...
        // Note: Zig assigned the ArrayList's `items` slice directly. `Stmt` is `Copy`;
