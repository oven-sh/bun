...
#[allow(non_snake_case)]
pub mod Runtime {
    // `Features` and the data-shaped Runtime types are canonical in
    // `bun_ast::runtime` so the printer (and any non-parser caller) sees one
    // definition. Re-exported here so `parser::Runtime::{Features, Imports,
    // ...}` and `bun_ast::runtime::{...}` are the same nominal types.
    pub use bun_ast::runtime::Features;
    pub(crate) use bun_ast::runtime::{Imports, ReplaceableExport};
}
