...
impl<'a, const TYPESCRIPT: bool, const SCAN_ONLY: bool> P<'a, TYPESCRIPT, SCAN_ONLY> {
    ...
    pub(crate) fn visit_stmts(
        ...
        let old_in_react_compiler_candidate = p.in_react_compiler_candidate;
        if rc_pending.is_some() {
            p.in_react_compiler_candidate = true;
        }
        ...
        let input = core::mem::replace(stmts, ListManaged::new_in(p.arena));
        *stmts = p.mangle_stmts(input, kind);
        Ok(())
    }
}
