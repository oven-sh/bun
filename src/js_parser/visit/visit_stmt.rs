...
impl<'a, const TYPESCRIPT: bool, const SCAN_ONLY: bool> P<'a, TYPESCRIPT, SCAN_ONLY> {
    ...
    fn s_import(
        ...
        if let Some(star_name_loc) = data.star_name_loc {
        ...
        if let Some(default_name) = data.default_name {
            let name_ref = default_name.ref_.expect("infallible: ref bound");
        ...
        if !items.is_empty() {
            for item in items.iter() {
                let name_ref = item.name.ref_.expect("infallible: ref bound");
                p.record_declared_symbol(name_ref);
                let name = p.load_name_from_ref(name_ref);
    ...
    fn s_enum(
        ...
        let name_ref = data.name.ref_.expect("infallible: ref bound");
