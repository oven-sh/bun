...
// `ListManaged.fromOwnedSlice` → `visitStmts` → `.items` pattern: copy the
...
// `StmtNodeList = StoreSlice<Stmt>` (arena-owned). The original `ListManaged.fromOwnedSlice`
...
// Originally `pub fn VisitStmt(comptime ts, comptime jsx, comptime scan_only) type { return struct { ... } }`
...
impl<'a, const TYPESCRIPT: bool, const SCAN_ONLY: bool> P<'a, TYPESCRIPT, SCAN_ONLY> {
    ...
    pub fn visit_and_append_stmt(
        ...
        // Originally `switch (@as(Stmt.Tag, stmt.data))` with `inline else` reflection over @tagName.
        ...
        match data_copy {
            ...
            // Originally `inline .s_enum, .s_local => |tag| return @field(visitors, @tagName(tag))(p, stmts, stmt, @field(stmt.data, @tagName(tag)), was_after_after_const_local_prefix)`
            ...
            // Originally `inline else => |tag| return @field(visitors, @tagName(tag))(p, stmts, stmt, @field(stmt.data, @tagName(tag)))`
    ...
    fn s_export_equals(
        ...
        // Originally `p.@"module.exports"(stmt.loc)` — mapped to `module_exports`
        // PORT NOTE: original evaluates lhs before rhs at the call site; preserve that order
    ...
    fn s_export_from(
        ...
        if p.options.features.replace_exports.count() > 0 {
            ...
            // TODO(port): dead branch in the original — `data.items.len = j;` runs first, so
    ...
    fn s_export_default(
        ...
        // Originally `defer { if (data.default_name.ref) |ref| p.recordDeclaredSymbol(ref) catch unreachable; }`
        ...
        // Originally `defer { p.is_control_flow_dead = orig_dead; }`
            ...
            js_ast::StmtOrExpr::Stmt(s2) => {
                ...
                match s2_data {
                    StmtData::SFunction(mut func_ref) => {
                        ...
                        // `func.func` to its default. The spec copies `func.func` by value into
                    ...
                    StmtData::SClass(mut class_ref) => {
                        ...
                        if p.options.features.server_components.wraps_exports() {
                            // TODO(port): spec mutates `data.value` *after* pushing `stmt` —
    ...
    fn s_function(
        ...
        // Spec unconditionally points p.react_refresh.hook_ctx_storage
        // at this stack-local before visit_func and defer-restores it. Field is now
        // `Option<NonNull<_>>` (Copy) matching the original `?*?HookContext`, so save/set/restore
            ...
            && p.enclosing_namespace_arg_ref.is_some()
        {
            ...
            if remove_overwritten {
                // Originally `defer { ... }` — restore on early return.
        ...
        if p.options.features.react_fast_refresh {
            ...
            if p.current_scope == p.module_scope {
                ...
                // p.react_refresh on the OOM path (the original `defer` covers all exits).
        ...
        // Originally `defer p.react_refresh.hook_ctx_storage = prev;`
        p.react_refresh.hook_ctx_storage = prev_hook_storage;
        // Originally `defer { if (mark_as_dead) p.is_control_flow_dead = original_is_dead; }`
        if mark_as_dead {
            p.is_control_flow_dead = original_is_dead;
        }
        rr
    ...
    fn s_local(
        ...
        // Spec: drop the whole statement when every decl was
    ...
    fn s_expr(
        ...
        // Originally `defer p.stmt_expr_value = .{ .e_missing = .{} };`
            ...
            if is_top_level {
                if matches!(data.value.data, js_ast::ExprData::EBinary(_)) {
                    ...
                    if to_convert != u32::MAX {
                        ...
                        // PORT NOTE: original directly swaps backing storage; commonjs_replacement_stmts
    ...
    fn s_for_in(
    ...
    ) -> Result<(), Error> {
        {
            p.push_scope_for_visit_pass(js_ast::scope::Kind::Block, stmt.loc)
                .expect("unreachable");
            // Originally `defer p.popScope();` — restructured: pop at end of block
    ...
    fn s_enum(
        ...
        // Originally `defer p.popScope();` — moved to end (no early returns).
