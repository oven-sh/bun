...
pub fn string_to_js(s: &E::String, global: &JSGlobalObject) -> Result<JSValue, ToJSError> {
    // TODO(port): Zig mutates `s` via `resolveRopeIfNeeded(allocator)`; `data_to_js`
    // only has `&ExprData` so cannot hand out `&mut`, and there is no bump arena in
    // scope here. Phase B should either thread a bump arena + interior-mut rope or
    // resolve ropes before reaching here. For now, assert non-rope (macros only feed
    // resolved literals on the current call paths).
    debug_assert!(
        s.next.is_none(),
