    ...
    let _line_offset_tables_guard = scopeguard::guard(
        ...
        |p| {
            if GENERATE_SOURCE_MAP {
                ...
                // SAFETY: `tables` was freshly generated by `get_source_map_builder`
                // (never borrowed from a caller on this path) and this guard is the
                // sole drop site; the `ManuallyDrop` field is not read after this.
