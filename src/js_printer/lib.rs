    ...
    let _line_offset_tables_guard = scopeguard::guard(
        ...
        |p| {
            if GENERATE_SOURCE_MAP {
                ...
                // SAFETY: when `opts.line_offset_tables` was `None`,
                // `get_source_map_builder` freshly generated this table and this is
                // its sole drop site. When `Some(&borrowed)` was provided (linker
                // path), the field is a bitwise `ptr::read` alias — see the comment
                // at `get_source_map_builder` line 7947. PRE-EXISTING: that path
                // would double-free here; current callers of `print_ast`/
                // `print_common_js` pass `None`.
