            ...
            match &expr.data {
                ...
                ExprData::ERequireString(e) => {
                    self.print_require_or_import_expr(
                        ...
                        WasUnwrappedRequire::from_bool(e.unwrapped_id.is_some()),
                        &[],
                        Expr::EMPTY,
                        level,
                        flags,
        ...
        pub(crate) fn print_binding(&mut self, binding: Binding, tlm: TopLevelAndIsExport) {
            ...
            match &binding.data {
                ...
                BindingData::BIdentifier(b) => {
                    ...
                    if Self::MAY_HAVE_MODULE_INFO && tlm.is_export == IsExport::Yes {
        ...
        pub(crate) fn print_stmt(&mut self, stmt: Stmt) -> crate::Result<()> {
            ...
            match &stmt.data {
                ...
                StmtData::SLocal(s) => {
                    ...
                    self.print_space_before_identifier();
                    self.add_source_mapping(stmt.loc);
                    self.print_decl_stmt(IsExport::from_bool(s.is_export), s.kind, s.decls.slice());
                }
                StmtData::SIf(s) => {
                    self.print_indent();
