...
use crate::unsafe_string_view::UnsafeStringView;
use crate::{
    CallFrame, JSGlobalObject, JSValue, JsClass, JsResult, StringJsc as _, UnsafeStringViewJsc as _,
};
...
impl BuildMessage {
    ...
    pub fn to_string_fn(&self, global: &JSGlobalObject) -> JSValue {
        ...
        let mut str = UnsafeStringView::init(&text);
        ...
        let mut str = UnsafeStringView::init(unsafe { bun_core::ffi::slice(ptr, len) });
        str.set_output_encoding();
        str.to_external_value(global)
    }

    /// `BuildMessage.create` — clone `msg` into a
    ...
    pub fn to_primitive(
        ...
        if !args.is_empty() {
            ...
            let str = args[0].get_unsafe_string_view(global)?;
    ...
    pub fn generate_position_object(msg: &bun_ast::Msg, global: &JSGlobalObject) -> JSValue {
        ...
        object.put(
            global,
            b"lineText",
            UnsafeStringView::init(location.line_text.as_deref().unwrap_or(b"")).to_js(global),
        );
        object.put(
            global,
            b"file",
            UnsafeStringView::init(&location.file).to_js(global),
        );
        object.put(
            global,
            b"namespace",
            UnsafeStringView::init(location.namespace).to_js(global),
        );
    ...
    pub fn get_message(&self, global: &JSGlobalObject) -> JsResult<JSValue> {
        Ok(UnsafeStringView::init(&self.msg.data.text).to_js(global))
    }

    #[crate::host_fn(getter)]
    pub fn get_level(&self, global: &JSGlobalObject) -> JsResult<JSValue> {
        Ok(UnsafeStringView::init(self.msg.kind.string()).to_js(global))
    }
}
