...
unsafe extern "C" {
    ...
    safe fn WebCore__DOMFormData__toQueryString(
        ...
        arg1: *mut c_void,
        arg2: extern "C" fn(arg0: *mut c_void, arg1: *mut BunString),
    );
    ...
    // handles; `&BunString` is ABI-identical to non-null `*const BunString` and
...
impl DOMFormData {
    ...
    #[track_caller]
    pub fn create_from_url_query(global: &JSGlobalObject, query: &BunString) -> JsResult<JSValue> {
        crate::from_js_host_call(global, || {
            WebCore__DOMFormData__createFromURLQuery(global, query)
        })
    }

    // PORT NOTE: Zig's `comptime Ctx: type, ctx: Ctx, comptime callback: fn(Ctx, BunString)`
    // is Zig's spelling of a monomorphized closure. Reshaped to `FnMut(BunString)` — the
    ...
    where
        F: FnMut(BunString),
    {
        extern "C" fn run<F: FnMut(BunString)>(c: *mut c_void, str_: *mut BunString) {
            // SAFETY: `c` is the `&mut F` passed below.
            let cb = unsafe { bun_ptr::callback_ctx::<F>(c) };
            // SAFETY: `str_` is a valid non-null *BunString for the synchronous callback scope.
            cb(unsafe { *str_ });
        }
    ...
    pub fn append(&mut self, name_: &BunString, value_: &BunString) {
    ...
    where
        F: FnMut(BunString, FormDataEntry<'_, B>),
    {
            ...
            let value = if is_blob == 0 {
                ...
                FormDataEntry::File {
                    ...
                    filename: if filename.is_null() {
                        ...
                        // SAFETY: non-null filename points to a valid BunString for this call.
                        unsafe { *filename }
                    },
                }
            };

            // SAFETY: name_ is always a valid non-null *BunString for the callback scope.
...
type ForEachFunction = extern "C" fn(
    ...
    name: *mut BunString,
    value_ptr: *mut c_void,
    filename: *mut BunString, // nullable
    is_blob: u8,
);
...
pub enum FormDataEntry<'a, B> {
    String(BunString),
    File { blob: &'a B, filename: BunString },
}
