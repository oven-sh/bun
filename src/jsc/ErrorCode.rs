...
impl ErrorCode {
    ...
    pub const WORKER_MESSAGING_ERRORED: ErrorCode = ErrorCode(318);
    /// `ERR_WORKER_MESSAGING_FAILED` (instanceof Error)
    pub const WORKER_MESSAGING_FAILED: ErrorCode = ErrorCode(319);
    /// `ERR_WORKER_MESSAGING_SAME_THREAD` (instanceof Error)
    pub const WORKER_MESSAGING_SAME_THREAD: ErrorCode = ErrorCode(320);
    /// `ERR_WORKER_MESSAGING_TIMEOUT` (instanceof Error)
    pub const WORKER_MESSAGING_TIMEOUT: ErrorCode = ErrorCode(321);
    /// `ERR_POSTGRES_CONNECTION_FAILED` (instanceof Error)
    pub const POSTGRES_CONNECTION_FAILED: ErrorCode = ErrorCode(322);
    /// `ERR_MYSQL_CONNECTION_FAILED` (instanceof Error)
    pub const MYSQL_CONNECTION_FAILED: ErrorCode = ErrorCode(323);
    /// `ERR_POSTGRES_CONNECTION_REFUSED` (instanceof Error)
    pub const POSTGRES_CONNECTION_REFUSED: ErrorCode = ErrorCode(324);
    /// `ERR_MYSQL_CONNECTION_REFUSED` (instanceof Error)
    pub const MYSQL_CONNECTION_REFUSED: ErrorCode = ErrorCode(325);
    /// `ERR_HTTP2_GOAWAY_SESSION` (instanceof Error)
    pub const HTTP2_GOAWAY_SESSION: ErrorCode = ErrorCode(326);
    /// `ERR_TLS_ALPN_CALLBACK_INVALID_RESULT` (instanceof TypeError)
    pub const TLS_ALPN_CALLBACK_INVALID_RESULT: ErrorCode = ErrorCode(327);
    /// `ERR_PROXY_TUNNEL` (instanceof Error)
    pub const PROXY_TUNNEL: ErrorCode = ErrorCode(328);
    /// `ERR_FS_CP_EEXIST` (instanceof Error)
    pub const FS_CP_EEXIST: ErrorCode = ErrorCode(329);
    /// `ERR_FS_CP_SYMLINK_TO_SUBDIRECTORY` (instanceof Error)
    pub const FS_CP_SYMLINK_TO_SUBDIRECTORY: ErrorCode = ErrorCode(330);
    /// `ERR_DIR_CONCURRENT_OPERATION` (instanceof Error)
    pub const DIR_CONCURRENT_OPERATION: ErrorCode = ErrorCode(331);
    /// `ERR_INVALID_BUFFER_SIZE` (instanceof RangeError)
    pub const INVALID_BUFFER_SIZE: ErrorCode = ErrorCode(332);
    /// `ERR_TRACE_EVENTS_CATEGORY_REQUIRED` (instanceof TypeError)
    pub const TRACE_EVENTS_CATEGORY_REQUIRED: ErrorCode = ErrorCode(333);
    /// `ERR_TRACE_EVENTS_UNAVAILABLE` (instanceof Error)
    pub const TRACE_EVENTS_UNAVAILABLE: ErrorCode = ErrorCode(334);
