...
// PORT NOTE: any u16 bit-pattern is a valid value (the discriminants are bitflags).
// A `#[repr(u16)] enum` would be UB for unlisted values, so use a transparent newtype
