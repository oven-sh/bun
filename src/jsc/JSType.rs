...
// PORT NOTE: any u8 value is a valid JSType (values are read directly from
// JSCell::m_type via FFI, including embedder-defined types). A plain
// `#[repr(u8)] enum` would be UB for unknown discriminants, so this is a
...
impl JSType {
    ...
    /// This means that we don't have native bindings for the type yet, but it
    /// implements .toJSON()
...
impl JSType {
    ...
    /// Stringify `arrayBuffer.typed_array_type` — `JSType` is a
