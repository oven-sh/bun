...
impl JSUint8Array {
    pub fn ptr(&self) -> *mut u8 {
        ...
        // pointer lives at a fixed byte offset computed by the C++ codegen (`sizes`).
        // `byte_add` preserves provenance (unlike round-tripping through `usize`).
        unsafe {
    ...
    pub fn slice(&mut self) -> &mut [u8] {
        // PORT NOTE: detached/empty JSUint8Array has ptr=null, len=0.
    ...
    // PORT NOTE: the global allocator IS mimalloc, so `Box<[u8]>` encodes the
    // required default_allocator ownership.
