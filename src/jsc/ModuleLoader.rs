...
/// `VirtualMachine.processFetchLog(global, specifier, log, &errorable, err)` —
/// synthesizes a JS error from the parser/resolve `log` of transpiling
/// `specifier` and writes it into `errorable` so the C++ side
