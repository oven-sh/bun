...
pub fn transpileSourceCode(
    ...
    if (comptime disable_transpilying) {
        if (!(loader.isJavaScriptLike() or loader == .toml or loader == .yaml or loader == .xml or loader == .json5 or loader == .text or loader == .json or loader == .jsonc)) {
    ...
    switch (loader) {
        .js, .jsx, .ts, .tsx, .json, .jsonc, .toml, .yaml, .xml, .json5, .text, .md => {
            ...
            if (loader == .json or loader == .jsonc or loader == .toml or loader == .yaml or loader == .xml or loader == .json5) {
