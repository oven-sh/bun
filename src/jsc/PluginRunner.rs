...
pub struct PluginRunner {
    ...
    // PORT NOTE: per-instance allocator field dropped per PORTING.md
    // (global mimalloc).
}
...
impl PluginResolver for PluginRunner {
    fn on_resolve(
        ...
        // `bun_core::String` is `Copy` (no `Drop`), so RAII-wrap the +1 WTF ref
        // across every remaining `?` / early-return.
        ...
        // PORT NOTE: `FsPath<'static>` borrows, so the formatted buffer is
        // leaked to model the caller-owns-forever contract (the linker arena
        // owns these for the build and never frees them).
        // PERF(port): heap-format-and-leak — profile in Phase B.
