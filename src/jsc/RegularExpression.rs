...
//
// Those lower-tier crates cannot name `jsc::RegularExpression`. The bodies
// live here as `#[no_mangle]` Rust-ABI fns, declared `extern "Rust"` on the
// low-tier side; link-time resolved.
// ──────────────────────────────────────────────────────────────────────────

#[unsafe(no_mangle)]
pub fn __bun_regex_compile(pattern: BunString) -> Option<core::ptr::NonNull<()>> {
    // `jsc::initialize(false)` before first compile (idempotent).
