...
impl SuppressMicrotaskDrain {
    /// `vm` is the live per-thread `*mut VirtualMachine` backref.
    #[inline]
    fn new(vm: *mut VirtualMachine) -> Self {
        // SAFETY: `vm` is the live per-thread VM (`SpawnSyncEventLoop.vm`).
        let prev = unsafe { (*vm).suppress_microtask_drain.replace(true) };
        Self { vm, prev }
    }
}
...
impl Drop for SuppressMicrotaskDrain {
    ...
    fn drop(&mut self) {
        // SAFETY: `vm` is the live per-thread VM (`SpawnSyncEventLoop.vm`).
        unsafe { (*self.vm).suppress_microtask_drain.replace(self.prev) };
    }
}
...
pub struct SpawnSyncEventLoop {
    ...
    // SAFETY: heap-owned (`bun_core::heap::{into_raw, take}` in `init`/`Drop`).
    event_loop: *mut EventLoop,

    /// `*mut VirtualMachine` backref (set in `init`/`prepare`).
    vm: *mut VirtualMachine,
...
impl SpawnSyncEventLoop {
    ...
    pub unsafe fn init(this: &mut core::mem::MaybeUninit<Self>, vm: *mut VirtualMachine) {
        ...
        // CRITICAL: On Windows, store our isolated loop pointer in `uws_loop`.
        let event_loop = {
            // SAFETY: `vm` is the live per-thread VM (caller contract).
            let vm_ref = unsafe { &mut *vm };
            let mut el = Box::new(EventLoop::default());
            el.global = NonNull::new(vm_ref.global);
            el.virtual_machine = NonNull::new(core::ptr::from_mut(vm_ref));
            #[cfg(windows)]
            {
                el.uws_loop = NonNull::new(loop_.as_ptr());
            }
            bun_core::heap::into_raw(el)
        };
        ...
        // `this.event_loop` is the live heap-owned `*mut EventLoop` allocated
        // immediately above — never null on a successful create.
        debug_assert!(!this.event_loop.is_null(), "spawn-sync event loop alloc");
        let (tag, ptr) = EventLoopHandle::Js {
            owner: crate::event_loop::js_event_loop_handle(this.event_loop),
        }
        .into_tag_ptr();
    ...
    /// `*mut EventLoop`, heap-owned by `self` (allocated in `init`, freed in
    /// `Drop`). See `js_bun_spawn_bindings::spawn_maybe_sync`.
    ///
    /// Intentionally raw-ptr (no `&`-returning variant): the `EventLoop` is
    /// mutated re-entrantly while this struct is live.
    #[inline]
    pub fn event_loop_ptr(&self) -> *mut EventLoop {
        self.event_loop
    }

    /// `*mut VirtualMachine` backref (set in `init`/`prepare`).
    ///
    /// Intentionally raw-ptr (no `&`-returning variant): the VM is mutated
    /// re-entrantly during `tick_with_timeout` (subprocess callbacks → JS) — a
    /// `&VirtualMachine` here would alias under Stacked Borrows.
    #[inline]
    pub fn vm_ptr(&self) -> *mut VirtualMachine {
        self.vm
    }
...
impl Drop for SpawnSyncEventLoop {
    fn drop(&mut self) {
        ...
        // SAFETY: paired with `heap::into_raw` in `init`.
        drop(unsafe { bun_core::heap::take(self.event_loop) });
...
impl SpawnSyncEventLoop {
    ...
    /// # Safety
    /// `vm` must point to the live per-thread `VirtualMachine`.
    pub unsafe fn prepare(&mut self, vm: *mut VirtualMachine) {
        {
            // Re-bind `event_loop.{global, virtual_machine}` to `vm`.
            // SAFETY: `vm` is the live per-thread VM (caller contract);
            // `event_loop` is heap-owned by `self` (see `init`).
            let vm_ref = unsafe { &mut *vm };
            // SAFETY: `event_loop` is heap-owned by `self` (see `init`).
            let el = unsafe { &mut *self.event_loop };
            el.global = NonNull::new(vm_ref.global);
            el.virtual_machine = NonNull::new(core::ptr::from_mut(vm_ref));
        }
        ...
        // SAFETY: `vm` is the live per-thread VM (caller contract).
        self.original_event_loop_handle = unsafe { (*vm).event_loop_handle.and_then(NonNull::new) };
        #[cfg(unix)]
        ...
        // SAFETY: `vm` is the live per-thread VM (caller contract).
        unsafe { (*vm).event_loop_handle = new_handle.map(NonNull::as_ptr) };
    }

    /// Restore the original event loop handle after spawnSync completes.
    /// # Safety
    /// `vm` must point to the live per-thread `VirtualMachine` and
    /// `prev_event_loop` to its previous (still-live) `event_loop`.
    pub unsafe fn cleanup(&mut self, vm: *mut VirtualMachine, prev_event_loop: *mut EventLoop) {
        // SAFETY: `vm` is the live per-thread VM; `prev_event_loop` is its
        // previous `event_loop` pointer (a `*mut EventLoop` into
        // `regular_event_loop`/`macro_event_loop`).
        unsafe {
            (*vm).event_loop_handle = self.original_event_loop_handle.map(NonNull::as_ptr);
            (*vm).install_event_loop(prev_event_loop);
        }
    ...
    pub fn handle(&mut self) -> EventLoopHandle {
        // `self.event_loop` is the live heap-owned `*mut EventLoop`
        ...
        EventLoopHandle::Js {
            owner: crate::event_loop::js_event_loop_handle(self.event_loop),
        }
    }
}
...
impl SpawnSyncEventLoop {
    ...
    pub fn tick_with_timeout(&mut self, timeout: Option<&Timespec>) -> TickState {
        ...
        // SAFETY: `event_loop` is heap-owned by `self` (see `init`); no other
        // borrow of it is live here.
        unsafe { &mut *self.event_loop }.tick_tasks_only();
