...
unsafe extern "C" {
    safe fn URLSearchParams__create(
        global_object: &JSGlobalObject,
        init: &UnsafeStringView,
    ) -> JSValue;
    ...
    safe fn URLSearchParams__toString(
        ...
        ctx: *mut c_void,
        callback: extern "C" fn(ctx: *mut c_void, str: *const UnsafeStringView),
    );
}

impl URLSearchParams {
    pub fn create(global_object: &JSGlobalObject, init: UnsafeStringView) -> JSValue {
    ...
    pub fn to_string<Ctx>(
        &mut self,
        ctx: &mut Ctx,
        callback: fn(ctx: &mut Ctx, str: UnsafeStringView),
    ) {
        // PORT NOTE: reshaped — the original captured `callback` statically so the
        // C trampoline only needed `ctx` through the void*. Rust cannot take a fn
        // pointer as a const generic, so pack (ctx, callback) on the stack and pass
        // that instead.
        struct Wrap<'a, Ctx> {
            ctx: &'a mut Ctx,
            callback: fn(&mut Ctx, UnsafeStringView),
        }

        extern "C" fn cb<Ctx>(c: *mut c_void, str: *const UnsafeStringView) {
            // SAFETY: `c` is the &mut Wrap<Ctx> we passed below; `str` is a valid
            // *const UnsafeStringView for the duration of this callback (borrowed from C++).
