...
impl VM {
    ...
    /// End-of-job WeakRef release. A checkpoint inside a nested domain run is
    /// not the end of the outer frame's job, so targets it observed stay alive
    /// until its own checkpoint.
