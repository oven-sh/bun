...
// In Rust the file itself is the module, so items live at module level.
// Callers use `bun_jsc::wtf::foo()`.
//
...
// without a T6 dep. Re-exported here to keep the original namespace shape.
...
// to keep the namespace shape (`bun_jsc::wtf::parse_double`).
pub use bun_core::fmt::{InvalidCharacter, parse_double};
/// Back-compat alias for the original error-set name.
...
/// Back-compat alias for the original namespace shape.
pub type ParseDateError = bun_core::wtf::InvalidDate;

// it without a bun_jsc dep. Re-exported here to keep the original namespace shape.
