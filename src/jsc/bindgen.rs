...
// them into `Vec<BunType>` even when `align_of::<BunType>() != align_of::<ExternType>()`.
...
// ──────────────────────────────────────────────────────────────────────────
// The "Bindgen*" adapter family shares one shape: an associated
// `BunType`/`ExternType` plus `convert_from_extern`, and optionally
// `OptionalBunType`/`OptionalExternType`/`convert_optional_from_extern`. In
// Rust this is a trait. The optional half is split into a separate trait
// (`BindgenOptionalRepr`) that a `Child` may opt into.
// ──────────────────────────────────────────────────────────────────────────
...
pub trait Bindgen {
    ...
    /// `true` when `BunType` and `ExternType` are layout-identical. Enables
    /// `BindgenArray`'s allocation-reuse fast path. Defaults to `false`;
    /// override per adapter.
    const SAME_REPR: bool = false;

    fn convert_from_extern(extern_value: Self::ExternType) -> Self::BunType;
}
...
impl Bindgen for BindgenStrongAny {
    type BunType = Strong;
    // Nullable `*mut jsc::strong::Impl` — must be single-word for #[repr(C)] union
    // placement, so `Option<NonNull<T>>` (niche-optimized), NOT `Option<*mut T>`.
...
impl BindgenOptionalRepr for BindgenStrongAny {
    type OptionalBunType = jsc::strong::Optional;
...
// `ExternTaggedUnion<(u8, Child::ExternType)>` and produce `Option<Child::BunType>`.
//
// PORT NOTE: stable Rust cannot specialize on "does Child impl
// BindgenOptionalRepr", so the bindgen codegen emits `BindgenOptional<Child>`
// vs `BindgenOptionalCustom<Child>` explicitly per call site.
impl<Child: Bindgen> Bindgen for BindgenOptional<Child> {
    type BunType = Option<Child::BunType>;
...
/// (i.e. that implement [`BindgenOptionalRepr`]).
...
impl<Child: BindgenOptionalRepr> Bindgen for BindgenOptionalCustom<Child> {
    type BunType = Child::OptionalBunType;
...
impl Bindgen for BindgenString {
    type BunType = WTFString;
    // Nullable `WTFStringImpl*` — `Option<NonNull<_>>` for single-word FFI layout.
...
impl BindgenOptionalRepr for BindgenString {
    type OptionalBunType = ExternalSharedOptional<WTFStringImplStruct>;
    type OptionalExternType = <Self as Bindgen>::ExternType;
...
//
// These are reflective combinators with no Rust equivalent — the Rust side
// must be generated per arity (or by a proc-macro from the bindgen codegen).
// ──────────────────────────────────────────────────────────────────────────

// PORT NOTE: the bindgen TS codegen emits a concrete `enum` + `#[repr(C)]`
...
/// `#[repr(C)] struct { data: ExternUnion(field_types), tag: u8 }`
///
/// We provide fixed-arity instantiations; the 2-ary case is the only one used
/// directly in this file (by `BindgenOptional`). Higher arities are emitted by
/// codegen alongside their consumers.
#[repr(C)]
...
/// trait bounds. Matches C `union` semantics (no auto-drop).
#[repr(C)]
...
impl<Child: Bindgen> Bindgen for BindgenArray<Child> {
    type BunType = bun_collections::ArrayListDefault<Child::BunType>;
    ...
    fn convert_from_extern(extern_value: Self::ExternType) -> Self::BunType {
        ...
        } else if size_of::<Child::BunType>() == size_of::<Child::ExternType>()
            && align_of::<Child::BunType>() == align_of::<Child::ExternType>()
            // PORT NOTE: Rust has no stable type-equality test in generic context.
            // Gate this fast-path on a `const SAME_REPR: bool` opt-in so it only
            ...
            && Child::SAME_REPR
        {
            ...
            let reused: Vec<Child::BunType> =
                unsafe { Vec::from_raw_parts(ptr.cast::<Child::BunType>(), len, cap) };
            return Self::BunType::from_unmanaged(reused);
        } else if size_of::<Child::BunType>() <= size_of::<Child::ExternType>()
            && align_of::<Child::BunType>() <= bun_alloc::mimalloc::MI_MAX_ALIGN_SIZE
        {
            // We can reuse the allocation, but we still need to convert the elements.
            //
            // PORT NOTE: materializing a `&mut [u8]` over the full capacity would
            // assert that every byte — including uninitialized tail elements and
            // `ExternType` padding — is a valid `u8`, which is UB in Rust. Work
            // entirely through raw `*mut u8` and `ptr::copy_nonoverlapping`
            // instead; no reference to the storage is ever formed.
            let mut v = ManuallyDrop::new(unmanaged);
            ...
            for i in 0..length {
                // PORT NOTE: keep the byte-wise copy to match behavior exactly
                // (in-place reinterpretation of overlapping element slots).
                ...
                // SAFETY: dest range lies within the block; `size_of BunType <=
                ...
                unsafe {
                    core::ptr::copy_nonoverlapping(
                        ...
                        storage_ptr.add(i * size_of::<Child::BunType>()),
                        size_of::<Child::BunType>(),
                    );
                }
            }

            let new_size_is_multiple =
                size_of::<Child::ExternType>() % size_of::<Child::BunType>() == 0;
            let new_capacity = if new_size_is_multiple {
                capacity * (size_of::<Child::ExternType>() / size_of::<Child::BunType>())
            } else {
                let new_capacity = storage_len / size_of::<Child::BunType>();
                let new_alloc_size = new_capacity * size_of::<Child::BunType>();
                if new_alloc_size != storage_len {
                    // Allocation isn't a multiple of `size_of::<Child::BunType>()`; we have to
                    // resize it.
            ...
            // `align_of::<BunType>()`; the first `length` slots were just written
            // with valid `BunType` values; the block is mimalloc-owned and the
            // global allocator is mimalloc (see static assert at top of file), so
            // `Vec`'s eventual dealloc — even with `BunType`'s layout — routes to
            // `mi_free`, which ignores layout.
            let items_ptr = storage_ptr.cast::<Child::BunType>();
            let new_unmanaged: Vec<Child::BunType> =
                unsafe { Vec::from_raw_parts(items_ptr, length, new_capacity) };
            return Self::BunType::from_unmanaged(new_unmanaged);
        }

        // Fallback: allocate fresh, convert, free old.
        // PORT NOTE: in Rust the global allocator IS mimalloc (per crate prereq)
        // and `USE_MIMALLOC` is `const true`, so dropping the `Vec` frees the
        // original C++-allocated block correctly.
        let mut result = bun_core::handle_oom(Self::BunType::init_capacity(length));
...
pub struct ExternArrayList<Child> {
    // Nullable `Child*` — single-word nullable pointer. `Option<*mut T>` has no niche
...
impl<T: ExternalSharedDescriptor> Bindgen for BindgenExternalShared<T> {
    type BunType = ExternalShared<T>;
    // Nullable `T*` — single-word FFI layout requires `Option<NonNull<T>>`, not `Option<*mut T>`.
...
impl<T: ExternalSharedDescriptor> BindgenOptionalRepr for BindgenExternalShared<T> {
    type OptionalBunType = ExternalSharedOptional<T>;
    type OptionalExternType = <Self as Bindgen>::ExternType;
