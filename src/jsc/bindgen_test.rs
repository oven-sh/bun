...
pub fn get_bindgen_test_functions(global: &JSGlobalObject) -> JsResult<JSValue> {
    // PORT NOTE: Rust has no field reflection, so a local `PojoFields` impl
    // enumerates the fields explicitly.
    struct Fns {
        add: JSValue,
...
pub fn add(global: &JSGlobalObject, a: i32, b: i32) -> JsResult<i32> {
    match a.checked_add(b) {
        Some(v) => Ok(v),
        None => {
            // Binding functions can propagate OOM and `JSError`. Other errors
            // (like integer overflow) must be converted to a thrown JS error.
...
pub fn required_and_optional_arg(a: bool, b: Option<usize>, c: i32, d: Option<u8>) -> i32 {
    ...
    // (b +% |c|) *% (d orelse 1), then truncate through u53 → isize → i32.
