...
{
#if ASAN_ENABLED
    // Without this, ASan will complain if Rust touches memory in the range
    // [storage + length, storage + capacity), which can happen when the
    // allocator poisons or reuses the unused tail on free.
