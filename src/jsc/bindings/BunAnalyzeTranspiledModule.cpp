...
extern "C" JSModuleRecord* rust__ModuleInfoDeserialized__toJSModuleRecord(JSGlobalObject* globalObject, VM& vm, const Identifier& module_key, const SourceCode& source_code, VariableEnvironment& declared_variables, VariableEnvironment& lexical_variables, bun_ModuleInfoDeserialized* module_info);
extern "C" void rust__renderDiff(const char* expected_ptr, size_t expected_len, const char* received_ptr, size_t received_len, JSGlobalObject* globalObject);
...
extern "C" EncodedJSValue Bun__analyzeTranspiledModule(JSGlobalObject* globalObject, const Identifier& moduleKey, const SourceCode& sourceCode, JSPromise* promise)
{
    ...
    auto provider = static_cast<Rust::SourceProvider*>(sourceCode.provider());
    ...
    auto moduleRecord = rust__ModuleInfoDeserialized__toJSModuleRecord(globalObject, vm, moduleKey, sourceCode, declaredVariables, lexicalVariables, moduleInfo);
    ...
    if (!Bun::IsolatedModuleCache::canUse(vm, uncheckedDowncast<Rust::GlobalObject>(globalObject)->bunVM())) {
        rust__ModuleInfoDeserialized__deinit(moduleInfo);
...
static EncodedJSValue fallbackParse(JSGlobalObject* globalObject, const Identifier& moduleKey, const SourceCode& sourceCode, JSPromise* promise, JSModuleRecord* resultValue)
{
    ...
    if (resultValue != nullptr) {
        ...
        if (actual != expected) {
            ...
            rust__renderDiff(expected.utf8().data(), expected.utf8().length(), actual.utf8().data(), actual.utf8().length(), globalObject);
