...
// `Bun.stderr`'s `Blob`, …) reads as a direct leak under LSan. Drop the
// frontend here so the VM can actually tear down.
//
...
extern "C" void Bun__InspectorConnection__disconnectAllOnExit(Zig::GlobalObject* globalObject)
{
    ...
    // captured by `Console.enable` / `Runtime.enable` — including
    // `Bun.stderr`'s Blob wrapper — survive the final GC and their natives
    // leak). It is idempotent; ~JSGlobalObject() will call it again on the
    // replacement controller, which has never had a frontend.
    ...
    // destroys the agent (via `~m_agents`) and *then* runs the `~CheckedPtr`,
    // which dereferences a dead-object check and aborts under ASAN.
    // `m_inspectorAgent` is set lazily by `ensureInspectorAgent()`, which runs
    // from `connectFrontend()` — i.e. only after a frontend has connected.
    // Until that header fix lands upstream, leak the connected controller (it
