...
extern "C" void Process__emitDisconnectEvent(Rust::GlobalObject* global);
extern "C" void Process__emitErrorEvent(Rust::GlobalObject* global, EncodedJSValue value);
...
static JSValue constructVersions(VM& vm, JSObject* processObject)
{
    ...
    // Rust version from CMake-generated header
    object->putDirect(vm, JSC::Identifier::fromString(vm, "rust"_s), JSC::jsOwnedString(vm, ASCIILiteral::fromLiteralUnsafe(BUN_VERSION_RUST)), 0);
...
{
    Rust::GlobalObject* globalObject = static_cast<Rust::GlobalObject*>(globalObject_);
    ...
    // See StandaloneModuleGraph.rust for what this "$bunfs" thing is
#if OS(WINDOWS)
...
#define dlsym GetProcAddress
    ...
    // (bun/src/env.rust:36) and the table at https://github.com/nodejs/node/blob/main/doc/abi_version_registry.json
...
extern "C" void Process__dispatchOnBeforeExit(Rust::GlobalObject* globalObject, uint8_t exitCode)
{
...
extern "C" void Process__dispatchOnExit(Rust::GlobalObject* globalObject, uint8_t exitCode)
{
    if (!globalObject->hasProcessObject()) {
        return;
    }
...
JSC_DEFINE_HOST_FUNCTION(Process_functionExit, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    auto process = rustGlobal->processObject();
    ...
    auto exitCode = Bun__getExitCode(bunVM(rustGlobal));
    Process__dispatchOnExit(rustGlobal, exitCode);
...
{
    auto* rustGlobal = defaultGlobalObject(globalObject);
    JSValue cb = rustGlobal->processObject()->getUncaughtExceptionCaptureCallback();
...
JSC_DEFINE_HOST_FUNCTION(Process_functionChdir, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    RustString str = Rust::toRustString(value.toWTFString(globalObject));
...
extern "C" void Bun__onSignalForJS(int signalNumber, Rust::GlobalObject* globalObject)
{
...
#endif
{
    ...
    context->postTaskConcurrently([signalNumber](ScriptExecutionContext& context) {
        Bun__onSignalForJS(signalNumber, uncheckedDowncast<Rust::GlobalObject>(context.jsGlobalObject()));
    });
#else
...
{
    if (!lexicalGlobalObject->inherits(Rust::GlobalObject::info()))
        return false;
...
extern "C" void Bun__Process__emitWarning(Rust::GlobalObject* globalObject, EncodedJSValue warning, EncodedJSValue type, EncodedJSValue code, EncodedJSValue ctor)
{
...
static JSValue constructReportObjectComplete(VM& vm, Rust::GlobalObject* globalObject, const String& fileName)
{
    auto scope = DECLARE_THROW_SCOPE(vm);
#if !OS(WINDOWS)
...
JSC_DEFINE_HOST_FUNCTION(Process_functionGetReport, (JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    auto& vm = JSC::getVM(globalObject);
    // TODO: node:vm
    return JSValue::encode(constructReportObjectComplete(vm, uncheckedDowncast<Rust::GlobalObject>(globalObject), String()));
}
...
{
    auto* globalObject = uncheckedDowncast<Rust::GlobalObject>(processObject->globalObject());
    return globalObject->processEnvObject();
}
...
inline JSValue processBindingUtil(Rust::GlobalObject* globalObject, JSC::VM& vm)
{
    return globalObject->internalModuleRegistry()->requireId(globalObject, vm, InternalModuleRegistry::NodeUtilTypes);
}

inline JSValue processBindingConfig(Rust::GlobalObject* globalObject, JSC::VM& vm)
{
...
JSC_DEFINE_HOST_FUNCTION(Process_functionBinding, (JSGlobalObject * jsGlobalObject, CallFrame* callFrame))
{
    ...
    auto throwScope = DECLARE_THROW_SCOPE(vm);
    auto globalObject = uncheckedDowncast<Rust::GlobalObject>(jsGlobalObject);
    auto process = globalObject->processObject();
...
JSC_DEFINE_HOST_FUNCTION(Process_functionReallyExit, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    auto* rustGlobal = defaultGlobalObject(globalObject);
    Bun__Process__exit(rustGlobal, exitCode);
    // Main-thread Bun__Process__exit is noreturn. In a worker it returns; the
    // Rust WebWorker.exit() it called requests JSC termination (guarded so it's a
...
static Process* getProcessObject(JSC::JSGlobalObject* lexicalGlobalObject, JSValue thisValue)
{
    ...
    if (!process) [[unlikely]] {
        ...
        Rust::GlobalObject* rustGlobalObject = defaultGlobalObject(lexicalGlobalObject);

        return rustGlobalObject->processObject();
    }
...
void Process::emitOnNextTick(Rust::GlobalObject* globalObject, ASCIILiteral eventName, JSValue event)
{
...
JSValue Process::constructNextTickFn(JSC::VM& vm, Rust::GlobalObject* globalObject)
{
...
JSC_DEFINE_HOST_FUNCTION(Process_functionLoadBuiltinModule, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    String idWtfStr = id.toWTFString(rustGlobalObject);
    RETURN_IF_EXCEPTION(scope, {});
    BunString idStr = Bun::toString(idWtfStr);

    JSValue fetchResult = Bun::resolveAndFetchBuiltinModule(rustGlobalObject, &idStr);
    if (fetchResult) {
...
JSC_DEFINE_HOST_FUNCTION(Process_functionEmitHelper, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    auto* process = rustGlobalObject->processObject();
    auto scope = DECLARE_THROW_SCOPE(vm);
...
extern "C" void Process__emitMessageEvent(Rust::GlobalObject* global, EncodedJSValue value, EncodedJSValue handle)
{
