...
JSC::JSString* toJS(JSC::JSGlobalObject* globalObject, BunString bunString)
{
    ...
    if (bunString.tag == BunStringTag::StaticUnsafeStringView) {
        return JSC::jsString(globalObject->vm(), Bun::toStringStatic(bunString.impl.view));
    }

    if (bunString.tag == BunStringTag::UnsafeStringView) {
        return Bun::toJSStringGC(bunString.impl.view, globalObject);
    }
...
{
    return {
        BunStringTag::UnsafeStringView,
        { .view = toUnsafeStringView(view) }
    };
}
...
extern "C" [[RUST_EXPORT(nothrow)]] void BunString__toWTFString(BunString* bunString)
{
    WTF::String str;
    if (bunString->tag == BunStringTag::UnsafeStringView) {
        if (Bun::isTaggedExternalPtr(bunString->impl.view.ptr)) {
            str = Bun::toString(bunString->impl.view);
        } else {
            str = Bun::toStringCopy(bunString->impl.view);
        }

    } else if (bunString->tag == BunStringTag::StaticUnsafeStringView) {
        str = Bun::toStringStatic(bunString->impl.view);
    } else {
        return;
    }
...
{
    if (this->tag == BunStringTag::UnsafeStringView) {
        if (Bun::isTaggedExternalPtr(this->impl.view.ptr)) {
            return Bun::toString(this->impl.view);
        } else {
            return Bun::toStringCopy(this->impl.view);
        }
    } else if (this->tag == BunStringTag::StaticUnsafeStringView) {
        return Bun::toStringCopy(this->impl.view);
...
void BunString::appendToBuilder(WTF::StringBuilder& builder) const
{
    ...
    if (this->tag == BunStringTag::UnsafeStringView || this->tag == BunStringTag::StaticUnsafeStringView) {
        Bun::appendToBuilder(this->impl.view, builder);
        return;
    }
...
{
    if (this->tag == BunStringTag::UnsafeStringView) {
        if (Bun::isTaggedUTF8Ptr(this->impl.view.ptr)) {
            return Bun::toStringCopy(this->impl.view);
        } else {
            return Bun::toString(this->impl.view);
        }
    } else if (this->tag == BunStringTag::StaticUnsafeStringView) {
        return Bun::toStringStatic(this->impl.view);
    } else if (this->tag == BunStringTag::WTFStringImpl) {
...
WTF::String BunString::transferToWTFString()
{
    if (this->tag == BunStringTag::UnsafeStringView) {
        if (Bun::isTaggedUTF8Ptr(this->impl.view.ptr)) {
            auto str = Bun::toStringCopy(this->impl.view);
            *this = Bun::BunStringEmpty;
            return str;
        } else {
            auto str = Bun::toString(this->impl.view);
            *this = Bun::BunStringEmpty;
            return str;
        }
    } else if (this->tag == BunStringTag::StaticUnsafeStringView) {
        auto str = Bun::toStringStatic(this->impl.view);
        *this = Bun::BunStringEmpty;
        return str;
    } else if (this->tag == BunStringTag::WTFStringImpl) {
        ASSERT(this->impl.wtf->refCount() > 0 && !this->impl.wtf->isEmpty());
...
bool BunString::isEmpty() const
{
    ...
    case BunStringTag::UnsafeStringView:
    case BunStringTag::StaticUnsafeStringView:
