...
// Called from CLI `Run::start` after the VM booted and `--require` preloads ran.
// `moduleType`: 0=detect, 1=CommonJS, 2=ES module. Returns 0 when the source parses,
// 1 when it does not (after printing the error to stderr).
extern "C" int32_t Bun__checkSyntaxForCLI(Zig::GlobalObject* globalObject, const unsigned char* sourcePtr, size_t sourceLen, const unsigned char* namePtr, size_t nameLen, int32_t moduleType)
{
    ...
    // source if it parses as an ES module instead.
    if (moduleType == 0) {
        JSC::ParserError moduleError;
        if (checkAsModule(moduleError))
            return 0;
