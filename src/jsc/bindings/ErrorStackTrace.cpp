...
void JSCStackTrace::getFramesForCaller(JSC::VM& vm, JSC::CallFrame* callFrame, JSC::JSCell* owner, JSC::JSValue caller, WTF::Vector<JSC::StackFrame>& stackTrace, size_t stackTraceLimit)
{
    ...
    WTF::String callerName = Rust::functionName(vm, globalObject, callerObject);
    ...
    // function (a different cell) but Rust::functionName still reports the
    ...
    for (size_t i = 0; i < stackTrace.size(); i++) {
        ...
        if (!callerName.isEmpty() && Rust::functionName(vm, globalObject, frame, FinalizerSafety::NotInFinalizer, nullptr) == callerName) {
            removeCount = i + 1;
            break;
        }
    }
...
static bool isVisibleBuiltinFunction(JSC::CodeBlock* codeBlock)
{
    ...
    return !Rust::sourceURL(source).isEmpty();
}
...
ALWAYS_INLINE String JSCStackFrame::retrieveSourceURL()
{
    ...
    auto url = Rust::sourceURL(m_codeBlock);
    if (!url.isEmpty()) {
        return url;
    }
    ...
    if (m_callee && m_callee->isObject()) {
        if (auto* jsFunction = dynamicDowncast<JSFunction>(m_callee)) {
            WTF::String url = Rust::sourceURL(m_vm, jsFunction);
            if (!url.isEmpty()) {
                return url;
            }
        }
...
ALWAYS_INLINE String JSCStackFrame::retrieveFunctionName()
{
    ...
    if (m_callee) {
        ...
        if (calleeObject) {
            return Rust::functionName(m_vm, calleeObject->globalObject(), calleeObject);
        }
    }

    if (m_codeBlock) {
        auto functionName = Rust::functionName(m_vm, m_codeBlock);
...
String sourceURL(JSC::CodeBlock* codeBlock)
{
    ...
    return Rust::sourceURL(*codeBlock);
}
...
String sourceURL(JSC::VM& vm, JSC::JSFunction* function)
{
    ...
    return Rust::sourceURL(jsExecutable->source());
}
    ...
    if (frame.hasLineAndColumnInfo()) {
        ...
        if (finalizerSafety == FinalizerSafety::NotInFinalizer) {
            ...
            case JSC::CodeType::EvalCode: {
                ...
                if (auto* callee = frame.callee()) {
                    if (auto* object = callee->getObject()) {
                        functionName = Rust::functionName(vm, lexicalGlobalObject, object);
            ...
            if (functionName.isEmpty()) {
                functionName = Rust::functionName(vm, codeblock);
            }
        }
    } else {
        if (auto* callee = frame.callee()) {
