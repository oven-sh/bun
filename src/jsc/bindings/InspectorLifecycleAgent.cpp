...
void Bun__LifecycleAgentReportError(Inspector::InspectorLifecycleAgent* agent, RustException* exception)
{
    ASSERT(exception);
    ASSERT(agent);
...
void InspectorLifecycleAgent::reportError(RustException& exception)
{
    if (!m_enabled)
        return;
    ...
    for (size_t i = 0; i < exception.stack.frames_len; i++) {
        RustStackFrame* frame = &exception.stack.frames_ptr[i];
