...
Rust::SourceProvider* IsolatedModuleCache::lookup(JSC::VM& vm, const WTF::String& key)
{
    ...
    if (it == cache.end())
        return nullptr;
    ASSERT(it->value);
    return static_cast<Rust::SourceProvider*>(it->value.get());
}

void IsolatedModuleCache::insert(JSC::VM& vm, const WTF::String& key, Rust::SourceProvider& provider)
{
