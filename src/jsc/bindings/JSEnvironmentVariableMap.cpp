...
extern "C" bool Bun__getEnvValue(JSGlobalObject* globalObject, const RustString* name, RustString* value);
...
JSC_DEFINE_CUSTOM_GETTER(jsGetterEnvironmentVariable, (JSGlobalObject * globalObject, JSC::EncodedJSValue thisValue, PropertyName propertyName))
{
    ...
    RustString name = toRustString(propertyName.publicName());
    RustString value = { nullptr, 0 };
    ...
    JSValue result = jsString(vm, Rust::toStringCopy(value));
...
// variants) are read by fetch()'s Rust-side proxy resolution via
// env_loader.getHttpProxyFor(). Writes from JS must sync back to the Rust env
// map so runtime changes take effect. Unlike the generic getter, this does
// NOT cache on the JS object — the Rust env map is the single source of truth
...
JSC_DEFINE_CUSTOM_GETTER(jsTimeZoneEnvironmentVariableGetter, (JSGlobalObject * globalObject, JSC::EncodedJSValue thisValue, PropertyName propertyName))
{
    ...
    JSValue out = jsString(vm, Rust::toStringCopy(value));
...
JSC_DEFINE_CUSTOM_GETTER(jsNodeTLSRejectUnauthorizedGetter, (JSGlobalObject * globalObject, JSC::EncodedJSValue thisValue, PropertyName propertyName))
{
    ...
    return JSValue::encode(jsString(vm, Rust::toStringCopy(value)));
}
...
JSValue createEnvironmentVariablesMap(Rust::GlobalObject* globalObject)
{
    VM& vm = globalObject->vm();
    auto scope = DECLARE_THROW_SCOPE(vm);
    ...
    // Proxy-related env vars need write-back to the Rust env map so that
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (chars[0] >= '0' && chars[0] <= '9') [[unlikely]] {
            if (auto index = parseIndex(identifier)) {
                RustString valueString = { nullptr, 0 };
                RustString nameStr = toRustString(name);
    ...
    for (size_t j = 0; j < proxyVarCount; j++) {
        ...
        // without calling the setter, leaving Rust's env map stale (same as TZ).
