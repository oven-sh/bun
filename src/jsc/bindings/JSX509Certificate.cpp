...
JSC_DEFINE_HOST_FUNCTION(x509CertificateConstructorConstruct, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    Structure* structure = bunGlobalObject->m_JSX509CertificateClassStructure.get(bunGlobalObject);
    JSValue newTarget = callFrame->newTarget();
    if (bunGlobalObject->m_JSX509CertificateClassStructure.constructor(bunGlobalObject) != newTarget) [[unlikely]] {
        if (!newTarget) {
...
extern "C" EncodedJSValue Bun__X509__toJS(X509* cert, JSGlobalObject* globalObject)
{
    ...
    auto* bunGlobalObject = defaultGlobalObject(globalObject);
    return JSValue::encode(JSX509Certificate::create(bunGlobalObject->vm(), bunGlobalObject->m_JSX509CertificateClassStructure.get(bunGlobalObject), globalObject, WTF::move(cert_ptr)));
}
