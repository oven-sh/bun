...
// `unhandledRejection` is an uncaught exception, as in Node. Null: the host's handlers.
...
// An unhandled rejection belongs to the graph whose context the promise is rejected in: the one that is current when
// it is, which JSC makes the one whose script made the promise.
...
// options.unhandledRejection(reason, promise), or options.uncaughtException(error, origin) with Node's origins
// ("uncaughtException", "unhandledRejection"). `promise`: the rejected one, or empty.
static bool deliverToHandler(Zig::GlobalObject* globalObject, JSModuleGraph* graph, GraphError kind, JSValue error, JSValue promise)
{
    ...
    if (scope.exception()) [[unlikely]] {
        ...
        // to the handler's owner and does not come back here (a handler that calls a function of its
...
extern "C" bool Bun__ModuleGraph__handleUncaughtException(JSGlobalObject* lexicalGlobalObject, EncodedJSValue encodedError)
{
    ...
    return deliverToHandler(globalObject, graph, GraphError::UncaughtException, error, JSValue());
}
