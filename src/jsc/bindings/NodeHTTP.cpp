...
static void writeFetchHeadersToUWSResponse(WebCore::FetchHeaders& headers, uWS::HttpResponse<isSSL>* res)
{
    ...
    for (const auto& header : internalHeaders.commonHeaders()) {
        ...
        // The user owns the Connection line; a "close" token also closes the
        // connection after the response (RFC 9112 §9.6).
    ...
    uWS::HttpResponse<isSSL>* response)
{
    ...
    // node:http's ServerResponse owns the Date and Connection lines (res.sendDate,
    // removeHeader, _storeHeader), so uWS writes neither.
