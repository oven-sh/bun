...
JSPromise* importModule(JSGlobalObject* globalObject, JSString* moduleName, RefPtr<JSC::ScriptFetchParameters> parameters, const SourceOrigin& sourceOrigin)
{
    ...
    if (isUseMainContextDefaultLoaderConstant(globalObject, dynamicImportCallback)) {
        ...
        Rust::GlobalObject* rustGlobalObject = defaultGlobalObject(globalObject);
        RELEASE_AND_RETURN(scope, rustGlobalObject->moduleLoaderImportModule(rustGlobalObject, rustGlobalObject->moduleLoader(), moduleName, WTF::move(parameters), sourceOrigin));
...
NodeVMGlobalObject* getGlobalObjectFromContext(JSGlobalObject* globalObject, JSValue contextValue, bool canThrow)
{
    ...
    JSValue scopeValue = rustGlobalObject->vmModuleContextMap()->get(context);
...
bool isContext(JSGlobalObject* globalObject, JSValue value)
{
    ...
    if (rustGlobalObject->vmModuleContextMap()->has(asObject(value))) {
        return true;
    }
...
bool getContextArg(JSGlobalObject* globalObject, JSValue& contextArg)
{
    if (contextArg.isUndefined()) {
        ...
        Rust::GlobalObject* rustGlobalObject = defaultGlobalObject(globalObject);
        if (contextArg == rustGlobalObject->m_nodeVMDontContextify.get(rustGlobalObject)) {
            contextArg = JSC::constructEmptyObject(globalObject);
            return true;
        }
    }
...
bool isUseMainContextDefaultLoaderConstant(JSGlobalObject* globalObject, JSValue value)
{
    if (value.isSymbol()) {
        Rust::GlobalObject* rustGlobalObject = defaultGlobalObject(globalObject);
        if (value == rustGlobalObject->m_nodeVMUseMainContextDefaultLoader.get(rustGlobalObject)) {
            return true;
        }
    }
...
{
    // Delegate to the parent Rust::GlobalObject so that unhandled rejections
    // in VM contexts are reported to the main process (matching Node.js behavior)
    auto* rustGlobalObject = defaultGlobalObject(globalObject);
    Rust::GlobalObject::promiseRejectionTracker(rustGlobalObject, promise, operation);
}
...
void NodeVMGlobalObject::finishCreation(JSC::VM& vm)
{
    ...
    // Share the async context data with the parent Rust::GlobalObject.
...
JSC_DEFINE_HOST_FUNCTION(vmModule_createContext, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    rustGlobalObject->vmModuleContextMap()->set(vm, sandbox, targetContext);
...
JSC::JSValue createNodeVMBinding(Rust::GlobalObject* globalObject)
{
    VM& vm = globalObject->vm();
...
void configureNodeVM(JSC::VM& vm, Rust::GlobalObject* globalObject)
{
