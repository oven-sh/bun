...
JSC_DEFINE_HOST_FUNCTION(vmModule_createContext, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    // A DONT_CONTEXTIFY context has no sandbox. The context object is the
    // global proxy, as in Node, so `this` and `globalThis` inside the context
    // are the same object as the one returned here.
    if (notContextified) {
        sandbox = targetContext->globalThis();
    } else {
