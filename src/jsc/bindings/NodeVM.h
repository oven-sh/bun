...
// Wraps serialized bytecode in a { magic, payloadLength, sourceHash, jscVersion,
// payloadHash } header so that cachedData coming back from JS can be verified
// before it reaches JSC's bytecode decoder, which assumes a well formed payload.
// Returns the JS-exposed Buffer.
...
// Checks the header written by createCachedDataBuffer. Returns the payload (as an
// owning copy, since the decoder can be retained by lazily decoded functions) or
// nullptr if the buffer must be rejected.
