...
// NodeVMScript.cpp: a worker-level termination must propagate, not become ERR_SCRIPT_EXECUTION_*.
...
#define VM_RETURN_IF_EXCEPTION(scope__, value__)               \
    ...
    std::ignore = scope.exception();
    if (vm.hasTerminationRequest() || vm.hasPendingTerminationException()) {
        // A worker-level termination falls through to VM_RETURN_IF_EXCEPTION below with the TerminationException pending.
        if (!propagateWorkerTermination(vm, scope)) {
            vm.drainMicrotasksForGlobalObject(nodeVmGlobalObject);
            DECLARE_TOP_EXCEPTION_SCOPE(vm).clearException();
            vm.clearHasTerminationRequest();
