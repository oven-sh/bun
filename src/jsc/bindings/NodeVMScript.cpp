...
constructScript(JSGlobalObject* globalObject, CallFrame* callFrame, JSValue newTarget = {})
{
    ...
    Structure* structure = rustGlobalObject->NodeVMScriptStructure();
    if (rustGlobalObject->NodeVMScript() != newTarget) [[unlikely]] {
        if (!newTarget) {
...
JSC_DEFINE_HOST_FUNCTION(scriptRunInNewContext, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    auto* targetContext = NodeVMGlobalObject::create(vm,
        rustGlobalObject->NodeVMGlobalObjectStructure(),
        contextOptions, importer);
    RETURN_IF_EXCEPTION(scope, {});

    if (notContextified) {
        auto* specialSandbox = NodeVMSpecialSandbox::create(vm, rustGlobalObject->NodeVMSpecialSandboxStructure(), targetContext);
        RETURN_IF_EXCEPTION(scope, {});
