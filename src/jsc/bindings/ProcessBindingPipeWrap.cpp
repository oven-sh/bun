...
// Matches the calling convention the class codegen emits for
// `Pipe__getConstructor` (see JSC_CALLCONV in generate-classes.ts).
#if !OS(WINDOWS)
extern "C" JSC::EncodedJSValue Pipe__getConstructor(Zig::GlobalObject*);
#else
