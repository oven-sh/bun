...
JSC_DEFINE_HOST_FUNCTION(jsErrname, (JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    for (const auto& error : uvErrors) {
        if (err == error.code)
            return JSValue::encode(JSC::jsString(vm, String(error.name)));
    }
...
JSC_DEFINE_HOST_FUNCTION(jsGetErrorMap, (JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    for (const auto& error : uvErrors) {
        ...
        // RETURN_IF_EXCEPTION
        arr->putDirectIndex(globalObject, 0, JSC::jsString(vm, String(error.name)));
        arr->putDirectIndex(globalObject, 1, JSC::jsString(vm, String(error.description)));
        map->set(globalObject, JSC::jsNumber(error.code), arr);
    }
