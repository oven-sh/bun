...
// A JavaScript RegExp (source + flags) compiled for `regexp.test(input)`
// without a VM. Unlike `Yarr::RegularExpression` above, which only accepts the
// i, m and v flags, this takes every RegExp flag with its JavaScript meaning.
// The interpreter scratch memory lives in `m_allocator`, so one matcher must
// be used by one thread at a time. Yarr names are fully qualified because the
