...
private:
    ...
    // contexts map. Worker teardown removes the entry explicitly (so
    // cross-thread posts see the context as gone instead of posting to a
    // VM that is about to stop ticking) and ~GlobalObject may try again
    // once GC finally collects the global; the flag makes the second call
    // a no-op. Only written on the context's own thread.
