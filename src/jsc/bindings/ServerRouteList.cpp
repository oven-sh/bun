...
/**
  ...
  The easier approach would be a list of JSC::Strong handles on the Rust side,
  but that would mean holding a Strong reference for every single callback. This
  would show up in profiling, and it's a lot of strong references. We could use
  a JSArray instead, but that would incur unnecessary overhead when reading
...
public:
    ...
    static ServerRouteList* create(
        ...
        std::span<UnsafeStringView> paths)
    {
...
private:
    ...
    ServerRouteList(JSC::VM& vm, JSC::Structure* structure, std::span<EncodedJSValue> callbacks, std::span<UnsafeStringView> paths)
        : Base(vm, structure)
    ...
    void finishCreation(JSC::VM& vm, std::span<EncodedJSValue> callbacks, std::span<UnsafeStringView> paths)
    {
        ...
        for (size_t i = 0; i < paths.size(); i++) {
            UnsafeStringView rawPath = paths[i];
...
extern "C" JSC::EncodedJSValue Bun__ServerRouteList__create(Bun::GlobalObject* globalObject, EncodedJSValue* callbacks, UnsafeStringView* paths, size_t pathsLength)
{
    auto* structure = globalObject->m_ServerRouteListStructure.get(globalObject);
    auto* routeList = ServerRouteList::create(globalObject->vm(), structure, std::span<EncodedJSValue>(callbacks, pathsLength), std::span<UnsafeStringView>(paths, pathsLength));
