...
private:
    ...
    ServerRouteList(JSC::VM& vm, JSC::Structure* structure, std::span<EncodedJSValue> callbacks, std::span<BunString> paths)
        : Base(vm, structure)
    ...
    void finishCreation(JSC::VM& vm, std::span<EncodedJSValue> callbacks, std::span<BunString> paths)
    {
        ...
        for (size_t i = 0; i < paths.size(); i++) {
            WTF::String path = paths[i].toWTFString(BunString::ZeroCopy);
...
extern "C" JSC::EncodedJSValue Bun__ServerRouteList__create(Zig::GlobalObject* globalObject, EncodedJSValue* callbacks, BunString* paths, size_t pathsLength)
{
    auto* structure = globalObject->m_ServerRouteListStructure.get(globalObject);
    auto* routeList = ServerRouteList::create(globalObject->vm(), structure, std::span<EncodedJSValue>(callbacks, pathsLength), std::span<BunString>(paths, pathsLength));
