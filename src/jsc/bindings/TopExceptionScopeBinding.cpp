...
static_assert(sizeof(TopExceptionScope) == ExpectedTopExceptionScopeSize, "TopExceptionScope.rust assumes TopExceptionScope is 56 bytes");
static_assert(alignof(TopExceptionScope) == ExpectedTopExceptionScopeAlignment, "TopExceptionScope.rust assumes TopExceptionScope is 8-byte aligned");
...
extern "C" void TopExceptionScope__construct(
    ...
    size_t alignment)
{
    // validate that Rust is correct about what the size and alignment should be
...
extern "C" void TopExceptionScope__assertNoException(void* ptr)
{
    ...
    // Rust should only call TopExceptionScope__assertNoException if it wants the assertion.
