...
extern "C" void TopExceptionScope__construct(
    ...
    size_t alignment)
{
    // validate that the Rust caller is correct about what the size and alignment should be
...
extern "C" void TopExceptionScope__assertNoException(void* ptr)
{
    ...
    // The Rust side should only call TopExceptionScope__assertNoException if it wants the assertion.
