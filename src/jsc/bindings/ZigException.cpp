/**
 * RustException handling and error processing utilities.
 *
 * This file contains functions for converting JavaScript exceptions to Rust exceptions,
...
using Rust::FinalizerSafety;

static void populateStackFrameMetadata(JSC::VM& vm, JSC::JSGlobalObject* globalObject, const JSC::StackFrame& stackFrame, RustStackFrame& frame, FinalizerSafety finalizerSafety)
{
    if (stackFrame.isWasmFrame()) {
        frame.code_type = RustStackFrameCodeWasm;

        auto name = Rust::functionName(vm, globalObject, stackFrame, finalizerSafety, nullptr);
        if (!name.isEmpty()) {
            frame.function_name = Bun::toStringRef(name);
        }

        auto sourceURL = Rust::sourceURL(vm, stackFrame);
    ...
    if (m_codeBlock) {
        ...
        case JSC::EvalCode: {
            frame.code_type = RustStackFrameCodeEval;
            return;
        }
        case JSC::ModuleCode: {
            frame.code_type = RustStackFrameCodeModule;
            return;
        }
        case JSC::GlobalCode: {
            frame.code_type = RustStackFrameCodeGlobal;
            return;
        }
        case JSC::FunctionCode: {
            frame.code_type = !m_codeBlock->isConstructor() ? RustStackFrameCodeFunction : RustStackFrameCodeConstructor;
            break;
        }
        default:
            ASSERT_NOT_REACHED();
    ...
    if (finalizerSafety == FinalizerSafety::MustNotTriggerGC) {
        ...
        functionName = Rust::functionName(vm, globalObject, stackFrame, finalizerSafety, nullptr);
    } else {
        // Use the richer callee-based path
        if (auto calleeCell = stackFrame.callee()) {
            if (auto* callee = calleeCell->getObject())
                functionName = Rust::functionName(vm, globalObject, callee);
        }
    }
...
static void populateStackFramePosition(const JSC::StackFrame& stackFrame, BunString* source_lines,
    ...
    RustStackFramePosition& position, JSC::SourceProvider** referenced_source_provider, PopulateStackTraceFlags flags)
{
    ...
    memcpy(&position, &location, sizeof(RustStackFramePosition));
...
static void populateStackFrame(JSC::VM& vm, RustStackTrace& trace, const JSC::StackFrame& stackFrame,
    RustStackFrame& frame, bool is_top, JSC::SourceProvider** referenced_source_provider, JSC::JSGlobalObject* globalObject, PopulateStackTraceFlags flags, FinalizerSafety finalizerSafety)
{
    if (flags == PopulateStackTraceFlags::OnlyPosition) {
...
static void populateStackTrace(JSC::VM& vm, const WTF::Vector<JSC::StackFrame>& frames, RustStackTrace& trace, JSC::JSGlobalObject* globalObject, PopulateStackTraceFlags flags, FinalizerSafety finalizerSafety = FinalizerSafety::NotInFinalizer)
{
    if (flags == PopulateStackTraceFlags::OnlyPosition) {
        uint8_t frame_i = 0;
        ...
        while (frame_i < frame_count && stack_frame_i < total_frame_count) {
            ...
            RustStackFrame& frame = trace.frames_ptr[frame_i];
        ...
        for (uint8_t i = 0; i < trace.frames_len; i++) {
            RustStackFrame& frame = trace.frames_ptr[i];
...
static void fromErrorInstance(RustException& except, JSC::JSGlobalObject* global,
                ...
                if (!stack.isEmpty()) {
                    ...
                    iterator.forEachFrame([&](const V8StackTraceIterator::StackFrame& frame, bool& stop) -> void {
                        ...
                        if (frame.isConstructor) {
                            current.code_type = RustStackFrameCodeConstructor;
                        } else if (frame.isGlobalCode) {
                            current.code_type = RustStackFrameCodeGlobal;
                        }
...
void exceptionFromString(RustException& except, JSC::JSValue value, JSC::JSGlobalObject* global)
{
    auto& vm = JSC::getVM(global);
...
extern "C" void JSC__Exception__getStackTrace(JSC::Exception* arg0, JSC::JSGlobalObject* global, RustStackTrace* trace)
{
    populateStackTrace(arg0->vm(), arg0->stack(), *trace, global, PopulateStackTraceFlags::OnlyPosition);
}

extern "C" [[RUST_EXPORT(check_slow)]] void JSC__JSValue__toRustException(JSC::EncodedJSValue jsException, JSC::JSGlobalObject* global, RustException* exception)
{
...
extern "C" void RustException__collectSourceLines(JSC::EncodedJSValue jsException, JSC::JSGlobalObject* global, RustException* exception)
{
    JSC::JSValue value = JSC::JSValue::decode(jsException);
    if (value == JSC::JSValue {}) {
        return;
