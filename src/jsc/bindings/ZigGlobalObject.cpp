...
extern "C" void JSCInitialize(const char* envp[], size_t envc, void (*onCrash)(const char* ptr, size_t length), bool evalMode, bool oneShotStartup)
{
    ...
    std::call_once(jsc_init_flag, [evalMode, oneShotStartup, envp, envc, onCrash]() {
        ...
        JSC::initialize([&] {
            ...
            // Lower the "large heap" threshold so JSC's own allocation-budgeted
            // eden pacing engages sooner; with Bun's per-tick heap sampler gone
            // (GarbageCollectionController.rs), this is the primary eden driver.
