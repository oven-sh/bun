...
void GlobalObject::handleRejectedPromises()
{
    ...
    while (auto* entry = m_aboutToBeNotifiedRejectedPromises.takeFirst(this)) {
        ...
        // installed while reporting an uncaught exception. Raw entries were
        // rejected with no async context, so they replay `undefined` — this
        // matters when the drain itself runs inside a context (it can be
        // re-entered synchronously from user code).
        InternalFieldTuple* asyncContextData = nullptr;
