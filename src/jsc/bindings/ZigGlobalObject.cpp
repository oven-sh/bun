...
uint8_t GlobalObject::drainMicrotasks()
{
    ...
    vm.drainMicrotasks();
    // The job that constructed the first AsyncLocalStorage entered with tracking off, so nothing restored a frame it installed (the entry module's top-level enterWith()): end the checkpoint as it started.
    if (!vm.entryScope)
        m_asyncContextData.get()->putInternalField(vm, 0, jsUndefined());
    if (auto* exception = scope.exception()) {
        if (vm.isTerminationException(exception)) {
