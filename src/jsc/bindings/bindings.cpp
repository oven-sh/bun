...
// A native abort listener the signal owns; `ctx` is freed once, never while its callback runs.
class NativeAbortListener final : public RefCounted<NativeAbortListener> {
public:
    using Call = void (*)(void*, JSC::EncodedJSValue);
    using Destroy = void (*)(void*);

    static Ref<NativeAbortListener> create(void* ctx, Call call, Destroy destroy)
    {
        return adoptRef(*new NativeAbortListener(ctx, call, destroy));
    }

    ~NativeAbortListener() { release(); }

    void listenTo(WebCore::AbortSignal& signal)
    {
        m_signal = signal;
        m_algorithm = signal.addAlgorithm([listener = Ref { *this }](JSC::JSValue reason) {
            listener->fire(reason);
        });
    }

    void cancel()
    {
        if (RefPtr signal = std::exchange(m_signal, nullptr).get())
            signal->removeAlgorithm(m_algorithm);
        if (!m_running)
            release();
        m_cancelled = true;
    }
...
private:
    NativeAbortListener(void* ctx, Call call, Destroy destroy)
        : m_ctx(ctx)
        , m_call(call)
        , m_destroy(destroy)
    {
    ...
    void fire(JSC::JSValue reason)
    {
        if (m_cancelled || !m_ctx)
            return;
        Ref protectedThis { *this };
        m_signal = nullptr;
        m_running = true;
        m_call(m_ctx, JSC::JSValue::encode(reason));
        m_running = false;
        release();
    }
    ...
    Call m_call;
    Destroy m_destroy;
    WeakPtr<WebCore::AbortSignal, WebCore::WeakPtrImplWithEventTargetData> m_signal;
...
extern "C" Bun::NativeAbortListener* WebCore__AbortSignal__addOwnedListener(WebCore::AbortSignal* abortSignal, void* ctx, Bun::NativeAbortListener::Call call, Bun::NativeAbortListener::Destroy destroy)
{
    if (abortSignal->aborted()) {
        auto* context = static_cast<WebCore::EventTarget*>(abortSignal)->scriptExecutionContext();
        auto reason = context ? abortSignal->jsReason(*context->jsGlobalObject()) : abortSignal->reason().getValue(jsNull());
        call(ctx, JSC::JSValue::encode(reason));
        destroy(ctx);
        return nullptr;
    }
    Ref listener = Bun::NativeAbortListener::create(ctx, call, destroy);
    listener->listenTo(*abortSignal);
    return &listener.leakRef();
}

extern "C" void Bun__NativeAbortListener__cancel(Bun::NativeAbortListener* listener)
{
    Ref adopted = adoptRef(*listener);
    adopted->cancel();
}
