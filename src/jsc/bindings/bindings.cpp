...
// `min <= value && value <= max`, inclusive on both ends. ComparisonResult::Undefined
// never occurs for integer comparands, so anything that is not the expected ordering
// is treated as out of range.
template<typename Bound>
static bool isBigIntInRange(JSC::EncodedJSValue value, Bound min, Bound max)
{
...
{
    return isBigIntInRange(value, min, max);
}
