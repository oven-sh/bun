...
// `space` is forwarded verbatim to `JSON.stringify(value, null, space)`.
// `jsUndefined()` selects JSC's FastStringifier, which is significantly faster
// than the general Stringifier used whenever space is a number (even 0).
void JSC__JSValue__jsonStringify(JSC::EncodedJSValue JSValue0, JSC::JSGlobalObject* arg1,
    JSC::EncodedJSValue space, BunString* arg3)
{
    ASSERT_NO_PENDING_EXCEPTION(arg1);
    auto& vm = JSC::getVM(arg1);
    auto scope = DECLARE_THROW_SCOPE(vm);
    JSC::JSValue value = JSC::JSValue::decode(JSValue0);
    WTF::String str = JSC::JSONStringify(arg1, value, JSC::JSValue::decode(space));
    RETURN_IF_EXCEPTION(scope, );
