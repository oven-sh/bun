...
typedef struct BunString {
    ...
    // It's only truly zero-copy if it was already a WTFStringImpl (which it is if it came from JS and we didn't use a StringView)
...
extern "C" void BunStringView__free(const unsigned char* ptr, size_t len, void* allocator);
...
extern "C" void BunStringView__freeGlobal(const unsigned char* ptr, size_t len);
