...
//
// Calling dlsym'd CG/ImageIO functions through Rust function pointers crashed
...
// natively, and the Rust boundary is two extern-C entry points with only
// scalar/pointer args.
//
...
// uninitialised buffer held (0xAA in Rust debug). vImageBuffer_InitWithCGImage
    ...
    uint32_t* out_w, uint32_t* out_h, uint8_t* out)
{
    ...
    // malloc and leave `out` uninitialised, which on a Rust debug build is
    ...
    int32_t format, int32_t quality, uint8_t* out, size_t* out_len)
{
    ...
    // time, and the two calls are back-to-back in codecs.rust.
...
#else
// Non-Apple: stubs so the link succeeds; Rust only references these under
