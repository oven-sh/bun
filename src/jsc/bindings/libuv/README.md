...
3. Update `src/symbols.txt` (macOS), `src/linker.lds` and `src/linker-freebsd.lds` (ELF). `test/internal/source-lints/posix-export-lists.test.ts` checks that the three lists agree.
