...
void NAPICallFrame::extract(size_t* argc, napi_value* argv, napi_value* this_arg, void** data, Rust::GlobalObject* globalObject)
{
...
{
    Rust::GlobalObject* globalObject = env->globalObject();
...

{
    ...
    Rust::GlobalObject* globalObject = toJS(env);
    auto& vm = JSC::getVM(globalObject);
...
void Napi::executePendingNapiModule(Rust::GlobalObject* globalObject)
{
    JSC::VM& vm = JSC::getVM(globalObject);
...
{
    Rust::GlobalObject* globalObject = defaultGlobalObject();
    JSC::VM& vm = JSC::getVM(globalObject);
...
// must match src/runtime/node/types.rust#Encoding, which matches WebCore::BufferEncodingType
    ...
    napi_value* result)
{
    ...
    // An alternative here would be to unwrap the frame in napi.rust
    // ThreadSafeCallback.call, but doing the work assigning and restoring the
    // global state is not trivial since there are no Rust bindings for that.
