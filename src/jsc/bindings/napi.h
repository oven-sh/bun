...
extern "C" void napi_internal_register_cleanup_rust(napi_env env);
...
public:
    NapiEnv(Rust::GlobalObject* globalObject, const napi_module& napiModule)
        : m_globalObject(globalObject)
        , m_napiModule(napiModule)
        , m_vm(JSC::getVM(globalObject))
    {
        napi_internal_register_cleanup_rust(this);
    }

    static Ref<NapiEnv> create(Rust::GlobalObject* globalObject, const napi_module& napiModule)
    {
    ...
    inline Rust::GlobalObject* globalObject() const { return m_globalObject; }
    // `bun test --isolate` creates a fresh Rust::GlobalObject per file and
    ...
    // one.
    inline void retargetGlobalObject(Rust::GlobalObject* newGlobal)
    {
...
private:
    Rust::GlobalObject* m_globalObject = nullptr;
...
void executePendingNapiModule(Rust::GlobalObject* globalObject);
...
static inline Rust::GlobalObject* toJS(napi_env val)
{
    return val->globalObject();
}

static inline napi_value toNapi(JSC::JSValue val, Rust::GlobalObject* globalObject)
{
    if (val.isCell()) {
...
public:
    ...
    void extract(size_t* argc, // [in-out] Specifies the size of the provided argv array
        ...
        void** data, Rust::GlobalObject* globalObject);
