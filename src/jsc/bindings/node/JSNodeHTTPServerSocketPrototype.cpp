...
JSC_DEFINE_HOST_FUNCTION(jsFunctionNodeHTTPServerSocketUpgradeToTunnel, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    // upgradeToTunnel(afterBody, response): with a truthy afterBody the switch
    // happens only once the request body has been fully parsed (Upgrade requests
    // with a body). response is the NodeHTTPResponse of the request that leaves HTTP.
