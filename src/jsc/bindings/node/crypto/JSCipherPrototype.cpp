...
JSC_DEFINE_HOST_FUNCTION(jsCipherSetAuthTag, (JSC::JSGlobalObject * globalObject, JSC::CallFrame* callFrame))
{
    ...
    // Redundant gate removed: Process::emitWarning already checks the live
    // per-Worker process.noDeprecation.
    if (cipher->m_ctx.isGcmMode() && !cipher->m_authTagLen.has_value() && tagLen != 16) {
