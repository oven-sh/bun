...
// Packed classification byte per codepoint (stringWidthTables.h, generated by
// scripts/generate-stringwidth-tables.mjs): bits 0-4 GraphemeBreakClass,
// bits 5-6 width class (0 zero, 1 narrow, 2 wide, 3 ambiguous), bit 7 Emoji.
...
// Ordinal values must match the generator.
...
// Terminal width of one grapheme cluster: the sum of its codepoint widths
// (emoji modifiers add nothing), overridden by the emoji sequence rules.
struct GraphemeState {
    ...
    size_t width() const
    {
        ...
        if (emojiBase && (skinTone || zwj))
            return 2;
        // VS16 widens an Emoji base; other bases keep their own width.
        if (vs15 || vs16) {
...
// Byte index of the longest prefix of whole clusters of UTF-8 `input` whose
// visible width is <= `maxWidth`. ANSI escapes are zero-width and included.
