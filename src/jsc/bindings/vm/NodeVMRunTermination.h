...
// bookkeeping cannot be retired while the caller is still inside the VM): a deadline registered
// with one process-wide timer thread by the constructor and withdrawn by finish(). finish() then
// decides whose termination is pending: this run's (it requested one) — consumed and turned into
// ERR_SCRIPT_EXECUTION_TIMEOUT / _INTERRUPTED — or someone else's (an enclosing run's deadline,
// or the VM's own worker terminate() / process.exit()), which is left in place for the caller to
// propagate.
...
public:
    ...
    // false otherwise; whatever is then pending — an ordinary exception, or a termination that is
    // not this run's — is the caller's to propagate.
    ...
    struct Deadline : public ThreadSafeRefCounted<Deadline> {
        const MonotonicTime at;
        WTF::Lock lock;
        JSC::VM* vm WTF_GUARDED_BY_LOCK(lock); // null once withdrawn
        bool fired WTF_GUARDED_BY_LOCK(lock) { false };

        Deadline(MonotonicTime at, JSC::VM& vm)
            : at(at)
            , vm(&vm)
        {
        }
...
private:
    bool withdrawDeadline(); // returns whether it requested a termination
    bool cutShort(); // this run's deadline fired or its SIGINT arrived (does not consume either)

    JSC::VM& m_vm;
    std::optional<double> m_timeoutMs;
    SigintReceiver* m_sigintReceiver;
    RefPtr<Deadline> m_deadline;
    NodeVMRunTermination* m_enclosing; // the run this one is nested in on this thread, if any
