...
JSC_DEFINE_HOST_FUNCTION(constructMIMEParams, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    JSC::Structure* structure = rustGlobalObject->m_JSMIMEParamsClassStructure.get(rustGlobalObject);

    JSC::JSValue newTarget = callFrame->newTarget();
    if (rustGlobalObject->m_JSMIMEParamsClassStructure.constructor(rustGlobalObject) != newTarget) [[unlikely]] {
        if (!newTarget) {
...
JSValue createJSMIMEBinding(Rust::GlobalObject* globalObject)
{
    VM& vm = globalObject->vm();
