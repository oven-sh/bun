...
{
    JSMIMEType* thisObject = uncheckedDowncast<JSMIMEType>(cell);
...
JSC_DEFINE_HOST_FUNCTION(constructMIMEType, (JSGlobalObject * globalObject, CallFrame* callFrame))
{
    ...
    JSC::Structure* structure = rustGlobalObject->m_JSMIMETypeClassStructure.get(rustGlobalObject);

    JSC::JSValue newTarget = callFrame->newTarget();
    if (rustGlobalObject->m_JSMIMETypeClassStructure.constructor(rustGlobalObject) != newTarget) [[unlikely]] {
        if (!newTarget) {
    ...
    JSC::Structure* paramsStructure = rustGlobalObject->m_JSMIMEParamsClassStructure.get(rustGlobalObject);
