...
static_assert(std::is_aggregate_v<GPUComputePipelineDescriptor>);
static_assert(IsIncreasing<
      0
    , offsetof(GPUComputePipelineDescriptor, compute)
>);
...
template<> ConversionResult<IDLDictionary<GPUComputePipelineDescriptor>> convertDictionary<GPUComputePipelineDescriptor>(JSGlobalObject& lexicalGlobalObject, JSValue value)
{
    ...
    if (isNullOrUndefined)
        ...
        computeValue = WebCore::get(object, &lexicalGlobalObject, Identifier::fromString(vm, "compute"_s));
        RETURN_IF_EXCEPTION(throwScope, ConversionResultException { });
    }
    ...
    return GPUComputePipelineDescriptor {
        ...
        computeConversionResult.releaseReturnValue(),
    };
}
