...
// `ComptimeStringMap` is implemented as a `phf::Map` *value* (see PORTING.md
// §Collections), so callers pass a `&'static phf::Map` and the value type is
// the generic `V`.
...
pub fn from_js<V: Copy>(
    ...
    // `phf` keys are `&[u8]`, so materialize UTF-8 bytes and do a direct
    // phf lookup.
