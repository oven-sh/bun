...
#![allow(
    ...
    // The generated `raw::` block intentionally re-declares many symbols already
    // bound elsewhere in `bun_jsc` with reference-typed params; both spellings are
    // ABI-identical (`&T` ≡ non-null `*const T`). The hand-written decls migrate
    // here over time.
