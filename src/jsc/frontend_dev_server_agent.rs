...
//!
//! The agent's per-VM state is a typed field on the `Debugger`
//! (`extension_agent: BunFrontendDevServerAgent`): `agent` is the C++
//! `Inspector::InspectorBunFrontendDevServerAgent*` pushed by
//! `Bun__InspectorBunFrontendDevServerAgent__setEnabled`, `sequence` is
//! `next_inspector_connection_id`.

use bun_core::String as BunString;

use crate::debugger::DebuggerId;
...
/// `BunFrontendDevServerAgent` — the `Debugger`'s dev-server inspector agent
/// state. The two high-tier types involved (`DevServer.RouteBundle.Index` in
...
/// before calling.
///
/// Both fields are `Copy` `Cell`s, so every method takes `&self` —
/// callers reach this through a shared `&Debugger` borrow.
pub struct BunFrontendDevServerAgent {
    /// C++ `Inspector::InspectorBunFrontendDevServerAgent*`; null while the
    /// domain is disabled. Set by `frontend_dev_server_agent_set_enabled`.
    agent: core::cell::Cell<*mut InspectorBunFrontendDevServerAgentHandle>,
    /// `next_inspector_connection_id` — free-running counter.
    sequence: core::cell::Cell<i32>,
}

impl Default for BunFrontendDevServerAgent {
    fn default() -> Self {
        Self {
            agent: core::cell::Cell::new(core::ptr::null_mut()),
...
pub fn frontend_dev_server_agent_set_enabled(agent: *mut InspectorBunFrontendDevServerAgentHandle) {
    ...
    if let Some(dbg) = VirtualMachine::get().debugger.as_deref() {
        dbg.extension_agent.agent.set(agent);
    }
}
