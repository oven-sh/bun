...
//!      `build/*/codegen/cpp.rs`). Each emits a
...
//!      `BunGeneratedClasses.cpp`). Each `JS${Type}` exposes
...
//
// The bindgen emits per-field newtypes with `.get()` (optional) /
...
// ──────────────────────────────────────────────────────────────────────────
// Extern-ABI helper layouts (matched 1:1 with the C++ `bindgen` shims).
//
// `ExternTaggedUnion` is `extern struct { data:
...
// ──────────────────────────────────────────────────────────────────────────
// SocketConfigHandlers (generated by bindgen)
// ──────────────────────────────────────────────────────────────────────────
...
impl SocketConfigHandlers {
    ...
    pub fn from_js(global: &JSGlobalObject, value: JSValue) -> JsResult<Self> {
        // Wrap this in an `ExceptionValidationScope` for the
...
// ──────────────────────────────────────────────────────────────────────────
// SSLConfig — bindgen option-struct.
// ──────────────────────────────────────────────────────────────────────────
...
// (= `bun_core::String`), which is `Copy` and has no `Drop`. The original
...
// (matches the bindgen `SSLConfig.deinit` shim).
//
        ...
        match ext.tag {
            ...
            4 => {
                ...
                if !arr.data.is_null() {
                    ...
                    // PORT NOTE: the original `BindgenArray` reuses the allocation in-place
                    // when the inner and extern types match. Phase A copies-then-frees the
...
// ──────────────────────────────────────────────────────────────────────────
// SocketConfig — bindgen option-struct.
// ──────────────────────────────────────────────────────────────────────────
...
// ──────────────────────────────────────────────────────────────────────────
// Per-class cached-accessor modules (`jsc.Codegen.JS*`).
//
...
///
/// Mirrors `jsc.Codegen.JS${T}` (one impl, generated once — see
...
macro_rules! js_class_module {
    ...
    (
        ...
        pub mod $mod_name {
            ...
            /// Compat alias for [`to_js`] with `(global, ptr)` argument
            ...
            /// subclass of) the wrapper type. Mirrors the upstream
            /// `js.dangerouslySetPtr` (BunGeneratedClasses.cpp).
            ///
            /// # Safety
