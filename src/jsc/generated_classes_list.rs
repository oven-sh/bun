//! Maps each `.classes.ts` class name to its native backing type.
//!
//! LAYERING: the original `Classes` namespace was a flat list of
...
//!
//! The Rust codegen output (`generated_classes.rs`) does **not** consume this
//! list — it resolves each class to its Rust struct via
//! `rustModuleResolver.resolveStruct` (`generate-classes.ts:2602`/`:3450`)
//! and is `include!`d into `bun_runtime` where every backing type is already
//! in scope.
//!
