...
/// (`headers-handwritten.h:123`).
///
/// The module body is generated by `src/codegen/bundle-modules.ts` (the same
/// codegen that emits `ResolvedSourceTag.zig` / `SyntheticModuleType.h`), so
/// the InternalModuleRegistry IDs (`(1 << 9) | id`) cannot drift from the
/// registry. See `${BUN_CODEGEN_DIR}/ResolvedSourceTag.rs`.
