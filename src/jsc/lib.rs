...
// ──────────────────────────────────────────────────────────────────────────
// WebCore — opaque FFI handles for `Request`/`Response`/`Blob` so lower-tier
// crates (`bun_js_parser_jsc::Macro`, `bun_bundler_jsc`) can downcast and call
// C-ABI accessors without depending on `bun_runtime` (cycle break). The full
// field layouts live in `bun_runtime::webcore::{Request,Response,Blob}`.
// ──────────────────────────────────────────────────────────────────────────
...
pub mod WebCore {
    ...
    /// `JSRequest`. Lower-tier crates only downcast (`JSValue::as_::<Request>()`)
    /// and call C-ABI accessors; the full field layout lives in
    /// `bun_runtime::webcore::Request`.
    #[repr(C)]
    pub struct Request {
        _opaque: core::cell::UnsafeCell<[u8; 0]>,
    ...
    unsafe extern "C" {
        ...
        /// pattern as `Bun__Blob__sharedView`). Bodies live at the higher tier.
    ...
    impl crate::JsClass for Request {
        ...
        fn to_js(self, _g: &crate::JSGlobalObject) -> crate::JSValue {
            // Opaque at this tier; construction via `bun_runtime`.
            crate::JSValue::UNDEFINED
        }
        unsafe fn to_js_ptr(_p: *mut Self, _g: &crate::JSGlobalObject) -> crate::JSValue {
            crate::JSValue::UNDEFINED
        }
    ...
    impl Response {
        /// `Response.getBlobWithoutCallFrame` (BodyMixin, body.zig).
        pub fn get_blob_without_call_frame(
            &mut self,
            global: &crate::JSGlobalObject,
        ) -> crate::JsResult<crate::JSValue> {
    ...
    /// `webcore.ByteListPool` — thread-local recycling pool for response body
    /// byte buffers. Full impl lives in `bun_runtime::webcore`; this stub lets
    /// the process-exit hook in `bun.rs` link.
    pub mod ByteListPool {
        /// No-op at this tier; `bun_runtime` overrides via the same symbol.
        pub fn delete_all() {}
    }
}
/// Lower-case alias for [`WebCore`].
