...
impl<'s> Scope<'s> {
    ...
    pub fn string(&self, s: &bun_core::String) -> JsResult<Local<'s>> {
        Ok(self.local(crate::bun_string_jsc::StringJsc::to_js(s, self.global)?))
    }
    ...
    pub fn transfer_string(&self, s: bun_core::String) -> JsResult<Local<'s>> {
        Ok(self.local(crate::bun_string_jsc::StringJsc::into_js(s, self.global)?))
    }
...
impl<'s> Local<'s> {
    ...
    /// String coercion into UTF-8 bytes that own their ref/allocation; may
    /// run a String subclass's `toString`.
    #[inline(always)]
    pub fn to_utf8(self, scope: &mut Scope<'s>) -> JsResult<bun_core::Utf8Bytes<'static>> {
        self.raw.to_utf8(scope.global)
    }
