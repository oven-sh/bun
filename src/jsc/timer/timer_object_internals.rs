...
impl TimerObjectInternals {
    ...
    fn set_enable_keeping_event_loop_alive(&self, vm: *mut VirtualMachine, enable: bool) {
        ...
        // SAFETY: `vm` is the live per-thread VM (hook contract); raw place read.
        let timer = unsafe { (*vm).timer };
        debug_assert!(!timer.is_null(), "VM timer not installed");
        ...
        match self.flags.get().kind() {
            // SAFETY: `timer` is the per-VM `All` heap (separate allocation from
            // the VM); single-threaded JS heap so no concurrent `&mut`.
            Kind::SetTimeout | Kind::SetInterval => unsafe {
                (*timer).increment_timer_ref(delta, uws_loop)
            },
            // setImmediate has slightly different event loop logic
            // SAFETY: as above.
            Kind::SetImmediate => unsafe { (*timer).increment_immediate_ref(delta, uws_loop) },
        }
    }
    ...
    pub fn init(
        ...
        *self = Self {
        ...
        // Record the current global epoch on the timer node. `kind` was just
        // written above, so `parent_ptr()`/`event_loop_timer()` resolve.
        // SAFETY: `(*vm).timer` is the live per-VM `All` heap (separate
        // allocation); single-threaded JS heap so no concurrent `&mut`.
        unsafe {
            debug_assert!(!(*vm).timer.is_null(), "VM timer not installed");
            (*self.event_loop_timer()).js_epoch = (*(*vm).timer).epoch;
        }
    ...
    /// `vm.event_loop().enter()` then re-enters JS which may itself touch the
    /// VM/EventLoop (and `(*timer_all())` via `cancel()`/`refresh()`);
                ...
                if kind == KindBig::SetInterval {
                    ...
                    match s.event_loop_timer_state() {
                        EventLoopTimerState::FIRED => {
                            ...
                            // SAFETY: `timer` is the live per-VM `All` heap
                            // (separate allocation from the VM); single-threaded
                            // JS heap so no concurrent `&mut`. `event_loop_timer()`
                            // derives a fresh raw ptr (no `&mut` aliasing across
                            // `update`).
                            unsafe { (*timer).update(s.event_loop_timer(), &time_before_call) };
    ...
    pub(crate) fn reschedule(
        ...
        // SAFETY: `vm` is the live per-thread VM; raw place read.
        let all = unsafe { (*vm).timer };
        debug_assert!(!all.is_null(), "VM timer not installed");
        ...
        if was_active {
            // SAFETY: `all` is the live per-VM `All` heap (separate allocation
            // from the VM); fresh `&mut` for this call only.
            unsafe { (*all).remove(self.event_loop_timer()) };
        } else {
            self.ref_();
        }
        ...
        unsafe { (*all).update(self.event_loop_timer(), &scheduled_time) };
    ...
    /// The per-thread `VirtualMachine` and its timer heap are installed (always
    ...
    pub unsafe fn deinit(&mut self) {
        ...
        if self.event_loop_timer_state() == EventLoopTimerState::ACTIVE {
            // SAFETY: `all` is the live per-VM `All` heap (separate allocation
            // from the VM); single-threaded JS heap so no concurrent `&mut`.
            unsafe { (*all).remove(self.event_loop_timer()) };
        }
        ...
        if self.flags.get().has_accessed_primitive() {
            // SAFETY: as above — fresh `&mut` to `.maps` for this call.
            let map = unsafe { (*all).maps.get(kind) };
            if map.remove(&self.id).is_some() {
                ...
                // SAFETY: as above.
                let _ = unsafe { (*all).maps.set_timeout.remove(&self.id) };
            }
        }
...
impl TimerObjectInternals {
    ...
    pub fn to_primitive(&self) -> JsResult<JSValue> {
        if !self.flags.get().has_accessed_primitive() {
            ...
            let all = crate::timer::timer_all();
            debug_assert!(!all.is_null(), "VM timer not installed");
            // Note: reshaped for borrowck — capture `event_loop_timer` ptr
            // before borrowing `(*all).maps`.
            let elt = self.event_loop_timer();
            // SAFETY: `all` is the live per-VM `All` heap; single-threaded JS
            // heap so no concurrent `&mut` to `.maps`.
            unsafe { (*all).maps.get(self.flags.get().kind()).put(self.id, elt) }?;
        }
    ...
    pub(crate) fn cancel(&self, vm: *mut VirtualMachine) {
        ...
        if was_active {
            // SAFETY: `vm` is the live per-thread VM (caller contract); raw place read.
            let all = unsafe { (*vm).timer };
            debug_assert!(!all.is_null(), "VM timer not installed");
            // SAFETY: `all` is the live per-VM `All` heap (separate allocation
            // from the VM); single-threaded JS heap so no concurrent `&mut`.
