...
/// captured `Ref<>`s are released while the JSC VM is still alive). Without
/// this, any `postTaskTo(workerCtxId, …)` that lands between the worker's
/// last `tick_concurrent()` and context-map removal leaks a `ConcurrentTask`
/// box plus its `EventLoopTask*` payload: `EventLoop::deinit()` walks only
...
pub fn drop_concurrent_cpp_tasks_for_worker(global: &JSGlobalObject) {
    ...
    // with the context already out of the global map (sole producer path
    // closed) and JSC still live; `bun_vm()` is the thread-local handle,
    // `event_loop()` never null for a Bun VM.
    unsafe {
