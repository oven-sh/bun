...
// `Bun__BunGlobalObject__uvLoop` is Windows-only; gated with `#[cfg(windows)]`
// on the fn definition itself.
//
...
pub fn read_origin_timer(vm: &VirtualMachine) -> u64 {
    ...
    // `origin_timer` is an `Instant`; report nanoseconds elapsed since origin.
...
#[cfg(windows)]
#[unsafe(no_mangle)]
pub extern "C" fn Bun__BunGlobalObject__uvLoop(jsc_vm: &mut VirtualMachine) -> *mut c_void {
    jsc_vm.uv_loop().cast()
}
...
pub fn add_source_provider_source_map(
    ...
    let slice = specifier.to_utf8();
    vm.source_mappings
        .put_bun_source_provider(opaque_source_provider, slice.slice());
}
...
pub fn remove_source_provider_source_map(
    ...
    let slice = specifier.to_utf8();
    vm.source_mappings
        .remove_bun_source_provider(opaque_source_provider, slice.slice());
}
