...
impl WebWorker {
    ...
    fn shutdown(&self) {
        ...
        if !vm_ptr.is_null() {
            ...
            // wakes this thread's loop when it finishes; the job slot itself
            // (and its WorkPool queue link) lives inside the VM box. Join it
            // before the loop teardown below and the VM free in `destroy()`.
