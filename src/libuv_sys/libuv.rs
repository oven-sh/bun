...
/// Win32 `SOCKET` is `UINT_PTR` (an integer), not a pointer; matches the original
...
impl uv_buf_t {
    ...
    pub fn slice(&self) -> &[u8] {
        // The original `this.base[0..this.len]` is well-defined for `(null, 0)`; Rust's
    ...
    /// Mutable view of the buffer (original `uv_buf_t::slice` returned `[]u8`, which
...
thread_local! {
    ...
    /// storage (zero-alloc; mirrors the original). `MaybeUninit` because the slot is
...
impl Loop {
    /// `bun.windows.libuv.Loop.get()`. Returns this thread's
    ...
    /// `bun.windows.libuv.Loop.shutdown()`. Closes this
    ...
    pub fn shutdown() {
        THREADLOCAL_LOOP.with(|slot| {
            ...
            if let Some(err) = unsafe { uv_loop_close(loop_) }.raw_errno() {
                // Originally `if (err == .BUSY)` — only EBUSY means handles are
                ...
                if err == (UV_EBUSY as c_int).unsigned_abs() as u16 {
                    ...
                    // NOTE the call is unconditional (original `bun.debugAssert`
    ...
    /// Aliases matching the original `pub const ref = inc`.
    #[inline]
    ...
    pub fn unref_count(&mut self, count: i32) {
        ...
        // Originally `-|= @as(u32, @intCast(count))` — `@intCast` is safety-checked
...
// ──────────────────────────────────────────────────────────────────────────
// `HandleMixin` — Rust ports this as a generic trait every
...
pub unsafe trait UvHandle: Sized {
    ...
    /// `handle.data = bun.new(T, ..)` pattern — callers never spell the
...
pub unsafe trait UvStream: UvHandle {
    ...
    /// Port of `StreamMixin::readStart` — high-level wrapper
    ...
    /// monomorphisation via the [`StreamReader`] trait (original captures them as
    ...
    ///
    /// Unlike the original (`error_cb` takes `bun.sys.E`), the Rust binding passes the
...
/// `comptime` fn-pointer parameters on the original `StreamMixin::readStart`:
/// Rust monomorphises the `extern "C"` trampolines over
/// this trait so the callbacks are baked into the codegen (zero-alloc, no
/// per-handle storage) exactly as in the original.
...
impl uv_write_t {
    ...
    /// Context-aware `uv_write` (original `uv_write_t::write` with
    ...
    ///
    /// PORT NOTE: original captures `onWrite` at *comptime* (one thunk per
    ...
    pub fn write<T>(
        ...
        unsafe extern "C" fn thunk<T>(req: *mut uv_write_t, status: ReturnCode) {
            ...
            // Pass the raw `*mut T` straight through (matches the original
...
impl Pipe {
    /// `uv_pipe_init`. Returns the raw `ReturnCode`; callers
    ...
    /// `StreamMixin::listen`. Caller supplies a plain
    ...
    /// `Pipe::closeAndDestroy` — close the pipe handle (if
    ...
    /// original); if a non-freeing callback was registered, the pipe leaks.
...
impl uv_fs_event_t {
    ...
    /// Port of `uv_fs_event_t.hash` — `std.hash.Wyhash` over
...
impl uv_stat_t {
    ...
    // POSIX `stat.mode`/`stat.size` (`st.mode`, `st.size`) can call
...
impl fs_t {
    ...
    /// Originally `pub const uninitialized: fs_t` — kept as a fn (raw-pointer fields
    ...
    /// `req.file.fd` (originally `union(.{ pathw, fd })` arm). The union is private
...
///
/// This is the leaf-crate copy of `ReturnCode.errno()`'s switch.
...
/// codes return `None`, matching the original `else => null`.
///
...
// the original. Auditing the bound once per type here lets every
...
impl ReturnCode {
    ...
    /// `bun.sys.E` value (e.g. 16 for `BUSY`) that the original `errno()` returns.
    #[inline]
    ...
    /// `errEnum()` — same translated value as
    ...
    /// Layer-free `< 0` check (originally `Maybe(void).isErr()` after `.toError`).
...
impl ReturnCodeI64 {
    ...
    /// Originally `toFD()` — `req.result` after a successful `uv_fs_open` is the
...
// caller as a raw `i32` (per `bun.O` on Windows —
...
pub mod O {
    ...
    // mirror `bun_sys` — cross-crate static asserts live in `bun_sys` (this
    // crate stays leaf). The original pulled these from `bun.O`; the constants
    // are stable.
    mod bun_o {
        ...
        // `.windows => { NOCTTY = 0 }` — meaningless on Windows.
        pub const NOCTTY: i32 = 0;
    ...
    pub fn from_bun_o(c_flags: i32) -> i32 {
        ...
        // NOTE: `& != 0` (any-overlap) matches the spec verbatim;
        // a DSYNC-only input also takes this branch — if that
...
pub mod StdioFlags {
    ...
    // SCREAMING_CASE aliases — the original exposes both via the implicit
...
// ──────────────────────────────────────────────────────────────────────────
// extern "C" — full libuv FFI surface.
// ──────────────────────────────────────────────────────────────────────────
unsafe extern "C" {
    // version / loop
