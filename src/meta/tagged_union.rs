...
//!
//! Rust enums *are* tagged unions and drop their active variant automatically,
//! so the per-arity `deinit` dispatch the original implementation needed
//! collapses to nothing — the compiler-generated `Drop` glue handles it.

// PORT NOTE: `deinitImpl` is intentionally not ported as a function. Dropping
// the active variant in place is precisely what Rust's auto-generated enum
// `Drop` glue does. There is no behavior to translate — every `TaggedUnion`
...
///
/// Usage: `tagged_union!(pub MyUnion; A, B, C)`.
///
/// Rust enums drop their active variant automatically, so no explicit `Drop`
/// impl is emitted here.
// TODO(port): the original `TaggedUnion` returned an *anonymous* type usable
// inline as an expression. Rust macros can only mint named item-position types,
// so callers must supply a name: `tagged_union!(pub MyUnion; A, B, C);`. Adjust
// call sites in Phase B.
#[macro_export]
macro_rules! tagged_union {
    // 0 types — compile error
    ($vis:vis $name:ident;) => {
