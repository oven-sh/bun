...
/// link roots in a `#![no_std]`/`no_main` binary; this crate has none. Provides
/// NUL-terminated length/span helpers that both consumers re-spelled verbatim
/// before this single audited copy existed.
pub mod ffi {
    /// Count u16 code units up to (and excluding) the first NUL. Single
    /// audited funnel for the hand-rolled `while *p.add(n) != 0 { n += 1 }`
    /// loop that appeared at every `LPCWSTR` / `char16_t*` ingestion point
    /// (Windows path APIs, N-API `napi_create_string_utf16`, libarchive `_w`
    /// accessors, env-block scan). Adds a `debug_assert!(!p.is_null())` —
    /// same precondition as `CStr::from_ptr`.
    ///
    /// # Safety
