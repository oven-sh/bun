...
pub struct TestOptions {
    ...
    /// significant figures when serializing snapshots (à la
    /// `Number.prototype.toPrecision`), so snapshots stay stable across CPU
    /// architectures. 0 = disabled (shortest round-trip). Valid range 1..=100.
