...
fn encodeResource(w: pb.Writer, res: *const span.Resource) OOM!void {
    for (res.attributes) |*a| {
        try w.writeSubmessage(tags.Resource.attributes, a, encodeKeyValue);
    }
...
fn encodeScope(w: pb.Writer, scope: *const span.InstrumentationScope) OOM!void {
    ...
    for (scope.attributes) |*a| {
        try w.writeSubmessage(tags.InstrumentationScope.attributes, a, encodeKeyValue);
    }
...
fn encodeSpan(w: pb.Writer, s: *const span.Span) OOM!void {
    ...
    for (s.attributes) |*a| {
        try w.writeSubmessage(tags.Span.attributes, a, encodeKeyValue);
    }
...
fn encodeEvent(w: pb.Writer, e: *const span.Event) OOM!void {
    ...
    for (e.attributes) |*a| {
        try w.writeSubmessage(tags.Span_Event.attributes, a, encodeKeyValue);
    }
...
fn encodeLink(w: pb.Writer, l: *const span.Link) OOM!void {
    ...
    for (l.attributes) |*a| {
        try w.writeSubmessage(tags.Span_Link.attributes, a, encodeKeyValue);
    }
...
fn encodeKeyValue(w: pb.Writer, kv: *const Attribute) OOM!void {
    if (kv.key.len > 0) try w.writeString(tags.KeyValue.key, kv.key);
    try w.writeSubmessage(tags.KeyValue.value, &kv.value, encodeAnyValue);
}

fn encodeAnyValue(w: pb.Writer, v: *const AnyValue) OOM!void {
    switch (v.*) {
        .string => |s| try w.writeString(tags.AnyValue.string_value, s),
        .boolean => |b| try w.writeBool(tags.AnyValue.bool_value, b),
        .int => |i| try w.writeInt64(tags.AnyValue.int_value, i),
        .double => |d| try w.writeDouble(tags.AnyValue.double_value, d),
        .bytes => |b| try w.writeBytes(tags.AnyValue.bytes_value, b),
        .array => |arr| try w.writeSubmessage(tags.AnyValue.array_value, arr, encodeArrayValue),
        .kvlist => |kvs| try w.writeSubmessage(tags.AnyValue.kvlist_value, kvs, encodeKeyValueList),
        .empty => {},
    }
}

fn encodeArrayValue(w: pb.Writer, arr: []const AnyValue) OOM!void {
    for (arr) |*v| {
        try w.writeSubmessage(tags.ArrayValue.values, v, encodeAnyValue);
    }
}

fn encodeKeyValueList(w: pb.Writer, kvs: []const Attribute) OOM!void {
    for (kvs) |*kv| {
        try w.writeSubmessage(tags.KeyValueList.values, kv, encodeKeyValue);
    }
}
