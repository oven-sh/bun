...
//!
//! Downstream crates reference scoped logging as `bun_core::declare_scope!` /
//! `bun_core::scoped_log!`. The actual implementation lives in `bun_core`
...
//
//     bun_core::declare_scope!(X, visible);
//     bun_core::scoped_log!(X, "fmt {} {}", a, b);
//
...
// (`bun_core::ScopedLogger`, `bun_core::Visibility::Hidden`).
pub use bun_core::output::{ScopedLogger, Visibility};

// ── pass-through of the rest of the Output surface ───────────────────────
// Downstream code also reaches for `bun_core::error_writer()`,
// `bun_core::pretty_fmt!`, etc. Glob-re-export the module + the other
...
/// `bun_core::scope_is_visible!(X)` — sugar for `X.is_visible()` on a
