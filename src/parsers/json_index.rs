//! Stage 1 of the JSON parser: the streaming structural index — the byte
//! offset of every token-significant position (structurals outside strings,
//! both quotes of every string, the first byte of every other scalar run),
//! terminated by two sentinels. Produced by the Highway SIMD kernel
//! (`src/jsc/bindings/highway_json.cpp`) or, for documents with comments /
//! single quotes and on targets without Highway, the scalar indexer below.
use bun_ast::Range;

// Generated from `scripts/build/jsonByteClass.ts`, the single definition of
// byte classification shared with the Highway JSON kernel. No byte >= 0x80
// classifies: multi-byte UTF-8 between tokens must stay in one run.
pub mod byte_class {
...
/// Input bytes indexed per SIMD refill (a multiple of 4096). Also bounds
/// the index window.
...
/// Errors the indexer itself can detect. The index stream is truncated at
/// the error and the driver reports it in preference to stage 2's outcome.
#[derive(Clone, Copy)]
pub enum IndexError {
    /// `/*` with no closing `*/`.
    ...
    /// The document is larger than a `Loc` (`i32`) can address.
    DocumentTooLarge,
}
...
pub struct StructuralIndex<'c> {
    ...
    /// sequence (absolute byte offsets, strictly increasing).
    win: Vec<u32>,
    base: usize,
    /// One bit per 64-byte input block: "a backslash or a control character
    /// inside a string lives here".
    dirty: Vec<u64>,
...
impl<'c> StructuralIndex<'c> {
    ...
    fn with_producer(contents: &'c [u8], use_scalar: bool) -> Self {
        ...
        // kernel's overshoot, the sentinels, and the look-behind band.
    ...
    /// Byte position of index `logical`, producing more of the index as
    /// needed. `logical` may lag the furthest position ever requested by at
    /// most [`LOOKBEHIND`].
    #[inline(always)]
    ...
    fn fill_to(&mut self, logical: usize) {
        while logical - self.base >= self.win.len() {
            ...
            // Slide, keeping `LOOKBEHIND` entries before the requested index.
    ...
    fn refill_once(&mut self) {
        ...
        if bun_core::env::IS_NATIVE && !self.use_scalar {
            ...
            if chunk_flags & FLAG_ODDITY != 0 {
                // Restart with the scalar indexer, swallowing a *count* of
                // already-delivered indices — this only works because the two
                // producers emit identical index streams.
    ...
    /// a backslash or a control character inside a string?
    #[inline(always)]
    ...
    /// Comment- and single-quote-aware scalar indexer; the reference
    /// implementation the SIMD kernel is differentially tested against.
    /// Comment bytes produce no indices at all. Resumable: it stops emitting
    /// at a token boundary when the window is full and continues from `s_i`.
    fn scalar_refill(&mut self) {
        let s = self.contents;
        let n = s.len();
        // An iteration emits at most 2 entries.
            ...
            match c {
                ...
                _ => {
                    ...
                    if cls & CLASS_STRUCTURAL != 0 {
                        ...
                        // A backslash outside of a string "escapes" the next
                        // byte, like the kernel's odd-backslash-run parity.
                        if c == b'\\' && !was_escaped {
...
/// U+2028 / U+2029 (3-byte UTF-8: E2 80 A8/A9), which terminate `//` comments.
#[inline]
...
mod tests {
    ...
    /// Index `contents` with both producers.
    ...
    fn simd_fallback_to_scalar_mid_document_is_seamless() {
        // Oddity far into the document, long after SIMD indices were consumed.
