...
//! against the `is_json = true` branch only — every JS-only arm (operators,
//! JSX, regexp, template literals, pragmas, hashbang, private identifiers,
//! bigint) is dropped or hard-errors via `add_unsupported_syntax_error`. The
//! full `T` token enum is collapsed to the tokens the JSON parser matches on.
//!
//! Covers `JSONOptions`, `NewLexer_` with `is_json = true`, `next`,
//! `parseStringLiteral`, `parseNumericLiteralOrDot`,
...
/// `js_lexer_tables.T` subset. This enum carries ONLY the variants the JSON
...
impl T {
    /// `tokenToString.get(token)`. Only the JSON subset has names; the rest
...
/// JSON-only lexer. Field layout mirrors the canonical `LexerType` for the
/// fields the JSON parser reads directly (`token`, `number`,
/// `has_newline_before`, `source`, `end`, `is_ascii_only`, `identifier`,
/// `indent_info`).
///
/// PORT NOTE — borrowck/Stacked Borrows: storing the `Log` pointer on both
/// the lexer *and* the parser would alias. The Rust port keeps a single
/// `*mut Log` here as the sole provenance chain; the parser does **not** hold
/// its own `&mut Log` (that would alias this pointer and any parser-side `&mut`
/// deref would invalidate the lexer's SharedReadWrite tag — UB the next time
/// the lexer reports an error). Parser-side log writes go through `log_mut()`.
/// Matches the toml lexer's shape.
    ...
    'bump: 'a,
{
    ...
    /// `init` — `NewLexer(opts).init`. The parser's 8 const-generic bools are
    /// flattened back into a runtime `JSONOptions` here; the JSON token loop is
    /// small enough that monomorphizing on the options buys nothing at this
    /// layer (the full JS lexer is where it mattered).
    pub fn init(
        log: &mut bun_ast::Log,
        source: &'a bun_ast::Source,
        bump: &'bump Bump,
    ...
    /// `initJSON` — identical body to `init` for the JSON lexer; kept as a
    ...
    fn expected_string(&mut self, text: &str) -> LexResult {
        ...
        // `expectedString` only logs via `addRangeError` and returns void; it
        // does NOT raise. `expect()` then falls through to `next()` for error
        // recovery.
        Ok(())
    }
    ...
    pub fn unexpected(&mut self) -> LexResult {
        ...
        // `unexpected` only logs and returns void; callers (e.g. `parse_expr`
        // else arm) follow with an explicit parser error if they need to
        // abort.
        Ok(())
    }
    ...
    pub fn parse_string_literal(&mut self, quote: CodePoint) -> LexResult {
        match quote {
            ...
            q if q == '\'' as CodePoint => self.parse_string_literal_inner::<b'\''>(),
            // Only the three above are ever passed; anything else is a port bug.
            _ => unreachable!("parse_string_literal: invalid quote {}", quote),
        }
    }

    /// `parseStringLiteral` with `is_json = true`, template-literal (`\``) and
    /// `$`-substitution arms removed. `QUOTE == 0` is the implicit (.env
    /// auto-quote) string.
    ///
    /// PERF: `QUOTE` is a const generic so the per-byte `match` folds to a
    /// guard-free jump table and the `QUOTE == 0` / SIMD-gate checks
    /// const-propagate away. `is_json` is hard-wired `true` (this module *is*
    /// the is_json slice — see file header) so the `< 0x20` control-char check
    /// is branchless on the option load.
    fn parse_string_literal_inner<const QUOTE: u8>(&mut self) -> LexResult {
            ...
            match self.code_point {
                ...
                cp => {
                    ...
                    if cp >= 0x80 {
                        ...
                        // SIMD skip-ahead over plain ASCII string content.
                        // Critical for inline-sourcemap JSON where
                        // `sourcesContent` can be hundreds of KB.
        ...
        // The "JSON strings must use double quotes" error is gated on
        // `!FeatureFlags.allow_json_single_quotes`, and that flag is hard-set
        // to `true`, so the spec NEVER emits this error.
    ...
    /// `decodeEscapeSequences`. Ported for the `is_json = true` arm only —
    ...
    // canonical lexer additionally accepts `\0` `\v` `\x` `\u{…}` etc. and then
    // errors post-hoc via `is_json` checks. Mirror that once the surrogate-pair
                ...
                cp if cp == '\\' as CodePoint => {
                    ...
                    match c2 {
                        ...
                        cp if cp == 'v' as CodePoint => {
                            // Vertical tab is invalid JSON, but the canonical
                            // lexer's is_json check is COMMENTED OUT with note
                            // "We're going to allow it" — unconditionally push.
                            buf.push(0x0b);
                        }
                        // `'8','9'` pass through as the digit char (no is_json gate).
                        cp if cp == '8' as CodePoint || cp == '9' as CodePoint => {
                            push_codepoint(buf, c2);
                        }
                        // `\xNN` 2-digit hex (no is_json gate).
                        ...
                        // Only `"` `\` `/` are whitelisted for the is_json
                        // `else` arm; `\'` falls through to syntaxError.
                        cp if cp == '"' as CodePoint
                            || cp == '\\' as CodePoint
                            || cp == '/' as CodePoint =>
                        {
                        ...
                        _ => {
                            // `is_json` is always true in this module (see file
                            // header) — the JS pass-through arm is unreachable
                            // here.
                            return self.syntax_error();
                        }
                    }
    ...
    /// `parseNumericLiteralOrDot`. The canonical lexer has NO is_json gate on
    /// the radix-prefix integer-literal path (`0x…/0b…/0o…`/legacy octal) or
    /// underscore separators, so they are accepted here too. Only the bigint
    /// `'n'` token is JS-only and left to the trailing `is_identifier_start`
    /// syntax error.
    fn parse_numeric_literal_or_dot(&mut self) -> LexResult {
        ...
        if base != 0.0 {
            ...
            if is_bigint_literal || is_invalid_legacy_octal_literal {
                ...
                // (the original allocates a filtered buffer here but discards it;
                // mirror the no-op semantics — `text` is reused below as-is.)
    ...
    /// `scanIdentifierWithEscapes(.normal)`. Minimal port for the JSON keyword
    /// set: scans `\uXXXX` / `\u{…}` escapes interleaved
    ...
    /// `next()` with every `is_json` branch taken. Operators and JS-only
    /// punctuation hard-error; comments are gated on `opts.allow_comments`.
        ...
        loop {
            ...
            match self.code_point {
                ...
                cp if cp == '\\' as CodePoint => {
                    ...
                    // No is_json gate; scan an identifier-with-escapes
                    // (e.g. `true` → t_true).
