...
// `js_ast.E.Object.Rope`. The MOVE_DOWN landed it at `js_ast::e::Rope`.
...
// `E.Object.setRope` / `E.Object.getOrPutArray`.
// ──────────────────────────────────────────────────────────────────────────
...
// upstream too). Ported for structural fidelity; verify and delete in Phase B
// if truly unused.
mod hash_map_pool {
    use super::*;
    ...
    thread_local! {
        // Originally a thread-local `LinkedList` + `loaded: bool` pair.
    ...
    pub(super) fn release(node: *mut Node) {
        // Originally modeled as a sticky `loaded: bool` + `list.prepend(node)`;
...
pub struct TOML<'a> {
    pub lexer: Lexer<'a>,
    // PORT NOTE: a parser-side `log: *logger.Log` field is never read — all
    // logging goes through `lexer.log`. Dropped here to avoid a second
    // `&mut Log` borrow overlapping `lexer.log`.
    pub bump: &'a Bump,
...
impl<'a> TOML<'a> {
    ...
    // The original auto-dereferenced pointer payloads. In Rust the deref is
    // implicit at call sites, so this collapses to a single generic forwarding
    // to `Expr::init`.
    ...
    pub fn parse(
        ...
        // PORT NOTE: the `Lexer` borrows `Source` (`&'a Source`) so
    ...
    pub fn parse_key(&mut self, bump: &'a Bump) -> Result<&'a mut Rope, bun_core::Error> {
        // TODO(port): lifetime — originally returned `*Rope` allocated from a
        // stack-fallback arena reset per-iteration. Here we allocate from the
    ...
    fn run_parser(&mut self) -> Result<Expr, bun_core::Error> {
        ...
        let mut head: *mut E::Object = root
            ...
            .as_ptr();
        // TODO(port): `head` aliases into `root.data`; using a raw pointer to
        // sidestep overlapping `&mut` on `root`.

        // PERF(port): was a stack-fallback allocator (6 ropes' worth of stack)
    ...
    pub fn parse_value(&mut self) -> Result<Expr, bun_core::Error> {
        // Stack overflow is guarded only by `StackCheck`. The Rust port
        // previously added a hard depth cap because release-mode frames are
        // smaller than the original's (which didn't emit LLVM lifetime
        // annotations, so `parse_value`'s frame was the union of all locals
        // including the stack-fallback Rope buffer; Rust's is just the live
        // set). The cap was an artificial limit on a feature — the test's
        // `depth = 25_000` was calibrated to the old frame size and is now
        // bumped to a value that exhausts the 18 MB stack regardless.
        ...
        match self.lexer.token {
            ...
            T::t_open_brace => {
                ...
                let obj: *mut E::Object = expr
                    ...
                    .expect("infallible: variant checked")
                    .as_ptr();
                // TODO(port): `obj` aliases into `expr.data`; raw pointer to sidestep overlapping &mut.
            ...
            T::t_open_bracket => {
                ...
                let array: *mut E::Array = array_
                    ...
                    .expect("infallible: variant checked")
                    .as_ptr();
                // TODO(port): `array` aliases into `array_.data`; raw pointer to sidestep overlapping &mut.
                let bump = self.bump;
                self.lexer.allow_double_bracket = false;
