...
pub const PatchFile = struct {
    ...
    pub fn apply(this: *const PatchFile, allocator: Allocator, patch_dir: bun.FileDescriptor) ?bun.sys.Error {
        ...
        for (this.parts.items) |*part| {
            ...
            switch (part.*) {
                .file_deletion => {
                    const pathz = arena.allocator().dupeZ(u8, part.file_deletion.path) catch |oe| bun.outOfMemory(oe);
                ...
                .file_rename => {
                    const from_path = arena.allocator().dupeZ(u8, part.file_rename.from_path) catch |oe| bun.outOfMemory(oe);
                    const to_path = arena.allocator().dupeZ(u8, part.file_rename.to_path) catch |oe| bun.outOfMemory(oe);
                ...
                .file_creation => {
                    const filepath = bun.PathString.init(arena.allocator().dupeZ(u8, part.file_creation.path) catch |oe| bun.outOfMemory(oe));
                    ...
                    const file_contents = brk: {
                        var contents = file_alloc.alloc(u8, count) catch |oe| bun.outOfMemory(oe);
                        var i: usize = 0;
                ...
                .file_mode_change => {
                    ...
                    const filepath = arena.allocator().dupeZ(u8, part.file_mode_change.path) catch |oe| bun.outOfMemory(oe);
    ...
    fn applyPatch(
        ...
        const file_path: [:0]const u8 = arena.allocator().dupeZ(u8, patch.path) catch |oe| bun.outOfMemory(oe);
        ...
        // TODO: i hate this
        var lines = std.ArrayListUnmanaged([]const u8).initCapacity(bun.default_allocator, lines_count) catch |oe| bun.outOfMemory(oe);
        defer lines.deinit(bun.default_allocator);
        {
            var iter = std.mem.splitScalar(u8, filebuf, '\n');
            var i: usize = 0;
            while (iter.next()) |line| : (i += 1) {
                lines.append(bun.default_allocator, line) catch |oe| bun.outOfMemory(oe);
            }
        ...
        for (patch.hunks.items) |*hunk| {
            ...
            for (hunk.parts.items) |*part_| {
                ...
                switch (part.type) {
                    ...
                    .insertion => {
                        const lines_to_insert = lines.addManyAt(bun.default_allocator, line_cursor, part.lines.items.len) catch |oe| bun.outOfMemory(oe);
                    ...
                    .deletion => {
                        // TODO: check if the lines match in the original file?
                        lines.replaceRange(bun.default_allocator, line_cursor, part.lines.items.len, &.{}) catch |oe| bun.outOfMemory(oe);
                        if (part.no_newline_at_end_of_file) {
                            lines.append(bun.default_allocator, "") catch |oe| bun.outOfMemory(oe);
                        }
...
pub fn spawnOpts(
    ...
    const argv: []const []const u8 = brk: {
        ...
        const argv_buf = bun.default_allocator.alloc([]const u8, ARGV.len + 2) catch |oe| bun.outOfMemory(oe);
        argv_buf[0] = git;
        for (1..ARGV.len) |i| {
            argv_buf[i] = ARGV[i];
        }
    ...
    const envp: [:null]?[*:0]const u8 = brk: {
        ...
        const envp_buf = bun.default_allocator.allocSentinel(?[*:0]const u8, env_arr.len + @as(usize, if (PATH != null) 1 else 0), null) catch |oe| bun.outOfMemory(oe);
        for (0..env_arr.len) |i| {
...
pub fn gitDiffPreprocessPaths(
    ...
    const old_folder = if (comptime bun.Environment.isWindows) brk: {
        ...
        const cpy = allocator.alloc(u8, old_folder_.len + bump) catch |oe| bun.outOfMemory(oe);
    ...
    const new_folder = if (comptime bun.Environment.isWindows) brk: {
        const cpy = allocator.alloc(u8, new_folder_.len + bump) catch |oe| bun.outOfMemory(oe);
    ...
    if (bun.Environment.isPosix and sentinel) {
        return .{
            allocator.dupeZ(u8, old_folder) catch |oe| bun.outOfMemory(oe),
            allocator.dupeZ(u8, new_folder) catch |oe| bun.outOfMemory(oe),
        };
    }
