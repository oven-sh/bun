...
//! (`bun_paths::PathUnit`, `bun_sys::make_path::MakePathUnit`, `bun_core::Ch`)
//! — they never redefine the core surface.
...
pub trait PathChar: strings::PathByte + Ord {
    ...
    /// drive-letter handling in `resolve_path`.
    #[inline]
    ...
    /// `match` in the `u16` impl). Zero allocation.
...
impl PathChar for u16 {
    ...
    fn lit(s: &'static [u8]) -> &'static [u16] {
        // Rust cannot widen an arbitrary `&'static [u8]` at
