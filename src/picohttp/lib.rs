...
// raw ptr/len pairs as aliasing slices with no lifetime tracking. The buffer
...
/// Rust `&[u8]` has no guaranteed field order in `#[repr(C)]`, so we spell the
/// `{ptr, len}` fields out and expose `.name()` / `.value()`.
#[repr(C)]
#[derive(Clone, Copy)]
pub struct Header {
    name_ptr: *const u8,
...
pub struct HeaderList<'a> {
    pub list: &'a [Header],
    // TODO(port): only ever read through a shared reference, so using
    // `&'a [Header]` here. Revisit if a caller mutates through it.
}
...
impl<'a> Request<'a> {
    ...
    pub fn parse(buf: &'a [u8], src: &'a mut [Header]) -> Result<Request<'a>, ParseRequestError> {
        ...
        if rc > -1 {
            ...
            // so writing a NUL there is in-bounds (the const-cast is intentional).
