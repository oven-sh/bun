...
///
/// Rust has no compile-time field-name reflection, so the field projection
/// becomes a trait method (typically implemented via `core::mem::offset_of!`).
...
// Inherent assoc types are unstable; expose at module level.
...
impl<T: HasWeakPtrData> WeakPtr<T> {
    ...
    unsafe fn deref_internal(&mut self, value: NonNull<T>) {
        ...
        if weak_data.finalized() && count == 0 {
            // The allocation came from `bun.new(T, ...)` (i.e. `Box::new` +
            // `heap::alloc`).
