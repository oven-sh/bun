...
use crate::reactive_scopes::visitors::Transformed;
...
impl<'a> CollectDependenciesVisitor<'a> {
    ...
    /// Memoization level for an instruction lvalue that would normally be `Memoized`
    /// (calls, new-expressions, etc). When the lvalue's inferred type is a primitive
    /// the value is cheaply comparable with Object.is and the producing scope does
    /// not need to be retained for it.
    fn allocating_lvalue_level(&self, lvalue: IdentifierId) -> MemoizationLevel {
        let type_id = self.env.identifiers[lvalue.0 as usize].type_;
        let ty = &self.env.types[type_id.0 as usize];
        if is_primitive_type(ty) {
            if self.options.force_memoize_primitives {
                MemoizationLevel::Conditional
            } else {
                MemoizationLevel::Never
            }
        ...
        match value {
            ...
            InstructionValue::TaggedTemplateExpression { tag, .. } => {
                ...
                if let Some(lv) = lvalue {
                    lvalues.push(LValueMemoization {
                        place_identifier: lv,
                        level: self.allocating_lvalue_level(lv),
                    });
                }
                if no_alias {
                    return (lvalues, vec![]);
