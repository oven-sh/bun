//! Port of `react_compiler/entrypoint/suppression.rs` — see DESIGN.md.
/*
...
/// Bun-side stand-in for `react_compiler_ast::common::CommentData`.
///
/// Bun's parser does not currently surface a flat program-level comment list
/// through [`crate::Host`], so callers pass an empty slice and suppression
/// scanning is a no-op (Bun users opt out via config, not eslint comments).
// TODO(port): wire to the parser's preserved-comment table once exposed on Host.
#[derive(Debug, Clone)]
...
pub(crate) fn find_program_suppressions(
...
pub(crate) fn suppressions_to_compiler_error(suppressions: &[SuppressionRange]) -> CompilerError {
    assert!(
    ...
    for suppression in suppressions {
        ...
        diagnostic = diagnostic.with_detail(CompilerDiagnosticDetail::Error {
            loc: suppression.disable_comment.loc,
