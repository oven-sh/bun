...
pub fn redisErrorToJS(globalObject: *JSC.JSGlobalObject, message: ?[]const u8, err: RedisError) JSC.JSValue {
    const error_code: JSC.Error = switch (err) {
        ...
        error.TLSNotAvailable => JSC.Error.ERR_REDIS_TLS_NOT_AVAILABLE,
        error.TLSUpgradeFailed => JSC.Error.ERR_REDIS_TLS_UPGRADE_FAILED,
        error.AuthenticationFailed => JSC.Error.ERR_REDIS_AUTHENTICATION_FAILED,
        error.InvalidPassword => JSC.Error.ERR_REDIS_INVALID_PASSWORD,
        error.InvalidUsername => JSC.Error.ERR_REDIS_INVALID_USERNAME,
        error.InvalidDatabase => JSC.Error.ERR_REDIS_INVALID_DATABASE,
