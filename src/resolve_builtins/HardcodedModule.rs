...
// auto-derive them. Equality is not needed for `Alias`.
#[derive(Copy, Clone)]
pub struct Alias {
...
// Three lookup tables are built by concatenating these const arrays
...
// `phf_codegen` in build.rs.
// PERF(port): linear scan placeholder — replace with a perfect-hash map.
