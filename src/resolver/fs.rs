...
// POSIX-only fd ceiling tracking (Windows handles aren't ordered ints).
// `set_max_fd` early-returns on Windows; the static is still declared so the
// cross-platform `MAX_FD` symbol resolves.
#[cfg(not(windows))]
pub(crate) static mut MAX_FD: bun_sys::RawFd = 0;
#[cfg(windows)]
pub(crate) static mut MAX_FD: i32 = 0;
