...
impl DirEntry {
    ...
    /// Erase to the `bun_sys::fs::DirEntry` opaque seam handle. Low-tier crates
    /// (`bun_dotenv`, `bun_router`) accept the seam type because they sit below
    /// `bun_resolver` in the crate graph; this is the **single** place the
    /// resolver→sys reinterpret happens, so high-tier callers (bun_install,
    /// bun_bundler, bun_runtime) never open-code `as *mut ZST` casts.
    #[inline]
    pub fn as_sys_seam(&self) -> &bun_sys::fs::DirEntry {
        // SAFETY: `bun_sys::fs::DirEntry` is a `#[repr(C)]` ZST opaque whose
        // every method dispatches through `SYS_FS_VTABLE` below, which casts
        // the pointer back to `*const Self`. Pointer identity round-trips; no
