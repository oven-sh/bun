...
pub mod dir_entry_accessor {
    ...
    pub(crate) struct DirEntryNameWrapper {
        // BACKREF: borrowed slice into an `Entry`'s `base_` (real-case
        // basename). Valid only while the parent `DirEntry` is live and not
        // regenerated by `read_directory`. Stored as [`bun_ptr::RawSlice`]
        // (not `&'static [u8]`) per PORTING.md §Forbidden — the bytes live in
        // the arena-resident `Entry` or its own heap allocation, so minting a
    ...
    impl AccessorDirIter for DirEntryDirIter {
        ...
        fn next(&mut self) -> Maybe<Option<DirEntryIterResult>> {
            if let Some(value) = &mut self.value {
                ...
                // BACKREF: wrap the entry's real-case basename in a `RawSlice`
                // instead of fabricating `&'static [u8]` (PORTING.md §Forbidden).
                // The HashMap key is the ASCII-lowercased basename, so yielding
                // it would make the glob walker's literal components miss
                // mixed-case directory names (and build wrong-case paths on
                // case-sensitive filesystems). `base_` is never mutated after
                // construction and lives in the arena-resident `Entry` (or its
                // own heap allocation), valid until the next `read_directory`
                // regeneration; `name_slice()` re-narrows the lifetime so it
                // never escapes the iter result.
