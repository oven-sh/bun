...
// (FileSystem, RealFS, Path, PathName, EntriesOption, Implementation) until
// the body switches to `fs_full::*` wholesale. Entry/DirEntry, the read-file
// helpers, `ModKey`, and the shared `kind_impl`/`adjust_ulimit`/temp-dir
// bodies already live in `fs_full` and are re-exported or delegated to from
// the inline module.
pub mod data_url;
pub mod dir_info;
#[path = "fs.rs"]
