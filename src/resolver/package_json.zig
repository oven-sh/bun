    ...
    fn loadDefineDefaults(
        ...
        for (json.properties.slice()) |prop| {
            ...
            env.defaults.appendAssumeCapacity(.{
                .key = prop.key.?.data.e_string.asWtf8CollapseRope(allocator) catch bun.outOfMemory(),
                .value = prop.value.?.data.e_string.asWtf8CollapseRope(allocator) catch bun.outOfMemory(),
            });
        }
    }
    ...
    fn loadDefineExpression(
        ...
        for (json.properties.slice()) |prop| {
            switch (prop.key.?.data) {
                .e_string => |e_str| {
                    const str = e_str.asWtf8CollapseRope(allocator) catch bun.outOfMemory();
                    ...
                    if (strings.eqlComptime(str, "defaults")) {
                        ...
                        switch (prop.value.?.data) {
                            .e_string => |value_str| {
                                env.setBehaviorFromPrefix(value_str.asWtf8CollapseRope(allocator) catch bun.outOfMemory());
                            },
                            else => {
            ...
            if (framework_object.expr.asProperty("router")) |router| {
                if (router.expr.asProperty("dir")) |route_dir| {
                    switch (route_dir.expr.data) {
                        .e_string => |estr| {
                            const str = estr.asWtf8CollapseRope(allocator) catch bun.outOfMemory();
                            if (str.len > 0) {
                                pair.router.dir = str;
                        ...
                        .e_array => |array| {
                            ...
                            switch (count) {
                                0 => {},
                                1 => {
                                    const str = items[0].data.e_string.asWtf8CollapseRope(allocator) catch bun.outOfMemory();
                                    if (str.len > 0) {
                                        pair.router.dir = str;
                                        pair.router.possible_dirs = &[_]string{};
                                    ...
                                    for (items) |item| {
                                        if (item.data == .e_string and !item.data.e_string.isEmpty()) {
                                            list[list_i] = item.data.e_string.asWtf8CollapseRope(allocator) catch bun.outOfMemory();
                                            list_i += 1;
                                        }
                                    }
                        ...
                        for (obj.properties.slice()) |*prop| {
                            ...
                            switch (value.data) {
                                .e_string => |str| {
                                    ...
                                    package_json.browser_map.put(key, str.asWtf8CollapseRope(allocator) catch bun.outOfMemory()) catch unreachable;
                                },
                                .e_boolean => |boolean| {
                                    if (!boolean.value) {
        ...
        pub fn visit(this: Visitor, expr: js_ast.Expr) Entry {
            ...
            switch (expr.data) {
                ...
                .e_string => |str| {
                    return Entry{
                        .data = .{
                            .string = str.asWtf8CollapseRope(this.allocator) catch bun.outOfMemory(),
                        },
                ...
                .e_object => |e_obj| {
                    ...
                    for (e_obj.properties.slice(), 0..) |prop, i| {
                        const key: string = prop.key.?.data.e_string.asWtf8CollapseRope(this.allocator) catch bun.outOfMemory();
