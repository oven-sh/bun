            ...
            'load_module_from_cache: {
                ...
                let mut resolved_package_id: Install::PackageID = 'brk: {
                    ...
                    if let Some(package_json) = package_json_for_dependencies {
                        ...
                        // A cached package's dependencies are in the lockfile.
                        // The project's own files (no id, or id 0 once appended
                        // as the lockfile root) answer from their map, and only
                        // when the import names no version of its own.
                        let resolve_from_lockfile =
                            PackageJSON::is_cached_package(package_json.package_manager_package_id);
    ...
    fn package_json_for_dependencies(
        ...
        while let Some(info) = current {
            if let Some(mut package_json) = info.package_json {
                ...
                // devDependencies count for the project's own files, as they
                // do for the root in `bun install`, not for a package under
                // node_modules.
                pkg.load_dependencies(self, !info.is_inside_node_modules());
                if PackageJSON::is_cached_package(pkg.package_manager_package_id) {
