            ...
            Status::Exact | Status::ExactEndsWithStar => {
                ...
                let entry_query = match entries.get(base) {
                    ...
                    None => {
                        ...
                        if ends_with_star {
                            if let Some(m) = self.probe_wildcard_extensions(
                                entries,
                                resolved_dir_info,
                                package_json,
                                base,
        ...
        if bun_paths::extension(base).is_empty() {
            ...
            for ext in self.opts.ext_order_slice(extension_order).iter() {
                ...
                if let Some(ext_query) = entries.get(&file_name[..]) {
                    if ext_query.entry().kind(rfs, self.store_fd)
                        == Fs::file_system::EntryKind::File
                    {
                        if let Some(debug) = self.debug_logs.as_mut() {
                            debug.add_note_fmt(format_args!(
                        ...
                        return Some(self.build_wildcard_match(
                            entries,
                            resolved_dir_info,
                            package_json,
                            ext_query,
                ...
                if !ts_exts.is_empty() {
                    ...
                    for &replacement in ts_exts.iter() {
                        ...
                        if let Some(ts_query) = entries.get(&file_name[..]) {
                            if ts_query.entry().kind(rfs, self.store_fd)
                                == Fs::file_system::EntryKind::File
                            {
                                if let Some(debug) = self.debug_logs.as_mut() {
                                    debug.add_note_fmt(format_args!(
