...
// TODO(port): `impl Route { pub use Sorter }` is not valid Rust; either make
// `Sorter` an inherent module on `Route` via a wrapper type or move callers to
// `crate::Sorter`. Callers currently use `crate::Sorter` directly.
