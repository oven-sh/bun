...
fn expr_to_js(
    ...
    bun_js_parser_jsc::expr_to_js(&expr, global).map_err(|e| match e {
        ToJSError::JSError => bun_jsc::JsError::Thrown,
        ToJSError::OutOfMemory => bun_jsc::JsError::OutOfMemory,
        ToJSError::JSTerminated => bun_jsc::JsError::Terminated,
        // The data-format parsers only produce nodes the converter handles.
        ToJSError::CannotConvertArgumentTypeToJS
        | ToJSError::CannotConvertIdentifierToJS
        | ToJSError::MacroError => global.throw(format_args!("Cannot convert value to JS")),
    })
}
