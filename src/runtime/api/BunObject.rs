...
mod static_adapters {
    ...
    pub fn parsed_shell_script_create(g: &JSGlobalObject, cf: &CallFrame) -> JsResult<JSValue> {
        // SAFETY: `CREATE_PARSED_SHELL_SCRIPT` is a `JSHostFn` (extern "C"
        // (global, callframe) -> JSValue) generated by `MarkedArgumentBuffer::wrap`;
        // both pointers are live for the call.
        let r = unsafe {
            crate::shell::parsed_shell_script::CREATE_PARSED_SHELL_SCRIPT(
                g as *const _ as *mut JSGlobalObject,
                cf as *const _ as *mut CallFrame,
            )
        };
        bun_jsc::from_js_host_call(g, r)
    }
