...
// Canonical home for the completion task: in Zig this lives under
// `bun.BundleV2.JSBundleCompletionTask`, defined inside `JSBundler.zig`
// (i.e. the `jsc.API` namespace). Hosting it in `bun_runtime` is the layering
// fix: `bun_bundler_jsc` cannot own it without depending back on `bun_runtime`
// for `Config` / `HTMLBundleRoute` (cycle). Lower crates hold it as an opaque
// `*mut ()` backref (`bundle_v2::BundleV2::completion`).
...
pub use bun_bundler::BundleThread::{BuildResult, BundleV2Result as CompletionResult};

pub struct JSBundleCompletionTask {
    ref_count: bun_ptr::RefCount<JSBundleCompletionTask>,
    pub config: js_bundler::Config,
    pub jsc_event_loop: *const bun_jsc::event_loop::EventLoop,
    pub global_this: *const JSGlobalObject,
    pub log: bun_logger::Log,
    /// Set by the owning `HTMLBundle.Route` when the build is superseded
    /// (e.g. server reload) so the bundle thread skips delivering results.
    pub cancelled: bool,
    /// Backref to the route that scheduled this build; the route holds a ref
    /// on itself for the duration of the build.
    pub html_build_task: Option<*mut crate::server::html_bundle::Route>,
    pub result: CompletionResult,
    pub plugins: Option<*mut js_bundler::Plugin>,
    pub started_at_ns: u64,
}

// `bun.ptr.ThreadSafeRefCount(@This(), "ref_count", deinit, .{})`
// PORT NOTE: Zig used the thread-safe mixin; `bun_ptr` currently exposes a
// single `RefCounted` trait. The JS-thread side (route) and the bundle thread
// each hold one ref; revisit once the thread-safe blanket lands in `bun_ptr`.
impl bun_ptr::RefCounted for JSBundleCompletionTask {
    type DestructorCtx = ();
    fn debug_name() -> &'static str {
        "JSBundleCompletionTask"
    }
    unsafe fn get_ref_count(this: *mut Self) -> *mut bun_ptr::RefCount<Self> {
        // SAFETY: caller contract — `this` points to a live task.
        unsafe { core::ptr::addr_of_mut!((*this).ref_count) }
    }
    unsafe fn destructor(this: *mut Self, _ctx: ()) {
        // SAFETY: refcount hit zero; allocated via Box (`create_and_schedule_completion_task`).
        drop(unsafe { Box::from_raw(this) });
    }
}

/// Hook installed by runtime init to dispatch a freshly-created
/// `JSBundleCompletionTask` onto the bundle thread. Mirrors the
/// `JSBundleThread.singleton.enqueue` call from Zig; kept as a hook so
/// `bun_bundler` (which owns `BundleThread<C>`) need not name the concrete
/// `C = JSBundleCompletionTask` from a higher crate.
pub static SCHEDULE_BUNDLE_HOOK: core::sync::atomic::AtomicPtr<()> =
    core::sync::atomic::AtomicPtr::new(core::ptr::null_mut());
pub type ScheduleBundleFn = unsafe fn(*mut JSBundleCompletionTask);

/// `bun.BundleV2.createAndScheduleCompletionTask` — JS-thread entry point used
/// by `HTMLBundle.Route.onPluginsResolved` and `Bun.build()`.
///
/// Returns the task with one ref held by the caller; a second ref is taken for
/// the bundle thread before scheduling.
pub fn create_and_schedule_completion_task(
    config: js_bundler::Config,
    plugins: Option<*mut js_bundler::Plugin>,
    global_this: &JSGlobalObject,
    event_loop: *const bun_jsc::event_loop::EventLoop,
) -> *mut JSBundleCompletionTask {
    let completion = Box::into_raw(Box::new(JSBundleCompletionTask {
        ...
        log: bun_logger::Log::init(),
        cancelled: false,
        html_build_task: None,
        result: CompletionResult::Pending,
        plugins,
        started_at_ns: 0,
    }));
    // Bundle-thread ref (matches the deref in `BundleThread::generateInNewThread`).
    // SAFETY: completion is a freshly Boxed task.
    unsafe { bun_ptr::RefCount::<JSBundleCompletionTask>::ref_(completion) };
    let hook = SCHEDULE_BUNDLE_HOOK.load(core::sync::atomic::Ordering::Relaxed);
    if !hook.is_null() {
        // SAFETY: hook was registered as a `ScheduleBundleFn` by runtime init.
        unsafe { core::mem::transmute::<_, ScheduleBundleFn>(hook)(completion) };
    }
    completion
}
