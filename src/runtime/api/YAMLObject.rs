...
impl<'a> Stringifier<'a> {
    ...
    fn read(
        ...
        let items = if unwrapped.is_array() {
            ...
            let mut items: Vec<(OwnedString, Item)> = Vec::new();
            while let Some((prop_name, value)) = properties.next()? {
                ...
                items.push((OwnedString::new(prop_name.dupe_ref()), item));
            }
            Items::Object(items)
        };
    ...
    fn write_scalar(
        ...
        let value_str = OwnedString::new(scalar.to_bun_string(global)?);
