...
impl H2FrameParser {
        ...
        if args_list.len > 4 && !args_list.ptr[4].is_empty_or_undefined_or_null() {
            ...
            if let Some(signal_arg) = options.get(global_object, "signal")? {
                if let Some(signal_ptr) = AbortSignal::from_js(signal_arg) {
                    ...
                    return Err(global_object.throw_invalid_argument_type_value("options.signal", "AbortSignal", signal_arg));
                }
            }
        }
    ...
    pub fn set_native_socket_from_js(this: &mut Self, global_object: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> {
        ...
        if let Some(socket) = JSTLSSocket::from_js(socket_js) {
            ...
            socket.ref_();
            this.native_socket = BunSocket::TlsWriteonly(socket as *mut TLSSocket);
            ...
            socket.ref_();
            this.native_socket = BunSocket::TcpWriteonly(socket as *mut TCPSocket);
            this.has_nonnative_backpressure = false;
            let _ = this.flush();
        }
        Ok(JSValue::UNDEFINED)
