...
impl H2FrameParser {
    ...
    pub(crate) fn get_stream_state(
        ...
        // nghttp2's NGHTTP2_DEFAULT_WEIGHT: node reports it for every stream, priorities are gone.
...
impl H2FrameParser {
    ...
    pub(crate) fn request(
        ...
        // padding_overhead` never exceeds `actual_max_frame_size`; otherwise the
        ...
        let padding: u8 = if encoded_size >= actual_max_frame_size {
            0
        } else {
            stream.get_padding(encoded_size, actual_max_frame_size - 1)
        };
        ...
        let headers_frame_max_payload = actual_max_frame_size - padding_overhead;
