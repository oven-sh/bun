...
pub fn spawn_maybe_sync<const IS_SYNC: bool>(
        ...
        if !args.is_empty() && args.is_object() {
            ...
            if !IS_SYNC {
                if let Some(terminal_val) = args.get_truthy(global_this, "terminal")? {
                    ...
                    if let Some(terminal_ptr) = terminal_mod::js::from_js(terminal_val) {
                        // SAFETY: `from_js` returned a live `*mut Terminal` whose JS
                        // wrapper is rooted by `terminal_val` for this stack frame.
                        let terminal = unsafe { &mut *terminal_ptr };
                        if terminal.is_closed() {
                            return Err(
                                global_this.throw_invalid_arguments("terminal is closed")
                            );
                        }
                        if terminal.is_inline_spawned() {
                            return Err(global_this.throw_invalid_arguments(
                                "terminal was created inline by a previous spawn and cannot be reused",
                            ));
                        }
                        #[cfg(unix)]
                        if terminal.get_slave_fd() == Fd::INVALID {
                            return Err(global_this.throw_invalid_arguments(
                        ...
                        existing_terminal = Some(terminal_ptr);
                        ...
                        match Terminal::create_from_spawn(global_this, &mut term_options) {
                            Ok(result) => *terminal_info = Some(result),
                            Err(err) => {
                                drop(term_options);
                                return Err(match err {
                                    ...
                                    TerminalInitError::NotSupported => global_this
                                        .throw("PTY not supported on this platform"),
                                    TerminalInitError::WriterStartFailed => global_this
                                        .throw("Failed to start terminal writer"),
                                    TerminalInitError::ReaderStartFailed => global_this
                                        .throw("Failed to start terminal reader"),
                                });
                            }
                        }
                    } else {
                    ...
                    #[cfg(unix)]
                    {
                        // SAFETY: one of the two arms above set either
                        // `existing_terminal` or `terminal_info`; both yield a
                        // live `*mut Terminal`.
                        let terminal = existing_terminal
                            .unwrap_or_else(|| terminal_info.as_ref().unwrap().terminal.data.as_ptr());
    ...
    if !override_env && env_array.is_empty() {
        ...
        let map = unsafe { &mut *(*jsc_vm.transpiler.env).map };
        match map.create_null_delimited_env_map() {
            Ok(envmap) => {
                let slice = envmap.as_slice();
                // Drop the trailing `None` sentinel; `env_array.push(None)` re-adds it
                // after the optional NODE_CHANNEL_FD entries below.
                let body = &slice[..slice.len().saturating_sub(1)];
                env_array.reserve(body.len());
                env_array.extend_from_slice(body);
                inherited_env_storage = Some(envmap);
            }
            Err(_err) => {
                return Err(global_this.throw_error(
                    bun_core::Error::OutOfMemory,
                    "in Bun.spawn",
                ));
            }
        }
