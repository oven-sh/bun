...
// `bun.ptr.CowString` — the owned/borrowed byte slice (has
...
impl Readable {
    ...
    /// and release the ref (`pipe.detach()`). Centralises what was the
    ...
    pub fn init(
        ...
        // PORT NOTE: an unused `allocator` param was dropped; global mimalloc assumed.
        Subprocess::assert_stdio_result(&result);

        // Ownership of any resource inside `stdio` (notably `.memfd`) is being
        // *transferred* into the returned `Readable` — `Readable.init`
    ...
    pub fn finalize(&mut self) {
        match self {
            ...
            Readable::Pipe(_) => {
                // PORT NOTE: reshaped for borrowck — the original captures `pipe` by-copy then overwrites `this.*`.
            ...
            Readable::Buffer(_) => {
                // PORT NOTE: the original calls `buf.deinit(default_allocator)` without resetting the tag.
    ...
    pub fn to_buffered_value(&mut self, global: &JSGlobalObject) -> JsResult<JSValue> {
        match self {
            ...
            Readable::Buffer(_) => {
                ...
                // JSC (freed via `MarkedArrayBuffer_deallocator`) — matches
