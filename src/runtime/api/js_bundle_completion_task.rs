...
pub struct JSBundleCompletionTask {
    ...
    /// This build's copy of the calling VM's env: the bundle thread reads it
    /// while the JS thread keeps writing the VM's own loader (`process.env`
    /// assignments). Boxed because the build's transpiler holds a pointer to it.
...
impl JSBundleCompletionTask {
    ...
    pub(crate) fn new(
        ...
        JSBundleCompletionTask {
            ...
            // JS thread, the same thread that writes the VM's loader.
            env: Box::new(bun_core::handle_oom(
                global_this.bun_vm().env_loader().snapshot(),
            )),
    ...
    pub(crate) fn schedule(mut self) {
        ...
        // reads this VM's plugin cell and posts into its queue, so the VM cancels
        // it at teardown (registry) and waits for it (`bundle_ticket`).
...
impl CompletionStruct for JSBundleCompletionTask {
    ...
    fn create_and_configure_transpiler<'a>(
        ...
        // Freed with `self`; the per-thread macro VM copies it (`Macro::init`).
