    ...
    const JSC::SourceOrigin& sourceOrigin)
{
    ...
    // Use Bun::GlobalObject's function
    return uncheckedDowncast<Bun::GlobalObject>(global)->moduleLoaderImportModule(global, moduleLoader, moduleNameValue, WTF::move(parameters), sourceOrigin);
}
    ...
    RefPtr<JSC::ScriptFetchParameters> parameters, RefPtr<JSC::ScriptFetcher> script)
{
    ...
    if (moduleKey.startsWith("bake:/"_s)) {
        if (global->m_perThreadData) [[likely]] {
            ...
            // BakeProdResolve in `bake::production`.
            //
