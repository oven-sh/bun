...
//!   2. `impl Drop for Entry` asserted `ref_count == 0`, which is only valid on
//!      the explicit `unrefAtIndex` release path — store teardown and
//!      `*gop.value_ptr = Entry { .. }` overwrites legitimately drop nonzero
//!      counts.
...
//! as a stub and re-exports the canonical items so any stale
//! `super::source_map_store_body::*` path that reappears resolves to the single
//! real type.
