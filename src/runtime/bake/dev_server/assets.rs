...
#[derive(Default)]
pub struct Assets {
    /// Keys are absolute paths. PORT NOTE: an earlier design shared memory with
    /// `IncrementalGraph(.client)` key storage (writing the interned path back
    /// into the map's key pointer). `StringArrayHashMap` stores owned
    /// `Box<[u8]>` keys; the borrow-from-graph optimization is dropped.
    // PERF(port): keys could alias IncrementalGraph storage to save allocations.
...
impl Assets {
    ...
    pub fn replace_path(
        ...
        debug_assert!(self.owner().magic == Magic::Valid);
        // PORT NOTE: reshaped for borrowck — the `files.count() == refs.len()`
        // invariant is re-checked before each return below instead of via `defer`.

        // Hex-encode the *native-endian bytes* of the u64.
        ...
        // PORT NOTE: reshaped for borrowck — instead of holding `gop` (key/value ptrs
        // into `path_map`) live across calls that take `&mut self`, capture `index` /
        // `found_existing` and re-derive the value slot at the end.
        ...
        if !found_existing {
            ...
            // PORT NOTE: an earlier design had `path_map` keys borrow `client_graph`'s
            // interned key storage (writing the interned slice back into the map's key
            // pointer). Rust `StringArrayHashMap` owns its keys as `Box<[u8]>`, and
            // `get_or_put` already boxed `abs_path` on insert, so the reassignment is a
            // no-op here — we still call `insert_empty` for its side effect of registering
            // the file in `client_graph`.
            ...
            if self.refs[entry_index.get_usize()] == 1 {
                // PORT NOTE: a struct-of-arrays map would expose key/value columns to mutate
                // directly. Rust `ArrayHashMap` exposes `keys_mut`/`values_mut` instead.
                ...
                // PORT NOTE: the key IS the content hash (no separate hash column).
                // The Rust `ArrayHashMap<u64, _>` upholds this; nothing to assert at runtime.
    ...
    pub fn put_or_increment_ref_count(
        ...
        // so re-derive the slot via `values_mut()[index]` after the
        // `files.count() == refs.len()` invariant assert.
        let file_index_gop = self.files.get_or_put(content_hash)?;
...
impl Drop for Assets {
    fn drop(&mut self) {
        // path_map/files/refs storage is freed by their own Drop; only the manual
        // StaticRoute derefs remain as a side effect.
        for &blob in self.files.values() {
