...
/// Line count newtype (a `GenericIndex<u32, u8>`).
...
///
/// PORT NOTE: a struct-of-arrays split buys nothing for a 2-word payload and
/// `MultiArrayElement` cannot be derived for an enum, so callers store
/// `Vec<Shared>`.
#[derive(Default)]
pub enum Shared {
    Some(Rc<PackedMap>),
...
impl Shared {
    ...
    pub fn take(&mut self) -> Option<Rc<PackedMap>> {
        match core::mem::replace(self, Shared::None) {
            ...
            other => {
                // PORT NOTE: reshaped for borrowck — only the `Some` arm should
                // overwrite to `None`, so restore the original on miss.
                *self = other;
                None
            }
        }
