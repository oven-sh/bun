...
// LAYERING: `UserOptions` (re-exported from `bake_body`) carries
// `bake_body::{Framework, SplitBundlerOptions}` — the arena-backed `&'static`
// shapes filled by `from_js`. `DevServer::Options` consumes the keystone
// `Cow`-backed shapes defined above. In Zig these are one type; until the two
// Rust copies are collapsed (see `framework_router` below for the precedent),
// these `From` impls move data across the seam so `server::init` can hand
// `config.bake.{framework,bundler_options}` straight to `DevServer::init`.
impl From<bake_body::ReactFastRefresh> for ReactFastRefresh {
    fn from(b: bake_body::ReactFastRefresh) -> Self {
        Self { import_source: Cow::Borrowed(b.import_source) }
    }
}
impl From<bake_body::ServerComponents> for ServerComponents {
    fn from(b: bake_body::ServerComponents) -> Self {
        Self {
            separate_ssr_graph: b.separate_ssr_graph,
            server_runtime_import: Cow::Borrowed(b.server_runtime_import),
            server_register_client_reference: Cow::Borrowed(b.server_register_client_reference),
            server_register_server_reference: Cow::Borrowed(b.server_register_server_reference),
            client_register_server_reference: Cow::Borrowed(b.client_register_server_reference),
        }
    }
}
impl From<bake_body::FileSystemRouterType> for FileSystemRouterType {
    fn from(b: bake_body::FileSystemRouterType) -> Self {
        Self {
            root: Cow::Borrowed(b.root),
            prefix: Cow::Borrowed(b.prefix),
            entry_client: b.entry_client.map(Cow::Borrowed),
            entry_server: Cow::Borrowed(b.entry_server),
            ignore_underscores: b.ignore_underscores,
            ignore_dirs: b.ignore_dirs.iter().map(|s| Cow::Borrowed(*s)).collect(),
            extensions: b.extensions.iter().map(|s| Cow::Borrowed(*s)).collect(),
            style: b.style,
            allow_layouts: b.allow_layouts,
        }
    }
}
impl From<bake_body::BuiltInModule> for BuiltInModule {
    fn from(b: bake_body::BuiltInModule) -> Self {
        match b {
            bake_body::BuiltInModule::Import(s) => BuiltInModule::Import(s.into()),
            bake_body::BuiltInModule::Code(s) => BuiltInModule::Code(s.into()),
        }
    }
}
impl From<bake_body::Framework> for Framework {
    fn from(b: bake_body::Framework) -> Self {
        let mut built_in_modules = bun_collections::StringArrayHashMap::new();
        for (k, v) in b.built_in_modules.iter() {
            bun_core::handle_oom(built_in_modules.put(*k, BuiltInModule::from(*v)));
        }
        Self {
            is_built_in_react: b.is_built_in_react,
            file_system_router_types: b
                .file_system_router_types
                .into_iter()
                .map(FileSystemRouterType::from)
                .collect(),
            server_components: b.server_components.map(ServerComponents::from),
            react_fast_refresh: b.react_fast_refresh.map(ReactFastRefresh::from),
            built_in_modules,
        }
    }
}
impl From<bake_body::BuildConfigSubset> for BuildConfigSubset {
    fn from(b: bake_body::BuildConfigSubset) -> Self {
        // Keystone `BuildConfigSubset` only carries the minify/DCE flags until
        // the `bun_schema` types it depends on are const-constructible (see the
        // `_blocked_tail` note above); the remaining `bake_body` fields are
        // consumed by `bake_body::Framework::init_transpiler_with_options`, not
        // by the keystone `init_transpiler`, so dropping them here is lossless
        // for the keystone path.
        Self {
            ignore_dce_annotations: b.ignore_dce_annotations,
            minify_syntax: b.minify_syntax,
            minify_identifiers: b.minify_identifiers,
            minify_whitespace: b.minify_whitespace,
            _blocked_tail: (),
        }
    }
}
impl From<bake_body::SplitBundlerOptions> for SplitBundlerOptions {
    fn from(b: bake_body::SplitBundlerOptions) -> Self {
        Self {
            // `bake_body::Plugin` is the typed `JSBundler.Plugin` handle; the
            // keystone field is `*mut c_void` (see `jsc::Plugin` above) — same
            // pointer, erase the type.
            plugin: b.plugin.map(NonNull::cast),
            client: b.client.into(),
