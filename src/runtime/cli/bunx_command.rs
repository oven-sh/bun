...
impl BunxCommand {
    ...
    fn link_bin_from_installed_package<'a>(
        ...
        let mut package_subpath = bun_paths::path_buffer_pool::get();
    ...
    pub(crate) fn exec(ctx: &mut ContextData, argv: &[&'static ZStr]) -> crate::Result<()> {
        ...
        let mut package_bin_abs_buf = bun_paths::path_buffer_pool::get();
        let mut cache_manifest_buf = bun_paths::path_buffer_pool::get();
        ...
        match Self::link_bin_from_installed_package(
            ...
            PackageBinLookup::BinNotFound | PackageBinLookup::MissingTarget => {
                Self::exit_package_bin_not_found(result_package_name, bin_name);
            }
            PackageBinLookup::PackageNotFound => {
                if opts.specified_package.is_some() {
                    Self::exit_package_bin_not_found(result_package_name, bin_name);
