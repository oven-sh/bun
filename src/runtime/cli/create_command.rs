...
impl CreateCommand {
    ...
    pub(crate) fn exec(
        ...
        let destination = {
            ...
            let Some(joined) = filesystem.abs_buf_checked(&[dirname], &mut destination_buf[..])
            else {
                Output::err_generic(
...
impl Example {
    ...
    pub(crate) fn fetch_all_local_and_remote(
        ...
        let mut examples: Vec<Example> = remote_examples.into_vec();
        {
            ...
            // A folder whose path does not fit stays invalid, like one that does not exist.
            if let Some(home_dir) = env_loader.map.get(b"BUN_CREATE_DIR")
                && let Some(outdir_path) =
                    filesystem.abs_buf_checked(&[home_dir], &mut home_dir_buf[..])
            {
                folders[0] = bun_sys::Dir::open(outdir_path)
                    .unwrap_or_else(|_| bun_sys::Dir::from_fd(bun_sys::Fd::invalid()));
            }

            if let Some(outdir_path) = filesystem.abs_buf_checked(
                &[filesystem.top_level_dir, BUN_CREATE_DIR],
                &mut home_dir_buf[..],
            ) {
            ...
            if let Some(home_dir) = env_loader.map.get(bun_core::env_var::HOME.key())
                && let Some(outdir_path) =
                    filesystem.abs_buf_checked(&[home_dir, BUN_CREATE_DIR], &mut home_dir_buf[..])
            {
