...
/// Shared by `bun outdated` and `bun update --interactive`: load the lockfile
/// from the current directory, reporting errors and exiting the process on
/// failure.
...
/// Shared by `bun outdated` and `bun update --interactive`: collect the
/// package IDs of the root package and every workspace package.
...
/// Shared by `bun outdated` and `bun update --interactive`: collect the
/// workspace package IDs matching the `--filter` patterns.
...
impl OutdatedCommand {
    ...
    fn outdated(
        ...
        load_lockfile_or_crash(ctx, manager);
    ...
    fn outdated_dispatch<const ENABLE_ANSI_COLORS: bool>(
        ...
        if !manager.options.filter_patterns.is_empty() {
            ...
            let workspace_pkg_ids = find_matching_workspaces(original_cwd, manager, filters);
            ...
            let all_workspaces = get_all_workspaces(manager);
            populate_manifest_cache::populate_manifest_cache(
                manager,
