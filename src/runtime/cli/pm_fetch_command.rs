...
use bun_install::{
    DependencyID, PackageID, PreinstallState, TaskCallbackContext, invalid_dependency_id,
};
...
impl PmFetchCommand {
    pub fn exec(
        ...
        if log_level != LogLevel::Silent {
            ...
            let cache_dir: &[u8] = match bun_sys::get_fd_path(
                get_cache_directory(pm).fd(),
                &mut cache_dir_buf,
            ) {
                Ok(p) => &p[..],
                Err(_) => b"",
...
impl WaitClosure {
    ...
    fn run_and_wait(this: &mut PackageManager) -> Result<(), bun_core::Error> {
        ...
        let mut closure = WaitClosure { manager: mgr, err: None };
