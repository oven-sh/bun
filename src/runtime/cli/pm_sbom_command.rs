...
// The serializers hand-format pretty-printed JSON line by line.
...
use bun_install::{ExternalSlice, INVALID_PACKAGE_ID, PackageID, PackageManager, Repository};
use bun_jsc::uuid::UUID;
use bun_paths::{path_buffer_pool, platform, resolve_path};
use bun_sys::Fd;

use crate::cli::package_manager_command::PackageManagerCommand;
use crate::command;
use crate::shell::builtins::ls::civil_from_days;
...
impl PmSbomCommand {
    pub fn exec(
        ...
        if let Some(path) = outfile {
            // `PackageManager::init()` has already chdir'd to the workspace root.
            let mut abs_buf = path_buffer_pool::get();
            let Some(abs_path) = resolve_path::join_abs_string_buf_checked::<platform::Auto>(
                original_cwd,
                &mut abs_buf[..],
                &[path],
            ) else {
                Output::err_generic("output path is too long: '{s}'", (bstr::BStr::new(path),));
                Global::exit(1);
            };
...
impl<'a> Generator<'a> {
    fn init(pm: &'a PackageManager) -> Generator<'a> {
        ...
        // Scope propagation (see `Scope`) mirrors what `bun install --production`
        // would install. Seeded from the lockfile root (PackageID 0), not
        // `pm.root_package_id`: the SBOM covers the whole lockfile even when run
        // from inside a workspace member.
        ...
        for idx in 0..pkg_len {
            ...
            match res.tag {
                ...
                ResolutionTag::Workspace => {
                    ...
                    if let Some(ws_version) = lockfile.workspace_versions.get(&pkg_name_hashes[idx])
                    {
                        ...
                        // Workspace names aren't validated as npm names.
                ...
                | ResolutionTag::Github => {
                    ...
                    if res.tag == ResolutionTag::RemoteTarball {
                        ...
                        download_url = vcs_locator(
                            res.repository(),
                            res.tag == ResolutionTag::Github,
                            string_bytes,
                        );
                    }
                }
            ...
            // bom-refs must be unique; aliases can yield duplicate name@version.
            let mut ref_ = ref_;
            ...
            // Deduplicated separately from `ref_`; see `Component::spdx_id`.
    ...
    fn write_cyclonedx_component(
        ...
        if let Some(alg) = cyclonedx_hash_alg(comp.integrity.tag) {
            let _ = write!(
                ...
                bun_core::fmt::hex_lower(comp.integrity.slice())
            );
        }
        let _ = write!(w, "\n{pad}}}");
    }
    ...
    fn write_spdx_relationships(&self, w: &mut Vec<u8>, comp: &Component) {
        ...
        for &dep_id in comp.deps.iter() {
            ...
            // The same package can appear under several dependency groups;
            // pick the strongest edge (required > optional > dev), as for scope.
            let rel_type = {
            ...
            match rel_type {
                ...
                // `*_OF` relationships point from the dependency to the dependent.
                RelType::OptionalOf => {
                    let _ = write!(
                        w,
...
fn collect_deps(
    ...
    for &resolved_id in resolved.iter() {
        // Self-edges (`"pkg": "file:."`) are skipped, as in the scope BFS.
...
fn purl_encode_into(out: &mut Vec<u8>, s: &[u8]) {
    for &c in s {
        match c {
            ...
            _ => {
                out.push(b'%');
                out.extend_from_slice(&bun_core::fmt::hex_byte_upper(c));
            }
        }
    }
}

/// SPDX 2.3 §7.7 VCS locator (`git+<url>@<revision>`), also used as the
/// CycloneDX distribution URL. GitHub resolutions become an https clone URL;
/// scp-style `user@host:path` becomes `ssh://user@host/path`.
fn vcs_locator(repo: &Repository, is_github: bool, buf: &[u8]) -> Vec<u8> {
    let mut out: Vec<u8> = Vec::with_capacity(96);
    out.extend_from_slice(b"git+");
    if is_github {
        out.extend_from_slice(b"https://github.com/");
        out.extend_from_slice(repo.owner.slice(buf));
        out.push(b'/');
        out.extend_from_slice(repo.repo.slice(buf));
        out.extend_from_slice(b".git");
    } else {
        let url = repo.repo.slice(buf);
        let url = url.strip_prefix(b"git+").unwrap_or(url);
        if strings::contains(url, b"://") {
            out.extend_from_slice(url);
        } else if let Some(colon) = strings::index_of_char_usize(url, b':') {
    ...
    // `resolved` is stored as `<prefix>-<sha>`; fall back to the committish.
    let mut rev = repo.resolved.slice(buf);
    if let Some(dash) = strings::last_index_of_char(rev, b'-') {
        rev = &rev[dash as usize + 1..];
    }
    if rev.is_empty() {
        rev = repo.committish.slice(buf);
    }
    if !rev.is_empty() {
        out.push(b'@');
