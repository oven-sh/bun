...
impl PmVersionCommand {
    pub fn exec(
        ...
        // PORT NOTE: ctx dropped its allocator field (global mimalloc),
...

        {
            ...
            // routed through bun_sys (no std::fs).
    ...
    fn get_current_version(ctx: &command::ContextData, cwd: &[u8]) -> Option<Vec<u8>> {
        // PORT NOTE: reshaped — originally returned a slice borrowing from allocator-owned
