    ...
    fn start(self) -> ! {
        ...
        if vm.is_watcher_enabled() {
            ...
            loop {
                ...
                // A beforeExit handler may have resolved the stalled await. Like Node, beforeExit
                // fires again only if that made the loop alive; a module that merely runs to
                // completion in microtasks does not get a second one.
                if let Some(entry) = pending_entry_module(vm) {
                    ...
                    // Still suspended with the loop idle: wait for the resumed module like the initial load did.
            ...
            if let Some(p) = vm.pending_internal_promise {
                ...
                match promise.status() {
                    // Like Node, an exit code that is already decided (`process.exitCode`, an
                    // uncaught error) wins over the unsettled-await report.
                    PromiseStatus::Pending
                        if vm.exit_handler.exit_code == 0 && vm.unhandled_error_counter == 0 =>
                    {
