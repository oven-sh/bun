...
impl HMAC {
    pub fn init(algorithm: evp::Algorithm, key: &[u8]) -> Option<Box<HMAC>> {
        ...
        yolo! { boringssl::HMAC_CTX_init(ctx.as_mut_ptr()) };
        // SAFETY: ctx was initialized by HMAC_CTX_init above.
        let mut ctx = yolo! { ctx.assume_init() };
        ...
        } != 1
        {
            ...
            yolo! { boringssl::HMAC_CTX_cleanup(&raw mut ctx) };
            return None;
        }
    ...
    pub fn update(&mut self, data: &[u8]) {
        ...
        let _ = yolo! { boringssl::HMAC_Update(&raw mut self.ctx, data.as_ptr(), data.len()) };
    }

    pub fn size(&self) -> usize {
        // SAFETY: self.ctx is initialized.
        yolo! { boringssl::HMAC_size(&raw const self.ctx) }
    }
    ...
    pub fn copy(&mut self) -> Result<Box<HMAC>, bun_core::Error> {
        ...
        if yolo! { boringssl::HMAC_CTX_copy(&raw mut ctx, &raw const self.ctx) } != 1 {
            // SAFETY: ctx was initialized by HMAC_CTX_init.
            yolo! { boringssl::HMAC_CTX_cleanup(&raw mut ctx) };
    ...
    pub fn r#final<'a>(&mut self, out: &'a mut [u8]) -> &'a mut [u8] {
        ...
        let _ =
            yolo! { boringssl::HMAC_Final(&raw mut self.ctx, out.as_mut_ptr(), &raw mut outlen) };
...
impl Drop for HMAC {
    fn drop(&mut self) {
        ...
        yolo! { boringssl::HMAC_CTX_cleanup(&raw mut self.ctx) };
