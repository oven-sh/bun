...
//! `JS2Native__*` thunks emitted into `generated_js2native.rs`.
//!
//! `src/codegen/generate-js2native.ts::rustTarget()` routes every
//! `$native(...)` call site whose source file lives **outside** `src/runtime/`
//! through `crate::dispatch::js2native::<mangled>` instead of the file's own
//! crate path. `bun_runtime` is the highest-tier crate (already depends on
//! every `*_jsc` bridge crate plus `bun_jsc` itself), so the cross-crate
//! fan-out lands here without introducing a dep cycle. Each entry below is a
//! `pub use` of the real hand-ported function — there are no local bodies and
//! no fallback panics; a missing target stays a compile error in the owning
//! crate.
//!
...
pub use bun_sys_jsc::error_jsc::TestingAPIs::sigaction_layout as sys_error_testing_ap_is_sigaction_layout;
pub use bun_sys_jsc::error_jsc::TestingAPIs::sys_error_name_from_libuv as sys_error_testing_ap_is_sys_error_name_from_libuv;
pub use bun_sys_jsc::error_jsc::TestingAPIs::translate_nt_status_to_e as sys_error_testing_ap_is_translate_nt_status_to_e;
pub use bun_sys_jsc::error_jsc::TestingAPIs::translate_uv_error_to_e as sys_error_testing_ap_is_translate_uv_error_to_e;
