...
pub(crate) fn to_array_buffer(
    ...
    match get_ptr_slice(global_this, value, byte_offset, value_length) {
        ...
        ValueOrError::Slice(ptr, len) => {
            ...
            // `bun:ffi` user asserts the pointer stays valid and that their
            // finalization callback/ctx pair is sound to invoke once at GC —
            // `toArrayBuffer(ptr, ...)` is an inherently trusting FFI API.
            unsafe {
