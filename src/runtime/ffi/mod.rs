//! `Bun.FFI` / `bun:ffi`.
//!
//! `ABIType` (CType) enum and formatters live in `abi_type`; everything else —
//! `FFI`/`Function`/`Step`/`Compiled` structs, dlopen data path, the JSC
//! host-fn entry points (`open`/`close`/`compile`/`generate_symbols`), and the
//! full TinyCC compile bodies (`CompileC`, `Function::compile`,
//! `cc`/`linkSymbols`/`callback`) — lives in `ffi_body` on top of
//! `bun_tcc_sys::State`.
