...
use bun_jsc::{
    self as jsc, CallFrame, EncodedSliceJsc as _, JSGlobalObject, JSPromise, JSValue, JsCell,
    JsClass as _, JsRef, JsResult, Local, Scope, StringJsc as _, Strong, SysErrorJsc as _,
};
use bun_sys as sys;
...
impl Image {
    ...
    pub(crate) fn get_width<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        Ok(scope.number(f64::from(this.last_width.get())))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_height<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
...
impl Image {
    ...
    pub(crate) fn do_placeholder<'s>(
        ...
        if args.len > 0 && !args.ptr[0].is_undefined_or_null() {
            let s = args.ptr[0].to_bun_string(scope)?;
    ...
    pub(crate) fn do_write<'s>(
        ...
        if output.is_none() && args.ptr[0].is_string() {
            let str = args.ptr[0].to_bun_string(scope)?;
            let utf8 = str.to_utf8();
        ...
        match result {
            TaskResult::Encoded { out, format, .. } => {
                ...
                match &mut js.deliver {
                    ...
                    Deliver::Buffer => promise.settle(global, {
                        // SAFETY: `out.bytes` is the live codec allocation and
                        // `out.free` (a thread-safe C allocator free ignoring
                        // the null ctx) releases it exactly once; the codec
                        // `Drop` is suppressed above.
                    ...
                    Deliver::WriteDest(dest) => {
                        ...
                        let data = match JSValue::create_buffer_from_foreign(global, foreign) {
                            Ok(d) => d,
