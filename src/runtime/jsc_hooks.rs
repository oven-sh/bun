...
fn timer_min_delay_ms() -> u32 {
    timer_all_opt().map_or(0, |all| all.fake_timers.min_delay_ms())
}
...
unsafe fn cancel_timers(vm: *mut VirtualMachine, only: Option<bun_jsc::ContextId>) {
    ...
    if let Some(context) = only {
        ...
        for (&timer, &kind) in timers.iter() {
            // SAFETY: tracked ⇒ live (each untracks itself when it is freed); a
            // cancel may free the one it cancels, never another tracked one. An
            // `Object` entry is the `event_loop_timer` slot of the
            // `TimeoutObject` / `ImmediateObject` that tracked it.
            unsafe {
                match kind {
                    bun_jsc::ContextTimer::Object => crate::dispatch::cancel_js_timer(
                        crate::timer::TimerRef::from_raw(timer.cast()),
                        &*vm,
                    ),
...
fn stop_active_handles(vm: &mut VirtualMachine, reason: bun_jsc::StopReason) -> SweepResult {
    ...
    //
    // Without a `RuntimeState` there is no `timer::All`; the contexts are
    // swept either way.
    if let Some(all) = timer_all_opt() {
