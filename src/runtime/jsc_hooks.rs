...
///
/// PORT NOTE (jsc/runtime crate cycle): `bun_jsc::VirtualMachine.timer` is a `()` placeholder;
...
unsafe fn auto_tick(vm: *mut VirtualMachine) {
    ...
    // drain_timers are real). No-op until the DateHeaderTimer body lands.
    // SAFETY: `el` is the live per-thread event loop.
    ...
    match loader {
        ...
        _ => {
            ...
            let value = if !unsafe { &*jsc_vm }.origin.is_empty() {
                ...
                // PORT NOTE: `jsc.API.Bun.getPublicPath` is a thin wrapper over
                // `get_public_path_with_asset_prefix` with `dir = VM.top_level_dir`,
                // `asset_prefix = ""`, `.loose`. Inline that body here (mirrors
                // filesystem_router.rs).
