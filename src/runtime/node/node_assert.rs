...
/// one line, compared byte-wise after UTF-8 conversion. Only the raw
/// `{ kind, value }` list ([`Output::List`]) is produced: `FromAny for &[u8]`
/// decodes UTF-8, while the renderers in [`emit`] would write the bytes out as
/// Latin-1.
