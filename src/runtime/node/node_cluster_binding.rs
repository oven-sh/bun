...
pub(crate) fn cluster_raw_bind(global: &JSGlobalObject, frame: &CallFrame) -> JsResult<JSValue> {
    #[cfg(windows)]
    {
        ...
        let host_owned: Vec<u8> = if address.is_string() {
            let s = address.as_string().to_slice(global);
    ...
    #[cfg(not(windows))]
    {
        ...
        if address_type.is_string() {
            let s = address_type.as_string().to_slice(global);
            is_udp = true;
        ...
        if atype == -1 {
            ...
            let path_slice = address.as_string().to_slice(global);
        ...
        if address.is_string() {
            let addr_slice = address.as_string().to_slice(global);
