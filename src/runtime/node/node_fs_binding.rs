...
impl Binding {
    ...
    pub(crate) fn get_dirent<'s>(_this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        let v = crate::node::Dirent::get_constructor(scope.unscoped_global());
        Ok(scope.local(v))
    }

    #[bun_jsc::host_fn(getter, scoped)]
    pub(crate) fn get_stats<'s>(_this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
...
pub(crate) fn create_memfd_for_testing<'s>(
    ...
    let size_arg = frame.scoped_argument(scope, 0);
    ...
    {
        let size = size_arg.to_int64(scope);
