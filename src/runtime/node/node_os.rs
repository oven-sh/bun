...
mod _impl {
    ...
    pub mod gen_ {
        use super::{CallFrame, EncodedSlice, JSGlobalObject, JSValue};
        use bun_jsc::host_fn;
    ...
    /// libuv's `uv__getpwuid_r`. `Ok(None)`: the effective uid has no entry.
    #[cfg(not(windows))]
    ...
    /// Copies, since the entry's strings die with the `getpwuid_r` buffer.
    ...
    /// `lib/os.js`'s `throw new ERR_SYSTEM_ERROR(ctx)`.
    fn throw_uv_error(global: &JSGlobalObject, err: &bun_sys::Error) -> bun_jsc::JsError {
        global.throw_value(
            SystemError::from(err.to_uv_system_error()).to_error_instance_with_info_object(global),
        )
    }
    ...
    /// `uv_passwd_t`; `shell` is `None` where libuv leaves it null.
    struct UserInfo {
        uid: f64,
        gid: f64,
        username: Vec<u8>,
    ...
    /// In Node.js, this is a wrapper around uv_os_get_passwd.
    pub(crate) fn user_info(
        ...
        let info = {
            ...
            // SAFETY: valid out-pointer
            if let Some(err) = unsafe { libuv::uv_os_get_passwd(&raw mut pwd) }
            ...
            let pwd = scopeguard::guard(pwd, |mut pwd| unsafe {
                libuv::uv_os_free_passwd(&raw mut pwd)
            });
            UserInfo {
