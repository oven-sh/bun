...
mod _impl {
    ...
    fn create_exec_argv(global_object: &JSGlobalObject) -> JsResult<JSValue> {
        ...
        // Re-parsing the process argv is rare, so it isn't done as part of the
        // CLI. The token builder lives alongside the worker execArgv policy so
        // `process.execArgv` and the inherit-path honoring scan see identical
        // tokens.
        let args: Vec<BunString> = crate::cli::worker_exec_argv::collect_process_exec_argv_tokens()
            .iter()
