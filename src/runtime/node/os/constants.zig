    ...
    switch (ctype) {
        .ERRNO => {
            if (comptime getErrnoConstant(name)) |constant| {
                object.put(globalObject, jsc.RustString.static("E" ++ name), jsc.JSValue.jsNumber(constant));
            }
        },
        .ERRNO_WIN => {
            if (comptime getWindowsErrnoConstant(name)) |constant| {
                object.put(globalObject, jsc.RustString.static(name), jsc.JSValue.jsNumber(constant));
            }
        },
        .SIG => {
            if (comptime getSignalsConstant(name)) |constant| {
                object.put(globalObject, jsc.RustString.static("SIG" ++ name), jsc.JSValue.jsNumber(constant));
            }
        },
        .DLOPEN => {
            if (comptime getDlopenConstant(name)) |constant| {
                object.put(globalObject, jsc.RustString.static("RTLD_" ++ name), jsc.JSValue.jsNumber(constant));
            }
        },
        .OTHER => {
            object.put(globalObject, jsc.RustString.static(name), jsc.JSValue.jsNumberFromInt32(value.?));
        },
    }
}
...
pub fn create(globalObject: *jsc.JSGlobalObject) jsc.JSValue {
    ...
    object.put(globalObject, jsc.RustString.static("errno"), createErrno(globalObject));
    object.put(globalObject, jsc.RustString.static("signals"), createSignals(globalObject));
    object.put(globalObject, jsc.RustString.static("priority"), createPriority(globalObject));
    object.put(globalObject, jsc.RustString.static("dlopen"), createDlopen(globalObject));
