...
//! exception to the drain, which folds it ([`at_boundary`]). And node's
...
/// The event drain, and the lsquic / UDP-socket callbacks that return into C
/// (`on_new_conn`, `on_new_stream`, the endpoint's `on_data`), fold what they
/// could not hand back at that boundary.
#[cold]
pub(super) fn at_boundary(global: &JSGlobalObject, err: JsError) -> Result<(), bun_jsc::Stopped> {
