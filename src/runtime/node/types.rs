...
impl<'a> StringOrBuffer<'a> {
    ...
    pub(crate) fn slice(&self) -> &[u8] {
        match self {
            ...
            Self::ThreadShareableString(str) => str.slice(),
...
impl bun_jsc::Unprotect for StringOrBuffer<'static> {
    ...
    /// `protect()` taken by [`StringOrBuffer::make_thread_shareable`] /
...
impl StringOrBuffer<'_> {
    pub fn into_js(self, ctx: &JSGlobalObject) -> JsResult<JSValue> {
        match self {
            Self::ThreadShareableString(str) | Self::String(str) => str.into_js(ctx),
    ...
    pub(crate) fn make_thread_shareable(&mut self) {
        match self {
            Self::String(s) => {
                ...
                *self = Self::ThreadShareableString(str);
            }
        ...
        match value.js_type() {
            str_type @ (JSType::String | JSType::StringObject | JSType::DerivedStringObject) => {
                ...
                *out = if flavor == Flavor::Async {
                    shared_or_utf8(
                        global,
                        str.into_utf8_with_string_thread_shareable(),
                        Self::ThreadShareableString,
                        Self::Utf8,
                    )
                } else {
...
///
/// `make_thread_shareable` / `into_thread_shareable` / `slice` / `estimated_size` are
...
fn path_like_from_string(
    ...
    let shared = if will_be_async {
        PathLike::ThreadShareableString
    } else {
        PathLike::String
    };
