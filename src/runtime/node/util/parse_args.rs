            ...
            TokenSubtype::ShortOptionGroup => {
                ...
                for idx_in_optgroup in 1..arg_len {
                    ...
                    if option_type != OptionValueType::String || idx_in_optgroup == arg_len - 1 {
                        ...
                        // This branch expands the char to "-<c>". When <c> is '-' that
                        // yields '--', the option terminator: remaining group chars and
                        // all remaining args become positionals.
                        if arg.char_at(idx_in_optgroup) == u16::from(b'-') {
                            ...
                            while rest_idx < arg_len {
                                ...
                                let rest_option_idx =
                                    find_option_by_short_name(&rest_short, options);
                                ...
                                let value = if rest_type != OptionValueType::String
                                    || rest_idx == arg_len - 1
                                {
                                    let owned = OwnedString::new(String::create_format(
                                        format_args!("-{rest_short}"),
                                    ));
                                    rest_idx += 1;
                                    owned.to_js(global)?
                                } else {
                                    let owned = OwnedString::new(String::create_format(
                                        format_args!("-{}", arg.substring(rest_idx)),
                                    ));
                                    rest_idx = arg_len;
                                    owned.to_js(global)?
                                };
