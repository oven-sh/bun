...
impl NodeHTTPResponse {
    ...
    fn write_head_impl(
        ...
        // an extra ref/deref pair (ZigStringSlice::WTF); instead hold the +1
        // from `to_bun_string` and borrow the bytes without the inner ref bump.
        let status_message_str;
        let status_message_slice;
        let status_message_bytes: &[u8] = if !status_message_value.is_undefined() {
            status_message_str = status_message_value.to_bun_string(global_object)?;
