...
pub fn parse_raw(header: &[u8]) -> Raw {
    ...
    if start_s.is_empty() {
        // RFC 9110 suffix-length is `1*DIGIT` — no sign. Match Zig's
        // `std.fmt.parseUnsigned`, which rejects a leading `+`/`-`.
    ...
    // RFC 9110 first-byte-pos / last-byte-pos are `1*DIGIT` — no sign.
