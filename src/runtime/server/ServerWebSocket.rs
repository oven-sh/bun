...
// ─── jsc.Codegen.JSServerWebSocket ───────────────────────────────────────────
// `data_{get,set}_cached` wraps the C++-side `WriteBarrier<Unknown>` slot;
// `to_js`/`from_js` are provided as inherent methods on this struct via
// `generated_classes.rs`, but that codegen targets a separate opaque
// placeholder type until the wiring is updated, so the externs are bound
...
impl ServerWebSocket {
    ...
    /// `pub const toJS = js.toJS` — wraps `*mut Self` in a freshly-allocated
    /// JS `ServerWebSocket` object. Ownership of `this` transfers to the
    /// wrapper (freed via `finalize`).
    #[inline]
    pub fn to_js(this: *mut ServerWebSocket, global: &JSGlobalObject) -> JSValue {
        // SAFETY: `global` is live; `this` was just heap-allocated via `Box::into_raw`
        // (bun.TrivialNew) and ownership transfers to the C++ wrapper.
    ...
    pub fn init(
        ...
        let this_value = ServerWebSocket::to_js(this, global_object);
    ...
    pub fn on_close(&mut self, _ws: AnyWebSocket, code: i32, message: &[u8]) {
        ...
        let _cleanup = scopeguard::guard(signal, move |sig| {
            if let Some(sig) = sig {
                // SAFETY: `signal` was set from a live C++-side AbortSignal; we hold the
                // last owning reference here (Zig: `sig.pendingActivityUnref(); sig.unref();`).
                unsafe {
        ...
        if !handler.on_close.is_empty_or_undefined_or_null() {
            ...
            if let Some(sig) = signal {
                // SAFETY: signal points at a live C++ WebCore::AbortSignal until unref'd in `_cleanup`.
    ...
    {
        Wrap::<ServerType, ServerWebSocket, SSL>::apply(opts)
    }
    ...
    pub fn finalize(this: *mut Self) {
        ...
        if let Some(signal) = this_ref.signal.take() {
            // SAFETY: signal points at a live C++ WebCore::AbortSignal; this is the
            // owning unref (Zig: `signal.pendingActivityUnref(); signal.unref();`).
            unsafe {
    ...
    pub fn get_remote_address(&self, global_this: &JSGlobalObject) -> JsResult<JSValue> {
        ...
        // PORT NOTE: std.net.Address — bun_core::fmt::format_ip takes &impl Display;
...
//   confidence: medium
//   notes:      handler.active_connections mutated through &; on_close defer
//               reshaped via scopeguard with raw ptr; std.net.Address replaced
//               with std::net::IpAddr + bun_core::fmt::format_ip
// ──────────────────────────────────────────────────────────────────────────
