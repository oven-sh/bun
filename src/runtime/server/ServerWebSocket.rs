...
use crate::server::jsc::{ArrayBuffer, CallFrame, JSString, JSType, JSUint8Array, ZigStringSlice};
...
trait AbortSignalExt {
    ...
    fn signal(&self, global: &JSGlobalObject, reason: jsc::CommonAbortReason) {
...
impl AbortSignalExt for AbortSignal {
    fn pending_activity_unref(&self) {
        AbortSignal::pending_activity_unref(self)
    }
}
...
impl ServerWebSocket {
    ...
    #[allow(unused_variables)]
    fn to_js(&mut self, global: &JSGlobalObject) -> JSValue {
        todo!("blocked_on: bun_jsc::JsClass codegen for ServerWebSocket::to_js")
    }
    ...
    pub fn behavior<ServerType, const SSL: bool>(opts: WebSocketBehavior) -> WebSocketBehavior {
        // TODO(port): `ServerType: WebSocketUpgradeServer<SSL>` bound dropped until
        // `NewServer<SSL, DEBUG>` implements it (server_body.rs). Re-add and call
        // `bun_uws_sys::web_socket::Wrap::<ServerType, Self, SSL>::apply(opts)` then.
        let _ = opts;
        todo!("blocked_on: bun_uws_sys::web_socket::WebSocketUpgradeServer impl for NewServer")
    }
