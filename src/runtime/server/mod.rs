...
mod server_body;
pub use server_body::{
    FrameworkRouterSeam, FrameworkRouterTypes, GetOrStartLoadResult, ServePluginsCallback,
...
pub struct DevServerSlotVTable {
    ...
    pub(crate) put_html_route: fn(
        core::ptr::NonNull<()>,
        path: &[u8],
        route: bun_ptr::ThisPtr<html_bundle::Route>,
    ) -> crate::Result<()>,
            ...
            match &entry.route {
                ...
                AnyRoute::Html(r) => {
                    ...
                    if let Some(dev) = dev_server {
                        bun_core::handle_oom(dev.put_html_route(&entry.path, r.this_ptr()));
                    }
                    needs_plugins = true;
                }
                AnyRoute::FrameworkRouter(_) => {}
