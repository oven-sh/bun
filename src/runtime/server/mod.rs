...
impl<const SSL: bool, const DEBUG: bool> NewServer<SSL, DEBUG> {
    ...
    // ───────────────────────── codegen prototype-method shims ─────────────────────────
    // Zig spells these as `pub const doX = host_fn.wrapInstanceMethod(ThisServer, "x", false)`
    // (server.zig:581-595). `wrapInstanceMethod` is comptime @typeInfo reflection that
    // walks the target's parameter types and emits per-param decode glue. Rust has no
    // value-level equivalent, so each alias is hand-expanded here using the *same*
    // decode/throw semantics as host_fn.zig:510-640 for the small set of param types
    // these methods actually use (JSValue, ?JSValue, ZigString, *Request).
    //
    // `generated_classes.rs` calls these by name via `host_fn_result(.., || T::do_x(..))`;
    // the bodies just unpack args and forward to the real implementations in
    // `server_body.rs`.

    /// `dispose = wrapInstanceMethod(disposeFromJS)` — zero JS args.
    #[inline]
    pub fn dispose(
        &mut self,
        ...
        self.dispose_from_js()
    }

    /// `doStop = wrapInstanceMethod(stopFromJS)` — `(abruptly: ?JSValue)`.
    #[inline]
    pub fn do_stop(
        &mut self,
        _global: &JSGlobalObject,
        ...
        self.stop_from_js(args.slice().first().copied())
    }

    /// `doReload = onReload` — direct alias.
    #[inline]
    pub fn do_reload(
        &mut self,
    ...
    /// `doTimeout = timeout` — direct alias.
    #[inline]
    pub fn do_timeout(
        &mut self,
        global: &JSGlobalObject,
    ...
    /// `doUpgrade = wrapInstanceMethod(onUpgrade)` — `(object: JSValue, optional: ?JSValue)`.
    pub fn do_upgrade(
        &mut self,
        global: &JSGlobalObject,
        callframe: &bun_jsc::CallFrame,
        ...
        let mut iter = args.slice().iter().copied();
        let object = iter
            .next()
            .ok_or_else(|| global.throw_invalid_arguments(format_args!("Missing argument")))?;
        let optional = iter.next();
        self.on_upgrade(global, object, optional)
    }

    /// `doPublish = wrapInstanceMethod(publish)` —
    /// `(topic: ZigString, message_value: JSValue, compress_value: ?JSValue)`.
    pub fn do_publish(
        &mut self,
        global: &JSGlobalObject,
        callframe: &bun_jsc::CallFrame,
        ...
        // ZigString param: missing → "Missing argument"; null/undef → "Expected string".
        ...
        let message_value = iter
            .next()
            .ok_or_else(|| global.throw_invalid_arguments(format_args!("Missing argument")))?;
        let compress_value = iter.next();

        self.publish(global, topic, message_value, compress_value)
    }

    /// `doRequestIP = wrapInstanceMethod(requestIP)` — `(request: *Request)`.
    pub fn do_request_ip(
        ...
        let args = callframe.arguments_old::<1>();
        let arg = args.slice().first().copied().ok_or_else(|| {
            global.throw_invalid_arguments(format_args!("Missing Request object"))
        })?;
        let request = <crate::webcore::Request as bun_jsc::JsClass>::from_js(arg).ok_or_else(|| {
            global.throw_invalid_arguments(format_args!("Expected Request object"))
        })?;
        // SAFETY: `from_js` returns a live *mut Request owned by the JS wrapper;
        // borrowed read-only for the duration of this synchronous call.
