...
impl<const SSL: bool, const DEBUG: bool> NewServer<SSL, DEBUG> {
    ...
    fn set_routes(&mut self) -> JSValue {
        ...
        if let Some(dev) = &mut self.dev_server {
            ...
            let _ = dev;
            has_dev_server_for_star_path = todo!("blocked_on: bake::dev_server::DevServer::set_routes");
...
// generated by .classes.ts codegen); hand-written until that codegen lands.
macro_rules! impl_server_jsclass {
    ($ty:ident) => {
        impl bun_jsc::JsClass for $ty {
            fn from_js(_value: JSValue) -> Option<*mut Self> {
                todo!(concat!("blocked_on: bun_jsc::JsClass codegen for ", stringify!($ty)))
            }
            ...
            fn get_constructor(_global: &JSGlobalObject) -> JSValue {
                todo!(concat!("blocked_on: bun_jsc::JsClass codegen for ", stringify!($ty)))
            }
        }
    };
}
impl_server_jsclass!(HTTPServer);
impl_server_jsclass!(HTTPSServer);
impl_server_jsclass!(DebugHTTPServer);
impl_server_jsclass!(DebugHTTPSServer);
