//! Body of `NewServer` — split from `mod.rs` for compile-time sharding.
...
use bun_core::{self as bstr, String as BunString, UnsafeStringView, ZStr, strings};
...
use bun_jsc::{StringJsc as _, UnsafeStringViewJsc as _};
...
// ─── RequestCtx trait ────────────────────────────────────────────────────────
// PORT NOTE: `NewRequestContext` originally exposed `Req`/`Resp`/`http3` as
...
// route-coverage bitset uses the `HTTP.Method.Set` spelling.
...
// *mut c_void)` + opaque user-data. Originally a per-(ctx,handler)
// trampoline was generated at comptime; we recover that here by monomorphising on the
...
// `generate()` builds the struct and
// hands it to `JSON.toAST`, which originally reflected over fields at comptime.
// Rust has no field reflection, so this is the hand-expanded output (cf.
...
pub mod BunInfo {
    ...
    /// `Bundler` is an unused type witness; the allocator parameter maps
    /// onto the global expr `Store` used by `Expr::init`.
...
impl AnyRoute {
    fn bundled_html_manifest_item_from_js(
        ...
        // PORT NOTE: `from_bun_string` clones
        ...
        let cwd: &[u8] = if StandaloneModuleGraph::is_bun_standalone_file_path(path_slice) {
            // `targetBasePublicPath(Environment.os, "root/")` — exposed as a const.
    ...
    ///
    /// See `src/bundler/HTMLImportManifest.rs`.
    fn bundled_html_manifest_from_js(
        ...
        // `UTF8Slice` impls `Drop` — freed at scope end.
    ...
    pub fn html_route_from_js(
        ...
        use bun_collections::unmanaged_hash_map::MapEntry as StdEntry;
        if let Some(html_bundle) = <HTMLBundle as bun_jsc::JsClass>::from_js(argument) {
            ...
            return Ok(Some(match entry {
                StdEntry::Vacant(v) => {
                    // Store the rc=1 `Route::init(..)` in the map and
                    // return that same value to the caller (the map slot is a
                    ...
                    // (`RefPtr<T>` has no `Drop`; this is a bit-copy).
    ...
    pub fn from_js(
        ...
        if argument.is_object() {
            if let Some(dir) = argument.get_optional_slice(global, b"dir")? {
                ...
                // Originally a borrow into the route key (arena-backed) was stored. Rather
...
impl ServePlugins {
    ...
    fn load_and_resolve_plugins(&mut self, global: &JSGlobalObject) -> JsResult<()> {
        ...
        // NOTE: the keep-alive ref/deref pair (`this.ref(); defer this.deref()`)
...
/// Per-process latch for the dev-mode idle-timeout warning. Originally a
/// per-monomorphization static inside `NewServer`, but the
...
/// Body of `onTimeoutForIdleWarn` — emits the once-only dev-mode
    ...
    pub(super) fn notify_inspector_server_stopped(&mut self) {
        if self.inspector_server_id.get() != 0 {
            ...
            if let Some(debugger) = &self.vm().as_mut().debugger {
                ...
                // originally took `AnyServer` and unpacked `inspector_server_id`
    ...
    NewRequestContext<Self, SSL, DEBUG, true>: super::request_context::RequestContextHostFns,
{
    ...
    // PORT NOTE: `getPluginsAsync` was referenced from `AnyServer.loadAndResolvePlugins`
    // but never defined on `ThisServer` — the dispatch arm was dead.
    // The Rust port omits both the
    ...
    pub fn get_or_load_plugins(
        ...
        if let Some(p) = self.plugins {
            ...
            // (`this.ref(); defer this.deref()`). The guard is built from the
    ...
    //
    // PORT NOTE: `host_fn.wrapInstanceMethod(ThisServer, "name", false)`
    // is a type-directed argument decoder.
    ...
    // server (`UnsafeStringView`, `JSValue`, `?JSValue`, `*WebCore.Request`) are
    // open-coded here. They mirror the original branches exactly: same error
    ...
    pub fn do_publish(
        ...
        let topic = UnsafeStringView::from(topic_value.get_unsafe_string_view(global)?);
        // jsc.JSValue
    ...
    pub fn on_upgrade(
        ...
        if let Some(node_http_response) = <NodeHTTPResponse as bun_jsc::JsClass>::from_js(object) {
            ...
            let mut sec_websocket_protocol = UnsafeStringView::EMPTY;
            let mut sec_websocket_extensions = UnsafeStringView::EMPTY;

            // Owned backing storage for the above when they come from options.headers.
            // fastGet returns a UnsafeStringView that borrows from the header map entry's
            // StringImpl, which fastRemove then frees — so we must copy the bytes
            // before removing the entry.
            let mut sec_websocket_protocol_owned = UTF8Slice::EMPTY;
            let mut sec_websocket_extensions_owned = UTF8Slice::EMPTY;
                    ...
                    if let Some(headers_value) = opts.fast_get(global, jsc::BuiltinName::headers)? {
                            ...
                            fetch_headers_to_use.fast_get(HTTPHeaderName::SecWebSocketProtocol)
                        {
                            ...
                            sec_websocket_protocol =
                                UnsafeStringView::init(sec_websocket_protocol_owned.slice());
                            ...
                            fetch_headers_to_use.fast_get(HTTPHeaderName::SecWebSocketExtensions)
                        {
                            ...
                            sec_websocket_extensions =
                                UnsafeStringView::init(sec_websocket_extensions_owned.slice());
                            // Remove from headers so it's not written twice (once here and once by upgrade())
        ...
        let mut sec_websocket_key_str = UnsafeStringView::EMPTY;
        let mut sec_websocket_protocol = UnsafeStringView::EMPTY;
        let mut sec_websocket_extensions = UnsafeStringView::EMPTY;

        // Owned backing storage for sec_websocket_*.
        // `UTF8Slice` impls `Drop`; reassignment drops the previous value.
        let mut sec_websocket_key_owned = bun_core::UTF8Slice::empty();
        let mut sec_websocket_protocol_owned = bun_core::UTF8Slice::empty();
        let mut sec_websocket_extensions_owned = bun_core::UTF8Slice::empty();
        ...
        if let Some(head) = crate::webcore::body::BodyMixin::get_fetch_headers(request) {
            ...
            if let Some(key) = head.fast_get(HTTPHeaderName::SecWebSocketKey) {
                ...
                sec_websocket_key_str = UnsafeStringView::init(sec_websocket_key_owned.slice());
            }
        ...
        if let Some(req_ptr) = upgrader.req {
            ...
            if sec_websocket_key_str.len == 0 {
                sec_websocket_key_str =
                    UnsafeStringView::init(r.header(b"sec-websocket-key").unwrap_or(b""));
            }
            if sec_websocket_protocol.len == 0 {
                sec_websocket_protocol =
                    UnsafeStringView::init(r.header(b"sec-websocket-protocol").unwrap_or(b""));
            }
            if sec_websocket_extensions.len == 0 {
                sec_websocket_extensions =
                    UnsafeStringView::init(r.header(b"sec-websocket-extensions").unwrap_or(b""));
            }
        }
        ...
        // `UnsafeStringView::Slice` impls `Drop` — freed at scope exit.
    ...
    pub fn on_reload_from_native(
        ...
        // PORT NOTE: original drained+freed `this.config.static_routes` then assigned
    ...
    pub fn on_reload(
        ...
        self.on_reload_from_native(&mut new_config, global);
    ...
    pub fn on_fetch(&mut self, ctx: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> {
        ...
        if self.config.on_request.is_none() {
            return Ok(
                JSPromise::dangerously_create_rejected_promise_value_without_notifying_vm(
                    ctx,
                    UnsafeStringView::init(b"fetch() requires the server to have a fetch handler")
        ...
        if arguments.is_empty() {
            ...
            return Ok(
                JSPromise::dangerously_create_rejected_promise_value_without_notifying_vm(
                    ctx,
                    UnsafeStringView::init(fetch_error.as_bytes()).to_error_instance(ctx),
                ),
            );
        }
        ...
        let existing_request: Box<Request> = if first_arg.is_string() {
            let url_unsafe_str_view = arguments[0].to_slice(ctx)?;
            let temp_url_str = url_unsafe_str_view.slice();
            ...
            if arguments.len() >= 2 && arguments[1].is_object() {
                ...
                if let Some(headers_) = opts.fast_get(ctx, jsc::BuiltinName::Headers)? {
                    if let Some(headers__) = FetchHeaders::cast_(headers_, ctx.vm()) {
                        ...
                        // refcount. It is stored directly in
                        // `Request.#headers` and
                    ...
                    match Blob::get::<true, false>(ctx, body__) {
                        ...
                        Err(_) => {
                            return Ok(JSPromise::dangerously_create_rejected_promise_value_without_notifying_vm(
                                ctx,
                                UnsafeStringView::init(b"fetch() received invalid body").to_error_instance(ctx),
                            ));
                        }
                    }
                }
            ...
            Box::new(Request::init2(
                ...
                headers,
                // Move `body` into the per-VM hive pool (ref_count = 1).
            ...
            .flatten()
        {
            ...
            // PORT NOTE: `request_.cloneInto(&existing_request, alloc, ctx, false)`
            // wrote into a default-initialized `existing_request`.
        ...
        // `Request.new(existing_request)` → raw
        // `*Request`. `Request::to_js` stores `self as *mut
        ...
        if response_value.is_empty_or_undefined_or_null() {
            return Ok(
                JSPromise::dangerously_create_rejected_promise_value_without_notifying_vm(
                    ctx,
                    UnsafeStringView::init(b"fetch() returned an empty value")
                        .to_error_instance(ctx),
                ),
            );
        }
    ...
    fn prepare_js_request_context_for<Ctx: RequestCtxOps<Server = Self>>(
        ...
        // `Ctx::create`. The original parsed inline at both sites and
        // forwarded the unresolved `method` arg, so `create` parsed again.
        ...
        // `.signal = signal.ref()` — bump once for the Request's owned
...
// `wrap{3,4}` emits the C calling convention and the C++ declarations in
