...
impl EnvStr {
    ...
    fn tag(self) -> Tag {
        ...
        // any other value is corruption — trap (safety-checked enum cast)
        // rather than silently folding to `Empty`.
    ...
    pub fn dupe_ref_counted(old_str: &[u8]) -> EnvStr {
        ...
        // PORT NOTE: global mimalloc + abort-on-OOM is the Rust default; ownership
        // of the duplicated bytes transfers to RefCountedStr.
    ...
    pub fn init_ref_counted(str: &[u8]) -> EnvStr {
        // TODO(port): the original `initRefCounted` handed the slice to RefCountedStr which
        // assumed ownership of the backing allocation. Revisit RefCountedStr::init ownership
        // contract in Phase B (caller-allocated vs. dupe-on-init).
        if str.is_empty() {
            return Self::pack(0, Tag::Empty, 0);
        }

        // PORT NOTE: `len` is defaulted to 0 here (only `ptr` + `tag` set); the slice
        // length is recovered via RefCountedStr::byte_slice(). Preserve that.
        // PORT NOTE: the original handed the borrowed slice to RefCountedStr which assumed
        // ownership of its backing allocation. Rust cannot transfer ownership through `&[u8]`, so we
    ...
    pub fn slice(&self) -> &[u8] {
        ...
        // responsible for keeping the backing storage alive (same contract as `slice()`).
        match self.tag() {
            Tag::Empty => b"",
...
fn to_ptr(ptr_val: *const c_void) -> u64 {
    // Mask the low 48 bits of the address.
