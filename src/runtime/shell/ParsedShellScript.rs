...
pub struct ParsedShellScript {
    ...
    // PORT NOTE: this Vec's backing storage used to live in `args.arena` (self-referential
...
impl ParsedShellScript {
    ...
    // PORT NOTE: reshaped from 5 out-params to a returned tuple; the original used out-params
    ...
    pub fn set_env(&self, global: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> {
        ...
        while let Some(key) = object_iter.next()? {
            ...
            // `UnsafeStringView::to_owned_slice` is infallible in the Rust port
            // (global alloc aborts on OOM).
...
pub const CREATE_PARSED_SHELL_SCRIPT: bun_jsc::JSHostFnSafe =
...
fn create_parsed_shell_script_impl(
    ...
    // Box<ShellArgs> drops automatically on every early `return`/`?` below, so
    ...
    // PERF(port): was a stack-fallback allocation — profile in Phase B.
    // Per-element deref + Vec free are handled by `JsStrings`'s `Drop`.
    let mut jsstrings = JsStrings::with_capacity(4);

    // PORT NOTE: `jsobjs` and `script` were originally allocated from `shargs.arena_allocator()`.
