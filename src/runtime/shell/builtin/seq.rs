...
pub struct Seq {
    ...
    separator: bun_core::ptr::RawSlice<u8>,
    terminator: bun_core::ptr::RawSlice<u8>,
    fixed_width: bool,
}
...
impl Default for Seq {
    fn default() -> Self {
        Self {
            ...
            increment: 1.0,
            separator: bun_core::ptr::RawSlice::new(b"\n"),
            terminator: bun_core::ptr::RawSlice::EMPTY,
            fixed_width: false,
        }
    }
}
    ...
    pub(crate) fn start(interp: &Interpreter, cmd: NodeId) -> Yield {
        ...
        while idx < argc {
            ...
            if arg == b"-s" || arg == b"--separator" {
                ...
                Self::state_mut(interp, cmd).separator = bun_core::ptr::RawSlice::new(bytes);
                idx += 1;
                continue;
            }
            if arg.starts_with(b"-s") && arg.len() > 2 {
                Self::state_mut(interp, cmd).separator = bun_core::ptr::RawSlice::new(&arg[2..]);
                idx += 1;
                continue;
            }
            if arg == b"-t" || arg == b"--terminator" {
                ...
                let bytes = Builtin::of(interp, cmd).arg_bytes(idx);
                Self::state_mut(interp, cmd).terminator = bun_core::ptr::RawSlice::new(bytes);
                idx += 1;
                continue;
            }
            if arg.starts_with(b"-t") && arg.len() > 2 {
                Self::state_mut(interp, cmd).terminator = bun_core::ptr::RawSlice::new(&arg[2..]);
                idx += 1;
                continue;
            }
