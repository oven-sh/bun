...
impl Yes {
    ...
    fn write_no_io_loop(interp: &Interpreter, cmd: NodeId) -> Yield {
        // The `.err` arm formats via
        // `fmtErrorArena(.yes, "{s}\n", .{e.name()})` and routes through
        // `writeFailingError`.
        //
        // Split-borrow the Cmd so the tiled buffer (in `impl_`) and `stdout`
        // are accessible simultaneously — passing the buffer zero-copy
        // matters for `yes` throughput.
        let err = {
        ...
        // PORT NOTE: `enqueue` ticks the event loop, which may
    ...
    fn enqueue_chunk(
        ...
        // so the tiled buffer is enqueued zero-copy as a slice.
...
// `buffer: Vec<u8>` drops with the owning `Box<Yes>`; no explicit `Drop`
// impl needed (PORTING.md §Allocators).

/// Re-queues `yes` onto the event loop after a burst of no-IO writes so we
/// don't block the main thread forever.
#[repr(C)]
pub struct YesTask {
    /// Back-ref to the owning [`Interpreter`] (NodeId-arena design replaces
    /// a `container_of` chain).
