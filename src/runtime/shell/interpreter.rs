...
//! 3. There is no coroutine/async runtime to lean on
//!
...
//! ## NodeId arena
//!
//! An earlier interpreter design had every state-machine node hold a `*Parent`
//! back-pointer and call `parent.childDone(this, exit)`. That pattern is
//! borrow-checker hostile (overlapping `&mut` of parent and child).
//!
//! The current design stores all state nodes in a flat `Vec<Node>` owned by
//! the `Interpreter`. Nodes refer to each other (and to their parent) by
//! `NodeId` — a `u32` index. Dispatch is a single hoisted `match` on the
//! parent's tag (`Interpreter::child_done`), which keeps the per-tick hot path
//! inlined (see PORTING.md §Dispatch hot-path).
//!
...
/// back-pointer in the state-machine tree.
#[repr(transparent)]
...
/// One slot in the interpreter's state arena. Earlier designs heap-allocated
/// each state struct individually via `parent.create(T)`; here they all
...
impl Node {
    ...
    /// hoisted accessor (replaces structural duck-typing on `.base`).
...
/// tag mismatch — same contract as `child.as(Ty).?`.
...
pub struct Interpreter {
    ...
    pub vm_args_utf8: JsCell<Vec<bun_core::UTF8Slice>>,
...
impl ShellArgs {
    /// `ShellArgs.init` — fresh arena + zeroed AST.
    ...
    pub fn init() -> Box<ShellArgs> {
        Box::new(ShellArgs {
            ...
            // `script_ast` is overwritten by `parse()` before
    ...
    /// `ShellArgs.memoryCost`.
    /// PORT NOTE: rather than walking `script_ast.memoryCost()`, this reports
...
impl Interpreter {
    ...
    /// `ctx` is stored for `bun run` argv access from builtins
    ...
    pub fn init(
        ...
        // ── export_env ─────────────────────────────────────────────────────
        // On `.js` event loop, take `export_env_` (or empty); on `.mini`,
        ...
        // NUL-terminate for `open()` and so `__cwd` is NUL-terminated
        ...
        let interpreter = Box::new(Interpreter {
            ...
            // PORT NOTE — intentional bug fix: an earlier implementation
            // declared `exit_code: ?ExitCode = 0` (the *non-null* value 0),
            // so its `asyncCmdDone`'s `exit_code != null` check was always
            // true and could fire `finish(0)` before the root script had
            // actually returned. Starting at `None` means `async_cmd_done`
            // only finishes once `on_root_child_done` has recorded the real
            // exit code.
        ...
        // ── optional cwd override (`init` tail) ────────────────────────────
        if let Some(c) = cwd_ {
            // `root_shell.changeCwdImpl(interp,
    ...
    /// `#deinitFromExec` — full teardown for the
    ...
    fn deinit_from_exec(self: Box<Self>) {
        ...
        // `vm_args_utf8` slices Drop themselves (UTF8Slice has a Drop
    ...
    /// directly; the `Result` only carries lexer/parser
    /// errors that escaped without a diagnostic.
    ...
    fn init_and_run_impl(
        ...
        // ── tick until done ────────────────────────────────────────────────
        // `mini.tick(&is_done, IsDone.isDone)` where `isDone` reads
...
// table. `StatePtrUnion` originally derived `start`/`next`/
// `childDone` from one `inline for (tags)` over a comptime type tuple, which
// was hand-unrolled into three parallel 11-arm matches. This
...
// (Stmt/Pipeline/Cmd). No vtable, no extra indirection.
//
...
macro_rules! shell_state_dispatch {
    ...
    (@norm [ $( ($v:ident, $h:ident) )+ ]) => {
        ...
        /// `parent.childDone(this, exit)` call.
...
impl Interpreter {
    ...
    /// `parent.create(T)` (which heap-allocated via the parent's
    ...
    /// Free a slot. Replaces `parent.destroy(this)`. The node's own
    ...
    /// per-variant `match` inlined at each callsite (Stmt/Binary).
    /// For `.subshell`, dupes the parent shell env first
    /// (`Subshell.initDupeShellState`); all other variants borrow `shell`.
    ///
    ...
    pub fn spawn_expr(
        ...
        let child = match expr {
            ...
            ast::Expr::Subshell(s) => {
                // `Subshell.initDupeShellState`: Stmt/Binary callers dupe
                ...
                match Subshell::init_dupe_shell_state(self, shell, *s, parent, io) {
                    ...
                    Err(e) => {
                        ...
                        // `Binary.makeChild` returns `null` here and
                        ...
                        // that fallthrough exactly (no `NodeId::NONE` sentinel
    ...
    /// `child.deinit()` + `parent.destroy(child)` pair.
    ...
    pub fn compute_estimated_size_for_gc(&self) -> usize {
        ...
        size += vm_args.capacity() * core::mem::size_of::<bun_core::UTF8Slice>();
        size
    }
    ...
    /// `estimatedSize`. Codegen-called accessor.
    ...
    /// `throwShellErr(err, event_loop)` — `mini` prints
    ...
    /// `setupIOBeforeRun` + `setupIOBeforeRunImpl`
    ...
    pub fn finish(&self, exit_code: ExitCode) -> Yield {
        ...
        // `decrPendingActivityFlag(...)` is deferred
    ...
    /// `runFromJS`. JS-host entrypoint — sets up root IO
    ...
    /// `#derefRootShellAndIOIfNeeded`. Idempotent
    ...
    fn deref_root_shell_and_io_if_needed(&self, free_buffered_io: bool) {
        ...
        // PORT NOTE: free the parse arena eagerly. A lightweight bump arena
        // (a few KB) could be left for the GC finalizer, but
        // `bun_alloc::Arena` is
    ...
    /// `deinitFromFinalizer`. GC finalizer body — runs
    ...
    pub unsafe fn deinit_from_finalizer(this: *mut Self) {
        ...
        // `args: Box<ShellArgs>` and `vm_args_utf8: Vec<UTF8Slice>` drop
        // with the box; `UTF8Slice` has a `Drop` impl that derefs its
        // WTF backing (per-item `str.deinit()` + list deinit).
    }

    /// `setQuiet` — JS `interp.setQuiet()`.
    pub fn set_quiet(
        &self,
    ...
    /// `setEnv` — JS `interp.setEnv({ FOO: "bar" })`.
    pub fn set_env(
        &self,
        global_this: &crate::jsc::JSGlobalObject,
        callframe: &crate::jsc::CallFrame,
        ...
        while let Some(key) = object_iter.next()? {
            ...
            let value_str = value.get_unsafe_string_view(global_this)?;
            let slice = value_str.to_owned_slice();
            // PORT NOTE: `initRefCounted` originally adopted the slice;
    ...
    /// `finalize`. GC finalizer hook — called from the
    ...
    /// `hasPendingActivity`. GC `hasPendingActivity()`.
    ...
    /// `getVmArgsUtf8`. Lazily caches the worker's
    /// `argv` as UTF-8 slices so `$@`/`$N` expansion (Expansion) can index
    ...
    /// `expandVarArgv`. Appends the value of `$N` to
...
/// `ioToJSValue`. Moves the captured stdout/stderr
...
fn io_to_js_value(
    ...
    // PORT NOTE: equivalent of `jsc.Node.Buffer{ .buffer = ArrayBuffer.fromBytes
...
/// `throwShellErr(e, event_loop)`. On the mini event
...
/// `JsError::Thrown`.
// PORT NOTE: takes ownership by value (both arms
// consume the `ShellErr`). `global` is `Option` because the
...
impl ShellExecEnv {
    ...
    pub fn memory_cost(&self) -> usize {
        ...
        // PORT NOTE: `async_pids` is a `Vec` here, so report its heap
        // capacity directly rather than walking a SmolList.
    ...
    /// `ShellExecEnv.dupeForSubshell`. Heap-allocates a
    ...
    /// `ShellExecEnv.deinit` — wraps `deinitImpl(true,
    ...
    pub fn deinit_impl(this: *mut ShellExecEnv) {
        ...
        // explicit close (`closefd(this.cwd_fd)`).
        ...
        // is a raw ptr so its drop is a no-op (matches
        // `if (== .owned) clearAndFree`).
        drop(boxed);
    }

    /// `ShellExecEnv.deinitImpl(false, free_buffered_io)`
    ...
    /// `ShellExecEnv.changeCwd` — thin `in_init = false`
    /// wrapper. We accept the un-terminated slice and let `change_cwd_impl`
    /// re-NUL into
    ...
    pub fn change_cwd_impl(&mut self, new_cwd_: &[u8], in_init: bool) -> bun_sys::Result<()> {
        ...
        // Bounds-check against a `[4096]u8` buffer
        // on *every* platform. Do NOT use
        // `bun_paths::PathBuffer` here: on Windows that is `MAX_PATH_BYTES =
        // 32767*3+1` ≈ 96 KiB of zero-filled stack per `cd`, and the
        // `>= buf.len()` check would diverge from the ENAMETOOLONG bound.
        let mut buf = [0u8; 4096];
        ...
        let new_cwd_len: usize = if is_abs {
            ...
            // `ResolvePath.joinZ(&.{cwd, new_cwd_},
            ...
            // remove trailing separator (Windows
    ...
    /// value, so callers must release `value` after.
    pub fn assign_var(
        &mut self,
...
impl CowFd {
    ...
    pub fn deref(this: *mut CowFd) {
        ...
        unsafe {
            (*this).refcount -= 1;
            if (*this).refcount == 0 {
                // `CowFd.deinit`: close the fd
                // before freeing. `closefd` tolerates EBADF like
...
/// `bun.sys.openNullDevice()` — open `/dev/null` `O_RDWR`
...
/// `closefd` → `fd.closeAllowingBadFileDescriptor`.
...
fn shell_get_path<'a>(
    ...
    // PORT NOTE: reshaped for borrowck — `joinZBuf(buf, &.{dirpath, to})`
    // would read `dirpath` (a slice of `buf`) while writing `buf`; copy
    // `dirpath` out first so the mutable borrow on `buf` is exclusive.
...
pub fn shell_openat(
    ...
    #[cfg(windows)]
    {
        ...
        // trailing `if (isWindows) makeLibUVOwned` in the reference was dead
...
/// `makeLibUVOwnedForSyscall` — the dead `if (isWindows)` tail
/// is unreachable after the early `return bun.sys.open(...)`.
#[allow(dead_code)] // no callers yet; ported for ShellSyscall surface parity
pub fn shell_open(
...
// ────────────────────────────────────────────────────────────────────────────
// Builtin flag-parsing infra (`ParseError` / `FlagParser`)
// ────────────────────────────────────────────────────────────────────────────

/// Custom parse error for invalid options. `ParseError`.
///
...
/// `unsupportedFlag` originally returned
/// the comptime-concatenated `"unsupported option, please open a GitHub issue
/// -- " ++ name ++ "\n"`. Every caller then wrapped that AGAIN in the same
/// prefix via `fmtErrorArena`, so stderr printed the prefix twice.
///
/// PORT NOTE — intentional bug fix: we return just `name` and let the
/// caller's `fmt_error_arena` add the prefix once. This diverges from the
/// historic observable doubled output; update any snapshot tests asserting
/// the doubled message rather than reproducing the duplication here. Reproducing
...
/// `FlagParser(Opts)` — originally a comptime
/// type-generator; here the per-opts hooks are a trait.
...
/// `FlagParser.parseFlags`. Returns the trailing
...
// ────────────────────────────────────────────────────────────────────────────
// OutputTask (`OutputTask` / `OutputSrc`)
// ────────────────────────────────────────────────────────────────────────────

/// `OutputSrc`. Owned bytes a builtin's async sub-task
...
/// `OutputTask` vtable. Originally a comptime struct
...
pub struct OutputTask<P: OutputTaskVTable> {
    /// Owning Cmd node (the builtin's `cmd` id). Replaces a `*Parent` back-ref.
    pub parent: NodeId,
...
impl<P: OutputTaskVTable> OutputTask<P> {
    ...
    pub unsafe fn on_io_writer_chunk(
        ...
        // The SystemError's drop handles its own deref.
        unsafe { Self::next(this, interp) }
    }

    /// `OutputTask.deinit` — fires `on_done` then frees.
...
/// `ShellTask(Ctx, runFromThreadPool, runFromMainThread, log)`.
///
/// Originally a comptime type-generator over the parent ctx + two fn pointers;
/// here it's a trait the per-builtin task struct implements. The
/// `container_of`-style chain (WorkPoolTask → InnerShellTask → Ctx)
...
/// JS-side `ConcurrentTask` (`concurrent_task.js.from(ctx, .manual_deinit)`
/// resolves the tag via the `Taskable` impl).
pub trait ShellTaskCtx: Sized + bun_event_loop::Taskable {
    ...
    /// outer `Self` (the two-hop `container_of` chain in
...
pub struct ShellTask {
    ...
    /// Back-ref to the owning [`Interpreter`]. Earlier designs threaded the
    /// interpreter through each builtin's parent-ptr chain; this design uses
    ...
    /// mini arm holds an `AnyTaskWithExtraContext` (`jsc.EventLoopTask`).
...
impl ShellTask {
    ...
    /// `InnerShellTask.schedule`. Installs the per-`C`
    ...
    /// [`WorkPool`] WITHOUT a `keep_alive.ref` — for tasks
    /// scheduled via raw `WorkPool.schedule(&this.task)` (e.g.
    ...
    /// `InnerShellTask.onFinish`. Called from the worker
    ...
    pub unsafe fn on_finish<C: ShellTaskCtx>(ctx: *mut C) {
        ...
        let (event_loop, task_ptr) = unsafe {
            ...
            let task_ptr = match &mut (*this).concurrent_task {
                EventLoopTask::Js(ct) => {
                    // `concurrent_task.js.from(ctx, .manual_deinit)` —
                ...
                EventLoopTask::Mini(at) => {
                    // `concurrent_task.mini.from(this, "runFromMainThreadMini")`.
    ...
    /// `InnerShellTask.runFromMainThread`. Unrefs the
    ...
    /// the seam (`this.ref.unref(this.event_loop)` before
...
/// `runFromThreadPool` — recover `*Ctx` from the
...
unsafe fn shell_task_trampoline<C: ShellTaskCtx>(task: *mut WorkPoolTask) {
    ...
    // embedded in `C` at `TASK_OFFSET` (two `container_of` hops). `ctx`
...
/// `InnerShellTask.runFromMainThreadMini` — mini-loop
...
// `Interpreter.createShellInterpreter`.
