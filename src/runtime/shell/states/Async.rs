    ...
    pub fn next(interp: &Interpreter, this: NodeId) -> Yield {
        ...
        match action {
            ...
            NextAction::SpawnChild => {
                ...
                // `.exec` arm, child==None: init the child WITHOUT starting it,
                // store it, enqueue self, return suspended. The child is
                // started on the NEXT event-loop tick via the `StartChild` arm
                // above. Restricted to pipeline/cmd/if/condexpr — other Expr
                // variants panic.
