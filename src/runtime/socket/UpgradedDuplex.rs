...
// `CertError.deinit` only freed `code`/`reason`; `Box<CStr>` drops automatically — no explicit Drop needed.
...
/// redundant `set_function_data` is preserved verbatim.
#[inline]
fn lazy_js_handler(
...
impl UpgradedDuplex {
    ...
    fn on_close(this: *mut Self) {
        ...
        // SAFETY: SSLWrapper handlers ctx is `self as *mut Self`; live for the wrapper's lifetime.
        let this = unsafe { &mut *this };
        // teardown runs after the two calls below.
    ...
    fn call_write_or_end(&mut self, data: Option<&[u8]>, msg_more: bool) {
        ...
        // `JSValue.getFunction` semantics — `.get()` + callable check.
    ...
    fn on_internal_receive_data(&mut self, data: &[u8]) {
        // PORT NOTE: reshaped for borrowck — the original borrowed `wrapper` then called
    ...
    pub fn ssl_error(&self) -> us_bun_verify_error_t {
        us_bun_verify_error_t {
            ...
            reason: self
                ...
                .as_deref()
                .map_or(b"\0".as_ptr().cast(), |c| c.as_ptr()),
            // TODO(port): us_bun_verify_error_t may have more fields; the original used implicit defaults.
        }
    }
