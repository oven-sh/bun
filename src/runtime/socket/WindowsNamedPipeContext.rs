...
pub struct WindowsNamedPipeContext {
    ...
    // — preserves the original `deinit` order (socket.deref() then named_pipe.deinit()).
    socket: SocketType,
...
/// `TLSSocket.Socket.fromNamedPipe(&this.named_pipe)` where
...
impl WindowsNamedPipeContext {
    ...
    pub fn create(
        ...
        // `jsc.AnyTask.New(WindowsNamedPipeContext, runEvent).init(this)` — the
...
impl Drop for WindowsNamedPipeContext {
    fn drop(&mut self) {
        ...
        // Deref the wrapped socket, then `named_pipe` drops.
