...
//! src/runtime/socket/socket_body.rs (re-exported as crate::socket::socket in mod.rs) during the Rust port,
...
//! exists only so the codegen can resolve `$rust("socket.rs", …)`. Deleting
