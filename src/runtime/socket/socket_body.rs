...
pub mod testing_apis {
    ...
    pub(crate) fn js_set_socket_fault(
        ...
        #[cfg(socket_fault_injection)]
        {
            ...
            } else if syscall_str.eql_comptime(b"setsockopt") {
                fi::SETSOCKOPT
            } else {
                // socket/close/shutdown have enum slots but no bsd.c hooks;
                // accepting them would arm rules that can never fire.
                return Err(global.throw(format_args!(
                    "rule.syscall must be one of: recv, send, writev, sendmsg, recvmsg, connect, accept, ssl_loop_buffer, poll_start, session_buffer, setsockopt"
                )));
            };
