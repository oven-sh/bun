...
pub fn create_binding(global_object: &JSGlobalObject) -> JSValue {
    ...
    binding.put(
        global_object,
        b"MySQLConnection",
        crate::sql_jsc::jsc::codegen::JSMySQLConnection::get_constructor(global_object),
    );
    crate::put_host_functions!(
        ...
        [
            ...
            (
                "createQuery",
                js_mysql_query::JSMySQLQuery::create_instance,
                6
            ),
            (
                "createConnection",
                js_mysql_connection::JSMySQLConnection::create_instance,
                2
            ),
        ]
    )
...
// Downstream consumers import via the `js_mysql_{connection,query}` spelling.
