...
impl Drop for Bytes {
    fn drop(&mut self) {
        if !self.pinned.is_empty() {
            ...
            // lifetime of this Value (see struct doc).
        ...
        match self {
            ...
            Value::StringData(data) | Value::BytesData(data) => {
                ...
                return Ok(if s.is_empty() {
                    ...
                    Data::Temporary(bun_core::RawSlice::new(s))
                });
            }
            Value::String(slice) => {
                let s = slice.slice();
    ...
    pub fn from_js(
        ...
        match field_type {
            ...
            FieldType::MYSQL_TYPE_FLOAT => Ok(Value::Float(
                value
                    .coerce_f64(global_object)
                    .map_err(AnyMySQLError::from)? as f32,
            )),
            ...
            | FieldType::MYSQL_TYPE_BLOB => {
                ...
                if let Some(blob) = value.as_class_ref::<Blob>() {
                    if blob.needs_to_read_file() {
                        return Err(AnyMySQLError::from(global_object.throw_invalid_arguments(
                ...
                if value.is_string() {
                    let str = OwnedString::new(
                        BunString::from_js(value, global_object).map_err(AnyMySQLError::from)?,
                    );
                    return Ok(Value::String(str.to_utf8()));
                }

                Err(AnyMySQLError::from(global_object.throw_invalid_arguments(
...
/// Feeds `Execute::write` without the protocol crate naming `Value`. A local
/// newtype is required: the orphan rule rejects a foreign-trait impl for the
/// (always-foreign) slice type `[Value]`.
pub struct Params<'a>(pub &'a [Value]);

impl bun_sql::mysql::protocol::prepared_statement::ParamSource for Params<'_> {
    fn count(&self) -> usize {
        self.0.len()
    }
    fn is_null(&self, i: usize) -> bool {
        matches!(self.0[i], Value::Null)
    }
    fn to_data(&self, i: usize, field_type: FieldType) -> Result<Data, any_mysql_error::Error> {
        self.0[i].to_data(field_type)
    }
}
...
impl DateTime {
    ...
    pub fn from_text(text: &[u8]) -> Option<DateTime> {
        let parsed = crate::sql_jsc::shared::datetime_text::parse_mysql(text)?;
        if parsed.month < 1
            || parsed.month > 12
            || parsed.day < 1
