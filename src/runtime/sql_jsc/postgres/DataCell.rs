...
pub use crate::sql_jsc::shared::sql_data_cell::SQLDataCell;
pub use crate::sql_jsc::shared::sql_data_cell::{Array, Flags, Raw, Tag, TypedArray, Value};
...
fn from_bytes_typed_array<Elem: bun_sql::postgres::types::tag::WireByteSwap>(
    ...
    let js_typed_array_type = crate::sql_jsc::postgres::types::tag_jsc::to_js_typed_array_type(tag)
            ...
            if binary && bytes.len() == 8 {
                match tag {
                    T::timestamptz => Ok(SQLDataCell::date_with_tz(
                        crate::sql_jsc::postgres::types::date::from_binary(bytes),
                    )),
                ...
                let date = match tag {
                    T::timestamp => crate::sql_jsc::postgres::types::date::timestamp_text_to_ms_utc(global_object, bytes),
                    _ => None,
                };
                let date = match date {
                    Some(d) => d,
