...
use crate::sql_jsc::postgres::Signature;
use crate::sql_jsc::shared::QueryBindingIterator;
...
pub fn write_bind<Context: WriterContext>(
    ...
    for i in 0..(len as usize) {
        ...
        let force_text = is_custom_type
            || (tag.is_binary_format_supported()
                && 'brk: {
                    iter.to(i as u32);
                    if let Some(value) = iter.next().map_err(AnyPostgresError::from)? {
    ...
    iter.to(0);
    let mut i: usize = 0;
    while let Some(value) = iter.next().map_err(AnyPostgresError::from)? {
        ...
        match effective_tag {
            ...
            types::Tag::timestamp | types::Tag::timestamptz => {
                let l = writer.length()?;
                writer.int8(
                    crate::sql_jsc::postgres::types::date::from_js(global, value)
                        .map_err(AnyPostgresError::from)?,
                )?;
                l.write_excluding_self()?;
            }
            ...
            types::Tag::int4 => {
                ...
                writer.int4(
                    value
                        .coerce::<i32>(global)
                        .map_err(AnyPostgresError::from)? as u32,
                )?;
                l.write_excluding_self()?;
            }
            ...
            types::Tag::float8 => {
                let l = writer.length()?;
                writer.f64(value.to_number(global).map_err(AnyPostgresError::from)?)?;
                l.write_excluding_self()?;
            }

            _ => {
                let str = bun_core::OwnedString::new(
                    BunString::from_js(value, global).map_err(AnyPostgresError::from)?,
                );
...
use crate::sql_jsc::postgres::postgres_sql_connection::{SslMode, TlsStatus};
