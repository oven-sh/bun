...
pub struct Execution {
    ...
    /// The `EntryData` for `on_stack_entry`, set/restored alongside it. Read by
    /// `BunTest::get_on_stack_or_current_state_data()`, which is how code
    /// re-entered from inside `run_test_callback` (a synchronous `done(error)`,
    /// a rejection drained before the callback returned, node:test's runtime
    /// `t.skip()`/`t.todo()` mark) finds its entry inside a concurrent group,
    /// where `get_current_state_data()` cannot name one.
