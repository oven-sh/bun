...
fn bind(value: JSValue, global: &JSGlobalObject, name: BunString) -> JsResult<JSValue> {
    // `#[bun_jsc::host_fn]` emits the raw `JSHostFn` thunk `__jsc_host_call_as_function`.
    // `name` is borrowed (bit-copy); `JSFunction__createFromZig` only reads it,
    // so a `clone()` would leak the StringImpl.
