...
///
/// Lives here (not in `bun_bundler_jsc::analyze_jsc`) because `DiffFormatter`
/// is a `bun_runtime` type and `bun_bundler_jsc` is a lower-tier crate that
/// cannot depend on it; the `extern "C"` symbol resolves the same at link time
/// regardless of which crate defines it.
#[unsafe(no_mangle)]
pub extern "C" fn bun__renderDiff(
