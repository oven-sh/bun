...
impl Expect {
    ...
    pub(crate) fn do_unreachable(
        ...
        if arg.is_empty_or_undefined_or_null() {
            let error_value =
                global_this.create_error_instance(format_args!("reached unreachable code"));
            error_value.put(
                global_this,
                b"name",
                bun_core::String::static_("UnreachableError").to_js(global_this)?,
            );
...
impl ExpectMatcherContext {
    ...
    pub(crate) fn get_promise<'s>(this: &Self, scope: &mut Scope<'s>) -> JsResult<Local<'s>> {
        match this.flags.promise() {
            Promise::Rejects => scope.string(&bun_core::String::static_("rejects")),
            Promise::Resolves => scope.string(&bun_core::String::static_("resolves")),
            _ => Ok(scope.local(JSValue::js_empty_string(scope.unscoped_global()))),
        }
    }
