...
impl Expect {
    ...
    pub fn to_be(
        ...
        // The bool is only used to select a literal format string; evaluated at runtime.
        ...
        if left.jest_deep_equals(right, global_this)? || left.jest_strict_deep_equals(right, global_this)? {
            // The format string varies on `has_custom_label`; Rust format strings must
        ...
        // PORT NOTE: a single `*Formatter` cannot be shared across both `toFmt` calls; the
        // `BunFormatter` adapter holds `&'a mut Formatter`, so two live adapters cannot alias
