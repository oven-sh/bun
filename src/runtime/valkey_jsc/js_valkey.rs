...
// `jsc.Codegen.JSRedisClient` cached-slot accessors — generated by
// `generate-classes.ts` into `ZigGeneratedClasses.cpp`. The Rust codegen
// emitter doesn't yet emit a `.rs` mirror, so declare the externs locally
// (same pattern as `bun_sql_jsc::jsc::codegen::cached_slot!`).
pub mod js {
    ...
    macro_rules! cached_slot {
        ($get:ident, $set:ident, $get_ext:ident, $set_ext:ident) => {
            unsafe extern "C" {
                ...
                fn $set_ext(this_value: JSValue, global: *const JSGlobalObject, value: JSValue);
            }
            ...
            pub fn $set(this_value: JSValue, global: &JSGlobalObject, value: JSValue) {
                ...
                unsafe { $set_ext(this_value, global, value) }
            }
        };
    }
