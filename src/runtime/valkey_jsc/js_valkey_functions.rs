...
impl JSValkeyClient {
    ...
    // Bitmap commands
    cmd_varargs!(bitop, b"bitop", "BITOP", strict, NotSubscriber);
    cmd_varargs!(bitpos, b"bitpos", "BITPOS", strict, NotSubscriber);
    cmd_varargs!(bitfield, b"bitfield", "BITFIELD", strict, NotSubscriber);

    // HyperLogLog commands
    cmd_varargs!(pfcount, b"pfcount", "PFCOUNT", strict, NotSubscriber);
    cmd_varargs!(pfmerge, b"pfmerge", "PFMERGE", strict, NotSubscriber);

    // Geo commands
    cmd_varargs!(geoadd, b"geoadd", "GEOADD", strict, NotSubscriber);
    cmd_varargs!(geodist, b"geodist", "GEODIST", strict, NotSubscriber);
    cmd_varargs!(geohash, b"geohash", "GEOHASH", strict, NotSubscriber);
    cmd_varargs!(geopos, b"geopos", "GEOPOS", strict, NotSubscriber);
    cmd_varargs!(geosearch, b"geosearch", "GEOSEARCH", strict, NotSubscriber);
    cmd_varargs!(
        geosearchstore,
        b"geosearchstore",
        "GEOSEARCHSTORE",
    ...
    // Scripting commands
    cmd_varargs!(eval, b"eval", "EVAL", strict, NotSubscriber);
    cmd_varargs!(evalsha, b"evalsha", "EVALSHA", strict, NotSubscriber);
    cmd_varargs!(fcall, b"fcall", "FCALL", strict, NotSubscriber);
    cmd_varargs!(function, b"function", "FUNCTION", strict, NotSubscriber);

    // Server / connection commands
    cmd!(dbsize, b"dbsize", "DBSIZE", NotSubscriber);
    cmd_varargs!(flushdb, b"flushdb", "FLUSHDB", strict, NotSubscriber);
    cmd_varargs!(flushall, b"flushall", "FLUSHALL", strict, NotSubscriber);
    cmd_varargs!(info, b"info", "INFO", strict, NotSubscriber);
    cmd!(time, b"time", "TIME", NotSubscriber);
    cmd!(echo, b"echo", "ECHO", "message", NotSubscriber);
    cmd!(lastsave, b"lastsave", "LASTSAVE", NotSubscriber);
    cmd_varargs!(js_client, b"client", "CLIENT", strict, DontCare);
    cmd_varargs!(config, b"config", "CONFIG", strict, NotSubscriber);
    cmd_varargs!(debug, b"debug", "DEBUG", strict, NotSubscriber);
    cmd_varargs!(command, b"command", "COMMAND", strict, NotSubscriber);

    // Generic key commands
    cmd_varargs!(object, b"object", "OBJECT", strict, NotSubscriber);
    cmd_varargs!(sort, b"sort", "SORT", strict, NotSubscriber);
    cmd!(
        wait,
        b"wait",
        "WAIT",
    ...
    cmd_varargs!(lcs, b"lcs", "LCS", strict, NotSubscriber);

    // Stream commands
    cmd_varargs!(xadd, b"xadd", "XADD", strict, NotSubscriber);
    cmd!(xlen, b"xlen", "XLEN", "key", NotSubscriber);
    cmd_varargs!(xrange, b"xrange", "XRANGE", strict, NotSubscriber);
    cmd_varargs!(xrevrange, b"xrevrange", "XREVRANGE", strict, NotSubscriber);
    cmd_varargs!(xread, b"xread", "XREAD", strict, NotSubscriber);
    cmd_varargs!(
        xreadgroup,
        b"xreadgroup",
        "XREADGROUP",
    ...
    cmd_varargs!(xdel, b"xdel", "XDEL", strict, NotSubscriber);
    cmd_varargs!(xtrim, b"xtrim", "XTRIM", strict, NotSubscriber);
    cmd_varargs!(xack, b"xack", "XACK", strict, NotSubscriber);
    cmd_varargs!(xclaim, b"xclaim", "XCLAIM", strict, NotSubscriber);
    cmd_varargs!(
        xautoclaim,
        b"xautoclaim",
        "XAUTOCLAIM",
    ...
    cmd_varargs!(xpending, b"xpending", "XPENDING", strict, NotSubscriber);
    cmd_varargs!(xinfo, b"xinfo", "XINFO", strict, NotSubscriber);
    cmd_varargs!(xgroup, b"xgroup", "XGROUP", strict, NotSubscriber);
    cmd_varargs!(xsetid, b"xsetid", "XSETID", strict, NotSubscriber);
