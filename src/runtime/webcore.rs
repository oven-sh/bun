...
// ─── webcore-local jsc shim ──────────────────────────────────────────────────
// Extends `crate::jsc` (lib.rs shim) with the additional opaque types webcore
...
// per-file `use crate::webcore::jsc as bun_jsc;` aliases.
...
pub mod jsc {
    ...
    // Types referenced by webcore drafts that the lib.rs shim does not provide.
    opaque!(
        VirtualMachine, SystemError, DOMFormData, URLSearchParams, CommonAbortReason,
        EventLoopHandle, Task, EnsureStillAlive, JSPromiseStrong, URL, JSObject,
        JSType, BuiltinName, ZigException,
    );
    /// `jsc.JSRef` — self-wrapper ref. Stub matching the real `bun_jsc::JsRef`
    /// constructor surface used in struct defaults.
    #[derive(Debug, Default)]
    pub struct JsRef(core::marker::PhantomData<*const ()>);
    impl JsRef {
        #[inline] pub const fn empty() -> Self { Self(core::marker::PhantomData) }
    }
    pub mod strong {
        #[derive(Debug, Default)]
        pub struct Optional(core::marker::PhantomData<*const ()>);
        impl Optional {
            #[inline] pub const fn empty() -> Self { Self(core::marker::PhantomData) }
        }
    }
