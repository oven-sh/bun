...
impl ArrayBufferSink {
    ...
    pub(crate) fn end_from_js(
        ...
        // Ownership transfers to JSC; the caller converts the returned
        // `ArrayBuffer` with `to_js_unchecked`, which installs
        // `MarkedArrayBuffer_deallocator` (frees via `mi_free` on GC). Not
        // `to_js`: its `mi_is_in_heap_region` probe skips the deallocator when
        // the global allocator isn't mimalloc.
