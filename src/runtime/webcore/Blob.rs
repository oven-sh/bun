...
/// owner: `RefPtr::drop` derefs exactly once when JSC collects it —
/// possibly on another thread, which `RefPtr<Store>: Send` (atomic
...
fn store_backed_buffer_to_js<const TYPED_ARRAY_VIEW: jsc::JSType>(
    ...
    fn hrtb<F: for<'a> FnOnce(&'a mut RefPtr<Store>) -> &'a mut [u8]>(f: F) -> F {
        f
    }
    let view = hrtb(move |store| {
        let store::Data::Bytes(bytes) = Store::data_mut(store) else {
    ...
    fn to_array_buffer_view_with_bytes<const TYPED_ARRAY_VIEW: jsc::JSType>(
        ...
        match bytes {
            ...
            BlobBytes::Share(_) => {
                ...
                // `RefPtr<Store>` owner derefs exactly once when JSC collects the
                // object (`RefPtr::drop`). The `view` closure
            ...
            BlobBytes::Transfer(_) => {
                ...
                // owned by the boxed `RefPtr<Store>`, so JSC becomes the sole
...
impl BlobBytes {
    ...
    /// live. No `RefPtr<Store>` is held here — an extra ref would flip
