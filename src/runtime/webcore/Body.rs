...
fn apply_blob_content_type(blob: &Blob, from_headers: Option<&[u8]>) {
    let content_type = match from_headers {
        ...
        {
            blob::BlobContentType::from(bun_http_types::MimeType::TEXT)
        }
        None => return,
    };
...
pub enum Action {
    ...
    GetArrayBuffer,
    GetBytes,
    /// The owner's `Content-Type` value when `blob()` was called ([`content_type_from_headers`]).
...
pub(crate) trait BodyMixin: BodyOwnerJs + Sized {
    ...
    /// The `Content-Type` value that types a `blob()` read ([`content_type_from_headers`]).
        ...
        if matches!(value, Value::Locked(_)) {
            if let Some(readable) = self.get_body_readable_stream() {
                ...
                if let Value::Locked(locked) = value {
                    return locked.set_promise(
                        global_object,
                        Action::GetBlob(self.get_blob_content_type()),
                        Some(readable),
                    );
                }
            }
