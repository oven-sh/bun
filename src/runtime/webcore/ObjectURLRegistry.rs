...
// PORT NOTE: reshaped for borrowck — instead of separate `lock` and `map`
// fields with manual lock()/unlock() around every access, the map is wrapped
// in a `Guarded` (mutex + value).
//
...
impl Drop for Entry {
    fn drop(&mut self) {
        ...
        self.blob.deinit();
        // `Box<Entry>` drop frees the entry itself.
    }
}
...
impl ObjectURLRegistry {
    ...
    pub fn revoke(&self, pathname: &[u8]) {
        ...
        // Box<Entry> dropped here releases the held blob.
...
pub fn bun_revoke_object_url(
    ...
    // `defer slice.deinit()` → UTF8Slice Drop
...
pub fn js_function_resolve_object_url(
    ...
    // path (exception, non-blob prefix, success) releases it on scope exit.
