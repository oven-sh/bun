...
use super::{ReadableStream, Request};
...
// Re-exported so that `crate::webcore::response::FetchHeaders` keeps resolving.
...
impl Response {
    ...
    #[inline]
    pub fn set_init_headers(&self, headers: Option<FetchHeaders>) {
        // old headers dropped (FetchHeaders::Drop derefs the C++ handle)
    ...
    /// Shared accessor over the C++ handle; mirrored by `Request::headers`.
    ...
    #[inline]
    pub fn swap_init_headers(&self) -> Option<FetchHeaders> {
...
impl Response {
    ...
    pub fn header(&self, name: HTTPHeaderName) -> Option<ZigString> {
        // `fast_get` writes through an out-param, so we return the owned
        // `ZigString` instead of a borrowed slice. Callers do `.slice()`.
        self.headers()?.fast_get(name)
    }
    ...
    ) -> JsResult<&FetchHeaders> {
        if self.init.get().headers.is_none() {
            self.init
                .with_mut(|init| init.headers = Some(FetchHeaders::create_empty()));
...
impl Response {
    ...
    pub fn clone_value(&self, global_this: &JSGlobalObject) -> JsResult<Response> {
        ...
        // Init's drop glue (FetchHeaders + OwnedString)
    ...
    fn destroy(this: *mut Response) {
        ...
        unsafe {
            ...
            //
            // - `Init` field drop glue releases `headers` (FetchHeaders::Drop →
    ...
    pub fn construct_json(
        ...
        // `Init`'s field drop glue (FetchHeaders + OwnedString)
...
impl Init {
    pub fn clone(&self, ctx: &JSGlobalObject) -> JsResult<Init> {
        let headers = match &self.headers {
            ...
            // `Option<FetchHeaders>` so a null clone leaves `headers` empty.
    ...
    pub fn init(global_this: &JSGlobalObject, response_init: JSValue) -> JsResult<Option<Init>> {
        ...
        // Init's drop glue on `result` (FetchHeaders + OwnedString)
        // handles cleanup on `?` below
        ...
        if let Some(headers) = response_init.fast_get(global_this, BuiltinName::headers)? {
            ...
            if let Some(orig) = FetchHeaders::cast(headers) {
                // Borrowed from the JS `Headers` wrapper (+0); `clone_this` is the +1.
                if !orig.is_empty() {
                    result.headers = orig.clone_this(global_this)?;
                }
            } else {
