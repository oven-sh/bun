...
impl HeadersRef {
    ...
    /// `Headers` object is deep-copied, anything else goes through
    /// `createFromJS`. Empty headers yield `None` on both paths.
    pub(crate) fn from_init_value(
        global: &JSGlobalObject,
        value: JSValue,
    ) -> JsResult<Option<Self>> {
        if let Some(existing) = FetchHeaders::cast(value) {
            // `existing` is a live `WebCore::FetchHeaders*` borrowed from the JS
            // wrapper; `FetchHeaders` is an opaque ZST FFI handle (S008) — safe deref.
            let existing = bun_opaque::opaque_deref_mut(existing.as_ptr());
