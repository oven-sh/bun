...
/// Shared prologue for the S3 static host fns: parse the first argument as a
...
/// Resolve a parsed argument to an S3 blob: reject file-descriptor paths and
/// construct the internal S3 store for path arguments. Returns the blob along
/// with the eaten options argument.
fn resolve_s3_blob(
    global: &JSGlobalObject,
    args: &mut bun_jsc::call_frame::ArgumentsSlice,
...
pub fn write(global: &JSGlobalObject, callframe: &CallFrame) -> JsResult<JSValue> {
    ...
    let (blob, options) = resolve_s3_blob(global, &mut args, path_or_blob, error_message)?;
    // `write_file_internal` takes `&mut PathOrBlob`; wrap the resolved blob.
...
/// Shared construction epilogue: copy the parsed S3 options onto the store,
/// wrap it in a `Blob`, and apply any `options.type` content-type override.
fn finish_s3_blob(
    global: &JSGlobalObject,
