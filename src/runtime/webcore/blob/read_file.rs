...
/// `(ctx, run, cancel)` a `ReadFile` job carries as its JS side: the
...
/// What a `ReadFile` does with the bytes (or the lack of them): `run` on completion,
...
// the JS thread, as a Job's `Js` side.
...
pub struct ReadFile {
    ...
    /// Not a regular file: its reported size cannot be trusted. On POSIX a read
    /// may also have to wait for readiness on the io thread; a Windows file
    /// handle has no readiness, so there the pool thread's read blocks.
...
impl ReadFile {
    ...
    pub(crate) fn create(
        ...
        let read_file = ReadFile {
            ...
            io_request: io::Request {
                ...
                #[cfg(windows)]
                callback: <Self as FileCloser>::schedule_close,
                scheduled: false,
            },
            #[cfg(not(windows))]
    ...
    pub(crate) fn do_read(
        ...
        // A Windows read never asks to wait for readiness.
        #[cfg(windows)]
        let _ = &retry;
