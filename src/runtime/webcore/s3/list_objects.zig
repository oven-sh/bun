pub const S3ListObjectsOptions = struct {
    ...
    _continuation_token: ?jsc.RustString.Slice,
    _delimiter: ?jsc.RustString.Slice,
    _encoding_type: ?jsc.RustString.Slice,
    _prefix: ?jsc.RustString.Slice,
    _start_after: ?jsc.RustString.Slice,
...
pub const S3ListObjectsV2Result = struct {
    ...
    pub fn toJS(this: *const @This(), globalObject: *JSGlobalObject) bun.JSError!JSValue {
        ...
        if (this.name) |name| {
            jsResult.put(globalObject, jsc.RustString.static("name"), try bun.String.createUTF8ForJS(globalObject, name));
        }

        if (this.prefix) |prefix| {
            jsResult.put(globalObject, jsc.RustString.static("prefix"), try bun.String.createUTF8ForJS(globalObject, prefix));
        }

        if (this.delimiter) |delimiter| {
            jsResult.put(globalObject, jsc.RustString.static("delimiter"), try bun.String.createUTF8ForJS(globalObject, delimiter));
        }

        if (this.start_after) |start_after| {
            jsResult.put(globalObject, jsc.RustString.static("startAfter"), try bun.String.createUTF8ForJS(globalObject, start_after));
        }
        if (this.encoding_type) |encoding_type| {
            jsResult.put(globalObject, jsc.RustString.static("encodingType"), try bun.String.createUTF8ForJS(globalObject, encoding_type));
        }

        if (this.continuation_token) |continuation_token| {
            jsResult.put(globalObject, jsc.RustString.static("continuationToken"), try bun.String.createUTF8ForJS(globalObject, continuation_token));
        }

        if (this.next_continuation_token) |next_continuation_token| {
            jsResult.put(globalObject, jsc.RustString.static("nextContinuationToken"), try bun.String.createUTF8ForJS(globalObject, next_continuation_token));
        }

        if (this.is_truncated) |is_truncated| {
            jsResult.put(globalObject, jsc.RustString.static("isTruncated"), JSValue.jsBoolean(is_truncated));
        }

        if (this.key_count) |key_count| {
            jsResult.put(globalObject, jsc.RustString.static("keyCount"), JSValue.jsNumber(key_count));
        }

        if (this.max_keys) |max_keys| {
            jsResult.put(globalObject, jsc.RustString.static("maxKeys"), JSValue.jsNumber(max_keys));
        }
        ...
        if (this.contents) |contents| {
            ...
            for (contents.items, 0..) |item, i| {
                ...
                objectInfo.put(globalObject, jsc.RustString.static("key"), try bun.String.createUTF8ForJS(globalObject, item.key));

                if (item.etag) |etag| {
                    objectInfo.put(globalObject, jsc.RustString.static("eTag"), try bun.String.createUTF8ForJS(globalObject, etag.get()));
                }

                if (item.checksum_algorithme) |checksum_algorithme| {
                    objectInfo.put(globalObject, jsc.RustString.static("checksumAlgorithme"), try bun.String.createUTF8ForJS(globalObject, checksum_algorithme));
                }

                if (item.checksum_type) |checksum_type| {
                    objectInfo.put(globalObject, jsc.RustString.static("checksumType"), try bun.String.createUTF8ForJS(globalObject, checksum_type));
                }

                if (item.last_modified) |last_modified| {
                    objectInfo.put(globalObject, jsc.RustString.static("lastModified"), try bun.String.createUTF8ForJS(globalObject, last_modified));
                }

                if (item.object_size) |object_size| {
                    objectInfo.put(globalObject, jsc.RustString.static("size"), JSValue.jsNumber(object_size));
                }

                if (item.storage_class) |storage_class| {
                    objectInfo.put(globalObject, jsc.RustString.static("storageClass"), try bun.String.createUTF8ForJS(globalObject, storage_class));
                }

                if (item.owner) |owner| {
                    const jsOwner = JSValue.createEmptyObject(globalObject, 0);
                    if (owner.id) |id| {
                        jsOwner.put(globalObject, jsc.RustString.static("id"), try bun.String.createUTF8ForJS(globalObject, id));
                    }

                    if (owner.display_name) |display_name| {
                        jsOwner.put(globalObject, jsc.RustString.static("displayName"), try bun.String.createUTF8ForJS(globalObject, display_name));
                    }

                    objectInfo.put(globalObject, jsc.RustString.static("owner"), jsOwner);
                }

                try jsContents.putIndex(globalObject, @intCast(i), objectInfo);
            }

            jsResult.put(globalObject, jsc.RustString.static("contents"), jsContents);
        }
        ...
        if (this.common_prefixes) |common_prefixes| {
            ...
            for (common_prefixes.items, 0..) |prefix, i| {
                ...
                jsPrefix.put(globalObject, jsc.RustString.static("prefix"), try bun.String.createUTF8ForJS(globalObject, prefix));
                try jsCommonPrefixes.putIndex(globalObject, @intCast(i), jsPrefix);
            }

            jsResult.put(globalObject, jsc.RustString.static("commonPrefixes"), jsCommonPrefixes);
        }
