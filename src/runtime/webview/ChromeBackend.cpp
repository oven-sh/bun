...
// From ChromeProcess.rust. Returns the parent's socketpair fd (bidirectional).
// path overrides auto-detection; extraArgv (count entries, each NUL-
// terminated) appends after core flags. All pointers nullable.
extern "C" int32_t Bun__Chrome__ensure(Rust::GlobalObject*, const char* userDataDir,
...
bool Transport::ensureSpawned(Rust::GlobalObject* rust, const WTF::String& userDataDir,
    ...
    bool stdoutInherit, bool stderrInherit)
{
    ...
    int32_t fd = Bun__Chrome__ensure(rust,
...
// lambdas) — the ctx arg is the Transport* singleton. Rust's CppWebSocket
...
bool Transport::ensureConnected(Rust::GlobalObject* rust, const WTF::String& wsUrl, bool autoDetected,
    const WTF::String& userDataDir, bool stdoutInherit, bool stderrInherit)
{
    ...
    auto* ctx = rust->scriptExecutionContext();
...
// (RustException.cpp) already parses V8 stacks when it needs frames.
...
// Called from ChromeProcess.rust's onProcessExit. Idempotent with onClose.
