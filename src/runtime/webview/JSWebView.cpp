...
// goes out of native scope after runTestCallback returns). This IS the root.
// ---------------------------------------------------------------------------
...
{
    auto* globalObject = defaultGlobalObject(g);
    auto& c = WK::client();
    if (!c.ensureSpawned(globalObject, stdoutInherit, stderrInherit)) return nullptr;

    auto impl = WebViewEventTarget::create(*globalObject->scriptExecutionContext());
    JSWebView* view = create(structure, globalObject, WTF::move(impl));
    ...
    bool stdoutInherit, bool stderrInherit, const WTF::String& wsUrl, bool skipAutoDetect)
{
    ...
    bool ok;
    if (!wsUrl.isEmpty()) {
        ok = t.ensureConnected(globalObject, wsUrl, /* autoDetected */ false);
    } else if (skipAutoDetect || !path.isEmpty() || !extraArgv.isEmpty()) {
        ok = t.ensureSpawned(globalObject, userDataDir, path, extraArgv, stdoutInherit, stderrInherit);
    } else {
        ...
        if (len > 0) {
            ok = t.ensureConnected(globalObject,
