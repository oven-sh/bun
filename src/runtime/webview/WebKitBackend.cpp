...
extern "C" int32_t Bun__WebViewHost__ensure(Rust::GlobalObject*, bool stdoutInherit, bool stderrInherit);
extern "C" void* Blob__fromMmapWithType(JSC::JSGlobalObject*, uint8_t* ptr, size_t len, const char* mime);
extern "C" JSC::EncodedJSValue SYSV_ABI Blob__create(Rust::GlobalObject*, void* impl);
extern "C" JSC::EncodedJSValue JSBuffer__fromMmap(Rust::GlobalObject*, void* ptr, size_t length);
...
extern "C" void Bun__EventLoop__enter(Rust::GlobalObject*);
extern "C" void Bun__EventLoop__exit(Rust::GlobalObject*);
...
bool HostClient::ensureSpawned(Rust::GlobalObject* rust, bool stdoutInherit, bool stderrInherit)
{
    ...
    int fd = Bun__WebViewHost__ensure(rust, stdoutInherit, stderrInherit);
    if (fd < 0) {
        dead = true;
        return false;
    }
...
// because the Rust allocator wrapper poisons with @memset(undefined) in
...
// Called from Rust's onProcessExit (EVFILT_PROC). The socket onClose may or
...
// HostProcess.rust references this unconditionally via @extern; Rust's dead-code
// elimination doesn't trigger because the TaggedPointer dispatch switch in
// process.rust pulls in all ProcessExitHandler arms. spawn() itself is gated
