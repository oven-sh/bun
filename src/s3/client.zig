...
pub fn downloadSlice(
    ...
    const range = brk: {
        if (size) |size_| {
            ...
            break :brk std.fmt.allocPrint(bun.default_allocator, "bytes={}-{}", .{ offset, end }) catch |oe| bun.outOfMemory(oe);
        }
        if (offset == 0) break :brk null;
        break :brk std.fmt.allocPrint(bun.default_allocator, "bytes={}-", .{offset}) catch |oe| bun.outOfMemory(oe);
    };
...
pub fn listObjects(
    ...
    search_params.append(bun.default_allocator, "?") catch |oe| bun.outOfMemory(oe);

    if (listOptions.continuation_token) |continuation_token| {
        var buff: [1024]u8 = undefined;
        const encoded = S3Credentials.encodeURIComponent(continuation_token, &buff, true) catch |oe| bun.outOfMemory(oe);
        search_params.appendFmt(bun.default_allocator, "continuation-token={s}", .{encoded}) catch |oe| bun.outOfMemory(oe);
    }

    if (listOptions.delimiter) |delimiter| {
        var buff: [1024]u8 = undefined;
        const encoded = S3Credentials.encodeURIComponent(delimiter, &buff, true) catch |oe| bun.outOfMemory(oe);

        if (listOptions.continuation_token != null) {
            search_params.appendFmt(bun.default_allocator, "&delimiter={s}", .{encoded}) catch |oe| bun.outOfMemory(oe);
        } else {
            search_params.appendFmt(bun.default_allocator, "delimiter={s}", .{encoded}) catch |oe| bun.outOfMemory(oe);
        }
    }
    ...
    if (listOptions.encoding_type != null) {
        if (listOptions.continuation_token != null or listOptions.delimiter != null) {
            search_params.append(bun.default_allocator, "&encoding-type=url") catch |oe| bun.outOfMemory(oe);
        } else {
            search_params.append(bun.default_allocator, "encoding-type=url") catch |oe| bun.outOfMemory(oe);
        }
    }
    ...
    if (listOptions.fetch_owner) |fetch_owner| {
        if (listOptions.continuation_token != null or listOptions.delimiter != null or listOptions.encoding_type != null) {
            search_params.appendFmt(bun.default_allocator, "&fetch-owner={}", .{fetch_owner}) catch |oe| bun.outOfMemory(oe);
        } else {
            search_params.appendFmt(bun.default_allocator, "fetch-owner={}", .{fetch_owner}) catch |oe| bun.outOfMemory(oe);
        }
    }
    ...
    if (listOptions.continuation_token != null or listOptions.delimiter != null or listOptions.encoding_type != null or listOptions.fetch_owner != null) {
        search_params.append(bun.default_allocator, "&list-type=2") catch |oe| bun.outOfMemory(oe);
    } else {
        search_params.append(bun.default_allocator, "list-type=2") catch |oe| bun.outOfMemory(oe);
    }

    if (listOptions.max_keys) |max_keys| {
        search_params.appendFmt(bun.default_allocator, "&max-keys={}", .{max_keys}) catch |oe| bun.outOfMemory(oe);
    }

    if (listOptions.prefix) |prefix| {
        var buff: [1024]u8 = undefined;
        const encoded = S3Credentials.encodeURIComponent(prefix, &buff, true) catch |oe| bun.outOfMemory(oe);
        search_params.appendFmt(bun.default_allocator, "&prefix={s}", .{encoded}) catch |oe| bun.outOfMemory(oe);
    }

    if (listOptions.start_after) |start_after| {
        var buff: [1024]u8 = undefined;
        const encoded = S3Credentials.encodeURIComponent(start_after, &buff, true) catch |oe| bun.outOfMemory(oe);
        search_params.appendFmt(bun.default_allocator, "&start-after={s}", .{encoded}) catch |oe| bun.outOfMemory(oe);
    }
    ...
    const headers = bun.http.Headers.fromPicoHttpHeaders(result.headers(), bun.default_allocator) catch |oe| bun.outOfMemory(oe);
...
pub fn writableStream(
    ...
    const task = bun.new(MultiPartUpload, .{
        ...
        .proxy = if (proxy_url.len > 0) bun.default_allocator.dupe(u8, proxy_url) catch |oe| bun.outOfMemory(oe) else "",
        .content_type = if (content_type) |ct| bun.default_allocator.dupe(u8, ct) catch |oe| bun.outOfMemory(oe) else null,
        .storage_class = storage_class,
...
pub const S3UploadStreamWrapper = struct {
    ...
    pub fn writeRequestData(this: *@This(), data: []const u8) bool {
        ...
        return this.task.writeBytes(data, false) catch |oe| bun.outOfMemory(oe);
    }
    ...
    pub fn writeEndRequest(this: *@This(), err: ?jsc.JSValue) void {
        ...
        if (err) |js_err| {
            ...
            _ = this.task.writeBytes("", true) catch |oe| bun.outOfMemory(oe);
        }
    }
...
pub fn downloadStream(
    ...
    const headers = brk: {
        if (range) |range_| {
            ...
            break :brk bun.http.Headers.fromPicoHttpHeaders(_headers, bun.default_allocator) catch |oe| bun.outOfMemory(oe);
        } else {
            break :brk bun.http.Headers.fromPicoHttpHeaders(result.headers(), bun.default_allocator) catch |oe| bun.outOfMemory(oe);
        }
    };
    const proxy = proxy_url orelse "";
    const owned_proxy = if (proxy.len > 0) bun.default_allocator.dupe(u8, proxy) catch |oe| bun.outOfMemory(oe) else "";
