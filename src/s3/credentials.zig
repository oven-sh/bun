pub const S3Credentials = struct {
    ...
    pub fn dupe(this: *const @This()) *S3Credentials {
        return bun.new(S3Credentials, .{
            ...
            .accessKeyId = if (this.accessKeyId.len > 0)
                bun.default_allocator.dupe(u8, this.accessKeyId) catch |oe| bun.outOfMemory(oe)
            else
                "",

            .secretAccessKey = if (this.secretAccessKey.len > 0)
                bun.default_allocator.dupe(u8, this.secretAccessKey) catch |oe| bun.outOfMemory(oe)
            else
                "",

            .region = if (this.region.len > 0)
                bun.default_allocator.dupe(u8, this.region) catch |oe| bun.outOfMemory(oe)
            else
                "",

            .endpoint = if (this.endpoint.len > 0)
                bun.default_allocator.dupe(u8, this.endpoint) catch |oe| bun.outOfMemory(oe)
            else
                "",

            .bucket = if (this.bucket.len > 0)
                bun.default_allocator.dupe(u8, this.bucket) catch |oe| bun.outOfMemory(oe)
            else
                "",

            .sessionToken = if (this.sessionToken.len > 0)
                bun.default_allocator.dupe(u8, this.sessionToken) catch |oe| bun.outOfMemory(oe)
            else
                "",
    ...
    pub fn signRequest(this: *const @This(), signOptions: SignOptions, comptime allow_empty_path: bool, signQueryOption: ?SignQueryOptions) !SignResult {
        ...
        if (content_md5) |content_md5_val| {
            ...
            const content_md5_as_base64 = bun.default_allocator.alloc(u8, len) catch |oe| bun.outOfMemory(oe);
        ...
        if (session_token) |token| {
            const session_token_value = bun.default_allocator.dupe(u8, token) catch |oe| bun.outOfMemory(oe);
        ...
        if (content_disposition) |cd| {
            const content_disposition_value = bun.default_allocator.dupe(u8, cd) catch |oe| bun.outOfMemory(oe);
        ...
        if (content_md5) |c_md5| {
            const content_md5_value = bun.default_allocator.dupe(u8, c_md5) catch |oe| bun.outOfMemory(oe);
