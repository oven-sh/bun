...
pub const MultiPartUpload = struct {
            ...
            switch (result) {
                ...
                .etag => |etag| {
                    ...
                    this.ctx.multipart_etags.append(bun.default_allocator, .{
                        ...
                        .etag = bun.default_allocator.dupe(u8, etag) catch |oe| bun.outOfMemory(oe),
    ...
    fn getCreatePart(this: *@This(), chunk: []const u8, allocated_size: usize, needs_clone: bool) ?*UploadPart {
        ...
        if (this.queue == null) {
            ...
            const queue = bun.default_allocator.alloc(UploadPart, queueSize) catch |oe| bun.outOfMemory(oe);
        ...
        const data = if (needs_clone) bun.default_allocator.dupe(u8, chunk) catch |oe| bun.outOfMemory(oe) else chunk;
    ...
    fn done(this: *@This()) void {
        if (this.state == .multipart_completed) {
            ...
            this.multipart_upload_list.append(bun.default_allocator, "<?xml version=\"1.0\" encoding=\"UTF-8\"?><CompleteMultipartUpload xmlns=\"http://s3.amazonaws.com/doc/2006-03-01/\">") catch |oe| bun.outOfMemory(oe);
            for (this.multipart_etags.items) |tag| {
                this.multipart_upload_list.appendFmt(bun.default_allocator, "<Part><PartNumber>{}</PartNumber><ETag>{s}</ETag></Part>", .{ tag.number, tag.etag }) catch |oe| bun.outOfMemory(oe);

                bun.default_allocator.free(tag.etag);
            }
            this.multipart_etags.deinit(bun.default_allocator);
            this.multipart_etags = .{};
            this.multipart_upload_list.append(bun.default_allocator, "</CompleteMultipartUpload>") catch |oe| bun.outOfMemory(oe);
