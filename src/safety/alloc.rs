...
//   confidence: medium
//   todos:      0
//   notes:      Reworked from `&dyn Allocator` fat-pointer transmutes to `StdAllocator` (the literal {ptr, vtable} port) so .ptr/.vtable compare exactly as in Zig. Higher-tier `is_instance` checks (arena/allocation_scope/LinuxMemFd/MaxHeap/MimallocArena/CachedBytecode/bundle_v2/heap_breakdown) push their vtable addresses into the `KNOWN_ALLOC_VTABLES` registry at init (data moved down; no fn-ptr hook). `MimallocArena::is_instance` for `transfer_ownership` is a direct call into `bun_alloc` (below us). `is_wtf_allocator`/c_allocator/z_allocator vtable checks are direct (already moved down to bun_alloc).
// ──────────────────────────────────────────────────────────────────────────
