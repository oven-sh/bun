/// A value that does not alias any other thread ID.
/// See `Thread/Mutex/Recursive.zig` in the Zig standard library.
// TODO(b0): `ThreadId` arrives from move-in (CYCLEBREAK MOVE_DOWN bun_threading::ThreadId → safety).
// Placeholder numeric alias until the move-in pass lands the real definition here.
// TODO(port): `std.Thread.Id` is a platform-specific unsigned integer in Zig; Rust's
// `std::thread::ThreadId` is opaque and has no MAX. Phase B must pick the concrete width.
pub type ThreadId = u64;
pub const INVALID: ThreadId = ThreadId::MAX;

// ──────────────────────────────────────────────────────────────────────────
// PORT STATUS
//   source:     src/safety/thread_id.zig (5 lines)
//   confidence: medium
//   todos:      1
//   notes:      needs a numeric ThreadId type with MAX; std::thread::ThreadId is opaque
// ──────────────────────────────────────────────────────────────────────────
