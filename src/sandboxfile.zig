...
/// Status of a running process in the sandbox
...
pub const RunningProcess = struct {
    name: []const u8,
    command: []const u8,
    ...
    pub fn init(allocator: Allocator, name: []const u8, command: []const u8) RunningProcess {
        return .{
            .name = name,
            .command = command,
            .status = .pending,
    ...
    pub fn deinit(self: *RunningProcess) void {
        self.stdout_buf.deinit(self.allocator);
        self.stderr_buf.deinit(self.allocator);
    }
};

/// Environment for running sandbox processes
pub const SandboxEnv = struct {
    /// Environment variables (with secrets masked for logging)
    env_map: std.process.EnvMap,
    /// Secret variable names (values are passed but masked in logs)
    ...
    pub fn init(allocator: Allocator) SandboxEnv {
        return .{
    ...
    pub fn deinit(self: *SandboxEnv) void {
        self.env_map.deinit();
    }

    /// Check if a variable name is a secret
    pub fn isSecret(self: *const SandboxEnv, name: []const u8) bool {
        for (self.secret_names) |secret| {
            if (std.mem.eql(u8, secret, name)) return true;
        }
        return false;
    }

    /// Get masked value for logging (masks secrets)
    pub fn getMaskedValue(self: *const SandboxEnv, name: []const u8) []const u8 {
        if (self.isSecret(name)) {
            return "********";
        }
        return self.env_map.get(name) orelse "";
    }

    /// Check if a host is allowed for network access
    pub fn isHostAllowed(self: *const SandboxEnv, host: []const u8) bool {
        // If no hosts specified, allow all (for backwards compatibility)
        if (self.allowed_hosts.len == 0) return true;

        for (self.allowed_hosts) |pattern| {
            if (matchHostPattern(pattern, host)) return true;
        }
        return false;
    }

    /// Match a host against a pattern (supports * wildcard prefix)
    fn matchHostPattern(pattern: []const u8, host: []const u8) bool {
        if (std.mem.startsWith(u8, pattern, "*.")) {
            ...
            return std.mem.endsWith(u8, host, suffix) or
                std.mem.eql(u8, host, pattern[2..]); // exact match without *.
        }
...
pub const SandboxResult = struct {
    /// Whether all RUN commands succeeded
    setup_success: bool,
    /// Whether all TEST commands succeeded
    tests_success: bool,
    /// Exit code of the DEV server (if it exited)
    dev_exit_code: ?u8,
    /// Output files that were modified
    modified_outputs: ArrayList([]const u8),
    /// Errors encountered
    errors: ArrayList([]const u8),

    pub fn init() SandboxResult {
        return .{
            .setup_success = true,
            .tests_success = true,
            .dev_exit_code = null,
    ...
    pub fn deinit(self: *SandboxResult, allocator: Allocator) void {
        self.modified_outputs.deinit(allocator);
        self.errors.deinit(allocator);
    }
};

/// Sandbox runner - executes a Sandboxfile
pub const Runner = struct {
    allocator: Allocator,
    config: Sandboxfile,
    env: SandboxEnv,
    services: ArrayList(RunningProcess),
    dev_process: ?RunningProcess,
    result: SandboxResult,

    const Output = bun.Output;

    pub fn init(allocator: Allocator, config: Sandboxfile) Runner {
        var env = SandboxEnv.init(allocator);
        env.secret_names = config.secrets.items;
        env.allowed_hosts = config.allowed_hosts.items;
    ...
    /// Run the sandbox - executes all directives
    pub fn run(self: *Runner) !SandboxResult {
        Output.prettyErrorln("<cyan>sandbox<r>: Starting sandbox...", .{});
        Output.flush();

        // Step 1: Set up environment
        try self.setupEnvironment();

        // Step 2: Run setup commands
        try self.runSetupCommands();
        if (!self.result.setup_success) {
            return self.result;
        }
    ...
    fn setupEnvironment(self: *Runner) !void {
        Output.prettyErrorln("<cyan>sandbox<r>: Setting up environment...", .{});
        Output.flush();

        // Inherit current environment
        var parent_env = try std.process.getEnvMap(self.allocator);
        defer parent_env.deinit();
        var env_iter = parent_env.iterator();
        while (env_iter.next()) |entry| {
            try self.env.env_map.put(entry.key_ptr.*, entry.value_ptr.*);
        }

        // Add sandbox-specific environment variables
        try self.env.env_map.put("BUN_SANDBOX", "1");
        try self.env.env_map.put("BUN_SANDBOX_WORKDIR", self.config.workdir);
    }

    /// Run all RUN commands sequentially
    fn runSetupCommands(self: *Runner) !void {
        for (self.config.run_commands.items) |run_cmd| {
            Output.prettyErrorln("<cyan>sandbox<r>: RUN <b>{s}<r>", .{run_cmd.command});
            Output.flush();

            const exit_code = try self.executeCommand(run_cmd.command);
            if (exit_code != 0) {
                self.result.setup_success = false;
                try self.result.errors.append(
                    self.allocator,
                    try std.fmt.allocPrint(self.allocator, "RUN command failed: {s} (exit code {d})", .{ run_cmd.command, exit_code }),
                );
                return;
            }
        }
    ...
    fn startServices(self: *Runner) !void {
        for (self.config.services.items) |service| {
            Output.prettyErrorln("<cyan>sandbox<r>: Starting service <b>{s}<r>: {s}", .{ service.name, service.command });
            Output.flush();

            var proc = RunningProcess.init(self.allocator, service.name, service.command);
            proc.pid = try self.spawnBackgroundProcess(service.command);
            proc.status = .running;
            try self.services.append(self.allocator, proc);
        }
    }

    /// Start the DEV server
    fn startDevServer(self: *Runner) !void {
        if (self.config.dev) |dev| {
            Output.prettyErrorln("<cyan>sandbox<r>: Starting dev server: <b>{s}<r>", .{dev.command});
            if (dev.port) |port| {
                Output.prettyErrorln("<cyan>sandbox<r>:   PORT={d}", .{port});
            }
            if (dev.watch) |watch| {
                Output.prettyErrorln("<cyan>sandbox<r>:   WATCH={s}", .{watch});
            }
            Output.flush();

            var proc = RunningProcess.init(self.allocator, "dev", dev.command);
            proc.pid = try self.spawnBackgroundProcess(dev.command);
            proc.status = .running;
    ...
    pub fn runTests(self: *Runner) !void {
        for (self.config.tests.items) |test_cmd| {
            Output.prettyErrorln("<cyan>sandbox<r>: TEST <b>{s}<r>", .{test_cmd.command});
            Output.flush();

            const exit_code = try self.executeCommand(test_cmd.command);
            if (exit_code != 0) {
                self.result.tests_success = false;
                try self.result.errors.append(
                    self.allocator,
                    try std.fmt.allocPrint(self.allocator, "TEST command failed: {s} (exit code {d})", .{ test_cmd.command, exit_code }),
                );
            }
        }
    }

    /// Execute a command synchronously and return exit code
    fn executeCommand(self: *Runner, command: []const u8) !u8 {
        const result = try std.process.Child.run(.{
            .allocator = self.allocator,
            .argv = &.{ "/bin/sh", "-c", command },
            .cwd = self.config.workdir,
            .env_map = &self.env.env_map,
        });
    ...
    fn spawnBackgroundProcess(self: *Runner, command: []const u8) !std.posix.pid_t {
        var child = std.process.Child.init(
            &.{ "/bin/sh", "-c", command },
            self.allocator,
        );
        child.cwd = self.config.workdir;
        child.env_map = &self.env.env_map;
    ...
    pub fn stop(self: *Runner) void {
        Output.prettyErrorln("<cyan>sandbox<r>: Stopping sandbox...", .{});
        Output.flush();

        // Stop dev server
        if (self.dev_process) |*dev| {
            if (dev.pid) |pid| {
                Output.prettyErrorln("<cyan>sandbox<r>: Stopping dev server (PID {d})", .{pid});
        ...
        var i = self.services.items.len;
        while (i > 0) {
            i -= 1;
            var svc = &self.services.items[i];
            if (svc.pid) |pid| {
                Output.prettyErrorln("<cyan>sandbox<r>: Stopping service <b>{s}<r> (PID {d})", .{ svc.name, pid });
                _ = std.posix.kill(pid, std.posix.SIG.TERM) catch {};
                svc.status = .killed;
            }
        }
    ...
    pub fn getStatus(self: *const Runner) void {
        Output.prettyErrorln("<cyan>sandbox<r>: Status:", .{});

        if (self.dev_process) |dev| {
            const status_str = switch (dev.status) {
                .pending => "<yellow>pending<r>",
                .running => "<green>running<r>",
                .succeeded => "<green>succeeded<r>",
                .failed => "<red>failed<r>",
                .killed => "<yellow>killed<r>",
            };
            Output.prettyErrorln("  dev: {s} (PID {?d})", .{ status_str, dev.pid });
        }

        for (self.services.items) |svc| {
            const status_str = switch (svc.status) {
                .pending => "<yellow>pending<r>",
                .running => "<green>running<r>",
                .succeeded => "<green>succeeded<r>",
                .failed => "<red>failed<r>",
            ...
            Output.prettyErrorln("  {s}: {s} (PID {?d})", .{ svc.name, status_str, svc.pid });
        }
        Output.flush();
    }
};

/// Run a sandbox from a Sandboxfile path
pub fn runFromFile(allocator: Allocator, path: []const u8) !SandboxResult {
    var config = try parseFile(allocator, path);
    defer config.deinit(allocator);

    var runner = Runner.init(allocator, config);
    defer runner.deinit();
