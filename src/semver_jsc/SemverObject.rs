...
pub fn create(global: &JSGlobalObject) -> JSValue {
    ...
    let _ = global;
    todo!("SemverObject::create — gated on bun_jsc::JSValue::{{create_empty_object,put}} / JSFunction::create")
}

// TODO(b2-blocked): bun_jsc::host_fn — `#[bun_jsc::host_fn]` proc-macro not yet
// exported from the stub `bun_jsc` crate; re-attach once available.
pub fn order(global: &JSGlobalObject, frame: &CallFrame) -> JsResult<JSValue> {
    ...
    let _ = (global, frame);
    todo!("SemverObject::order — gated on bun_jsc stub methods (CallFrame/JSValue/JSGlobalObject)")
}
...
pub fn satisfies(global: &JSGlobalObject, frame: &CallFrame) -> JsResult<JSValue> {
    #[cfg(any())]
    {
        ...
        let left_result = Version::parse(SlicedString::init(left.slice(), left.slice()));
        if left_result.wildcard != query::Wildcard::None {
            return Ok(JSValue::FALSE);
        }
    ...
    let _ = (global, frame);
    todo!("SemverObject::satisfies — gated on bun_jsc stub methods (CallFrame/JSValue/JSGlobalObject)")
}
...
//   confidence: medium
//   todos:      1
//   notes:      arena/stack-fallback dropped (non-AST); ZigString::static_ used for keyword collision; query::Wildcard path; bodies re-gated on bun_jsc stub-surface gaps
// ──────────────────────────────────────────────────────────────────────────
