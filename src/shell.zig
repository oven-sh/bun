...
pub const Braces = struct {
    src: []const u8,
    alloc: Allocator,
    tokens: ArrayList(Braces.Token),
    i: usize = 0,
    state: State = .Normal,
    ...
    const Token = union(enum) {
        ...
        pub fn toText(self: *Braces.Token) SmolStr {
            return switch (self.*) {
    ...
    pub const DebugToken = union(enum) {
        open,
        comma,
        text: []const u8,
        close,

        pub fn fromNormal(allocator: Allocator, token: *const Braces.Token) !DebugToken {
            return switch (token.*) {
                .open => .open,
                .comma => .comma,
                .text => |txt| {
    ...
    pub fn expand(self: *Braces, out: []std.ArrayList(u8)) !void {
        const BraceState = struct {
            str_idx: u32,
            out_key: u16,
        };

        var allocator = std.heap.stackFallback(@sizeOf(BraceState) * 4, self.alloc);

        var brace_stack = std.ArrayList(BraceState).init(allocator);

        const out_len = out.len;
        _ = out_len;

        // u16 because there's probably something wrong if user has more than
        // 65535 variants
        var out_key: u16 = 0;
        ...
        while (i < self.tokens.items.len) {
            switch (self.tokens.items[i]) {
                ...
                .comma => {
                    if (bun.Environment.allow_assert) {
                        std.debug.assert(brace_stack.items.len > 0);
                    }
                    var top = &brace_stack.items[brace_stack.items.len - 1];
                    top.out_key += 1;

                    i += 1;
                },
                .text => |*txt| {
                    try out[out_key].append(txt.slice());
                    i += 1;
                },
                .close => {
                    _ = brace_stack.pop();
    ...
    pub fn calculateExpandedAmount(self: *Braces) !u32 {
    ...
    pub fn tokenize(alloc: Allocator, src: []const u8) !Braces {
        var this = Braces{
            .src = src,
            .tokens = ArrayList(Braces.Token).init(alloc),
            .alloc = alloc,
        };
    ...
    fn tokenize_impl(self: *Braces) !void {
    ...
    fn flattenTokens(self: *Braces) !void {
        var i: u32 = 0;
        var j: u32 = 1;
    ...
    fn rollbackBraces(self: *Braces, starting_idx: u32) !void {
        if (bun.Environment.allow_assert) {
    ...
    fn replaceTokenWithString(self: *Braces, token_idx: u32) !void {
        var tok = &self.tokens.items[token_idx];
        var tok_text = tok.toText();
        tok.* = .{ .text = tok_text };
    }

    fn appendChar(self: *Braces, char: u8) !void {
    ...
    fn read_char(self: *Braces) ?InputChar {
        if (self.i >= self.src.len) return null;
        var char = self.src[self.i];
        if (char != '\\' or self.state == .Single) return .{ .char = char };
