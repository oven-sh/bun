...
pub const ShellTouchTask = struct {
    ...
    pub fn create(touch: *Touch, opts: Opts, filepath: [:0]const u8, cwd_path: [:0]const u8) *ShellTouchTask {
        const task = bun.default_allocator.create(ShellTouchTask) catch |oe| bun.outOfMemory(oe);
        task.* = ShellTouchTask{
            .touch = touch,
            .opts = opts,
            .cwd_path = cwd_path,
        ...
        if (node_fs.utimes(args, .sync).asErr()) |err| out: {
            if (err.getErrno() == .NOENT) {
                ...
                switch (Syscall.open(filepath, bun.O.CREAT | bun.O.WRONLY, perm)) {
                    ...
                    .err => |e| {
                        this.err = e.withPath(bun.default_allocator.dupe(u8, filepath) catch |oe| bun.outOfMemory(oe)).toShellSystemError();
                        break :out;
                    },
                }
            }
            this.err = err.withPath(bun.default_allocator.dupe(u8, filepath) catch |oe| bun.outOfMemory(oe)).toShellSystemError();
        }
