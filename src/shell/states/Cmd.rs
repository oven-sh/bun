...
use bun_shell::ast;
use bun_shell::interpret::{
    log, stderr_no, stdin_no, stdout_no, CowFd, ShellSyscall, StatePtrUnion,
};
use bun_shell::subproc::{self, ShellSubprocess as Subprocess, Stdio};
        ...
        while !matches!(self.state, CmdState::Done) {
            match &mut self.state {
                ...
                CmdState::WaitingWriteErr => {
                    bun_shell::unreachable_state("Cmd.next", "waiting_write_err");
                }
                CmdState::Exec => {
                    bun_shell::unreachable_state("Cmd.next", "exec");
                }
