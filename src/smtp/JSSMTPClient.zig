...
fn resolveSendPromise(this: *JSSMTPClient, response: []const u8) void {
    ...
    if (js.sendPromiseGetCached(this_js)) |pv| {
        ...
        if (pv.asPromise()) |p| {
            ...
            // Build accepted array of email addresses
            const acc = jsc.JSValue.createEmptyArray(go, this.conn.accepted_count) catch .js_undefined;
            for (0..this.conn.accepted_count) |i| {
                const idx = this.conn.accepted_indices[i];
            ...
            // Build rejected array of email addresses
            const rej = jsc.JSValue.createEmptyArray(go, this.conn.rejected_count) catch .js_undefined;
            for (0..this.conn.rejected_count) |i| {
                const idx = this.conn.rejected_indices[i];
                if (idx < this.conn.envelope_to.len) {
                    const addr_str = bun.String.createFormat("{s}", .{this.conn.envelope_to[idx]}) catch bun.String.empty;
