...
/// if the JSON doesn't look like a valid sourcemap. Non-fatal parse
/// failures return `null` — callers fall back to the raw file bytes.
///
...
pub fn parse(json_bytes: []const u8) ?*InputSourceMap {
    ...
    var json = bun.json.parse(&json_src, &log, arena_allocator, false) catch return null;

    if (json.get("version")) |version| {
        if (version.data != .e_number or version.data.e_number.value != 3.0) return null;
    }

    const mappings_str = json.get("mappings") orelse return null;
    if (mappings_str.data != .e_string) return null;

    const sources_paths = switch ((json.get("sources") orelse return null).data) {
        .e_array => |arr| arr,
        else => return null,
    };
    ...
    if (sources_content_opt) |arr| {
        if (arr.items.len != sources_paths.items.len) return null;
    }

    const source_count = sources_paths.items.len;

    // Allocate sources_paths / sources_content slices up-front so we can
    // errdefer their cleanup cleanly.
    var source_paths_slice = allocator.alloc([]const u8, source_count) catch return null;
    ...
    for (sources_paths.items.slice()) |item| {
        if (item.data != .e_string) return null;
        const str = item.data.e_string.string(arena_allocator) catch return null;
        source_paths_slice[paths_written] = allocator.dupe(u8, str) catch return null;
        paths_written += 1;
    }

    var sources_content_slice = allocator.alloc([]const u8, source_count) catch return null;
        ...
        for (arr.items.slice()) |item| {
            if (item.data == .e_string) {
                ...
                sources_content_slice[contents_written] = if (str.len == 0)
                    ...
                    allocator.dupe(u8, str) catch return null;
            } else {
    ...
    // The `ref_count` was zero-initialized by `bun.new`; calling code holds
    // the only reference, which is released via `InputSourceMap.deinit`.
...
fn parseDataUrl(url: []const u8) ?*InputSourceMap {
    ...
    if (is_base64) {
        ...
        var buf = bun.default_allocator.alloc(u8, decoded_len) catch return null;
