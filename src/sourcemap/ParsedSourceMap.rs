...
pub struct ParsedSourceMap {
    ...
    /// The input file's own source map (from a `//# sourceMappingURL=` comment
    /// or sibling `.map`), chained after this map's own mappings. Set for
    /// runtime-transpiled modules whose on-disk input was itself generated by
    /// another tool (tsc, esbuild, swc) so stack traces resolve to the
    /// original source rather than the intermediate `.js`.
...
impl ParsedSourceMap {
    ...
    /// Like [`Self::from_internal`], chaining the transpiler's map through the
    /// input file's own source map so lookups land in the original source.
    ...
    pub fn find_mapping(&self, line: Ordinal, column: Ordinal) -> Option<Mapping> {
        ...
        // Chain through the input's own map: our `original` positions are its
        // `generated` positions. A miss in the input map still reports the
        // intermediate position so the frame is at least remapped to the
        // on-disk `.js`.
        let own = own?;
        input_map
