        ...
        match attempt {
            Err(err) => {
                match err.get_errno() {
                    ...
                    // The spawn fails below, so kill the child before the reap.
                    // A child that waits on a pipe this process still holds
                    // (`cat` with stdin: "pipe") would block wait4 forever.
                    _ => {
