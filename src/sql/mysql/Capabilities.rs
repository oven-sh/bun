...
//
// PORT NOTE: This is a plain struct of `bool` fields (NOT a packed bitfield),
// with explicit bit-value constants and unrolled field iteration for
// `to_int`/`from_int`/`Display`. Field names stay SCREAMING_SNAKE_CASE because
// `Display` emits them verbatim.
...
impl Capabilities {
    ...
    // PORT NOTE: a force-reference block for the constants above was dropped
    // per PORTING.md §Don't translate.
...
impl fmt::Display for Capabilities {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let mut first = true;
        // Emit a comma-separated list of the set flags by field name.
        macro_rules! emit {
            ($field:ident) => {
                if self.$field {
                    if !first {
