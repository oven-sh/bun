...
/// Some callers name this `AnyMySQLError` (the historical file basename); the
/// Rust enum is `Error` per convention. Re-export both spellings so cross-crate
/// `use` lines (`bun_sql::mysql::protocol::any_mysql_error::AnyMySQLError`)
/// resolve.
