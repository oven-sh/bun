...
pub fn createMySQLError(
    ...
    opts_obj.put(globalObject, bun.String.static("code"), try bun.String.createUTF8ForJS(globalObject, options.code));
    if (options.errno) |errno| {
        opts_obj.put(globalObject, bun.String.static("errno"), JSC.JSValue.jsNumber(errno));
    }
    if (options.sqlState) |state| {
        opts_obj.put(globalObject, bun.String.static("sqlState"), try bun.String.createUTF8ForJS(globalObject, state[0..]));
    }
    opts_obj.put(globalObject, bun.String.static("message"), try bun.String.createUTF8ForJS(globalObject, message));
