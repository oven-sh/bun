...
/// Required methods a writer context must implement; `NewWriterWrap` calls
/// them through the trait bound.
...
impl<C: WriterContext> NewWriterWrap<C> {
    ...
    /// `Query` calls `writer.write_null_bitmap(this.params)` on the
    /// `NewWriter` value. The params branch is never instantiated for
    /// COM_QUERY in practice, but Rust must have it to typecheck. Mirrors the
    /// bitmap logic from `PreparedStatement::write_null_bitmap`, keyed on
    ...
    pub fn write_null_bitmap(self, params: &[crate::shared::Data]) -> Result<(), AnyMySQLError> {
        ...
        // PERF(port): a small Vec keeps stack usage bounded for the
        // never-taken path.
...
// TODO(port): ensure no caller double-wraps; if needed, model via a
// `MySQLWriter` trait with a blanket impl for `NewWriterWrap<C>`.
pub type NewWriter<C> = NewWriterWrap<C>;

/// Free helper that wraps a raw context into `NewWriterWrap` before forwarding
/// to `write_fn`. The per-packet `write` impls call this directly.
// TODO(port): callers should pass `impl WriterContext` and let this helper
// wrap unconditionally; already-wrapped values go straight to `write_fn`.
#[inline]
