...
pub struct OKPacket {
    ...
    // TODO(port): wire format is u24 — using u32, callers must ensure value fits in 24 bits
    pub packet_size: u32,
}

// Field defaults: header=0x00, affected_rows=0, last_insert_id=0, status_flags={},
...
// Cleanup only needs `info` and `session_state_changes` released. `Data` owns its
// buffer and has `Drop`, so Rust drops fields automatically — no explicit
// `impl Drop` needed.
...
impl OKPacket {
    ...
    pub fn decode_internal<Context: ReaderContext>(
        ...
        if !reader.peek().is_empty() && (self.packet_size as usize) > read_size {
            ...
            // Intentional truncating cast to reader.read's arg type
