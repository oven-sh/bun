// PORT NOTE: any u8 value is a valid PacketType (the wire protocol is open-ended).
// A Rust `#[repr(u8)] enum` would make unlisted values UB on transmute, so this is
// modeled as a transparent u8 newtype with associated consts instead.
#[repr(transparent)]
