...
// stack (LIFETIMES.tsv has no entry; classified BORROW_PARAM). Modeled as
// `&'a Cell<usize>` to keep `Copy` + interior mutability without `unsafe`.
#[derive(Clone, Copy)]
...
impl<'a> StackReader<'a> {
    ...
    pub fn init(
        ...
        // TODO(port): NewReader field name assumed `wrapped`
        NewReader {
            wrapped: StackReader {
                buffer,
                offset,
