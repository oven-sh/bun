// Variant names intentionally use SCREAMING_CASE where the wire-level error
// codes do, so that `IntoStaticStr` yields the exact strings that JS
// `error.code` / snapshot tests depend on.
