...
// unlisted byte into it is UB — so this is a transparent `u8` newtype with
// associated consts for the known field codes.

#[repr(transparent)]
#[derive(Copy, Clone, PartialEq, Eq, Hash, Debug)]
pub struct FieldType(pub(crate) u8);
...
impl FieldType {
    ...
    pub(crate) const SEVERITY: Self = Self(b'S');

    /// Code: the SQLSTATE code for the error (see Appendix A). Not localizable. Always present.
    pub(crate) const CODE: Self = Self(b'C');

    /// Message: the primary human-readable error message. This should be accurate but terse (typically one line). Always present.
    pub(crate) const MESSAGE: Self = Self(b'M');

    /// Detail: an optional secondary error message carrying more detail about the problem. Might run to multiple lines.
    pub(crate) const DETAIL: Self = Self(b'D');

    /// Hint: an optional suggestion what to do about the problem. This is intended to differ from Detail in that it offers advice (potentially inappropriate) rather than hard facts. Might run to multiple lines.
    pub(crate) const HINT: Self = Self(b'H');

    /// Position: the field value is a decimal ASCII integer, indicating an error cursor position as an index into the original query string. The first character has index 1, and positions are measured in characters not bytes.
    pub(crate) const POSITION: Self = Self(b'P');
    ...
    pub(crate) const INTERNAL_POSITION: Self = Self(b'p');
    ...
    pub(crate) const INTERNAL: Self = Self(b'q');
    ...
    pub(crate) const WHERE: Self = Self(b'W');
    ...
    pub(crate) const SCHEMA: Self = Self(b's');
    ...
    pub(crate) const TABLE: Self = Self(b't');
    ...
    pub(crate) const COLUMN: Self = Self(b'c');
    ...
    pub(crate) const DATATYPE: Self = Self(b'd');
    ...
    pub(crate) const CONSTRAINT: Self = Self(b'n');

    /// File: the file name of the source-code location where the error was reported.
    pub(crate) const FILE: Self = Self(b'F');

    /// Line: the line number of the source-code location where the error was reported.
    pub(crate) const LINE: Self = Self(b'L');

    /// Routine: the name of the source-code routine reporting the error.
    pub(crate) const ROUTINE: Self = Self(b'R');
}
