...
/// Trait expressing the structural requirements `NewWriterWrap` needs from the
/// wrapped context (`offset`, `write`, `pwrite`).
// TODO(port): if a caller needs to wrap a context with *different* fns than its
...
/// Generic wrapper over a `WriterContext`.
#[derive(Copy, Clone)]
...
impl<C: WriterContext> NewWriter<C> {
    ...
    pub fn f64(self, value: f64) -> Result<(), AnyPostgresError> {
        // Bitcast to u64, then big-endian bytes.
    ...
    where
        T: TryInto<u16>,
    {
        // Narrow to u16; out-of-range values yield TooManyParameters.
        let v: u16 = value
            .try_into()
    ...
    // TODO(port): the original method name was `String` (capital S); snake_cased
    // it collides with `string(&[u8])` above. Renamed to `bun_string`. Update
    // callers in Phase B.
...
// `NewWriter<C>` where `C: WriterContext` — the trait already binds
// `offset`/`write`/`pwrite` to the context's inherent methods.
