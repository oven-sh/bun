...
// `writeFn` needs to be bound at type-construction time, not passed per-call.
// Stable Rust cannot take a fn as a const generic, so we model it as a ZST type
// parameter `F: WriteFn<Container>` — callers supply a unit struct that impls the trait.
// This keeps `write(this, context)` 2-arg.
...
impl<Container, F: WriteFn<Container>> WriteWrap<Container, F> {
    pub fn write<Ctx: WriterContext>(
        ...
        // The generic `<Ctx>` carries the context type.
