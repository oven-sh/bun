...
impl JSMySQLConnection {
    ...
    pub fn close(&self) {
        // `this.ref(); defer { updateReferenceType(); deref(); }`. Re-enter
    ...
    fn drain_internal(&self) {
        ...
        // `this.ref(); defer this.deref();` — raw-pointer RAII guard so no
    ...
    pub fn create_instance(
        ...
        // PORT NOTE: the original packed all five strings into one
        // `StringBuilder`-owned arena and handed sub-slices of it to
        // `MySQLConnection.init`. The Rust `init` takes `Box<[u8]>` per field
        // (each separately owned), so we just copy each string into its own
        // allocation. `options_buf` (the original arena handle, kept only so
        // `cleanup()` could free it) becomes an empty box.
    ...
    pub fn do_close(
        ...
        // `defer this.updateReferenceType();` — R-2: `&Self` is `Copy`, so
    ...
    fn fail_with_js_value(&self, value: JSValue) {
        // `this.ref(); defer { ...; updateReferenceType(); deref(); }` —
    ...
    pub fn on_result_row<C: bun_sql::mysql::protocol::ReaderContext>(
        ...
        // below. Stash a `ParentRef` (lifetime-erased `&T`) and `as_deref` at
        // the `to_js` call site — `*statement`
...
/// Referenced by the socket dispatch layer (kind = `.mysql[_tls]`).
pub struct SocketHandler<const SSL: bool>;

// PORT NOTE: an inherent `SocketType = uws.NewSocketHandler(ssl)` associated
// type is unstable in Rust (`feature(inherent_associated_types)`).
...
impl<const SSL: bool> SocketHandler<SSL> {
    ...
    pub fn on_close(
        ...
        // `defer this.deref();` — releases the socket ref taken in on_open.
    ...
    pub fn on_data(this: &JSMySQLConnection, _: NewSocketHandler<SSL>, data: &[u8]) {
        // `this.ref(); defer this.deref();` + `defer { resetConnectionTimeout(); ... }`.
...
/// Re-export: `MySQLQuery` / `MySQLRequestQueue` / `JSMySQLQuery` import the
/// JS-wrapper type under the bare `MySQLConnection` name (the connection
/// state-machine struct lives in `my_sql_connection`). Surface the alias here
/// so `super::js_mysql_connection::MySQLConnection` resolves to this type, not
/// the protocol-layer struct.
