...
impl JSMySQLConnection {
    ...
    pub(crate) fn create_instance(
        ...
        let username: Box<[u8]> = Box::from(args.username_str.to_utf8().slice());
        let password: Box<[u8]> = Box::from(args.password_str.to_utf8().slice());
        let database: Box<[u8]> = Box::from(args.database_str.to_utf8().slice());
        let path: Box<[u8]> = Box::from(path_str.to_utf8().slice());
