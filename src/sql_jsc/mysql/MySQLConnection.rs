...
impl MySQLConnection {
    ...
    /// embeds `self`). Route through raw pointers and pass the parent as a raw
    /// `*mut JSMySQLConnection`; `MySQLRequestQueue::advance` dereferences it
    /// only at disjoint-field call sites, never forming a full
    /// `&mut JSMySQLConnection` while `&mut self` (the queue) is live.
    fn advance(&mut self) {
        let js_connection = self.get_js_connection();
        let queue = &mut self.queue as *mut MySQLRequestQueue;
        // SAFETY: `queue` and the rest of `*js_connection` are disjoint fields;
        // advance() reads/writes connection state but not the queue's own
        // backing storage through `js_connection`.
        unsafe { (*queue).advance(js_connection) };
    }
    ...
    pub fn set_status(&mut self, status: ConnectionState) {
        ...
        match status {
            ConnectionState::Connected => {
                // SAFETY: get_js_connection returns the parent struct embedding self.
                unsafe { (*self.get_js_connection()).on_connection_established() };
            }
            _ => {}
        }
    }
...
impl Reader {
    #[inline]
    fn conn(self) -> &'static mut MySQLConnection {
        // SAFETY: self.connection points at a live MySQLConnection for the
        // duration of the read call (NewReader is constructed by
        // `MySQLConnection::buffered_reader(&mut self)` and not stored). The
        // 'static lifetime decouples the returned borrow from `self` (matches
        // Zig's `*MySQLConnection` field which has no lifetime).
...
impl ReaderContext for Reader {
    fn mark_message_start(self) {
        let c = self.conn();
        c.last_message_start = c.read_buffer.head;
    }

    fn set_offset_from_start(self, offset: usize) {
        let c = self.conn();
        c.read_buffer.head = c.last_message_start + (offset as u32);
    }

    fn peek(&self) -> &[u8] {
        // SAFETY: see `Reader::conn`.
    ...
    fn skip(self, count: isize) {
        ...
        if count < 0 {
            ...
            if abs_count > c.read_buffer.head as usize {
                c.read_buffer.head = 0;
                return;
            }
            c.read_buffer.head -= u32::try_from(abs_count).unwrap();
            return;
        }

        let ucount: usize = usize::try_from(count).unwrap();
        if c.read_buffer.head as usize + ucount > c.read_buffer.byte_list.len as usize {
            c.read_buffer.head = c.read_buffer.byte_list.len;
            return;
        }

        c.read_buffer.head += u32::try_from(ucount).unwrap();
    }

    fn ensure_capacity(self, count: usize) -> bool {
        self.conn().read_buffer.remaining().len() >= count
    }
...
//   confidence: medium
//   gating:     do_handshake / upgrade_to_tls / handle_handshake / handle_auth /
//               handle_command / handle_prepared_statement / handle_result_set /
//               handle_result_set_ok / check_if_prepared_statement_is_done /
//               send_handshake_response / send_auth_switch_response /
//               handle_handshake_decode_public_key bodies remain
//               unimplemented!()-gated on bun_sql::mysql::protocol decode/write
//               surfaces (Decode trait, writeWrap, PacketType repr, Capabilities
