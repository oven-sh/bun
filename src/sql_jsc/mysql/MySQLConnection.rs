...
impl MySQLConnection {
        ...
        if let Some(statement) = request.get_statement() {
            ...
            match statement.status {
                ...
                mysql_statement::Status::Failed => {
                    ...
                    //
                    // `on_error_packet` is `&self`; route through the
    ...
    pub fn handle_prepared_statement<C: ReaderContext>(
        ...
        match PacketType(first_byte) {
            ...
            PacketType::ERROR => {
                ...
                // `on_error_packet` is `&self`; `js_connection_ref()` is
    ...
    // is `&JSMySQLQuery` (fully interior-mutable, so a shared borrow is
    ...
    fn handle_result_set_ok(
        ...
        // `on_query_result` is `&self`; `js_connection_ref()` is the
    ...
    fn handle_result_set<C: ReaderContext>(
        ...
        match PacketType(first_byte) {
            PacketType::ERROR => {
                ...
                // `on_error_packet` is `&self`; route through the audited
            ...
            packet_type => {
                    ...
                    .contains(mysql_statement::ExecutionFlags::HEADER_RECEIVED)
                {
                    ...
                    // `on_result_row` is `&self`; route through the audited
