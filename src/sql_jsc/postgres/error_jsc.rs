...
pub fn create_postgres_error(
    ...
    // PORT NOTE: the original reflected over every optional field and `put` it by name
    // when `Some`. Rust has no field reflection; expand by hand. Property names must
    // match the original struct field names verbatim (camelCase: `internalPosition`,
    // `internalQuery`, `dataType`) since the JS consumer reads `options.internalPosition` etc.
