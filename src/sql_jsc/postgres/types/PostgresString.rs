...
/// `toJSWithType` was originally type-switched at compile time. Rust models this
/// as a trait with per-type impls; an unsupported type is the natural "no impl"
/// compile error.
pub trait ToJsWithType {
    fn to_js_with_type(self, global: &JSGlobalObject) -> Result<JSValue, AnyPostgresError>;
}

// Covers all byte-slice flavors (mutable/const, NUL-terminated or not) — they collapse to `&[u8]`.
...
impl ToJsWithType for &mut Data {
    fn to_js_with_type(self, global: &JSGlobalObject) -> Result<JSValue, AnyPostgresError> {
        ...
        // `defer str.deinit()` → Drop on bun_core::String
        // TODO(port): the original consumes the Data here. In Rust, Data's Drop should
        // handle this at the caller's scope; revisit ownership if Data must be freed
...
pub fn to_js<T: ToJsWithType>(
    ...
    // TODO(port): the original body binds the JSValue result of `toJSWithType` to `str`, then
