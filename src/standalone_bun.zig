...
pub const StandaloneModuleGraph = struct {
    ...
    pub fn toExecutable(allocator: std.mem.Allocator, output_files: []const bun.options.OutputFile, root_dir: std.fs.IterableDir, outfile: []const u8) !void {
        const bytes = try toBytes(allocator, output_files);
    ...
    pub fn toBytes(allocator: std.mem.Allocator, output_files: []const bun.options.OutputFile) ![]u8 {
        ...
        for (output_files) |output_file| {
            ...
            var module = CompiledModuleGraphFile{
                .name = string_builder.appendCount(output_file.path),
