...
#[derive(Default)]
pub struct Mutex {
    // `#[doc(hidden)] pub` so `bun_threading::Condition` can reach `srwlock` /
    // `locking_thread` for `SleepConditionVariableSRW`.
    #[doc(hidden)]
    pub impl_: Impl,
}
...
impl Mutex {
    ...
    pub fn lock_guard(&self) -> MutexGuard {
        self.lock();
        MutexGuard {
            mutex: core::ptr::NonNull::from(self),
...
impl Drop for MutexGuard {
    #[inline]
    fn drop(&mut self) {
        // SAFETY: `lock_guard`'s caller contract — the `Mutex` outlives the
        // guard, so the pointer is still valid here.
        unsafe { self.mutex.as_ref() }.unlock()
    }
}
...
/// `SRWLOCK` (Win32 `struct { PVOID Ptr; }`); static-init is all-zero.
/// Declared here (rather than reusing `bun_sys::windows::SRWLOCK`) because
/// this crate sits below `bun_sys` in the crate graph.
#[cfg(windows)]
...
// `repr(transparent)`: embedded by value in C-shared structs (see `FutexImpl`).
#[cfg(windows)]
#[derive(Default)]
#[repr(transparent)]
pub struct WindowsImpl {
    #[doc(hidden)]
    pub srwlock: core::cell::UnsafeCell<SRWLOCK>,
}
...
// `ReleaseSRWLockExclusive` keeps the raw-pointer extern: MSDN documents
// "results are undefined" when called without ownership (unlike
// `os_unfair_lock_unlock`, which aborts), so that one retains its block.
#[cfg(windows)]
#[link(name = "kernel32")]
unsafe extern "system" {
    safe fn AcquireSRWLockExclusive(lock: &core::cell::UnsafeCell<SRWLOCK>);
    // Returns BOOLEAN (u8), not BOOL — compare against 0, not the i32 `FALSE`.
    safe fn TryAcquireSRWLockExclusive(lock: &core::cell::UnsafeCell<SRWLOCK>) -> u8;
...
impl WindowsImpl {
    pub(crate) const fn new() -> Self {
        Self {
            srwlock: core::cell::UnsafeCell::new(SRWLOCK_INIT),
        }
    }
    ...
    fn unlock(&self) {
        ...
        unsafe { ReleaseSRWLockExclusive(self.srwlock.get()) }
    }
}
...
// `repr(transparent)`: embedded by value in C-shared structs (`us_loop_t.data.mutex`).
#[cfg(not(any(windows, target_vendor = "apple")))]
#[derive(Default)]
#[repr(transparent)]
