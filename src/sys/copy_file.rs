...
pub fn copy_file_with_state(
    ...
    #[cfg(windows)]
    {
        ...
        if rc == 0 {
            return Err(crate::Error::from_win32(
                crate::windows::Win32Error::get(),
                Tag::copyfile,
            ));
        }
        return Ok(());
...
pub(crate) fn copy_file_read_write_loop(in_: fd_t, out: fd_t, len: usize) -> crate::Result<usize> {
    ...
    match crate::read(
        Fd::from_native(cfg_select! {
            bun_portable => in_,
            _ => in_ as _,
        }),
        &mut buf[0..adjusted_count],
    ) {
        Ok(amt_read) => {
            ...
            while amt_written < amt_read {
                {
                    let wrote = crate::write(
                        Fd::from_native(cfg_select! {
                            bun_portable => out,
                            _ => out as _,
                        }),
                        &buf[amt_written..amt_read],
                    )?;
                    if wrote == 0 {
