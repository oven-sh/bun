...
//
// PORT NOTE: built once at first access via `LazyLock`. The per-OS (name →
// message) string tables themselves live canonically in
...
/// The `EnumMap` is total, so we treat the `UNKNOWN` sentinel as `None` to
/// preserve the fallthrough behaviour (callers format `"unknown error {errno}"`).
#[inline]
