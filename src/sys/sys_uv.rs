...
// the original `ReturnCode::errno()` does inline; without it the raw `UV_E*` magnitude
...
//re-exports the bun.sys versions of these (libuv has no
...
// `pub const mkdirOSPath = bun.sys.mkdirOSPath;` — on Windows that's the
...
//. The `flags` param is ignored on Windows (`_ = flags;`) but is
// part of the public 2-arg signature, so we cannot `pub use mkdir_w as ...` —
// that would drop the second arg and break ported callers that pass a mode.
#[inline]
...
pub fn open(file_path: &ZStr, c_flags: i32, perm_: Mode) -> Result<Fd> {
    // `defer req.deinit();` — libuv heap-allocates the WCHAR path copy
...
pub fn statfs(file_path: &ZStr) -> Result<StatFS> {
    // `defer req.deinit();` — `uv_fs_statfs` heap-allocates the
    ...
    if let Some(errno) = rc.err_enum_e() {
        ...
        // the original used `*align(1)` — read_unaligned to match.
        // PORT: on Windows `StatFS == uv_statfs_t`, so the libuv result *is* the
        // public type — no original-side `.init(*align(1) StatFS)` wrapper to call.
        // SAFETY: libuv guarantees `req.ptr` points to a valid `uv_statfs_t`
        // on success; we read it by value (the original used `*align(1)` → unaligned).
...
pub fn readlink<'a>(file_path: &ZStr, buf: &'a mut [u8]) -> Result<&'a mut ZStr> {
    // `defer req.deinit();` — `uv_fs_readlink` heap-allocates the target
...
pub fn ftruncate(fd: Fd, size: i64) -> Result<()> {
    // The original spec types `size: isize` (= i64 on every supported Windows target);
...
pub fn preadv(fd: Fd, bufs: &[PlatformIOVec], position: i64) -> Result<usize> {
    ...
    while !remaining_bufs.is_empty() {
        ...
        // `defer req.deinit();` — `uv_fs_read` heap-allocates
...
pub fn pwritev(fd: Fd, bufs: &[PlatformIOVecConst], position: i64) -> Result<usize> {
    ...
    while !remaining_bufs.is_empty() {
        ...
        // `defer req.deinit();` — `uv_fs_write` heap-allocates
        // `req->fs.info.bufs` when `nbufs > 4` and self-points it at
        // `req->fs.info.bufsml` when `nbufs <= 4`; `FsReq::drop` runs
        // `uv_fs_req_cleanup` in place every iteration (early-return included).
        let mut req = FsReq::new();
...
pub fn writev(fd: Fd, bufs: &[PlatformIOVec]) -> Result<usize> {
    // TODO(port): the original signature is `[]bun.PlatformIOVec` (mutable) but pwritev takes
...
// PORT NOTE: the original `write()` builds a `[1]PlatformIOVecConst` and calls `writev` (which
// takes `[]PlatformIOVec`). The two types alias on Windows so the original coerces silently. Rust
