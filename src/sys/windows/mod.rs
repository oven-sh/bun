...
#[cfg(windows)]
pub use bun_core::PathBuffer;
#[cfg(windows)]
pub use bun_core::WPathBuffer;
...
/// `to_system_errno()` / `to_e()` / `to_result()` — extension trait from
/// `bun_core::errno` (the `SystemErrno` mapping table is a higher-tier concern
/// than the tier-0 newtype).
pub use bun_core::errno::windows::Win32ErrorExt;

pub(crate) use bun_core::errno::translate_uv_error_to_e;
...
pub fn CreateHardLinkW(
    ...
    #[cfg(debug_assertions)]
    {
        ...
        let new_w = unsafe { bun_opaque::ffi::wstr_units(new_file_name) };
        // SAFETY: caller guarantees both LPCWSTR args are NUL-terminated wide strings
        let existing_w = unsafe { bun_opaque::ffi::wstr_units(existing_file_name) };
        bun_sys::syslog!(
            "CreateHardLinkW({}, {}) = {}",
...
/// adds the debug-build `Output::debug_warn` diagnostics (which `bun_core::errno`
/// cannot emit without an upward dep).
pub fn translate_nt_status_to_errno(err: NTSTATUS) -> E {
    let e = bun_core::errno::windows::translate_ntstatus_to_errno(err);
    #[cfg(debug_assertions)]
    {
...
pub fn exe_path_w() -> &'static bun_core::WStr {
    ...
    // `peb()` lives in the tier-0 leaf `bun_windows_sys` (needs inline asm).
    unsafe {
...
pub mod rescle {
    ...
    pub fn set_windows_metadata(
        ...
        let mut icon_buf = bun_core::WPathBuffer::uninit();
...
pub fn move_opened_file_at(
    ...
    const STRUCT_BUF_LEN: usize =
        size_of::<win32::FILE_RENAME_INFORMATION_EX>() + (bun_core::PATH_MAX_WIDE * 2 - 1);
...
#[cfg(test)]
mod tests {
    use super::{E, SystemErrno, Win32Error, Win32ErrorExt as _};

    /// A Win32 code with no entry in `SystemErrno::init_win32_error`.
    const UNMAPPED: Win32Error = Win32Error(0xFFFE);

    #[test]
    fn to_result_success_is_ok() {
        assert!(Win32Error::SUCCESS.to_result().is_ok());
    }

    #[test]
    fn to_result_mapped_is_err() {
        assert!(Win32Error::FILE_NOT_FOUND.to_result().is_err());
    }
    ...
    #[test]
    fn to_result_unmapped_is_err() {
        assert!(UNMAPPED.to_system_errno().is_none());
        assert!(UNMAPPED.to_result().is_err());
    }
