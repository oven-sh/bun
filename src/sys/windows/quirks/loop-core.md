...
- **Bun disposition**: should-port. Bun's Rust layer should avoid CRT fd APIs entirely, but Bun *does* expose CRT fds via `uv_open_osfhandle`/N-API and links the CRT; installing the no-op handler at process init prevents addon-triggered process death. Decide once, document the process-wide effect. Target: engine
