...
- **Bun disposition**: must-port (the lesson). Target: bun_core Windows version helpers (cross-ref: LOOP). Bun's 1809+ baseline makes the _1709 probe_ itself unnecessary (always true) — but any future version probe must use RtlGetVersion with a fully-initialized struct.
...
- **What Windows does**: Disconnecting a connected UDP socket is `connect` with family AF_UNSPEC. Windows (like z/OS) validates `namelen` against the _currently stored_ address length: passing a 16-byte `struct sockaddr` to disconnect an IPv6-connected socket fails WSAEINVAL/EINVAL.
