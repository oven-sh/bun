...
// string field on return, so the marshalled struct must hold exactly the refs
// `self` held — i.e. a bitwise field copy with NO extra `ref_()`. The caller's
// `bun_sys::SystemError` is consumed (its strings reach refcount-0) —
// `Error::to_system_error()` builds a temporary that `.to_error_instance()`
// consumes.
// ──────────────────────────────────────────────────────────────────────────
...
impl SystemErrorJsc for bun_sys::SystemError {
    /// Build a JS `Error` instance for this system error.
    fn to_error_instance(&self, global: &JSGlobalObject) -> JSValue {
        marshal(self).to_error_instance(global)
    }
    /// Like `to_error_instance`, but also attaches the promise's await chain
    /// as async stack frames so threadpool-rejected promises get a useful
    /// trace.
    fn to_error_instance_with_async_stack(
        &self,
        global: &JSGlobalObject,
