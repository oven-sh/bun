...
pub fn resolve(p: &PresetInput<'_>, env: EnvGet<'_>) -> Result<OtlpExporterConfig, String> {
    let key = |vars: &[&str]| -> Option<String> {
        p.api_key.clone().or_else(|| vars.iter().find_map(|v| env(v))).filter(|s| !s.is_empty())
    };
    let site = |vars: &[&str], default: &str| -> String {
        p.site.clone().or_else(|| vars.iter().find_map(|v| env(v))).filter(|s| !s.is_empty()).unwrap_or_else(|| default.to_string())
    };
    ...
    let url = match p.name {
        ...
        "datadog" => match key(&["DD_API_KEY"]) {
            Some(k) => {
                ...
                join(p.endpoint.as_deref().unwrap_or(&format!("https://otlp.{s}")), "/v1/traces")
            }
            None => join(
                p.endpoint.as_deref().or(env("DD_OTLP_ENDPOINT").as_deref()).unwrap_or("http://localhost:4318"),
                "/v1/traces",
            ),
        },
        "honeycomb" => {
            let k = key(&["HONEYCOMB_API_KEY"]).ok_or_else(|| need("apiKey (or HONEYCOMB_API_KEY)", p.name))?;
        ...
        "grafana" => {
            let token = key(&["GRAFANA_CLOUD_API_KEY", "GRAFANA_OTLP_TOKEN"]).ok_or_else(|| need("apiKey (a Cloud Access Policy token)", p.name))?;
            let instance = p.id.clone().or_else(|| env("GRAFANA_CLOUD_INSTANCE_ID")).ok_or_else(|| need("id (the stack's instance id)", p.name))?;
            let mut auth = Vec::new();
            bun_base64_encode(format!("{instance}:{token}").as_bytes(), &mut auth);
            headers.push(("authorization".into(), format!("Basic {}", String::from_utf8(auth).unwrap_or_default())));
            let base = match (&p.endpoint, p.site.clone().or_else(|| env("GRAFANA_CLOUD_ZONE"))) {
                (Some(e), _) => e.clone(),
                (None, Some(zone)) if zone.starts_with("http") => zone,
                (None, Some(zone)) => format!("https://otlp-gateway-{zone}.grafana.net/otlp"),
                (None, None) => return Err(need("site (the OTLP gateway zone, e.g. prod-us-east-0) or endpoint", p.name)),
            };
            join(&base, "/v1/traces")
        }
        "newrelic" => {
            let k = key(&["NEW_RELIC_LICENSE_KEY", "NEW_RELIC_API_KEY"]).ok_or_else(|| need("apiKey (or NEW_RELIC_LICENSE_KEY)", p.name))?;
            ...
            let base = match &p.endpoint {
                ...
                None if s.eq_ignore_ascii_case("fedramp") || s.eq_ignore_ascii_case("gov") => "https://gov-otlp.nr-data.net".into(),
                None => "https://otlp.nr-data.net".into(),
            };
            join(&base, "/v1/traces")
        }
        "axiom" => {
            let k = key(&["AXIOM_TOKEN", "AXIOM_API_TOKEN"]).ok_or_else(|| need("apiKey (or AXIOM_TOKEN)", p.name))?;
            let ds = p.id.clone().or_else(|| env("AXIOM_DATASET")).ok_or_else(|| need("id (dataset name, or AXIOM_DATASET)", p.name))?;
            headers.push(("authorization".into(), format!("Bearer {k}")));
            headers.push(("x-axiom-dataset".into(), ds));
            let base = p.endpoint.clone().or_else(|| p.site.clone().map(|d| if d.starts_with("http") { d } else { format!("https://{d}") })).or_else(|| env("AXIOM_DOMAIN").map(|d| if d.starts_with("http") { d } else { format!("https://{d}") })).unwrap_or_else(|| "https://api.axiom.co".into());
            join(&base, "/v1/traces")
        }
        "dynatrace" => {
            let k = key(&["DT_API_TOKEN", "DYNATRACE_API_TOKEN"]).ok_or_else(|| need("apiKey (an API token with openTelemetryTrace.ingest)", p.name))?;
            headers.push(("authorization".into(), format!("Api-Token {k}")));
            let base = match (&p.endpoint, p.id.clone().or_else(|| env("DT_ENVIRONMENT_ID"))) {
                (Some(e), _) => e.clone(),
                (None, Some(envid)) => format!("https://{envid}.live.dynatrace.com/api/v2/otlp"),
                (None, None) => return Err(need("id (environment id) or endpoint (…/api/v2/otlp)", p.name)),
            };
            join(&base, "/v1/traces")
        }
        // Sentry's OTLP traces URL is per project; take it verbatim.
        "sentry" => {
            let e = p.endpoint.clone().or_else(|| env("SENTRY_OTLP_TRACES_ENDPOINT")).ok_or_else(|| need("endpoint (the project's OTLP traces URL)", p.name))?;
            let k = key(&["SENTRY_PUBLIC_KEY"]).ok_or_else(|| need("apiKey (the DSN public key)", p.name))?;
            headers.push(("x-sentry-auth".into(), format!("sentry sentry_key={k}")));
            e
        }
        // Local Collector / Jaeger / Tempo.
        "otlp" | "collector" | "jaeger" | "tempo" => join(p.endpoint.as_deref().unwrap_or("http://localhost:4318"), "/v1/traces"),
        other => return Err(format!("unknown exporter preset {other:?} (known: datadog, honeycomb, grafana, newrelic, axiom, dynatrace, sentry, otlp)")),
    };
...
fn bun_base64_encode(input: &[u8], out: &mut Vec<u8>) {
    ...
    for chunk in input.chunks(3) {
        ...
        out.push(if chunk.len() > 1 { T[((b[1] & 15) << 2 | b[2] >> 6) as usize] } else { b'=' });
        out.push(if chunk.len() > 2 { T[(b[2] & 63) as usize] } else { b'=' });
    }
}
