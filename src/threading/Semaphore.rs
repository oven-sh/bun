//! Semaphore built on Bun's `Mutex` + `Condition`. A semaphore is an unsigned
//! integer that blocks the calling thread when it would go negative.
//!
//! Supports `const fn new()` static initialization and needs no `deinit`.
//! Only the subset Bun uses is implemented (`wait`/`post`); `timedWait` is
//! omitted because no caller needs it and it would pull in a monotonic timer.
