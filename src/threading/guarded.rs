...
pub struct GuardedLock<'a, Value, M: RawMutex> {
    ...
    // Preserve the same `!Send`/`!Sync` surface as sibling mutex guards.
    // Some OS backends require unlock on the locking thread, so a guard must
    // not be movable to another thread for drop.
