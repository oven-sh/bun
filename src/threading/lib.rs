//! bun_threading crate root — thin re-exports of Bun's threading primitives.
...
/// `parking_lot::Once` parity. Bun has no custom `Once`, and `std::sync::Once`
/// has no poisoning concern, so just re-export it for callers migrating off
/// `parking_lot::Once`.
pub use std::sync::Once;
pub use thread_pool::ThreadPool;
pub use unbounded_queue::{Link, Linked, UnboundedQueue};
pub use wait_group::WaitGroup;
/// `WorkPoolTask` is an alias for `ThreadPool::Task`.
...
///
/// Delegates to the tier-0 implementation in [`bun_safety::thread_id::current`]
/// (which uses `pthread_threadid_np` on Darwin / `pthread_getthreadid_np` on
/// FreeBSD / `gettid` on Linux), widened to `u64` so callers can store it in an
/// `AtomicU64` regardless of the platform's native `ThreadId` width.
#[inline]
