...
pub const Transpiler = struct {
    ...
    pub fn resolveEntryPoint(transpiler: *Transpiler, entry_point: string) !_resolver.Result {
        return _resolveEntryPoint(transpiler, entry_point) catch |err| {
            ...
            transpiler.log.addErrorFmt(null, logger.Loc.Empty, transpiler.allocator, "{s} resolving \"{s}\" (entry point)", .{ @errorName(err), entry_point }) catch |oe| bun.outOfMemory(oe);
            return err;
        };
    }
        ...
        switch (loader) {
            ...
            .css => {
                ...
                if (sheet.minify(alloc, bun.css.MinifyOptions.default(), &extra).asErr()) |e| {
                    transpiler.log.addErrorFmt(null, logger.Loc.Empty, transpiler.allocator, "{} while minifying", .{e.kind}) catch |oe| bun.outOfMemory(oe);
                    return null;
                }
                ...
                const result = switch (sheet.toCss(
                    ...
                    .result => |v| v,
                    .err => |e| {
                        transpiler.log.addErrorFmt(null, logger.Loc.Empty, transpiler.allocator, "{} while printing", .{e}) catch |oe| bun.outOfMemory(oe);
                        return null;
                    },
                };
