...
//! core-semantics.md §8 (socket creation, option sequences, addr plumbing,
...
/// Clear TCP_CORK on Linux; no-op elsewhere (core-semantics.md R4.6 — the
...
/// Windows placeholder until the winsock port lands (size/alignment of
/// `SOCKADDR_STORAGE`).
#[cfg(windows)]
#[repr(C, align(8))]
pub(crate) struct ConnectAddr {
...
/// Windows: the C layer routes these through winsock/libuv (win32
/// nonblocking via FIONBIO, WSA error mapping, SIO_TCP_INITIAL_RTO, null-addr
/// loopback rewrite). Rust plumbing lands with the libuv backend shard;
/// POSIX signatures preserved.
#[cfg(windows)]
mod imp {
    ...
    pub(crate) fn send(_fd: LIBUS_SOCKET_DESCRIPTOR, _data: &[u8]) -> isize {
        todo!("windows winsock port")
    }
    pub(crate) fn writev(_fd: LIBUS_SOCKET_DESCRIPTOR, _iov: &[UsIoVec]) -> isize {
        todo!("windows winsock port")
    }
    /// `bsd_write2` (bsd.c:943-953): two sequential sends; the payload send
    ...
    pub(crate) fn recv(_fd: LIBUS_SOCKET_DESCRIPTOR, _buf: &mut [u8]) -> isize {
        todo!("windows winsock port")
    }
    ...
    pub(crate) fn connect_probe(fd: LIBUS_SOCKET_DESCRIPTOR) -> c_int {
        unsafe extern "system" {
            fn recv(s: usize, buf: *mut u8, len: c_int, flags: c_int) -> c_int;
        ...
        if unsafe { recv(fd, core::ptr::null_mut(), 0, MSG_PUSH_IMMEDIATE) } != SOCKET_ERROR {
            return 0;
        }
        // SAFETY: thread-local error-slot read.
        match unsafe { WSAGetLastError() } {
            10035 /* WSAEWOULDBLOCK */ | 10004 /* WSAEINTR */ => 0,
            e => e,
        }
    }
    pub(crate) fn close(_fd: LIBUS_SOCKET_DESCRIPTOR, _rst: bool) {
        todo!("windows winsock port")
    }
    pub(crate) fn shutdown(_fd: LIBUS_SOCKET_DESCRIPTOR) {
        todo!("windows winsock port")
    }
    pub(crate) fn shutdown_read(_fd: LIBUS_SOCKET_DESCRIPTOR) {
        todo!("windows winsock port")
    }
    /// No-op on Windows: libuv sets FIONBIO at poll init (R8.1).
    pub(crate) fn set_nonblocking(fd: LIBUS_SOCKET_DESCRIPTOR) -> i32 {
        fd as i32
    }
    /// No-op off Darwin.
    ...
    pub(crate) fn set_errno(v: i32) {
        unsafe extern "system" {
            fn WSASetLastError(err: c_int);
        }
        // SAFETY: thread-local error-slot write; no pointers involved.
        unsafe { WSASetLastError(v) }
    }
    ...
    pub(crate) fn local_addr(_fd: LIBUS_SOCKET_DESCRIPTOR) -> Option<BsdAddr> {
        todo!("windows winsock port")
    }
    pub(crate) fn remote_addr(_fd: LIBUS_SOCKET_DESCRIPTOR) -> Option<BsdAddr> {
        todo!("windows winsock port")
    }
    pub(crate) fn accept(
    ...
    pub(crate) fn nodelay(_fd: LIBUS_SOCKET_DESCRIPTOR, _enabled: bool) {
        todo!("windows winsock port")
    }
    pub(crate) fn keepalive(_fd: LIBUS_SOCKET_DESCRIPTOR, _on: bool, _delay: u32) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn set_tos(_fd: LIBUS_SOCKET_DESCRIPTOR, _tos: i32) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn get_tos(_fd: LIBUS_SOCKET_DESCRIPTOR) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn broadcast(_fd: LIBUS_SOCKET_DESCRIPTOR, _enabled: bool) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn multicast_loopback(_fd: LIBUS_SOCKET_DESCRIPTOR, _enabled: bool) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn multicast_interface(
        _fd: LIBUS_SOCKET_DESCRIPTOR,
        _addr: &crate::udp::sockaddr_storage,
    ) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn set_membership(
        _fd: LIBUS_SOCKET_DESCRIPTOR,
        _addr: &crate::udp::sockaddr_storage,
        _iface: Option<&crate::udp::sockaddr_storage>,
        _drop: bool,
    ) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn set_source_specific_membership(
        _fd: LIBUS_SOCKET_DESCRIPTOR,
        _source: &crate::udp::sockaddr_storage,
        _group: &crate::udp::sockaddr_storage,
        _iface: Option<&crate::udp::sockaddr_storage>,
        _drop: bool,
    ) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn ttl_unicast(_fd: LIBUS_SOCKET_DESCRIPTOR, _ttl: i32) -> i32 {
        todo!("windows winsock port")
    }
    pub(crate) fn ttl_multicast(_fd: LIBUS_SOCKET_DESCRIPTOR, _ttl: i32) -> i32 {
        todo!("windows winsock port")
    }
    ...
    /// Includes the win32 null-addr→loopback rewrite + SIO_TCP_INITIAL_RTO
    /// fast-fail for loopback (bsd.c:1760-1790).
    pub(crate) fn create_connect_socket(
        _addr: &super::ConnectAddr,
        _local_addr: Option<&super::ConnectAddr>,
        _options: c_int,
    ) -> Result<LIBUS_SOCKET_DESCRIPTOR, i32> {
        todo!("windows winsock port")
    }
    pub(crate) fn set_reuse(_fd: LIBUS_SOCKET_DESCRIPTOR, _options: c_int) -> Result<(), i32> {
        todo!("windows winsock port")
    }
    pub(crate) fn addr_from_entry(
        _info: *const bun_dns::addrinfo,
        _port: u16,
    ) -> super::ConnectAddr {
        todo!("windows winsock port")
    }
    pub(crate) fn try_parse_ip(_host: &CStr, _port: u16) -> Option<super::ConnectAddr> {
        todo!("windows winsock port")
    }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// UDP edges (core-semantics.md §9, consumers/05-udp.md) — raw-pointer field
...
mod udp_imp {
    ...
    /// so the tag fits the low 4 bits). Dispatch shard: mask with 0xf, match
...
/// Windows: UDP rides libuv's uv_poll + Winsock recvfrom/sendto; plumbing
/// lands with the libuv backend shard. Signatures preserved so udp.rs stays
/// platform-neutral.
#[cfg(windows)]
mod udp_imp {
    use core::ffi::{c_char, c_int, c_uint, c_void};
    ...
    pub(crate) fn udp_poll_start(_loop_: *mut Loop, _s: *mut udp::Socket, _events: u32) -> c_int {
        todo!("windows libuv UDP port")
    }
    pub(crate) fn udp_poll_change(_loop_: *mut Loop, _s: *mut udp::Socket, _events: u32) {
        todo!("windows libuv UDP port")
    }
    pub(crate) fn udp_poll_stop(_loop_: *mut Loop, _s: *mut udp::Socket) {
        todo!("windows libuv UDP port")
    }
    pub(crate) fn udp_bind_fd(
        _host: *const c_char,
    ...
    pub(crate) fn udp_local_port(_fd: LIBUS_SOCKET_DESCRIPTOR) -> c_int {
        todo!("windows libuv UDP port")
    }
    pub(crate) fn udp_addr_ip(
        _fd: LIBUS_SOCKET_DESCRIPTOR,
    ...
    pub(crate) fn udp_disconnect_fd(_fd: LIBUS_SOCKET_DESCRIPTOR) -> c_int {
        todo!("windows libuv UDP port")
    }
    pub(crate) fn udp_recvbuf_zeroed() -> PacketBuffer {
        PacketBuffer {
...
// ── SO_ERROR (R3.22e / R6.8; socket shard) ────────────────────────────────────
...
pub(crate) fn so_error(fd: LIBUS_SOCKET_DESCRIPTOR) -> i32 {
    ...
    const SOL_SOCKET: core::ffi::c_int = 0xFFFF;
    const SO_ERROR: core::ffi::c_int = 0x1007;
    let mut error: core::ffi::c_int = 0;
