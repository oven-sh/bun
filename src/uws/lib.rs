...
=======
// bun_uws_sys modules are un-gated; re-export the FFI handle types directly so
// `bun_uws::Foo` and `bun_uws_sys::Foo` are the SAME type for higher tiers.
//
// `SocketGroup`, `SocketContext`, `InternalSocket`/`NewSocketHandler`/
// `AnySocket`, `AnyRequest`, `AnyResponse`, and `Response<SSL>` keep local
// definitions further down — those add behaviour on top of the raw sys types
// and are not yet collapsed into the lower tier.
//
// The bun_runtime::* items (dispatch, WindowsNamedPipe, UpgradedDuplex) are
// upward refs and intentionally remain local stub modules.
>>>>>>> Stashed changes
...
=======
pub use bun_uws_sys::us_socket::{us_socket_t, us_socket_stream_buffer_t};
pub use bun_uws_sys::timer::Timer;
pub use bun_uws_sys::socket_kind::SocketKind;
pub use bun_uws_sys::connecting_socket::ConnectingSocket;
pub use bun_uws_sys::request::Request;
pub use bun_uws_sys::listen_socket::ListenSocket;
pub use bun_uws_sys::response::State;
pub use bun_uws_sys::response::c::uws_res;
pub use bun_uws_sys::web_socket::{RawWebSocket, AnyWebSocket, WebSocketBehavior};
pub use bun_uws_sys::body_reader_mixin::BodyReaderMixin;
pub use bun_uws_sys::app::NewApp;
pub use bun_uws_sys::{udp, vtable, quic};
pub use bun_uws_sys::h3 as H3;
>>>>>>> Stashed changes
...
=======
// Upward-tier refs (`bun_runtime::socket::{uws_dispatch, windows_named_pipe,
// upgraded_duplex}`). bun_runtime is a higher tier and cannot be a dependency
// here, so these stay as empty namespace stubs; the real bodies live upstream.
>>>>>>> Stashed changes
pub mod dispatch {}
