...
impl<const SSL: bool> ListenSocket<SSL> {
    ...
    fn as_core(&mut self) -> &mut UwsListenSocket {
        // SAFETY: live per the invariant above; exclusive — listen lambdas fire
        // after group-listen returns and close_listen_socket never dispatches
        // on_close (R3.29). ZST→header widening: FFI pointer's whole-header provenance.
        unsafe { &mut *std::ptr::from_mut::<Self>(self).cast::<UwsListenSocket>() }
    }
    ...
    pub fn get_local_port(&mut self) -> i32 {
        self.as_core().get_local_port()
    }

    pub fn socket(&mut self) -> crate::NewSocketHandler<SSL> {
        self.as_core().socket::<SSL>()
    }
}
