...
impl AnyResponse {
    ...
    pub fn force_close(self) {
        // A uWS `HttpResponse*` IS a live slab-resident `us_socket_t*`; caller
        // must pass a live one — from_live stamps the slot's CURRENT generation,
        // so this gives NO staleness protection. <false> is fine: close() is const-generic-blind.
        fn close_raw(s: *mut us_socket_t) {
            // uWS never hands out a null response: panic (as_handle's null
            // policy) instead of a silent no-op the C path never had.
            let p = core::ptr::NonNull::new(s).expect("force_close: null uWS response handle");
            bun_usockets::NewSocketHandler::<false>::from(bun_usockets::SocketRef::from_live(p))
                .close(bun_usockets::CloseCode::failure);
        }
        match self {
            // Cast the FFI pointer directly — no intermediate ZST `&mut` retag —
            // so the slab-header read keeps the C++ pointer's full provenance.
            AnyResponse::SSL(ptr) => close_raw(ptr.cast::<us_socket_t>()),
            AnyResponse::TCP(ptr) => close_raw(ptr.cast::<us_socket_t>()),
