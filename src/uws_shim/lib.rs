#![allow(
    ...
    // C++-shim FFI bindings: fn bodies are `unsafe { ffi_call(args) }`; the
    // safety contract is documented at each body and identical whether the
    // wrapper is `unsafe fn` or not.
    clippy::not_unsafe_ptr_arg_deref
)]
#![warn(unused_must_use)]
//! `bun_uws_shim` — Rust bindings for the surviving uWebSockets C++ layer
...
// ───────────────────────────── module map ────────────────────────────────────
// Snake-case module names; `#[path]` points at the PascalCase files on disk
// (same shape as the old `bun_uws_sys`).
...
// ───────────────── shim-owned surface (moved C++ bindings) ──────────────────
...
pub use response::{AnyResponse, State, WebSocketUpgradeContext, WriteResult};
...
pub type Response<const SSL: bool> = response::Response<SSL>;
...
pub mod SocketContext {
    ...
    /// `SSLConfig.asUSockets()` produces and `ssl_ctx_from_options` consumes.
    /// The struct body, `Default`, `digest()` and `create_ssl_context()` live
    /// in `bun_usockets::tls::context`.
    pub use bun_usockets::BunSocketContextOptions;
}
...
/// `SSLConfig.asUSockets()` produces and `ssl_ctx_from_options` consumes.
pub type us_bun_socket_context_options_t = BunSocketContextOptions;
...
impl Opcode {
    ...
    // (`uWS::OpCode::TEXT` etc.). Same bit values.
    pub const CONTINUATION: Opcode = Opcode(0);
    pub const TEXT: Opcode = Opcode(1);
    pub const BINARY: Opcode = Opcode(2);
    pub const CLOSE: Opcode = Opcode(8);
...
/// Owned socket-address shape (boxed IP). Distinct from
/// [`response::SocketAddress`] by design — that one stores the IP text inline
/// as returned from `Response::get_remote_socket_info`.
...
// Called by the surviving `root_certs.cpp` when NODE_EXTRA_CA_CERTS fails.
// Also #[no_mangle]-defined in src/uws until D1 deletes that crate; the two
// crates must never both link into bun_bin (duplicate symbol).
#[unsafe(no_mangle)]
...
pub fn get_default_ciphers() -> &'static ZStr {
    let ciphers = bun_usockets::tls::context::default_ciphers();
    // SAFETY: `default_ciphers` returns a process-static NUL-terminated
    // string; `ZStr::from_raw` rebuilds the length-carrying slice
    // (excluding the NUL).
    unsafe { ZStr::from_raw(ciphers.as_ptr().cast::<u8>(), ciphers.to_bytes().len()) }
}
...
pub mod ssl_wrapper {
    ...
    impl<T: Copy> SSLWrapper<T> {
        ...
        pub fn init_with_ctx(
            ...
            unsafe {
                if is_client {
                        ...
                        == boring_sys::SSL_VERIFY_NONE
                    {
                        ...
                        if let Some(roots) = NonNull::new(
                            us_tls::shared_default_ca_store().cast::<boring_sys::X509_STORE>(),
                        ) {
            ...
            // handler will drain them (see `flush_pending_events`); the
            // BoringSSL callbacks skip un-opted SSLs entirely.
            if handlers.on_session.is_some() || handlers.on_keylog.is_some() {
                us_tls::enable_pending_events(ssl.as_ptr().cast());
            }
        ...
        pub fn init_from_options(
            ...
            let Some(ssl_ctx) = (*ctx_opts)
                .create_ssl_context(&mut err)
                .map(|p| p.cast::<boring_sys::SSL_CTX>())
        ...
        fn get_verify_error(&self) -> us_bun_verify_error_t {
            ...
            us_bun_verify_error_t::from_ssl(ssl.as_ptr().cast_const().cast())
        }
        ...
        fn flush_pending_events(this: *mut Self, buffer: &mut [u8; BUFFER_SIZE]) {
            if Self::r(this).handlers.on_session.is_some() {
                loop {
                    let Some(ssl) = Self::r(this).ssl else { return };
                    // BUFFER_SIZE covers the 64 KB parking cap.
                    let len = us_tls::pop_pending_session(ssl.as_ptr().cast(), &mut buffer[..]);
                    if len == 0 {
                        break;
                    }
                    if let Some(on_session) = Self::r(this).handlers.on_session {
                        on_session(Self::r(this).handlers.ctx, &buffer[..len]);
                    }
                }
            }
            if Self::r(this).handlers.on_keylog.is_some() {
                loop {
                    let Some(ssl) = Self::r(this).ssl else { return };
                    // Keylog entries are capped at 4 KB+1, well within BUFFER_SIZE.
                    let len = us_tls::pop_pending_keylog(ssl.as_ptr().cast(), &mut buffer[..]);
                    if len == 0 {
                        break;
                    }
                    if let Some(on_keylog) = Self::r(this).handlers.on_keylog {
                        on_keylog(Self::r(this).handlers.ctx, &buffer[..len]);
                    }
                }
            }
        }
...
// ═══════════════════════════════════════════════════════════════════════════
// Loop parent-handle extension (was `bun_uws`)
// ═══════════════════════════════════════════════════════════════════════════
...
/// Extension methods on `bun_usockets::InternalLoopData` for the typed
/// parent-loop accessors. The core crate only stores tag+ptr; this tier adds
/// the trait-generic setter and the raw getter that callers use.
