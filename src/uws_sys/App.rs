...
pub mod c {
    ...
    /// `(listen_socket, dns_error, user_data)`: `dns_error` is the raw
    /// `getaddrinfo(3)` return code when the host failed to resolve (the
    /// socket is then null), 0 otherwise. Mirrors `uws_listen_handler` in
    /// `_libusockets.h`.
