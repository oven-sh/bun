...
unsafe extern "C" {
    ...
    // safe: no args; frees the thread-local uWS loop wrapper if one exists.
...
/// Frees this thread's uWS loop wrapper (`us_loop_t` + `LoopData` + its
/// recv/send/cork buffers) regardless of `cleanMe`. On Windows
/// `WindowsLoop::get()` passes a non-null `uv_loop_t` hint so `cleanMe` stays
/// false and [`on_thread_exit`] is a no-op; this frees the wrapper anyway.
/// Must be called BEFORE the native `uv_loop_t` is closed: `us_loop_free`
/// queues `uv_close` callbacks that the caller's subsequent `uv_run` flushes.
