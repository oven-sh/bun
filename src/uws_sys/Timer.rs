...
impl Timer {
    ...
    // PORT NOTE: trailing underscore because `as` is a Rust keyword.
    pub fn as_<T>(&mut self) -> T {
        unsafe {
            // SAFETY: reinterpret the ext slot (`*?*anyopaque`) as a `*T`,
            // deref. The slot was allocated with `size_of::<T>()` and written
            // via [`set`] as a bare `T`, so read it as `T` directly. A nullable
            // pointer is one word with a null niche, but Rust's `Option<*mut T>`
            // is two words — wrapping in `Option<T>` here over-reads and
            // misinterprets the bytes. Callers pass pointer-ish `T` and
