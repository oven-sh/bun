...
/// `LIBUS_RECV_BUFFER_PADDING` (libusockets.h): writable bytes the loop keeps
/// on each side of its receive buffer. A parser given read bytes may touch
/// that many bytes past either end of them.
