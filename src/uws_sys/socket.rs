...
//!
//! THIS IS THE ONE CANONICAL `InternalSocket` / `NewSocketHandler`
//! implementation. `bun_uws` re-exports everything here; do NOT add a
//! parallel copy in `bun_uws` again — the earlier "thin placeholder"
//! that grew full bodies there has been deleted.
//!
//! Shape: `InternalSocket` is a `Copy` tagged raw pointer (passed by
//! value through the entire HTTP-client state machine), all
...
// ──────────────────────────────────────────────────────────────────────────
// `bun_uws_sys::CloseCode` (us_socket_t.rs) keeps the original snake-case variant
...
/// State of a single connection. `Copy` — the handle is passed by value
/// through the entire HTTP-client / Bun.Socket state machines, so it must
/// be a trivially-copyable tagged pointer. The `UpgradedDuplex` / `Pipe`
...
// `InternalSocket` equality — variant + pointer-identity equality.
// PORT NOTE: the `Pipe` arm returns `false` even for `(Pipe, Pipe)` on
// non-Windows (the variant carries no payload there, so identity is
// meaningless). Debug-asserts that compare sockets rely on this.
...
/// SSL/non-SSL socket handler. The const generic only
...
impl<const IS_SSL: bool> NewSocketHandler<IS_SSL> {
    ...
    /// Lower-case constructor form alias for `DETACHED`.
    #[inline]
    ...
    pub fn mark_needs_more_for_sendfile(&self) {
        // SSL sockets do not support sendfile yet — keep as a const assert.
    ...
    pub fn from_fd<This>(
        ...
        // `Option<NonNull<This>>` is null-niche optimized (8 bytes); the dispatch
        ...
        // ext storage was sized for a nullable pointer above; `raw` is a freshly-created
    ...
    pub fn connect_group<Owner>(
        ...
        // `Option<NonNull<Owner>>` is null-niche optimized (8 bytes); the dispatch
        // trampolines read the ext slot as `Option<NonNull<_>>`, so size and
    ...
    pub fn connect_unix_group<Owner>(
        ...
        // `Option<NonNull<Owner>>` — see connect_group above for layout rationale.
        let ext_size = size_of::<Option<NonNull<Owner>>>() as c_int;
    ...
    ///
    /// `set_socket_field` replaces a compile-time field-name reflection
    /// scheme — the closure writes the resulting
...
impl AnySocket {
    ...
    pub fn group(&self) -> *mut SocketGroup {
        // Always panics on a missing group via `.unwrap()`.
        match self {
