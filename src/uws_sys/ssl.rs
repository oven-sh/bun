...
use bun_boringssl_sys::{OwnedX509Store, SSL, SSL_CTX, SSL_SESSION, X509_STORE};
...
unsafe extern "C" {
    ...
    safe fn us_ssl_ctx_has_user_ca(ctx: &SSL_CTX) -> core::ffi::c_int;
}
...
/// Whether `ctx`'s store was configured from user CAs (`ca` / `addCACert` /
/// pfx extras) rather than being the empty `SSL_CTX_new()` store or the
/// shared default roots.
pub fn ctx_has_user_ca(ctx: &SSL_CTX) -> bool {
    us_ssl_ctx_has_user_ca(ctx) != 0
}
