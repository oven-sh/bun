...
// TODO(port): there is no structural reflection to decide which vtable
// slots are populated, so handlers implement this trait and set the
// `HAS_ON_*` associated consts for each method they actually provide.
// Default impls are `unreachable!()` and the corresponding vtable slot is
// left `None` when the const is `false`.
pub trait Handler: 'static {
    ...
    // TODO(port): the `HAS_EXT == false` path conceptually drops the `ext` arg
    // entirely (handlers take `(s, …)`). Rust can't change a trait method's
    // arity by a const, so the no-ext variants are separate methods. Only
    // called when `HAS_EXT == false`.
    #[allow(unused_variables)]
...
impl<H: Handler> Trampolines<H> {
    // Conditionally prepends the ext pointer to the arg tuple. Rust can't
    // splat tuples into a call, so each trampoline inlines the HAS_EXT branch.
    //
    // TODO(port): `H::Ext` is the *pointee* type directly and
