...
impl INotifyWatcher {
    pub fn watch_path(&mut self, pathname: &ZStr) -> bun_sys::Result<EventListIndex> {
        #[cfg(any())]
        {
            // TODO(b2-blocked): bun_sys::linux::inotify_add_watch
            // TODO(b2-blocked): bun_sys::linux::IN
            ...
            let rc = unsafe {
                bun_sys::linux::inotify_add_watch(self.fd.cast(), pathname.as_ptr(), watch_file_mask)
            };
            bun_core::scoped_log!(watcher, "inotify_add_watch({}) = {}", self.fd, rc);
            let result = if rc < 0 {
                Err(bun_sys::Error::from_code_int(bun_sys::last_errno(), bun_sys::Tag::TODO)
        ...
        let _ = pathname;
        todo!("INotifyWatcher::watch_path — bun_sys::linux::inotify_add_watch")
    }
    ...
    pub fn watch_dir(&mut self, pathname: &ZStr) -> bun_sys::Result<EventListIndex> {
        #[cfg(any())]
        {
            ...
            // SAFETY: fd is a valid inotify fd (loaded == true), pathname is NUL-terminated.
            let rc = unsafe {
                bun_sys::linux::inotify_add_watch(self.fd.cast(), pathname.as_ptr(), watch_dir_mask)
            };
            bun_core::scoped_log!(watcher, "inotify_add_watch({}) = {}", self.fd, rc);
            let result = if rc < 0 {
                Err(bun_sys::Error::from_code_int(bun_sys::last_errno(), bun_sys::Tag::TODO)
        ...
        let _ = pathname;
        todo!("INotifyWatcher::watch_dir — bun_sys::linux::inotify_add_watch")
    }

    pub fn unwatch(&mut self, wd: EventListIndex) {
        #[cfg(any())]
        {
            // TODO(b2-blocked): bun_sys::linux::inotify_rm_watch
            debug_assert!(self.loaded);
            let _ = self.watch_count.fetch_sub(1, Ordering::Release);
            // SAFETY: fd is a valid inotify fd (loaded == true).
            let _ = unsafe { bun_sys::linux::inotify_rm_watch(self.fd.cast(), wd) };
            return;
        }
        let _ = wd;
        todo!("INotifyWatcher::unwatch — bun_sys::linux::inotify_rm_watch")
    }
    ...
    pub fn init(&mut self, _root: &[u8]) -> Result<(), bun_core::Error> {
        #[cfg(any())]
        {
            // TODO(b2-blocked): bun_sys::linux::inotify_init1
            // TODO(b2-blocked): bun_sys::linux::IN
            debug_assert!(!self.loaded);
            self.loaded = true;
        ...
        todo!("INotifyWatcher::init — bun_sys::linux::inotify_init1")
    }

    pub fn read(&mut self) -> bun_sys::Result<&[*const Event]> {
        #[cfg(any())]
        {
            // TODO(b2-blocked): bun_sys::linux::{read, errno, Errno, pollfd, POLL, timespec, ppoll}
            debug_assert!(self.loaded);
                ...
                'outer: loop {
                    ...
                    match errno {
                        system::Errno::SUCCESS => {
                            ...
                            const DOUBLE_READ_THRESHOLD: usize =
                                Event::LARGEST_SIZE * (max_count / 2);
                            if read_len < DOUBLE_READ_THRESHOLD {
                                ...
                                let poll_n =
                                    unsafe { system::ppoll(&mut fds, &mut timespec, core::ptr::null()) }
                                        .unwrap_or(0);
                                if poll_n > 0 {
                                    'inner: loop {
            ...
            self.read_ptr = None;
            return Ok(&self.eventlist_ptrs[..count as usize]);
        }
        todo!("INotifyWatcher::read — bun_sys::linux raw syscalls")
    }
...
pub fn watch_loop_cycle(this: &mut Watcher) -> bun_sys::Result<()> {
    #[cfg(any())]
    {
        ...
        // `this.watchlist.items_eventlist_index()` typed-column accessor.
    ...
    let _ = this;
    todo!("watch_loop_cycle — bun_collections::MultiArrayElement derive")
}
...
pub fn watch_event_from_inotify_event(event: &Event, index: WatchItemIndex) -> WatchEvent {
    #[cfg(any())]
    {
        ...
        return WatchEvent { op, index, ..Default::default() };
    }
    let _ = (event, index);
    todo!("watch_event_from_inotify_event — bun_sys::linux::IN")
}
