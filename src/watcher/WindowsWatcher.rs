...
// Not yet declared in `bun_sys::windows::kernel32`.
#[link(name = "kernel32")]
unsafe extern "system" {
    fn CancelIoEx(hFile: HANDLE, lpOverlapped: *mut w::OVERLAPPED) -> w::BOOL;
}
...
impl WindowsWatcher {
    ...
    pub(crate) fn wake(&self) {
        // A packet without an OVERLAPPED; `dequeue` reports it as `Packet::Wake`.
    ...
    pub(crate) fn stop(&mut self) {
        if self.armed {
            ...
            // SAFETY: dir_handle is the open directory handle from init(); a null OVERLAPPED cancels all I/O issued on it.
            let _ = unsafe { CancelIoEx(self.watcher.dir_handle, ptr::null_mut()) };
            while self.armed {
