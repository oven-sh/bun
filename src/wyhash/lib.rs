...
// A single unaligned little-endian load of `BYTES` bytes. Mirrors the
// `Wyhash::read4`/`read8` treatment below (~lib.rs:600): the per-byte
// `[data[0], data[1], ...]` spelling left a cmp/je-to-panic ladder per byte
// in `WyhashStateless::final_`/`round`, and `final_` is the `StringHashMap`
// short-key hash — hit on every parser / resolver / module-registry HashMap
// probe during startup (identifier/path keys are <32B so `aligned_len == 0`
// and the whole key goes through `final_`). Every caller proves the length
// first (the `1..=31` arms of `final_` slice `rem_key = &b[0..rem_len]` with
// `rem_len` == the match scrutinee; `round` takes a `&[u8]` it
// `debug_assert!`s == 32), so emit a single bounds check.
#[inline(always)]
...
impl WyhashStateless {
    ...
    // bun_js_parser/bun_collections crate boundary. Force-inline the whole
    // hash into the caller.
    #[inline(always)]
...
/// `RuntimeTranspilerCache`, the router, and the `HashMap` autocontext.
///
...
impl Wyhash {
    ...
    // A single unaligned little-endian load. The previous per-byte
    // `[data[0], data[1], ...]` spelling left a cmp/je-to-panic ladder per
    // byte in the `small_key` disasm (8 bounds checks for the 4× `read4` at
    // len∈[4,16]). All call sites slice from a buffer whose length is already
    // proven ≥4/≥8 by the enclosing branch (`small_key`: `len >= 4`; `round`:
    // `&[u8; 48]`; `final1`: `i + 16 < len` / `len - 16` / `len - 8`), so
    // emit a single bounds check.
    #[inline(always)]
    ...
    pub fn hash(seed: u64, input: &[u8]) -> u64 {
        ...
        if input.len() <= 16 {
            ...
            if input.len() >= 48 {
                ...
                // safe-Rust spellings here — `input[i..i+48].try_into()`
                // plus per-byte shift-or — left a slice bounds-check + panic
                // edge per iteration and 48 scalar byte loads per round,
                // which made `Wyhash::hash` the #1 self-time symbol when
                // `RuntimeTranspilerCache` hashes multi-MB sources. Take the
                // base pointer once and issue six `read_unaligned::<u64>`
                // per round.
                //
...
/// it on every `init`/`shallow_copy` — a measurable zero-fill cost), this
/// folds each `write` through the stateless one-shot path seeded by the
/// running hash. No buffering, no per-key allocation.
///
/// Hash values are deterministic across runs (seed 0) but are NOT
/// bit-identical to a one-shot `Wyhash::hash(0, key)` because Rust's
/// `Hash for [T]` prepends a length prefix; that's fine — these are in-memory
/// tables only (lockfile/on-disk hashes go through [`Wyhash11`] / [`hash`]
/// directly).
...
impl core::hash::Hasher for OneShotHasher {
    ...
    /// `Wyhash11::hash(0, bytes)` — the exact shape of
...
/// for the length-prefix divergence from a raw `Wyhash::hash(0, asBytes(&k))`.
#[inline]
...
/// the runtime hash byte-for-byte). The runtime
...
/// Covers the dedicated widths (16/32/64) via a sealed trait. All current
/// callers pass `u32`.
#[inline]
...
mod tests {
    ...
    fn hash_const_matches_runtime() {
        ...
        // produces compile-time symbol suffixes that must match the runtime
        // `Wyhash::hash(0, name)` byte-for-byte. Guard drift.
        for s in [
            &b""[..],
            b"a",
            b"abc",
