#![allow(unused)]
// TODO(b1): bun_str::ZStr missing — local stub until lower-tier exposes it.
type ZStr = core::ffi::CStr;

// ─── B-1 stub surface ─────────────────────────────────────────────────────
// Phase-A draft bodies preserved below in `#[cfg(any())] mod draft`.
// Un-gating happens in B-2 once zstd_sys / thiserror / bun_str land.
...
pub fn compress(_dest: &mut [u8], _src: &[u8], _level: Option<i32>) -> Result {
    todo!("b1-stub: zstd::compress")
}

pub fn compress_bound(_src_size: usize) -> usize {
    todo!("b1-stub: zstd::compress_bound")
}

pub fn decompress(_dest: &mut [u8], _src: &[u8]) -> Result {
    todo!("b1-stub: zstd::decompress")
}

pub fn decompress_alloc(_src: &[u8]) -> core::result::Result<Vec<u8>, ZstdError> {
    todo!("b1-stub: zstd::decompress_alloc")
}

pub fn get_decompressed_size(_src: &[u8]) -> usize {
    todo!("b1-stub: zstd::get_decompressed_size")
}

/// Opaque B-1 stub. Real fields gated in `draft::ZstdReaderArrayList`.
pub struct ZstdReaderArrayList<'a> {
    _input: &'a [u8],
    _list_ptr: &'a mut Vec<u8>,
}
...
impl<'a> ZstdReaderArrayList<'a> {
    ...
    pub fn read_all(&mut self, _is_done: bool) -> core::result::Result<(), ZstdError> {
        todo!("b1-stub")
    }
}

// ─── Phase-A draft (gated; preserved for B-2) ─────────────────────────────
#[cfg(any())]
mod draft {
