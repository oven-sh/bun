...
devTest("html route strips its link tag when the stylesheet is bundled alongside a failing version of itself", {
  // When a stylesheet fails, the route that links it is re-bundled together
  // with the broken stylesheet (on first load, or when the route is requested
  // while it is broken). The HTML generated by that bundle is what gets served
  // once the stylesheet recovers, since only the stylesheet is rebuilt then.
  files: {
    "index.html": emptyHtmlFile({
      styles: ["styles.css"],
  ...
  async test(dev) {
    ...
    // A stylesheet that parses but fails import resolution, then recovery.
    await dev.write(
      "styles.css",
      `
...
devTest("script importing a stylesheet still boots after the stylesheet fails and recovers", {
  // Same as above for a stylesheet imported from JavaScript: the importer is
  // re-bundled alongside the broken stylesheet, and the module it produced
  // then must not list the stylesheet as a JavaScript dependency.
  files: {
    "index.html": emptyHtmlFile({
      scripts: ["index.ts"],
      body: `<div class="a">hello</div>`,
