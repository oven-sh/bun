...
devTest("html route strips its link tag when the stylesheet is bundled alongside a failing version of itself", {
  // The route that links a broken stylesheet gets bundled together with it on
  // first load, when the route is requested while it is broken, and when the
  // HTML file itself is edited while it is broken. The HTML generated by that
  // bundle is what gets served once the stylesheet recovers, since only the
  // stylesheet is rebuilt then.
  files: {
    "index.html": emptyHtmlFile({
      styles: ["styles.css"],
  ...
  async test(dev) {
    ...
    const recovered = await fetchRouteHtml(dev);
    const links = recovered.match(/<link [^>]*>/g);
    expect(links).toEqual([expect.stringMatching(injectedStylesheetLink)]);
    await dev.fetch(links![0].match(injectedStylesheetLink)![1]).expect.toMatch(/color:\s*#00f/);
    ...
    expect(await fetchRouteHtml(dev)).toBe(recovered);

    // A stylesheet that parses but fails import resolution, HTML file edited
    // while broken, then recovery.
    await dev.write(
      "styles.css",
      `
...
devTest("script importing a stylesheet still boots after the stylesheet fails and recovers", {
  // Same three situations as above for a stylesheet imported from JavaScript:
  // the module generated for the importer while the stylesheet is broken must
  // not list the stylesheet as a JavaScript dependency.
  files: {
    "index.html": emptyHtmlFile({
      scripts: ["index.ts"],
      body: `<div class="a">hello</div>`,
...
const injectedStylesheetLink = /^<link rel="stylesheet" href="(\/_bun\/asset\/[0-9a-f]{16}\.css)">$/;

/**
 * Serves "/" and masks the route script's URL, which embeds a generation
 * number that changes every time the route is re-bundled.
 */
async function fetchRouteHtml(dev: Dev): Promise<string> {
