// DevServer debugging features: the visualizer pages under `/_bun/`, the
// incremental graph feed behind them, and the `.bake-debug` source dumps.
//
// The visualizers exist in builds with `BAKE_DEBUGGING_FEATURES` (canary or
// debug). The `.bake-debug` dumps are written by debug builds only.
...
// `feature_flags::BAKE_DEBUGGING_FEATURES` is `IS_CANARY || IS_DEBUG`; canary
// builds are the ones whose version string carries a `-canary.N` tag.
...
function decodeVisualizerFrame(buffer: Uint8Array): VisualizerGraph {
  ...
  // Files and edges are emitted in the graph's internal slot order, which the
  // visualizer does not care about; sort (by code unit) so the assertions
  // below do not depend on it either.
...
/**
 * Opens a second `/_bun/hmr` socket and subscribes it to the incremental
 * visualizer topic (`s` = subscribe, `v` = topic), exactly like the page served
 * at `/_bun/incremental_visualizer` does.
 */
...
describe.skipIf(!hasBakeDebuggingFeatures)("visualizers", () => {
  devTest("the visualizer pages are served under /_bun/", {
    files: {
      // A single HTML file is mounted at `/*`, so the pages must win over the
      // app's catch-all (which otherwise serves index.html for these URLs).
  ...
  devTest("subscribing to the incremental visualizer topic streams the incremental graph", {
    ...
    async test(dev) {
      ...
      // Subscribing answers with the current graph right away. Nothing has
      // been requested yet, so both graphs are empty.
  ...
  devTest("the incremental visualizer frame covers the server graph", {
    ...
    async test(dev) {
      // Paths are sent relative to the project root, so the framework's entry
      // point (which lives in the repository) shows up as a `../` path.
      ...
      // Scanning the routes at startup registers the route files and the
      // framework entry point as stale server files.
      ...
      // A "use client" component is a boundary in the server graph and an HMR
      // root (the same flag byte) in the client graph. Its server-side stub
      // imports `registerClientReference` from the framework entry point.
      ...
      // Dropping the directive demotes the boundary, which deletes the
      // component's file from the client graph. A deleted file keeps its slot
      // (indices stay valid) and is sent as an empty name.
...
// The dev server is spawned with the project root as its cwd, which is where
// `.bake-debug` is created.
...
describe.skipIf(!isDebug)(".bake-debug dumps", () => {
  devTest("client bundles are dumped to .bake-debug", {
    ...
    async test(dev) {
      ...
      // The chunk and the source map handed to the browser are dumped as the
      // latest ones, byte for byte.
  ...
  devTest("server bundles are dumped to .bake-debug", {
    ...
    async test(dev) {
      ...
      // Files outside the project root (here the framework's server entry
      // point) keep their relative path, with each `..` made into a directory name.
      ...
      // The server chunk is loaded straight into the server VM; the dump is
      // the only way to read it, alongside the source map it was loaded with.
