...
// Bun.build hands the calling VM's dotenv loader to the transpiler it sets up on
// the bundle thread. Whether loading env files prints the `[0.12ms] ".env"`
// line is a setting on that loader, owned by the VM, derived from the VM's own
// log level. That transpiler used to re-derive it from Bun.build's log level and
// write it back into the VM's loader (as did `new Bun.Transpiler({ logLevel })`
// on the JS thread). A Worker's VM is where the two levels can differ, so the
// line must show up during a build exactly when it showed up while the worker
// started. Which row of the matrix actually differs depends on the build: in a
// debug build the default row does (worker logs at info, Bun.build at warn), in
// a release build the bunfig row does (worker logs at warn, Bun.build at debug).
describe.concurrent("Bun.build does not change the calling VM's env loader output", () => {
  const envLines = (s: string) => s.split('".env"').length - 1;

  test.each([
    { name: "default log level", bunfig: null, transpilerLogLevel: "error" },
    { name: 'logLevel = "debug"', bunfig: 'logLevel = "debug"\n', transpilerLogLevel: "verbose" },
  ])("$name", async ({ bunfig, transpilerLogLevel }) => {
    using dir = tempDir("bun-build-env-loader-output", {
      ".env": "FOO=bar\n",
      ...
      "main.ts": `
        ...
        worker.terminate();
      `,
      ...(bunfig === null ? {} : { "bunfig.toml": bunfig }),
    });
    ...
    const segments = stderr.split(/--(?:main started|worker started|build 1 done|build 2 done)--\r?\n/);
    expect(segments).toHaveLength(5);
    const [mainStartup, workerStartup, build1, build2] = segments;
    // The main VM logs at the process-wide level Bun.build's transpiler uses,
    // so this pins down what that transpiler would have written.
    expect(envLines(mainStartup)).toBe(bunfig === null ? 0 : 1);
    const atWorkerStartup = envLines(workerStartup);
    expect({ build1: envLines(build1), build2: envLines(build2) }).toEqual({
      build1: atWorkerStartup,
      build2: atWorkerStartup,
    });
    expect(stdout).toBe("");
    expect(exitCode).toBe(0);
  });
