...
import { bunEnv, bunExe, hostLooksNix, isArm64, isLinux, isMacOS, isMusl, isPosix, isWindows, tempDir } from "harness";
...
if (isLinux) {
  describe("ELF section", () => {
    ...
    // On a Nix/Guix host the final `Bun.spawn({cmd:[outfile]})` would fail because
    // the FHS ldso is a stub there, not because the fix is broken.
