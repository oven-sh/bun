...
server.close();`,
  ...
  // A Windows path buffer holds 98302 bytes, so the path would fit there and
  ...
  test.skipIf(isWindows)(
    ...
    async () => {
      ...
      expect(stdout + stderr).toContain(`is too long (File name too long): ${longDir}`);
...
describe.concurrent("compile with an outfile longer than a path buffer", () => {
  ...
  const variants: Record<string, { options: string; shown: (long: string, dir: string) => string }> = {
    ...
    "relative compile.outfile for another platform": {
      options: `{ compile: { outfile: long, target: "bun-windows-x64" } }`,
      shown: long => long,
    },
  ...
  // The CLI names the embedded entry point after --outfile, so the bundler's limit on
  // output paths rejects an over-long name before the executable writer ever sees it.
  // 5000 bytes still fits in a command line on every platform (100_000 would not on
  // Windows). On Windows it also fits a path buffer, and the OS rejects it at the end.
  test("bun build --compile --outfile of 5000 bytes fails instead of crashing", async () => {
    ...
    if (isWindows) {
      ...
      expect(stderr).toContain(
        `Output path for "app.js" is too long (${"./".length + long.length} bytes, the limit on this platform is ${MAX_PATH_BYTES - 1 - ".map".length})`,
      );
      // Nothing was written: neither the executable nor a temporary copy of it.
