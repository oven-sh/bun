...
import { chmodSync, closeSync, copyFileSync, openSync, readSync, rmSync, truncateSync } from "fs";
...
// `bun build --compile` on Linux extends the writable PT_LOAD segment to cover the
// appended module graph. A truncated download (common failure mode for large binaries)
// still satisfies the kernel's ELF loader, which maps the full `p_filesz` range regardless
