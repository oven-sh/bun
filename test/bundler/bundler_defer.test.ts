...
`,
  ...
  const filesForTestingErrorCases = {
    "/index.ts": /* ts */ `
      import { lmao } from "./lmao.ts";
      console.log("Foo", foo, lmao);
    `,
  ...
  itBundled("calling defer after returning", {
    experimentalCss: true,
    files: filesForTestingErrorCases,
    entryPoints: ["index.ts"],
    plugins: [
      {
        name: "demo",
        setup(build) {
          build.onLoad({ filter: /foo\.ts$/ }, ({ defer, path }) => {
            setImmediate(() => defer());
            return {
              contents: `e`,
              loader: "js",
          ...
          build.onLoad({ filter: /\.(ts)/ }, ({ defer, path }) => {
            if (path.includes("index.ts")) {
              return undefined;
            }
            setImmediate(() => defer());
