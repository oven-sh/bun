...
describe("bundler", () => {
  ...
  itBundled("mangle-props/JSXShorthandWithPropertyKeyComment", {
    files: {
      "/entry.jsx": /* jsx */ `
        function h(tag, props) { return JSON.stringify({ tag, props }); }
        const foo = { bar_: 1, plain: 2 };
        console.log(<div {foo[/* @__KEY__ */ "bar_"]} {foo["plain"]} />);
      `,
    },
    jsx: { runtime: "classic", factory: "h" },
    mangleProps: /_$/,
    // The shorthand prop takes its name from the index, mangled or not.
    run: { stdout: '{"tag":"div","props":{"a":1,"plain":2}}' },
  });

  itBundled("mangle-props/MangledKeysAreStillConstants", {
    files: {
      "/entry.js": /* js */ `
        const unusedObject = { ["foo_"]: 1 };
        class UnusedClass { ["foo_"] = 2 }
        const unusedAnnotated = { [/* @__KEY__ */ "bar_"]: 3 };
        /* @__KEY__ */ "unusedStatement_";
        console.log(typeof /* @__KEY__ */ "baz_", typeof ((/* @__KEY__ */ "foo_") ?? unusedFallback()));
      `,
    },
    minifySyntax: true,
    mangleProps: /_$/,
    mangleQuoted: true,
    // A mangled name is a constant string: the unused object, class and
    // statement are dead code, typeof folds, and ?? keeps only its left side.
    run: { stdout: "string string" },
    onAfterBundle(api) {
      const code = api.readFile("/out.js");
      expect(code).not.toMatch(/unusedObject|UnusedClass|unusedAnnotated|unusedStatement_|unusedFallback/);
      expect(code).toContain('console.log("string", "string")');
    },
  });
