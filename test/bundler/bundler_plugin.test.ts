...
describe("bundler", () => {
  ...
  // An import's `with { type }` loader applies to the path that onResolve
  // returned, and that loader is part of the module's identity: one file
  // imported as text and as JSON is two modules. The second text import finds
  // the text module again, not the JSON one.
    ...
    return {
      files: {
        "index.ts": /* ts */ `
          ...
          import again from "virtual:data" with { type: "text" };
          console.log(JSON.stringify({ text: asText.trim(), json: asJson, again: again === asText }));
        `,
        "as-text.ts": /* ts */ `
      ...
      run: { stdout: '{"text":"{\\"a\\":1}","json":{"a":1},"again":true}' },
    };
  });
