...
test("issue 28921: same asset from HTML and CSS keeps the emitted file for the HTML reference", async () => {
  using dir = tempDir("28921-html-and-css", {
    ...
    "src/font.woff2": Buffer.alloc(256, 0x43).toString(),
    "src/styles.css": `
@font-face { font-family: 'F'; src: url('./font.woff2') format('woff2'); }
body { background: url('./logo.png'); font-family: 'F'; }
`,
...
<html>
  ...
  // Both CSS references are inlined.
  const cssText = await result.outputs.find(o => o.path.endsWith(".css"))!.text();
  expect(cssText).toContain("data:image/png;base64");
  expect(cssText).toContain("data:font/woff2;base64");

  // The <img>, <link rel="icon"> and <link rel="preload" as="font"> tags
  // reference the files by URL, so the physical files must be emitted and
  // every attribute rewritten to the hashed path.
  const htmlText = await result.outputs.find(o => o.path.endsWith(".html"))!.text();
  expect(htmlText).not.toContain("./logo.png");
  expect(htmlText).not.toContain("./font.woff2");
  expect(htmlText).toMatch(/rel="icon" href="\.\/logo-[^"]+\.png"/);
  expect(htmlText).toMatch(/<img src="\.\/logo-[^"]+\.png"/);
  expect(htmlText).toMatch(/as="font" href="\.\/font-[^"]+\.woff2"/);
  const assets = readdirSync(join(String(dir), "dist"))
    .filter(f => f.endsWith(".png") || f.endsWith(".woff2"))
    .map(f => f.replace(/-[^.]+\./, "-[hash]."))
    .sort();
  expect(assets).toEqual(["font-[hash].woff2", "logo-[hash].png"]);
});
