...
//
// Modes:
//   entry          in-memory (`files`) entry point at the long path, named without [dir]
//   entry-dir      like `entry`, with the default entry naming, which starts with [dir]
//                  (the root of an in-memory build is the cwd, so the output path
//                  contains the long directory too)
//   entry-on-disk  entry point on disk at the long path (the root is its directory)
...
//                  and the HTML chunk is emitted under the long directory)
...
//
// A plugin or the `files` map can name a path of any length, so `entry`,
// `entry-dir` and the last two modes are the ones that accept a `length` beyond
// the path buffer.
...
switch (mode) {
  ...
  case "entry-dir": {
    ...
    options = {
      entrypoints: [path],
      files: { [path]: "console.log(1);" },
      naming: mode === "entry" ? "[name].[ext]" : undefined,
    };
    break;
  }
  case "entry-on-disk": {
...
console.log(
  JSON.stringify({
    ...
    // Only a BuildMessage has notes.
    notes: result.logs.flatMap(log => (log as { notes?: { message: string }[] }).notes ?? []).map(note => note.message),
    cwd: process.cwd(),
    path,
