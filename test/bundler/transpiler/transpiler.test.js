...
console.log(foo, array);
    ...
    it("keeps `(0, value)` where a fold would let a binding name an anonymous function or class", () => {
      ...
      expectPrinted("x = ({}).y ??= class {}", "x = (0, class {\n})");
      expectPrinted("x = ({}).y ||= () => {}", "x = (0, () => {})");
      ...
      // A call target that is a member access keeps the comma, so the call passes no `this`.
      expectPrinted("([] ? obj.m : 0)()", "(0, obj.m)()");
      expectPrinted("(1 ? obj.m : 0)()", "(0, obj.m)()");
      expectPrinted("(({}).y ??= obj.m)()", "(0, obj.m)()");
      expectPrinted("x = ({}).y ??= obj.m", "x = obj.m");
    });
