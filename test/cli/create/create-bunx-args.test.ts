...
function createTarball(name: string, version: string, binName: string, script: string): Uint8Array {
  ...
  for (const [path, content] of Object.entries(files)) {
    ...
    // tar checksum = sum of header bytes, checksum field treated as spaces.
    let checksum = 0;
    for (let i = 0; i < 512; i++) {
...
describe("issue #29087", () => {
  ...
  /** Run `bun create <args>` against the local registry; return the argv the create script received. */
  async function runCreate(...createArgs: string[]): Promise<string[]> {
    // Isolate the bunx cache (derived from TMPDIR) per invocation so
    // concurrent tests don't race on /tmp/bunx-<uid>-<pkg>.
  ...
  test.concurrent("strips a single leading `--` separator before forwarding to the create script", async () => {
    const argv = await runCreate("bun-issue29087-argv-printer", "--", "-t", "v3");
    expect(argv).toEqual(["-t", "v3"]);
  });

  test.concurrent("forwards long-form flags with no `--` separator unchanged", async () => {
    const argv = await runCreate("bun-issue29087-argv-printer", "--template", "v3");
    expect(argv).toEqual(["--template", "v3"]);
  });

  test.concurrent("preserves a second `--` as a literal (only the first is the separator)", async () => {
    const argv = await runCreate("bun-issue29087-argv-printer", "--", "--", "-t", "v3");
    expect(argv).toEqual(["--", "-t", "v3"]);
  });

  test.concurrent("does not leak `--bun` (consumed by wrapper) into forwarded argv", async () => {
    const argv = await runCreate("bun-issue29087-argv-printer", "--bun", "--", "-t", "v3");
    expect(argv).toEqual(["-t", "v3"]);
  });

  test.concurrent("preserves `--bun` after the separator as a literal arg for the create script", async () => {
    const argv = await runCreate("bun-issue29087-argv-printer", "--", "--bun", "-t", "v3");
