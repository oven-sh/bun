...
test("inspector listen failure warns but does not kill the app", async () => {
  ...
  expect(stripped).toContain("Failed to start inspector");
  // Exactly one warning line, no internal source dump or stack trace
  expect(stripped.match(/Failed to start inspector/g)?.length).toBe(1);
  expect(stripped).not.toContain("internal:debugger");
  expect(stripped).not.toContain("Bun.serve(");
