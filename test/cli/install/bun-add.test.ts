...
describe.concurrent("bun-add", () => {
  it("should add existing package", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      const add_path = relative(ctx.package_dir, add_dir);
  ...
  it("bun add --analyze should scan dependencies", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await writeFile(join(ctx.package_dir, "entry-point.ts"), `import "./local-file.ts";`);
      await writeFile(join(ctx.package_dir, "local-file.ts"), `export * from "bar";`);
      console.log(ctx.package_dir);
      const { stdout, stderr, exited } = spawn({
    ...
    it.each(["file:///", "file://", "file:/", "file:", "", "//////"])(
      ...
      async protocolPrefix => {
        await withContext(defaultOpts, async ctx => {
          ...
          const add_path_rel = relative(ctx.package_dir, add_dir);
  ...
  it.each(["fileblah://"])("should reject invalid path without segfault: %s", async protocolPrefix => {
    await withContext(defaultOpts, async ctx => {
      ...
      const add_path = relative(ctx.package_dir, add_dir).replace(/\\/g, "\\\\");
  ...
  it("should handle semver-like names", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      const err = await stderr.text();
      expect(err.split(/\r?\n/)).toContain(`error: GET ${ctx.registry_url}1.2.3 - 404`);
      const out = await stdout.text();
      expect(out).toEqual(expect.stringContaining("bun add v1."));
      expect(await exited).toBe(1);
      expect(urls.sort()).toEqual([`${ctx.registry_url}1.2.3`]);
  ...
  it("should handle @scoped names", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      const err = await stderr.text();
      expect(err.split(/\r?\n/)).toContain(`error: GET ${ctx.registry_url}@bar%2fbaz - 404`);
      const out = await stdout.text();
      expect(out).toEqual(expect.stringContaining("bun add v1."));
      expect(await exited).toBe(1);
      expect(urls.sort()).toEqual([`${ctx.registry_url}@bar%2fbaz`]);
      expect(ctx.requested).toBe(1);
      try {
        await access(join(ctx.package_dir, "bun.lockb"));
        expect(() => {}).toThrow();
  ...
  it("should add exact version with install.exact", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await appendFile(join(ctx.package_dir, "bunfig.toml"), `exact = true\n`);
      const { stdout, stderr, exited } = spawn({
        cmd: [bunExe(), "add", "BaR"],
        cwd: ctx.package_dir,
        stdout: "pipe",
  ...
  it("should add dependency with package.json in it and http tarball", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      expect(ctx.requested).toBe(2);
      expect(await readdirSorted(join(ctx.package_dir, "node_modules"))).toEqual([".bin", ".cache", "bap", "booop"]);
      expect(await readdirSorted(join(ctx.package_dir, "node_modules", ".bin"))).toHaveBins(["baz-run"]);
      expect(await readdirSorted(join(ctx.package_dir, "node_modules", "bap"))).toEqual(["index.js", "package.json"]);
      expect(await file(join(ctx.package_dir, "node_modules", "bap", "package.json")).json()).toEqual({
        name: "baz",
        version: "0.0.5",
        bin: {
          "baz-exec": "index.js",
      ...
      expect(await file(join(ctx.package_dir, "package.json")).json()).toStrictEqual({
        name: "foo",
        version: "0.0.1",
        dependencies: {
          bap: "npm:baz@0.0.5",
  ...
  it("should add dependency alongside workspaces", async () => {
    await withContext({ linker: "isolated" }, async ctx => {
      ...
      await writeFile(
        join(ctx.package_dir, "packages", "bar", "package.json"),
        JSON.stringify({
          name: "bar",
          version: "0.0.2",
        }),
  ...
  it("should handle Git URL in dependencies (SCP-style)", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      expect((await readdirSorted(join(ctx.package_dir, "node_modules", ".cache")))[0]).toBe("9d05c118f06c3b4c.git");
      expect(await readdirSorted(join(ctx.package_dir, "node_modules", "uglify-js"))).toEqual([
        ".bun-tag",
        ".gitattributes",
        ".github",
  ...
  it("should not save git urls twice", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      const package_json_content = await file(join(ctx.package_dir, "package.json")).json();
      ...
      const package_json_content2 = await file(join(ctx.package_dir, "package.json")).json();
  ...
  it("should add dependency without duplication (GitHub)", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      const package_json1 = await file(join(ctx.package_dir, "node_modules", "uglify-js", "package.json")).json();
      ...
      const package_json2 = await file(join(ctx.package_dir, "node_modules", "uglify-js", "package.json")).json();
  ...
  it("should add dependencies to workspaces directly", async () => {
    await withContext({ linker: "isolated" }, async ctx => {
      ...
      await writeFile(join(ctx.package_dir, "package.json"), barPackage);
      ...
      const add_path = relative(join(ctx.package_dir, "moo"), add_dir);
      const dep = `file:${add_path}`.replace(/\\/g, "/");
      const { stdout, stderr, exited } = spawn({
      ...
      expect(out.replace(/\s*\[[0-9\.]+m?s\]\s*$/, "").split(/\r?\n/)).toEqual([
        expect.stringContaining("bun add v1."),
        "",
        `installed foo@${relative(ctx.package_dir, add_dir).replace(/\\/g, "/")}`,
        "",
        "2 packages installed",
      ]);
      expect(await exited).toBe(0);
      expect(await readdirSorted(join(ctx.package_dir))).toEqual([
        "bun.lockb",
        "bunfig.toml",
        "moo",
        "node_modules",
      ...
      expect(await file(join(ctx.package_dir, "package.json")).text()).toEqual(barPackage);
      expect(await readdirSorted(join(ctx.package_dir, "moo"))).toEqual([
        "bunfig.toml",
        "node_modules",
        "package.json",
      ]);
      expect(await readdirSorted(join(ctx.package_dir, "moo", "node_modules", "foo"))).toEqual(["package.json"]);
      if (process.platform === "win32") {
        expect(await file(join(ctx.package_dir, "moo", "node_modules", "foo", "package.json")).json()).toEqual(
          fooPackage,
        );
      } else {
        expect(await file(join(ctx.package_dir, "moo", "node_modules", "foo", "package.json")).json()).toEqual(
      ...
      expect(await file(join(ctx.package_dir, "moo", "package.json")).json()).toEqual({
        name: "moo",
        version: "0.3.0",
        dependencies: {
  ...
  it("should redirect 'install --save X' to 'add'", async () => {
    await withContext(defaultOpts, async ctx => {
      await installRedirectsToAdd(ctx, true);
    });
  });

  it("should redirect 'install X --save' to 'add'", async () => {
    await withContext(defaultOpts, async ctx => {
      await installRedirectsToAdd(ctx, false);
    });
  });

  async function installRedirectsToAdd(ctx: TestContext, saveFlagFirst: boolean) {
    const add_dir = tmpdirSync();
    await writeFile(
      join(add_dir, "package.json"),
      JSON.stringify({
    ...
    expect(await exited).toBe(0);
    expect(await file(join(ctx.package_dir, "package.json")).text()).toInclude("bun.test.");
  }
  ...
  it("should add local tarball dependency", async () => {
    await withContext(defaultOpts, async ctx => {
      ...
      await copyFile(absolutePath, join(ctx.package_dir, tarball));
      const { stdout, stderr, exited } = spawn({
      ...
      expect(ctx.requested).toBe(0);
      expect(await readdirSorted(join(ctx.package_dir, "node_modules", "baz"))).toEqual(["index.js", "package.json"]);
      const package_json = await file(join(ctx.package_dir, "node_modules", "baz", "package.json")).json();
      expect(package_json.name).toBe("baz");
      expect(package_json.version).toBe("0.0.3");
      (expect(await file(join(ctx.package_dir, "package.json")).text()).toInclude('"baz-0.0.3.tgz"'),
        await access(join(ctx.package_dir, "bun.lockb")));
    });
  });
