...
// points at `bin/<name>.exe` (a text placeholder stub with no shebang that
// postinstall is meant to replace) while the platform optionalDependency ships
// the real binary at the package root under the bin name. The redirect must
// find it there instead of linking the un-execable stub. This one runs on
// Windows too, where the redirect used to be disabled entirely.
...
`,
      ...
      const binPath = join(binDir, isWindows ? "altpath-cmd.exe" : "altpath-cmd");
      // The bin must resolve into the platform-specific package, not back into
      // the parent package's placeholder stub. POSIX: follow the symlink.
      // Windows: the `.bunx` shim metadata (UTF-16LE) embeds the target path.
      const resolvedTarget = isWindows
        ? readFileSync(join(binDir, "altpath-cmd.bunx")).toString("utf16le")
        : realpathSync(binPath);
      expect(resolvedTarget).toContain("test-native-binlink-altpath-target");
