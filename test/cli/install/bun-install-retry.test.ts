...
describe.each(["hoisted", "isolated"])("linker=%s", linker => {
  ...
  describe("a failed download that an optional and a required dependency share", () => {
    let tarball: string;
    /** How many of the next tarball requests get a 404. */
    let failures: number;
    let tarballGets: number;
    let tarballFailed: PromiseWithResolvers<void>;

    function serveTarball(request: Request) {
      if (new URL(request.url).pathname !== "/baz-0.0.3.tgz") return new Response("unexpected", { status: 404 });
      tarballGets++;
      if (failures === 0) return new Response(file(join(import.meta.dir, "baz-0.0.3.tgz")));
      failures--;
    ...
    // A registry with baz@0.0.3 (the tarball above) and `packages`, each at 1.0.0.
    async function serveRegistry(packages: Record<string, object> = {}) {
      const tarballs = new Map<string, Uint8Array>();
      for (const [name, pkg] of Object.entries(packages)) {
        tarballs.set(`/${name}-1.0.0.tgz`, await packageTarball(name, pkg));
      }
      const manifest = (name: string, version: string, pkg: object = {}) =>
        Response.json({
          name,
          "dist-tags": { latest: version },
          versions: { [version]: { name, version, ...pkg, dist: { tarball: `${root_url}/${name}-${version}.tgz` } } },
        });
      setHandler(request => {
        const { pathname } = new URL(request.url);
        const name = pathname.slice(1);
        if (name === "baz") return manifest("baz", "0.0.3");
        if (name in packages) return manifest(name, "1.0.0", packages[name]);
        const packed = tarballs.get(pathname);
        return packed ? new Response(packed) : serveTarball(request);
      });
    }

    beforeEach(async () => {
      tarball = `${root_url}/baz-0.0.3.tgz`;
      failures = 0;
      tarballGets = 0;
      tarballFailed = Promise.withResolvers();
      setHandler(serveTarball);
      await writeFile(
        join(package_dir, "bunfig.toml"),
    ...
    async function install(pkg: object, ...args: string[]) {
      await writeFile(join(package_dir, "package.json"), JSON.stringify({ name: "foo", version: "0.0.1", ...pkg }));
    ...
    async function failTarballOnColdCache(times = Infinity) {
      await rm(join(package_dir, "node_modules"), { recursive: true, force: true });
      failures = times;
      tarballGets = 0;
    }

    const installed = { warnLines: [], errorLines: [], tarballGets: 1, exitCode: 0 };
    const warned = () => ({ warnLines: [`warn: GET ${tarball} - 404`], errorLines: [], tarballGets: 1, exitCode: 0 });
    // The isolated linker names the package by its resolution: a version, or the tarball URL.
    const errorLine = (resolution = tarball) =>
      linker === "hoisted"
    ...
    const failed = (resolution?: string) => ({
      warnLines: [],
    ...
    const failedAgain = (resolution?: string) => ({
    ...
    // bun.lock resolves `a`, so only the new `b` asks for the tarball while
    ...
    it("reports the required one after the optional one's download failed", async () => {
      expect(await install({ dependencies: { a: tarball } })).toEqual(installed);
      await failTarballOnColdCache();
      expect(await install({ dependencies: { a: tarball }, optionalDependencies: { b: tarball } })).toEqual(
        failedAgain(),
      );
    });
    ...
    it("installs the required one when its own download succeeds", async () => {
      expect(await install({ dependencies: { a: tarball } })).toEqual(installed);
      await failTarballOnColdCache(1);
      expect(await install({ dependencies: { a: tarball }, optionalDependencies: { b: tarball } })).toEqual({
        warnLines: [`warn: GET ${tarball} - 404`],
        errorLines: [],
        tarballGets: 2,
      ...
      expect(await file(join(package_dir, "node_modules", "a", "package.json")).json()).toMatchObject({ name: "baz" });
    });

    // bun.lock resolves both. optionalDependencies sort first, so the installer
    // starts the download for `b` and `a` joins it.
    it("reports the required one when it joins the optional one's download", async () => {
      const pkg = { dependencies: { a: tarball }, optionalDependencies: { b: tarball } };
      expect(await install(pkg)).toEqual(installed);
      await failTarballOnColdCache();
      expect(await install(pkg)).toEqual(failed());
    });
    ...
    it("reports the required alias of an npm package after the optional alias's download failed", async () => {
      ...
      failures = Infinity;
      expect(
        await install({ dependencies: { a: "npm:baz@0.0.3" }, optionalDependencies: { b: "npm:baz@0.0.3" } }),
      ).toEqual(failedAgain("0.0.3"));
    });
    ...
    it("reports the required one when a callback is still queued for the failed download", async () => {
      const d = await packageTarball("d", { optionalDependencies: { c: tarball } });
      setHandler(async request => {
        if (new URL(request.url).pathname !== "/d.tgz") return serveTarball(request);
        await tarballFailed.promise;
      ...
      expect(
        await install({
          dependencies: { a: tarball, d: `${root_url}/d.tgz` },
          optionalDependencies: { b: tarball },
        }),
      ).toEqual(failedAgain());
    });
    ...
    it("reports the required one when a peer asked for the failed download in between", async () => {
      ...
      expect(
        await install({
          dependencies: { a: tarball },
          optionalDependencies: { b: tarball },
          peerDependencies: { p: tarball },
        }),
      ).toEqual(failedAgain());
    });
    ...
    describe("when the optional dependency is the one the linker asks through", () => {
      ...
      beforeEach(() => serveRegistry({ bar: { dependencies: { baz: "0.0.3" } } }));

      it("reports the required one on a fresh install", async () => {
        failures = Infinity;
        expect(await install(pkg)).toEqual(failedAgain("0.0.3"));
      });

      it("reports the required one from bun.lock on a cold cache", async () => {
        expect(await install(pkg)).toEqual(installed);
        await failTarballOnColdCache();
        expect(await install(pkg)).toEqual(failed("0.0.3"));
      });
      ...
      it("reports the required one that --offline does not find in the cache", async () => {
        ...
        for (const dir of [".", ".bun", ".cache"]) {
          for (const entry of await readdir(join(node_modules, dir)).catch(() => [])) {
            if (entry.startsWith("baz")) await rm(join(node_modules, dir, entry), { recursive: true, force: true });
          }
        }
        tarballGets = 0;
        expect(await install(pkg, "--offline")).toEqual({
          warnLines: [],
      ...
      it.each(["optionalDependencies", "dependencies"])(
        ...
        async first => {
          ...
          await serveRegistry({ x: { [first]: { baz: "0.0.3" } }, y: { [second]: { baz: "0.0.3" } } });
          const parents = { dependencies: { x: "1.0.0", y: "1.0.0" } };
          expect(await install(parents)).toEqual(installed);
          await failTarballOnColdCache();
          expect(await install(parents)).toEqual(failed("0.0.3"));
        },
      );
    });

    // Nothing that this install links requires baz, so its failed download stays a
    // warning and the install succeeds. Hoisted only: the isolated linker fails the
    ...
    describe.skipIf(linker === "isolated")("stays a warning", () => {
      it("when only a devDependency requires the package and --production skips it", async () => {
        await serveRegistry({ bar: { dependencies: { baz: "0.0.3" } } });
        const pkg = { devDependencies: { bar: "1.0.0" }, optionalDependencies: { baz: "0.0.3" } };
        expect(await install(pkg)).toEqual(installed);
        await failTarballOnColdCache();
        expect(await install(pkg, "--production")).toEqual(warned());
      });
      ...
      it("when only a workspace that --filter skips requires the package", async () => {
        ...
        for (const [name, pkg] of Object.entries({
          ...
          await writeFile(join(package_dir, "packages", name, "package.json"), JSON.stringify({ name, ...pkg }));
        }
        expect(await install({ workspaces: ["packages/*"] })).toEqual(installed);
        await failTarballOnColdCache();
        expect(await install({ workspaces: ["packages/*"] }, "--filter", "opt")).toEqual(warned());
      });

      // `x` comes first, so its optional peer is the dependency the linker asks through.
      it.each(["fresh install", "bun.lock on a cold cache"])(
        "when an optional peer and an optional dependency share the package (%s)",
        async flow => {
          await serveRegistry({
          ...
          const parents = { dependencies: { x: "1.0.0", y: "1.0.0" } };
          if (flow === "fresh install") {
            failures = Infinity;
          } else {
            expect(await install(parents)).toEqual(installed);
            await failTarballOnColdCache();
          }
          expect(await install(parents)).toEqual(warned());
        },
      );
    });
  });
