...
// Helper functions to write/read from package_dir
...
function read(ctx: TestContext, path: string) {
  return file(join(ctx.package_dir, path));
}
    ...
    async () => {
      await withContext(defaultOpts, async ctx => {
        ...
        if (typeof options.scanner === "string") {
          await write(ctx, scannerPath, options.scanner);
        } else {
          const s = `export const scanner = {
  version: "1",
  scan: ${options.scanner.toString()},
};`;
          await write(ctx, scannerPath, s);
        }

        const bunfig = await read(ctx, "./bunfig.toml").text();
        if (options.bunfigScanner !== false) {
          const scannerPath = options.bunfigScanner ?? "./scanner.ts";
          await write(ctx, "./bunfig.toml", `${bunfig}\n[install.security]\nscanner = "${scannerPath}"`);
        }
...
describe.concurrent("Security Scanner Tests", () => {
  test("basic", {
    fails: true,
  ...
  test("stdout contains all input package metadata", {
    ...
    expect: ({ out }) => {
      ...
      expect(out).toMatch(/"tarball":"http:\/\/localhost:\d+\/test-\d+\/bar-0\.0\.2\.tgz"/);
    },
  });
