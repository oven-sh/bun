...
`;
  ...
  it("should patch a dependency", async () => {
    $.throws(true);
  ...
  it("should patch a transitive dependency", async () => {
    $.throws(true);
    const filedir = tempDirWithFiles("patch1", {
      "package.json": JSON.stringify({
        "name": "bun-patch-test",
  ...
  it("should patch a dependency after it was already installed", async () => {
    const filedir = tempDirWithFiles("patch1", {
      "package.json": JSON.stringify({
        "name": "bun-patch-test",
        "module": "index.ts",
  ...
  it("should update a dependency when the patchfile changes", async () => {
    $.throws(true);
    const filedir = tempDirWithFiles("patch1", {
      "package.json": JSON.stringify({
        "name": "bun-patch-test",
      ...
      patches: {
        "is-even@1.0.0.patch": is_even_patch2,
      },
      "index.ts": /* ts */ `import isEven from 'is-even'; isEven(2); console.log('lol')`,
    });
    ...
    await $`echo ${is_even_patch2} > patches/is-even@1.0.0.patch; ${bunExe()} i`.env(bunEnv).cwd(filedir);
