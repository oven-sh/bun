...
it("a file: peer reuses its own package's entry for the folder, not a sibling package's", async () => {
  const { packageDir, packageJson } = await registry.createTestDir();

  // Both plugins depend on the same folder, so two entries with the same
  // resolution exist; zz-plugin-h's peer has to land on zz-plugin-h's own.
  await Promise.all([
    write(
      packageJson,
      JSON.stringify({
