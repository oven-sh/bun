...
describe("bun pm version", () => {
  ...
  it("handles various error conditions", async () => {
    const testDir1 = setupTest();
    await Bun.write(`${testDir1}/package.json`, "");

    const { error: error1, code: code1 } = await runCommand(
      [bunExe(), "pm", "version", "patch", "--no-git-tag-version"],
      testDir1,
      false,
    );
    expect(code1).toBe(1);
    expect(error1).toContain("No version field found in package.json");
  ...
  "dependencies": {
    ...
    "name":   "test-package",
       "version"   :   "2.5.0"   ,
  "description": "A test package with weird formatting",
    "main":    "index.js",
       "scripts":{
         "test":"npm test",
      "build"  :  "webpack"
...
"keywords":["test","package"],
  "author": "Test Author"
    ...
    expect(code2).toBe(0);
    expect(output2.trim()).toBe("v2.6.0");

    const updatedJson2 = await Bun.file(`${testDir2}/package.json`).text();

    expect(updatedJson2).toContain('"version"   :   "2.6.0"   ,');
    expect(updatedJson2).toContain('"name":   "test-package"');
    expect(updatedJson2).toContain('"main":    "index.js"');
    expect(updatedJson2).toContain('"scripts":{');
    expect(updatedJson2).toContain('"build"  :  "webpack"');

    const originalLines1 = originalJson1.split("\n");
    const updatedLines1 = updatedJson1.split("\n");
    expect(updatedLines1.length).toBe(originalLines1.length);

    const originalLines2 = originalJson2.split("\n");
    const updatedLines2 = updatedJson2.split("\n");
    expect(updatedLines2.length).toBe(originalLines2.length);
  });
