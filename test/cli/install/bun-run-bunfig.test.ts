...
describe.each(["bun run", "bun"])(`%s`, cmd => {
  ...
  test("--bun=false overrides `bun = true` in bunfig (same as --no-bun)", async () => {
    const cwd = tempDirWithFiles("run.where.node.bun-false", {
      "bunfig.toml": toTOMLString({ run: { bun: true } }),
      "package.json": JSON.stringify({ scripts: { "where-node": `which node` } }, null, 2),
    });

    const result = Bun.spawnSync({
      cmd: [bunExe(), "--silent", "--bun=false", ...runCmd, "where-node"],
      env: bunEnv,
      stderr: "inherit",
      stdout: "pipe",
      stdin: "ignore",
    ...
    expect(realpathSync(result.stdout.toString().trim())).toBe(realpathSync(node));
    expect(result.success).toBeTrue();
  });

  test("--bun=<invalid> is an error", async () => {
    const cwd = tempDirWithFiles("run.bun.invalid", {
      "package.json": JSON.stringify({ scripts: { noop: "true" } }, null, 2),
    });

    const result = Bun.spawnSync({
      cmd: [bunExe(), "--bun=maybe", ...runCmd, "noop"],
      env: bunEnv,
      stderr: "pipe",
      stdout: "pipe",
      stdin: "ignore",
    ...
    expect(result.stderr.toString()).toContain("Invalid value for --bun");
    expect(result.success).toBeFalse();
  });
