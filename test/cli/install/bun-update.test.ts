...
// into the three-char mojibake `â` (C3 A2, C2 86, C2 91). Force ANSI on and
// assert the raw UTF-8 arrow bytes survive.
it("should render unicode arrows in `update --latest` summary (not mojibake) #30789", async () => {
  const urls: string[] = [];
  const registry = {
    "0.0.3": {},
    "0.0.5": {},
  ...
  const out = await updateStdout.text();
  const err = await updateStderr.text();
  expect(err).not.toContain("error:");
  expect(await updateExited).toBe(0);
  ...
  expect(stripAnsi(out)).toContain("↑ baz 0.0.3 → 0.0.5");
});
