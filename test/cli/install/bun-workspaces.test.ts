...
// Longer than a path buffer (MAX_PATH_BYTES) on every platform.
...
describe("relative tarballs", async () => {
  ...
  // no room for the NUL terminator; above it the join itself does not fit. The
  ...
  test.concurrent.each([
    ...
    const spec = "file:" + tarballPathOfLength(MAX_PATH_BYTES + offset - joinedPrefixBytes);
  ...
  test.concurrent.each(["root", "workspace"] as const)(
    ...
    async owner => {
      ...
      // The tarball is read while resolving, so the lockfile is enough to show that the path was found.
