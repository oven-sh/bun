...
describe("bun.lock workspace version tracks package.json version", () => {
  ...
  for (const args of [["install", "--lockfile-only"], ["install"]]) {
    test(`"bun ${args.join(" ")}" after bumping a workspace package.json version`, async () => {
  ...
  test(`"bun install" after adding a version where there was none`, async () => {
    ...
    let lockfile = await file(join(packageDir, "bun.lock")).text();
