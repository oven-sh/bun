...
const sectionNote = (section: string) => `note: ${section} in package.json changed since bun.lock was saved`;
...
describe.concurrent("--frozen-lockfile fails on a package.json edit that bun install writes to bun.lock", () => {
  for (const [name, edit] of Object.entries(rewrittenByInstall)) {
    test(name, async () => {
      ...
      expect(frozen.stderr).toContain(sectionNote(edit.section));
...
const notRecorded: Record<string, Pick<Edit, "root" | "member">> = {
  ...
  "package.json key order and formatting": {
    root: json => ({ trustedDependencies: json.trustedDependencies, ...json, dependencies: { "no-deps": "^1.0.0", "a-dep": "1.0.1" } }),
  },
};

describe.concurrent("--frozen-lockfile passes on a package.json edit that bun install does not write to bun.lock", () => {
  ...
  // Older `turbo prune` output drops the section from bun.lock while the copied root package.json keeps it.
  test("a trustedDependencies list that bun.lock never recorded", async () => {
