...
// whose path is longer than the buffer the destination path of each entry is
// built in. Such a package has to fail with ENAMETOOLONG, as it does with the
// hoisted linker, instead of crashing the install.
...
// segments below the temp dir, which fits the path limit of every platform.
const CHUNK_DEPTH = 4;
const CHUNK = Array(CHUNK_DEPTH).fill(SEGMENT).join("/");
const CHUNK_BYTES = CHUNK.length + 1;

// Size of the destination path buffers on POSIX. On Windows they hold 32767
// UTF-16 units, which is also the filesystem's limit, so a tree that overflows
// them cannot be created there; a tree this deep has to install fine instead.
const PATH_MAX = isMacOS ? 1024 : 4096;
const OVERFLOWING_CHUNKS = Math.ceil((PATH_MAX + 256) / CHUNK_BYTES);
const FITTING_CHUNKS = isWindows ? Math.ceil(4096 / CHUNK_BYTES) : Math.floor(PATH_MAX / 2 / CHUNK_BYTES);
...
function createDeepChain(pkgDir: string, staging: string, chunks: number, leaf?: string): string {
  ...
  return Array(chunks * CHUNK_DEPTH)
    .fill(SEGMENT)
    .join("/");
}
...
test.concurrent("isolated linker installs a folder dependency with a deep tree that fits the path buffer", async () => {
...
describe.skipIf(isWindows)("isolated linker and a folder dependency with a tree deeper than PATH_MAX", () => {
  test.concurrent("fails the package with ENAMETOOLONG when a file does not fit", async () => {
  ...
  test.concurrent("fails the package with ENAMETOOLONG when only directories do not fit", async () => {
