...
// npm and yarn both write scp-form git repos ("git@host:path") with the same
// "ssh://" prefix bun.lock uses, so the migrated resolution must parse it
// back off the same way or its repo never byte-matches the parsed dependency
// and a cold-cache isolated install waits forever on a checkout id nothing
// completes. npm >= 7 rewrites the package.json spec to the prefixed form,
// older npm and yarn keep the literal; one test per shape so each install
// runs a single ssh clone.
function testScpMigration(flavor: "npm" | "yarn", shape: string, spec: (repo: string) => string) {
  test.skipIf(isWindows)(
    `migrating a ${flavor} lockfile with a ${shape} scp git dependency installs on a cold cache`,
    async () => {
      ...
      const resolved = `git+ssh://git@localhost:${join(String(dir), "repo.git")}#${sha}`;
        ...
        fs.writeFileSync(
          ...
          JSON.stringify({
            ...
            packages: {
              ...
              "node_modules/scp-dep": { version: "1.0.0", resolved },
            },
          }),
        );
      } else {
        fs.writeFileSync(
          join(projectDir, "yarn.lock"),
...
testScpMigration("npm", "prefixed-spec", repo => `git+ssh://git@localhost:${repo}`);
testScpMigration("npm", "bare-spec", repo => `git@localhost:${repo}`);
testScpMigration("yarn", "prefixed-spec", repo => `git+ssh://git@localhost:${repo}`);
testScpMigration("yarn", "bare-spec", repo => `git@localhost:${repo}`);
