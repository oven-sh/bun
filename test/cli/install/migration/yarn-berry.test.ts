...
`,
  ...
  test("v4 format (Yarn 2.x) with bare versions", async () => {
    using dir = tempDir("yarn-berry-v4", {
      "package.json": JSON.stringify({
        name: "test-v4",
        dependencies: {
          lodash: "^4.17.21",
      ...
      // Yarn 2.x lockfile format - no npm: prefix in resolution
      "yarn.lock": `__metadata:
  version: 4
  cacheKey: 8
...
"lodash@^4.17.21":
  ...
  checksum: 8/bf8b0fd37dcae45ca1b3e60c0c3cc69c88078baf73f95ffb8ea8d46c91cca4940bd5a8f42e2c0a6be0e48420f8b98a31edca44efbd26257fc77e8dcbb45d2e03
  languageName: node
  linkType: hard
`,
    });
...
`,
  ...
  test("git and GitHub dependencies", async () => {
    using dir = tempDir("yarn-berry-git-deps", {
      "package.json": JSON.stringify({
        name: "git-deps-test",
        version: "1.0.0",
        dependencies: {
      ...
      // Real Yarn Berry format: key keeps original ref, resolution has commit hash
      "yarn.lock": `__metadata:
  version: 8
  cacheKey: 10c0
...
"git-pkg@git+https://github.com/example/repo.git#v1.0.0":
  version: 1.0.0
  resolution: "git-pkg@https://github.com/example/repo.git#commit=abc123def456"
  checksum: 10c0-gitpkghash
  languageName: node
  linkType: hard

"github-pkg@github:user/repo#main":
  version: 2.0.0
  resolution: "github-pkg@https://github.com/user/repo.git#commit=deadbeef1234"
  dependencies:
    is-number: "npm:^7.0.0"
  checksum: 10c0-githubpkghash
  languageName: node
  linkType: hard
...
`,
    ...
    expect(lockfile).toMatchSnapshot("git-deps-berry");

    expect(lockfile).toContain("git-pkg");
    expect(lockfile).toContain("github-pkg");
    expect(lockfile).toContain("github.com");
  });

  test("non-GitHub git dependencies (gitlab, bitbucket)", async () => {
    using dir = tempDir("yarn-berry-gitlab-deps", {
      "package.json": JSON.stringify({
...
"gitlab-pkg@git+https://gitlab.com/nickvision/tube.git#v1.0.0":
  version: 1.0.0
  resolution: "gitlab-pkg@https://gitlab.com/nickvision/tube.git#commit=abc123def456"
  checksum: 10c0-gitlabpkghash
  languageName: node
  linkType: hard
`,
    });
    ...
    expect(exitCode).toBe(0);
    const lockfile = await Bun.file(join(String(dir), "bun.lock")).text();
    expect(lockfile).toMatchSnapshot("gitlab-deps-berry");

    expect(lockfile).toContain("gitlab-pkg");
    expect(lockfile).toContain("gitlab.com");
  });

  test("npm aliases in Berry format", async () => {
    using dir = tempDir("yarn-berry-npm-aliases", {
      "package.json": JSON.stringify({
        name: "npm-alias-test",
        version: "1.0.0",
        dependencies: {
...
"object-assign@npm:^4.1.1":
  version: 4.1.1
  resolution: "object-assign@npm:4.1.1"
  checksum: 10c0-objectassignhash
  languageName: node
  linkType: hard
`,
    });
    ...
    expect(exitCode).toBe(0);
    const lockfile = await Bun.file(join(String(dir), "bun.lock")).text();
    expect(lockfile).toMatchSnapshot("npm-aliases-berry");

    // Verify aliases are preserved
    expect(lockfile).toContain("my-lodash");
    expect(lockfile).toContain("old-react");
    expect(lockfile).toContain("lodash@4.17.21");
    expect(lockfile).toContain("react@17.0.2");
  });

  test("custom registry URLs", async () => {
    using dir = tempDir("yarn-berry-custom-registry", {
      "package.json": JSON.stringify({
...
"private-pkg@npm:^1.0.0":
  version: 1.0.0
  resolution: "private-pkg@npm:1.0.0::__archiveUrl=https%3A%2F%2Fmy-private-registry.com%2Fprivate-pkg%2F-%2Fprivate-pkg-1.0.0.tgz"
  checksum: 10c0-privatepkghash
  languageName: node
  linkType: hard
`,
    });
    ...
    expect(exitCode).toBe(0);
    const lockfile = await Bun.file(join(String(dir), "bun.lock")).text();
    expect(lockfile).toMatchSnapshot("custom-registry-berry");

    expect(lockfile).toContain("private-pkg");
    expect(lockfile).toContain("my-private-registry.com");
  });

  test("multiple resolutions for same package (version aliasing)", async () => {
    using dir = tempDir("yarn-berry-multi-resolution", {
      "package.json": JSON.stringify({
        name: "multi-res-test",
        version: "1.0.0",
        dependencies: {
...
`,
    ...
    expect(exitCode).toBe(0);
    const lockfile = await Bun.file(join(String(dir), "bun.lock")).text();
    expect(lockfile).toMatchSnapshot("multi-resolution-berry");

    // Should have both versions
    expect(lockfile).toContain("is-number@7.0.0");
    expect(lockfile).toContain("is-number@6.0.0");
    expect(lockfile).toContain("is-odd");
  });
});
