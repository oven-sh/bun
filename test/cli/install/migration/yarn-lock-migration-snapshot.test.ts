...
describe("yarn.lock migration with snapshots", () => {
...
`,
  ...
  test("migration with yarn CLI repository yarn.lock", async () => {
    // Use the actual yarn.lock and package.json from yarn's CLI repository
    const yarnCliRepoDir = join(import.meta.dir, "yarn", "yarn-cli-repo");
    const packageJsonContent = fs.readFileSync(join(yarnCliRepoDir, "package.json"), "utf8");
    const yarnLockContent = fs.readFileSync(join(yarnCliRepoDir, "yarn.lock"), "utf8");

    const tempDir = tempDirWithFiles("yarn-migration-yarn-cli-repo", {
