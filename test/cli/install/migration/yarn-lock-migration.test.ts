...
// The manifest pass after a migration only backfills bin/os/cpu, so a manifest it cannot get is a warning for an optional dependency as well; what fails an install is a required package's own download. The registry answers 404 to everything; the `#sha1` suffix is what makes yarn's `resolved` a registry resolution rather than a plain tarball URL.
test.concurrent.each([
  ["optionalDependencies", "warn", 0],
  ["dependencies", "error", 1],
])(
  "installing from a yarn.lock whose %s entry the registry no longer serves warns about the manifest and %ss about the tarball",
  async (group, tarballSeverity, expectedExitCode) => {
    using registry = Bun.serve({ port: 0, fetch: () => new Response("gone", { status: 404 }) });
    const tarball = `${registry.url.origin}/leaf/-/leaf-1.0.0.tgz#0123456789abcdef0123456789abcdef01234567`;
    await using tmpDir = tempDir("yarn-migration-gone", {
      "package.json": JSON.stringify({ name: "gone", [group]: { leaf: "1.0.0" } }),
...
`,
    ...
    expect(stderr.split("\n").filter(line => line.startsWith("warn:") || line.startsWith("error:"))).toStrictEqual([
      `warn: GET ${registry.url.origin}/leaf - 404`,
      `${tarballSeverity}: GET ${tarball} - 404`,
    ]);
    expect(fs.existsSync(join(String(tmpDir), "bun.lock"))).toBe(expectedExitCode === 0);
