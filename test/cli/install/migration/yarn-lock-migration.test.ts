...
// treats a missing `resolved` the same way.
describe.concurrent("yarn.lock entries without a resolved field", () => {
  ...
  test("bun pm migrate keeps them as registry packages", async () => {
    ...
    const aliasIntegrity = "sha512-" + Buffer.alloc(64, 4).toString("base64");
...
`,
    ...
    // The package an alias resolved to, and the tarball a URL dependency is fetched from, are
    // only known from `resolved`, so `from-url` and `lodash-alias` are still left for
