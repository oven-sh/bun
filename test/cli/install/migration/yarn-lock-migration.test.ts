...
test("yarn.lock migration rejects off-registry tarball URLs that lack integrity", async () => {
  // A yarn.lock entry whose `resolved` URL points outside the npm registry and
  // carries no integrity hash must not be migrated into bun.lock.
  const rejectedDir = tempDirWithFiles("yarn-migration-off-registry-no-integrity", {
    "package.json": JSON.stringify(
      {
        name: "off-registry-test",
        version: "1.0.0",
        dependencies: {
          "lodash": "^4.17.21",
        },
...
lodash@^4.17.21:
  version "4.17.21"
  resolved "https://tarballs.example.com/lodash-4.17.21.tgz"
`,
  });

  const rejectedResult = await Bun.spawn({
    cmd: [bunExe(), "pm", "migrate", "-f"],
    cwd: rejectedDir,
    env: bunEnv,
    stdout: "pipe",
  ...
  expect(rejectedStderr).toContain("InvalidYarnLockfile");
  expect(fs.existsSync(join(rejectedDir, "bun.lock"))).toBe(false);
  expect(rejectedExitCode).not.toBe(0);

  // The same off-registry URL is still accepted when the entry carries a
  // verifiable integrity hash.
  const allowedDir = tempDirWithFiles("yarn-migration-off-registry-with-integrity", {
    "package.json": JSON.stringify(
      {
        name: "off-registry-integrity-test",
        version: "1.0.0",
        dependencies: {
          "lodash": "^4.17.21",
        },
...
`,
  ...
  const allowedResult = await Bun.spawn({
    cmd: [bunExe(), "pm", "migrate", "-f"],
    cwd: allowedDir,
    env: bunEnv,
    stdout: "pipe",
  ...
  expect(allowedStderr).not.toContain("InvalidYarnLockfile");
  expect(fs.existsSync(join(allowedDir, "bun.lock"))).toBe(true);
  expect(fs.readFileSync(join(allowedDir, "bun.lock"), "utf8")).toContain(
    "https://tarballs.example.com/lodash-4.17.21.tgz",
  );
