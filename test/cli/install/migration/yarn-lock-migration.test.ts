...
describe.concurrent("yarn.lock migration of resolved URLs", () => {
  const integrity = {
    alias: "sha512-nne9/IiQ/hzIhY6pdDnbBtz7DjPTKrY00P/zvPSm5pOFkl6xuGrGnXn/VtTNNfNtAfZ9/1RtehkszU9qcTii0Q==",
...
${yarnLockEntries}`,
  ...
  test("default registry: scoped and npm-aliased entries without yarn's #sha1 suffix stay npm packages", async () => {
...
"leaf-alias@npm:leaf@^1.0.0":
  ...
  integrity ${integrity.alias}
`,
    );

    expect(bunLock).toContain(`"@scope/leaf": ["@scope/leaf@2.0.1", "", {}, "${integrity.leaf}"]`);
    expect(bunLock).toContain(`"leaf-alias": ["leaf@1.0.0", "", {}, "${integrity.alias}"]`);
  });
