#!/usr/bin/env bun
/**
 * Generates the `bundled-shipped-host` and `bundled-shipped-inner` fixtures used by
 ...
 *
 * A dependency nested under the host that needs another `no-deps` must not hoist into the
 * folder the tarball shipped.
 */
