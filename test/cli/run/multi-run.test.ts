...
describe.concurrent.skipIf(isWindows)("signals", () => {
  ...
  test("a SIGINT the runner inherited as ignored still stops the run", async () => {
    using dir = tempDir("mr-sig-bg", trapPackage("SIGINT"));
    // A shell without job control starts every `&` job with SIGINT ignored,
    // and `kill -INT $!` must still stop it. `trap "" INT` gives the same start state.
    await using proc = Bun.spawn({
      cmd: ["sh", "-c", 'trap "" INT; exec "$0" run --parallel a b', bunExe()],
      env: { ...bunEnv, NO_COLOR: "1" },
      cwd: String(dir),
      stderr: "pipe",
      stdout: "pipe",
    ...
    stdoutPromise.then(text => ready.reject(new Error(`stdout ended before 2 ready lines:\n${text}`)));
    await ready.promise;
    proc.kill("SIGINT");
