...
describe.concurrent("a failed registry lookup is asked again by the next resolve", () => {
  ...
  const failures: Record<string, { respond: (deadPort: number) => Response; requestsPerLookup?: number; busy?: true }> =
    {
      ...
      // retried, so one lookup is several requests
      "500": { respond: () => new Response("{}", { status: 500 }) },
      "a manifest that does not parse": { respond: () => new Response("not json"), requestsPerLookup: 1 },
      // the connection closes before a response arrives; retried as well
      "a dropped connection": { respond: port => Response.redirect(`http://127.0.0.1:${port}/flaky-pkg`, 302) },
  ...
  for (const [name, { respond, requestsPerLookup, busy }] of Object.entries(failures)) {
    test(name, async () => {
      ...
      const [first, second, afterHeal] = registry.lookups;
      expect(new Set(first)).toEqual(new Set(["/flaky-pkg"]));
      if (requestsPerLookup) expect(first).toHaveLength(requestsPerLookup);
      // The second failing resolve asks exactly as the first did.
      expect(second).toEqual(first);
      expect(afterHeal).toEqual(["/flaky-pkg", "/flaky-pkg/-/flaky-pkg-1.0.0.tgz"]);
