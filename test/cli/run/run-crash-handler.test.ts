...
// first. ASAN builds of JSC do not use mimalloc, so the limits below do not
// reach them.
test.if(isWindows && !isASAN)("Windows: a failed must-succeed JSC allocation crashes at the allocator", async () => {
  await using proc = Bun.spawn({
    cmd: [
      bunExe(),
      "--debug-crash-handler-use-trace-string",
