...
import { bunEnv, bunExe, isASAN, isOhos, normalizeBunSnapshot, tempDir } from "harness";
...
describe.concurrent("bun test --isolate", () => {
  ...
  test.skipIf(isOhos)(
    // OHOS: the server side never observes the isolate-forced close (the
    // sandbox drops TCP FIN propagation — same defect family as the
    // valkey-gc connection-closed case), so b-check's closeFile never
    // appears no matter the wait budget.
