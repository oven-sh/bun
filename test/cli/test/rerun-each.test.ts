...
// so runs 2 and 3 register nothing. That must produce a warning, not silence.
describe.concurrent("--rerun-each warns when a rerun registers no tests", () => {
  const files = {
    "helper.cjs": `
  ...
  test.each([
    ...
    await using proc = Bun.spawn({
      cmd: [bunExe(), "test", "--rerun-each=3", ...flags, "./cached.test.ts", "./own.test.ts"],
      env: bunEnv,
      cwd: String(dir),
      stderr: "pipe",
      stdout: "pipe",
    ...
    expect(stderr.match(/^warn: .*$/gm)).toEqual([
      "warn: cached.test.ts registered 0 tests on run #2 (run #1 registered 1). A module that stays cached between runs registers its tests once.",
      "warn: cached.test.ts registered 0 tests on run #3 (run #1 registered 1). A module that stays cached between runs registers its tests once.",
    ]);
    expect(stderr).not.toContain("warn: own.test.ts");
    // The warning does not change the result.
    expect(stdout + stderr).toMatch(/4 pass/);
