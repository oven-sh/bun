...
describe("system-wide bunfig.toml", () => {
  ...
  test("explicit BUN_SYSTEM_CONFIG with bad path fails loudly", async () => {
    ...
    // Explicit override must error with the offending path, not silently ignore.
    expect(stdout).not.toContain("should not run");
    expect(stderr).toContain("while reading config");
    expect(stderr).toContain("nonexistent.toml");
    expect(exitCode).not.toBe(0);
  });

  test("malformed BUN_SYSTEM_CONFIG fails loudly and prints the path", async () => {
    // Two regressions in one:
    //  1. Policy typos must fail loudly. TOML parse/validation errors propagate
    //     as Err from Bunfig::parse; for an explicit BUN_SYSTEM_CONFIG,
    //     load_system_bunfig surfaces them instead of warning, so the process
    //     exits nonzero. Previously the TOML parse error was logged but the
    //     process exited 0, which silently disables the admin policy.
