...
// Minimal DER encoder, enough to issue X.509 v3 certificates (RFC 5280).
const der = {
  tlv(tag: number, body: Buffer): Buffer {
    const n = body.length;
    const length =
      n < 0x80 ? Buffer.from([n]) : n < 0x100 ? Buffer.from([0x81, n]) : Buffer.from([0x82, n >> 8, n & 0xff]);
    return Buffer.concat([Buffer.from([tag]), length, body]);
  },
  sequence: (...parts: Buffer[]): Buffer => der.tlv(0x30, Buffer.concat(parts)),
  set: (...parts: Buffer[]): Buffer => der.tlv(0x31, Buffer.concat(parts)),
  oid: (hex: string): Buffer => der.tlv(0x06, Buffer.from(hex, "hex")),
  integer: (body: Buffer): Buffer => der.tlv(0x02, body),
  boolean: (value: boolean): Buffer => der.tlv(0x01, Buffer.from([value ? 0xff : 0x00])),
  octetString: (body: Buffer): Buffer => der.tlv(0x04, body),
  bitString: (body: Buffer, unusedBits = 0): Buffer => der.tlv(0x03, Buffer.concat([Buffer.from([unusedBits]), body])),
  utf8String: (text: string): Buffer => der.tlv(0x0c, Buffer.from(text, "utf8")),
  explicit: (index: number, body: Buffer): Buffer => der.tlv(0xa0 | index, body),
  // UTCTime through 2049, GeneralizedTime from 2050 on (RFC 5280 4.1.2.5).
  time(date: Date): Buffer {
    const digits = date.toISOString().replace(/[-:T]/g, "").slice(0, 14);
    return date.getUTCFullYear() < 2050
      ? der.tlv(0x17, Buffer.from(digits.slice(2) + "Z"))
      : der.tlv(0x18, Buffer.from(digits + "Z"));
  },
};

const x509 = {
  ecdsaWithSha256: der.sequence(der.oid("2a8648ce3d040302")),
  commonName: (cn: string): Buffer => der.sequence(der.set(der.sequence(der.oid("550403"), der.utf8String(cn)))),
  extension: (oidHex: string, value: Buffer, critical = false): Buffer =>
    der.sequence(der.oid(oidHex), ...(critical ? [der.boolean(true)] : []), der.octetString(value)),
  serialNumber(): Buffer {
    const serial = crypto.randomBytes(16);
    serial[0] = (serial[0] & 0x7f) | 0x01;
    return der.integer(serial);
  },
  generalName(name: string): Buffer {
    if (net.isIPv4(name)) return der.tlv(0x87, Buffer.from(name.split(".").map(Number)));
    if (net.isIPv6(name)) {
      const [head, tail = ""] = name.split("::");
      const groups = head ? head.split(":") : [];
      const tailGroups = tail ? tail.split(":") : [];
      groups.push(...Array(8 - groups.length - tailGroups.length).fill("0"), ...tailGroups);
      const bytes = Buffer.alloc(16);
      groups.forEach((group, i) => bytes.writeUInt16BE(parseInt(group, 16), i * 2));
      return der.tlv(0x87, bytes);
    }
    return der.tlv(0x82, Buffer.from(name, "ascii"));
  },
  pem(label: string, body: Buffer): string {
    const lines = body.toString("base64").match(/.{1,64}/g)!;
    return `-----BEGIN ${label}-----\n${lines.join("\n")}\n-----END ${label}-----\n`;
  },
  sign: (tbs: Buffer, key: crypto.KeyObject): Buffer =>
    der.sequence(tbs, x509.ecdsaWithSha256, der.bitString(crypto.sign("sha256", tbs, key))),
};

/**
 * Issues a throwaway CA and a server certificate signed by it, valid from one
 * day ago to one year from now. Prefer it over a certificate checked into the
 * repository: a checked-in certificate expires one day and breaks CI.
 *
 * `names` are the subject alternative names (DNS names or IP literals). The
 * first one is also the subject CN. With `passphrase`, `key` is an encrypted
 * PKCS#8 key. Pass `ca` to the client only. A server that receives `ca`
 * requests client certificates.
 *
 * @example
 * const { ca, cert, key } = generateTlsCertificateChain();
 * using server = Bun.serve({ port: 0, tls: { cert, key }, fetch: () => new Response("ok") });
 * await fetch(`https://localhost:${server.port}/`, { tls: { ca } });
 */
export function generateTlsCertificateChain(options: { names?: string[]; passphrase?: string } = {}): {
  ca: string;
  cert: string;
  key: string;
  passphrase: string | undefined;
} {
  const { names = ["localhost", "127.0.0.1", "::1"], passphrase } = options;
  const day = 24 * 60 * 60 * 1000;
  const validity = der.sequence(der.time(new Date(Date.now() - day)), der.time(new Date(Date.now() + 365 * day)));
  const version = der.explicit(0, der.integer(Buffer.from([2])));

  const ca = crypto.generateKeyPairSync("ec", { namedCurve: "prime256v1" });
  const leaf = crypto.generateKeyPairSync("ec", { namedCurve: "prime256v1" });
  const caPublicKey = ca.publicKey.export({ type: "spki", format: "der" }) as Buffer;
  const caKeyId = crypto.createHash("sha1").update(caPublicKey).digest();
  const caName = x509.commonName("Bun test CA");

  const caCertificate = der.sequence(
    version,
    x509.serialNumber(),
    ...
    der.explicit(
      3,
      der.sequence(
        x509.extension("551d13", der.sequence(der.boolean(true)), true), // basicConstraints CA:TRUE
        x509.extension("551d0f", der.bitString(Buffer.from([0x06]), 1), true), // keyUsage keyCertSign, cRLSign
        x509.extension("551d0e", der.octetString(caKeyId)), // subjectKeyIdentifier
      ),
    ),
  );

  const serverCertificate = der.sequence(
    version,
    x509.serialNumber(),
    x509.ecdsaWithSha256,
    caName,
    validity,
    x509.commonName(names[0]),
    leaf.publicKey.export({ type: "spki", format: "der" }) as Buffer,
    der.explicit(
      3,
      der.sequence(
        x509.extension("551d13", der.sequence(), true), // basicConstraints CA:FALSE
        x509.extension("551d0f", der.bitString(Buffer.from([0x80]), 7), true), // keyUsage digitalSignature
        x509.extension("551d25", der.sequence(der.oid("2b06010505070301"))), // extendedKeyUsage serverAuth
        x509.extension("551d23", der.sequence(der.tlv(0x80, caKeyId))), // authorityKeyIdentifier
        x509.extension("551d11", der.sequence(...names.map(x509.generalName))), // subjectAltName
      ),
    ),
  );

  return {
    ca: x509.pem("CERTIFICATE", x509.sign(caCertificate, ca.privateKey)),
    cert: x509.pem("CERTIFICATE", x509.sign(serverCertificate, ca.privateKey)),
    key: leaf.privateKey.export(
      passphrase
