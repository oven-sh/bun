...
// FIXME: Unable to connect. Is the computer able to access the url?
test.todo("astro/ssr");

describe.each(["basics", "blog"])("astro/%s", (template: string) => {
  const cwd = mkdtempSync(join(tmpdir(), `astro-${template}-`));
  console.log(`astro/${template}`, "->", cwd);
  ...
  test(
    ...
    async () => {
      const { exited } = spawn({
        cwd,
        cmd: [bunExe(), "create", "astro", ".", "--template", template, "--yes"],
        env: bunEnv,
        stdout: "ignore",
        stderr: "inherit",
      });
