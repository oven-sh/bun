...
expect(Promise.resolve(1)).resolves.not.toBe(2);
expect(Promise.resolve(1)).resolves.toBeOneOf([1, 2, 3]);
expect(Promise.resolve(1)).resolves.toBeOneOf([1, 2, 3]);
...
expect("foo" as const).toBe("bar");
expect("foo" as const).not.toBe("bar");
expect("foo" as const).toBeOneOf(["foo", "bar", "baz"]);
expect("foo" as const).toBeOneOf(["foo", "bar", "baz"]);
expect("foo" as const).toBeTypeOf("number");

expect({ name: "bun" as const }).toEqual({ name: "bun" });
expect({ name: "bun" as const }).not.toEqual({ name: "bun" });
expect({ name: "bun" as const, [Symbol.for("test")]: "test" }).toContainKey("name");
expect({ name: "bun" as const, [Symbol.for("test")]: "test" }).toContainKey(Symbol.for("test"));
expect({ name: "bun" as const, [Symbol.for("test")]: "test" }).toContainAllKeys(["name", "version"]);
expect({ name: "bun" as const, [Symbol.for("test")]: "test" }).toContainEqual({ name: "bun" });

////////////
// Passes /
////////////

declare function myFunction(input: string): "bun" | "baz" | null;

it.each([
  ["bun", "foo"],
  ["baz", "bar"],
  [null, "Zorg"],
])("returns %s given %s", (expectedResult, input) => {
  expect(myFunction(input)).toBe(expectedResult);
  expect(myFunction(input)).toEqual(expectedResult);
});
...
// @ts-expect-error - definitely no numerical keys
expect({ name: "bun" as const, [Symbol.for("test")]: "test" }).toContainKey(2);

// @ts-expect-error - not a string
expect("foo").toBe(2);

// @ts-expect-error - not a similar shape
expect([{ bun: true as const }]).toBe([{ name: "hey" }]);
