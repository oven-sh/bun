 ...
 * what the bump actually touched. These drive the real CLI logic against two
 * synthetic release tarballs served from a local socket.
 */
...
import { fetchDep } from "../../scripts/build/fetch-cli.ts";
...
async function serve(files: string[]) {
  const byName = new Map(files.map(f => [f.split("/").pop()!, readFileSync(f)]));
  ...
  return {
    url: (file: string) => `http://127.0.0.1:${port}/${file.split("/").pop()}`,
...
test("a version bump keeps unchanged files (and their mtimes), replaces changed ones, adds and removes the rest", async () => {
  ...
  const v1 = makeTarball(work, "v1", {
    ...
    "link": { symlink: "include/same.h" },
    "relinked": { symlink: "include/same.h" },
  });
  const v2 = makeTarball(work, "v2", {
    ...
    "link": { symlink: "include/same.h" },
    "relinked": { symlink: "include/changed.h" },
  });
