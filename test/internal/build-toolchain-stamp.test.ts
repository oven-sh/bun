/**
 * The build directory's toolchain stamp (scripts/build/toolchain-stamp.ts).
 *
 * ninja names a tool by path, so a compiler replaced behind a stable path (a package manager's `current` link)
 * leaves every command line unchanged and the old compiler's objects in place. The stamp records what each tool
...
import { toolchainStampPath, writeToolchainStamp } from "../../scripts/build/toolchain-stamp.ts";

/** The fields the stamp and the two edge helpers below read. */
...
test("the stamp changes when a tool's path resolves to another file, and only then", () => {
  using dir = tempDir("toolchain-stamp", {
  ...
  const stamp = writeToolchainStamp(cfg);
  expect(stamp).toBe(toolchainStampPath(cfg));
  ...
  const written = statSync(stamp).mtimeMs;
  writeToolchainStamp(cfg);
  expect(statSync(stamp).mtimeMs).toBe(written);
  ...
  writeToolchainStamp(cfg);
  const after = readFileSync(stamp, "utf8");
...
test("edges that run a tool take the stamp as an input", () => {
  using dir = tempDir("toolchain-stamp-edges", {});
  ...
  for (const edge of edges) {
    ...
    expect(implicitInputs?.split(" ")).toContain("toolchain.stamp");
  }
});
