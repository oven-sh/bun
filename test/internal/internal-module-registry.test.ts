...
import {
  createInternalModuleRegistry,
  idToPublicSpecifierOrEnumName,
} from "../../src/codegen/internal-module-registry-scanner";
...
function registryTagNames(): Set<string> {
  const { moduleList, nativeStartIndex } = createInternalModuleRegistry(jsDir);
  const names = new Set<string>();
  for (let i = 0; i < nativeStartIndex; i++) {
    names.add(idToPublicSpecifierOrEnumName(moduleList[i]));
  }
...
describe("internal module registry codegen", () => {
  ...
  test("the Rust ResolvedSourceTag table is generated, not hardcoded", () => {
    ...
    expect(libRs).toContain('include!(concat!(env!("BUN_CODEGEN_DIR"), "/ResolvedSourceTag.rs"))');
    expect(libRs).not.toMatch(/=> ResolvedSourceTag\(\d+\)/);

    const bundleModules = fs.readFileSync(path.join(repoRoot, "src/codegen/bundle-modules.ts"), "utf8");
    expect(bundleModules).toContain('"ResolvedSourceTag.zig"');
    expect(bundleModules).toContain('"ResolvedSourceTag.rs"');
  });
