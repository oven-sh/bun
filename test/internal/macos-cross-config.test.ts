...
describe("baseline build config", () => {
  // A prebuilt "baseline" macOS build links the Haswell macOS WebKit
  // (oven-sh/WebKit ships no Nehalem macOS tarball), whose bmalloc startup
  // constructors use BMI2/AVX2 and SIGILL on pre-Haswell Intel Macs. This is
  // oven-sh/bun#32511: the shipped bun-darwin-x64-baseline crashed at launch
  ...
  test("allows a baseline macOS build with local WebKit (compiled for Nehalem)", () => {
    const cfg = resolveDarwin({ arch: "x64", baseline: true, webkit: "local" });
    expect(cfg.baseline).toBe(true);
    expect(cfg.webkit).toBe("local");
  });

  test("allows a baseline macOS rust-only split build (no WebKit linked)", () => {
    const cfg = resolveDarwin({ arch: "x64", baseline: true, mode: "rust-only" });
    expect(cfg.baseline).toBe(true);
    expect(cfg.mode).toBe("rust-only");
  });

  test("non-baseline macOS builds resolve to the plain (Haswell) WebKit tarball", () => {
    const source = webkit.source(resolveDarwin({ arch: "x64" }));
    if (source.kind !== "prebuilt") throw new Error(`expected prebuilt WebKit source, got ${source.kind}`);
    expect(source.url).toContain("bun-webkit-macos-amd64.tar.gz");
    expect(source.url).not.toContain("-baseline");
  });

  test("linux x64 baseline still resolves to the baseline WebKit tarball", () => {
    const cfg = resolveConfig(
      { os: "linux", arch: "x64", abi: "gnu", baseline: true, buildType: "Release" },
      mockToolchain({ ld64Lld: undefined, llvmStrip: undefined, dsymutil: undefined }),
    );
    expect(cfg.baseline).toBe(true);
    const source = webkit.source(cfg);
    if (source.kind !== "prebuilt") throw new Error(`expected prebuilt WebKit source, got ${source.kind}`);
    expect(source.url).toContain("bun-webkit-linux-amd64-baseline");
  });
});
