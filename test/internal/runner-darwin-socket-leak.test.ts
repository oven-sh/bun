...
function fakeHost(options: {
  ...
  const { counts, rebootStarts = true } = options;
  const calls: string[] = [];
  let reads = 0;
  return {
    calls,
...
const leaked = 64_728;
const notRebooted =
  "annotate `darwin-arm64-hardtack` holds 64728 leaked kernel TCP sockets (limit 40000) and did not reboot " +
  "when a test job asked it to. Its network fails at about 1.6 times the limit. Reboot it by hand.";
