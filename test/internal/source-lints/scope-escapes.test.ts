...
// `bun ./test/internal/source-lints/scope-escapes.test.ts`.
//
...
if (typeof describe === "undefined") {
  // Standalone mode (`bun ./test/internal/source-lints/scope-escapes.test.ts`):
      ...
      if (count > limit) {
        ...
        throw new Error(
          `${source} has ${count} scope escape hatches, down from ${limit}.\n` +
            `Update the inventory with \`bun ./test/internal/source-lints/scope-escapes.test.ts\`.`,
        );
      }
    });
  }
