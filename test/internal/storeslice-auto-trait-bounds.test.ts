...
function extractImpls(): { slice: string[]; ref: string[] } {
  ...
  const sliceRe = /^unsafe impl(<[^>]*>)? (Send|Sync) for StoreSlice<T>\s*\{\s*\}$/gm;
  const refRe = /^unsafe impl(<[^>]*>)? (Send|Sync) for StoreRef<T>\s*\{\s*\}$/gm;
...
test.skipIf(!canRunRustc)("StoreSlice<Cell<u32>> is NOT Sync (bound propagates !Sync payload)", async () => {
...
test.skipIf(!canRunRustc)("StoreRef<Cell<u32>> is NOT Sync (sibling bound; regression guard)", async () => {
...
test.skipIf(!canRunRustc)("StoreSlice<*mut u32> is neither Send nor Sync (raw pointer propagates)", async () => {
  const { slice } = extractImpls();
  expect(slice).toHaveLength(2);
