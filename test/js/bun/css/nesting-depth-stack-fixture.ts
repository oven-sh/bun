...
// the parser starts rejecting the input (wherever this build's frame sizes put
// that point), then at every single depth around that point so that the edge
// is hit at every alignment. Each stylesheet must either parse or be rejected
// with the nesting error; a stack overflow anywhere kills the whole process,
// which is what the test that spawns this checks for.
...
// 512-block depth cap, so the cap itself never triggers here: every rejection
// comes from the stack check.
const MAX_DEPTH = 511;
...
if (Bun.isMainThread) {
  ...
  // Every depth from one coarse step below the first rejection through a few
  // depths past it, split into the ones that parsed and the ones that were
  // rejected.
  const parsedDepths: number[] = [];
  const rejectedDepths: number[] = [];
  if (firstRejectedDepth !== null) {
    const last = Math.min(firstRejectedDepth + DEPTHS_PAST_THE_EDGE, MAX_DEPTH);
    for (let depth = Math.max(firstRejectedDepth - COARSE_STEP + 1, 1); depth <= last; depth++) {
      (isRejected(depth) ? rejectedDepths : parsedDepths).push(depth);
    }
  }
  self.postMessage({ firstRejectedDepth, parsedDepths, rejectedDepths });
}
