...
__attribute__((noinline)) static long initialized(int seed) {
...
__attribute__((noinline)) static long stored(int *p, int seed) {
  int i = 0;
  FIFTY_THOUSAND(p[i] = seed + i; i++;)
  // Loads among them see the stores before them.
  long sum = p[0] + p[49999];
...
__attribute__((noinline)) static long by_constant_offsets(int *p, int seed) {
...
int main(void) {
  ...
  long first = initialized(7), second = stored(storage, 3), third = by_constant_offsets(storage, 9);
  printf("%ld %ld %ld\n", first, second, third);
  return 0;
}
