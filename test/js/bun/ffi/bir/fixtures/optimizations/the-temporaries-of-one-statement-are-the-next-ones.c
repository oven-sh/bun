...
struct big { long a[4]; };
__attribute__((noinline)) static struct big make(long n) { struct big b = { { n, n + 1, n + 2, n + 3 } }; return b; }
__attribute__((noinline)) static long double widen(long n) { return (long double)n + 0.5L; }
...
#define LONG_ONE(k) s += (long)(widen(n) * (long double)(k + 1));
#define WIDE_ONE(k) s += (long)(((__int128)n * (k + 1) * 0x100000001LL) >> 32);

__attribute__((noinline)) static long descend(long n, long depth) {
  long s = 0;
...
__attribute__((noinline)) static long kept(long n) {
  long first = ({ struct big b = make(n); make(n + 1); b; }).a[2] + ({ make(n + 2); }).a[3];
  return first;
}
