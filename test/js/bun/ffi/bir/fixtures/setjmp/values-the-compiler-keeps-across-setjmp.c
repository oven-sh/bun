...
struct aligned { _Alignas(32) long long a[4]; };
static jmp_buf jb;
#define NOINLINE __attribute__((noinline))
NOINLINE static long long sink(struct S *p) { return p->a[0] + p->a[1] * 3 + p->a[2] * 5; }
NOINLINE static long long sink_aligned(struct aligned *p) { return p->a[0] + p->a[1] * 3 + p->a[2] * 5; }
NOINLINE static void fill(struct aligned *p, long long v) { p->a[0] = v; p->a[1] = v + 1; p->a[2] = v + 2; p->a[3] = 0; }
NOINLINE static long long opaque(long long x) { return x * 7 + 1; }
NOINLINE static void thrower(long long x) { if (x) longjmp(jb, 1); }
NOINLINE static void two_down(long long x) { thrower(opaque(x)); }
...
NOINLINE static long long six(struct S a, struct S b, struct S c, struct S d, struct S e, struct S g, long long k) {
  if (setjmp(jb)) return sink(&a) + sink(&b) * 2 + sink(&c) * 3 + sink(&d) * 4 + sink(&e) * 5 + sink(&g) * 6;
  thrower(MUCH(k));
  return 0;
}
NOINLINE static long long eight(struct S a, struct S b, struct S c, struct S d, struct S e, struct S g, struct S h, struct S i, long long k) {
  if (setjmp(jb)) return sink(&a) + sink(&b) * 2 + sink(&c) * 3 + sink(&d) * 4 + sink(&e) * 5 + sink(&g) * 6 + sink(&h) * 7 + sink(&i) * 8;
  two_down(MUCH(k));
  return 0;
}
NOINLINE static long long over_aligned_locals(long long k) {
...
static _Thread_local long long for_this_thread[4] = { 1, 2, 3, 4 };
NOINLINE static long long a_thread_local(struct S a, struct S b, struct S c, struct S d, struct S e, long long k) {
  long long before = for_this_thread[1];
...
NOINLINE static long long in_a_loop(struct S a, struct S b, struct S c, struct S d, struct S e, struct S g) {
  volatile long long times = 0;
  long long total = 0;
  for (;;) {
    switch (setjmp(jb)) {
    case 0:
...
struct wide { long long a[6]; };
NOINLINE static struct wide returns_a_structure(struct S a, struct S b, struct S c, struct S d, struct S e, long long k) {
...
NOINLINE static long long out_of_qsort(struct S a, struct S b, struct S c, struct S d, struct S e, struct S g) {
...
NOINLINE static long long what_locals_are(long long k) {
  volatile long long changed = 1;
  long long untouched = k * 2;
  long long also = 5;
