...
NOINLINE static long changes_huge(struct huge h, int at) { long before = h.words[at]; for (int i = 0; i < 4096; i++) h.words[i] = -1; return before; }
NOINLINE static long two_of_them(struct huge first, struct large between, struct huge second) { first.words[0]++; second.words[0]--; return first.words[0] * 1000L + second.words[0] + between.bytes[7]; }
...
int main(void) {
  ...
  long sum = changes_huge(h, 10) + changes_huge(other, 10);
