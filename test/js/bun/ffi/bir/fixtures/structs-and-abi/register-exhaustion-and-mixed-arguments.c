struct P { long long a, b; }; struct D { double a, b; }; struct M { int i; double d; }; struct Big { long long v[3]; };
         static long long after_seven(int a, int b, int c, int d, int e, int f, int g, struct P p, int h) { return a + b + c + d + e + f + g + p.a * 1000 + p.b * 100000 + h * 10000000; }
         long long ints_then_struct(void) { struct P p = { 3, 4 }; return after_seven(1, 1, 1, 1, 1, 1, 1, p, 5); }
         static long long one_register_left(int a, int b, int c, int d, int e, struct P p, int h) { return a + b + c + d + e + p.a * 1000 + p.b * 100000 + h * 10000000; }
         long long split_is_not_allowed(void) { struct P p = { 3, 4 }; return one_register_left(1, 1, 1, 1, 1, p, 5); }
         ...
         struct Big make_big(long long a, long long b, long long c) { struct Big r = { { a, b, c } }; return r; }
         long long big_result(void) { struct Big r = make_big(7, 8, 9); return r.v[0] * 100 + r.v[1] * 10 + r.v[2] + make_big(1, 2, 3).v[1] * 1000; }
         static struct Big shifted(struct Big in, long long by) { for (int i = 0; i < 3; i++) in.v[i] += by; return in; }
         long long chained(void) { struct Big r = make_big(1, 2, 3); return shifted(shifted(r, 10), 100).v[2] + r.v[2]; }
