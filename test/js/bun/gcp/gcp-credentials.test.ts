...
describe.concurrent("Bun.gcp", () => {
  ...
  test("fetch(url, { gcp }) adds the bearer token and quota project", async () => {
    ...
    const { stdout, stderr, exitCode } = await run(
      `
        ...
        const a = await (await fetch(echo, { gcp: true })).json();
        const b = await (await fetch(echo, { gcp: { scopes: ["bigquery"] }, headers: { "x-goog-user-project": "mine" } })).json();
        process.env.GOOGLE_APPLICATION_CREDENTIALS = ${JSON.stringify(join(sa, "sa.json"))};
        const c = await (await fetch(echo, { gcp: { audience: "https://run.app/x" } })).json();
        const errors = [];
        for (const init of [{ gcp: "yes" }, { gcp: { audience: "a", scopes: "b" } }, { gcp: true, aws: true }, { gcp: true, headers: { Authorization: "x" } }]) {
          try { await fetch(echo, init); errors.push("no error"); } catch (e) { errors.push(e.message); }
        }
    ...
    expect(errors).toEqual([
      expect.stringContaining("gcp must be true or an object"),
      expect.stringContaining("mutually exclusive"),
      expect.stringContaining("cannot use both aws and gcp"),
      expect.stringContaining('"Authorization" header is generated by gcp'),
    ]);
    expect(exitCode).toBe(0);
  });
