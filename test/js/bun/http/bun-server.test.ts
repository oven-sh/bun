...
describe("Server", () => {
  ...
  test("server should have URL fields set", async () => {
    const server = Bun.serve({
      fetch(req) {
        return new Response("Hello World!");
      },
    ...
    try {
      expect(server.url.hash).toBe("");
      expect(server.url.host).toBe(`0.0.0.0:${server.port}`);
      expect(server.url.hostname).toBe("0.0.0.0");
      expect(server.url.href).toBe(`http://0.0.0.0:${server.port}/`);
      expect(server.url.origin).toBe(`http://0.0.0.0:${server.port}`);
      expect(server.url.password).toBe("");
      expect(server.url.pathname).toBe("/");
      expect(server.url.port).toBe(`${server.port}`);
      expect(server.url.protocol).toBe("http");
      expect(server.url.search).toBe("");
      expect(server.url.username).toBe("");
      expect(server.url.searchParams).toEqual({});
      expect(server.url.toJSON()).toBe(`http://0.0.0.0:${server.port}/`);
    } finally {
      server.stop(true);
    }
  });
  ...
  test("server should have URL fields set with IPv6", async () => {
    ...
    try {
      expect(server.url.hash).toBe("");
      expect(server.url.host).toStartWith("[::1]");
      expect(server.url.hostname).toBe("[::1]");
      expect(server.url.href).toStartWith("http://[::1]:");
      expect(server.url.href).toEndWith("/");
      expect(server.url.origin).toStartWith("http://[::1]:");
      expect(server.url.password).toBe("");
      expect(server.url.pathname).toBe("/");
      expect(server.url.port).toBeDefined();
      expect(server.url.protocol).toBe("http");
      expect(server.url.search).toBe("");
      expect(server.url.username).toBe("");
      expect(server.url.searchParams).toEqual({});
      expect(server.url.toJSON()).toStartWith("http://[::1]:");
    } finally {
      server.stop(true);
    }
  });
