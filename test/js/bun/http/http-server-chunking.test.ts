...
describe("HTTP server streaming response framing", () => {
  // https://github.com/oven-sh/bun/issues/28019
  // A close-delimited HTTP/1.0 streaming response has no framing, so ending the
  // stream must not write anything but body bytes. uWS::internalEnd used to
  // inject "Content-Length: <n>\r\n\r\n" into the body when the sink ended with
  // leftover buffered data after body writes had already started.
  test.concurrent("ending an HTTP/1.0 streaming response does not inject a Content-Length header", async () => {
    const N = 2000;
    await using server = Bun.serve({
      port: 0,
      async fetch() {
        const chunks = Array.from(new Array(N), () => [
          Buffer.from("Hello"),
          Buffer.from(" "),
          Buffer.from("Bun"),
          Buffer.from("!\n"),
    ...
    expect(body).toBe(Buffer.alloc(N * 11, "Hello Bun!\n").toString());
  });
});
