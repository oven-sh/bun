...
import { bunEnv, bunExe, expiredTls, isASAN, isIPv6, isWindows, tls as tlsCert } from "harness";
...
describe.concurrent("proxy environment", () => {
  ...
  test("ALL_PROXY is the fallback proxy, a socks5 one included", async () => {
    const results = await run(
      () => ({}),
      `
      const out = {};
...
describe("SOCKS5 proxy", () => {
  ...
  test.each(["SOCKS5H", "socks5"])("%s forwards a hostname as domain ATYP", async scheme => {
    const target = new URL(httpServer.url);
    target.hostname = "localhost";
    const fixture = await createSocks5Fixture({ fragmentResponses: true });
    try {
      const response = await fetch(target, {
        proxy: fixture.proxy(scheme),
        keepalive: false,
      });
      expect(response.status).toBe(200);
      expect(fixture.observations[0]).toMatchObject({ atyp: 3, host: "localhost" });
    } finally {
      await fixture.close();
    }
  });

  test("keep-alive requests do not reuse a SOCKS tunnel", async () => {
    using a = Bun.serve({ port: 0, fetch: () => new Response("a") });
    using b = Bun.serve({ port: 0, fetch: () => new Response("b") });
    const fixture = await createSocks5Fixture();
    try {
      const proxy = fixture.proxy("socks5");
      expect(await (await fetch(`http://127.0.0.1:${a.port}/`, { proxy })).text()).toBe("a");
      expect(await (await fetch(`http://127.0.0.1:${b.port}/`, { proxy })).text()).toBe("b");
      expect(await (await fetch(`http://127.0.0.1:${a.port}/`, { proxy })).text()).toBe("a");
  ...
  test("socks5h sends an IPv4 literal as ATYP 1", async () => {
    const target = new URL(httpServer.url);
    target.hostname = "127.0.0.1";
    const fixture = await createSocks5Fixture({ fragmentResponses: true });
    try {
      ...
      expect(response.status).toBe(200);
      expect(fixture.observations[0]).toMatchObject({ atyp: 1, host: "127.0.0.1" });
    } finally {
      await fixture.close();
    }
  });
