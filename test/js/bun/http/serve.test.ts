...
async function runTest(
	{ port, ...serverOptions }: Serve<any>,
	test: (server: Server) => Promise<void> | void,
) {
	if (server) {
								...
								isDirect
									...
									: new ReadableStream({
											async pull(controller) {
												await Bun.sleep(0);
												controller.enqueue(
													Buffer.from(hasher.digest("base64")),
												);
												controller.close();
											},
										}),
		...
		it(`good range: ${start} - ${end}`, async () => {
			await getServer(async (server) => {
				const response = await fetch(
					`${server.url.origin}/?start=${start}&end=${end}`,
					{
						verbose: true,
					},
				);
...
it("request body and signal life cycle", async () => {
	renderToReadableStream = (await import("react-dom/server.browser"))
...
it("does propagate type for Blob", async () => {
	using server = Bun.serve({
		...
		async fetch(req) {
			...
			return new Response(
				new Blob(["hey"], { type: "text/plain;charset=utf-8" }),
			);
		},
	});
...
it("unix socket connection in Bun.serve", async () => {
	const unix = join(
		tmpdir(),
		"bun." + Date.now() + ((Math.random() * 32) | 0).toString(16) + ".sock",
	);
	using server = Bun.serve({
		unix,
...
it.if(isIPv6())("server.requestIP (v6)", async () => {
	...
	const response = await fetch(`http://localhost:${server.port}`).then((x) =>
		x.json(),
	);
	expect(response).toEqual({
		address: "::1",
...
it("should not instanciate error instances in each request", async () => {
	...
	expect(heapStats().objectTypeCounts.Error || 0).toBeLessThanOrEqual(
		startErrorCount,
	);
});
