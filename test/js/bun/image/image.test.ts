...
describe("Bun.Image", () => {
  ...
  describe("ICC profile", () => {
    ...
    // The wide byte range (0-255, generated by the prime-step LCG) exercises
    // binary-safe transport so a stray text-mode decode would corrupt and
    // fail the round-trip. 384 bytes fits comfortably in a single APP2
    // segment (libjpeg-turbo's split threshold is 65 519 bytes); the
    // reassembly loop below is generic enough to handle either case.
