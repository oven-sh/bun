...
describe("transpiler", () => {
  ...
  test("phase imports reject inside TS namespace / after export like other import forms", async () => {
    // parse_stmt.rs routes `import <ident> ...` inside a non-declare TS
    // namespace (or after `export import`) to parse_type_script_import_equals_stmt,
    // which expects `=` and errors. The defer/source early-return branches
    // (`can_be_esm_phase_import`) must not bypass that gate.
  ...
  test("REPL transform preserves phase when lowering static import to dynamic", async () => {
    // repl_transforms.rs rewrites `import * as X from "m"` -> `var X = await import("m")`.
