...
const MAX_ALLOWED_MEMORY_USAGE = 328;
...
await run(new Buffer(1024 * 1024 * 30).fill("A".charCodeAt(0)).toString("utf-8"));

// ~15 MB, UTF-16 emoji
await run(new Buffer(1024 * 1024 * 30).fill("😃").toString("utf-8"));

// 30 MB, ArrayBufferView
await run(new Uint8Array(1024 * 1024 * 30).fill("B".charCodeAt(0)));
