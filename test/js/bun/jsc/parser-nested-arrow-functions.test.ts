...
const nest = ([before, after]: [string, string]) => before.repeat(depth) + "'innermost'" + after.repeat(depth);

// A parser that does not finish cannot stop itself, and nothing else stops the child once this process is gone. So the
// child is killed after `childTimeout`, and the tests wait longer than that to report how it ended.
const childTimeout = 15_000;
const testTimeout = 2 * childTimeout;
...
test.concurrent(
  "eval() of arrow functions nested in parameter default values finishes",
  async () => {
    const source = `
    ...
    expect(await run(["-e", source])).toEqual({
      stdout: "function,".repeat(shapes.length * 2) + "innermost\n",
      stderr: "",
      exitCode: 0,
      signalCode: null,
    });
...
test.concurrent(
  "a module with arrow functions nested in parameter default values loads",
  async () => {
