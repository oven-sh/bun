...
describe("arrays", () => {
  ...
  test("no comma array (throws)", () => {
    const input: string = "[\n    true\n    false\n]";
    expect(() => JSON5.parse(input)).toThrow("Expected ',' or ']' in array");
  });
...
describe("numbers", () => {
  ...
  test("hexadecimal empty (throws)", () => {
    ...
    expect(() => JSON5.parse(input)).toThrow("Expected hex digits after '0x'");
  });
  ...
  test("lone decimal point (throws)", () => {
    const input: string = ".\n";
    expect(() => JSON5.parse(input)).toThrow("Invalid number: lone decimal point");
  });
