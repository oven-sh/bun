...
describe("heading_ids option", () => {
  ...
  test("heading levels 1-6 all get ids", () => {
    for (let i = 1; i <= 6; i++) {
      ...
      const result = Markdown.html(md, { heading_ids: true });
  ...
  test("empty heading produces empty id", () => {
    const result = Markdown.html("##\n", { heading_ids: true, permissive_atx_headers: true });
    expect(result).toBe('<h2 id=""></h2>\n');
  });

  test("heading_ids defaults to false", () => {
...
describe("autolink_headings option", () => {
  ...
  test("autolink with deduplication", () => {
    const md = "## Foo\n\n## Foo\n";
    const result = Markdown.html(md, { heading_ids: true, autolink_headings: true });
    expect(result).toContain('<h2 id="foo"><a href="#foo">Foo</a></h2>');
    expect(result).toContain('<h2 id="foo-1"><a href="#foo-1">Foo</a></h2>');
  });

  test("autolink_headings without heading_ids has no effect", () => {
    const result = Markdown.html("## Test\n", { autolink_headings: true });
    expect(result).toBe("<h2>Test</h2>\n");
  });
});

describe("heading_ids with render callback", () => {
  test("heading callback receives id in metadata", () => {
    const result = Markdown.render("## Hello World\n", {
      heading_ids: true,
      heading: (children: string, meta: { level: number; id?: string }) =>
        `<h${meta.level} id="${meta.id}">${children}</h${meta.level}>`,
    });
    expect(result).toBe('<h2 id="hello-world">Hello World</h2>');
  });

  test("callback receives deduplicated ids", () => {
    const ids: string[] = [];
    Markdown.render("## Foo\n\n## Foo\n\n## Foo\n", {
      heading_ids: true,
      heading: (_children: string, meta: { level: number; id?: string }) => {
        ids.push(meta.id!);
        return "";
      },
    });
  ...
  test("without heading_ids, no id in metadata", () => {
    const result = Markdown.render("## Hello\n", {
      heading: (_children: string, meta: { level: number; id?: string }) => {
        return meta.id === undefined ? "no-id" : `id:${meta.id}`;
      },
    });
