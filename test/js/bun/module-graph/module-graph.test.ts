...
describe("Bun.ModuleGraph — nested graphs, stack traces, misc host integration", () => {
  ...
  test("a rejection nothing handles is the graph's that made the promise, whoever rejects it", async () => {
    expect(
      await runBun([
        "-e",
        `
        const seen = [];
        process.on("unhandledRejection", e => seen.push("host: " + e.message));
        const graphOf = name => new Bun.ModuleGraph({ onError: (e, kind) => seen.push(name + ": " + e.message) });
        const maker = graphOf("maker"), other = graphOf("other");
        const made = maker.run(() => ({
          constructed: (() => { let reject; new Promise((_, r) => { reject = r; }); return reject; })(),
          withResolvers: Promise.withResolvers().reject,
        }));
        let ofTheHost;
        new Promise((_, r) => { ofTheHost = r; });

        made.constructed(new Error("new Promise, rejected by the host"));
        other.run(() => made.withResolvers(new Error("withResolvers, rejected by another graph")));
        other.run(() => ofTheHost(new Error("the host's, rejected by a graph")));
        // Every other promise is rejected by a job, which runs in the context it was scheduled in.
        const passedOn = maker.run(() => Promise.withResolvers());
        passedOn.promise.catch(() => {});
        maker.run(() => passedOn.promise.then(() => {}));
        other.run(() => passedOn.reject(new Error("then, called by the maker")));

        (function wait() {
          if (seen.length < 4) return setImmediate(wait);
      ...
      stdout: JSON.stringify([
        "host: the host's, rejected by a graph",
        "maker: new Promise, rejected by the host",
        "maker: then, called by the maker",
        "maker: withResolvers, rejected by another graph",
      ]),
      exitCode: 0,
    });
  });
  test("a rejection nothing handles, of a promise whose graph is gone, is dropped", async () => {
    expect(
      await runBun([
        "-e",
        `
        ...
        process.on("unhandledRejection", e => seen.push("host: " + e.message));
        let gone = false;
        const registry = new FinalizationRegistry(() => { gone = true; });
        const reject = (() => {
          const graph = new Bun.ModuleGraph({ onError: e => seen.push("graph: " + e.message) });
          registry.register(graph, 0);
          return graph.run(() => Promise.withResolvers().reject);
        })();
        (function wait(turns) {
          if (!gone && turns < 200) { Bun.gc(true); return setImmediate(wait, turns + 1); }
          reject(new Error("of a graph that is gone"));
          Promise.reject(new Error("the host's own"));
          setImmediate(() => setImmediate(() => console.log(JSON.stringify({ gone, seen }))));
        })(0);
      `,
      ]),
    ).toMatchObject({ stdout: `{"gone":true,"seen":["host: the host's own"]}`, exitCode: 0 });
  });
