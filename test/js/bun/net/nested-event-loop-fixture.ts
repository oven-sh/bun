...
// Same as above with child-process pipes, which are polled one-shot: unlike a
// socket, the kernel does not report the pipe again to the nested tick, so this
// only holds if the nested tick dispatches what the outer one collected.
