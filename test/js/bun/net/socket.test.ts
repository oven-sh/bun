...
describe.concurrent("socket", () => {
  ...
  it("should not call drain before handshake", async () => {
    // Use a local TLS server so the test does not depend on external DNS
    // (sandboxed CI has no outbound resolver for www.example.com).
    using server = Bun.serve({
      port: 0,
      tls,
      fetch() {
    ...
    using socket = await Bun.connect({
      ...
      socket: {
        drain() {
          if (!handshakeReached) {
            reject(new Error("drain fired before handshake"));
          }
        },
        handshake() {
