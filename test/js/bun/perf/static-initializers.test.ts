...
describe("static initializers", () => {
  ...
  it.skipIf(!isMacOSVersionAtLeast(14.0))("should have the expected number of static initializers", () => {
    ...
    // and mi_process_attach. CPU feature detection reads cpuid directly
    // (CPUFeatures.cpp), so x86_64 has no ___cpu_indicator_init from
    // __builtin_cpu_supports. Removals are wins; only additions are regressions.
    expect(
