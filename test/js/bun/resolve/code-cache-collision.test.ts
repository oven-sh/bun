...
test("bun test: colliding test files each run their own tests", async () => {
  // The worst presentation of the bug: the second file's top level runs the
  // first file's cached module block, so the runner registers the first file's
  // test twice under two filenames and the second file's tests never run. The
  // suite still reports every test as passing.
  const { tags, sources } = mineCollidingFiles(
    tag => `import { test } from "bun:test";\ntest("case ${tag}", () => {\n  console.log("ran ${tag}");\n});\n`,
  );
  using dir = tempDir("codecache-collision-bun-test", { "a.test.js": sources[0], "b.test.js": sources[1] });
  ...
  expect(tags.filter(tag => output.includes(`ran ${tag}`))).toEqual(tags);
