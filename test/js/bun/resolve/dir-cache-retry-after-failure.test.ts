...
test("a failed resolution sees files and packages created after the miss", async () => {
  using dir = tempDir("dir-cache-retry", {
    ...
    "main.mjs": `
      ...
      // Bun.build() shares the cache across calls. Three shapes of "installed
      // after the first build failed".
      const build = async (label, entry) => {
        try {
          console.log(label, "OK", (await Bun.build({ entrypoints: [entry] })).outputs.length);
        } catch (e) {
