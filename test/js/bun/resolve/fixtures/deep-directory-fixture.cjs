...
//
// process.chdir() needs the new cwd to fit a path buffer afterwards.
// `makeDirectoryOfLength` only enters directories at least two bytes shorter
// than the one it returns, so `length` may go up to the limit.
// `writeFileIn` and `mkdirIn` enter `dir` itself, so `dir` has to be below the
// limit; what they create inside it may be of any length.
"use strict";
const fs = require("fs");
