...
describe.concurrent("import.meta.resolve of an existing file", () => {
  ...
  it("returns the URL the module loads under", async () => {
    const { resolved, exitCode, url, joined } = await resolveAll();
    expect(resolved).toEqual({
      ...
      // The file name must match exactly, also where the file system ignores case.
  ...
  it("keeps the joined URL with --preserve-symlinks", async () => {
    const { resolved, exitCode, url, joined } = await resolveAll("--preserve-symlinks");
    expect(resolved).toEqual({
