...
describe.concurrent("long import path overflow", () => {
  ...
  // Longer than a path buffer, shorter than the specifier limit (1.5 path
  // buffers) that is rejected before the resolver runs.
  const overPathBuffer = MAX_PATH_BYTES + Math.floor(MAX_PATH_BYTES / 4);
  ...
  it("absolute path to a file, longer than a path buffer (load_as_file copy)", async () => {
    ...
    await run(String(dir), `"/" + Buffer.alloc(${overPathBuffer}, "a").toString()`);
  });

  it("require() of an absolute path to a file, longer than a path buffer", async () => {
    using dir = makeDir();
    await runScript(
      String(dir),
      `let code;
       try { require("/" + Buffer.alloc(${overPathBuffer}, "a").toString()); } catch (e) { code = e.code; }
       console.log(code === "MODULE_NOT_FOUND" ? "ok" : "unexpected: " + code);`,
    );
  });
  ...
  it("relative path that fits a path buffer until an extension is appended (load_extension)", async () => {
    ...
    await runScript(
      String(dir),
      `const dirLength = Buffer.byteLength(process.cwd()) + 1;
       for (let joined = ${MAX_PATH_BYTES - 8}; joined <= ${MAX_PATH_BYTES + 1}; joined++) {
  ...
  // itself longer than a path buffer. A relative one is joined with the tsconfig
  // directory in parse_tsconfig; an absolute one is joined with an exact `paths`
  // target in match_tsconfig_paths. Every component is a valid name. (It fits a
  // Windows path buffer, where the import is not found for the usual reason.)
