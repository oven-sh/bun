...
it("#imports", async () => {
  ...
  try {
    await resolve("package-json-imports/#foo");
    throw new Error("Test failed");
  } catch (exception) {
    expect(exception instanceof ResolveMessage).toBe(true);
    expect(exception.referrer).toBe(import.meta.path);
...
it("#imports with wildcard", async () => {
  const run = resolveFrom(pathResolve(import.meta.dir + "/node_modules/package-json-imports/package.json"));

  const wildcard = pathResolve(import.meta.dir + "/node_modules/package-json-imports/foo/wildcard.js");
...
it("require.resolve", async () => {
  ...
  expect(await resolve("package-json-exports/package.json")).toBe(
  ...
  expect(await resolve("package-json-exports/baz.js")).toBe(
    join(import.meta.path, "../node_modules/package-json-exports/foo/bar.js"),
  );

  // works with TypeScript compiler edgecases like:
  // - If the file ends with .js and it doesn't exist, try again with .ts and .tsx
  expect(await resolve("./resolve-typescript-file.js")).toBe(join(import.meta.path, "../resolve-typescript-file.tsx"));
  expect(await resolve("./resolve-typescript-file.tsx")).toBe(join(import.meta.path, "../resolve-typescript-file.tsx"));

  // throws a ResolveMessage on failure
  try {
    await resolve("THIS FILE DOESNT EXIST");
    throw new Error("Test failed");
  } catch (exception) {
    expect(exception instanceof ResolveMessage).toBe(true);
    expect(exception.referrer).toBe(import.meta.path);
...
it("self-referencing imports works", async () => {
  const baz = await resolve("package-json-exports/baz");
  const namespace = await resolve("package-json-exports/references-baz");
