...
describe.concurrent("process._debugProcess", () => {
  ...
  test("rejects a missing pid", async () => {
    expect(await runSnippet(`try { process._debugProcess(); } catch (e) { console.log(e.code); }`)).toEqual({
      stdout: "ERR_MISSING_ARGS\n",
      stderr: "",
      exitCode: 0,
    });
  });
  ...
  test.skipIf(isWindows)("reports kill() failures as system errors", async () => {
    expect(
      await runSnippet(`try { process._debugProcess(2147483646); } catch (e) { console.log(e.code, e.syscall); }`),
    ).toEqual({ stdout: "ESRCH kill\n", stderr: "", exitCode: 0 });
  });
  ...
  test.skipIf(!isWindows)("reports a missing target with the Win32 message", async () => {
    expect(
      await runSnippet(`try { process._debugProcess(2147483646); } catch (e) { console.log(e.message); }`),
    ).toEqual({ stdout: "The system cannot find the file specified.\n", stderr: "", exitCode: 0 });
  });
});
