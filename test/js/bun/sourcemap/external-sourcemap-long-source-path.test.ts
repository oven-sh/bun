...
// `sourcesContent`. Both joins used to write into fixed-size buffers with no
// length check, so a long enough `sources` entry aborted the process:
//
//   panic: range end index 4224 out of range for slice of length 4095
//
// 4200 overflows the 4096-byte buffer behind the display path on every
// platform. The preview reads through a path buffer, which is smaller than
// that on macOS and far larger on Windows.
const nameLengths = [...new Set([4200, MAX_PATH_BYTES + 104])];
