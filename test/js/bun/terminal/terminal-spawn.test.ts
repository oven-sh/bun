...
describe("Bun.Terminal subprocess integration", () => {
  ...
  test.skipIf(isWindows)("stdin stays open while consuming a streamed file fetch with terminal stdin activity", async () => {
    const childEnv = JSON.stringify({ ...bunEnv, BUN_DEBUG_QUIET_LOGS: "1", NO_COLOR: "1" });

    using dir = tempDir("terminal-stdin-fetch", {
      "stdin-fetch.fixture.ts": `
        import { pathToFileURL } from "node:url";

        if (!process.stdin.isTTY || !process.stdout.isTTY) {
          console.log("FAIL:missing-tty");
          process.exit(1);
        }
        ...
        let failure = "";
        let replyBytes = 0;
        const readyForFetch = Promise.withResolvers<void>();
        ...
        process.stdout.write("READY\\n");
        await readyForFetch.promise;
        console.log("START_FETCH");

        const response = await fetch(fileURL);
        const reader = response.body?.getReader();
        if (!reader) {
          console.log("FAIL:missing-body");
          process.exit(1);
        }
        ...
        process.exit(0);
      `,
      "terminal-harness.fixture.ts": `
        import { join } from "node:path";

        const child = join(import.meta.dir, "stdin-fetch.fixture.ts");
        const payload = join(import.meta.dir, "payload.txt");
        const childEnv = ${childEnv};
        const replyBurst = "reply-bytes-1234567890";
        let output = "";
        let lineBuffer = "";
        let primed = false;
        let repliesSent = 0;
        ...
        const proc = Bun.spawn({
          cmd: [process.execPath, child, payload],
          env: childEnv,
          terminal,
        });

        const exitCode = await proc.exited;
        const passed = exitCode === 0 && repliesSent > 0 && output.includes("PASS") && !output.includes("FAIL:");

        if (!passed) {
          console.log(JSON.stringify({ exitCode, repliesSent, output }));
          process.exit(1);
        }

        console.log("PASS");
      `,
      "payload.txt": Buffer.alloc(2 * 1024 * 1024, "x").toString() + "\n",
    });

    await using proc = Bun.spawn({
      cmd: [bunExe(), join(String(dir), "terminal-harness.fixture.ts")],
      env: bunEnv,
      stdout: "pipe",
      stderr: "pipe",
    });
