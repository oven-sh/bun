...
// never a tail call, so the runtime transpiler must not rewrite `new Error()` into `Error()`.
test("a function returning `new Error()` is in the error's stack", () => {
  const factories: Record<string, () => Error> = {
    ...
    viaConst() {
      // a single-use binding that the transpiler may inline into the return statement
