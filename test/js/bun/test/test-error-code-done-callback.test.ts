...
test("verify we print error messages passed to done callbacks", () => {
  ...
  let stderrStr = stderr
    ...
    .replaceAll(Bun.version_with_sha, "<version>")
    .trim();
  expect(stdoutStr).toMatchInlineSnapshot(`"bun test <version>"`);
  expect(stderrStr).toMatchInlineSnapshot(`
    ...
    (fail) error done callback (sync) [<time>ms]
    27 |   done(new Error(msg + "(sync)"));
    28 | });
    29 | 
    ...
    (fail) error done callback (async with await) [<time>ms]
    32 |   done(new Error(msg + "(async with await)"));
    33 | });
    34 | 
    ...
    (fail) error done callback (async with Bun.sleep) [<time>ms]
    37 |   done(new Error(msg + "(async with Bun.sleep)"));
    38 | });
    39 | 
    ...
    (fail) error done callback (async) [<time>ms]
    43 |   });
    44 | });
    45 | 
    ...
    (fail) error done callback (async, setTimeout) [<time>ms]
    49 |   }, 0);
    50 | });
    51 | 
    ...
    (fail) error done callback (async, setImmediate) [<time>ms]
    55 |   });
    56 | });
    57 | 
    ...
    (fail) error done callback (async, nextTick) [<time>ms]
    62 | });
    63 | 
    ...
    (fail) error done callback (async, setTimeout, Promise.resolve) [<time>ms]
    70 | });
    71 | 
    ...
    (fail) error done callback (async, setImmediate, Promise.resolve) [<time>ms]

     0 pass
     9 fail
    Ran 9 tests across 1 files. [<time>ms]"
  `);
});
