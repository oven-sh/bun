...
describe("Bun.CSRF", () => {
  ...
  test("generates a token with different formats", () => {
    // Base64 format
    const base64Token = Bun.CSRF.generate({ secret, format: "base64" });
    expect(typeof base64Token).toBe("string");
    expect(base64Token).toMatch(/^[A-Za-z0-9+/]+={0,2}$/);

    // Hex format
    const hexToken = Bun.CSRF.generate({ secret, format: "hex" });
  ...
  test("rejects an invalid token", () => {
    ...
    const isValid = Bun.CSRF.verify({ token: tamperedToken, secret });
  ...
  test("verification respects maxAge parameter", async () => {
    // Generate a token with default expiration (30 minutes)
    const token = Bun.CSRF.generate({ secret });
  ...
  test("token format doesn't affect verification", () => {
    ...
    const base64Token = Bun.CSRF.generate({ secret, format: "base64" });
    const base64urlToken = Bun.CSRF.generate({ secret, format: "base64url" });
    const hexToken = Bun.CSRF.generate({ secret, format: "hex" });

    expect(Bun.CSRF.verify({ token: base64Token, secret })).toBe(true);
    expect(Bun.CSRF.verify({ token: base64urlToken, secret })).toBe(true);
    expect(Bun.CSRF.verify({ token: hexToken, secret })).toBe(true);
  });

  test("error handling", () => {
    // Empty token
    expect(() => Bun.CSRF.verify({ token: "", secret })).toThrow();

    // Empty secret for generation
    expect(() => Bun.CSRF.generate({ secret: "" })).toThrow();

    // Empty secret for verification
    expect(() => Bun.CSRF.verify({ token: "some-token", secret: "" })).toThrow();
  });
});
