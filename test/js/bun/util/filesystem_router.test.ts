...
//   panic: range end index 8209 out of range for slice of length 8191
it("a dir longer than two path buffers is a missing directory", async () => {
  const code = /* js */ `
    try {
      new Bun.FileSystemRouter({ dir: Buffer.alloc(${MAX_PATH_BYTES * 2 + 8}, "a").toString(), style: "nextjs" });
      console.log("constructed");
    } catch (e) {
      console.log(e.message.startsWith("Unable to find directory: ") ? "missing directory" : e.message.slice(0, 200));
    }
  `;
  await using proc = Bun.spawn({
    cmd: [bunExe(), "-e", code],
  ...
  const [stdout, stderr, exitCode] = await Promise.all([proc.stdout.text(), proc.stderr.text(), proc.exited]);
  expect({ stdout: stdout.trim(), stderr, exitCode }).toEqual({ stdout: "missing directory", stderr: "", exitCode: 0 });
});
