// Verifies the OS memory-pressure watcher (src/jsc/MemoryPressureWatcher.rs).
//
...
describe.concurrent("MemoryPressureWatcher", () => {
  ...
  // on_pressure_dispatch can be between its shutting_down.load() and
  // enqueue_task_concurrent() while uninstall() returns and
  // VirtualMachine::destroy() proceeds to has_terminated=true — which makes
  // that enqueue panic under debug_assertions. The seam installs a
