...
describe("Bun.ms", () => {
  describe("parse (string -> number)", () => {
    ...
    test("empty string throws (matches ms: requires a non-empty string)", () => {
      expect(() => Bun.ms("")).toThrow(TypeError);
    });
  });
  ...
  describe("format (number -> string)", () => {
    ...
    test("`long` is read for truthiness (matches ms: `options.long ? ... : ...`)", () => {
      expect(Bun.ms(60000, { long: false })).toBe("1m");
      expect(Bun.ms(60000, { long: 0 } as any)).toBe("1m");
      expect(Bun.ms(60000, { long: "" } as any)).toBe("1m");
      expect(Bun.ms(60000, { long: undefined })).toBe("1m");
      expect(Bun.ms(60000, {})).toBe("1m");
      expect(Bun.ms(60000, { long: 1 } as any)).toBe("1 minute");
      expect(Bun.ms(60000, { long: "yes" } as any)).toBe("1 minute");
    });
    ...
    test("rejects non-finite numbers", () => {
      expect(() => Bun.ms(NaN)).toThrow(TypeError);
      expect(() => Bun.ms(Infinity)).toThrow(TypeError);
      expect(() => Bun.ms(-Infinity)).toThrow(TypeError);
    });
  });
  ...
  describe("strict input types (matches ms: string or finite number only)", () => {
    test("throws on non-string / non-number values instead of coercing", () => {
      ...
      expect(() => (Bun.ms as any)(true)).toThrow(TypeError);
      expect(() => (Bun.ms as any)(null)).toThrow(TypeError);
      expect(() => (Bun.ms as any)(undefined)).toThrow(TypeError);
      expect(() => (Bun.ms as any)([])).toThrow(TypeError);
      expect(() => (Bun.ms as any)({})).toThrow(TypeError);
      expect(() => (Bun.ms as any)()).toThrow(TypeError);
    });
    ...
    test("boxed String/Number objects throw (typeof is 'object')", () => {
      ...
      expect(() => (Bun.ms as any)(new String("1h"))).toThrow(TypeError);
      expect(() => (Bun.ms as any)(new Number(60000))).toThrow(TypeError);
    });
  });
});
