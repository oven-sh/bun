...
describe.skipIf(skip)("socketFaultInjection control surface", () => {
  ...
  test("set() rejects 'zero' for syscalls with no zero-return semantics", () => {
    for (const syscall of ["connect", "accept", "ssl_loop_buffer", "setsockopt"] as const) {
  ...
  test("set() accepts setsockopt with action 'errno'", () => {
    expect(fault.set({ syscall: "setsockopt", action: "errno", errno: "ENOBUFS" })).toBe(true);
  });
