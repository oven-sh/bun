...
describe.concurrent("unhandledRejection async context", () => {
  ...
  // An enterWith() inside the callback that rejects must not outlive the dispatch: the
  // listener and what it queues see the rejection-time store, and a callback registered
  // with no context (the exit handler) reads none afterwards, as on node. Every
  // non-default mode also drains microtasks inside the dispatch.
  test.each(
