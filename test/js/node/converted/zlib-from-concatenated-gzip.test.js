...
test('gunzip concatenated data', async () => {
  await expect(
    new Promise((resolve) => zlib.gunzip(data, (_, result) => resolve(result)))
  ).resolves.toEqual(expect.any(Buffer));
});

test('unzip concatenated data', async () => {
  await expect(
    new Promise((resolve) => zlib.unzip(data, (_, result) => resolve(result)))
  ).resolves.toEqual(expect.any(Buffer));
});

test('unzip deflated data', async () => {
  const deflatedData = Buffer.concat([
    zlib.deflateSync('abc'),
    zlib.deflateSync('def'),
  ]);
  await expect(
    new Promise((resolve) => zlib.unzip(deflatedData, (_, result) => resolve(result)))
  ).resolves.toEqual(expect.any(Buffer));
});
