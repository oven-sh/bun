...
// BoringSSL only represents the RSASSA-PSS (RFC 4055) parameter sets where
// the MGF1 hash matches the message hash (sha256, sha384, or sha512) and the
// salt length equals the digest length. Everything below stays in that family.
describe("rsa-pss keys", () => {
  // Generated by Node.js 26:
  //   crypto.generateKeyPairSync("rsa-pss", { modulusLength: 2048,
  //     hashAlgorithm: "sha256", mgf1HashAlgorithm: "sha256", saltLength: 32 })
  const nodeSpkiDer = Buffer.from(
    "",
    "base64",
  );
  const nodePkcs8Der = Buffer.from(
    "",
    "base64",
  );
  const nodeMessage = Buffer.from("aW50ZXJvcC1zaGEyNTY=", "base64");
  const nodeSignature = Buffer.from(
    "",
    "base64",
  );
  const nodeKeyDetails = {
    modulusLength: 2048,
    publicExponent: 65537n,
    hashAlgorithm: "sha256",
    mgf1HashAlgorithm: "sha256",
  ...
  const saltLengthFor = { sha256: 32, sha384: 48, sha512: 64 } as const;

  test.each(["sha256", "sha384", "sha512"] as const)("generateKeyPairSync('rsa-pss', %s) restricts the key", hash => {
    const saltLength = saltLengthFor[hash];
    ...
    expect(publicKey.type).toBe("public");
    expect(publicKey.asymmetricKeyType).toBe("rsa-pss");
    expect(publicKey.asymmetricKeyDetails).toEqual(expectedDetails);
    expect(privateKey.type).toBe("private");
    expect(privateKey.asymmetricKeyType).toBe("rsa-pss");
    expect(privateKey.asymmetricKeyDetails).toEqual(expectedDetails);

    // Signing with the restricted hash succeeds; any other hash is refused.
    const message = Buffer.from("rsa-pss " + hash);
    const signature = sign(hash, message, privateKey);
    expect(verify(hash, message, publicKey, signature)).toBeTrue();
    ...
    const reimportedPublic = createPublicKey({ key: spki, format: "der", type: "spki" });
    const reimportedPrivate = createPrivateKey({ key: pkcs8, format: "der", type: "pkcs8" });
    expect(reimportedPublic.asymmetricKeyType).toBe("rsa-pss");
    expect(reimportedPublic.asymmetricKeyDetails).toEqual(expectedDetails);
    expect(reimportedPrivate.asymmetricKeyType).toBe("rsa-pss");
    expect(reimportedPrivate.asymmetricKeyDetails).toEqual(expectedDetails);
    expect(verify(hash, message, reimportedPublic, signature)).toBeTrue();
    ...
    expect(createPublicKey(spkiPem).asymmetricKeyType).toBe("rsa-pss");
    expect(createPrivateKey(pkcs8Pem).asymmetricKeyType).toBe("rsa-pss");
  });

  test("generateKeyPairSync defaults mgf1HashAlgorithm and saltLength from hashAlgorithm", () => {
    const { publicKey } = generateKeyPairSync("rsa-pss", { modulusLength: 2048, hashAlgorithm: "sha384" });
    expect(publicKey.asymmetricKeyType).toBe("rsa-pss");
  ...
  test("generateKeyPair (async) produces an rsa-pss key", async () => {
    const { publicKey, privateKey } = await new Promise<Record<string, KeyObject>>((resolve, reject) => {
      generateKeyPair(
        "rsa-pss",
        { modulusLength: 2048, hashAlgorithm: "sha256", mgf1HashAlgorithm: "sha256", saltLength: 32 },
        (err, publicKey, privateKey) => (err ? reject(err) : resolve({ publicKey, privateKey })),
      );
    });
    expect(publicKey.asymmetricKeyType).toBe("rsa-pss");
    expect(privateKey.asymmetricKeyType).toBe("rsa-pss");
    const signature = sign("sha256", Buffer.from("async"), privateKey);
    expect(verify("sha256", Buffer.from("async"), publicKey, signature)).toBeTrue();
  });

  test("imports a Node.js-generated rsa-pss SPKI and verifies a Node.js signature", () => {
    const publicKey = createPublicKey({ key: nodeSpkiDer, format: "der", type: "spki" });
    expect(publicKey.asymmetricKeyType).toBe("rsa-pss");
    expect(publicKey.asymmetricKeyDetails).toEqual(nodeKeyDetails);
    expect(verify("sha256", nodeMessage, publicKey, nodeSignature)).toBeTrue();
    // Re-exporting produces the exact bytes Node.js produced.
    expect(publicKey.export({ type: "spki", format: "der" })).toEqual(nodeSpkiDer);
  });

  test("imports a Node.js-generated rsa-pss PKCS#8 and signs interoperably", () => {
    const privateKey = createPrivateKey({ key: nodePkcs8Der, format: "der", type: "pkcs8" });
    expect(privateKey.asymmetricKeyType).toBe("rsa-pss");
    expect(privateKey.asymmetricKeyDetails).toEqual(nodeKeyDetails);
    expect(privateKey.export({ type: "pkcs8", format: "der" })).toEqual(nodePkcs8Der);

    const publicKey = createPublicKey({ key: nodeSpkiDer, format: "der", type: "spki" });
    const signature = sign("sha256", nodeMessage, privateKey);
    expect(verify("sha256", nodeMessage, publicKey, signature)).toBeTrue();
  });
...

yT4jsG9gmIJhaqYfWUJWHHpNf3Y4
-----END PRIVATE KEY-----
`;
    const certPem = `-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
`;
    ...
    // X509Certificate.publicKey is a separate path from createPublicKey(pem).
    const certKey = new X509Certificate(certPem).publicKey;
    expect(certKey.asymmetricKeyType).toBe("rsa-pss");
    expect(certKey.asymmetricKeyDetails).toEqual(nodeKeyDetails);

    const message = Buffer.from("rsa-pss cert interop");
    const signature = sign("sha256", message, privateKey);
    expect(verify("sha256", message, publicKey, signature)).toBeTrue();
    expect(verify("sha256", message, certKey, signature)).toBeTrue();

    // Node.js 26 signed the same message with the same private key.
    const nodeSig = Buffer.from(
      "",
      "base64",
    );
  ...
  test.each([
    ...
    [{ modulusLength: 2048, hashAlgorithm: "sha256", mgf1HashAlgorithm: "sha384" }, /mgf1HashAlgorithm must match/],
    [{ modulusLength: 2048, hashAlgorithm: "sha256", saltLength: 20 }, /saltLength must equal the digest length/],
  ])("rejects an rsa-pss parameter set BoringSSL cannot represent: %o", (options, message) => {
    expect(() => generateKeyPairSync("rsa-pss", options as any)).toThrow(message);
  });
});
