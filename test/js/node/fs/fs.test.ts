...
// Counter for unique filenames in concurrent tests
let uniqueId = 0;
const getUniqueId = () => `${Date.now()}_${uniqueId++}`;
...
describe.concurrent("fs", () => {
  ...
  describe("mkdirSync", () => {
    ...
    it("throws for invalid options", () => {
      const path = `${tmpdir()}/${getUniqueId()}.rm.dir2/foo/bar`;
  ...
  describe("writeFileSync", () => {
    ...
    it("returning Buffer works", () => {
      const buffer = new Buffer([
        70, 105, 108, 101, 32, 119, 114, 105, 116, 116, 101, 110, 32, 115, 117, 99, 99, 101, 115, 115, 102, 117, 108,
        108, 121,
      ]);
  ...
  describe("rm", () => {
    it("removes a file", () => {
      const path = `${tmpdir()}/${getUniqueId()}.rm.txt`;
      writeFileSync(path, "File written successfully", "utf8");
    ...
    it("removes a dir", () => {
      const path = `${tmpdir()}/${getUniqueId()}.rm.dir`;
      try {
        mkdirSync(path);
      } catch (e) {}
      expect(existsSync(path)).toBe(true);
    ...
    it("removes a dir recursively", () => {
      const path = `${tmpdir()}/${getUniqueId()}.rm.dir/foo/bar`;
      try {
        mkdirSync(path, { recursive: true });
      } catch (e) {}
      expect(existsSync(path)).toBe(true);
  ...
  describe("rmdir", () => {
    ...
    it("removes a dir x 512", async () => {
      ...
      for (let i = 0; i < 512; i++) {
        const path = `${tmpdir()}/${getUniqueId()}.rm.dir${i}`;
        try {
          mkdirSync(path);
        } catch (e) {}
        paths[i] = path;
  ...
  describe("fs/promises", () => {
    ...
    // These tests check file descriptor leak detection and must run serially to avoid interference
    describe.serial("fd leak tests", () => {
    ...
    }); // end describe.serial("fd leak tests")
  ...
  it.if(isWindows)("writing to windows hidden file is possible", async () => {
    const temp = tmpdir();
    writeFileSync(join(temp, "file.txt"), "FAIL");
    await using proc = Bun.spawn({
      cmd: ["cmd", "/C", "attrib +h file.txt"],
