...
describe("aborting an in-flight http client request", () => {
  ...
  it("surfaces the error from destroy(err) called before the request is dispatched", async () => {
    // destroy(err) before any end()/write()/flushHeaders(): Node still
    // delivers the caller's error even though the request was never sent.
    await using server = createServer(() => {});
    const port = await listen(server);
  ...
  it("emits a single error when all custom-lookup addresses fail to connect", async () => {
    // A custom options.lookup that resolves to an address nothing listens on
    // must emit exactly one 'error' (ECONNREFUSED), not a second synthetic one.
    const probe = createServer();
