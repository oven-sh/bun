...
it("'upgrade' head is the bytes past a Content-Length body (same read)", async () => {
  ...
  let reqBody = "";
  let sockBytes = "";
  const { promise: upgraded, resolve: onUpgrade } = Promise.withResolvers<import("node:net").Socket>();
...
it("'upgrade' head is empty when the Content-Length body has not fully arrived", async () => {
