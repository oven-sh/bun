...
it("http.request keeps the configured host as the request authority when path is an absolute URI", async () => {
  // A gateway pattern like http.request({ host: BACKEND, path: req.url }) can be handed an
  // absolute-form request target such as "http://other-vhost.example/admin" in the path option.
  // The path option must only influence the request target: the Host header must stay pinned to
  // the configured host:port (Node.js behavior), and an https:// value must not turn the request
  // into a CONNECT tunnel through the configured backend.
  const seen: { method: string | undefined; url: string | undefined; host: string | undefined }[] = [];
  const server = createServer((req, res) => {
  ...
  try {
    ...
    // Absolute http:// URI in the path option: the request still goes to the configured backend
    // and the Host header is the configured authority, not the host embedded in the path.
    await sendRequest("http://other-vhost.example/admin");
    expect(seen).toHaveLength(1);
    expect(seen[0].method).toBe("GET");
    expect(seen[0].host).toBe(authority);

    // Absolute https:// URI in the path option: still delivered as a plain GET to the configured
    // backend (no CONNECT tunnel), with the Host header unchanged.
    ...
    expect(seen[2].url).toBe("/admin");
    expect(seen[2].host).toBe(authority);
  } finally {
    server.close();
  }
});
