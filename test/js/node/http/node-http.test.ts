...
describe("HTTP Server Security Tests - Advanced", () => {
  ...
  describe("insecureHTTPParser", () => {
    ...
    test("accepts obs-fold header continuation lines", async () => {
    ...
    test("tolerates Content-Length together with Transfer-Encoding: chunked", async () => {
      ...
      const { served, clientErrors } = await runServer(true, raw);
      expect(clientErrors).toEqual([]);
      expect(served).toEqual([{ url: "/clte", xA: undefined, cl: "5", te: "chunked", body: "hello" }]);
    });

    test("strict parser (default) still rejects obs-fold and CL+TE", async () => {
