...
it("statusCode assigned as a numeric string is coerced like writeHead()", async () => {
  // Node's implicit header path routes through writeHead(this.statusCode),
  // which coerces via |= 0 and range-checks; '404' reaches the wire as 404
  // with the body intact, '204' still drops the body, and a non-numeric
  // string throws ERR_HTTP_INVALID_STATUS_CODE from end().
