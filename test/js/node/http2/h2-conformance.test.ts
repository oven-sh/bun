...
describe("PUSH_PROMISE parent stream (RFC 9113 §6.6)", () => {
  ...
  /** A raw server and an `http2.connect()` client whose request (stream 1) is on the wire, with
   *  the SETTINGS exchange done. A GET ends the client's half of stream 1; a POST leaves it open. */
  async function clientWithRequest(method: "GET" | "POST") {
    const raw = await RawH2Server.listen();
    const client = http2.connect(`http://127.0.0.1:${raw.port}`);
    const errors: Error[] = [];
    ...
    return {
      ...
      async send(...frames: Buffer[]): Promise<"GOAWAY" | "PING ACK"> {
        ...
        return answer.type === FrameType.GOAWAY ? "GOAWAY" : "PING ACK";
      },
      close() {
        client.destroy();
        raw.close();
  ...
  test.each([
    {
      ...
      error: { code: "ERR_HTTP2_ERROR", errno: -505, message: "Protocol error" },
      pushedIds: [],
    },
    {
    ...
    },
    {
      ...
      error: { code: "ERR_HTTP2_ERROR", errno: -510, message: "Stream was already closed or invalid" },
      pushedIds: [],
    },
  ])("a PUSH_PROMISE on $parent fails the session", async ({ method, accepted, rejected, error, pushedIds }) => {
    const peer = await clientWithRequest(method);
    try {
  ...
  ])("a PUSH_PROMISE on $parent is delivered", async ({ method, before }) => {
    const peer = await clientWithRequest(method);
    try {
  ...
  ])("a PUSH_PROMISE on a cleanly closed parent, $when, does not fail the session", async ({ segments }) => {
    const peer = await clientWithRequest("GET");
    try {
