...
/** One side of an in-process pair that delivers each write inside the write call. duplexPair() waits for the reader. */
class SyncDuplexSide extends Duplex {
  other!: SyncDuplexSide;
  _read() {}
...
describe("inbound flow control after setLocalWindowSize() (RFC 9113 §6.9.1)", () => {
  // https://github.com/oven-sh/bun/issues/43893
  // setLocalWindowSize() in a frame callback cannot reach the engine, so its growth is queued.
  // If the callback also overflows the 16384-byte cork, the WINDOW_UPDATE reaches the transport
  // inside the callback, and a synchronous peer answers with DATA before the callback returns.
  // The server reads that DATA after the callback. The queued growth must apply first, or the
  // server counts the DATA against the old 65535-byte window and sends FLOW_CONTROL_ERROR.
  test("DATA that answers a WINDOW_UPDATE inside the same dispatch counts against the grown window", async () => {
    // One byte more than the old connection window. The stream window is 1 MiB.
    const body = 65536;
    const clientSide = new SyncDuplexSide();
    const serverSide = new SyncDuplexSide();
    ...
    let session: http2.ServerHttp2Session | undefined;
    let inStreamHandler = false;
    server.on("session", s => {
      session = s;
      s.on("error", (err: NodeJS.ErrnoException) => result.resolve(`${err.code}: ${err.message}`));
    });
    server.on("stream", stream => {
      inStreamHandler = true;
      let received = 0;
      stream.on("error", () => {});
      stream.on("data", (chunk: Buffer) => (received += chunk.length));
      stream.on("end", () => result.resolve(received));
      stream.session!.setLocalWindowSize(1 << 20);
      stream.respond({ ":status": 200 });
      stream.write(Buffer.alloc(20000, "r"));
      inStreamHandler = false;
    });

    const c = new RawH2(clientSide);
    // The peer sends its whole body as soon as it has the larger connection window and the
    // response HEADERS. RawH2 parsed this chunk already: its 'data' listener was added first.
    let bodySentInsideHandler: boolean | undefined;
    clientSide.on("data", () => {
      if (bodySentInsideHandler !== undefined) return;
      const grown = c.frames.some(f => f.type === FrameType.WINDOW_UPDATE && f.streamId === 0);
      const responded = c.frames.some(f => f.type === FrameType.HEADERS && f.streamId === 1);
      if (!grown || !responded) return;
      bodySentInsideHandler = inStreamHandler;
      const frames: Buffer[] = [];
      for (let offset = 0; offset < body; offset += 16384) {
        const length = Math.min(16384, body - offset);
        const last = offset + length === body;
        frames.push(encodeFrame(FrameType.DATA, last ? 0x1 /* END_STREAM */ : 0, 1, Buffer.alloc(length, 0x61)));
      }
    ...
    try {
      await openPostStream(c);
      // bodySentInsideHandler is the premise: the DATA arrived while the 'stream' handler ran.
      expect({ received: await result.promise, bodySentInsideHandler }).toEqual({
        received: body,
        bodySentInsideHandler: true,
      });
    } finally {
      c.destroy();
      session?.destroy();
