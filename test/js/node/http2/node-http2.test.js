...
describe("a reply that a synchronous JS transport delivers before socket.write() returns", () => {
  ...
  // event sends it.
  it("two responses larger than the window complete over duplexPair", async () => {
  ...
  // the stream) keeps its default size and the other one is 1 MiB. When the request body has
  // used up the default window, the server answers from inside the transport's _write.
  it.each([
    ...
    const headersSent = Promise.withResolvers();
    const socket = new Duplex({
      read() {},
      write(chunk, encoding, callback) {
