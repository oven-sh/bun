...
// Regression: https://github.com/oven-sh/bun/issues/30307
// req.setTimeout(ms, cb) on a completed Http2Stream must not fire after an
// idle gap on the session, nor should a session-level socket timeout
// cascade 'timeout' to every tracked stream. @fastify/reply-from installs a
// per-stream requestTimeout on each Http2Stream; on Bun, idle gaps >= 5s
// produced FST_REPLY_FROM_HTTP2_REQUEST_TIMEOUT bursts for every still-
// tracked completed stream.
describe("Http2Stream.setTimeout per-stream idle timer", () => {
  it("does not fire on completed streams after a session-idle gap", async () => {
    const server = http2.createServer();
    server.on("stream", stream => {
      stream.respond({ ":status": 200 });
      stream.end("ok");
    ...
    const { promise: listening, resolve: listened } = Promise.withResolvers();
    server.listen(0, listened);
    await listening;

    const port = server.address().port;
    const client = http2.connect(`http://localhost:${port}`);
    try {
      // Warm the session up first so subsequent requests return quickly and
      // never approach the per-stream timeout during their own lifetime.
      await new Promise(r => client.once("connect", r));
      {
        const warmup = client.request({ ":path": "/" });
        warmup.resume();
        warmup.end();
        await new Promise(r => warmup.on("end", r));
      }

      const timeoutFires = [];
      async function doRequest(label) {
        const req = client.request({ ":path": "/" });
      ...
      // If the stream timers were not cleared (or if the session socket
      // timer broadcasts 'timeout' to every still-tracked completed
      // stream), we would see one 'timeout' fire per completed request.
      await new Promise(r => setTimeout(r, 500));

      // A follow-up request that also succeeds. On the buggy path the
  ...
  it("session-level setTimeout does not cascade 'timeout' to tracked streams", async () => {
    ...
    try {
      ...
      // Arm a session-level socket idle timeout. Per Node.js semantics this
      // must emit 'timeout' on the session only, never on any stream.
      client.setTimeout(150);

      await new Promise(r => setTimeout(r, 400));

      expect(streamFired).toEqual([]);
      expect(sessionFired.v).toBe(true);
    } finally {
      client.close();
      server.close();
    }
