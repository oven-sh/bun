...
it("http2 session.state returns an empty object after destroy()", async () => {
  // node: "An object describing the current status of this Http2Session. If the Http2Session is no
  // longer usable, an empty object is returned." Property access on the post-destroy value must
  // not throw.
  const server = http2.createServer();
  let serverSession;
  server.on("session", session => {
