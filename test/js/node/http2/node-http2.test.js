...
describe("http2.connect() authority protocol", () => {
  ...
  function requestJSON(client, headers, requestOptions) {
  ...
  function pseudoHeadersSeenByServer(client, requestOptions) {
    const failed = new Promise((_, reject) => client.on("error", reject));
  ...
  it("throws ERR_HTTP2_UNSUPPORTED_PROTOCOL with Node's message", () => {
    function thrownBy(...args) {
      let session;
      try {
        session = http2.connect(...args);
      } catch (e) {
  ...
  it("is not checked when options.createConnection opens the socket", async () => {
    const { server, clientSide } = echoServerOverDuplexPair();
    const createConnection = mock(() => clientSide);
    let client;
    try {
      client = http2.connect("ftp://example.test", { createConnection });
  ...
  it("from options.protocol is the default :scheme of a request", async () => {
    const server = echoServer();
    let client;
    try {
      const port = await new Promise(resolve => server.listen(0, "127.0.0.1", () => resolve(server.address().port)));
      client = http2.connect({ hostname: "127.0.0.1", port }, { protocol: "http:" });
      expect(await pseudoHeadersSeenByServer(client)).toEqual([
  ...
  it("is not overridden by a protocol in the request() options", async () => {
    const { server, clientSide } = echoServerOverDuplexPair();
    let client;
    try {
      client = http2.connect({ hostname: "example.test", port: 443 }, { createConnection: () => clientSide });
