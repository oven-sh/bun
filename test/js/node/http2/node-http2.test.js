...
it("http2 client stops sending DATA when a mid-stream SETTINGS_INITIAL_WINDOW_SIZE reduction drives the stream send window negative (RFC 9113 §6.9.2)", async () => {
  ...
  let dataAtShrink = -1;
  let peer;
  const { promise: shrinkAck, resolve: resolveShrinkAck, reject: rejectOutcome } = Promise.withResolvers();
  ...
  const server = net.createServer(socket => {
    ...
    socket.on("data", chunk => {
      ...
      while (buf.length >= 9) {
        ...
        } else if (type === 0x6 /* PING */ && flags & 0x1 && payload.equals(Buffer.from("shrinkpt"))) {
