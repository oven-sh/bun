...
// close(code) event contract: NO_ERROR/CANCEL end the readable cleanly; any other code emits
// 'error' and must not emit 'end' on a body that was killed mid-stream by RST_STREAM.
describe("ClientHttp2Stream.close(code) event sequence", () => {
  ...
  async function collectEvents(port, code) {
    const events = [];
    const { promise, resolve, reject } = Promise.withResolvers();
    ...
    ses.on("remoteSettings", () => {
      ...
      st.on("error", e => events.push("error:" + e.code));
      st.on("close", () => {
  ...
  ])("close(%p) after data", async (code, expected) => {
    const srv = rawH2Server();
    await new Promise(r => srv.listen(0, "127.0.0.1", r));
    try {
      const events = await collectEvents(srv.address().port, code);
