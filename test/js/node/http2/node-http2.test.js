...
it("http2 client session.destroy() with a flow-control-blocked write does not emit 'drain' or accept further writes", async () => {
  // The peer never sends WINDOW_UPDATE, so a POST body larger than the 65535-byte initial
  // window leaves a DATA frame queued in native with its Writable callback held. Destroying
  // the session drops that frame: the callback must receive an error so the stream does not
  // emit 'drain', and any subsequent write() must not report success. Previously the dropped
  // frame's callback reported success, so 'drain' woke a backpressured producer and every
  // later write() returned true (once the native handle was gone _write fell through to
