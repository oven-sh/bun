...
// node registers the connect() listener with session.once('connect', listener), so a throw
// from it is an ordinary uncaught exception and the session stays usable.
...
it("http2 connect() listener is a one-time 'connect' listener of the session, like node", async () => {
  const server = http2.createServer();
  await new Promise(resolve => server.listen(0, "127.0.0.1", resolve));
  const clients = [];
  try {
    const connect = (...args) => {
      const client = http2.connect(`http://127.0.0.1:${server.address().port}`, ...args);
      clients.push(client);
      return client;
    };
    ...
    const calls = [];
    const listener = function (session, socket) {
...
it("http2 connect() listener and 'connect' still fire for a session destroyed before its socket connected", async () => {
  const server = http2.createServer();
  await new Promise(resolve => server.listen(0, "127.0.0.1", resolve));
