...
console.log("survived", require("./late.js"));`,
  ...
  test("Overwriting Module._load preserves require(id, options)", async () => {
    await using proc = Bun.spawn({
      cmd: [bunExe(), "run", path.join(import.meta.dir, "moduleLoadOverwriteAttrs.cjs")],
      env: bunEnv,
      stderr: "inherit",
      stdout: "pipe",
    });
  ...
  test("Overwriting Module._load together with Module.prototype.require and _resolveFilename", async () => {
    await using proc = Bun.spawn({
      cmd: [bunExe(), "run", path.join(import.meta.dir, "moduleLoadOverwriteAllHooks.cjs")],
      env: bunEnv,
      stderr: "inherit",
      stdout: "pipe",
    });
