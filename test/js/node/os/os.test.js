        ...
        `const os = require("node:os");
         ...
         for (const [name, fn] of [["userInfo", () => os.userInfo()], ["homedir", () => os.homedir()]]) {
           try {
             ...
             out[name] = {
               threw: { name: e.name, code: e.code, message: e.message, errno: e.errno, syscall: e.syscall, info: e.info },
             };
           }
         }
    ...
    expect(JSON.parse(stdout)).toEqual({
      userInfo: {
        threw: {
          ...
          message: "A system error occurred: uv_os_get_passwd returned ENOENT (no such file or directory)",
          errno: -2,
          syscall: "uv_os_get_passwd",
          info: { errno: -2, code: "ENOENT", message: "no such file or directory", syscall: "uv_os_get_passwd" },
        },
      },
    ...
    async () => {
      ...
      await using proc = Bun.spawn({
        cmd: [
         ...
         catch (e) { process.stdout.write(JSON.stringify({ name: e.name, code: e.code, message: e.message, info: e.info })) }`,
        ],
        env: envWithoutHome,
        stderr: "pipe",
      });
      ...
      expect(JSON.parse(stdout)).toEqual({
        name: "SystemError",
        code: "ERR_SYSTEM_ERROR",
        message: "A system error occurred: uv_os_homedir returned ENOENT (no such file or directory)",
        info: { errno: -2, code: "ENOENT", message: "no such file or directory", syscall: "uv_os_homedir" },
      });
      expect(exitCode).toBe(0);
    },
  );
