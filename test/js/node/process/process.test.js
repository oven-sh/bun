...
import { bunEnv, bunExe, isMacOS, isWindows, MAX_PATH_BYTES, tempDir, tmpdirSync } from "harness";
...
// getcwd() can return one byte less than MAX_PATH_BYTES. Appending a separator
// to a cached cwd of that length wrote past the buffer and aborted the process.
// Windows' limit (~98 KB of UTF-8) is not reachable with a real directory tree.
it.skipIf(isWindows)("process.chdir() into a directory whose path is MAX_PATH_BYTES - 1 bytes long", async () => {
  const target = MAX_PATH_BYTES - 1;
