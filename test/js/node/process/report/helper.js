...
function assert(condition, message = '') {
    ...
    it(`process.report assert ${message || condition}`, () => {
        if (condition === undefined) console.log(err.stack);
...
assert.deepStrictEqual = function (a, b, message = '') {
    it(`process.report assert.deepStrictEqual ${message || `${a} === ${b}`}`, () => {
        expect(a).toStrictEqual(b);
    });
};
assert.throws = function (fn, expected, message = '') {
    it(`process.report assert.throws ${message || `${fn.name} -> ${expected}`}`, () => {
        expect(fn).toThrow(expected);
    });
};
assert.doesNotThrow = function (fn, message = '') {
    it(`process.report assert.doesNotThrow ${message || fn.name}`, () => {
        expect(fn).not.toThrow();
    });
};
assert.notStrictEqual = function (a, b, message = '') {
    it(`process.report assert.notStrictEqual ${message || `${a} === ${b}`}`, () => {
...
function validateContent(report, fields = []) {
  ...
  try {
    ...
    try {
      ...
      // Logging this out instead because Bun doesn't respect error stack changes properly yet. (Nor the %O in util.format)
      console.error(util.format('\n------\nFailing Report:\n%s', Bun.inspect(report)));
    } catch {
...
function _validateContent(report, fields = []) {
  ...
  fields.forEach((field) => {
    function checkLoop(actual, rest, expect) {
      if (actual === undefined) return assert.strictEqual(actual, expect, field + ' is undefined');
      actual = actual[rest.shift()];
      if (rest.length === 0 && actual !== undefined) {
        assert.strictEqual(actual, expect);
      } else {
        assert(actual, `Expected ${field} to be present`);
  ...
  assert(Array.isArray(header.cpus), 'cpus is not an array');
  ...
  header.cpus.forEach((cpu) => {
    ...
    }), 'CPU model check failed');
  });

  assert(Array.isArray(header.networkInterfaces), 'networkInterfaces is not an array');
  ...
  assert(Array.isArray(report.nativeStack), 'nativeStack is not an array');
  report.nativeStack.forEach((frame) => {
    assert(typeof frame === 'object' && frame !== null, 'frame is not an object');
  ...
  if (isJavaScriptThreadReport) {
    ...
    if (report.javascriptStack.stack !== undefined) {
      assert(Array.isArray(report.javascriptStack.stack), 'stack is not an array');
    ...
    //  );
    //}
    //assert(typeof heap.heapSpaces === 'object' && heap.heapSpaces !== null, 'heapSpaces is not an object');
  ...
  if (report.resourceUsage.constrained_memory) {
    assert(typeof report.resourceUsage.constrained_memory, 'string', 'constrained_memory is not a string');
  }
  //assert(typeof usage.pageFaults === 'object' && usage.pageFaults !== null, 'pageFaults is not an object');
  ...
  if (report.uvthreadResourceUsage) {
    ...
    //assert.strictEqual(typeof usage.userCpuConsumptionPercent, 'number');
    //assert.strictEqual(typeof usage.kernelCpuConsumptionPercent, 'number');
    //assert(typeof usage.fsActivity === 'object' && usage.fsActivity !== null, 'fsActivity is not an object');
    //checkForUnknownFields(usage.fsActivity, ['reads', 'writes']);
    //assert(Number.isSafeInteger(usage.fsActivity.reads));
    //assert(Number.isSafeInteger(usage.fsActivity.writes));
  }
  ...
  for (const [key, value] of Object.entries(report.environmentVariables)) {
    ...
    // This breaks due to an error in Bun unrelated to process.report but process.env instead.
    //assert.strictEqual(typeof value, 'string', `value ${value} of key ${key} is not a string`);
  }
  ...
  if (!isWindows) {
    ...
    for (const [type, limits] of Object.entries(report.userLimits)) {
      ...
      assert(typeof limits === 'object' && limits !== null, `Invalid ${type} limits`);
...
export default { findReports, validate, validateContent };
