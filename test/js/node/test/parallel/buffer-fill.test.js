...
test("Buffer fill with invalid end", () => {
  expect(() => {
    ...
    expect.objectContaining({
      code: "ERR_INVALID_ARG_TYPE",
      message: expect.stringContaining('The "end" argument must be of type number'),
    }),
  );
});
...
test("alloc with no fill", () => {
  const buf = Buffer.alloc(5);
  expect(buf.toString().length).toBe(5);
  expect(buf.toString()).toBe("\x00\x00\x00\x00\x00");
});

test("alloc with fill: empty string", () => {
  const buf = Buffer.alloc(5, "");
  expect(buf.toString().length).toBe(5);
  expect(buf.toString()).toBe("\x00\x00\x00\x00\x00");
});

test("alloc with fill: empty array", () => {
  const buf = Buffer.alloc(5, []);
  expect(buf.toString().length).toBe(5);
  expect(buf.toString()).toBe("\x00\x00\x00\x00\x00");
});

test("alloc with fill: zero", () => {
...
test("alloc with fill: buffer", () => {
  ...
  expect(bufEmptyBuffer.toString().length).toBe(5);
