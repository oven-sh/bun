...
if (process.argv[2] === 'child') {
  ...
  function recursive(depth = 0) {
    ...
    new Promise(() => {});
    // Bun: JavaScriptCore implements proper tail calls in strict mode, so
    // `return recursive(...)` would never overflow the stack. Keep the call
    // in non-tail position to actually trigger the stack overflow.
    recursive(depth + 1);
    return depth;
  }
