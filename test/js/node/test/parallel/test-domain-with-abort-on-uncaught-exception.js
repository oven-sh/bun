...
if (process.argv[2] === 'child') {
  ...
  d.run(function doStuff() {
    ...
    // Note for Bun: upstream also throws synchronously from the domain.run
    // callback here. That is omitted because Bun reports a synchronous throw
    // from the main module after the nextTick queue has drained (node
    // dispatches it at throw time, before), so the nextTick error's domain
    // cleanup runs first and the synchronous error can no longer be paired
    // with this domain. Synchronous d.run() throw routing is covered by
    // test-domain-uncaught-exception.js and
    // test-domain-no-error-handler-abort-on-uncaught-{0,1}.js.
  });
} else {
