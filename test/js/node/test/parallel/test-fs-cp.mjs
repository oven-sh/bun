...
// TODO: isInsideDirWithUnusualChars is not yet implemented in Bun's test infrastructure
const isInsideDirWithUnusualChars = false;
...
{
  const src = join(testDir, 'fixtures/copy/utf/新建文件夹');
  const dest = nextdir();
...
{
  const src = join(testDir, 'fixtures/copy/kitchen-sink');
  const dest = nextdir();
  cpSync(src, dest, mustNotMutateObjectDeep({ recursive: true }));
  assertDirEquivalent(src, dest);
}
...
if (!isInsideDirWithUnusualChars) {
  const src = nextdir();
  mkdirSync(src, mustNotMutateObjectDeep({ recursive: true }));
  const dest = join(testDir, 'fixtures/copy/kitchen-sink/README.md');
  assert.throws(
    () => cpSync(src, dest),
...
{
  const srcFile = join(testDir, 'fixtures/copy/kitchen-sink/index.js');
...
if (!isInsideDirWithUnusualChars) {
  const src = join(testDir, 'fixtures/copy/kitchen-sink/README.md');
  const dest = nextdir();
  mkdirSync(dest, mustNotMutateObjectDeep({ recursive: true }));
  assert.throws(
    () => cpSync(src, dest),
...
{
  const src = join(testDir, 'fixtures/copy/kitchen-sink');
  const dest = join(testDir, 'fixtures/copy/kitchen-sink/a');
  assert.throws(
    () => cpSync(src, dest),
    { code: 'ERR_FS_CP_EINVAL' }
  );
...
// It allows file to be copied to a file path.
{
  const srcFile = join(testDir, 'fixtures/copy/kitchen-sink/README.md');
  const destFile = join(nextdir(), 'index.js');
