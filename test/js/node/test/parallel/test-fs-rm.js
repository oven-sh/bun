    ...
    fs.rm(dir, common.mustNotMutateObjectDeep({ recursive: false }), common.mustCall((err) => {
      ...
      fs.rm(dir, common.mustNotMutateObjectDeep({ recursive: true }), common.mustSucceed(() => {
        ...
        fs.rm(dir, common.mustCall((err) => {
          ...
          // Bun uses one native operation for rmSync, printing 'rm' is much more obvious to the end user
          assert.strictEqual(err.syscall, 'rm');
        }));
      }));
    }));
...
(async () => {
  ...
  // Attempted removal should fail now because the directory is gone.
  await assert.rejects(fs.promises.rm(dir), {
    // Bun uses one native operation for rmSync, printing 'rm' is much more obvious to the end user
    // syscall: 'lstat'
    syscall: 'rm'
  });
...
// Bun: does not have a `validateRmOptionsSync` internal function, so these
// tests are best-effort (catching the ones that throw errors only)
{
  const dir = nextDirPath();
  makeNonEmptyDirectory(4, 10, 2, dir, true);
  ...
  // Bun
  function validateRmOptionsSync(file, options) {
