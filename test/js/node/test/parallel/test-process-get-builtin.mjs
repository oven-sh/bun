...
// and bundled third-party packages ("undici", "ws"); they are not
// importable through the node: scheme this loop exercises.
for (const id of [...publicBuiltins]) {
  if (id === 'bun' || id.startsWith('bun:') || id === 'undici' || id === 'ws') {
    publicBuiltins.delete(id);
  }
}
