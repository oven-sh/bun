...
publicBuiltins.delete('bun');
publicBuiltins.delete('bun:ffi');
publicBuiltins.delete('bun:jsc');
publicBuiltins.delete('bun:sqlite');
publicBuiltins.delete('bun:test');
publicBuiltins.delete('bun:wrap');
publicBuiltins.delete('detect-libc');
publicBuiltins.delete('undici');
publicBuiltins.delete('ws');

// Check that import(id).default returns the same thing as process.getBuiltinModule(id).
for (const id of publicBuiltins) {
  const imported = await import(`node:${id}`);
  // assert.strictEqual(process.getBuiltinModule(id), imported.default); // TODO: most pass, c++ builtins fail
}
...
for (const id of ids) {
  ...
  // assert.strictEqual(process.getBuiltinModule(prefixed), imported.default); // TODO: most pass, c++ builtins fail
}
