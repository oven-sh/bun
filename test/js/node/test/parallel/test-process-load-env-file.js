...
describe('process.loadEnvFile()', () => {
  ...
  it('should throw when `.env` does not exist', async () => {
    ...
    try {
      if (isMainThread) {
        // Bun's repository has no `lib/` directory; `src/` serves the same
        // purpose (a tracked directory that will not contain a stray .env).
  ...
  // Bun does not implement Node's permission model (--permission).
  it.skip('should check for permissions', async () => {
    const code = `
