...
suite('accessing the node:sqlite module', () => {
  ...
  test('can be disabled with --no-experimental-sqlite flag', { skip: typeof Bun !== 'undefined' }, async (t) => { // BUN: no --no-experimental-sqlite flag; node:sqlite is always available.
    const {
      stdout,
      stderr,
      code,
...
suite('SQL APIs enabled at build time', () => {
  ...
  test('percentile is enabled', { skip: typeof Bun !== 'undefined' }, (t) => { // BUN: Node appends ext/misc/percentile.c to its SQLite amalgamation build; Bun's bundled sqlite3.c doesn't include that extension yet.
    const db = new DatabaseSync(':memory:');
    db.exec(`
