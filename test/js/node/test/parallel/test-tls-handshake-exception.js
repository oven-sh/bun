...
if (process.argv[2] === 'child') {
  ...
  // Bun: the uncaught-exception banner prints `error: <message>` for a plain
  // Error (named error classes keep their name); accept either casing.
  const ok = /error: eyecatcher/i.test(stderr);
  assert(ok, stderr);
}
