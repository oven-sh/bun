//#FILE: test-zlib-crc32.js
//#SHA1: a46ac98c5c568eec892b62f9da1effc368081b07
//-----------------
"use strict";
...
describe("zlib.crc32", () => {
  test("crc32 test case", () => {
    for (const [crc, data, len, expected] of tests) {
      ...
      expect(zlib.crc32(buf, crc)).toBe(expected);
      expect(zlib.crc32(buf.toString(), crc)).toBe(expected);
      if (crc === 0) {
        expect(zlib.crc32(buf)).toBe(expected);
        expect(zlib.crc32(buf.toString())).toBe(expected);
      }
    }
  });
  ...
  test("invalid input types", () => {
    [undefined, null, true, 1, () => {}, {}].forEach(invalid => {
      expect(() => zlib.crc32(invalid)).toThrow(
  ...
  test("invalid crc types", () => {
    [null, true, () => {}, {}].forEach(invalid => {
      expect(() => zlib.crc32("test", invalid)).toThrow(
        expect.objectContaining({
          code: "ERR_INVALID_ARG_TYPE",
          message: expect.any(String),
        }),
