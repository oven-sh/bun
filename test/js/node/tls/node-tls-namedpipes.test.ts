...
it.if(isWindows)("named-pipe tls.connect({ socket }) does not re-emit bytes on the original socket", async () => {
  ...
  try {
    ...
    client.on("data", data => resolveClientReceived(data.toString()));
