// Regression test for the undici 8.0.3+ failure: before markAsUncloneable was
// implemented, every undici constructor that called webidl.util.markAsUncloneable
// (CacheStorage, Response, Request, Headers, FormData, WebSocket, EventSource)
// crashed at module-load time with:
...
//
// TODO(user): after filing the tracking issue, move this file to
// test/regression/issue/<N>.test.ts.
...
test("undici 8.1+ loads webidl without crashing (regression for missing markAsUncloneable)", async () => {
  ...
  try {
    ...
    if (!match) {
      console.warn(`[markAsUncloneable-undici-regression] Skipping: could not parse undici version '${pkg.version}'.`);
      return;
    }
