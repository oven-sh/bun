...
// process.mainModule is set up, so every Worker started from a file populates
// it, and the worker's VM teardown used to skip releasing it: one string
// leaked per Worker. Eval workers have no file to resolve and never hit this.
//
...
test.concurrent.skipIf(!isASAN || isWindows).each([
  ...
  async (_, subdir) => {
    using dir = tempDir("worker-main-path-leak", {
      ...
      [`${subdir}/worker.js`]: "",
      "leaksan.supp":
        readFileSync(join(import.meta.dirname, "../../../leaksan.supp"), "utf8") + "\nleak:WebCore::eventNames\n",
    });
    await using proc = Bun.spawn({
      cmd: [bunExe(), "main.mjs"],
      cwd: String(dir),
  ...
  // A clean run takes several seconds under debug+ASAN. A failing run spends
  // tens of seconds more in LSan's report symbolization.
  90_000,
);
