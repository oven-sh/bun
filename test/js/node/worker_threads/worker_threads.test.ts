...
describe("exit code when a worker dies from an error", () => {
  ...
  test.concurrent("unhandled promise rejection exits 1", async () => {
    expect(
      await run(
  ...
  test.concurrent("unhandled rejection runs the worker's process.on('exit') with code 1", async () => {
    await using proc = Bun.spawn({
      cmd: [
        bunExe(),
        "-e",
  ...
  test.concurrent("process.on('unhandledRejection') handler suppresses the nonzero exit", async () => {
    expect(
      await run(
