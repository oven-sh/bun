...
//
// A server-side disconnect (pg_terminate_backend, idle_in_transaction_session_timeout,
...
import { listeningServer, pgAuthenticationOk, pgCommandComplete, pgReadyForQuery } from "./wire-frames";
...
function firstInterleaving(received: Received[]): string | null {
  ...
  for (const { conn, sql } of received) {
    ...
    if (word === "BEGIN") {
      if (d !== 0) return `BEGIN inside an open transaction on conn ${conn}: ${JSON.stringify(received)}`;
      depth.set(conn, 1);
