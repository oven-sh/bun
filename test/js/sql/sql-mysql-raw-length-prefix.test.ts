...
// MYSQL_TYPE_* values used below. From src/sql/mysql/MySQLTypes.rs.
...
test(".raw() strips length-prefix bytes (#30039) — text protocol", async () => {
  ...
  try {
    ...
    // (src/sql_jsc/mysql/protocol/ResultSet.rs) that used to call rawEncodeLenData.
...
test(".raw() strips length-prefix bytes (#30039) — binary protocol", async () => {
  ...
  try {
    ...
    // branches (src/sql_jsc/mysql/protocol/DecodeBinaryValue.rs) that used to call
