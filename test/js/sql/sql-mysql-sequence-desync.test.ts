...
// Two concurrent simple queries A and B on a max:1 pool (B queues client-side).
// The server answers A with its result set PLUS an unsolicited "GHOST" result
// set in the SAME write. The ghost packets carry the continuation sequence ids
// (5..8); a real answer to B would restart at seq 1 after B's COM_QUERY (seq 0).
// Without sequence-id validation, A's terminator advances the queue, B's
// COM_QUERY is sent, and the trailing ghost bytes in the buffer are routed to
// B, which resolves with rows the server never produced for it.
...
// Same shape with A receiving an ERR packet instead of a result set: the ERR
// settles A (rejected), advances the queue, and trailing ghost bytes carrying
// continuation sequence ids must not be routed to B.
...
// Baseline: when the server answers each query with a well-formed result set
// whose sequence ids restart at 1 (as the protocol requires), both queries
// resolve with their own rows. This guards against the validation rejecting
// the legitimate case.
