  ...
  describe("RESP3 Data Type Handling", () => {
    test("should handle RESP3 Map type (HGETALL)", async () => {
      ...
      try {
        ...
        const mapResult = await client.sendCommand("HGETALL", [hashKey]);
    ...
    test("should handle RESP3 Set type", async () => {
      ...
      try {
        ...
        const setResult = await client.sendCommand("SMEMBERS", [setKey]);
    ...
    test("should handle RESP3 Number types", async () => {
      ...
      try {
        ...
        const doubleResult = await client.sendCommand("INCRBYFLOAT", [counterKey, "1.5"]);
        ...
        const hashDoubleResult = await client.sendCommand("HINCRBYFLOAT", [hashKey, "field", "10.5"]);
    ...
    test("should handle RESP3 Null type", async () => {
      ...
      try {
        ...
        await client.sendCommand("HSET", [hashKey, "existing", "value"]);
        
        const nullFieldResult = await client.sendCommand("HGET", [hashKey, "nonexistent"]);
        expect(nullFieldResult).toBeNull();
        
        // BLPOP with timeout
        const listKey = testKey("empty-list");
        const timeoutResult = await client.sendCommand("BLPOP", [listKey, "1"]);
    ...
    test("should handle RESP3 Error type", async () => {
      ...
      try {
        // Syntax error
        await client.sendCommand("SYNTAX-ERROR", []);
        expect(false).toBe(true); // Should not reach here
      } catch (error) {
  ...
  describe("Protocol Parsing Edge Cases", () => {
    test("should handle nested array structures", async () => {
      ...
      try {
        ...
        await client.sendCommand("XADD", [streamKey, "*", "field1", "value1", "field2", "value2"]);
        await client.sendCommand("XADD", [streamKey, "*", "field1", "value3", "field2", "value4"]);
        
        // Get range
        const rangeResult = await client.sendCommand("XRANGE", [streamKey, "-", "+"]);
    ...
    test("should handle empty strings in bulk strings", async () => {
      ...
      try {
        ...
        await client.sendCommand("HSET", [hashKey, "empty-field", ""]);
        await client.sendCommand("HSET", [hashKey, "", "empty-field-name"]);
        
        // Get them back
        const emptyValue = await client.sendCommand("HGET", [hashKey, "empty-field"]);
        expect(emptyValue).toBe("");
        
        const emptyField = await client.sendCommand("HGET", [hashKey, ""]);
    ...
    test("should handle large arrays", async () => {
      ...
      try {
        ...
        for (let i = 0; i < itemCount; i += 100) {
          ...
          await client.sendCommand("SADD", [setKey, ...members]);
        }
        
        // Get all members
        const membersResult = await client.sendCommand("SMEMBERS", [setKey]);
    ...
    test("should handle rapidly switching between command types", async () => {
      ...
      try {
        ...
        for (let i = 0; i < iterations; i++) {
          ...
          await client.sendCommand("KEYS", [`${prefix}-str-${i}`]);
          
          // Hash operations (map responses)
          await client.sendCommand("HSET", [`${prefix}-hash-${i}`, "field", "value"]);
          await client.sendCommand("HGETALL", [`${prefix}-hash-${i}`]);
          
          // Set operations
          await client.sendCommand("SADD", [`${prefix}-set-${i}`, "member1", "member2"]);
          await client.sendCommand("SMEMBERS", [`${prefix}-set-${i}`]);
        }
      ...
      try {
        ...
        const allPromises = clients.flatMap((client, clientIndex) => {
          ...
          for (let i = 0; i < operationsPerClient; i++) {
            ...
            promises.push(client.sendCommand("HSET", [`${key}-hash`, "field", "value"]));
          }
