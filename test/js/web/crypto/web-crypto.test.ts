...
import mlKemFixtures from "../../node/test/fixtures/crypto/ml-kem.js";
...
describe("ChaCha20-Poly1305 and AKP review fixes", () => {
  ...
  ])("%s: generateKey, export, import and encap/decap round-trip", async (name, pubLen, ctLen) => {
    expect(SubtleCrypto.supports("generateKey", { name })).toBe(true);

    const pair = (await crypto.subtle.generateKey({ name }, true, [
      "encapsulateBits",
      "decapsulateBits",
    ])) as CryptoKeyPair;
    expect(pair.publicKey.algorithm.name).toBe(name);

    const spki = await crypto.subtle.exportKey("spki", pair.publicKey);
    const pkcs8 = await crypto.subtle.exportKey("pkcs8", pair.privateKey);
    const raw = await crypto.subtle.exportKey("raw-public", pair.publicKey);
    expect(raw.byteLength).toBe(pubLen);

    const pub = await crypto.subtle.importKey("spki", spki, { name }, true, ["encapsulateBits"]);
    const priv = await crypto.subtle.importKey("pkcs8", pkcs8, { name }, true, ["decapsulateBits"]);

    const { ciphertext, sharedKey } = await crypto.subtle.encapsulateBits({ name }, pub);
    expect(ciphertext.byteLength).toBe(ctLen);
    expect(sharedKey.byteLength).toBe(32);

    const ss2 = await crypto.subtle.decapsulateBits({ name }, priv, ciphertext);
    expect(Buffer.from(ss2)).toEqual(Buffer.from(sharedKey));

    const jwk = await crypto.subtle.exportKey("jwk", pair.publicKey);
    expect({ kty: jwk.kty, alg: jwk.alg }).toEqual({ kty: "AKP", alg: name });
  });

  it("ML-KEM-512 decapsulates the NIST known-answer vector", async () => {
    // Importing node's seed-only fixture and decapsulating a fixed ciphertext
    // exercises the eta1=3 CBD sampler that only ML-KEM-512 uses; the expected
    // shared key is from node's fixtures/crypto/ml-kem.js, generated by
    // OpenSSL 3.5.
    const v = mlKemFixtures().find((x: any) => x.name === "ML-KEM-512")!;
    const priv = await crypto.subtle.importKey(
      "pkcs8",
      createPrivateKey(v.privateKeyPem).export({ type: "pkcs8", format: "der" }),
      { name: "ML-KEM-512" },
      true,
      ["decapsulateBits"],
    );
    const ss = await crypto.subtle.decapsulateBits({ name: "ML-KEM-512" }, priv, v.results.ciphertext);
    expect(Buffer.from(ss)).toEqual(v.results.sharedKey);

    // Seed-to-public-key expansion: deriving the public key from the seed must
    ...
    const fixturePub = createPublicKey(fs.readFileSync(path.join(keysDir, "ml_kem_512_public.pem")));
    const derived = createPublicKey(seedPriv).export({ type: "spki", format: "der" });
