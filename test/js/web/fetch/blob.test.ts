...
test("structuredClone of a named File round-trips its name without leaking or double-freeing", async () => {
  // `File.name` is stored on the blob's backing store as an owned, heap-allocated
  // buffer (Store.Bytes.stored_name). structuredClone serializes that buffer and
  // deserializes it into a fresh store whose path payload owns its own copy.
  // Both the source and clone free that buffer on teardown, so an ownership bug
  // (missing free -> leak, or double free) is only caught by repeating the
  // round-trip under GC pressure. This exercises the owned-buffer path directly;
  // under ASAN a double free is a hard crash.
  const script = `
