...
test("Blob constructor with multiple File/Blob parts", async () => {
  const file = Bun.file(path.join(import.meta.dir, "fixtures", "slice.txt"));
  ...
  const textFile = Bun.file(path.join(import.meta.dir, "fixtures", "slice.txt"));
  const blob8 = new Blob([textFile, "extra text"]);
  const text = await blob8.text();
  expect(text).toContain("extra text");
  expect(blob8.size).toBe(textFile.size + 10);
});
