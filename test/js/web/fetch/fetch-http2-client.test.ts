...
type RawConn = {
  ...
  /** Send a PING and resolve once the matching ACK arrives. Used as a
   *  barrier: by the time the client ACKs, it has parsed every frame we
...
// Serial: each test pushes 12 MiB through a debug-build subprocess over TLS
// and keeps that process alive until a PING round-trip completes; running
// them inside the `describe.concurrent` block above starves unrelated
// siblings past their 5s timeout on constrained CI hosts.
describe("fetch() over HTTP/2 — per-stream receive-window backpressure", () => {
  ...
  // is 16 MiB with an 8 MiB replenish threshold. Conn-level (id 0) credit is
  // intentionally receipt-based and is filtered out of the assertions.
  //
  ...
  // Push 12 MiB of DATA to `streamId` in 384 KiB batches, round-tripping
  // a PING after each so the client has fully parsed (and flushed any
  // WINDOW_UPDATE reply) before the next batch lands. Dumping the whole
  // body in one burst tickles a pre-existing uSockets-TLS quirk: the h2
  // client's `onData` calls `socket.write()` mid-callback, and
  // `us_internal_ssl_socket_write` zeroes `ssl_read_input_length`
  // (bun-usockets openssl.c:1815 — the comment there acknowledges it).
  // When SSL_read then hits WANT_WRITE with input still queued,
  // `ssl_on_data` closes the socket with code 0 (openssl.c:562). Pacing
  // keeps each `ssl_on_data` invocation below that threshold.
    ...
    await withRawH2Server(
      ...
      async (url, state) => {
        await using proc = spawnFetch(`
          ...
          await new Promise(() => {}); // hold reader open; test kills us
        `);
