...
describe("FormData with Bun.file() parts", () => {
  ...
  // /dev/zero has no EOF, so "read to EOF" applies to regular files only. The
  // part is read on the JS thread: a read with no bound never returns, so the
  // serialize runs in a child that this process can outlive.
  it.skipIf(isWindows)("serializes a bounded part for a device with no EOF", async () => {
    await using proc = Bun.spawn({
      cmd: [
        bunExe(),
        "-e",
        `const formData = new FormData();
         formData.append("f", Bun.file("/dev/zero"), "zero.bin");
         console.log((await new Response(formData).bytes()).length);`,
      ],
      env: bunEnv,
      stdout: "pipe",
      stderr: "pipe",
    ...
    expect(stderr).toBe("");
    const serialized = Number(stdout);
    expect(serialized).toBeGreaterThan(0);
    expect(serialized).toBeLessThanOrEqual(1024 * 1024);
    expect(exitCode).toBe(0);
  });
});
