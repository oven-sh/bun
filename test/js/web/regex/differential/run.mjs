...
//
//   node run.mjs --seed 1 --count 500 > oracle.jsonl
//   bun  run.mjs --seed 1 --count 500 --capabilities "$(head -1 oracle.jsonl)" > bun.jsonl
...
// stream in another engine (see capabilities.mjs). Use --case '<source>'
// --flags '<flags>' to run one explicit case, or --index N to reproduce the
// Nth generated case.
...
if (args.caseSource !== undefined) {
  ...
  if (!args.noHeader) out(canonicalJson({ capabilities, seed: args.seed, count: args.count }));
  ...
  for (let i = start; i < end; i++) {
    out(canonicalJson({ index: i, record: executeCase(cases[i]) }));
  }
}
