...
describe("WebSocket over Unix domain sockets", () => {
  ...
  test("should handle query string in ws+unix:// URL", async () => {
    ...
    try {
      const { promise, resolve, reject } = Promise.withResolvers<void>();
      const ws = new WebSocket(`ws+unix://${socketPath}:/api/endpoint?foo=bar&baz=qux`);
      ...
      expect(receivedUrl).toContain("?foo=bar&baz=qux");
      expect(receivedUrl).toContain("/api/endpoint");
  ...
  test("should handle path without leading slash in ws+unix:// URL", async () => {
    ...
    try {
      const { promise, resolve, reject } = Promise.withResolvers<void>();
      const ws = new WebSocket(`ws+unix://${socketPath}:api/test`);
