...
// pool and reads the worker's VirtualMachine allocation throughout run(): the
// Transpiler (defines table), test_isolation_enabled, source_mappings, and the
// job slot itself (the first 64 slots are an inline array in the VM box, so a
// queued job's WorkPool link is inside the allocation too). terminate() freed
// the VM mid-transpile. VM teardown now joins in-flight transpiler jobs first.
// Only ASAN reliably turns the stale reads into a crash.
// https://github.com/oven-sh/bun/issues/33936
test.skipIf(!isASAN)(
  "terminate() racing an in-flight dynamic import transpile does not UAF",
  async () => {
    // Big enough that the pool-thread transpile outlasts the terminated
    // worker's teardown by a wide margin under debug+ASAN.
    ...
    using dir = tempDir("worker-terminate-transpile", {
      ...
      "racer-worker.js": `
        ...
        postMessage("started");
      `,
      // Untouched worker transpiling the same module keeps the process alive
      // for the whole window in which the racer's orphaned pool-thread job
      // reads the freed VM on broken builds.
      "ref-worker.js": `
