  ...
  async () => {
    await using proc = Bun.spawn({
      cmd: [
        ...
        // With the fix: drop_concurrent_cpp_tasks frees the drain task in
        // shutdown(), the Ref is dropped, and the allocator reuses pages
        // (observed near-zero or slightly negative). ASAN's quarantine
        // retains freed allocations (default ~256 MB) across the 24 worker
        // VMs, so widen the threshold there — the unfixed leak is ~900 MB,
        // so this still catches the regression with >500 MB margin.
        if (deltaMB > ${isASAN ? 300 : 100}) {
