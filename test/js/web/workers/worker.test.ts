...
describe("web worker", () => {
  ...
  test("worker-env: new Worker(url) without options copies the parent's current process.env", async () => {
    using dir = tempDir("worker-env-default", {
      "worker.js": `
        postMessage({
  ...
  test("worker-env: a nested Worker(url) without options copies the env its parent worker was given", async () => {
    using dir = tempDir("worker-env-default-nested", {
