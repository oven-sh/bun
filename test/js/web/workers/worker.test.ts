...
describe("web worker", () => {
  ...
  describe("terminate() races and lifecycle edges", () => {
    ...
    // terminate() landing inside one long BigInt division: JSC polls for the
    // request from the division loop, at the top of a quotient row, and must
    // leave from there although the remainder is not reduced yet. The worker
    // repeats the division, so the request lands in one however late it is.
    // A digit is 64 bits.
    ...
    ])("terminate() during BigInt %s", async (_, operator, divisor) => {
      await using proc = Bun.spawn({
        cmd: [
          bunExe(),
          "-e",
