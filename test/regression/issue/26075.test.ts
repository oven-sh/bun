...
// We understand that Bun has built-in test timeouts (5 seconds, currently).
// However, with the debug build, on a fast laptop, this test takes 6.69 seconds (100ms on release build).
// To avoid flaky CI while replicating this issue, we extend the allowed time
// to account for slow bundling of workspace projects on debug builds.
// The performance may be a different issue completely, and was not investigated.
const EXTRA_TIME_FOR_DEBUG_BUILD_HMR = 60_000;

test(
  "server.reload() with HMR should handle workspace packages on multiple requests",
  async () => {
...
server.stop(true);`,
  ...
  EXTRA_TIME_FOR_DEBUG_BUILD_HMR,
);
