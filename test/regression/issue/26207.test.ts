...
// Each test spawns an independent subprocess in its own temp dir, so run concurrently.
test.concurrent("bun run --workspaces creates node symlink when NODE env points to non-existent path", async () => {
...
test.concurrent("bun run --filter creates node symlink when NODE env points to non-existent path", async () => {
...
test.concurrent.skipIf(isWindows)(
  "bun run --workspaces runs scripts that have #!/usr/bin/env node shebang",
  async () => {
