...
// Regression test for https://github.com/oven-sh/bun/issues/27391
// Segfault in wasm_trampoline_wasm_ipint_call_wide32 on Linux x86_64
// caused by a known JavaScriptCore IPInt bug (https://bugs.webkit.org/show_bug.cgi?id=289009)
test("WASM execution does not crash with segfault on Linux x86_64", async () => {
  // This test compiles and runs a WASM module that exercises function calls,
  // which would trigger the IPInt segfault/SIGILL bug on Linux x86_64.
  const code = `
