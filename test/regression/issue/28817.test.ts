...
const GET_ORDER = 'import dns from "node:dns"; console.log(dns.getDefaultResultOrder());';

describe.concurrent("NODE_OPTIONS", () => {
  test("--dns-result-order honored via env (=, space, double-quoted forms)", async () => {
    // --flag=value form
    const eq = await runWith("--dns-result-order=ipv4first", GET_ORDER);
    expect(eq.stdout).toBe("ipv4first");
    expect(eq.exitCode).toBe(0);

    // --flag value (space separated) form
    const sp = await runWith("--dns-result-order ipv6first", GET_ORDER);
    expect(sp.stdout).toBe("ipv6first");
    expect(sp.exitCode).toBe(0);

    // double-quoted value (quotes stripped, matching Node)
    const q = await runWith(`--dns-result-order="verbatim"`, GET_ORDER);
    expect(q.stdout).toBe("verbatim");
    expect(q.exitCode).toBe(0);
  });

  test("default order is verbatim without NODE_OPTIONS", async () => {
    const r = await runWith("", GET_ORDER);
  ...
  test("unknown flags and positional args are dropped", async () => {
    ...
    const r1 = await runWith("--unknown-flag --dns-result-order=ipv4first", GET_ORDER);
  ...
  test.skipIf(isWindows)("backslashes outside quotes are literal (Node semantics)", async () => {
    // Node only honors backslash escapes inside double quotes; a Windows-style
    // path outside quotes keeps its backslashes rather than having them eaten.
    const r = await runWith("--title C:\\Users\\foo", "console.log(process.title);");
    expect(r.stdout).toBe("C:\\Users\\foo");
    expect(r.exitCode).toBe(0);
  });

  test("bare required-value flag does not hijack the entrypoint", async () => {
    // A required-value flag (--require, --dns-result-order, ...) with no value
    // in NODE_OPTIONS must be DROPPED; otherwise clap would bind the user's
    // -e script (or entrypoint) as the flag's value and the script would
    // never run.
