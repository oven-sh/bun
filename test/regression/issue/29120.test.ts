...
// in-process `MachoSigner` writes. Previously, `writeSection` in `src/exe_format/macho.rs`
...
// IS the cross-compile template (isDefault() in src/options_types/compile_target.rs), so
...
//    macho.rs (the linkedit/datasize resize must not be gated on size_diff)
