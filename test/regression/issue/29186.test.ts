...
test.concurrent(
  "parent messages buffered before the worker comes online are discarded if self.close() runs synchronously",
  async () => {
...
test.concurrent(
  "self.postMessage then self.close() then top-level await delivers the message and exits cleanly",
  async () => {
