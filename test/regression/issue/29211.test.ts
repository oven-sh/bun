...
test.concurrent(
  "parentPort.removeEventListener unwraps listeners registered via .on() (#29211)",
  async () => {
    // Node's NodeEventTarget unifies `on`/`addListener`/`addEventListener` and
    // their remove counterparts in a single registry, so cross-API removal is
    // supported: a listener registered with `parentPort.on(event, fn)` can be
    // removed with `parentPort.removeEventListener(event, fn)`. `.on` wraps
    // `fn` into a callback (via `injectFakeEmitter`) and stores it on
    // `fn[wrappedListener]`; `removeEventListener` has to resolve that
    // indirection before looking the entry up, or the registration leaks and
    // the capture forwarder's event-loop ref is never released.
    using dir = tempDir("issue-29211-crossapi-remove", {
      "worker.mjs": String.raw`
        import { parentPort } from 'node:worker_threads';
        let fired = 0;
        function handler(msg) {
    ...
    // `handler` fires exactly once (on 'remove-me'); after
    // `removeEventListener`, the three follow-up messages must not reach it.
    // Pre-fix, `parentPortRemoveEventListener` didn't resolve
    // `listener[wrappedListener]`, so the lookup missed and `handler` stayed
    // subscribed — `fired` would reach 4.
    expect(JSON.parse(stdout.trim())).toEqual({ fired: 1 });
    expect(exitCode).toBe(0);
  },
  30000,
