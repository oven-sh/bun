...
test.concurrent("bun build --format=cjs still rejects a live top-level await", async () => {
...
test.concurrent("await can still be used as an identifier at module scope in CJS output", async () => {
...
test.concurrent("await inside a non-async function nested in a CJS file still reports a useful error", async () => {
...
test.concurrent("bun build --format=cjs drops a literal if (false) top-level await", async () => {
...
test.concurrent("await as a tagged-template call identifier keeps working in CJS", async () => {
...
// a dead branch.
test.concurrent("bun build --format=cjs drops an unreachable await of an object literal", async () => {
...
test.concurrent("bun build --format=cjs drops an unreachable top-level for await loop", async () => {
...
test.concurrent("bun build --format=cjs still rejects a live top-level for await loop", async () => {
...
test.concurrent("bun build --format=cjs rejects await in a default parameter of a non-async function", async () => {
...
test.concurrent("bun build --format=cjs underlines the await token of a live for-await loop", async () => {
...
// is just DCE fodder.
test.concurrent("bun build --format=cjs allows a top-level return alongside a dead top-level await", async () => {
...
test.concurrent("bun build --format=cjs leaves a TS `await!` identifier use alone", async () => {
...
test.concurrent("bun build --format=cjs keeps `for (await of ...)` parsing as for-of", async () => {
...
test.concurrent("bun build --format=cjs treats TS namespace bodies as not-at-module-scope", async () => {
