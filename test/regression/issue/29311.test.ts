...
test("issue #29311: minify prefers scientific notation when shorter", async () => {
  using dir = tempDir("issue-29311-minify", {
    "index.js": [
      ...
      "console.log(-1e300);",
      "console.log(1.5e20);",
      // Integer fast-path coverage: `20000` and `2000000000` are emitted
      // by the printer's integer fast path, which would otherwise print the
      // full decimal form regardless of trailing zeros.
      "console.log(20000);",
...
// Non-minified output should preserve the full decimal form so existing
// source maps / snapshots don't shift under users. This is a guard against
// over-eager normalisation that would change unrelated output.
