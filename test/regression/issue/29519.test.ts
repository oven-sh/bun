...
//
// Both --isolate and ShadowRealm construct a fresh Rust::GlobalObject on a
...
describe.skipIf(isWindows).concurrent("Rust::GlobalObject creation on a warm VM under concurrent GC", () => {
  test("bun test --isolate survives concurrent GC while swapping globals", async () => {
    const files: Record<string, string> = {};
    // Six files is enough to recycle the Rust::GlobalObject IsoSubspace slot
  ...
  // Rust::GlobalObject on a warm VM; cover it under the same GC pressure so the
