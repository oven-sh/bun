...
if (isDockerEnabled()) {
  ...
  // Docker-less sandboxes (including the fail-before/pass-after validator)
  // have a bare local Postgres but no docker-compose. Probe once at module
  // load; if nothing answers, the whole suite collapses to `describe.todo`
  // rather than emitting 15 connection-error failures.
  const candidates = [process.env.DATABASE_URL, "postgres://postgres@localhost:5432/postgres"].filter(
