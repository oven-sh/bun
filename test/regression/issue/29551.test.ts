...
// Define every assertion once; invoke from whichever describe block has a
// live Postgres. CI uses docker-compose (postgres_plain); local dev / sandbox
// runs use either an explicit `DATABASE_URL` or a conventional local role.
function defineTests(getSql: () => SQL) {
...
/// Resolve a Postgres URL from the environment. Used when Docker is
/// unavailable. Checks `DATABASE_URL` first, then the sandbox's default
/// `bun_test` role (`/opt/start-services.sh`) so the gate-check sandbox can
...
async function resolveLocalPostgresUrl(): Promise<string | null> {
  ...
  for (const url of candidates) {
    try {
      const probe = new SQL({ url, max: 1, connectionTimeout: 2, idleTimeout: 1 });
