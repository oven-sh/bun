...
describe("GitHub tarball integrity", () => {
  ...
  test("should verify integrity passes on re-install with matching hash", async () => {
    // First do a fresh install to get the correct hash
    await clearGitHubCache("jonschlinkert-is-number");
    ...
    const lockfileContent = await file(join(String(dir), "bun.lock")).text();
    const integrityMatch = lockfileContent.match(/"sha512-[A-Za-z0-9+/]+=*"/);
    expect(integrityMatch).not.toBeNull();
    const integrityHash = integrityMatch![0].slice(1, -1); // Remove quotes
  ...
  test("should accept GitHub dependency from cache without re-downloading", async () => {
    // This test verifies that when the package is already in the global cache,
    // installation succeeds without needing to re-download (regardless of integrity)
    ...
    // Don't clear the cache - rely on cache hit
    await using proc = Bun.spawn({
      cmd: [bunExe(), "install"],
      cwd: String(dir),
      env: bunEnv,
