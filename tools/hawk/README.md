...
(`src/runtime`) is a `staticlib`, the final Rust crate of the native build. An
`rlib` and a `[[bin]]` there would be more for every build to plan and compile,
so the root is applied only for the duration of a run:

```sh
git apply tools/hawk/analysis-root.patch      # adds rlib + a throwaway [[bin]] to src/runtime
